...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> TheClawChatHistoryPayload
{
    TheClawChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> TheClawChatSessionEntry {
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> TheClawChatSessionEntry
{
    TheClawChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> TheClawChatModelChoice {
    TheClawChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [TheClawChatHistoryPayload],
    sessionsResponses: [TheClawChatSessionsListResponse] = [],
    modelResponses: [[TheClawChatModelChoice]] = [],
    ...
    -> (TestChatTransport, TheClawChatViewModel)
{
...
private func sendUserMessage(_ vm: TheClawChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, TheClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [TheClawChatHistoryPayload]
    private let sessionsResponses: [TheClawChatSessionsListResponse]
    private let modelResponses: [[TheClawChatModelChoice]]
    ...
    private let stream: AsyncStream<TheClawChatTransportEvent>
    private let continuation: AsyncStream<TheClawChatTransportEvent>.Continuation
        ...
        setSessionThinkingHook: (@Sendable (String) async throws -> Void)? = nil)
    {
        ...
        var cont: AsyncStream<TheClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> TheClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? TheClawChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
    ...
    func sendMessage(
        ...
        attachments _: [TheClawChatAttachmentPayload]) async throws -> TheClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        await self.state.sentThinkingLevelsAppend(thinking)
        return TheClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> TheClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? TheClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: TheClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let sessions = TheClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesUseResolvedMainSessionKeyInsteadOfLiteralMain() async throws {
        ...
        let sessions = TheClawChatSessionsListResponse(
            ...
            path: nil,
            count: 2,
            defaults: TheClawChatSessionsDefaults(
                model: nil,
                contextTokens: nil,
    ...
    @Test func bootstrapsModelSelectionFromSessionAndDefaults() async throws {
        ...
        let sessions = TheClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: TheClawChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func selectingDefaultModelPatchesNilAndUpdatesSelection() async throws {
        ...
        await MainActor.run { vm.selectModel(TheClawChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == TheClawChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func selectingProviderQualifiedModelDisambiguatesDuplicateModelIDs() async throws {
        ...
        let sessions = TheClawChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: TheClawChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func lateModelCompletionDoesNotReplayCurrentSessionSelectionIntoPreviousSession() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = TheClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = TheClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func explicitThinkingLevelWinsOverHistoryAndPersistsChanges() async throws {
        let history = TheClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
...
"""]],
        ...
        let vm = await MainActor.run { TheClawChatViewModel(sessionKey: "main", transport: transport) }
