...
@MainActor
public struct TheClawChatView: View {
    public enum Style {
        case standard
        case onboarding
    }

    @State private var viewModel: TheClawChatViewModel
    ...
    public init(
        viewModel: TheClawChatViewModel,
    ...
    private var visibleMessages: [TheClawChatMessage] {
        let base: [TheClawChatMessage]
    ...
    private func mergeToolResults(in messages: [TheClawChatMessage]) -> [TheClawChatMessage] {
    ...
    private func isToolResultMessage(_ message: TheClawChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: TheClawChatMessage) -> Bool {
    ...
    private func primaryText(in message: TheClawChatMessage) -> String {
    ...
    private func hasInlineAttachments(in message: TheClawChatMessage) -> Bool {
    ...
    private func toolCalls(in message: TheClawChatMessage) -> [TheClawChatMessageContent] {
    ...
    private func inlineToolResults(in message: TheClawChatMessage) -> [TheClawChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: TheClawChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: TheClawChatMessage) -> String {
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(TheClawChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
