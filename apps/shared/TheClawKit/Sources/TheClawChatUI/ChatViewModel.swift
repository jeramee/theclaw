...
private let chatUILogger = Logger(subsystem: "ai.theclaw", category: "TheClawChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class TheClawChatViewModel {
    public static let defaultModelSelectionID = "__default__"

    public private(set) var messages: [TheClawChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [TheClawChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [TheClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [TheClawChatPendingToolCall] = []
    public private(set) var sessions: [TheClawChatSessionEntry] = []
    private let transport: any TheClawChatTransport
    private var sessionDefaults: TheClawChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: TheClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any TheClawChatTransport,
    ...
    public var sessionChoices: [TheClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [TheClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: TheClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [TheClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: TheClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: TheClawChatMessage) -> TheClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> TheClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return TheClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: TheClawChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: TheClawChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: TheClawChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private static func reconcileMessageIDs(
        previous: [TheClawChatMessage],
        incoming: [TheClawChatMessage]) -> [TheClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [TheClawChatMessage]) -> [TheClawChatMessage] {
        var result: [TheClawChatMessage] = []
    ...
    private static func dedupeKey(for message: TheClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [TheClawChatMessageContent] = [
            TheClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> TheClawChatAttachmentPayload in
            TheClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> TheClawChatSessionEntry {
        TheClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
        ...
        syncSelection: Bool)
    {
        if let index = self.sessions.firstIndex(where: { $0.key == sessionKey }) {
            ...
            self.sessions[index] = TheClawChatSessionEntry(
                key: current.key,
                kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: TheClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: TheClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: TheClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = TheClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> TheClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
