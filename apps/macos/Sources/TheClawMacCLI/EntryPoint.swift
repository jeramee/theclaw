...
struct TheClawMacCLI {
    static func main() async {
        ...
        default:
            fputs("theclaw-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      theclaw-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      theclaw-mac discover [--timeout <ms>] [--json] [--include-local]
      theclaw-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      theclaw-mac connect
      theclaw-mac connect --url ws://127.0.0.1:18789 --json
      theclaw-mac discover --timeout 3000 --json
      theclaw-mac wizard --mode local
    """)
}
