...
private let webChatSwiftLogger = Logger(subsystem: "ai.theclaw", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "theclaw.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: TheClawChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> TheClawChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(TheClawChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            TheClawChatSessionsDefaults(
                model: $0.model,
            ...
            contextTokens: nil,
            mainSessionKey: mainSessionKey)
        return TheClawChatSessionsListResponse(
            ts: decoded.ts,
            path: decoded.path,
            count: decoded.count,
            defaults: defaults,
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> TheClawChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                TheClawGatewayHealthOK.self,
    ...
    private static func mapModelChoice(_ model: TheClawProtocol.ModelChoice) -> TheClawChatModelChoice {
        TheClawChatModelChoice(
            modelID: model.id,
            name: model.name,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<TheClawChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any TheClawChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = TheClawChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: TheClawChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<TheClawChatView>) -> NSViewController
    {
