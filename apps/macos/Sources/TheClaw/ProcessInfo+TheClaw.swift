...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.theclaw.mac`) even if the shipped
    ...
    {
        if environment["THECLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "theclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "theclaw.nixMode") { return true }
