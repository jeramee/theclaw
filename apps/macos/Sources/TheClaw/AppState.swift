...
final class AppState {
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = TheClawConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            TheClawConfigFile.canonicalHostForComparison(host) ==
                TheClawConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = TheClawConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: TheClawConfigFile.loadDict(),
            draft: .init(
        ...
        TheClawConfigFile.saveDict(synced.root)
    }
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/theclaw"
        state.remoteCliPath = ""
        return state
    }
}
