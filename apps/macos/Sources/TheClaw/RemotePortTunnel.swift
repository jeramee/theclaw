...
final class RemotePortTunnel {
    private static let logger = Logger(subsystem: "ai.theclaw", category: "remote.tunnel")
    ...
    private static func resolveRemotePortOverride(for sshHost: String) -> Int? {
        ...
        guard let sshKey = TheClawConfigFile.canonicalHostForComparison(sshHost),
              let urlKey = TheClawConfigFile.canonicalHostForComparison(host)
        else {
            return nil
        }
    ...
    private static func findPort(preferred: UInt16?, allowRandom: Bool) async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { cont in
            let queue = DispatchQueue(label: "ai.theclaw.remote.tunnel.port", qos: .utility)
            do {
