...
enum GatewayEnvironment {
    private static let logger = Logger(subsystem: "ai.theclaw", category: "gateway.env")
    private static let supportedBindModes: Set<String> = ["loopback", "tailnet", "lan", "auto"]

    static func gatewayPort() -> Int {
        if let raw = ProcessInfo.processInfo.environment["THECLAW_GATEWAY_PORT"] {
            let trimmed = raw.trimmingCharacters(in: .whitespacesAndNewlines)
            if let parsed = Int(trimmed), parsed > 0 { return parsed }
        }
        if let configPort = TheClawConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.theclawExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "theclaw CLI not found in PATH; install the CLI.")
            }
    ...
    private static func preferredGatewayBind() -> String? {
        ...
        if let env = ProcessInfo.processInfo.environment["THECLAW_GATEWAY_BIND"] {
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "theclaw@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "theclaw@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "theclaw@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "theclaw@\(target)"])
            }

        statusHandler("Installing theclaw@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed theclaw@\(target)")
        } else {
    ...
    static func normalizeGatewayVersionOutput(_ raw: String?) -> String? {
        ...
        if normalized.lowercased().hasPrefix("theclaw ") {
            normalized = String(normalized.dropFirst("theclaw ".count))
        }
