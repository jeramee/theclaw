...
private struct SkillRow: View {
    ...
    private var sourceLabel: String {
        ...
        case "theclaw-bundled":
            "Bundled"
        case "theclaw-managed":
            "Managed"
        case "theclaw-workspace":
            "Workspace"
        case "theclaw-extra":
            "Extra"
        case "theclaw-plugin":
...
private struct EnvEditorView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Text("Saved to theclaw.json under skills.entries.\(self.editor.skillKey)")
                .font(.caption2)
            ...
            do {
                if isPrimary {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        apiKey: value)
                    self.statusMessage = "Saved API key — stored in theclaw.json (skills.entries.\(skillKey))"
                } else {
                    _ = try await GatewayConnection.shared.skillsUpdate(
                        skillKey: skillKey,
                        env: [envKey: value])
                    self.statusMessage = "Saved \(envKey) — stored in theclaw.json (skills.entries.\(skillKey).env)"
                }
            } catch {
                self.statusMessage = error.localizedDescription
            }
