...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.theclaw", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous TheClaw run already has an SSH listener on the expected port (common after restarts),
