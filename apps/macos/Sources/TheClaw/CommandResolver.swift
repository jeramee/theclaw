...
enum CommandResolver {
    private static let projectRootDefaultsKey = "theclaw.gatewayProjectRootPath"
    private static let helperName = "theclaw"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let theclawEntry = root.appendingPathComponent("theclaw.mjs").path
        if FileManager().isReadableFile(atPath: theclawEntry) { return theclawEntry }
        let binEntry = root.appendingPathComponent("bin/theclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/theclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let theclawPaths = self.theclawManagedPaths(home: home)
        if !theclawPaths.isEmpty {
            extras.insert(contentsOf: theclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + theclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func theclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".theclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func theclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectTheClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("theclaw.mjs").path,
            root.appendingPathComponent("bin/theclaw.js").path,
        ]
    ...
    static func hasAnyTheClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.theclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func theclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let theclawPath = self.projectTheClawExecutable(projectRoot: root) {
            return [theclawPath, subcommand] + extraArgs
        }
        if let theclawPath = self.theclawExecutable(searchPaths: searchPaths) {
            return [theclawPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "theclaw", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            theclaw entrypoint missing (looked for dist/index.js or theclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real theclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/theclaw"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/theclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/theclaw.mjs"
            node "$PRJ/theclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/theclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/theclaw.js"
            node "$PRJ/bin/theclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent theclaw"
          pnpm --silent theclaw \(quotedArgs);
        else
          echo "theclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? TheClawConfigFile.loadDict()
