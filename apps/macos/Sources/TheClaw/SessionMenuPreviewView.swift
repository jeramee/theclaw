...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.theclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> TheClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: TheClawSessionPreviewEntry,
    ...
    private static func cache(payload: TheClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: TheClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [TheClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [TheClawKit.AnyCodable]) -> [TheClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(TheClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: TheClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: TheClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: TheClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: TheClawChatMessage) -> String? {
