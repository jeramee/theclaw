...
enum DebugActions {
    private static let verboseDefaultsKey = "theclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "theclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = TheClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "TheClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = TheClawPaths.configURL
        guard
