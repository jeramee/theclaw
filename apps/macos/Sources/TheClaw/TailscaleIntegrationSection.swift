...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.theclaw/theclaw.json. Prefer THECLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.theclaw/theclaw.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.theclaw/theclaw.json. Restart the gateway to apply."
        }
