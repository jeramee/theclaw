...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: TheClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.theclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: TheClawKit.AnyCodable]()) {
                $0[$1.key] = TheClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: TheClawProtocol.AnyCodable?) -> [String: TheClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: TheClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: TheClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: TheClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("theclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("theclaw.control.agent")
}
