...
enum TheClawLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = TheClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = TheClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        TheClawLogging.bootstrapIfNeeded()
        let label = TheClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct TheClawOSLogHandler: AppLogLevelBackedHandler {
...
struct TheClawFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = TheClawLogging.parseLabel(self.label)
