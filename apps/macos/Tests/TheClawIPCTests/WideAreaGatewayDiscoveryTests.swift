...
import Foundation
import Testing
@testable import TheClawDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("THECLAW_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("THECLAW_WIDE_AREA_DOMAIN", "theclaw.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("THECLAW_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("THECLAW_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._theclaw-gw._tcp.theclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
                    return "0 0 18789 steipetacstudio.theclaw.internal."
                }
                if recordType == "TXT" {
                    return "\"displayName=Peter\\226\\128\\153s Mac Studio (TheClaw)\" \"gatewayPort=18789\" \"tailnetDns=peters-mac-studio-1.sheep-coho.ts.net\" \"cliPath=/Users/steipete/theclaw/src/entry.ts\""
                }
                return ""
            })
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (TheClaw)"
        ...
        #expect(beacon.cliPath == "/Users/steipete/theclaw/src/entry.ts")
    }
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._theclaw-gw._tcp.theclaw.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
