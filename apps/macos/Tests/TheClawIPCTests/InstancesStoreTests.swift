...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: TheClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: TheClawProtocol.AnyCodable] = [
            "presence": .init([TheClawProtocol.AnyCodable(entry)]),
        ]
        let payload = TheClawProtocol.AnyCodable(payloadMap)
