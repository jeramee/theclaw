...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["THECLAW_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "theclaw-gateway",
                executablePath: "/tmp/theclaw-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 theclaw-gateway @ /tmp/theclaw-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
