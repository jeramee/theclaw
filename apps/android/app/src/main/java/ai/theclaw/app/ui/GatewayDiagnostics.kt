...
internal fun buildGatewayDiagnosticsReport(
  ...
  return """
    Help diagnose this TheClaw Android gateway connection failure.
    ...
    - tell me whether `theclaw devices list` should show a pending pairing request
    - if more signal is needed, ask for `theclaw qr --json`, `theclaw devices list`, and `theclaw nodes status`
...
internal fun copyGatewayDiagnosticsReport(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("TheClaw gateway diagnostics", report))
