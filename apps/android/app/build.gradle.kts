...
val androidStoreFile = providers.gradleProperty("THECLAW_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("THECLAW_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("THECLAW_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("THECLAW_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set THECLAW_ANDROID_STORE_FILE, " +
      "THECLAW_ANDROID_STORE_PASSWORD, THECLAW_ANDROID_KEY_ALIAS, and " +
      "THECLAW_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/TheClawKit/Sources/TheClawKit/Resources")
    }
  }

  defaultConfig {
    applicationId = "ai.theclaw.app"
    minSdk = 31
    targetSdk = 36
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "theclaw-$versionName-$buildType.apk"
          } else {
            "theclaw-$versionName-$flavorName-$buildType.apk"
          }
