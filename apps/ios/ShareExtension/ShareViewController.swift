...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.theclaw.ios", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to TheClaw", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to TheClaw gateway…")
        ...
        do {
            ...
            self.showStatus("Sent to TheClaw.")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "TheClawShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "TheClaw is not connected to a gateway yet."])
        }
        ...
        let makeOptions: (String) -> GatewayConnectOptions = { clientId in
            GatewayConnectOptions(
                ...
                clientId: clientId,
                clientMode: "node",
                clientDisplayName: "TheClaw Share",
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("theclaw-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
