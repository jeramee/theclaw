...
@MainActor
enum TheClawAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class TheClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.theclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.theclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.theclaw.ios.bgrefresh"
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? TheClawAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "theclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "theclaw.watch.promptId"
    static let sessionKeyKey = "theclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "theclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "theclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "theclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "theclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "theclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "theclaw.watch.action.secondary"
    static let actionIdentifierPrefix = "theclaw.watch.action."
    static let actionIDKeyPrefix = "theclaw.watch.action.id."
    static let actionLabelKeyPrefix = "theclaw.watch.action.label."
    static let categoryPrefix = "theclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: TheClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> TheClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return TheClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "TheClaw" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [TheClawWatchAction]) -> [UNNotificationAction] {
...
@main
struct TheClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(TheClawAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        TheClawAppModelRegistry.appModel = appModel
...
extension TheClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("TheClaw: installing uncaught exception handler")
