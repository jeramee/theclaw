...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> TheClawDeviceStatusPayload {
        ...
        return TheClawDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    func info() -> TheClawDeviceInfoPayload {
        ...
        return TheClawDeviceInfoPayload(
    ...
    private func batteryStatus() -> TheClawBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: TheClawBatteryState = switch device.batteryState {
        ...
        return TheClawBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> TheClawThermalStatusPayload {
        let state: TheClawThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return TheClawThermalStatusPayload(state: state)
    }

    private func storageStatus() -> TheClawStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return TheClawStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
