...
struct IOSGatewayChatTransport: TheClawChatTransport {
    private static let logger = Logger(subsystem: "ai.theclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> TheClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(TheClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> TheClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(TheClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [TheClawChatAttachmentPayload]) async throws -> TheClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [TheClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(TheClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(TheClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TheClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: TheClawChatEventPayload.self)
                        {
