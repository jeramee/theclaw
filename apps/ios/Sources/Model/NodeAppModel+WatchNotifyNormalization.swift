...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: TheClawWatchNotifyParams) -> TheClawWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [TheClawWatchAction]?,
        kind: String?,
        promptId: String?) -> [TheClawWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> TheClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return TheClawWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                TheClawWatchAction(id: "approve", label: "Approve"),
                TheClawWatchAction(id: "decline", label: "Decline", style: "destructive"),
                TheClawWatchAction(id: "open_phone", label: "Open iPhone"),
                TheClawWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            TheClawWatchAction(id: "done", label: "Done"),
            TheClawWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            TheClawWatchAction(id: "open_phone", label: "Open iPhone"),
            TheClawWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: TheClawWatchRisk?,
        priority: TheClawNotificationPriority?) -> TheClawWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: TheClawNotificationPriority?,
        risk: TheClawWatchRisk?) -> TheClawNotificationPriority?
    {
