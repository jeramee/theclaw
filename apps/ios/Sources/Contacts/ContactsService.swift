...
final class ContactsService: ContactsServicing {
    ...
    func search(params: TheClawContactsSearchParams) async throws -> TheClawContactsSearchPayload {
        ...
        return TheClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: TheClawContactsAddParams) async throws -> TheClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return TheClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return TheClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> TheClawContactPayload {
        TheClawContactPayload(
