...
final class RemindersService: RemindersServicing {
    func list(params: TheClawRemindersListParams) async throws -> TheClawRemindersListPayload {
        ...
        let payload: [TheClawReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return TheClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: TheClawRemindersAddParams) async throws -> TheClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = TheClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return TheClawRemindersAddPayload(reminder: payload)
    }
