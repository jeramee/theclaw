...
final class MotionService: MotionServicing {
    func activities(params: TheClawMotionActivityParams) async throws -> TheClawMotionActivityPayload {
        ...
        let mapped: [TheClawMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return TheClawMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = TheClawPedometerPayload(
