...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__THECLAW_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<TheClawTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.theclaw", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = TheClawTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> TheClawTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return TheClawTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> TheClawTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> TheClawTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return TheClawTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> TheClawTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = TheClawTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: TheClawTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(
                payload,
                as: TheClawAgentEventPayload.self)
            else {
                continue
            }
