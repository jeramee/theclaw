...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": TheClawLocationMode.always.rawValue,
            ...
            #expect(caps.contains(TheClawCapability.canvas.rawValue))
            #expect(caps.contains(TheClawCapability.screen.rawValue))
            #expect(caps.contains(TheClawCapability.camera.rawValue))
            #expect(caps.contains(TheClawCapability.location.rawValue))
            #expect(caps.contains(TheClawCapability.voiceWake.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": TheClawLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(TheClawLocationCommand.get.rawValue))
        }
    }
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(!commands.contains(TheClawSystemCommand.run.rawValue))
            #expect(!commands.contains(TheClawSystemCommand.which.rawValue))
            #expect(!commands.contains(TheClawSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(TheClawSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.theclaw.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(prior, service: "ai.theclaw.gateway", account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.theclaw.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.theclaw.gateway", account: "lastConnection")
