...
const SYNTHETIC_TRANSCRIPT_REPAIR_RESULT =
  "[theclaw] missing tool result in session history; inserted synthetic error result for transcript repair.";
...
function hasTranscriptMeta(message: unknown): boolean {
  return Boolean(
    ...
    (message as { __theclaw?: unknown }).__theclaw &&
    typeof (message as { __theclaw?: unknown }).__theclaw === "object",
  );
}
...
export function handleChatEvent(state: ChatState, payload?: ChatEventPayload) {
  ...
  // See https://github.com/theclaw/theclaw/issues/1909
