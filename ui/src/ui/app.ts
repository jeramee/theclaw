...
declare global {
  interface Window {
    __THECLAW_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
@customElement("theclaw-app")
export class TheClawApp extends LitElement {
  ...
  async toggleRealtimeTalk() {
    ...
    const session = new RealtimeTalkSession(this.client, this.sessionKey, {
      ...
      onTranscript: (entry) => {
        this.realtimeTalkTranscript = `${entry.role === "user" ? "You" : "TheClaw"}: ${entry.text}`;
      },
    });
