...
import { readJsonFileWithFallback, writeJsonFileAtomically } from "theclaw/plugin-sdk/json-store";
...
function resolveStateDirFromEnv(env: NodeJS.ProcessEnv = process.env): string {
  // Explicit THECLAW_STATE_DIR overrides take precedence (including
  // per-test mkdtemp dirs in this module's test suite).
  if (env.THECLAW_STATE_DIR?.trim()) {
    return resolveStateDir(env);
  }
  // Default test isolation: per-pid tmpdir, no bleed into real ~/.theclaw.
  // Use resolvePreferredTheClawTmpDir + string concat (mirrors
  ...
  if (env.VITEST || env.NODE_ENV === "test") {
    const name = "theclaw-vitest-" + process.pid;
    return path.join(resolvePreferredTheClawTmpDir(), name);
  }
  // Canonical TheClaw state dir: honors `~` expansion + legacy/new
