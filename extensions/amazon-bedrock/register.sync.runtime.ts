...
import { resolvePluginConfigObject } from "theclaw/plugin-sdk/plugin-config-runtime";
...
} from "theclaw/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When TheClaw's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if TheClaw identifies this as an Anthropic model via the ARN heuristic.
 ...
 * the TheClaw config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: TheClawPluginApi): void {
  ...
  function resolveCurrentPluginConfig(
    config: TheClawConfig | undefined,
