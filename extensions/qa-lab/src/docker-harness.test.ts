...
describe("qa docker harness", () => {
  it("writes compose, env, config, and workspace scaffold files", async () => {
    ...
    expect(result.files).toEqual(
      expect.arrayContaining([
        ...
        path.join(outputDir, "state", "theclaw.json"),
    ...
    expect(compose).toContain('      - "http://theclaw-qa-gateway:18789/"');
    expect(compose).toContain("      - --send-kickoff-on-start");
    expect(compose).toContain("      - --ui-dist-dir");
    expect(compose).toContain('      - "/opt/theclaw-qa-lab-ui"');
    expect(compose).toContain(":/opt/theclaw-repo:ro");
    expect(compose).toContain("./state:/opt/theclaw-scaffold:ro");
    expect(compose).toContain(
      "cp -R /opt/theclaw-scaffold/seed-workspace/. /tmp/theclaw/workspace/",
    );
    expect(compose).toContain("THECLAW_CONFIG_PATH: /tmp/theclaw/theclaw.json");
    expect(compose).toContain("THECLAW_STATE_DIR: /tmp/theclaw/state");
    expect(compose).toContain('THECLAW_NO_RESPAWN: "1"');

    const envExample = await readFile(path.join(outputDir, ".env.example"), "utf8");
    expect(envExample).toContain("THECLAW_GATEWAY_TOKEN=qa-token");
    ...
    const config = await readFile(path.join(outputDir, "state", "theclaw.json"), "utf8");
    ...
    expect(config).toContain('"/tmp/theclaw/workspace"');
  ...
  it("builds the reusable QA image with bundled QA extensions", async () => {
    ...
    const result = await buildQaDockerHarnessImage(
      {
        repoRoot: "/repo/theclaw",
        imageName: "theclaw:qa-local-prebaked",
      },
      {
    ...
    expect(result.imageName).toBe("theclaw:qa-local-prebaked");
    expect(calls).toEqual([
      expect.stringContaining(
        "docker build -t theclaw:qa-local-prebaked --build-arg THECLAW_EXTENSIONS=qa-channel qa-lab -f Dockerfile . @/repo/theclaw",
      ),
    ]);
  });
});
