...
const QA_LAB_UI_OVERLAY_DIR = "/opt/theclaw-qa-lab-ui";
...
function renderImageBlock(params: {
  ...
  return `    build:\n      context: ${context}\n      dockerfile: Dockerfile\n      args:\n        THECLAW_EXTENSIONS: "qa-channel qa-lab"\n`;
}
...
${imageBlock}    pull_policy: never
    ...
    environment:
      THECLAW_CONFIG_PATH: /tmp/theclaw/theclaw.json
      ...
      THECLAW_PROFILE: ""
    volumes:
      - ./state:/opt/theclaw-scaffold:ro
      - ${repoMount}:/opt/theclaw-repo:ro
    healthcheck:
      test:
        - CMD
        - node
...
`
    ...
    command:
      - sh
      - -lc
      - mkdir -p /tmp/theclaw/workspace /tmp/theclaw/state && cp /opt/theclaw-scaffold/theclaw.json /tmp/theclaw/theclaw.json && cp -R /opt/theclaw-scaffold/seed-workspace/. /tmp/theclaw/workspace/ && ln -snf /opt/theclaw-repo /tmp/theclaw/workspace/repo && exec node dist/index.js gateway run --port 18789 --bind lan --allow-unconfigured
`;
}
...
1. Build the prebaked image once:
   - \`docker build -t theclaw:qa-local-prebaked --build-arg THECLAW_EXTENSIONS="qa-channel qa-lab" -f Dockerfile .\`
2. Start the stack:
...
export async function writeQaDockerHarnessFiles(params: {
  ...
  const imageName = params.imageName ?? "theclaw:qa-local-prebaked";
  ...
  const files = [
    ...
    path.join(params.outputDir, "state", "theclaw.json"),
  ];
