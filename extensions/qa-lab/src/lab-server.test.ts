...
vi.mock("@theclaw/qa-channel/api.js", async () => await import("../../qa-channel/api.js"));
...
vi.mock("theclaw/plugin-sdk/proxy-capture", () => ({
  ...
  resolveDebugProxySettings: () => ({
    dbPath: process.env.THECLAW_DEBUG_PROXY_DB_PATH ?? "",
    blobDir: process.env.THECLAW_DEBUG_PROXY_BLOB_DIR ?? "",
    proxyUrl: process.env.THECLAW_DEBUG_PROXY_URL ?? "",
...
describe("qa-lab server", () => {
  ...
  it("aborts an in-flight runner model catalog when the lab stops", async () => {
    ...
    await writeFile(
      ...
      [
        ...
        "  process.exit(0);",
        "});",
        `fs.writeFileSync(${JSON.stringify(markerPath)}, process.env.THECLAW_CODEX_DISCOVERY_LIVE || "", "utf8");`,
        "setInterval(() => {}, 1000);",
      ].join("\n"),
      "utf8",
    );
  ...
  it("serves proxy capture sessions, events, and query rows", async () => {
    ...
    process.env.THECLAW_DEBUG_PROXY_DB_PATH = path.join(tempDir, "capture.sqlite");
    process.env.THECLAW_DEBUG_PROXY_BLOB_DIR = path.join(tempDir, "blobs");
    ...
    cleanups.push(async () => {
      delete process.env.THECLAW_DEBUG_PROXY_DB_PATH;
      delete process.env.THECLAW_DEBUG_PROXY_BLOB_DIR;
      await lab.stop();
    });
