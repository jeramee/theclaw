...
import { formatErrorMessage } from "theclaw/plugin-sdk/error-runtime";
import { fetchWithSsrFGuard } from "theclaw/plugin-sdk/ssrf-runtime";
...
const TELEGRAM_QA_CAPTURE_CONTENT_ENV = "THECLAW_QA_TELEGRAM_CAPTURE_CONTENT";
const QA_REDACT_PUBLIC_METADATA_ENV = "THECLAW_QA_REDACT_PUBLIC_METADATA";
const QA_SUITE_PROGRESS_ENV = "THECLAW_QA_SUITE_PROGRESS";
...
export function resolveTelegramQaRuntimeEnv(
  ...
  const groupId = resolveEnvValue(env, "THECLAW_QA_TELEGRAM_GROUP_ID");
  if (!/^-?\d+$/u.test(groupId)) {
    throw new Error("THECLAW_QA_TELEGRAM_GROUP_ID must be a numeric Telegram chat id.");
  }
  return {
    groupId,
    driverToken: resolveEnvValue(env, "THECLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN"),
    sutToken: resolveEnvValue(env, "THECLAW_QA_TELEGRAM_SUT_BOT_TOKEN"),
  };
}
...
async function runInstalledTheClawTelegramOnboardingPreflight(params: {
  openClawCommand: string;
  providerMode: ReturnType<typeof normalizeQaProviderMode>;
  sutToken: string;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-npm-telegram-"),
  );
  const homeDir = path.join(tempRoot, "home");
  const stateDir = path.join(homeDir, ".theclaw");
  ...
  const env = {
    ...process.env,
    HOME: homeDir,
    THECLAW_HOME: stateDir,
    THECLAW_CONFIG_PATH: path.join(stateDir, "theclaw.json"),
    THECLAW_STATE_DIR: stateDir,
    THECLAW_GATEWAY_TOKEN: "npm-telegram-live-onboard",
    ...(params.providerMode === "live-frontier"
      ? {}
      : { OPENAI_API_KEY: process.env.OPENAI_API_KEY ?? "sk-theclaw-npm-telegram-preflight" }),
  };
  try {
    await execFileAsync(
...
export async function runTelegramQaLive(params: {
  ...
  try {
    if (params.sutTheClawCommand && params.preflightInstalledOnboarding === true) {
      writeTelegramQaProgress(progressEnabled, "installed package onboarding preflight start");
      await runInstalledTheClawTelegramOnboardingPreflight({
        openClawCommand: params.sutTheClawCommand,
        providerMode,
    ...
    const gatewayHarness = await startQaLiveLaneGateway({
      repoRoot,
      command: params.sutTheClawCommand
        ? {
            executablePath: params.sutTheClawCommand,
  ...
  const publishedCleanupIssues = redactPublicMetadata
    ? cleanupIssues.map(() => "details redacted (THECLAW_QA_REDACT_PUBLIC_METADATA=1)")
    : cleanupIssues;
