...
describe("qa suite runtime agent process helpers", () => {
    ...
    const pending = runQaCli(
      {
        ...
        gateway: {
          tempRoot: "/tmp/runtime",
          runtimeEnv: { PATH: "/usr/bin", THECLAW_STATE_DIR: "/tmp/default-state" },
        },
        primaryModel: "openai/gpt-5.5",
        alternateModel: "openai/gpt-5.5-mini",
        providerMode: "mock-openai",
      ...
      {
        env: {
          THECLAW_STATE_DIR: "/tmp/isolated-state",
          THECLAW_CONFIG_PATH: "/tmp/isolated-state/theclaw.json",
        },
      },
    );
  ...
  it("finds managed dreaming cron jobs across legacy and current payload contracts", () => {
    ...
    const current = {
      ...
      payload: {
        kind: "agentTurn",
        message: "__theclaw_memory_core_short_term_promotion_dream__",
        lightContext: true,
      },
