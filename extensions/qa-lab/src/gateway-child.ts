...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    THECLAW_HOME: params.homeDir,
    THECLAW_CONFIG_PATH: params.configPath,
    THECLAW_STATE_DIR: params.stateDir,
    THECLAW_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    THECLAW_TEST_FAST: "1",
    THECLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    THECLAW_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { THECLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { THECLAW_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: TheClawConfig) => TheClawConfig;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-qa-suite-"),
  );
  ...
  const keepTemp = process.env.THECLAW_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
  let child: ReturnType<typeof spawn> | null = null;
  let cfg!: TheClawConfig;
