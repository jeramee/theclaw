...
export async function resolveQaOwnerPluginIdsForProviderIds(params: {
  ...
  for (const sourceRoot of resolveQaBundledPluginScanRoots(params.repoRoot)) {
    for (const entry of await fs.readdir(sourceRoot, { withFileTypes: true })) {
      ...
      const manifestPath = path.join(sourceRoot, entry.name, "theclaw.plugin.json");
...
function shouldStageQaBundledPluginPath(sourcePath: string) {
  ...
  return (
    ...
    !basename.startsWith(".theclaw-runtime-deps-copy-")
  );
}
...
export async function resolveQaRuntimeHostVersion(params: {
  ...
  for (const pluginId of stagedPluginIds) {
    ...
    const candidate = parseStableSemverFloor(packageJson.theclaw?.install?.minHostVersion);
...
export async function createQaBundledPluginsDir(params: {
  ...
  const stagedTheClawPackageDir = path.join(stagedRoot, "node_modules", "theclaw");
  await fs.mkdir(stagedTheClawPackageDir, { recursive: true });
  await fs.copyFile(
    path.join(params.repoRoot, "package.json"),
    path.join(stagedTheClawPackageDir, "package.json"),
  );
  ...
  await symlinkQaStagedDirEntry({
    ...
    targetPath: path.join(stagedTheClawPackageDir, "dist"),
    directory: true,
  });
  return {
    bundledPluginsDir,
