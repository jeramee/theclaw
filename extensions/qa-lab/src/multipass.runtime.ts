...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/theclaw-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/theclaw";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/theclaw-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `theclaw-qa-${createVmSuffix()}`;
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          key !== "CODEX_HOME" &&
          key !== "THECLAW_CONFIG_PATH" &&
          key !== "THECLAW_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `THECLAW_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `THECLAW_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm theclaw qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), `${plan.vmName}-qa-suite-`),
  );
