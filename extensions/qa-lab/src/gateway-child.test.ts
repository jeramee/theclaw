...
vi.mock("theclaw/plugin-sdk/ssrf-runtime", () => ({
  fetchWithSsrFGuard: fetchWithSsrFGuardMock,
}));

vi.mock("theclaw/plugin-sdk/temp-path", () => ({
  resolvePreferredTheClawTmpDir: () => qaTempPathState.preferredTmpDir,
}));
...
function createParams(baseEnv?: NodeJS.ProcessEnv) {
  return {
    configPath: "/tmp/theclaw-qa/theclaw.json",
    gatewayToken: "qa-token",
    homeDir: "/tmp/theclaw-qa/home",
    stateDir: "/tmp/theclaw-qa/state",
    xdgConfigHome: "/tmp/theclaw-qa/xdg-config",
    xdgDataHome: "/tmp/theclaw-qa/xdg-data",
    xdgCacheHome: "/tmp/theclaw-qa/xdg-cache",
    bundledPluginsDir: "/tmp/theclaw-qa/bundled-plugins",
...
describe("buildQaRuntimeEnv", () => {
  ...
  it("keeps the slow-reply QA opt-out enabled under fast mode", () => {
    ...
    expect(env.THECLAW_TEST_FAST).toBe("1");
    expect(env.THECLAW_QA_ALLOW_LOCAL_IMAGE_PROVIDER).toBe("1");
    expect(env.THECLAW_ALLOW_SLOW_REPLY_TESTS).toBe("1");
    expect(env.THECLAW_BUNDLED_PLUGINS_DIR).toBe("/tmp/theclaw-qa/bundled-plugins");
    expect(env.THECLAW_COMPATIBILITY_HOST_VERSION).toBe("2026.4.8");
  });

  it("maps live frontier key aliases into provider env vars", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        THECLAW_LIVE_OPENAI_KEY: "openai-live",
        THECLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        THECLAW_LIVE_GEMINI_KEY: "gemini-live",
      }),
  ...
  it("preserves Codex CLI auth home for live frontier runs while sandboxing TheClaw home", async () => {
    ...
    expect(env.HOME).toBe("/tmp/theclaw-qa/home");
    expect(env.THECLAW_HOME).toBe("/tmp/theclaw-qa/home");
    expect(env.CODEX_HOME).toBe(codexHome);
  });

  it("forwards host HOME for live Claude CLI runs while keeping TheClaw home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
    ...
    expect(env.THECLAW_HOME).toBe("/tmp/theclaw-qa/home");
    expect(env.THECLAW_STATE_DIR).toBe("/tmp/theclaw-qa/state");
  });

  it("can forward host HOME for browser-backed QA runs while keeping TheClaw home sandboxed", async () => {
    const hostHome = await mkdtemp(path.join(os.tmpdir(), "qa-host-home-"));
    cleanups.push(async () => {
      await rm(hostHome, { recursive: true, force: true });
    });
  ...
  it("preserves the live Anthropic key for live Claude CLI runs without writing it into config", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        HOME: hostHome,
        THECLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
        THECLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.THECLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP","ANTHROPIC_API_KEY"]');
    expect(env.THECLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("api-key");
  });
  ...
  it("removes preserved Anthropic keys for live Claude CLI subscription runs", async () => {
    ...
    const env = buildQaRuntimeEnv({
      ...createParams({
        ...
        THECLAW_LIVE_CLI_BACKEND_PRESERVE_ENV: '["SAFE_KEEP","ANTHROPIC_API_KEY"]',
      }),
      providerMode: "live-frontier",
      forwardHostHomeForClaudeCli: true,
    ...
    expect(env.ANTHROPIC_API_KEY).toBe("anthropic-live");
    expect(env.THECLAW_LIVE_CLI_BACKEND_PRESERVE_ENV).toBe('["SAFE_KEEP"]');
    expect(env.THECLAW_LIVE_CLI_BACKEND_AUTH_MODE).toBe("subscription");
  });

  it("does not pass QA setup-token values to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        THECLAW_LIVE_SETUP_TOKEN_VALUE: `sk-ant-oat01-${"a".repeat(80)}`,
        THECLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN: `sk-ant-oat01-${"b".repeat(80)}`,
      }),
      providerMode: "live-frontier",
    });

    expect(env.THECLAW_LIVE_SETUP_TOKEN_VALUE).toBeUndefined();
    expect(env.THECLAW_QA_LIVE_ANTHROPIC_SETUP_TOKEN).toBeUndefined();
  });

  it("does not pass Convex credential broker secrets to the gateway child env", () => {
    const env = buildQaRuntimeEnv({
      ...createParams({
        THECLAW_QA_CONVEX_SECRET_CI: "convex-ci-secret",
        THECLAW_QA_CONVEX_SECRET_MAINTAINER: "convex-maintainer-secret",
      }),
      providerMode: "live-frontier",
    });

    expect(env.THECLAW_QA_CONVEX_SECRET_CI).toBeUndefined();
    expect(env.THECLAW_QA_CONVEX_SECRET_MAINTAINER).toBeUndefined();
  });
    ...
    (providerMode) => {
      const env = buildQaRuntimeEnv({
        ...createParams({
          ...
          THECLAW_LIVE_ANTHROPIC_KEY: "anthropic-live",
          THECLAW_LIVE_ANTHROPIC_KEYS: "anthropic-a,anthropic-b",
          THECLAW_LIVE_GEMINI_KEY: "gemini-live",
          THECLAW_LIVE_OPENAI_KEY: "openai-live",
        }),
        providerMode,
      ...
      expect(env.THECLAW_LIVE_OPENAI_KEY).toBeUndefined();
      expect(env.THECLAW_LIVE_ANTHROPIC_KEY).toBeUndefined();
      expect(env.THECLAW_LIVE_ANTHROPIC_KEYS).toBeUndefined();
      expect(env.THECLAW_LIVE_GEMINI_KEY).toBeUndefined();
    },
  );
  ...
  it("stages a live Anthropic setup-token profile for isolated QA workers", async () => {
    ...
    const cfg = await __testing.stageQaLiveAnthropicSetupToken({
      ...
      stateDir,
      env: {
        THECLAW_LIVE_SETUP_TOKEN_VALUE: token,
      },
    });
  ...
  it("preserves only sanitized gateway debug artifacts", async () => {
    ...
    await writeFile(
      stdoutLogPath,
      [
        "THECLAW_GATEWAY_TOKEN=qa-suite-token",
        'OPENAI_API_KEY="openai-live"',
        "THECLAW_QA_CONVEX_SECRET_CI=convex-ci-secret",
        "THECLAW_QA_CONVEX_SECRET_MAINTAINER=convex-maintainer-secret",
    ...
    await expect(readFile(path.join(artifactDir, "gateway.stdout.log"), "utf8")).resolves.toBe(
      [
        "THECLAW_GATEWAY_TOKEN=<redacted>",
        "OPENAI_API_KEY=<redacted>",
        "THECLAW_QA_CONVEX_SECRET_CI=<redacted>",
        "THECLAW_QA_CONVEX_SECRET_MAINTAINER=<redacted>",
        "botToken=<redacted>",
  ...
  it("rejects preserved gateway artifacts outside the repo root", async () => {
    await expect(
      __testing.assertQaArtifactDirWithinRepo("/tmp/theclaw-repo", "/tmp/outside"),
  ...
  it("cleans startup temp roots when they are not preserved", async () => {
    ...
    await writeFile(path.join(tempRoot, "theclaw.json"), "{}", "utf8");
...
describe("qa bundled plugin dir", () => {
  ...
  it("uses a source bundled plugin when the built copy is missing CLI metadata", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "memory-core", "theclaw.plugin.json"),
    ...
    await writeFile(
      path.join(repoRoot, "extensions", "memory-core", "theclaw.plugin.json"),
      JSON.stringify({ id: "memory-core", kind: "memory" }),
      "utf8",
    );
    await writeFile(
  ...
  it("creates a scoped bundled plugin tree for allowed plugins plus always-allowed runtime facades", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "package.json"),
      JSON.stringify({ name: "@theclaw/qa-channel", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "qa-channel", "index.js"),
      [
        'import { normalizeAccountId } from "theclaw/plugin-sdk/account-id";',
  ...
  it("skips transient runtime dependency artifacts while staging built bundled plugins", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "package.json"),
      JSON.stringify({ name: "theclaw", type: "module" }, null, 2),
      "utf8",
    );
    const pluginDir = path.join(repoRoot, "dist", "extensions", "qa-channel");
    await mkdir(path.join(pluginDir, ".theclaw-runtime-deps-copy-active", "node_modules"), {
      recursive: true,
    });
    await writeFile(
    ...
    await writeFile(path.join(pluginDir, ".theclaw-runtime-deps.json"), "{}\n", "utf8");
    await writeFile(path.join(pluginDir, ".theclaw-runtime-deps-stamp.json"), "{}\n", "utf8");
    await writeFile(
      path.join(pluginDir, ".theclaw-runtime-deps-copy-active", "node_modules", "transient.js"),
      "export {};\n",
      "utf8",
    );
    ...
    await expect(lstat(path.join(stagedPluginDir, ".theclaw-runtime-deps.json"))).rejects.toThrow(
      /ENOENT/u,
    );
    await expect(
      lstat(path.join(stagedPluginDir, ".theclaw-runtime-deps-stamp.json")),
    ).rejects.toThrow(/ENOENT/u);
    await expect(
      lstat(path.join(stagedPluginDir, ".theclaw-runtime-deps-copy-active")),
    ).rejects.toThrow(/ENOENT/u);
  });
  ...
  it("preserves dist-runtime-only root chunks when dist also exists", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ name: "@theclaw/runtime-only", type: "module" }, null, 2),
      "utf8",
    );
    await writeFile(
  ...
  it("maps cli backend provider ids to their owning bundled plugin ids", async () => {
    ...
    await writeFile(
      path.join(repoRoot, "dist", "extensions", "openai", "theclaw.plugin.json"),
      JSON.stringify({
        id: "openai",
  ...
  it("copies selected live provider configs from the host config", async () => {
    ...
    await expect(
      __testing.readQaLiveProviderConfigOverrides({
        providerIds: ["custom-openai"],
        env: { THECLAW_QA_LIVE_PROVIDER_CONFIG_PATH: configPath },
      }),
    ).resolves.toEqual({
  ...
  it("raises the QA runtime host version to the highest allowed plugin floor", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ theclaw: { install: { minHostVersion: ">=2026.4.8" } } }),
      "utf8",
    );
    ...
    await writeFile(
      ...
      JSON.stringify({ theclaw: { install: { minHostVersion: ">=2026.4.7" } } }),
      "utf8",
    );
  ...
  it("includes always-allowed runtime facade plugins when raising the QA runtime host version", async () => {
    ...
    await writeFile(
      ...
      JSON.stringify({ theclaw: { install: { minHostVersion: ">=2026.4.9" } } }),
      "utf8",
    );
