...
import { loginOpenAICodexOAuth } from "theclaw/plugin-sdk/provider-auth-login";
import {
...
import { fetchCodexUsage } from "theclaw/plugin-sdk/provider-usage";
import { normalizeLowercaseStringOrEmpty, readStringValue } from "theclaw/plugin-sdk/text-runtime";
import {
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.theclaw.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `theclaw models auth login --provider openai-codex` or `theclaw configure`.";
}
