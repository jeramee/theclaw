...
import type { ResolvedTtsConfig } from "theclaw/plugin-sdk/agent-runtime";
import type { TheClawConfig } from "theclaw/plugin-sdk/config-types";
import { encodePngRgba, fillPixel } from "theclaw/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "theclaw/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "theclaw/plugin-sdk/provider-test-contracts";
import { getRuntimeConfig } from "theclaw/plugin-sdk/runtime-config-snapshot";
import { describe, expect, it } from "vitest";
import plugin from "./index.js";

const OPENAI_API_KEY = process.env.OPENAI_API_KEY ?? "";
const LIVE_MODEL_ID = process.env.THECLAW_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.THECLAW_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.THECLAW_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-4.1-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.THECLAW_LIVE_TEST === "1";
...
function createLiveConfig(): TheClawConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "TheClaw integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing TheClaw OpenAI realtime transcription integration test OK.";
