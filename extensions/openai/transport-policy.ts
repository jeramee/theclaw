...
function resolveSessionHeaders(params: {
  ...
  return {
    ...
    "x-theclaw-session-id": sessionId,
  };
}
...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-theclaw-turn-id": turnId,
      "x-theclaw-turn-attempt": attempt,
    },
    metadata: {
      theclaw_session_id: sessionHeaders["x-theclaw-session-id"] ?? "",
      theclaw_turn_id: turnId,
      theclaw_turn_attempt: attempt,
      theclaw_transport: ctx.transport,
    },
  };
}
