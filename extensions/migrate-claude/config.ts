    ...
    if (settingsPath && settings.hooks !== undefined) {
      items.push(
        createManualItem({
          ...
          recommendation: "Review hook commands before recreating equivalent TheClaw automation.",
        }),
      );
    }
    ...
    if (settingsPath && settings.env !== undefined) {
      items.push(
        createManualItem({
          ...
          recommendation:
            "Move non-secret values manually and store credentials through TheClaw credential flows.",
        }),
      );
    }
  }
