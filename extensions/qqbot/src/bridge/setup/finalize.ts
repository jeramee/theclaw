...
import { DEFAULT_ACCOUNT_ID } from "theclaw/plugin-sdk/setup";
...
function isQQBotAccountConfigured(cfg: TheClawConfig, accountId: string): boolean {
...
export async function finalizeQQBotSetup(params: {
  ...
  if (mode === "qr") {
    ...
    await params.prompter.note(
      ["您可以稍后运行以下命令重新选择 QQ Bot 进行配置：", "  theclaw channels add"].join("\n"),
      "QQ Bot",
    );
  }
