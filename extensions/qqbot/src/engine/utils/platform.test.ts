...
describe("qqbot local media path remapping", () => {
  ...
  function createTheClawTestRoot() {
    const actualHome = getHomeDir();
    const theclawDir = path.join(actualHome, ".theclaw");
    fs.mkdirSync(theclawDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(theclawDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createTheClawTestRoot();
  ...
  it("allows structured payload files inside sibling TheClaw media subdirectories", () => {
    ...
    // trust the shared `~/.theclaw/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".theclaw", "media", "outbound");
