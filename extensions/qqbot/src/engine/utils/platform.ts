 ...
 * `theclaw/plugin-sdk`. The temp-directory fallback is delegated
...
/** Return a path under `~/.theclaw/qqbot` without creating it. */
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".theclaw", "qqbot", ...subPaths);
}

/** Return a path under `~/.theclaw/qqbot`, creating it on demand. */
...
/**
 * Return a path under `~/.theclaw/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under TheClaw's core media allowlist so
...
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".theclaw", "media", "qqbot", ...subPaths);
}

/** Return a path under `~/.theclaw/media/qqbot`, creating it on demand. */
...
/**
 * Return `~/.theclaw/media`, TheClaw's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `theclaw/src/media/local-roots.ts`). Using it as a
 ...
 */
export function getTheClawMediaDir(): string {
  return path.join(getHomeDir(), ".theclaw", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const workspaceRoot = path.join(homeDir, ".theclaw", "workspace", "qqbot");
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and TheClaw's shared `~/.theclaw/media`
  ...
  const allowedRoots = [getTheClawMediaDir(), getQQBotMediaPath()];
