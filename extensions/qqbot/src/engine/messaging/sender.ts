...
let _theclawVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${_pluginVersion} (Node/${process.versions.node}; ${os.platform()}; TheClaw/${_theclawVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; theclawVersion?: string }): void {
  if (options.pluginVersion) {
    _pluginVersion = options.pluginVersion;
  }
  if (options.theclawVersion) {
    _theclawVersion = options.theclawVersion;
  }
}

/** Update the TheClaw framework version in the User-Agent (called after runtime injection). */
export function setTheClawVersion(version: string): void {
  if (version) {
    _theclawVersion = version;
  }
}
