 ...
 *   2. `$THECLAW_STATE_DIR` / `$CLAWDBOT_STATE_DIR`
 *   3. `~/.theclaw/agents/{agentId}/sessions/sessions.json`
 */
export function resolveSessionStorePath(
  ...
  const stateDir =
    process.env.THECLAW_STATE_DIR?.trim() ||
    process.env.CLAWDBOT_STATE_DIR?.trim() ||
    path.join(process.env.HOME || process.env.USERPROFILE || "", ".theclaw");
