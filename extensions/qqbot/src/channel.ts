import { getExecApprovalReplyMetadata } from "theclaw/plugin-sdk/approval-runtime";
...
import { toGatewayAccount, writeTheClawConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped theclaw.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // theclaw.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeTheClawConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe theclaw.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeTheClawConfigThroughRuntime(getQQBotRuntime(), nextCfg as TheClawConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as TheClawConfig, accountId);
