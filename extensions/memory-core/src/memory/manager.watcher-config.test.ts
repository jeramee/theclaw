...
import type {
  MemorySearchConfig,
  TheClawConfig,
} from "theclaw/plugin-sdk/memory-core-host-engine-foundation";
...
const { createdWatchers, watchMock } = vi.hoisted(() => {
  ...
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("theclaw.test.memoryWatchFactory")] =
    result.watchMock;
  return result;
});
...
describe("memory watcher config", () => {
  ...
  afterAll(() => {
    Reflect.deleteProperty(globalThis, Symbol.for("theclaw.test.memoryWatchFactory"));
  });
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): TheClawConfig {
    const defaults: NonNullable<NonNullable<TheClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: TheClawConfig) {
