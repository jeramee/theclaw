...
import { withFileLock } from "theclaw/plugin-sdk/file-lock";
import {
...
const QMD_UPDATE_QUEUE_KEY = Symbol.for("theclaw.qmdUpdateQueueState");
...
function normalizeHanBm25Query(query: string): string {
  ...
  // Keep Han/CJK BM25 queries intact so TheClaw search semantics match direct qmd search.
  return trimmed;
}
