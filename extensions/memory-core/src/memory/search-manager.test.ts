...
import type { checkQmdBinaryAvailability as checkQmdBinaryAvailabilityFn } from "theclaw/plugin-sdk/memory-core-host-engine-qmd";
...
vi.mock("theclaw/plugin-sdk/memory-core-host-engine-qmd", () => ({
...
function createBuiltinCfg(agentId: string): TheClawConfig {
  return {
    agents: {
      defaults: {
...
describe("getMemorySearchManager caching", () => {
  it("repairs an invalid shared singleton cache shape before using qmd cache maps", async () => {
    await closeAllMemorySearchManagers();
    vi.resetModules();
    const cacheKey = Symbol.for("theclaw.memorySearchManagerCache");
  ...
  it("creates a missing agent workspace before probing qmd availability", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-qmd-workspace-"));
