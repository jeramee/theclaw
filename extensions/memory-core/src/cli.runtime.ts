...
import type { MemoryEmbeddingProbeResult } from "theclaw/plugin-sdk/memory-core-host-engine-storage";
import { resolveMemoryRemDreamingConfig } from "theclaw/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "theclaw/plugin-sdk/routing";
...
function resolveMemoryPluginConfig(cfg: TheClawConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: TheClawConfig): string {
...
function resolveAgent(cfg: TheClawConfig, agent?: string) {
...
function resolveAgentIds(cfg: TheClawConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: theclaw memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredTheClawTmpDir(), "theclaw-rem-backfill-"),
      );
      try {
