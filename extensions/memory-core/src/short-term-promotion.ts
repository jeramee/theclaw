...
import { formatMemoryDreamingDay } from "theclaw/plugin-sdk/memory-core-host-status";
import { appendMemoryHostEvent } from "theclaw/plugin-sdk/memory-host-events";
...
const PROMOTION_MARKER_PREFIX = "theclaw-memory-promotion:";
...
function isContaminatedDreamingSnippet(raw: string): boolean {
  ...
  if (
    /<!--\s*theclaw-memory-promotion:/i.test(snippet) ||
...
function extractPromotionMarkers(memoryText: string): Set<string> {
  ...
  const matches = memoryText.matchAll(/<!--\s*theclaw-memory-promotion:([^\n]+?)\s*-->/gi);
