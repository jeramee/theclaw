...
import { resolveMemoryDreamingConfig } from "theclaw/plugin-sdk/memory-core-host-status";
...
function resolveMemoryCorePluginConfig(cfg: TheClawConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: TheClawConfig, enabled: boolean): TheClawConfig {
...
function formatStatus(cfg: TheClawConfig): string {
...
export function registerDreamingCommand(api: TheClawPluginApi): void {
  api.registerCommand({
    name: "dreaming",
