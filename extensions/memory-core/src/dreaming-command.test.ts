...
function resolveStoredDreaming(config: TheClawConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: TheClawConfig = {}) {
  const registered: { command?: TheClawPluginCommandDefinition } = {};
  let runtimeConfig: TheClawConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: TheClawConfig }) => {
        runtimeConfig = nextConfig;
      }),
      writeConfigFile: vi.fn(async (nextConfig: TheClawConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
  ...
  const api = {
    runtime,
    registerCommand: vi.fn((definition: TheClawPluginCommandDefinition) => {
