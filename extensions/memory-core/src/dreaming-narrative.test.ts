...
} from "theclaw/plugin-sdk/error-runtime";
import { resolveGlobalMap } from "theclaw/plugin-sdk/global-singleton";
import * as memoryCoreHostRuntimeCoreModule from "theclaw/plugin-sdk/memory-core-host-runtime-core";
import * as runtimeConfigSnapshotModule from "theclaw/plugin-sdk/runtime-config-snapshot";
import * as sessionStoreRuntimeModule from "theclaw/plugin-sdk/session-store-runtime";
...
const DREAMS_FILE_LOCKS_KEY = Symbol.for("theclaw.memoryCore.dreamingNarrative.fileLocks");
...
describe("backfill diary entries", () => {
  ...
  it("builds a marked backfill diary entry", () => {
    ...
    expect(entry).toContain("theclaw:dreaming:backfill-entry");
    expect(entry).toContain("What Happened");
  });

  it("writes and replaces backfill diary entries", async () => {
    const workspaceDir = await createTempWorkspace("theclaw-dreaming-backfill-");
    ...
    expect(content.match(/theclaw:dreaming:backfill-entry/g)?.length).toBe(1);
  });
...
describe("appendNarrativeEntry", () => {
  it("creates DREAMS.md with diary header on fresh workspace", async () => {
    const workspaceDir = await createTempWorkspace("theclaw-dreaming-narrative-");
    ...
    expect(content).toContain("<!-- theclaw:dreaming:diary:start -->");
    expect(content).toContain("<!-- theclaw:dreaming:diary:end -->");
  });
  ...
  it("appends a second entry within the diary markers", async () => {
    ...
    const start = content.indexOf("<!-- theclaw:dreaming:diary:start -->");
    const end = content.indexOf("<!-- theclaw:dreaming:diary:end -->");
  ...
  it("prepends diary before existing managed blocks", async () => {
    ...
    await fs.writeFile(
      dreamsPath,
      "## Light Sleep\n<!-- theclaw:dreaming:light:start -->\n- Candidate: test\n<!-- theclaw:dreaming:light:end -->\n",
      "utf-8",
    );
    await appendNarrativeEntry({
      workspaceDir,
  ...
  it("dedupes only exact diary duplicates while keeping distinct timestamps", async () => {
    const workspaceDir = await createTempWorkspace("theclaw-dreaming-dedupe-");
    const dreamsPath = path.join(workspaceDir, "DREAMS.md");
    await fs.writeFile(
      dreamsPath,
      [
...
describe("generateAndAppendDreamNarrative", () => {
  ...
  it("scrubs stale dreaming entries and orphan transcripts after cleanup", async () => {
    const workspaceDir = await createTempWorkspace("theclaw-dreaming-narrative-");
    const stateDir = await createTempWorkspace("theclaw-dreaming-state-");
  ...
  it("isolates narrative sessions across workspaces even at the same timestamp", async () => {
    const firstWorkspaceDir = await createTempWorkspace("theclaw-dreaming-narrative-");
    const secondWorkspaceDir = await createTempWorkspace("theclaw-dreaming-narrative-");
...
describe("runDetachedDreamNarrative", () => {
  ...
  it("caps the number of in-flight detached narratives at 3", async () => {
    ...
    const workspaceDir = await createTempWorkspace("theclaw-dreaming-detach-");
    const logger = createMockLogger();
