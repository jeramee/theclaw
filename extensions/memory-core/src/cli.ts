...
import {
  ...
  formatHelpExamples,
  theme,
} from "theclaw/plugin-sdk/memory-core-host-runtime-cli";
import type {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw memory status", "Show index and provider status."],
          [
            "theclaw memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["theclaw memory status --deep", "Probe embedding provider readiness."],
          ["theclaw memory index --force", "Force a full reindex."],
          ['theclaw memory search "meeting notes"', "Quick search using positional query."],
          [
            'theclaw memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `theclaw memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "theclaw memory rem-backfill --path ./memory",
          ...
          ["theclaw memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.theclaw.ai/cli/memory")}\n`,
    );
