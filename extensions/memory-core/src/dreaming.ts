...
import { peekSystemEventEntries } from "theclaw/plugin-sdk/system-event-runtime";
...
export function registerShortTermPromotionDreaming(api: TheClawPluginApi): void {
  ...
  const resolveCurrentConfig = (): TheClawConfig =>
    (api.runtime.config?.current?.() ?? api.config) as TheClawConfig;
