...
import * as sessionTranscriptHit from "theclaw/plugin-sdk/session-transcript-hit";
import { afterEach, describe, expect, it, vi } from "vitest";
import { filterMemorySearchHitsBySessionVisibility } from "./session-search-visibility.js";
import { asTheClawConfig } from "./tools.test-helpers.js";
...
vi.mock("theclaw/plugin-sdk/session-transcript-hit", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("theclaw/plugin-sdk/session-transcript-hit")>();
  return {
    ...actual,
...
describe("filterMemorySearchHitsBySessionVisibility", () => {
  ...
  it("drops sessions-sourced hits when requester key is missing (fail closed)", async () => {
    const cfg = asTheClawConfig({ tools: { sessions: { visibility: "all" } } });
