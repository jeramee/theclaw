...
import { resolvePreferredTheClawTmpDir } from "../api.js";
...
type ThinkingPolicy = ReturnType<TheClawPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<TheClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return !!level && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: TheClawPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via theclaw.invoke.",
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<TheClawPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      try {
        tmpDir = await fs.mkdtemp(
          path.join(resolvePreferredTheClawTmpDir(), "theclaw-llm-task-"),
        );
