...
`theclaw.invoke --each`) without adding new TheClaw code per workflow.
...
This extension depends on TheClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** TheClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.theclaw/extensions` as a standalone plugin directory.
