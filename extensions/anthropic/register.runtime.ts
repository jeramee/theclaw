import { formatCliCommand, parseDurationMs } from "theclaw/plugin-sdk/cli-runtime";
import type {
  TheClawPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type TheClawConfig as ProviderAuthConfig,
  type ProviderAuthResult,
  suggestOAuthProfileIdForLegacyDefault,
  upsertAuthProfile,
  validateAnthropicSetupToken,
} from "theclaw/plugin-sdk/provider-auth";
import {
  cloneFirstTemplateModel,
  isClaudeOpus47ModelId,
  type ProviderPlugin,
  resolveClaudeThinkingProfile,
} from "theclaw/plugin-sdk/provider-model-shared";
import { fetchClaudeUsage } from "theclaw/plugin-sdk/provider-usage";
...
const ANTHROPIC_SETUP_TOKEN_NOTE_LINES = [
  ...
  `If you want a direct API billing path instead, use ${formatCliCommand("theclaw models auth login --provider anthropic --method api-key --set-default")} or ${formatCliCommand("theclaw models auth login --provider anthropic --method cli --set-default")}.`,
] as const;
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("theclaw doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: TheClawPluginApi): void {
