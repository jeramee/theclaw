...
} from "theclaw/plugin-sdk/provider-web-search";
import {
...
function readLegacyGrokFallbackAuth(cfg?: TheClawConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: TheClawConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function readLegacyGrokApiKey(cfg?: TheClawConfig): string | undefined {
...
export function readPluginXaiWebSearchApiKey(cfg?: TheClawConfig): string | undefined {
  const resolved = readPluginXaiWebSearchApiKeyResult(cfg);
  return resolved.status === "available" ? resolved.value : undefined;
}

export function resolveFallbackXaiAuth(cfg?: TheClawConfig): XaiFallbackAuth | undefined {
...
export function resolveFallbackXaiApiKey(cfg?: TheClawConfig): string | undefined {
