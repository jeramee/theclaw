...
import { OPENAI_COMPATIBLE_REPLAY_HOOKS } from "theclaw/plugin-sdk/provider-model-shared";
import { defaultToolStreamExtraParams } from "theclaw/plugin-sdk/provider-stream-shared";
import { jsonResult, readProviderEnvValue } from "theclaw/plugin-sdk/provider-web-search";
    ...
    execute: async (toolCallId: string, args: Record<string, unknown>) => {
      ...
      if (!tool) {
        return jsonResult({
          ...
          docs: "https://docs.theclaw.ai/tools/code-execution",
        });
      }
