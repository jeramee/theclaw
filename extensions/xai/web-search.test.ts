import { createTestWizardPrompter } from "theclaw/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "theclaw/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "theclaw/plugin-sdk/runtime-env";
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      await expect(maybeTool.execute({ query: "TheClaw" })).resolves.toMatchObject({
