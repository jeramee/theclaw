...
import { createSandboxTestContext } from "theclaw/plugin-sdk/test-fixtures";
import {
...
const THECLAW_OPENSHELL_E2E = process.env.THECLAW_E2E_OPENSHELL === "1";
const THECLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const THECLAW_OPENSHELL_COMMAND =
  process.env.THECLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "theclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && THECLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: THECLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(THECLAW_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(THECLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-openshell-e2e-"));
      ...
      const gatewayName = `theclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `theclaw-policy-allow-${scopeSuffix}`;
      ...
      const sandboxCfg = {
        ...
        ssh: createSandboxSshConfig("/tmp/theclaw-sandboxes"),
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: THECLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("theclaw-openshell-e2e");
