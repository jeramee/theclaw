...
import type { DiscordAccountConfig, TtsConfig } from "theclaw/plugin-sdk/config-types";
...
import { parseTtsDirectives } from "theclaw/plugin-sdk/speech";
...
function resolveVoiceTtsConfig(params: { cfg: TheClawConfig; override?: TtsConfig }): {
  cfg: TheClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredTheClawTmpDir(), "discord-voice-"));
