...
import * as commandRegistryModule from "theclaw/plugin-sdk/command-auth";
import type { ChatCommandDefinition, CommandArgsParsing } from "theclaw/plugin-sdk/command-auth";
import type { ModelsProviderData } from "theclaw/plugin-sdk/command-auth";
import type { TheClawConfig } from "theclaw/plugin-sdk/config-types";
import * as globalsModule from "theclaw/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  saveSessionStore,
} from "theclaw/plugin-sdk/session-store-runtime";
import * as commandTextModule from "theclaw/plugin-sdk/text-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "theclaw-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          {
            id: "pi",
            label: "TheClaw Pi Default",
            description: "Use the built-in TheClaw Pi runtime.",
          },
          {
            id: "codex",
            label: "codex",
