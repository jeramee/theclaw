...
function createThreadBinding(
  overrides?: Partial<import("theclaw/plugin-sdk/conversation-runtime").SessionBindingRecord>,
) {
  return {
    ...
    ...overrides,
  } satisfies import("theclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
}

function createPreflightArgs(params: {
  cfg: import("theclaw/plugin-sdk/config-types").TheClawConfig;
...
async function runThreadBoundPreflight(params: {
  ...
  threadBinding: import("theclaw/plugin-sdk/conversation-runtime").SessionBindingRecord;
  discordConfig: DiscordConfig;
...
describe("preflightDiscordMessage", () => {
  ...
  it("preflights direct-message voice notes without mention gating", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hello theclaw from dm audio");
    ...
    expect(result?.preflightAudioTranscript).toBe("hello theclaw from dm audio");
  });
  ...
  it("bypasses mention gating in bound threads for allowed bot senders", async () => {
    ...
    const result = await preflightDiscordMessage(
      createPreflightArgs({
        cfg: {
          ...DEFAULT_PREFLIGHT_CFG,
        } as import("theclaw/plugin-sdk/config-types").TheClawConfig,
        discordConfig: {
          allowBots: true,
        } as DiscordConfig,
        data: createGuildEvent({
  ...
  it("allows bot messages with explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "hi <@theclaw-bot>",
      mentionedUsers: [{ id: "theclaw-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
        username: "Relay",
  ...
  it("hydrates mention metadata from REST when bot mention syntax is present but mentions are missing", async () => {
    ...
    client.rest = {
      get: vi.fn(async () => ({
        ...
        mentions: [{ id: botId, username: "TheClaw", bot: true }],
        mention_roles: [],
        mention_everyone: false,
      })),
  ...
  it("still allows bot control commands with an explicit mention when allowBots=mentions", async () => {
    ...
    const message = createDiscordMessage({
      ...
      channelId,
      content: "<@theclaw-bot> /new incident room",
      mentionedUsers: [{ id: "theclaw-bot" }],
      author: {
        id: "relay-bot-1",
        bot: true,
  ...
  it("uses attachment content_type for guild audio preflight mention detection", async () => {
    transcribeFirstAudioMock.mockResolvedValue("hey theclaw");
    ...
    expect(result).not.toBeNull();
    expect(result?.wasMentioned).toBe(true);
    expect(result?.preflightAudioTranscript).toBe("hey theclaw");
  });
  ...
  it("drops guild message without mention when channel has configuredBinding and requireMention: true", async () => {
    const conversationRuntime = await import("theclaw/plugin-sdk/conversation-runtime");
