...
import type { DiscordAccountConfig } from "theclaw/plugin-sdk/config-types";
...
} from "theclaw/plugin-sdk/proxy-capture";
import { danger } from "theclaw/plugin-sdk/runtime-env";
...
const DISCORD_GATEWAY_INFO_TIMEOUT_ENV = "THECLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS";
...
type ResolveDiscordGatewayIntentsParams =
  | import("theclaw/plugin-sdk/config-types").DiscordIntentsConfig
  | {
      intentsConfig?: import("theclaw/plugin-sdk/config-types").DiscordIntentsConfig;
...
function createGatewayPlugin(params: {
  ...
  class SafeGatewayPlugin extends carbonGateway.GatewayPlugin {
    ...
    public override connect(resume = false): void {
      ...
      // firstHeartbeatTimeout race (theclaw/theclaw#65009, #64011, #63387).
    ...
    override registerClient(client: Parameters<carbonGateway.GatewayPlugin["registerClient"]>[0]) {
      ...
      // awaiting them. Mark the promise handled immediately, then let TheClaw
...
export function createDiscordGatewayPlugin(params: {
  ...
  const options = {
    ...
    intents,
    // TheClaw registers its own async interaction listener. Carbon's default
