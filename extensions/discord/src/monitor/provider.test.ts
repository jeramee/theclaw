...
let runtimeEnvModule: typeof import("theclaw/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): TheClawConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("theclaw/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("theclaw/plugin-sdk/runtime-env");
  ...
  it("passes TheClaw EventQueue defaults to Carbon Client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
