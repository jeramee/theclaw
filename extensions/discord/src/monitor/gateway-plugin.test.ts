...
describe("SafeGatewayPlugin.connect()", () => {
  ...
  it("resolves gateway metadata timeout from config, env, then default", () => {
    ...
    expect(
      resolveDiscordGatewayInfoTimeoutMs({
        env: { THECLAW_DISCORD_GATEWAY_INFO_TIMEOUT_MS: "25000" },
      }),
    ).toBe(25_000);
  ...
  it("leaves Carbon autoInteractions disabled so TheClaw owns interaction handoff", () => {
    const plugin = createPlugin();
  ...
  it("keeps TheClaw metadata timeout out of Carbon gateway options", () => {
