import { DEFAULT_EMOJIS } from "theclaw/plugin-sdk/channel-feedback";
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/theclaw-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("theclaw/plugin-sdk/reply-runtime", () => ({
...
vi.mock("theclaw/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveStorePath.mockReturnValue("/tmp/theclaw-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/theclaw-discord-process-test-sessions.json" },
      },
    });
