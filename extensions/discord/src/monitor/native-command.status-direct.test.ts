...
vi.mock("theclaw/plugin-sdk/reply-dispatch-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/reply-dispatch-runtime")>(
...
vi.mock("theclaw/plugin-sdk/command-status-runtime", () => ({
...
function createConfig(params?: { requireMention?: boolean }): TheClawConfig {
  return {
    commands: {
...
describe("discord native /status", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setDispatchReplyWithDispatcher(
      runtimeModuleMocks.dispatchReplyWithDispatcher as typeof import("theclaw/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithDispatcher,
    );
