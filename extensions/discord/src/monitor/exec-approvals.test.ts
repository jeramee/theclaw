...
vi.mock("theclaw/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("theclaw/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<TheClawConfig["channels"]>["discord"]>["execApprovals"],
): TheClawConfig {
  return {
    channels: {
      discord: {
