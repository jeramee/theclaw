...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: TheClawConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): TheClawConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("theclaw/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/acp-runtime")>(
    "theclaw/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/command-auth", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/command-auth")>(
    "theclaw/plugin-sdk/command-auth",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("theclaw/plugin-sdk/native-command-config-runtime")
  >("theclaw/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/runtime-env")>(
    "theclaw/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/error-runtime")>(
    "theclaw/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
