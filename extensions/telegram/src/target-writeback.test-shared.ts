...
vi.mock("theclaw/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/config-mutation")>(
    "theclaw/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/cron-store-runtime")>(
    "theclaw/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeConfigFile).toHaveBeenCalledWith(
        ...
        expect.objectContaining({ expectedConfigPath: "/tmp/theclaw.json" }),
      );
