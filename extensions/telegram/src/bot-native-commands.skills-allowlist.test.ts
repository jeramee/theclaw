...
import { listSkillCommandsForAgents as listActualSkillCommandsForAgents } from "theclaw/plugin-sdk/skill-commands-runtime";
...
describe("registerTelegramNativeCommands skill allowlist integration", () => {
  ...
  it("registers only allowlisted skills for the bound agent menu", async () => {
    const workspaceDir = await makeWorkspace("theclaw-telegram-skills-");
    await writeSkill({
    ...
    listSkillCommandsForAgents.mockImplementation(
      ({ cfg, agentIds }: { cfg: TheClawConfig; agentIds?: string[] }) =>
