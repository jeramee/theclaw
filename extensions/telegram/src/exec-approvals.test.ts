...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<TheClawConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<TheClawConfig["channels"]>["telegram"]>>,
): TheClawConfig {
  return {
    channels: {
      telegram: {
