...
export function maybeRepairTelegramApiRoots(cfg: TheClawConfig): {
...
async function repairTelegramConfig(params: { cfg: TheClawConfig }): Promise<{
  config: TheClawConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: TheClawConfig): Promise<{
  config: TheClawConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("theclaw/plugin-sdk/runtime-secret-resolution");
