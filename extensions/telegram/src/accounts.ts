...
import {
  ...
  type TheClawConfig,
} from "theclaw/plugin-sdk/account-core";
import type { TelegramAccountConfig, TelegramActionConfig } from "theclaw/plugin-sdk/config-types";
import { formatSetExplicitDefaultInstruction } from "theclaw/plugin-sdk/routing";
import { createSubsystemLogger, isTruthyEnvValue } from "theclaw/plugin-sdk/runtime-env";
...
const debugAccounts = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.THECLAW_DEBUG_TELEGRAM_ACCOUNTS)) {
...
export function listTelegramAccountIds(cfg: TheClawConfig): string[] {
...
export function resolveDefaultTelegramAccountId(cfg: TheClawConfig): string {
...
export function listEnabledTelegramAccounts(cfg: TheClawConfig): ResolvedTelegramAccount[] {
