...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): TheClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as TheClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): TheClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as TheClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: TheClawConfig, accountId = "media") {
    await handleTelegramAction(
