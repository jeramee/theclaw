...
vi.mock("theclaw/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/channel-pairing")>(
...
vi.mock("theclaw/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/media-store")>(
    "theclaw/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
const HOME = path.join(os.tmpdir(), `theclaw-inbound-media-${crypto.randomUUID()}`);
