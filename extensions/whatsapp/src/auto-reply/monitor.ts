import { resolveAccountEntry } from "theclaw/plugin-sdk/account-core";
import { resolveInboundDebounceMs } from "theclaw/plugin-sdk/channel-inbound-debounce";
import { formatCliCommand } from "theclaw/plugin-sdk/cli-runtime";
import { hasControlCommand } from "theclaw/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "theclaw/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "theclaw/plugin-sdk/reply-history";
import { resolveAgentRoute } from "theclaw/plugin-sdk/routing";
import { logVerbose } from "theclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "theclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "theclaw/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  type RuntimeEnv,
} from "theclaw/plugin-sdk/runtime-env";
...
async function clearTerminalWebAuthState(params: {
  ...
  try {
    ...
    params.runtime.error(
      `WhatsApp Web cleanup failed after terminal close (status ${params.statusLabel}). Run \`${formatCliCommand("theclaw channels logout --channel whatsapp")}\`, then relink with \`${formatCliCommand("theclaw channels login --channel whatsapp")}\`.`,
    );
  }
}
    ...
    while (true) {
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          ...
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("theclaw channels login --channel whatsapp")}\` to relink.`,
          );
          ...
          runtime.error(
            `WhatsApp Web connection closed (status ${decision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("theclaw channels login --channel whatsapp")}\`. Stopping web monitoring.`,
          );
        } else {
          reconnectLogger.warn(
            {
