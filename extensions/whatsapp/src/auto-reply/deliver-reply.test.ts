...
import { sleep } from "theclaw/plugin-sdk/text-runtime";
...
vi.mock("theclaw/plugin-sdk/text-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/text-runtime")>(
    "theclaw/plugin-sdk/text-runtime",
  );
  return {
    ...actual,
    sleep: vi.fn(async () => {}),
