...
import { generateSecureUuid } from "theclaw/plugin-sdk/core";
import { redactIdentifier } from "theclaw/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "theclaw/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "theclaw/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "theclaw/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "theclaw/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: TheClawConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`theclaw channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: TheClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
