...
import type { ChannelSetupWizard } from "theclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "theclaw/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<TheClawConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: TheClawConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: TheClawConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: TheClawConfig, accountId: string): string {
  if (
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so TheClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for TheClaw" },
    ],
  });
...
export async function finalizeWhatsAppSetup(params: {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      `Run \`${formatCliCommand("theclaw channels login")}\` later to link WhatsApp.`,
      "WhatsApp",
    );
  }
