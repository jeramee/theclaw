...
import { VERSION } from "theclaw/plugin-sdk/cli-runtime";
import {
  resolveEnvHttpProxyUrl,
  shouldUseEnvHttpProxyForUrl,
} from "theclaw/plugin-sdk/fetch-runtime";
import { danger, success } from "theclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "theclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "theclaw/plugin-sdk/text-runtime";
import {
  readCredsJsonRaw,
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("theclaw channels login")}`,
              ),
            );
          }
        }
