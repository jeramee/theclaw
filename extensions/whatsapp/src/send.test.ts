...
let resetLogger: typeof import("theclaw/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("theclaw/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: TheClawConfig = {
  channels: { whatsapp: {} },
};
...
vi.mock("theclaw/plugin-sdk/media-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/media-runtime")>(
    "theclaw/plugin-sdk/media-runtime",
  );
  return {
    ...actual,
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("theclaw/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `theclaw-outbound-${crypto.randomUUID()}.log`);
