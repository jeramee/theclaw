...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "theclaw-test-web-logout-"));
    previousOAuthDir = process.env.THECLAW_OAUTH_DIR;
    process.env.THECLAW_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  afterAll(async () => {
    if (previousOAuthDir === undefined) {
      ...
      process.env.THECLAW_OAUTH_DIR = previousOAuthDir;
    }
  ...
  it("does not delete through symlinked auth dirs inside the TheClaw auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the TheClaw auth root", async () => {
