...
import { resetInboundDedupe } from "theclaw/plugin-sdk/reply-dedupe";
...
const WEB_AUTO_REPLY_SOCKETS_KEY = Symbol.for("theclaw:webAutoReplySessionSockets");
...
export function installWebAutoReplyTestHomeHooks() {
  beforeAll(async () => {
    tempHomeRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-web-home-suite-"));
  });
...
export async function makeSessionStore(
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-session-"));
