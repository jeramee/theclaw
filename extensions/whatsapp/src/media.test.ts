...
import { captureEnv } from "theclaw/plugin-sdk/test-env";
import { mockPinnedHostnameResolution } from "theclaw/plugin-sdk/test-env";
import { optimizeImageToPng } from "theclaw/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub THECLAW_STATE_DIR.
    // Also keep it outside the TheClaw temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["THECLAW_STATE_DIR"]);
    process.env.THECLAW_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "theclaw-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredTheClawTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("allows default TheClaw state workspace and sandbox roots", async () => {
  ...
  it("rejects default TheClaw state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
