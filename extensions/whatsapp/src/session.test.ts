...
import { resetLogger, setLoggerOverride } from "theclaw/plugin-sdk/runtime-env";
...
function mockCredsJsonSpies(readContents: string) {
  const credsSuffix = path.join("/tmp", "theclaw-oauth", "whatsapp", "default", "creds.json");
...
describe("web session", () => {
  ...
  it("creates WA socket with QR handler", async () => {
    const authDir = createTempAuthDir("theclaw-wa-creds-test");
  ...
  it("serializes creds.update saves to avoid overlapping writes", async () => {
    ...
    const authDir = createTempAuthDir("theclaw-wa-queue");
    const openMock = mockFsOpenForCredsWrites({
  ...
  it("lets different authDir queues flush independently", async () => {
    ...
    const authDirA = createTempAuthDir("theclaw-wa-a");
    const authDirB = createTempAuthDir("theclaw-wa-b");
    const onError = vi.fn();
  ...
  it("writes creds.json atomically via temp file and rename", async () => {
    ...
    await writeCredsJsonAtomically("/tmp/theclaw-oauth/whatsapp/default", {
    ...
    expect(String(renameArgs[1] ?? "")).toContain(
      path.join("/tmp", "theclaw-oauth", "whatsapp", "default", "creds.json"),
    );
  ...
  it("keeps the previous creds.json valid if the atomic rename fails", async () => {
    const authDir = createTempAuthDir("theclaw-wa-creds-atomic");
