...
import { dispatchInboundReplyWithBase } from "theclaw/plugin-sdk/inbound-reply-dispatch";
import {
...
export async function handleQaInbound(params: {
  ...
  const route = runtime.channel.routing.resolveAgentRoute({
    cfg: params.config as TheClawConfig,
  ...
  const wasMentioned = isGroup
    ? runtime.channel.mentions.matchesMentionPatterns(
        ...
        runtime.channel.mentions.buildMentionRegexes(
          params.config as TheClawConfig,
          route.agentId,
        ),
      )
    : undefined;
  ...
  const body = runtime.channel.reply.formatAgentEnvelope({
    ...
    previousTimestamp,
    envelope: runtime.channel.reply.resolveEnvelopeFormatOptions(params.config as TheClawConfig),
    body: inbound.text,
  });
