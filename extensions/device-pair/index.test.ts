...
import type { TheClawPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredTheClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "theclaw-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredTheClawTmpDir: pluginApiMocks.resolvePreferredTheClawTmpDir,
...
function createApi(params?: {
  runtime?: TheClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as TheClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: TheClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as TheClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredTheClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredTheClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the TheClaw iOS app:");
    ...
    expect(text).not.toContain("![TheClaw pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the TheClaw iOS app:");
