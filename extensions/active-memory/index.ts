...
} from "theclaw/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type TheClawPluginApi } from "theclaw/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "theclaw/plugin-sdk/routing";
import {
...
function resolvePersistentTranscriptBaseDir(api: TheClawPluginApi, agentId: string): string {
  return path.join(
...
function resolveToggleStatePath(api: TheClawPluginApi): string {
  return path.join(
    api.runtime.state.resolveStateDir(),
    "plugins",
    "active-memory",
...
function isActiveMemoryGloballyEnabled(cfg: TheClawConfig): boolean {
...
async function runRecallSubagent(params: {
  ...
  const tempDir = params.config.persistTranscripts
    ? undefined
    : await fs.mkdtemp(path.join(resolvePreferredTheClawTmpDir(), "theclaw-active-memory-"));
  ...
  register(api: TheClawPluginApi) {
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as TheClawConfig;
          if (action === "status") {
            return {
