...
import { createPluginSetupWizardStatus } from "theclaw/plugin-sdk/plugin-test-runtime";
...
describe("zalouser setup plugin", () => {
  it("builds setup status without an initialized runtime", async () => {
    const stateDir = await mkdtemp(path.join(os.tmpdir(), "theclaw-zalouser-setup-"));

    try {
      await withEnvAsync({ THECLAW_STATE_DIR: stateDir }, async () => {
        await expect(
