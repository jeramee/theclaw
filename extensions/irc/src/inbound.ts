import { createChannelPairingController } from "theclaw/plugin-sdk/channel-pairing";
import {
...
import type { RuntimeEnv } from "theclaw/plugin-sdk/runtime";
import {
...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as TheClawConfig);
  ...
  if (message.isGroup && commandGate.shouldBlock) {
    const { logInboundDrop } = await import("theclaw/plugin-sdk/channel-inbound");
    logInboundDrop({
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as TheClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as TheClawConfig);
  ...
  const { dispatchInboundReplyWithBase } =
    await import("theclaw/plugin-sdk/inbound-reply-dispatch");
