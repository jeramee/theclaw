...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "theclaw/plugin-sdk/speech-core";
...
vi.mock("theclaw/plugin-sdk/media-runtime", () => ({
...
function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "theclaw-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.THECLAW_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("theclaw/plugin-sdk/media-runtime")>(
