...
} from "theclaw/plugin-sdk/account-resolution";
import type { TheClawConfig } from "theclaw/plugin-sdk/config-types";
import { normalizeSecretInputString, type SecretInput } from "theclaw/plugin-sdk/secret-input";
...
function resolveConfiguredDefaultNostrAccountId(cfg: TheClawConfig): string | undefined {
 ...
 */
export function listNostrAccountIds(cfg: TheClawConfig): string[] {
  const nostrCfg = (cfg.channels as Record<string, unknown> | undefined)?.nostr as
    | NostrAccountConfig
    | undefined;
 ...
 */
export function resolveDefaultNostrAccountId(cfg: TheClawConfig): string {
