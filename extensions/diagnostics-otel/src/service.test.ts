...
import type { TheClawPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/theclaw-diagnostics-otel-test";
...
const ORIGINAL_THECLAW_OTEL_PRELOADED = process.env.THECLAW_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  captureContent?: NonNullable<
    NonNullable<TheClawPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["captureContent"];
};
...
function createTraceOnlyContext(endpoint: string): TheClawPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.THECLAW_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_THECLAW_OTEL_PRELOADED === undefined) {
      delete process.env.THECLAW_OTEL_PRELOADED;
    } else {
      process.env.THECLAW_OTEL_PRELOADED = ORIGINAL_THECLAW_OTEL_PRELOADED;
    }
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("theclaw.webhook.received")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("theclaw.webhook.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("theclaw.message.queued")?.add).toHaveBeenCalled();
    expect(telemetryState.counters.get("theclaw.message.processed")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("theclaw.message.duration_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.histograms.get("theclaw.queue.wait_ms")?.record).toHaveBeenCalled();
    expect(telemetryState.counters.get("theclaw.session.stuck")?.add).toHaveBeenCalled();
    expect(
      telemetryState.histograms.get("theclaw.session.stuck_age_ms")?.record,
    ).toHaveBeenCalled();
    expect(telemetryState.counters.get("theclaw.run.attempt")?.add).toHaveBeenCalled();

    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("theclaw.webhook.processed");
    expect(spanNames).toContain("theclaw.message.processed");
    expect(spanNames).toContain("theclaw.session.stuck");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.THECLAW_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    expect(telemetryState.histograms.get("theclaw.run.duration_ms")?.record).toHaveBeenCalledWith(
      100,
      expect.objectContaining({
        "theclaw.provider": "openai",
        "theclaw.model": "gpt-5.4",
      }),
    );
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("theclaw.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "theclaw.exporter": "diagnostics-otel",
      "theclaw.signal": "logs",
      "theclaw.status": "started",
      "theclaw.reason": "configured",
    });
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("theclaw.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "theclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("theclaw.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "theclaw.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("theclaw.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "theclaw.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpan = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.liveness.warning",
    );
    expect(livenessSpan?.[1]).toMatchObject({
      attributes: {
        "theclaw.liveness.reason": "event_loop_delay:cpu",
        "theclaw.liveness.active": 2,
        "theclaw.liveness.queued": 4,
      },
    });
    const span = telemetryState.spans.find((item) => item.name === "theclaw.liveness.warning");
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "theclaw.exporter": "diagnostics-otel",
      "theclaw.signal": "logs",
      "theclaw.status": "failure",
      "theclaw.reason": "emit_failed",
      "theclaw.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("theclaw.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
      type: "telemetry.exporter",
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["theclaw.token"];
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(emitCall?.attributes).toEqual(
      expect.not.objectContaining({
        "theclaw.traceId": expect.anything(),
        "theclaw.spanId": expect.anything(),
        "theclaw.traceFlags": expect.anything(),
      }),
    );
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/theclaw/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/theclaw/src/private.ts:42",
      },
    ...
    expect(emitCall?.attributes).toMatchObject({
      "theclaw.good": expect.stringMatching(/^y+/),
      "code.lineno": 42,
      "code.function": "handler",
    });
    expect(String(emitCall?.attributes?.["theclaw.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, `theclaw.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "theclaw.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "theclaw.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall?.attributes ?? {},
        "theclaw.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(emitCall?.attributes).toEqual(
      expect.not.objectContaining({
        "theclaw.bad key": expect.anything(),
        "code.filepath": expect.anything(),
        "theclaw.code.location": expect.anything(),
      }),
    );
    await service.stop?.(ctx);
  });
  ...
  test("does not parent diagnostic event spans from plugin-emittable trace context", async () => {
    ...
    const modelUsageCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.model.usage",
    );
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("theclaw.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "theclaw.channel": "webchat",
      "theclaw.agent": "ops",
      "theclaw.provider": "openai",
      "theclaw.model": "gpt-5.4",
      "theclaw.token": "input",
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("theclaw.tokens")?.add).toHaveBeenCalledWith(2, {
      "theclaw.channel": "unknown",
      "theclaw.agent": "unknown",
      "theclaw.provider": "openai",
      "theclaw.model": "gpt-5.4",
      "theclaw.token": "input",
    });
    expect(
      JSON.stringify(telemetryState.counters.get("theclaw.tokens")?.add.mock.calls),
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    expect(modelUsageCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.sessionKey": expect.anything(),
        "theclaw.sessionId": expect.anything(),
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.model.call",
    );
    ...
    expect(modelCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "gen_ai.provider.name": expect.anything(),
        "theclaw.callId": expect.anything(),
        "theclaw.runId": expect.anything(),
        "theclaw.sessionKey": expect.anything(),
      }),
      startTime: expect.any(Number),
    ...
    const harnessCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.harness.run",
    );
    expect(harnessCall?.[1]).toMatchObject({
      attributes: {
        "theclaw.harness.id": "codex",
        "theclaw.harness.plugin": "codex-plugin",
        "theclaw.outcome": "completed",
        "theclaw.provider": "codex",
        "theclaw.model": "gpt-5.4",
        "theclaw.channel": "qa",
        "theclaw.harness.result_classification": "reasoning-only",
        "theclaw.harness.yield_detected": true,
        "theclaw.harness.items.started": 3,
        "theclaw.harness.items.completed": 2,
        "theclaw.harness.items.active": 1,
      },
      startTime: expect.any(Number),
    });
    ...
    const toolCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.tool.execution",
    );
    expect(toolCall?.[1]).toMatchObject({
      attributes: {
        "theclaw.toolName": "read",
        "theclaw.errorCategory": "TypeError",
        "theclaw.errorCode": "429",
        "theclaw.tool.params.kind": "object",
        "gen_ai.tool.name": "read",
      },
    });
    expect(toolCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.toolCallId": expect.anything(),
        "theclaw.runId": expect.anything(),
        "theclaw.sessionKey": expect.anything(),
      }),
      startTime: expect.any(Number),
    ...
    expect(
      telemetryState.histograms.get("theclaw.model_call.duration_ms")?.record,
    ...
    expect(
      telemetryState.histograms.get("theclaw.model_call.request_bytes")?.record,
    ).toHaveBeenCalledWith(
      1234,
      expect.objectContaining({
        "theclaw.provider": "openai",
    ...
    expect(
      telemetryState.histograms.get("theclaw.model_call.response_bytes")?.record,
    ).toHaveBeenCalledWith(
      567,
      expect.objectContaining({
        "theclaw.provider": "openai",
    ...
    expect(
      telemetryState.histograms.get("theclaw.model_call.time_to_first_byte_ms")?.record,
    ).toHaveBeenCalledWith(
      45,
      expect.objectContaining({
        "theclaw.provider": "openai",
    ...
    const modelCallSpan = telemetryState.spans.find((span) => span.name === "theclaw.model.call");
    expect(modelCallSpan?.setAttributes).toHaveBeenCalledWith(
      expect.objectContaining({
        "theclaw.model_call.request_bytes": 1234,
        "theclaw.model_call.response_bytes": 567,
        "theclaw.model_call.time_to_first_byte_ms": 45,
      }),
    );
    expect(telemetryState.histograms.get("theclaw.run.duration_ms")?.record).toHaveBeenCalledWith(
      100,
    ...
    expect(
      telemetryState.histograms.get("theclaw.harness.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      90,
      expect.objectContaining({
        "theclaw.harness.id": "codex",
    ...
    expect(
      telemetryState.histograms.get("theclaw.tool.execution.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      20,
      expect.not.objectContaining({
        "theclaw.errorCode": expect.anything(),
        "theclaw.runId": expect.anything(),
      }),
    );

    const toolSpan = telemetryState.spans.find((span) => span.name === "theclaw.tool.execution");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "theclaw.model.call")
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    expect(modelCall?.[1]).toEqual({
      attributes: expect.objectContaining({
        "theclaw.failureKind": "terminated",
      }),
      startTime: expect.any(Number),
    });
    expect(modelCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.upstreamRequestIdHash": expect.anything(),
      }),
      startTime: expect.any(Number),
    });
    const span = telemetryState.spans.find((candidate) => candidate.name === "theclaw.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("theclaw.provider.request", {
      "theclaw.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    expect(
      telemetryState.histograms.get("theclaw.model_call.duration_ms")?.record,
    ).toHaveBeenCalledWith(
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.context.assembled",
    );
    const runSpan = telemetryState.spans.find((span) => span.name === "theclaw.run");
    ...
    expect(contextCall?.[1]).toMatchObject({
      attributes: {
        ...
        "theclaw.model": "gpt-5.4",
        "theclaw.channel": "webchat",
        "theclaw.trigger": "message",
        "theclaw.context.message_count": 12,
        "theclaw.context.history_text_chars": 1234,
        "theclaw.context.history_image_blocks": 2,
        "theclaw.context.max_message_text_chars": 456,
        "theclaw.context.system_prompt_chars": 789,
        "theclaw.context.prompt_chars": 42,
        "theclaw.context.prompt_images": 1,
        "theclaw.context.token_budget": 128_000,
        "theclaw.context.reserve_tokens": 4096,
      },
    });
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("theclaw.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "theclaw.toolName": "process",
      "theclaw.loop.level": "critical",
      "theclaw.loop.action": "block",
      "theclaw.loop.detector": "known_poll_no_progress",
      "theclaw.loop.count": 20,
      "theclaw.loop.paired_tool": "read",
    });
    const loopSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.tool.loop",
    );
    ...
    const loopSpan = telemetryState.spans.find((span) => span.name === "theclaw.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("theclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("theclaw.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "theclaw.memory.level": "critical",
        "theclaw.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("theclaw.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "theclaw.memory.level": "critical",
      "theclaw.memory.reason": "rss_growth",
    });
    const pressureCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.memory.pressure",
    );
    expect(pressureCall?.[1]).toMatchObject({
      attributes: {
        "theclaw.memory.level": "critical",
        "theclaw.memory.reason": "rss_growth",
        "theclaw.memory.rss_bytes": 200,
        "theclaw.memory.heap_used_bytes": 50,
        "theclaw.memory.heap_total_bytes": 90,
        "theclaw.memory.external_bytes": 20,
        "theclaw.memory.array_buffers_bytes": 6,
        "theclaw.memory.threshold_bytes": 512,
        "theclaw.memory.rss_growth_bytes": 256,
        "theclaw.memory.window_ms": 60_000,
      },
    });
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "theclaw.memory.pressure",
    );
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "theclaw.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "theclaw.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "theclaw.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName).toMatchObject({
      "theclaw.run": undefined,
      "theclaw.model.call": runSpanId,
      "theclaw.tool.execution": modelSpanId,
    });
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName).toMatchObject({
      "theclaw.run": undefined,
      "theclaw.model.call": undefined,
      "theclaw.tool.execution": undefined,
    });
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "theclaw.run"),
    ).toHaveLength(1);
    expect(
  ...
  test("exports exec process spans without command text", async () => {
    ...
    expect(telemetryState.histograms.get("theclaw.exec.duration_ms")?.record).toHaveBeenCalledWith(
      30,
      expect.objectContaining({
        "theclaw.exec.target": "host",
        "theclaw.exec.mode": "child",
        "theclaw.outcome": "failed",
        "theclaw.failureKind": "runtime-error",
      }),
    );

    const execCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "theclaw.exec",
    );
    expect(execCall?.[1]).toMatchObject({
      attributes: {
        ...
        "theclaw.exec.mode": "child",
        "theclaw.outcome": "failed",
        "theclaw.exec.command_length": 42,
        "theclaw.exec.exit_code": 1,
        "theclaw.exec.timed_out": false,
        "theclaw.failureKind": "runtime-error",
      },
      startTime: expect.any(Number),
    });
    expect(execCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.exec.command": expect.anything(),
        "theclaw.exec.workdir": expect.anything(),
        "theclaw.sessionKey": expect.anything(),
      }),
      startTime: expect.any(Number),
    });

    const execSpan = telemetryState.spans.find((span) => span.name === "theclaw.exec");
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("theclaw.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "theclaw.channel": "matrix",
      "theclaw.delivery.kind": "text",
    });
    expect(
      telemetryState.histograms.get("theclaw.message.delivery.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "theclaw.channel": "matrix",
    ...
    expect(
      ...
      40,
      expect.objectContaining({
        "theclaw.channel": "discord",
        "theclaw.delivery.kind": "media",
    ...
    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "theclaw.message.delivery",
    );
    ...
    expect(deliverySpanCalls[0]?.[1]).toMatchObject({
      attributes: {
        ...
        "theclaw.delivery.kind": "text",
        "theclaw.outcome": "completed",
        "theclaw.delivery.result_count": 1,
      },
      startTime: expect.any(Number),
    });
    ...
    for (const call of deliverySpanCalls) {
      expect(call[1]).toEqual({
        attributes: expect.not.objectContaining({
          "theclaw.sessionKey": expect.anything(),
          "theclaw.messageId": expect.anything(),
          "theclaw.conversationId": expect.anything(),
          "theclaw.content": expect.anything(),
          "theclaw.to": expect.anything(),
        }),
        startTime: expect.any(Number),
      });
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "theclaw.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    expect(modelCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.content.input_messages": expect.anything(),
        "theclaw.content.output_messages": expect.anything(),
        "theclaw.content.system_prompt": expect.anything(),
      }),
      startTime: expect.any(Number),
    });
    expect(toolCall?.[1]).toEqual({
      attributes: expect.not.objectContaining({
        "theclaw.content.tool_input": expect.anything(),
        "theclaw.content.tool_output": expect.anything(),
      }),
      startTime: expect.any(Number),
    });
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs).toMatchObject({
      "theclaw.content.output_messages": "model reply",
      "theclaw.content.system_prompt": "system prompt",
    });
    expect(String(modelAttrs?.["theclaw.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs).toMatchObject({
      "theclaw.content.tool_input": "tool input",
    });
    expect(String(toolAttrs?.["theclaw.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["theclaw.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionCounter = telemetryState.counters.get("theclaw.session.state");
    expect(sessionCounter?.add).toHaveBeenCalledWith(
      1,
      expect.objectContaining({
        "theclaw.reason": expect.stringContaining("…"),
      }),
    );
    const attrs = sessionCounter?.add.mock.calls[0]?.[1] as Record<string, unknown> | undefined;
    expect(typeof attrs?.["theclaw.reason"]).toBe("string");
    expect(String(attrs?.["theclaw.reason"])).not.toContain(
