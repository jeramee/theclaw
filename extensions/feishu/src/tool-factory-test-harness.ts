...
export function createToolFactoryHarness(cfg: TheClawPluginApi["config"]) {
  const registered: RegisteredTool[] = [];

  const api: Pick<TheClawPluginApi, "config" | "logger" | "registerTool"> = {
    config: cfg,
    logger: {
      info: () => {},
      warn: () => {},
