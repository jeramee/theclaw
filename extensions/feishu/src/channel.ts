...
import { getSessionBindingService } from "theclaw/plugin-sdk/conversation-runtime";
import {
...
} from "theclaw/plugin-sdk/interactive-runtime";
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("theclaw/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("theclaw/plugin-sdk/config-mutation");
