...
vi.mock("theclaw/plugin-sdk/media-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("theclaw/plugin-sdk/media-runtime")>();
  return {
    ...actual,
...
function expectPathIsolatedToTmpRoot(pathValue: string, key: string): void {
  ...
  const tmpRoot = path.resolve(resolvePreferredTheClawTmpDir());
...
describe("sendMediaFeishu msg_type routing", () => {
  ...
  it("passes mediaLocalRoots as localRoots to loadWebMedia for local paths (#27884)", async () => {
    ...
    const roots = ["/allowed/workspace", "/tmp/theclaw"];
    await sendMediaFeishu({
      cfg: emptyConfig,
      to: "user:ou_target",
