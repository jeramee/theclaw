...
import { mediaKindFromMime } from "theclaw/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "theclaw/plugin-sdk/media-runtime";
import {
  resolvePreferredTheClawTmpDir,
  withTempDownloadPath,
} from "theclaw/plugin-sdk/temp-path";
...
export async function downloadImageFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "theclaw-feishu-img-",
...
export async function downloadMessageResourceFeishu(params: {
  ...
  const buffer = await readFeishuResponseBuffer({
    response,
    tmpDirPrefix: "theclaw-feishu-resource-",
