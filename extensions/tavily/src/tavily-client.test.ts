...
vi.mock("theclaw/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: theclaw", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "theclaw" });
  });

  it("runTavilyExtract sends X-Client-Source: theclaw", async () => {
