...
import type { TheClawPluginApi } from "theclaw/plugin-sdk/plugin-runtime";
...
function fakeApi(): TheClawPluginApi {
  return {
    config: {},
  } as TheClawPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.theclaw.ai", "", "theclaw.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.theclaw.ai", "theclaw.ai"],
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as TheClawConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
