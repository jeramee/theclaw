...
const matrixWrapperGlobal = globalThis as typeof globalThis & {
  __theclawMatrixWrapperJitiOptions?: unknown;
};
const PLUGIN_SDK_ROOT = ["theclaw", "plugin-sdk"].join("/");
const SCOPED_PLUGIN_SDK_ROOT = ["@theclaw", "plugin-sdk"].join("/");
...
function writeCapturingJitiFixture(fixtureRoot: string) {
  writeFixtureFile(
    ...
    [
      ...
      "  globalThis.__theclawMatrixWrapperJitiOptions = options;",
...
function writeTheClawPackageFixture(fixtureRoot: string) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
  ...
  writeFixtureFile(fixtureRoot, "theclaw.mjs", "export {};\n");
  writeFixtureFile(fixtureRoot, "dist/plugin-sdk/index.js", "export {};\n");
}

function writeTheClawAliasFixture(fixtureRoot: string, extraExports?: Record<string, string>) {
  writeFixtureFile(
    fixtureRoot,
    "package.json",
    JSON.stringify(
...
function writeTrustedTheClawBinFixture(
  fixtureRoot: string,
...
function writeCapturingSourceRuntimeWrapperFixture(fixtureRoot: string) {
  delete matrixWrapperGlobal.__theclawMatrixWrapperJitiOptions;
  writeTheClawAliasFixture(fixtureRoot);
...
function expectSourcePluginSdkAliases(fixtureRoot: string) {
  expect(matrixWrapperGlobal.__theclawMatrixWrapperJitiOptions).toMatchObject({
    alias: {
...
it("loads the source-checkout runtime wrapper through native ESM import", async () => {
  ...
  writeTheClawPackageFixture(fixtureRoot);
...
it("resolves extension-api aliases through the same source extension family", async () => {
  ...
  expect(matrixWrapperGlobal.__theclawMatrixWrapperJitiOptions).toMatchObject({
    alias: {
      "theclaw/extension-api": path.join(fixtureRoot, "src", "extensionAPI.mts"),
    },
  });
}, 240_000);
...
it("keeps wrapper plugin-sdk aliases deterministic and ignores unsafe subpaths", async () => {
  ...
  const aliasKeys = Object.keys(
    (
      (matrixWrapperGlobal.__theclawMatrixWrapperJitiOptions ?? {}) as {
...
it("ignores nearby untrusted theclaw package stubs when resolving the wrapper root", async () => {
...
it("treats string bin hints case-insensitively when trusting wrapper package roots", async () => {
  ...
  delete matrixWrapperGlobal.__theclawMatrixWrapperJitiOptions;
  writeTrustedTheClawBinFixture(fixtureRoot, "TheClaw.MJS");
  writeCapturingJitiFixture(fixtureRoot);
  writeSourceRuntimeWrapperFixture(fixtureRoot);
  await importFixtureModule(fixtureRoot, "extensions/matrix/src/plugin-entry.runtime.js");
