...
const THECLAW_PLUGIN_SDK_PACKAGE_NAMES = [
  ["theclaw", "plugin-sdk"].join("/"),
  ["@theclaw", "plugin-sdk"].join("/"),
];
...
function hasTrustedTheClawRootIndicator(packageRoot, packageJson) {
  ...
  const hasTheClawBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("theclaw")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.theclaw === "string");
  const hasTheClawEntrypoint = fs.existsSync(path.join(packageRoot, "theclaw.mjs"));
  return hasCliEntryExport || hasTheClawBin || hasTheClawEntrypoint;
}

function findTheClawPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "theclaw" && hasTrustedTheClawRootIndicator(cursor, pkg)) {
...
function buildPluginSdkAliasMap(moduleUrl) {
  const location = findTheClawPackageRoot(path.dirname(fileURLToPath(moduleUrl)));
  if (!location) {
    return {};
  }
  ...
  if (rootAlias) {
    for (const packageName of THECLAW_PLUGIN_SDK_PACKAGE_NAMES) {
  ...
  if (extensionApi) {
    aliasMap["theclaw/extension-api"] = extensionApi;
  }
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findTheClawPackageRoot(moduleDir);
  if (location) {
