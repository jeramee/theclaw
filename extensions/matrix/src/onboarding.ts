...
import type { DmPolicy } from "theclaw/plugin-sdk/config-types";
import {
...
import { isPrivateNetworkOptInEnabled } from "theclaw/plugin-sdk/ssrf-policy";
import {
...
async function configureMatrixInviteAutoJoin(params: {
  ...
  await params.prompter.note(
    [
      ...
      "TheClaw agents will not join invited rooms or fresh DM-style invites unless you set autoJoin.",
  ...
  const deviceName =
    normalizeStringifiedOptionalString(
      await params.prompter.text({
        ...
        initialValue: existing.deviceName ?? "TheClaw Gateway",
      }),
    ) ?? "";
