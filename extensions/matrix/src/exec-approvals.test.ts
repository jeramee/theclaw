...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<TheClawConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<TheClawConfig["channels"]>["matrix"]>>,
): TheClawConfig {
  return {
    channels: {
      matrix: {
