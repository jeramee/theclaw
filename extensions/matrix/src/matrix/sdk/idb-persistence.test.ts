...
} from "theclaw/plugin-sdk/file-lock";
...
describe("Matrix IndexedDB persistence", () => {
  ...
  it("persists and restores database contents for the selected prefix", async () => {
    ...
    await seedDatabase({
      name: "theclaw-matrix-test::matrix-sdk-crypto",
      storeName: "sessions",
    ...
    await persistIdbToDisk({
      snapshotPath,
      databasePrefix: "theclaw-matrix-test",
    });
  ...
  it("serializes concurrent persist operations via file lock", async () => {
    ...
    await Promise.all([
      persistIdbToDisk({ snapshotPath, databasePrefix: "theclaw-matrix-test" }),
      persistIdbToDisk({ snapshotPath, databasePrefix: "theclaw-matrix-test" }),
    ]);
