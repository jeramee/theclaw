...
function formatMatrixPostHealthySyncDecryptionHint(accountId: string): string {
  return (
    ...
    `Check 'theclaw matrix verify status --verbose --account ${accountId}' and 'theclaw matrix devices list --account ${accountId}'.`
  );
}
...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another TheClaw runtime is using the same account. " +
    `Check 'theclaw matrix verify status --verbose --account ${accountId}' and 'theclaw matrix devices list --account ${accountId}'.`
  );
}
