...
export async function runMatrixStartupMaintenance(
  ...
  try {
    ...
    if (deviceHealth.staleTheClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale TheClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleTheClawDevices.map((device) => device.deviceId).join(", ")}. Run 'theclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
    if (isMatrixStartupAbortError(err)) {
