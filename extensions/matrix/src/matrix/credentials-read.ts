...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  try {
    ...
    const homeDir = env.THECLAW_HOME?.trim() || env.HOME?.trim() || os.homedir();
    return path.join(homeDir, ".theclaw");
  }
}
...
function shouldReadLegacyCredentialsForAccount(accountId?: string | null): boolean {
  ...
  const cfg = getMatrixRuntime().config.current() as TheClawConfig;
