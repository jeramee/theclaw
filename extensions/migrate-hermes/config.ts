...
} from "theclaw/plugin-sdk/migration";
import type { MigrationItem, MigrationProviderContext } from "theclaw/plugin-sdk/plugin-entry";
  ...
  if (params.hasMemoryFiles || memoryProvider) {
    items.push(
      createConfigPatchItem({
        ...
        message: "Use TheClaw built-in file memory for imported Hermes memory files.",
        conflict:
    ...
    items.push(
      createConfigPatchItem({
        ...
        message: "Select the default TheClaw memory plugin for imported file memory.",
        conflict:
          !params.ctx.overwrite &&
  ...
  if (memoryProvider === "honcho") {
    ...
    items.push(
      createManualItem({
        ...
        message:
          "Hermes used Honcho memory. TheClaw keeps built-in memory selected until the matching plugin is installed and reviewed.",
        recommendation:
    ...
    items.push(
      createManualItem({
        ...
        source: "config.yaml:memory.provider",
        message: `Hermes memory provider "${memoryProvider}" does not have a known TheClaw mapping.`,
        recommendation: "Install or configure an equivalent TheClaw memory plugin manually.",
      }),
    );
  }
