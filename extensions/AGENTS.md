...
- Extension production code should import from `theclaw/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `theclaw.plugin.json` and the package
  `theclaw` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `theclaw/plugin-sdk/*`.
