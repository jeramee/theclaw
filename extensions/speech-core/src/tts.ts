...
import { resolveChannelTtsVoiceDelivery } from "theclaw/plugin-sdk/channel-targets";
import type {
  TheClawConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { isVerbose, logVerbose } from "theclaw/plugin-sdk/runtime-env";
import { resolvePreferredTheClawTmpDir } from "theclaw/plugin-sdk/sandbox";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.THECLAW_TTS_PREFS?.trim();
  if (envPath) {
...
function sortSpeechProvidersForAutoSelection(cfg?: TheClawConfig) {
...
function _resolveRegistryDefaultSpeechProviderId(cfg?: TheClawConfig): TtsProvider {
  return sortSpeechProvidersForAutoSelection(cfg)[0]?.id ?? "";
}

function resolveTtsRuntimeConfig(cfg: TheClawConfig): TheClawConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: TheClawConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredTheClawTmpDir();
