...
} from "theclaw/plugin-sdk/speech-core";
...
vi.mock("theclaw/plugin-sdk/channel-targets", () => ({
...
function createTtsConfig(prefsName: string): TheClawConfig {
  return {
    messages: {
      tts: {
        enabled: true,
...
describe("speech-core native voice-note routing", () => {
  ...
  it("marks Discord auto TTS replies as native voice messages", async () => {
    await expectTtsPayloadResult({
      channel: "discord",
      prefsName: "theclaw-speech-core-tts-test",
  ...
  it("keeps BlueBubbles synthesis on mp3 audio-file output but delivers it as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "bluebubbles",
      prefsName: "theclaw-speech-core-tts-bluebubbles-mp3-test",
  ...
  it("does not mark unsupported BlueBubbles audio-file output as a voice memo", async () => {
    await expectTtsPayloadResult({
      channel: "bluebubbles",
      prefsName: "theclaw-speech-core-tts-bluebubbles-ogg-test",
  ...
  it("pre-transcodes BlueBubbles synthesized mp3 to opus-in-CAF when the host can satisfy preferAudioFileFormat", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "bluebubbles",
      prefsName: "theclaw-speech-core-tts-bluebubbles-caf-transcode-test",
  ...
  it("falls back to the original mp3 buffer when the host transcoder fails", async () => {
    ...
    await expectTtsPayloadResult({
      channel: "bluebubbles",
      prefsName: "theclaw-speech-core-tts-bluebubbles-caf-fallback-test",
  ...
  it.each(["feishu", "whatsapp"] as const)(
    ...
    async (channel) => {
      ...
      await expectTtsPayloadResult({
        channel,
        prefsName: `theclaw-speech-core-tts-${channel}-mp3-test`,
  ...
  it("keeps non-native voice-note channels as regular audio files", async () => {
    await expectTtsPayloadResult({
      channel: "slack",
      prefsName: "theclaw-speech-core-tts-slack-test",
  ...
  it("selects persona preferred provider before config fallback", () => {
    ...
    const prefsPath = "/tmp/theclaw-speech-core-persona-provider.json";
    ...
    const cfg: TheClawConfig = {
      messages: {
        tts: {
          enabled: true,
          provider: "mock",
          prefsPath: "/tmp/theclaw-speech-core-persona-merge.json",
          providers: {
            mock: {
              model: "base-model",
              voice: "base-voice",
