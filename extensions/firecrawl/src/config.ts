...
import { canResolveEnvSecretRefInReadOnlyPath } from "theclaw/plugin-sdk/extension-shared";
import { resolveSecretInputString, normalizeSecretInput } from "theclaw/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<TheClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<TheClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: TheClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: TheClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: TheClawConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: TheClawConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: TheClawConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: TheClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: TheClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: TheClawConfig, override?: number): number {
