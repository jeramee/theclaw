...
Read-only diff viewer plugin for **TheClaw** agents.
...
Set plugin-wide defaults in `~/.theclaw/theclaw.json`:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/theclaw",
        },
      },
    },
  },
...
Before:
TheClaw supports plugins.

After:
TheClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/theclaw-diffs`).
