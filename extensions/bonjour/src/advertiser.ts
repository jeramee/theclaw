...
import type { PluginLogger } from "theclaw/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "theclaw/plugin-sdk/runtime-env";
...
// See https://github.com/theclaw/theclaw/issues/72481
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.THECLAW_DISABLE_BONJOUR;
...
function truncateToDnsLabel(name: string, fallback = "TheClaw"): string {
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? truncateToDnsLabel(trimmed) : "TheClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(TheClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.THECLAW_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "theclaw";
    ...
    const recreateAdvertiser = async (reason: string) => {
      ...
      recreatePromise = (async () => {
        ...
        if (consecutiveRestarts > MAX_CONSECUTIVE_RESTARTS) {
          disabled = true;
          logger.warn(
            `bonjour: disabling advertiser after ${MAX_CONSECUTIVE_RESTARTS} failed restarts (${reason}); set discovery.mdns.mode="off" or THECLAW_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
