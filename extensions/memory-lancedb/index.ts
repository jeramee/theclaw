...
} from "theclaw/plugin-sdk/memory-core-host-engine-embeddings";
import { resolveDefaultAgentId } from "theclaw/plugin-sdk/memory-host-core";
import { resolveLivePluginConfigObject } from "theclaw/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "theclaw/plugin-sdk/runtime-env";
import {
  normalizeLowercaseStringOrEmpty,
  truncateUtf16Safe,
} from "theclaw/plugin-sdk/text-runtime";
import { Type } from "typebox";
import { definePluginEntry, type TheClawPluginApi } from "./api.js";
import {
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as TheClawConfig;
...
function createEmbeddings(api: TheClawPluginApi, cfg: MemoryConfig): Embeddings {
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as TheClawConfig
          : undefined,
        "memory-lancedb",
