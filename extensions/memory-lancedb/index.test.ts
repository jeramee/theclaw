...
const TEST_RUNTIME_MANIFEST = {
  name: "theclaw-memory-lancedb-runtime",
...
function createRuntimeLoader(
  ...
  return createLanceDbRuntimeLoader({
    ...
    arch: overrides.arch,
    resolveStateDir: () => "/tmp/theclaw-state",
...
describe("memory plugin e2e", () => {
  const { getDbPath } = installTmpDirHarness({ prefix: "theclaw-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("theclaw/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      const mockApi = {
        ...
        runtime: {
          ...
          agent: {
            resolveAgentDir: vi.fn(() => "/tmp/theclaw-agent"),
          },
        },
        logger: {
          info: vi.fn(),
      ...
      vi.doUnmock("theclaw/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
  ...
  test("runs auto-recall through the registered before_prompt_build hook", async () => {
    ...
    vi.resetModules();
    vi.doMock("theclaw/plugin-sdk/runtime-env", () => ({
    ...
    try {
      ...
      vi.doUnmock("theclaw/plugin-sdk/runtime-env");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
...
describe("lancedb runtime loader", () => {
  test("uses the bundled module when it is already available", async () => {
    ...
    const installRuntime = vi.fn(async () => "/tmp/theclaw-state/plugin-runtimes/lancedb.js");
  ...
  test("reuses an existing user runtime install before attempting a reinstall", async () => {
    ...
    const resolveRuntimeEntry = vi.fn(
      () => "/tmp/theclaw-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const installRuntime = vi.fn(
      async () => "/tmp/theclaw-state/plugin-runtimes/memory-lancedb/runtime-entry.js",
    );
    const loader = createRuntimeLoader({
      importResolved,
      resolveRuntimeEntry,
    ...
    expect(resolveRuntimeEntry).toHaveBeenCalledWith(
      expect.objectContaining({
        runtimeDir: "/tmp/theclaw-state/plugin-runtimes/memory-lancedb/lancedb",
      }),
    );
    expect(installRuntime).not.toHaveBeenCalled();
  });
  ...
  test("installs LanceDB into user state when the bundled runtime is unavailable", async () => {
    ...
    expect(logger.warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "installing runtime deps under /tmp/theclaw-state/plugin-runtimes/memory-lancedb/lancedb",
      ),
    );
  });
  ...
  test("fails fast in nix mode instead of attempting auto-install", async () => {
    ...
    const loader = createRuntimeLoader({
      env: { THECLAW_NIX_MODE: "1" } as NodeJS.ProcessEnv,
      installRuntime,
    });
  ...
  test("clears the cached failure so later calls can retry the install", async () => {
    ...
    const installRuntime = vi
      ...
      .mockResolvedValueOnce(
        "/tmp/theclaw-state/plugin-runtimes/memory-lancedb/lancedb/node_modules/@lancedb/lancedb/index.js",
      );
    const importResolved = vi.fn(async () => runtimeModule);
    const loader = createRuntimeLoader({
      installRuntime,
