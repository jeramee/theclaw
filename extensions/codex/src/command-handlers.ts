import type { PluginCommandContext, PluginCommandResult } from "theclaw/plugin-sdk/plugin-entry";
...
async function bindConversation(
  ...
  if (!ctx.sessionFile) {
    return {
      text: "Cannot bind Codex because this command did not include an TheClaw session file.",
    };
  }
...
async function resumeThread(
  ...
  if (!ctx.sessionFile) {
    return "Cannot attach a Codex thread because this command did not include an TheClaw session file.";
  }
  ...
  return `Attached this TheClaw session to Codex thread ${effectiveThreadId}.`;
}
...
async function stopConversationTurn(
  ...
  if (!sessionFile) {
    return "Cannot stop Codex because this command did not include an TheClaw session file.";
  }
...
async function steerConversationTurn(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot steer Codex because this command did not include an TheClaw session file.";
  }
  return (
...
async function setConversationModel(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex model because this command did not include an TheClaw session file.";
  }
...
async function setConversationFastMode(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex fast mode because this command did not include an TheClaw session file.";
  }
...
async function setConversationPermissions(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return "Cannot set Codex permissions because this command did not include an TheClaw session file.";
  }
...
async function startThreadAction(
  ...
  const sessionFile = await resolveControlSessionFile(ctx);
  if (!sessionFile) {
    return `Cannot start Codex ${label} because this command did not include an TheClaw session file.`;
  }
  const binding = await deps.readCodexAppServerBinding(sessionFile);
  if (!binding?.threadId) {
    return `No Codex thread is attached to this TheClaw session yet.`;
  }
