...
} from "theclaw/plugin-sdk/agent-harness";
import {
...
import { createMockPluginRegistry } from "theclaw/plugin-sdk/plugin-test-runtime";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/theclaw-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt", () => {
  beforeEach(async () => {
    resetAgentEventsForTest();
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-codex-run-"));
  });
  ...
  it("releases completion when a projector callback throws during turn/completed", async () => {
    // Regression for theclaw/theclaw#67996: a throw inside the projector's
  ...
  it("builds resume and turn params from the currently selected TheClaw model", () => {
