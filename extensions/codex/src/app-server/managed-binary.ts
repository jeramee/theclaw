...
async function findManagedCodexAppServerCommandPath(params: {
  ...
  throw new Error(
    [
      ...
      "Run TheClaw with bundled plugin runtime dependencies enabled, or run pnpm install in a source checkout.",
      "Set plugins.entries.codex.config.appServer.command or THECLAW_CODEX_APP_SERVER_BIN to use a custom Codex binary.",
    ].join(" "),
  );
}
