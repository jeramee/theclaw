...
import { embeddedAgentLog, THECLAW_VERSION } from "theclaw/plugin-sdk/agent-harness-runtime";
...
export function defaultServerRequestResponse(
  ...
  if (request.method === "item/tool/call") {
    return {
      contentItems: [
        {
          type: "inputText",
          text: "TheClaw did not register a handler for this app-server tool call.",
        },
      ],
      success: false,
    };
  ...
  if (isCodexAppServerApprovalRequest(request.method)) {
    return {
      decision: "decline",
      reason: "TheClaw codex app-server bridge does not grant native approvals yet.",
    };
  }
...
function assertSupportedCodexAppServerVersion(response: CodexInitializeResponse): void {
  ...
  if (!detectedVersion) {
    throw new Error(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but TheClaw could not determine the running Codex version. Update the configured Codex app-server binary, or remove custom command overrides to use the managed binary.`,
    );
  }
...
export function readCodexVersionFromUserAgent(userAgent: string | undefined): string | undefined {
  ...
  // TheClaw, Codex Desktop, or an env override, so only the slash-delimited
