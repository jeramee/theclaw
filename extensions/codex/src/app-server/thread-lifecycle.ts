...
export function buildDeveloperInstructions(params: EmbeddedRunAttemptParams): string {
  ...
  const sections = [
    "You are running inside TheClaw. Use TheClaw dynamic tools for messaging, cron, sessions, and host actions when available.",
    "Preserve the user's existing channel/session context. If sending a channel reply, use the TheClaw messaging tool instead of describing that you would reply.",
    promptOverlay,
...
function resolveCodexAppServerModelProvider(provider: string): string | undefined {
  ...
  if (!normalized || normalized === "codex") {
    // `codex` is TheClaw's virtual provider; let Codex app-server keep its
