import type { AnyAgentTool } from "theclaw/plugin-sdk/agent-harness";
import { wrapToolWithBeforeToolCallHook } from "theclaw/plugin-sdk/agent-harness-runtime";
import {
  installCodexToolResultMiddleware,
  installTheClawOwnedToolHooks,
  mediaToolResult,
  resetTheClawOwnedToolHooks,
  textToolResult,
...
describe("TheClaw-owned tool runtime contract — Codex app-server adapter", () => {
  afterEach(() => {
    resetTheClawOwnedToolHooks();
  });
  ...
  it("wraps unwrapped dynamic tools with before/after tool hooks", async () => {
    ...
    const hooks = installTheClawOwnedToolHooks({ adjustedParams });
  ...
  it("fails closed when before_tool_call blocks a dynamic tool", async () => {
    const hooks = installTheClawOwnedToolHooks({ blockReason: "blocked by policy" });
  ...
  it("records successful Codex messaging text, media, and target telemetry", async () => {
    const hooks = installTheClawOwnedToolHooks();
