...
import { upsertAuthProfile } from "theclaw/plugin-sdk/provider-auth";
...
vi.mock("theclaw/plugin-sdk/agent-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("theclaw/plugin-sdk/agent-runtime")>();
  return {
    ...actual,
...
describe("bridgeCodexAppServerStartOptions", () => {
  it("clears inherited API-key env vars when the default Codex profile is subscription auth", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-codex-app-server-"));
