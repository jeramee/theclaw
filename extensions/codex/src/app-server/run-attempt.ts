...
type TheClawCodingToolsOptions = NonNullable<
  Parameters<(typeof import("theclaw/plugin-sdk/agent-harness"))["createTheClawCodingTools"]>[0]
>;
...
async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const agentDir = params.agentDir ?? resolveTheClawAgentDir();
  const { createTheClawCodingTools } = await import("theclaw/plugin-sdk/agent-harness");
  const allTools = createTheClawCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as TheClawCodingToolsOptions["modelCompat"])
        : undefined,
