...
import { resolveUserPath } from "theclaw/plugin-sdk/agent-harness";
...
function parseTrajectoryEnabled(env: NodeJS.ProcessEnv): boolean {
  const value = env.THECLAW_TRAJECTORY?.trim().toLowerCase();
...
function resolveTrajectoryFilePath(params: {
  ...
  const dirOverride = params.env.THECLAW_TRAJECTORY_DIR?.trim();
  if (dirOverride) {
