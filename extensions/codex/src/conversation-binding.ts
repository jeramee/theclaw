import { formatErrorMessage } from "theclaw/plugin-sdk/agent-harness-runtime";
import type {
...
const CODEX_CONVERSATION_GLOBAL_STATE = Symbol.for("theclaw.codex.conversationBinding");
  ...
  const response: CodexThreadStartResponse = await client.request(
    "thread/start",
    {
      ...
      developerInstructions:
        "This Codex thread is bound to an TheClaw conversation. Answer normally; TheClaw will deliver your final response back to the conversation.",
    ...
    async (request): Promise<JsonValue | undefined> => {
      if (request.method === "item/tool/call") {
        return {
          contentItems: [
            {
              type: "inputText",
              text: "TheClaw native Codex conversation binding does not expose dynamic TheClaw tools yet.",
            },
          ],
          success: false,
        };
      ...
      if (
        ...
        return {
          decision: "decline",
          reason:
            "TheClaw native Codex conversation binding cannot route interactive approvals yet; use the Codex harness or explicit /acp spawn codex for that workflow.",
        };
      }
