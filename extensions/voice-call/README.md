...
Official Voice Call plugin for **TheClaw**.
...
Docs: `https://docs.theclaw.ai/plugins/voice-call`
Plugin system: `https://docs.theclaw.ai/plugin`

## Install (local dev)

### Option A: install via TheClaw (recommended)
...
```bash
PLUGIN_HOME=~/.theclaw/extensions
mkdir -p "$PLUGIN_HOME"
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `theclaw doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.theclaw.ai/plugins/voice-call`
...
`https://docs.theclaw.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.theclaw.ai/plugins/voice-call#tts-for-calls`
