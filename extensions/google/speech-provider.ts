import { transcodeAudioBufferToOpus } from "theclaw/plugin-sdk/media-runtime";
import {
  assertOkOrThrowProviderError,
  postJsonRequest,
  sanitizeConfiguredModelProviderRequest,
} from "theclaw/plugin-sdk/provider-http";
import type { TheClawConfig } from "theclaw/plugin-sdk/provider-onboard";
...
import { asObject, trimToUndefined } from "theclaw/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: TheClawConfig): string | undefined {
...
function isTheClawGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isTheClawGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
