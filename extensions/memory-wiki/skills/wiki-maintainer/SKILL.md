...
description: Maintain the TheClaw memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `theclaw wiki ingest`, `theclaw wiki compile`, and `theclaw wiki lint` as the default maintenance loop.
- In `bridge` mode, run `theclaw wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `theclaw wiki unsafe-local import` only when the user explicitly opted into private local path access.
