...
import {
  ...
  withTrailingNewline,
} from "theclaw/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".theclaw-wiki/cache/agent-digest.json";
const CLAIMS_DIGEST_PATH = ".theclaw-wiki/cache/claims.jsonl";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- theclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- theclaw:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- theclaw:wiki:index:start -->",
      endMarker: "<!-- theclaw:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        filePath,
        title: group.heading,
        startMarker: `<!-- theclaw:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- theclaw:wiki:${group.dir}:index:end -->`,
