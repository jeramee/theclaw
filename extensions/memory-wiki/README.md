...
Persistent wiki compiler and Obsidian-friendly knowledge vault for **TheClaw**.
...
Key beliefs can live in structured `claims` frontmatter with per-claim evidence, confidence, and status. Compile also emits machine-readable digests under `.theclaw-wiki/cache/` so agent/runtime consumers do not have to scrape markdown pages.
...
theclaw wiki doctor
theclaw wiki init
theclaw wiki ingest ./notes/alpha.md
theclaw wiki compile
theclaw wiki lint
theclaw wiki search "alpha"
theclaw wiki get entity.alpha --from 1 --lines 80

theclaw wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

theclaw wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

theclaw wiki bridge import
theclaw wiki unsafe-local import

theclaw wiki obsidian status
theclaw wiki obsidian search "alpha"
theclaw wiki obsidian open syntheses/alpha-summary.md
theclaw wiki obsidian command workspace:quick-switcher
theclaw wiki obsidian daily
```
...
- The compiled agent digests in `.theclaw-wiki/cache/agent-digest.json` and `.theclaw-wiki/cache/claims.jsonl` are the stable machine-facing view of the wiki.
