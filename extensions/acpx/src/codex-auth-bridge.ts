...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--theclaw-run-configured";
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep TheClaw's isolated CODEX_HOME wrapper, but launch the plugin-local
...
if (!command) {
  console.error("[theclaw] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[theclaw] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in TheClaw; fallback must not float to an unpatched newer release.
...
async function prepareIsolatedCodexHome(baseDir: string): Promise<string> {
  ...
  await fs.writeFile(
    ...
    "# Generated by TheClaw for Codex ACP sessions.\n",
    "utf8",
  );
  return codexHome;
}
