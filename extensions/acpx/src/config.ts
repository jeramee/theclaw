...
import { formatPluginConfigIssue } from "theclaw/plugin-sdk/extension-shared";
...
export const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "theclaw-plugin-tools";
export const ACPX_THECLAW_TOOLS_MCP_SERVER_NAME = "theclaw-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "theclaw.plugin.json")) &&
...
function resolveAcpxPluginRootFromTheClawLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common TheClaw layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromTheClawLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveTheClawRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
...
export function resolvePluginToolsMcpServerConfig(
  ...
  const openClawRoot = resolveTheClawRoot(pluginRoot);
...
export function resolveTheClawToolsMcpServerConfig(
  moduleUrl: string = import.meta.url,
): McpServerConfig {
  const pluginRoot = resolveAcpxPluginRoot(moduleUrl);
  const openClawRoot = resolveTheClawRoot(pluginRoot);
  const distEntry = path.join(openClawRoot, "dist", "mcp", "theclaw-tools-serve.js");
  if (fs.existsSync(distEntry)) {
    return {
      command: process.execPath,
      args: [distEntry],
  ...
  const sourceEntry = path.join(openClawRoot, "src", "mcp", "theclaw-tools-serve.ts");
  return {
    command: process.execPath,
    args: ["--import", resolveTsxImportSpecifier(), sourceEntry],
  };
...
function resolveConfiguredMcpServers(params: {
  ...
  if (params.openClawToolsMcpBridge && resolved[ACPX_THECLAW_TOOLS_MCP_SERVER_NAME]) {
    throw new Error(
      `mcpServers.${ACPX_THECLAW_TOOLS_MCP_SERVER_NAME} is reserved when openClawToolsMcpBridge=true`,
    );
  }
  ...
  if (params.openClawToolsMcpBridge) {
    resolved[ACPX_THECLAW_TOOLS_MCP_SERVER_NAME] = resolveTheClawToolsMcpServerConfig(
      params.moduleUrl,
    );
  }
  return resolved;
