...
const ENABLE_STARTUP_PROBE_ENV = "THECLAW_ACPX_RUNTIME_STARTUP_PROBE";
...
function resolveAllowedAgentsProbeAgent(ctx: TheClawPluginServiceContext): string | undefined {
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: TheClawPluginServiceContext): Promise<void> {
      if (process.env.THECLAW_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (THECLAW_SKIP_ACPX_RUNTIME=1)");
        return;
      }
      ...
      if (!shouldRunStartupProbe() || process.env.THECLAW_SKIP_ACPX_RUNTIME_PROBE === "1") {
        return;
      }
    ...
    async stop(_ctx: TheClawPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
