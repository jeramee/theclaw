...
The bundled ACPX extension is a thin TheClaw wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `theclaw/acpx`, not in this extension.
...
Use this flow when TheClaw needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `theclaw/acpx` repo first.
2. In TheClaw, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild TheClaw and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for TheClaw ACP validation.
...
If a change feels like shared ACP runtime behavior instead of TheClaw-specific glue, move it to `theclaw/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
