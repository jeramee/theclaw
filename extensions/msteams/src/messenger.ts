...
import { normalizeOptionalLowercaseString, sleep } from "theclaw/plugin-sdk/text-runtime";
import { loadWebMedia } from "theclaw/plugin-sdk/web-media";
import type { MarkdownTableMode, MSTeamsReplyStyle, TheClawConfig } from "../runtime-api.js";
...
export function renderReplyPayloadsToMessages(
  ...
  const tableMode =
    ...
    getMSTeamsRuntime().channel.text.resolveMarkdownTableMode({
      cfg: getMSTeamsRuntime().config.current() as TheClawConfig,
      channel: "msteams",
    });
