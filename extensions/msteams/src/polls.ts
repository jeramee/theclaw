...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["theclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["theclaw", "pollId"]) ??
    readNestedString(value, ["theclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "theclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "theclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          theclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "theclaw poll vote",
          displayText: "Vote recorded",
          value: { theclawPollId: pollId, pollId },
        },
      },
    ],
  };
