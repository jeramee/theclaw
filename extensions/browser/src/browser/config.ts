...
export type ResolvedBrowserProfile = {
  ...
  mcpArgs?: string[];
  color: string;
  driver: "theclaw" | "existing-session";
  executablePath?: string;
  headless: boolean;
...
export const THECLAW_BROWSER_HEADLESS_ENV = "THECLAW_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_THECLAW_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_THECLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_THECLAW_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  cfg: BrowserConfig | undefined,
  rootConfig?: TheClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_THECLAW_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_THECLAW_BROWSER_PROFILE_NAME]
        ? DEFAULT_THECLAW_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "existing-session" ? "existing-session" : "theclaw";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[THECLAW_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  const sourceHint =
    mode.source === "request"
      ? "request override"
      : mode.source === "env"
        ? `${THECLAW_BROWSER_HEADLESS_ENV}=0`
        : mode.source === "profile"
  ...
  return (
    ...
    `Set ${THECLAW_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
