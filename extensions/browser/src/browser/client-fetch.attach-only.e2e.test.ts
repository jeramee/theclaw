...
import { clearRuntimeConfigSnapshot } from "theclaw/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("theclaw-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".theclaw", "theclaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to TheClaw");
      expect(message).toContain("Restarting the TheClaw gateway will not launch it");
      expect(message).not.toContain("Restart the TheClaw gateway");
