...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".theclaw-profile-decorated");
}
 ...
 */
export function decorateTheClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_THECLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_THECLAW_BROWSER_COLOR).toUpperCase();
