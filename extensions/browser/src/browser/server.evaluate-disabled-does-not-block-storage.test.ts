...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        theclaw: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.THECLAW_GATEWAY_PORT;
    process.env.THECLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.THECLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.THECLAW_GATEWAY_PASSWORD;
