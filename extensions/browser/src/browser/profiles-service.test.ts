...
import { resolveTheClawUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn<(cfg: TheClawConfig) => Promise<void>>(async (_cfg) => {}),
}));
...
vi.mock("./chrome.js", () => ({
  resolveTheClawUserDataDir: vi.fn(() => "/tmp/theclaw-test/theclaw/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "theclaw-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveTheClawUserDataDir).mockReturnValue(userDataDir);
