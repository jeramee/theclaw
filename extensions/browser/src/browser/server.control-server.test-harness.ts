...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.THECLAW_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "theclaw",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/theclaw" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    theclaw: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchTheClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveTheClawUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopTheClawChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgDefaultProfile = "theclaw";
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.THECLAW_GATEWAY_PORT;
  process.env.THECLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.THECLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.THECLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.THECLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.THECLAW_GATEWAY_PASSWORD;
  } else {
    process.env.THECLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
