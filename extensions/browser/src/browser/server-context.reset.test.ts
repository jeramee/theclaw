...
function localTheClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "theclaw",
...
function createLocalTheClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localTheClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveTheClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-reset-"));
    const profileDir = path.join(tempRoot, "theclaw");
    ...
    const ops = createLocalTheClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveTheClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "theclaw");
    fs.mkdirSync(profileDir, { recursive: true });
