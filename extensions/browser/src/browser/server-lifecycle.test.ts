...
const { stopTheClawChromeMock } = vi.hoisted(() => ({
  stopTheClawChromeMock: vi.fn(async () => {}),
}));
...
vi.mock("./chrome.js", () => ({
  stopTheClawChrome: stopTheClawChromeMock,
}));
...
beforeEach(() => {
  ...
  stopTheClawChromeMock.mockClear();
});
...
describe("stopKnownBrowserProfiles", () => {
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["theclaw", "user"]);
    ...
    expect(stopMap.theclaw).toHaveBeenCalledTimes(1);
  ...
  it("stops tracked runtime browsers even when the profile no longer resolves", async () => {
    ...
    expect(stopTheClawChromeMock).toHaveBeenCalledWith(launchedBrowser);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("theclaw browser stop failed: Error: oops");
  });
});
