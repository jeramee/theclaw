...
const resolvePreferredTheClawTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/theclaw"));
const THECLAW_TMP_ROOT = "/tmp/theclaw";
const TRASH_SOURCE = `${THECLAW_TMP_ROOT}/demo`;
...
describe("browser trash", () => {
  beforeEach(() => {
    ...
    resolvePreferredTheClawTmpDirMock.mockReturnValue("/tmp/theclaw");
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    await expect(movePathToTrash("/etc/theclaw-demo")).rejects.toThrow(
