...
vi.mock("theclaw/plugin-sdk/ssrf-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/ssrf-runtime")>(
    "theclaw/plugin-sdk/ssrf-runtime",
  );
  return {
    ...actual,
...
const mocks = vi.hoisted(() => ({
  loadConfig: vi.fn<() => TheClawConfig>(() => ({
    gateway: {
      auth: {
...
describe("fetchBrowserJson loopback auth", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("THECLAW_GATEWAY_TOKEN", "loopback-token");
  ...
  it("preserves dispatcher timeout context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["Chrome CDP handshake timeout", "Restart the TheClaw gateway"],
      omits: ["Can't reach the TheClaw browser control service", "Do NOT retry the browser tool"],
    });
  });
  ...
  it("preserves dispatcher abort context without no-retry hint", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      contains: ["operation aborted", "Restart the TheClaw gateway"],
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        contains: [
          ...
          "browser profile is external to TheClaw",
          "Restarting the TheClaw gateway will not launch it",
        ],
        omits: ["Restart the TheClaw gateway", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps restart-gateway guidance for managed local dispatcher timeouts", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=theclaw"),
      {
        contains: ["Chrome CDP handshake timeout", "Restart the TheClaw gateway"],
        omits: ["browser profile is external to TheClaw", "Do NOT retry the browser tool"],
      },
    );
  });
  ...
  it("keeps no-retry hint but not restart guidance for persistent external profile failures", async () => {
    ...
    await expectThrownBrowserFetchError(
      () => fetchBrowserJson<{ ok: boolean }>("/tabs?profile=manual"),
      {
        ...
        omits: ["Restart the TheClaw gateway"],
      },
    );
  });
  ...
  it("keeps no-retry hint for persistent dispatcher failures", async () => {
    ...
    await expectThrownBrowserFetchError(() => fetchBrowserJson<{ ok: boolean }>("/tabs"), {
      ...
      omits: ["Can't reach the TheClaw browser control service"],
    });
  });
  ...
  it("keeps absolute URL failures wrapped as reachability errors", async () => {
    ...
    await expectThrownBrowserFetchError(
      ...
      {
        contains: [
          "Can't reach the TheClaw browser control service",
          "Do NOT retry the browser tool",
        ],
      },
    );
