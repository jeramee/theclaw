...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredTheClawTmpDir: vi.fn(() => "/tmp/theclaw"),
}));
...
let tmpDirModule: typeof import("../infra/tmp-theclaw-dir.js");
...
describe("pw-tools-core", () => {
  ...
  beforeAll(async () => {
    ...
    tmpDirModule = await import("../infra/tmp-theclaw-dir.js");
    vi.spyOn(tmpDirModule, "resolvePreferredTheClawTmpDir").mockImplementation(
      tmpDirMocks.resolvePreferredTheClawTmpDir,
    );
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredTheClawTmpDir.mockReturnValue("/tmp/theclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".theclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredTheClawTmpDir.mockReturnValue("/tmp/theclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "theclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "theclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredTheClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(outPath)).toBe(
      path.resolve(path.join(path.sep, "tmp", "theclaw-preferred", "downloads")),
    );
    expect(path.basename(outPath)).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "theclaw-preferred", "downloads")}${path.sep}`),
    );
  });
