...
const mocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => TheClawConfig>(),
  ...
  resolveGatewayAuth: vi.fn(
    ({
      ...
      authConfig?: NonNullable<NonNullable<TheClawConfig["gateway"]>["auth"]>;
    }) => {
      const token =
  ...
  ensureGatewayStartupAuth: vi.fn(async ({ cfg }: { cfg: TheClawConfig }) => ({
    cfg: {
      ...cfg,
      gateway: {
        ...cfg.gateway,
...
function readPersistedConfig(): TheClawConfig {
...
async function expectUnresolvedBrowserSecretRefSkipsPersistence(cfg: TheClawConfig) {
