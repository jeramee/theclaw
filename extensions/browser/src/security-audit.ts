import type { TheClawPluginSecurityAuditContext } from "theclaw/plugin-sdk/plugin-entry";
import { hasConfiguredSecretInput } from "theclaw/plugin-sdk/secret-input";
import { formatCliCommand } from "theclaw/plugin-sdk/setup-tools";
import { isPrivateNetworkOptInEnabled, isPrivateIpAddress } from "theclaw/plugin-sdk/ssrf-policy";
...
export function collectBrowserSecurityAuditFindings(ctx: TheClawPluginSecurityAuditContext) {
  ...
  try {
    ...
    findings.push({
      ...
      detail: String(err),
      remediation: `Fix browser.cdpUrl in ${ctx.configPath} and re-run "${formatCliCommand("theclaw security audit --deep")}".`,
    });
    return findings;
  }
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(ctx.env.THECLAW_GATEWAY_TOKEN) ||
  ...
  const passwordConfigured =
    ...
    (passwordCanWin &&
      (hasNonEmptyString(ctx.env.THECLAW_GATEWAY_PASSWORD) ||
