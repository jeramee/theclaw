...
import type { TheClawPluginApi } from "./runtime-api.js";
import setupPlugin from "./setup-api.js";

type BrowserAutoEnableProbe = Parameters<TheClawPluginApi["registerAutoEnableProbe"]>[0];
...
describe("browser plugin", () => {
  ...
  it("bundles the browser automation skill with the plugin", () => {
    const manifest = JSON.parse(
      fs.readFileSync(path.join(__dirname, "theclaw.plugin.json"), "utf8"),
    ) as { skills?: string[] };
  ...
  it("lazy-loads the browser service on start", async () => {
    ...
    await service.start({ config: {}, stateDir: "/tmp/theclaw", logger: { warn: vi.fn() } });
  ...
  it("declares setup auto-enable reasons for browser config surfaces", () => {
    ...
    expect(probe({ config: { browser: { defaultProfile: "theclaw" } }, env: {} })).toBe(
      "browser configured",
    );
    ...
    expect(
      probe({ config: { browser: { defaultProfile: "theclaw", enabled: false } }, env: {} }),
    ).toBeNull();
  });
});
