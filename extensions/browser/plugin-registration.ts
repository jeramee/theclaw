...
const BROWSER_CLI_DESCRIPTOR = {
  name: "browser",
  description: "Manage TheClaw's dedicated browser (Chrome/Chromium)",
  hasSubcommands: true,
};
...
function createLazyBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via TheClaw's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      "Browser choice: omit profile by default for the isolated TheClaw-managed browser (`theclaw`).",
...
export const browserPluginNodeHostCommands: TheClawPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: TheClawPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): TheClawPluginService {
  let service: TheClawPluginService | null = null;
...
export function registerBrowserPlugin(api: TheClawPluginApi) {
  api.registerTool(((ctx: TheClawPluginToolContext) =>
    createLazyBrowserTool({
      sandboxBridgeUrl: ctx.browser?.sandboxBridgeUrl,
      allowHostControl: ctx.browser?.allowHostControl,
      agentSessionKey: ctx.sessionKey,
    })) as TheClawPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
