/**
 * Twitch channel plugin for TheClaw.
 *
...
} from "theclaw/plugin-sdk/channel-pairing";
import type { TheClawConfig } from "theclaw/plugin-sdk/config-types";
import { buildPassiveProbedChannelStatusSummary } from "theclaw/plugin-sdk/extension-shared";
import {
 ...
 * for TheClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: TheClawConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: TheClawConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: TheClawConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: TheClawConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("theclaw/plugin-sdk/runtime-env").RuntimeEnv;
