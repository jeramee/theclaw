...
import { normalizeOptionalAccountId } from "theclaw/plugin-sdk/account-id";
import { getChatChannelMeta, type ChannelPlugin } from "theclaw/plugin-sdk/core";
import {
  formatDocsLink,
...
function resolveSetupAccountId(cfg: TheClawConfig, requestedAccountId?: string): string {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: THECLAW_TWITCH_ACCESS_TOKEN",
...
export async function configureWithEnvToken(
  ...
  accountId: string = resolveSetupAccountId(cfg),
): Promise<{ cfg: TheClawConfig } | null> {
  const resolvedAccountId = accountId.trim()
    ? normalizeRequestedSetupAccountId(accountId)
    : resolveSetupAccountId(cfg);
  ...
  const useEnv = await prompter.confirm({
    message: "Twitch env var THECLAW_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId: requestedAccountId, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.THECLAW_TWITCH_ACCESS_TOKEN?.trim();
