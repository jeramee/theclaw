import type { MockFn } from "theclaw/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/theclaw-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("theclaw/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("theclaw/plugin-sdk/runtime-config-snapshot")
  >("theclaw/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/session-store-runtime")>(
    "theclaw/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/reply-runtime")>(
...
vi.mock("theclaw/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/conversation-runtime")>(
    "theclaw/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/security-runtime")>(
    "theclaw/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("theclaw/plugin-sdk/system-event-runtime")>(
    "theclaw/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/transport-ready-runtime", () => ({
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("theclaw/plugin-sdk/reply-runtime"),
      import("theclaw/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
