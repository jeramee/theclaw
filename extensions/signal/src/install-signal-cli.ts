...
import { runPluginCommandWithTimeout } from "theclaw/plugin-sdk/run-command";
import type { RuntimeEnv } from "theclaw/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "theclaw/plugin-sdk/setup-tools";
...
async function installSignalCliFromRelease(runtime: RuntimeEnv): Promise<SignalInstallResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredTheClawTmpDir(), "theclaw-signal-"));
