...
import type { RealtimeVoiceProviderPlugin } from "theclaw/plugin-sdk/realtime-voice";
...
describe("google-meet plugin", () => {
  ...
  it("defaults to chrome realtime with safe read-only tools", () => {
    ...
    expect(resolveGoogleMeetConfig({}).realtime.instructions).toContain("theclaw_agent_consult");
  });
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    expect(
      resolveGoogleMeetConfigWithEnv(
        {},
        {
          THECLAW_GOOGLE_MEET_CLIENT_ID: "client-id",
          GOOGLE_MEET_CLIENT_SECRET: "client-secret",
          THECLAW_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
          GOOGLE_MEET_ACCESS_TOKEN: "access-token",
          THECLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
          GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
          THECLAW_GOOGLE_MEET_PREVIEW_ACK: "true",
        },
      ),
    ).toMatchObject({
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["theclaw-audio-bridge", "capture"],
          audioOutputCommand: ["theclaw-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "theclaw-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "theclaw-google-meet-tool-dry-run-"));
              ...
              payload: {
                result: {
                  result: JSON.stringify({
                    ...
                    manualActionMessage: "Admit the TheClaw browser participant in Google Meet.",
                    title: "Meet",
                    url: "https://meet.google.com/abc-defg-hij?authuser=me@example.com",
                  }),
                },
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the TheClaw browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("theclaw node run");
  });
