...
vi.mock("theclaw/plugin-sdk/system-event-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("theclaw/plugin-sdk/system-event-runtime")>();
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/approval-gateway-runtime", () => ({
  resolveApprovalOverGateway: (arg: unknown) => resolveApprovalOverGatewayMock(arg),
}));

vi.mock("theclaw/plugin-sdk/security-runtime", () => ({
...
describe("registerSlackInteractionEvents", () => {
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "theclaw:verify" }],
            },
          ],
        },
      },
  ...
  it("registers a matcher that accepts plugin action ids beyond the TheClaw prefix", () => {
    const { ctx, getActionMatcher } = createContext();
    registerSlackInteractionEvents({ ctx: ctx as never });

    const matcher = getActionMatcher();
    expect(matcher).toBeTruthy();
    expect(matcher?.test("theclaw:verify")).toBe(true);
  ...
  it("treats Slack reply buttons as plain interaction events instead of plugin dispatch", async () => {
    ...
    await handler!({
          ...
          blocks: [
            {
              ...
              elements: [{ type: "button", action_id: "theclaw:reply_button" }],
            },
          ],
        },
      },
      action: {
        type: "button",
        action_id: "theclaw:reply_button",
        block_id: "reply_actions",
        value: "codex",
    ...
    expect(enqueueSystemEventMock).toHaveBeenCalledWith(
      expect.stringContaining('"actionId":"theclaw:reply_button"'),
      expect.any(Object),
    );
    expect(app.client.chat.update).toHaveBeenCalledTimes(1);
  });
  ...
  it("drops modal lifecycle payloads when mismatch guard triggers", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V123",
          callback_id: "theclaw:deploy_form",
          ...
          blocks: [
            {
              ...
              elements: [{ type: "multi_static_select", action_id: "theclaw:multi" }],
            },
          ],
        },
      },
          ...
          blocks: [
            {
              type: "actions",
              block_id: "date_block",
              elements: [{ type: "datepicker", action_id: "theclaw:date" }],
            },
            {
              type: "actions",
              block_id: "time_block",
              elements: [{ type: "timepicker", action_id: "theclaw:time" }],
            },
            {
              type: "actions",
              block_id: "datetime_block",
              elements: [{ type: "datetimepicker", action_id: "theclaw:datetime" }],
            },
          ],
        },
      },
  ...
  it("captures modal submissions and enqueues view submission event", async () => {
    ...
    expect(payload).toMatchObject({
      ...
      actionId: "view:theclaw:deploy_form",
      callbackId: "theclaw:deploy_form",
      viewId: "V123",
      userId: "U777",
  ...
  it("keeps no-channel modal events open when allowFrom is unset", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V444",
          callback_id: "theclaw:routing_form",
            ...
            values: {
              ...
              email_block: {
                email_input: {
                  ...
                  value: "team@theclaw.ai",
                },
              },
              url_block: {
                url_input: {
                  type: "url_text_input",
                  value: "https://docs.theclaw.ai",
                },
              },
              richtext_block: {
                richtext_input: {
    ...
    expect(payload.inputs).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          inputKind: "email",
          inputEmail: "team@theclaw.ai",
        }),
        expect.objectContaining({
          actionId: "url_input",
          inputKind: "url",
          inputUrl: "https://docs.theclaw.ai/",
        }),
        expect.objectContaining({
  ...
  it("truncates rich text preview to keep payload summaries compact", async () => {
    ...
    await viewHandler!({
      ...
      body: {
        ...
        view: {
          id: "V555",
          callback_id: "theclaw:long_richtext",
