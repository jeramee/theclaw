...
export const THECLAW_SLACK_CLIENT_PING_TIMEOUT_MS = 15_000;
...
export function shouldSkipTheClawSlackSelfEvent(args: SlackSelfFilterArgs): boolean {
...
export function createSlackBoltApp(params: {
  ...
  const socketModeReceiverOptions: SlackSocketModeReceiverOptions = {
    ...
    clientPingTimeout:
      params.socketMode?.clientPingTimeout ?? THECLAW_SLACK_CLIENT_PING_TIMEOUT_MS,
    installerOptions: {
  ...
  const app = new params.interop.App({
    ...
    // consumes that promise, tripping TheClaw's fatal unhandled-rejection path.
    tokenVerificationEnabled: false,
  });
  app.use(async (args) => {
    if (shouldSkipTheClawSlackSelfEvent(args)) {
      return;
    }
    await args.next();
  });
