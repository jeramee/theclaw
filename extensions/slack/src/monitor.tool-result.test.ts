import { CURRENT_MESSAGE_MARKER } from "theclaw/plugin-sdk/channel-mention-gating";
import { expectPairingReplyText } from "theclaw/plugin-sdk/channel-test-helpers";
import { resetInboundDedupe } from "theclaw/plugin-sdk/reply-dedupe";
import { HISTORY_CONTEXT_MARKER } from "theclaw/plugin-sdk/reply-history";
...
describe("monitorSlackProvider tool results", () => {
  ...
  async function expectMentionPatternMessageAccepted(text: string): Promise<void> {
    setRequireMentionChannelConfig(["\\btheclaw\\b"]);
  ...
  it("accepts channel messages when mentionPatterns match", async () => {
    await expectMentionPatternMessageAccepted("theclaw: hello");
  });

  it("accepts channel messages when mentionPatterns match even if another user is mentioned", async () => {
    await expectMentionPatternMessageAccepted("theclaw: hello <@U2>");
  });
