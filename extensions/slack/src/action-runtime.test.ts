...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): TheClawConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as TheClawConfig;
  ...
  async function resolveReadToken(cfg: TheClawConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: TheClawConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/theclaw-media/report.pdf",
    ...
    expect(result.content[0]).toEqual(
      expect.objectContaining({
        type: "text",
        text: expect.stringContaining("/tmp/theclaw-media/report.pdf"),
      }),
    );
    ...
    expect(result.details).toEqual(
      expect.objectContaining({
        ...
        contentType: "application/pdf",
        media: {
          mediaUrl: "/tmp/theclaw-media/report.pdf",
          contentType: "application/pdf",
        },
      }),
    );
