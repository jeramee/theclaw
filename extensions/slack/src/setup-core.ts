import { hasConfiguredSecretInput } from "theclaw/plugin-sdk/secret-input";
import {
  ...
  type TheClawConfig,
} from "theclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "theclaw/plugin-sdk/setup-tools";
import {
...
function enableSlackAccount(cfg: TheClawConfig, accountId: string): TheClawConfig {
...
function hasSlackInteractiveRepliesConfig(cfg: TheClawConfig, accountId: string): boolean {
...
function createSlackTokenCredential(params: {
  ...
  return {
    ...
    inspect: ({ cfg, accountId }: { cfg: TheClawConfig; accountId: string }) => {
    ...
    applyUseEnv: ({ cfg, accountId }: { cfg: TheClawConfig; accountId: string }) =>
...
export function createSlackSetupWizardBase(handlers: {
  ...
  return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: TheClawConfig; accountId: string }) =>
        resolveSlackAccount({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: TheClawConfig; accountId: string }) =>
        Object.entries(resolveSlackAccount({ cfg, accountId }).config.channels ?? {})
          .filter(([, value]) => value?.enabled !== false)
          .map(([key]) => key),
      updatePrompt: ({ cfg, accountId }: { cfg: TheClawConfig; accountId: string }) =>
    ...
    disable: (cfg: TheClawConfig) => setSetupChannelEnabled(cfg, channel, false),
