...
import { patchChannelConfigForAccount } from "theclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "theclaw/plugin-sdk/setup-tools";
import type { ResolvedSlackAccount } from "./accounts.js";
import type { TheClawConfig } from "./channel-api.js";

export const SLACK_CHANNEL = "slack" as const;

function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "TheClaw";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for TheClaw`,
    },
    features: {
      bot_user: {
...
export function buildSlackSetupLines(botName = "TheClaw"): string[] {
  return [
