...
function getChannelConfig(cfg: TheClawConfig): SynologyChatChannelConfig {
  return (cfg.channels?.[channel] as SynologyChatChannelConfig | undefined) ?? {};
}

function getRawAccountConfig(cfg: TheClawConfig, accountId: string): SynologyChatAccountRaw {
...
function isSynologyChatConfigured(cfg: TheClawConfig, accountId: string): boolean {
...
function resolveExistingAllowedUserIds(cfg: TheClawConfig, accountId: string): string[] {
  ...
  textInputs: [
    {
      ...
      helpLines: [
        ...
        "This is the URL TheClaw uses to send replies back to Chat.",
      ],
