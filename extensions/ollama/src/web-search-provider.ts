...
import { resolveEnvApiKey } from "theclaw/plugin-sdk/provider-auth-runtime";
import {
...
function resolveConfiguredOllamaWebSearchApiKey(config?: TheClawConfig): string | undefined {
...
function resolveOllamaWebSearchApiKey(config?: TheClawConfig): string | undefined {
  return resolveConfiguredOllamaWebSearchApiKey(config) ?? resolveEnvOllamaWebSearchApiKey();
}

function resolveOllamaWebSearchBaseUrl(config?: TheClawConfig): string {
...
export function createOllamaWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    docsUrl: "https://docs.theclaw.ai/tools/web",
    autoDetectOrder: 110,
    credentialPath: "",
