...
function createOllamaConfig(provider: OllamaProviderConfigOverride = {}): TheClawConfig {
  return {
    models: {
      providers: {
        ollama: {
...
function createOllamaConfigWithWebSearchBaseUrl(baseUrl: string): TheClawConfig {
  return {
    ...createOllamaConfig(),
    plugins: {
      entries: {
...
describe("ollama web search provider", () => {
  ...
  it("maps generic search args into the local Ollama proxy endpoint", async () => {
    ...
    const result = await tool.execute({ query: "theclaw docs", count: 3 });
    ...
    expect(result).toMatchObject({
      ...
      provider: "ollama",
      count: 1,
      results: [{ url: "https://theclaw.ai/docs" }],
    });
  ...
  it("tries the future local direct endpoint when the local proxy endpoint is missing", async () => {
    ...
    await expect(
      runOllamaWebSearch({ config: createOllamaConfig(), query: "theclaw" }),
  ...
  it("surfaces Ollama signin guidance for 401 responses", async () => {
    ...
    await expect(runOllamaWebSearch({ query: "latest theclaw release" })).rejects.toThrow(
      "ollama signin",
    );
  });
