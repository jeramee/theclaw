...
import { loadQaRuntimeModule } from "theclaw/plugin-sdk/qa-runner-runtime";
...
function shouldWriteMatrixQaProgress() {
  const override = process.env.THECLAW_QA_MATRIX_PROGRESS;
  if (override === "0") {
    return false;
  }
  if (override === "1") {
...
async function cleanupMatrixQaResource(params: {
  ...
  const timeoutMs = parsePositiveMatrixQaEnvMs(
    "THECLAW_QA_MATRIX_CLEANUP_TIMEOUT_MS",
...
async function startMatrixQaLiveLaneGateway(params: {
  ...
  transport: {
    ...
    createGatewayConfig: (params: {
      baseUrl: string;
    }) => Pick<TheClawConfig, "channels" | "messages">;
  };
...
export async function runMatrixQaLive(params: {
  ...
  const topology = buildMatrixQaTopologyForScenarios({
    defaultRoomName: `TheClaw Matrix QA ${runSuffix}`,
    scenarios,
  });
  const observedEvents: MatrixQaObservedEvent[] = [];
  const includeObservedEventContent = process.env.THECLAW_QA_MATRIX_CAPTURE_CONTENT === "1";
      ...
      return await measureMatrixQaStep(() =>
        withMatrixQaRunDeadline(runDeadline, "Matrix topology provisioning", () =>
          provisionMatrixQaRoom({
            ...
            roomName: `TheClaw Matrix QA ${runSuffix}`,
          ...
          const measuredScenario = await measureMatrixQaStep(() =>
            withMatrixQaRunDeadline(runDeadline, `Matrix scenario ${scenario.id}`, () =>
              runMatrixQaScenario(scenario, {
                ...
                gatewayStateDir: scenarioGateway.harness.gateway.runtimeEnv?.THECLAW_STATE_DIR,
