import { spawn as startTheClawCliProcess } from "node:child_process";
...
import { redactSensitiveText } from "theclaw/plugin-sdk/logging-core";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `theclaw ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaTheClawCliEntryPath(cwd: string): string {
...
export function startMatrixQaTheClawCli(params: {
  ...
  const distEntryPath = resolveMatrixQaTheClawCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startTheClawCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
    env: params.env,
...
export async function runMatrixQaTheClawCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaTheClawCli(params).wait();
}
...
export async function createMatrixQaTheClawCliRuntime(params: {
  accountId: string;
  accessToken: string;
