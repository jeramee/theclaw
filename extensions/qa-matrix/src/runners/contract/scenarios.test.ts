...
const {
  ...
  resolveMatrixQaTheClawCliEntryPath,
  runMatrixQaTheClawCli,
  startMatrixQaTheClawCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `theclaw ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaTheClawCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaTheClawCli: vi.fn(),
  startMatrixQaTheClawCli: vi.fn(),
}));
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaTheClawCli.mockReset();
    startMatrixQaTheClawCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "TheClaw Matrix QA run",
        scenarios: [
    ...
    try {
      ...
      await expect(
        runMatrixQaScenario(scenario!, {
          ...
          gatewayRuntimeEnv: {
            THECLAW_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaTheClawCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaTheClawCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.THECLAW_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.THECLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      await expect(
        runMatrixQaScenario(scenario!, {
          ...
          driverPassword: "driver-password",
          gatewayRuntimeEnv: {
            THECLAW_CONFIG_PATH: "/tmp/gateway-config.json",
            THECLAW_STATE_DIR: "/tmp/gateway-state",
            PATH: process.env.PATH,
          },
          outputDir,
        }),
      ...
      expect(startMatrixQaTheClawCli).toHaveBeenCalledTimes(1);
      expect(startMatrixQaTheClawCli.mock.calls[0]?.[0].args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(startMatrixQaTheClawCli.mock.calls[0]?.[0].timeoutMs).toBe(16_000);
      ...
      expect(runMatrixQaTheClawCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaTheClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(runMatrixQaTheClawCli.mock.calls[0]?.[0].stdin).toBe("encoded-recovery-key\n");
      const cliEnv = startMatrixQaTheClawCli.mock.calls[0]?.[0].env;
      expect(cliEnv?.THECLAW_STATE_DIR).toContain("theclaw-matrix-cli-qa-");
      expect(cliEnv?.THECLAW_CONFIG_PATH).toContain("theclaw-matrix-cli-qa-");
      const configPath = String(cliEnv?.THECLAW_CONFIG_PATH);
      ...
      await expect(readdir(String(cliEnv?.THECLAW_STATE_DIR))).rejects.toThrow();
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaTheClawCli.mockImplementation(async ({ args, env }) => {
        if (env.THECLAW_CONFIG_PATH) {
      ...
      expect(runMatrixQaTheClawCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          ...
          "cli-add-password",
          "--device-name",
          "TheClaw Matrix QA CLI Account Add E2EE",
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Account Add Owner",
          registrationToken: "registration-token",
        }),
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaTheClawCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.THECLAW_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.THECLAW_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Encryption Setup Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Encryption Idempotent Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("theclaw matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaTheClawCli.mockReturnValue({
      ...
      expect(startMatrixQaTheClawCli.mock.calls[0]?.[0].env.THECLAW_CONFIG_PATH).toContain(
        "theclaw-matrix-e2ee-setup-qa-",
      );
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Encryption Failure Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Invalid Recovery Key Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaTheClawCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.THECLAW_CONFIG_PATH);
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Multi Account Owner",
          registrationToken: "registration-token",
        }),
      );
      expect(loginWithPassword).toHaveBeenCalledWith(
  ...
  it("runs Matrix CLI setup then gateway encrypted reply through the QA scenario", async () => {
    ...
    try {
      ...
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Setup Gateway",
          registrationToken: "registration-token",
        }),
      );
      expect(registerWithToken).toHaveBeenCalledWith(
        expect.objectContaining({
          deviceName: "TheClaw Matrix QA CLI Setup Driver",
          registrationToken: "registration-token",
        }),
      );
      expect(createPrivateRoom).toHaveBeenCalledWith({
