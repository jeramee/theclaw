...
metadata: { "theclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == TheClaw `sessions_spawn`
- **File I/O** == TheClaw `read`/`write`
- **Remote fetch** == TheClaw `web_fetch` (or `exec` with curl when POST is required)
