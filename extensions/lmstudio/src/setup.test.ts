import { CUSTOM_LOCAL_AUTH_MARKER } from "theclaw/plugin-sdk/provider-auth";
import type { TheClawConfig } from "theclaw/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "theclaw/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "theclaw/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "theclaw/plugin-sdk/provider-setup";
import type { WizardPrompter } from "theclaw/plugin-sdk/setup";
...
vi.mock("theclaw/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("theclaw/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("theclaw/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("theclaw/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
function buildConfig(): TheClawConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("THECLAW_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
