...
import type { WizardPrompter } from "theclaw/plugin-sdk/plugin-test-runtime";
import { bundledPluginRoot } from "theclaw/plugin-sdk/test-fixtures";
import ts from "typescript";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import type { TheClawConfig, PluginRuntime, ResolvedLineAccount } from "../api.js";
...
function collectRuntimeApiPreExports(runtimeApiPath: string): string[] {
  ...
  const removedLineRuntimeSpecifier = ["theclaw", "plugin-sdk", "line-runtime"].join("/");
...
describe("line setup wizard", () => {
  ...
  it("reports account-scoped config keys for named accounts", async () => {
    expect(lineSetupWizard.dmPolicy?.resolveConfigKeys?.({} as TheClawConfig, "work")).toEqual({
