...
function resolveQianfanPreset(cfg: TheClawConfig): {
  api: ModelApi;
  baseUrl: string;
...
const qianfanPresetAppliers = createDefaultModelsPresetAppliers({
  ...
  resolveParams: (cfg: TheClawConfig) => {
...
export function applyQianfanProviderConfig(cfg: TheClawConfig): TheClawConfig {
  return qianfanPresetAppliers.applyProviderConfig(cfg);
}

export function applyQianfanConfig(cfg: TheClawConfig): TheClawConfig {
