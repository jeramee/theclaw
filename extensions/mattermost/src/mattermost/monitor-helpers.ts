import { formatInboundFromLabel as formatInboundFromLabelShared } from "theclaw/plugin-sdk/channel-inbound";
import { createDedupeCache, type TheClawConfig } from "theclaw/plugin-sdk/core";
import { resolveThreadSessionKeys as resolveThreadSessionKeysShared } from "theclaw/plugin-sdk/routing";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalString,
} from "theclaw/plugin-sdk/text-runtime";
import { rawDataToString } from "theclaw/plugin-sdk/webhook-ingress";
...
type AgentEntry = NonNullable<NonNullable<TheClawConfig["agents"]>["list"]>[number];

function isAgentEntry(entry: unknown): entry is AgentEntry {
  return Boolean(entry && typeof entry === "object");
}

function listAgents(cfg: TheClawConfig): AgentEntry[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(isAgentEntry) : [];
}

function resolveAgentEntry(cfg: TheClawConfig, agentId: string): AgentEntry | undefined {
  const id = normalizeAgentId(agentId);
  return listAgents(cfg).find((entry) => normalizeAgentId(entry.id) === id);
}

export function resolveIdentityName(cfg: TheClawConfig, agentId: string): string | undefined {
