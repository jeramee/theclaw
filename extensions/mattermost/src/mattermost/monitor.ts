...
import { isReasoningReplyPayload } from "theclaw/plugin-sdk/reply-payload";
...
export async function monitorMattermostProvider(opts: MonitorMattermostOpts = {}): Promise<void> {
  ...
  const cfg = (opts.config ?? core.config.current()) as TheClawConfig;
      ...
      authorizeButtonClick: async ({ payload, post }) => {
        ...
        return {
          ...
          response: {
            ...
            ephemeral_text: `TheClaw ignored this action for ${decision.roomLabel}.`,
          },
        };
      },
