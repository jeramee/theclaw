...
import { isTrustedProxyAddress, resolveClientIp, type TheClawConfig } from "./runtime-api.js";

const INTERACTION_MAX_BODY_BYTES = 64 * 1024;
const INTERACTION_BODY_TIMEOUT_MS = 10_000;
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__theclaw_channel_id";
...
type InteractionCallbackConfig = Pick<TheClawConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "theclaw-mattermost-interactions").update(botToken).digest("hex");
}
