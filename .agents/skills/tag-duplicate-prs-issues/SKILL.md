...
description: Use gitcrawl to search duplicate TheClaw PRs/issues, group related work in prtags, and sync duplicate state to GitHub.
---
...
Use `gitcrawl` first because it is the local TheClaw history and clustering source.
...
```bash
gitcrawl threads theclaw/theclaw --numbers <issue-or-pr-number> --include-closed --json
gitcrawl neighbors theclaw/theclaw --number <issue-or-pr-number> --limit 20 --json
```

Then search key phrases and subsystem terms:

```bash
gitcrawl search theclaw/theclaw --query "<key phrase from title or body>" --mode hybrid --limit 20 --json
gitcrawl search theclaw/theclaw --query "<subsystem or error phrase>" --mode hybrid --limit 20 --json
```

Inspect likely clusters:

```bash
gitcrawl cluster-detail theclaw/theclaw --id <cluster-id> --member-limit 20 --body-chars 280 --json
```
...
```bash
gh search prs --repo theclaw/theclaw --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo theclaw/theclaw --match title,body --limit 50 -- "<key phrase>"
gh search issues --repo theclaw/theclaw --match comments --limit 50 -- "<error or maintainer phrase>"
```
...
```bash
prtags search text -R theclaw/theclaw "<problem phrase>" --types group --limit 10
prtags search similar -R theclaw/theclaw "<problem summary>" --types group --limit 10
prtags group list -R theclaw/theclaw
```
...
```bash
prtags group create -R theclaw/theclaw \
  --kind mixed \
...
```bash
prtags field ensure -R theclaw/theclaw --name duplicate_status --scope pull_request --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R theclaw/theclaw --name duplicate_status --scope issue --type enum --enum-values not_duplicate,candidate,confirmed --filterable
prtags field ensure -R theclaw/theclaw --name duplicate_confidence --scope pull_request --type enum --enum-values low,medium,high --filterable
prtags field ensure -R theclaw/theclaw --name duplicate_confidence --scope issue --type enum --enum-values low,medium,high --filterable
prtags field ensure -R theclaw/theclaw --name duplicate_rationale --scope pull_request --type text --searchable
prtags field ensure -R theclaw/theclaw --name duplicate_rationale --scope issue --type text --searchable
```

Recommended group-level fields:

```bash
prtags field ensure -R theclaw/theclaw --name duplicate_confidence --scope group --type enum --enum-values low,medium,high --filterable
prtags field ensure -R theclaw/theclaw --name duplicate_rationale --scope group --type text --searchable
prtags field ensure -R theclaw/theclaw --name cluster_summary --scope group --type text --searchable
```
...
```bash
prtags annotation pr set -R theclaw/theclaw <pr-number> \
...
```bash
prtags annotation issue set -R theclaw/theclaw <issue-number> \
  duplicate_status=confirmed \
  duplicate_confidence=high \
...
```bash
prtags group list-comment-sync-targets -R theclaw/theclaw
```
