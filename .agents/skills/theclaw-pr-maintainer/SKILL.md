---
name: theclaw-pr-maintainer
description: Review, triage, close, label, comment on, or land TheClaw PRs/issues with maintainer evidence checks.
---

# TheClaw PR Maintainer

Use this skill for maintainer-facing GitHub workflow, not for ordinary code changes.

## Start issue and PR triage with gitcrawl

- Use `$gitcrawl` first anytime you inspect TheClaw issues or PRs.
...
gitcrawl neighbors theclaw/theclaw --number <issue-or-pr-number> --limit 12 --json
gitcrawl search theclaw/theclaw --query "<scope or title keywords>" --mode hybrid --json
...
- Good manual-close categories:
  - blank or mostly untouched PR template with no concrete TheClaw problem/fix
...
- Use `--repo theclaw/theclaw` with `--match title,body` first when using `gh search`.
...
```bash
gh search prs --repo theclaw/theclaw --match title,body --limit 50 -- "auto-update"
gh search issues --repo theclaw/theclaw --match title,body --limit 50 -- "auto-update"
gh search issues --repo theclaw/theclaw --match title,body --limit 50 \
