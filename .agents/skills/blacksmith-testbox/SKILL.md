...
TheClaw maintainers can opt into Testbox-first validation by setting
`THECLAW_TESTBOX=1` in their environment or standing agent rules. This mode is
maintainers-only and requires Blacksmith access.

When `THECLAW_TESTBOX=1` is set in TheClaw:
...
- Use local commands only when the task explicitly sets
  `THECLAW_LOCAL_CHECK_MODE=throttled|full`, or when the user asks for local
  proof.
...
For TheClaw specifically, normal local iteration stays local unless maintainer
Testbox mode is enabled with `THECLAW_TESTBOX=1`:
...
If `THECLAW_TESTBOX=1` is enabled, run those same repo commands inside the
warm Testbox. If the user wants laptop-friendly local proof for one command, use
the explicit escape hatch `THECLAW_LOCAL_CHECK_MODE=throttled`.
...
For TheClaw maintainer Testbox mode, pre-warm at the start of longer or wider
tasks:
...
In TheClaw, raw commit SHAs are not reliable dispatch refs for `warmup --ref`;
use a branch or tag. The build-artifact workflow resolves `theclaw@beta` and
`theclaw@latest` to SHA cache keys internally.
...
TheClaw maintainer exception: if `THECLAW_TESTBOX=1` is set by the user or
agent environment, treat Testbox as the normal validation path for this repo.
Use `THECLAW_LOCAL_CHECK_MODE=throttled|full` as the explicit local escape
hatch.
...
1. Decide whether the repo's local loop is the right default. For TheClaw,
   `THECLAW_TESTBOX=1` makes Testbox the maintainer default.
...
7. If a narrow PR reports a full sync or the box was reused/expired, sanity
   ...
   warm a fresh one. Use `THECLAW_TESTBOX_ALLOW_MASS_DELETIONS=1` only for an
...
## TheClaw full test suite

For TheClaw, use the repo package manager and the measured stable full-suite
...
shard to one worker to avoid worker OOMs on Testbox:
    ...
    blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 THECLAW_TEST_PROJECTS_PARALLEL=6 THECLAW_VITEST_MAX_WORKERS=1 pnpm test"
...
If you need a longer session, increase the timeout at warmup time. For TheClaw
