---
name: gitcrawl
description: Use gitcrawl for TheClaw issue and PR archive search, duplicate discovery, related-thread clustering, and local GitHub mirror freshness checks.
metadata:
  theclaw:
    requires:
      bins:
...
Use this skill before live GitHub search when triaging TheClaw issues or PRs.
...
gitcrawl search theclaw/theclaw --query "<scope or title keywords>" --mode hybrid --limit 20 --json
```
...
```bash
gitcrawl init --portable-store git@github.com:theclaw/gitcrawl-store.git --json
```
