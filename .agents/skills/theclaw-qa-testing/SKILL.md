---
name: theclaw-qa-testing
description: Run, watch, debug, extend, or explain TheClaw qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
THECLAW_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
...
5. If the user wants to watch the live UI, find the current `theclaw-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm theclaw qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
THECLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- That item is the first place to look for:
  ...
  - `THECLAW_QA_PROVIDER_MODE`
  - `THECLAW_NPM_TELEGRAM_PACKAGE_SPEC`
- Convex QA secrets currently live in 1Password items:
  - vault: `TheClaw`
  - item: `THECLAW_QA_CONVEX_SITE_URL`
  - item: `THECLAW_QA_CONVEX_SECRET_MAINTAINER`
  - item: `THECLAW_QA_CONVEX_SECRET_CI`
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `THECLAW QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `THECLAW_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `THECLAW_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `TheClaw/THECLAW_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
THECLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
THECLAW_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
THECLAW_QA_PROVIDER_MODE="mock-openai" \
THECLAW_NPM_TELEGRAM_PACKAGE_SPEC="theclaw@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `THECLAW_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `THECLAW_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `THECLAW_QA_PROVIDER_MODE`, map it explicitly to `THECLAW_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    - `THECLAW_QA_CREDENTIAL_SOURCE=convex`
    - `THECLAW_QA_CREDENTIAL_ROLE=maintainer`
    - `THECLAW_QA_CONVEX_SITE_URL`
    - `THECLAW_QA_CONVEX_SECRET_MAINTAINER`
    - `THECLAW_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal TheClaw identity combines with the character.
...
```bash
pnpm theclaw qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `THECLAW_HOME` sandboxed.
