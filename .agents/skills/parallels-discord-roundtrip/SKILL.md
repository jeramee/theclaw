...
```bash
export THECLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.theclaw/theclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env THECLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `theclaw` wrapper for guest `message send/read`; `node theclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/theclaw-parallels-smoke.*`
...
- host inbound nonce appears in `theclaw message read` output
