---
name: theclaw-testing
description: Choose, run, rerun, or debug TheClaw tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
THECLAW_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
- `TheClaw Release Checks` for install smoke, cross-OS release checks, live and
...
important run ids in the private `theclaw/releases-private` evidence ledger.
Use the manual `TheClaw Release Evidence`
(`theclaw-release-evidence.yml`) workflow there. It writes durable summaries
...
```text
full-release-validation theclaw/theclaw <run-id> blocking
package-acceptance theclaw/theclaw <run-id> blocking
release-checks theclaw/theclaw <run-id> blocking
```
...
`THECLAW_RELEASES_PRIVATE_DISPATCH_TOKEN` is configured in the public repo, it
requests the private `TheClaw Release Evidence From Full Validation` workflow.
...
`TheClaw Release Checks` (`theclaw-release-checks.yml`) is the release child
...
```bash
gh workflow run theclaw-release-checks.yml \
  --repo theclaw/theclaw \
  --ref main \
  -f ref=<branch-or-sha> \
  -f provider=openai \
...
`pnpm theclaw qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm theclaw qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `THECLAW_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
Matrix selection. `TheClaw Release Checks` uses explicit fast Matrix; run the
all-lanes workflow when release investigation needs full Matrix media/E2EE
inventory.

### Reusable Live/E2E Checks

`TheClaw Live And E2E Checks (Reusable)`
(`theclaw-live-and-e2e-checks-reusable.yml`) is the preferred entry point for
targeted live, Docker, model, and E2E proof. Inputs let you turn off unrelated
lanes:

```bash
gh workflow run theclaw-live-and-e2e-checks-reusable.yml \
  --repo theclaw/theclaw \
  --ref main \
  -f ref=<sha> \
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `THECLAW_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`THECLAW_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
THECLAW_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
THECLAW_DOCKER_ALL_DRY_RUN=1 THECLAW_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
THECLAW_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
THECLAW_DOCKER_ALL_LANES=<lane> \
THECLAW_DOCKER_ALL_BUILD=0 \
THECLAW_DOCKER_ALL_PREFLIGHT=0 \
THECLAW_SKIP_DOCKER_BUILD=1 \
THECLAW_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
THECLAW_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  ...
  and `npm view theclaw@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/theclaw-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=theclaw@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=theclaw@YYYY.M.D-beta.N` or
  `package_spec=theclaw@YYYY.M.D`.
- For stable package proof, use `package_spec=theclaw@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `theclaw@beta`,
  `theclaw@latest`, or exact TheClaw release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `theclaw update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `theclaw@beta`, `theclaw@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an TheClaw branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-theclaw-for-docker.mjs` is the
