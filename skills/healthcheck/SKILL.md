---
name: healthcheck
description: Audit and harden hosts running TheClaw for SSH, firewall, updates, exposure, cron checks, and risk posture.
---

# TheClaw Host Hardening

## Overview

Assess and harden the host running TheClaw, then align it to a user-defined risk tolerance without breaking access. Use TheClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim TheClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. TheClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if TheClaw can access sensitive data.
...
### 2) Run TheClaw security audits (read-only)

As part of the default read-only checks, run `theclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `theclaw security audit` (faster, non-probing)
2. `theclaw security audit --json` (structured output)

Offer to apply TheClaw safe defaults (numbered):

1. `theclaw security audit --fix`

Be explicit that `--fix` only tightens TheClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check TheClaw version/update status (read-only)

As part of the default read-only checks, run `theclaw update status`.
...
- Credential hygiene notes (location of TheClaw creds, prefer disk encryption)
...
After TheClaw install or first hardening pass, run at least one baseline audit and version check:

- `theclaw security audit`
- `theclaw security audit --deep`
- `theclaw update status`

Ongoing monitoring is recommended. Use the TheClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `theclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `theclaw update status`
...
Before creating, `theclaw cron list` and match on exact `name`. If found, `theclaw cron edit <id> ...`.
If not found, `theclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `theclaw update status` (preferred for source checkouts and channels)
2. `npm view theclaw version` (published npm version)

## TheClaw command accuracy

Use only supported commands and flags:

- `theclaw security audit [--deep] [--fix] [--json]`
- `theclaw status` / `theclaw status --deep`
- `theclaw health --json`
- `theclaw update status`
- `theclaw cron add|list|runs|run`

Do not invent CLI flags or imply TheClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by TheClaw compaction:
