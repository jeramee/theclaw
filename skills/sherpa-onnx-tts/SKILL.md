...
1. Download the runtime for your OS (extracts into `$THECLAW_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.theclaw/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$THECLAW_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.theclaw/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active TheClaw config file (`$THECLAW_CONFIG_PATH`, default `~/.theclaw/theclaw.json`):
