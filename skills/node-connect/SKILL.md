---
name: node-connect
description: Diagnose TheClaw Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures.
---

# Node Connect

Goal: find the one real route from node -> gateway, verify TheClaw is advertising that route, then fix pairing/auth.
...
- whether `theclaw devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `theclaw qr --json`. It uses the same setup-code payload Android scans.

```bash
theclaw config get gateway.mode
theclaw config get gateway.bind
theclaw config get gateway.tailscale.mode
theclaw config get gateway.remote.url
theclaw config get gateway.auth.mode
theclaw config get gateway.auth.allowTailscale
theclaw config get plugins.entries.device-pair.config.publicUrl
theclaw qr --json
theclaw devices list
theclaw nodes status
```

If this TheClaw instance is pointed at a remote gateway, also run:

```bash
theclaw qr --remote --json
```
...
If `theclaw qr --json` says `Gateway is only bound to loopback`:
...
- `theclaw devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run theclaw qr again, rescan, then approve the pending device pairing.`
