...
declare const __THECLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "theclaw";
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["THECLAW_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["THECLAW_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["THECLAW_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.THECLAW_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current TheClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __THECLAW_VERSION__ === "string" ? __THECLAW_VERSION__ : undefined,
  bundledVersion: process.env.THECLAW_BUNDLED_VERSION,
});
