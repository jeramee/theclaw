...
export function shouldEnableTheClawCompileCache(params: {
...
export type TheClawCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
};

export function buildTheClawCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
...
}): TheClawCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
  if (!isSourceCheckoutInstallRoot(params.installRoot)) {
    return undefined;
  }
  if (env.THECLAW_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return undefined;
  }
...
export function respawnWithoutTheClawCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
...
export function enableTheClawCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableTheClawCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache();
