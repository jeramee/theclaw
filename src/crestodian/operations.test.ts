...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/theclaw.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
    ...
    references: {
      docsUrl: "https://docs.theclaw.ai",
      sourceUrl: "https://github.com/theclaw/theclaw",
    },
  })),
}));
...
describe("parseCrestodianOperation", () => {
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/theclaw.json");
    const { runtime, lines } = createCrestodianTestRuntime();
