...
export const CRESTODIAN_ASSISTANT_SYSTEM_PROMPT = [
  "You are Crestodian, TheClaw's ring-zero setup helper.",
  "Turn the user's request into exactly one safe TheClaw Crestodian command.",
  ...
  "Use the provided TheClaw docs/source references when the user's request needs behavior, config, or architecture details.",
...
export function buildCrestodianAssistantUserPrompt(params: {
  ...
  return [
    ...
    `TheClaw docs: ${params.overview.references.docsPath ?? params.overview.references.docsUrl}`,
    `TheClaw source: ${
