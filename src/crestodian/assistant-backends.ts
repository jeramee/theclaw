...
export type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => TheClawConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): TheClawConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): TheClawConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
