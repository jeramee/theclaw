...
export class TheClawChannelBridge {
  ...
  constructor(
    private readonly cfg: TheClawConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`theclaw mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
    ...
    try {
      ...
      if (this.verbose && !this.closed) {
        process.stderr.write(
          `theclaw mcp: notification ${notification.method} failed: ${String(error)}\n`,
        );
      }
    }
  }
