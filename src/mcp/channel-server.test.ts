...
import { createTheClawChannelMcpServer, TheClawChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createTheClawChannelMcpServer({
...
describe("theclaw channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new TheClawChannelBridge({} as never, {
