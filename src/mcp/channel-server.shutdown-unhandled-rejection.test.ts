...
vi.mock("./channel-bridge.js", () => ({
  TheClawChannelBridge: class MockTheClawChannelBridge {
...
describe("serveTheClawChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveTheClawChannelMcp } = await import("./channel-server.js");

    const servePromise = serveTheClawChannelMcp({ verbose: false });
