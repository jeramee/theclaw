...
describe("trajectory metadata", () => {
  it("redacts harness argv and local paths with the support redaction rules", () => {
    ...
    process.argv = [
      "node",
      "/Users/tester/project/theclaw.js",
      "--api-key",
      "super-secret",
      "--config=/Users/tester/.theclaw/theclaw.json",
    ];
    try {
      const metadata = buildTrajectoryRunMetadata({
        env: {
          HOME: "/Users/tester",
          THECLAW_STATE_DIR: "/Users/tester/.theclaw",
        },
      ...
      expect(harness.invocation).toEqual([
        ...
        "--api-key",
        "<redacted>",
        "--config=$THECLAW_STATE_DIR/theclaw.json",
      ]);
      expect(harness.entrypoint).toBe("~/project/theclaw.js");
