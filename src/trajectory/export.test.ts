...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-trajectory-"));
let tempDirId = 0;
...
describe("exportTrajectoryBundle", () => {
  it("sanitizes session ids in default export directory names", () => {
    ...
    expect(outputDir).toBe(
      path.join(
        ...
        ".theclaw",
        "trajectory-exports",
        "theclaw-trajectory-___evil_-2026-04-22T08-00-00",
      ),
    );
  });
  ...
  it("uses the recorded runtime pointer before current environment overrides", () => {
    ...
    const previous = process.env.THECLAW_TRAJECTORY_DIR;
    process.env.THECLAW_TRAJECTORY_DIR = envRuntimeDir;
    try {
      const bundle = exportTrajectoryBundle({
        outputDir,
        sessionFile,
      ...
      if (previous === undefined) {
        delete process.env.THECLAW_TRAJECTORY_DIR;
      } else {
        process.env.THECLAW_TRAJECTORY_DIR = previous;
      }
    }
  });
  ...
  it("exports merged runtime and transcript events plus convenience files", () => {
    ...
    const runtimeEvents: TrajectoryEvent[] = [
      ...
      },
      {
        ...
        sessionId: "session-1",
        data: {
          harness: { type: "theclaw", version: "0.1.0" },
