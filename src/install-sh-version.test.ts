...
function resolveInstallerVersionCases(params: { stdinCwd: string }): string[] {
  const output = execFileSync(
    ...
    [
      "-c",
      `${versionHelperSource}
fake_theclaw_decorated() { printf '%s\\n' 'TheClaw 2026.3.10 (abcdef0)'; }
fake_theclaw_raw() { printf '%s\\n' "TheClaw dev's build"; }
THECLAW_BIN=fake_theclaw_decorated resolve_theclaw_version
THECLAW_BIN=fake_theclaw_raw resolve_theclaw_version
(
  cd "$1"
  source /dev/stdin <<'THECLAW_STDIN_INSTALLER'
${versionHelperSource}
fake_theclaw_stdin() { printf '%s\\n' 'TheClaw 2026.3.10 (abcdef0)'; }
THECLAW_BIN=fake_theclaw_stdin
resolve_theclaw_version
THECLAW_STDIN_INSTALLER
)`,
      "theclaw-version-test",
      params.stdinCwd,
    ],
    {
      cwd: process.cwd(),
...
describe("install.sh version resolution", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    () => {
      const hostileCwd = makeTempDir(tempRoots, "theclaw-install-stdin-");
...
`,
      ...
      ).toEqual(["2026.3.10", "TheClaw dev's build", "2026.3.10"]);
    },
  );
});
