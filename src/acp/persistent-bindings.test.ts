...
type ConfiguredBinding = NonNullable<TheClawConfig["bindings"]>[number];
...
function createCfgWithBindings(
  ...
  overrides?: Partial<TheClawConfig>,
): TheClawConfig {
  return {
    ...baseCfg,
    ...overrides,
...
function resolveBindingRecord(cfg: TheClawConfig, overrides: Partial<BindingRecordInput> = {}) {
...
describe("resolveConfiguredAcpBindingRecord", () => {
  ...
  it("derives configured binding cwd from an explicit agent workspace", () => {
    const cfg = createCfgWithBindings(
      ...
      {
        agents: {
          list: [{ id: "codex", workspace: "/workspace/theclaw" }, { id: "claude" }],
        },
      },
    );
    const resolved = resolveBindingRecord(cfg);
