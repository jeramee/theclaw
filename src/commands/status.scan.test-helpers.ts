...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/theclaw-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = TheClawConfig>(
  overrides: T = {} as T,
): TheClawConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): TheClawConfig {
