...
type EnvSnapshot = {
  HOME?: string;
  THECLAW_HOME?: string;
  THECLAW_STATE_DIR?: string;
  THECLAW_OAUTH_DIR?: string;
};

function captureEnv(): EnvSnapshot {
  return {
    HOME: process.env.HOME,
    THECLAW_HOME: process.env.THECLAW_HOME,
    THECLAW_STATE_DIR: process.env.THECLAW_STATE_DIR,
    THECLAW_OAUTH_DIR: process.env.THECLAW_OAUTH_DIR,
  };
}
...
function setupSessionState(cfg: TheClawConfig, env: NodeJS.ProcessEnv, homeDir: string) {
  const agentId = "main";
...
function createAgentDir(agentId: string, includeNestedAgentDir = true) {
  ...
  if (!stateDir) {
    throw new Error("THECLAW_STATE_DIR is not set");
  }
...
async function runStateIntegrity(cfg: TheClawConfig) {
...
async function runStateIntegrityText(cfg: TheClawConfig): Promise<string> {
...
describe("doctor state integrity oauth dir checks", () => {
  ...
  beforeEach(() => {
    ...
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-state-integrity-"));
    ...
    fs.mkdirSync(process.env.THECLAW_STATE_DIR, { recursive: true, mode: 0o700 });
    noteMock.mockClear();
  });
  ...
  it("prompts for oauth dir when THECLAW_OAUTH_DIR is explicitly configured", async () => {
    process.env.THECLAW_OAUTH_DIR = path.join(tempHome, ".oauth");
    const cfg: TheClawConfig = {};
    const confirmRuntimeRepair = await runStateIntegrity(cfg);
    expect(confirmRuntimeRepair).toHaveBeenCalledWith(OAUTH_PROMPT_MATCHER);
  ...
  it("does not warn when a case-mismatched dir resolves to the configured agent path", async () => {
    ...
    const resolvedResearchAgentDir = realpathNative(
      path.join(process.env.THECLAW_STATE_DIR ?? "", "agents", "Research", "agent"),
    );
    const realpathSpy = vi
      .spyOn(fs.realpathSync, "native")
      .mockImplementation((target, options) => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      try {
        ...
        process.env.THECLAW_HOME = symlinkHome;
        process.env.THECLAW_STATE_DIR = path.join(symlinkHome, ".theclaw");
  ...
  it("prints theclaw-only verification hints when recent sessions are missing transcripts", async () => {
    const cfg: TheClawConfig = {};
    writeSessionStore(cfg, {
      "agent:main:main": {
    ...
    expect(text).toMatch(/theclaw sessions --store ".*sessions\.json"/);
    expect(text).toMatch(/theclaw sessions cleanup --store ".*sessions\.json" --dry-run/);
    expect(text).toMatch(
      /theclaw sessions cleanup --store ".*sessions\.json" --enforce --fix-missing/,
    );
