...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".theclaw-work");
    const defaultStateDir = path.join(homeDir, ".theclaw");
    const profileConfigPath = path.join(profileStateDir, "theclaw.json");
    ...
    vi.stubEnv("THECLAW_HOME", homeDir);
    vi.stubEnv("THECLAW_PROFILE", "work");
    vi.stubEnv("THECLAW_STATE_DIR", profileStateDir);
    vi.stubEnv("THECLAW_CONFIG_PATH", profileConfigPath);
