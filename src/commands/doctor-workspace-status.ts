...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("theclaw tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("theclaw tasks flow cancel <flow-id>")}`,
    ]
...
export function noteWorkspaceStatus(cfg: TheClawConfig) {
