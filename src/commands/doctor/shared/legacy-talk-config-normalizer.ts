...
export function normalizeLegacyTalkConfig(cfg: TheClawConfig, changes: string[]): TheClawConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as TheClawConfig["talk"]) ?? {};
