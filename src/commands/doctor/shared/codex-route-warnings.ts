...
function isCodexPluginEnabled(cfg: TheClawConfig): boolean {
...
function resolveRuntime(params: {
  ...
  return (
    normalizeString(params.env?.THECLAW_AGENT_RUNTIME) ??
...
export function collectCodexRouteWarnings(params: {
  ...
  return [
    [
      "- Codex plugin is enabled, but `openai-codex/*` model refs still use the TheClaw PI runner unless `agentRuntime.id` is `codex`.",
      ...hits.map(
