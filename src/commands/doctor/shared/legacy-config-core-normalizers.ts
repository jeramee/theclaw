...
export function normalizeLegacyCommandsConfig(
  ...
  return {
    ...cfg,
    commands: commands as TheClawConfig["commands"],
  };
}
...
export function normalizeLegacyBrowserConfig(
  ...
  return {
    ...cfg,
    browser: browser as TheClawConfig["browser"],
  };
}
...
export function seedMissingDefaultAccountsFromSingleAccountBase(
  ...
  return {
    ...cfg,
    channels: nextChannels as TheClawConfig["channels"],
  };
}

type ModelProviderEntry = Partial<
  NonNullable<NonNullable<TheClawConfig["models"]>["providers"]>[string]
>;
type ModelsConfigPatch = Partial<NonNullable<TheClawConfig["models"]>>;
type ModelDefinitionEntry = NonNullable<ModelProviderEntry["models"]>[number];
type AgentRuntimePolicyPatch = NonNullable<
  NonNullable<NonNullable<TheClawConfig["agents"]>["defaults"]>["agentRuntime"]
>;
...
export function normalizeLegacyRuntimeModelRefs(
  ...
  return changed
    ? {
        ...cfg,
        agents: nextAgents as TheClawConfig["agents"],
      }
    : cfg;
}
...
export function normalizeLegacyOpenAICodexModelsAddMetadata(
  ...
  return {
    ...
    models: {
      ...rawModels,
      providers: nextProviders as NonNullable<TheClawConfig["models"]>["providers"],
    },
  };
}
...
export function normalizeLegacyNanoBananaSkill(
  ...
  if (!hasGoogleApiKey && legacyApiKey) {
    ...
    rawModels.providers = rawProviders as NonNullable<TheClawConfig["models"]>["providers"];
    next = {
      ...next,
      models: rawModels as TheClawConfig["models"],
    };
    changes.push(
...
export function normalizeLegacyMediaProviderOptions(
  ...
  return {
    ...
    tools: {
      ...cfg.tools,
      media: nextMedia as NonNullable<TheClawConfig["tools"]>["media"],
    },
  };
}
