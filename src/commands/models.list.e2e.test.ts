...
const ensureTheClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveTheClawAgentDir = vi.fn().mockReturnValue("/tmp/theclaw-agent");
...
beforeEach(() => {
  ...
  ensureTheClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureTheClawModelsJson).not.toHaveBeenCalled();
  });
