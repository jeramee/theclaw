...
function writeBundledChannelOwnerPlugin(
  ...
  writeJson(path.join(root, "dist", "extensions", id, "theclaw.plugin.json"), {
    id,
    channels,
...
function readRetainedRuntimeDepsManifest(installRoot: string): string[] {
  const manifestPath = path.join(installRoot, ".theclaw-runtime-deps.json");
...
describe("doctor bundled plugin runtime deps", () => {
  ...
  it("skips source checkouts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-bundled-"));
  ...
  it("reports missing deps and conflicts", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-bundled-"));
    writeJson(path.join(root, "package.json"), { name: "theclaw" });
  ...
  it("reports default-enabled gateway startup sidecar deps", () => {
    ...
    writeJson(path.join(root, "dist", "extensions", "browser", "theclaw.plugin.json"), {
      id: "browser",
      enabledByDefault: true,
      configSchema: { type: "object" },
    });
  ...
  it("reports explicitly enabled provider deps", () => {
    ...
    writeJson(path.join(root, "dist", "extensions", "bedrock", "theclaw.plugin.json"), {
      id: "bedrock",
      enabledByDefault: true,
      providers: ["bedrock"],
      configSchema: { type: "object" },
  ...
  it("does not report allowlist-excluded default-enabled bundled plugin deps", () => {
    ...
    writeJson(path.join(root, "dist", "extensions", "openai", "theclaw.plugin.json"), {
      id: "openai",
      enabledByDefault: true,
      configSchema: { type: "object" },
    });
  ...
  it("repairs missing deps into an external stage dir when configured", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-bundled-"));
    const stageDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-bundled-stage-"));
    writeJson(path.join(root, "package.json"), { name: "theclaw", version: "2026.4.22" });
  ...
  it("repairs only missing deps into the final layered stage dir", async () => {
    ...
    const baselineStageDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "theclaw-doctor-bundled-baseline-"),
    );
    const writableStageDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "theclaw-doctor-bundled-writable-"),
    );
    writeJson(path.join(root, "package.json"), { name: "theclaw", version: "2026.4.25" });
  ...
  it("drops stale retained bundled deps when repairing a subset", async () => {
    ...
    writeBundledChannelPlugin(root, "slack", { "@slack/web-api": "7.15.1" });
    const installRoot = resolveBundledRuntimeDependencyPackageInstallRoot(root);
    writeJson(path.join(installRoot, ".theclaw-runtime-deps.json"), {
