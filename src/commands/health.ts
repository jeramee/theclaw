...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.THECLAW_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: TheClawConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: TheClawConfig) => {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: TheClawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.THECLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<TheClawConfig> {
