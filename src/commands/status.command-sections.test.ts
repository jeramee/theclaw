...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:theclaw security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:theclaw security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:theclaw status --all",
      "  Need to debug live? cmd:theclaw logs --follow",
      "  Fix reachability first: cmd:theclaw gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:theclaw devices approve req-123)",
      "muted(Fallback: cmd:theclaw devices approve --latest)",
      "muted(Inspect: cmd:theclaw devices list)",
    ]);
  });
