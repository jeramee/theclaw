...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("theclaw security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("theclaw status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("theclaw tasks maintenance --apply")}`,
