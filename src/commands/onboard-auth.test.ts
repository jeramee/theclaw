...
vi.mock("../agents/agent-paths.js", () => ({
  resolveTheClawAgentDir: () => process.env.THECLAW_AGENT_DIR ?? "/tmp/theclaw-agent",
}));

vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.THECLAW_STATE_DIR ?? "/tmp/theclaw-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  return {
    upsertAuthProfile: (params: { profileId: string; credential: unknown; agentDir?: string }) => {
      const agentDir = params.agentDir ?? process.env.THECLAW_AGENT_DIR ?? "/tmp/theclaw-agent";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under THECLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("theclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-oauth-sync-"));
    ...
    process.env.THECLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-oauth-nosync-"));
    process.env.THECLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside THECLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-oauth-external-"));
    process.env.THECLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* THECLAW_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("theclaw-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("theclaw-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  ...
  it("writes to THECLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("theclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
