...
import { TheClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: TheClawConfig): {
  config: TheClawConfig;
  removed: string[];
} {
  const parsed = TheClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: TheClawConfig): void {
