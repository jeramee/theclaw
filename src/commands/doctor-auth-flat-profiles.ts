...
function listAuthProfileRepairCandidates(cfg: TheClawConfig): AuthProfileRepairCandidate[] {
  const candidates = new Map<string, AuthProfileRepairCandidate>();
  addCandidate(candidates, resolveTheClawAgentDir());
...
export async function maybeRepairLegacyFlatAuthProfileStores(params: {
  ...
  note(
    [
      ...
      `- The gateway expects the canonical version/profiles store; ${formatCliCommand("theclaw doctor --fix")} rewrites this legacy shape with a backup.`,
    ].join("\n"),
    "Auth profiles",
  );
