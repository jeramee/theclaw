...
beforeAll(() => {
  envSnapshot = captureEnv(["THECLAW_PROFILE"]);
  process.env.THECLAW_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByTheClaw: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/theclaw/pnpm-lock.yaml",
        markerPath: "/tmp/theclaw/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.THECLAW_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.theclaw.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.theclaw.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.THECLAW_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/theclaw-root.js", () => ({
  resolveTheClawPackageRoot: vi.fn().mockResolvedValue("/tmp/theclaw"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("theclaw status --all") ||
          line.includes("theclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("theclaw --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("THECLAW_GATEWAY_TOKEN", "abcd1234", async () => {
