...
export const resolveTheClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadTheClawPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("../infra/theclaw-root.js", () => ({
  ...
  resolveTheClawPackageRootSync: vi.fn(() => "/tmp/theclaw"),
}));
...
beforeEach(() => {
  ...
  resolveTheClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.THECLAW_STATE_DIR;
  originalUpdateInProgress = process.env.THECLAW_UPDATE_IN_PROGRESS;
  process.env.THECLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-doctor-state-"));
