...
function usesClaudeCliModelSelection(cfg: TheClawConfig): boolean {
...
function hasClaudeCliConfigSignals(cfg: TheClawConfig): boolean {
...
function resolveClaudeCliCommand(cfg: TheClawConfig): string {
...
function formatWorkspaceHealthLine(workspaceDir: string, health: ClaudeCliDirHealth): string {
  ...
  if (health === "missing") {
    return `- Workspace: ${display} (missing; TheClaw will create it on first run).`;
  }
...
export function noteClaudeCliHealth(
  ...
  if (!storedProfile) {
    lines.push(`- TheClaw auth profile: missing (${CLAUDE_CLI_PROFILE_ID}) in ${authStorePath}.`);
    fixHints.push(
    ...
    lines.push(
      `- TheClaw auth profile: ${CLAUDE_CLI_PROFILE_ID} is wired to provider "${storedProfile.provider}" instead of "${CLAUDE_CLI_PROVIDER}".`,
    );
    fixHints.push(
    ...
    lines.push(
      `- TheClaw auth profile: ${CLAUDE_CLI_PROFILE_ID} (provider ${CLAUDE_CLI_PROVIDER}).`,
    );
  }
