...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => "/tmp/theclaw-migrate-command-test",
}));
...
describe("migrateApplyCommand", () => {
  ...
  beforeEach(async () => {
    await fs.rm("/tmp/theclaw-migrate-command-test", { force: true, recursive: true });
    ...
    mocks.backupCreateCommand.mockResolvedValue({ archivePath: "/tmp/theclaw-backup.tgz" });
  });
  ...
  it("creates a verified backup before applying a conflict-free migration", async () => {
    ...
    expect(mocks.provider.apply).toHaveBeenCalledWith(
      expect.objectContaining({
        backupPath: "/tmp/theclaw-backup.tgz",
        reportDir: expect.stringContaining("/migration/hermes/"),
      }),
      planned,
    );
    expect(result.backupPath).toBe("/tmp/theclaw-backup.tgz");
  });
