...
function makeTempDir() {
  return makeTrackedTempDir("theclaw-doctor-plugin-manifests", tempDirs);
}
...
function makePluginWorkspace() {
  ...
  return {
    workspaceDir,
    pluginsRoot: path.join(workspaceDir, ".theclaw", "extensions"),
    env: {
      ...process.env,
...
function configWithPluginLoadPath(pluginRoot: string): TheClawConfig {
  return {
    plugins: {
      load: {
        paths: [pluginRoot],
...
describe("doctor plugin manifest legacy contract repair", () => {
  ...
  it("rewrites legacy top-level capability keys into contracts", async () => {
    ...
    const next = JSON.parse(fs.readFileSync(path.join(root, "theclaw.plugin.json"), "utf-8")) as {
