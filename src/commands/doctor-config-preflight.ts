...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".theclaw");
  const targetPath = path.join(targetDir, "theclaw.json");
  try {
...
export async function runDoctorConfigPreflight(
  ...
  if (
    ...
    note("Removed non-JSON prefix from theclaw.json; original saved as .clobbered.*.", "Config");
