...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    isCodexNativeWebSearchRelevant: vi.fn(({ config }: { config: TheClawConfig }) =>
      Boolean(config.auth?.profiles?.["openai-codex:default"]),
    ),
    setupChannels: vi.fn(async (cfg: TheClawConfig) => cfg),
  };
});
...
vi.mock("../config/config.js", () => ({
  CONFIG_PATH: "~/.theclaw/theclaw.json",
...
vi.mock("./onboard-helpers.js", () => ({
  DEFAULT_WORKSPACE: "~/.theclaw/workspace",
  applyWizardMetadata: (cfg: TheClawConfig) => cfg,
...
function setupBaseWizardState(config: TheClawConfig = {}) {
...
describe("runConfigureWizard", () => {
  beforeEach(() => {
    ...
    mocks.setupSearch.mockImplementation(async (cfg: TheClawConfig) => cfg);
  });
  ...
  it("persists provider-owned web search config changes returned by setupSearch", async () => {
    setupBaseWizardState();
    mocks.setupSearch.mockImplementation(async (cfg: TheClawConfig) =>
  ...
  it("retries without dropping nested plugin config written during wizard flow (issue #64188)", async () => {
    ...
    expect(retryCall.nextConfig).toMatchObject({
      agents: {
        defaults: {
          workspace: expect.stringContaining("/.theclaw/workspace"),
        },
      },
      plugins: {
        entries: {
