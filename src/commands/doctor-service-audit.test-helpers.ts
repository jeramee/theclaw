...
export function readEmbeddedGatewayTokenForTest(
  ...
  return isEnvironmentFileOnlySource(command?.environmentValueSources?.THECLAW_GATEWAY_TOKEN)
    ? undefined
    : normalizeOptionalString(command?.environment?.THECLAW_GATEWAY_TOKEN);
}
