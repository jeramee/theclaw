...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.theclaw.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.theclaw.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("skips gateway service install when a system TheClaw gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        platform: "linux",
        label: "theclaw-gateway.service",
        detail: "unit: /etc/systemd/system/theclaw-gateway.service",
        scope: "system",
        marker: "theclaw",
        legacy: false,
      },
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      expect.stringContaining("System-level TheClaw gateway service detected"),
      "Gateway",
    );
  });
