...
function collectPackagedRuntimeDepsRepairPluginIds(params: {
  ...
  for (const entry of fs.readdirSync(params.bundledPluginsDir, { withFileTypes: true })) {
    ...
    try {
      manifest = JSON.parse(
        fs.readFileSync(path.join(pluginDir, "theclaw.plugin.json"), "utf-8"),
...
export async function maybeRepairBundledPluginRuntimeDeps(params: {
  ...
  if (conflicts.length > 0) {
    ...
    note(
      [
        ...
        ...conflictLines,
        `Update bundled plugins and rerun ${formatCliCommand("theclaw doctor")}.`,
      ].join("\n"),
      "Bundled plugins",
    );
  }
  ...
  note(
    [
      ...
      `Fix: run ${formatCliCommand("theclaw doctor --fix")} to install them.`,
    ].join("\n"),
    "Bundled plugins",
  );
  ...
  try {
    ...
    progress = createCliProgress({
      ...
      indeterminate: true,
      enabled: process.env.VITEST !== "true" || process.env.THECLAW_TEST_RUNTIME_LOG === "1",
    });
