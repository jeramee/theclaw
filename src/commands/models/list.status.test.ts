...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTheClawAgentDir: vi.fn().mockReturnValue("/tmp/theclaw-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/theclaw-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/theclaw-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/theclaw-agent/auth-profiles.json"),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/theclaw-dev/theclaw.json",
    }),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTheClawAgentDir: mocks.resolveTheClawAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/theclaw-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTheClawAgentDir).toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-6");
    expect(payload.configPath).toBe("/tmp/theclaw-dev/theclaw.json");
    expect(payload.auth.storePath).toBe("/tmp/theclaw-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/theclaw-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/theclaw-agent-custom");
