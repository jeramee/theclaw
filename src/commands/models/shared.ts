...
export async function loadValidConfigOrThrow(): Promise<TheClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: TheClawConfig) => TheClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: TheClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: TheClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("theclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
