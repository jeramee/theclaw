...
vi.mock("../provider-auth-helpers.js", () => {
  ...
  return {
    ...
    applyProviderAuthConfigPatch: vi.fn(
      (cfg: TheClawConfig, patch: unknown, options?: { replaceDefaultModels?: boolean }) => {
    ...
    applyDefaultModel: vi.fn((cfg: TheClawConfig, model: string) => ({
      ...cfg,
      agents: {
        ...cfg.agents,
        defaults: {
...
describe("modelsAuthLoginCommand", () => {
  ...
  let currentConfig: TheClawConfig;
  let lastUpdatedConfig: TheClawConfig | null;
  ...
  beforeEach(() => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/theclaw/agents/main");
    mocks.resolveAgentWorkspaceDir.mockReturnValue("/tmp/theclaw/workspace");
    mocks.resolveDefaultAgentWorkspaceDir.mockReturnValue("/tmp/theclaw/workspace");
    mocks.isRemoteEnvironment.mockReturnValue(false);
    mocks.loadValidConfigOrThrow.mockImplementation(async () => currentConfig);
    mocks.updateConfig.mockImplementation(
      async (mutator: (cfg: TheClawConfig) => TheClawConfig) => {
  ...
  function useCoderAgentConfig() {
    currentConfig = {
      agents: {
        list: [{ id: "main" }, { id: "coder", workspace: "/tmp/theclaw/workspaces/coder" }],
      },
    };
    const originalConfig = currentConfig;
    mocks.resolveAgentDir.mockImplementation((_cfg: TheClawConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/theclaw/agents/coder" : "/tmp/theclaw/agents/main",
    );
    mocks.resolveAgentWorkspaceDir.mockImplementation((_cfg: TheClawConfig, agentId: string) =>
      agentId === "coder" ? "/tmp/theclaw/workspaces/coder" : "/tmp/theclaw/workspace",
    );
  ...
  it("runs plugin-owned openai-codex login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/theclaw/agents/main");
    expect(mocks.clearAuthProfileCooldown).toHaveBeenCalledWith({
      store: fakeStore,
      profileId: "openai-codex:user@example.com",
      agentDir: "/tmp/theclaw/agents/main",
    });
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Tip: Codex-capable models can use native Codex web search. Enable it with theclaw configure --section web (recommended mode: cached). Docs: https://docs.theclaw.ai/tools/web",
    );
  });
  ...
  it("uses the requested agent store for provider auth login", async () => {
    ...
    expect(mocks.loadAuthProfileStoreForRuntime).toHaveBeenCalledWith("/tmp/theclaw/agents/coder");
    expect(runProviderAuth).toHaveBeenCalledWith(
      expect.objectContaining({
        agentDir: "/tmp/theclaw/agents/coder",
        workspaceDir: "/tmp/theclaw/workspaces/coder",
      }),
    );
    expect(mocks.upsertAuthProfile).toHaveBeenCalledWith(
      expect.objectContaining({ agentDir: "/tmp/theclaw/agents/coder" }),
    );
  });
  ...
  it("loads the owning plugin for an explicit provider even in a clean config", async () => {
    ...
    expect(mocks.resolvePluginProviders).toHaveBeenCalledWith(
      expect.objectContaining({
        config: {},
        workspaceDir: "/tmp/theclaw/workspace",
  ...
  it("runs the requested anthropic cli auth method with the full login context", async () => {
    ...
    const runClaudeCliMigration = vi.fn().mockImplementation(async (ctx) => {
      ...
      expect(ctx.agentDir).toBe("/tmp/theclaw/agents/main");
      expect(ctx.workspaceDir).toBe("/tmp/theclaw/workspace");
  ...
  it("reports loaded plugin providers when requested provider is unavailable", async () => {
    ...
    await expect(modelsAuthLoginCommand({ provider: "anthropic" }, runtime)).rejects.toThrow(
      'Unknown provider "anthropic". Loaded providers: openai-codex. Verify plugins via `theclaw plugins list --json`.',
    );
  });
  ...
  it("writes pasted Anthropic setup-tokens and logs the preference note", async () => {
    ...
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic setup-token auth is supported in TheClaw.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "TheClaw prefers Claude CLI reuse when it is available on the host.",
    );
    expect(runtime.log).toHaveBeenCalledWith(
      "Anthropic staff told us this TheClaw path is allowed again.",
    );
  });
  ...
  it("rejects an unknown agent before prompting for pasted tokens", async () => {
    ...
    await expect(
      ...
      'Unknown agent id "missing". Use "theclaw agents list" to see configured agents.',
    );
