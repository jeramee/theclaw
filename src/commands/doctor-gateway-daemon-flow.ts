...
function renderBlockingSystemGatewayServices(services: ExtraGatewayService[]): string {
  return [
    "System-level TheClaw gateway service detected while the user gateway service is not installed.",
    ...services.map((svc) => `- ${svc.label} (${svc.detail})`),
    "TheClaw will not install a second user-level gateway service automatically.",
    "Run `theclaw gateway status --deep` or `theclaw doctor --deep` to inspect duplicate services.",
...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.THECLAW_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("theclaw gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
