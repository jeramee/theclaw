...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: TheClawConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => TheClawConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: TheClawConfig, sourceConfig: TheClawConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: TheClawConfig;
      effectiveConfig: TheClawConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "theclaw-agent-" });
}

function mockConfig(home: string, storePath: string): TheClawConfig {
  const cfg = {
    agents: {
      defaults: {
