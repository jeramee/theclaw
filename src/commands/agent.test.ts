...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".theclaw", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "theclaw-agent-", skipSessionCleanup: true });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<TheClawConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<TheClawConfig["channels"]>["telegram"]>>,
...
beforeEach(() => {
  ...
  configIoMocks.readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as TheClawConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand", () => {
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly THECLAW-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
    ...
    await withTempHome(async (home) => {
      ...
      expect(callArgs).toEqual(
        expect.objectContaining({
          provider: "openrouter",
          model: "openrouter/auto",
          prompt: "Reply with exactly THECLAW-MODEL-OK",
          modelRun: true,
          promptMode: "none",
          disableTools: true,
        }),
