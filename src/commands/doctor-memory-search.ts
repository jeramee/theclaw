...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("theclaw doctor --fix")} or ${formatCliCommand("theclaw memory status --fix")}`
    : `Verify: ${formatCliCommand("theclaw memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: TheClawConfig): Promise<void> {
  try {
 ...
 * Runs as part of `theclaw doctor` — config-only checks where possible;
    ...
    if (!qmdCheck.available) {
      note(
        [
          ...
          `- Set an explicit binary path: ${formatCliCommand("theclaw config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("theclaw config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("theclaw memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (resolved.provider !== "auto") {
      ...
      note(
        [
          ...
          suggestedRemoteProvider
            ? `- Switch to a remote provider: ${formatCliCommand(`theclaw config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
    ...
    note(
      [
        ...
        `- Configure credentials: ${formatCliCommand("theclaw configure --section model")}`,
        `- To disable: ${formatCliCommand("theclaw config set agents.defaults.memorySearch.enabled false")}`,
        "",
        `Verify: ${formatCliCommand("theclaw memory status --deep")}`,
      ].join("\n"),
      "Memory search",
