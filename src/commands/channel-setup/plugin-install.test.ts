...
import { bundledPluginRoot, bundledPluginRootAt } from "theclaw/plugin-sdk/test-fixtures";
...
const discoverTheClawPlugins = vi.fn((_args?: unknown) => ({ candidates: [], diagnostics: [] }));
vi.mock("../../plugins/discovery.js", () => ({
  clearPluginDiscoveryCache: () => clearPluginDiscoveryCache(),
  discoverTheClawPlugins: (args: unknown) => discoverTheClawPlugins(args),
}));

import fs from "node:fs";
import type { ChannelPluginCatalogEntry } from "../../channels/plugins/catalog.js";
import type { TheClawConfig } from "../../config/config.js";
import { loadTheClawPlugins } from "../../plugins/loader.js";
...
const bundledChatNpmSpec = "@theclaw/bundled-chat@1.2.3";
  ...
  resolveBundledPluginSources.mockReturnValue(
    new Map([
      [
        ...
        {
          pluginId: "bundled-chat",
          localPath: bundledPluginRootAt("/opt/theclaw", "bundled-chat"),
          npmSpec: bundledChatNpmSpec,
        },
      ],
    ]),
...
function createManifestRecord(
  ...
  return {
    ...
    hooks: [],
    origin: "bundled",
    rootDir: `/tmp/theclaw-test/${id}`,
    source: `/tmp/theclaw-test/${id}/index.ts`,
    manifestPath: `/tmp/theclaw-test/${id}/theclaw.plugin.json`,
    ...rest,
  };
}
...
function expectSetupSnapshotDoesNotScopeToPlugin(params: {
  ...
  expect(loadTheClawPlugins).toHaveBeenCalledWith(
    expect.not.objectContaining({
      onlyPluginIds: [params.pluginId],
    }),
  );
  const firstLoadCall = vi.mocked(loadTheClawPlugins).mock.calls[0]?.[0] as
...
beforeEach(() => {
  ...
  discoverTheClawPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
...
async function runInitialValueForChannel(channel: "dev" | "beta") {
  ...
  const select = vi.fn((async <T extends string>() => "skip" as T) as WizardPrompter["select"]);
  const prompter = makePrompter({ select: select as unknown as WizardPrompter["select"] });
  const cfg: TheClawConfig = { update: { channel } };
...
describe("ensureChannelSetupPluginInstalled", () => {
  it("installs from npm and enables the plugin", async () => {
    ...
    const cfg: TheClawConfig = { plugins: { allow: ["bundled-chat"] } };
  ...
  it("installs npm channel plugins into the active profile extensions dir", async () => {
    ...
    const profileStateDir = "/tmp/theclaw-ledger-channel";
    process.env.THECLAW_STATE_DIR = profileStateDir;
    vi.mocked(fs.existsSync).mockReturnValue(false);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: true,
      pluginId: "bundled-chat",
  ...
  it("defaults to bundled local path on beta channel when available", async () => {
    ...
    const cfg: TheClawConfig = { update: { channel: "beta" } };
    vi.mocked(fs.existsSync).mockReturnValue(false);
    ...
    expect(select).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        options: expect.arrayContaining([
          expect.objectContaining({
            value: "local",
            hint: bundledPluginRootAt("/opt/theclaw", "bundled-chat"),
          }),
        ]),
      }),
    );
  ...
  it("clears discovery cache before reloading the setup plugin registry", () => {
    ...
    expect(clearPluginDiscoveryCache.mock.invocationCallOrder[0]).toBeLessThan(
      vi.mocked(loadTheClawPlugins).mock.invocationCallOrder[0] ?? Number.POSITIVE_INFINITY,
    );
  });
