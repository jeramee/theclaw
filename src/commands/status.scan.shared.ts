...
export function hasExplicitMemorySearchConfig(cfg: TheClawConfig, agentId: string): boolean {
  if (
    cfg.agents?.defaults &&
...
export function resolveMemoryPluginStatus(cfg: TheClawConfig): MemoryPluginStatus {
...
export async function resolveSharedMemoryStatusSnapshot(params: {
  ...
  resolveMemoryConfig: (cfg: TheClawConfig, agentId: string) => { store: { path: string } } | null;
