...
const SEARCH_TOOL = "https://docs.theclaw.ai/mcp.SearchTheClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.theclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('theclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.theclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('theclaw docs "your query"')}`);
    }
    return;
  }
