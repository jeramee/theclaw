...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "THECLAW_ENV_TEST_A";
    const keyB = "THECLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "THECLAW_ENV_TEST_ADDED";
  ...
  it("withEnv applies values only inside callback", () => {
    const key = "THECLAW_ENV_TEST_SYNC";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "THECLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "THECLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "THECLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "THECLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "THECLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "theclaw-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousTheClawHome = process.env.THECLAW_HOME;
    const previousStateDir = process.env.THECLAW_STATE_DIR;
    const previousBundledDir = process.env.THECLAW_BUNDLED_PLUGINS_DIR;
    process.env.THECLAW_HOME = "/srv/theclaw-home";
    process.env.THECLAW_STATE_DIR = "/srv/theclaw-state";
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = "/srv/theclaw-bundled";
    ...
    try {
      ...
      expect(env.THECLAW_HOME).toBeUndefined();
      expect(env.THECLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.THECLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("THECLAW_HOME", previousTheClawHome);
      restoreEnvKey("THECLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("THECLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { THECLAW_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processTheClawHome: process.env.THECLAW_HOME,
          processBundledDir: process.env.THECLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.THECLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processTheClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.THECLAW_HOME).toBe("/srv/theclaw-home");
    } finally {
      restoreEnvKey("THECLAW_HOME", previousTheClawHome);
    }
  });
