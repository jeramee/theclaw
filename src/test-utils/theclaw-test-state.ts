...
export type TheClawTestStateLayout = "home" | "state-only" | "split";

export type TheClawTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "gateway-loopback"
...
export type TheClawTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: TheClawTestStateLayout;
  scenario?: TheClawTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "theclaw-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "theclaw.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "theclaw.json"),
...
function scenarioConfig(options: TheClawTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "theclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: TheClawTestStateOptions): Record<string, string | undefined> {
...
export async function createTheClawTestState(
  options: TheClawTestStateOptions = {},
): Promise<TheClawTestState> {
...
export async function withTheClawTestState<T>(
  options: TheClawTestStateOptions,
  fn: (state: TheClawTestState) => Promise<T>,
): Promise<T> {
  const state = await createTheClawTestState(options);
  try {
