...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses THECLAW_HOME for tilde expansion", () => {
    vi.stubEnv("THECLAW_HOME", "/srv/theclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/theclaw-home", "cron", "jobs.json"));
  });
});
