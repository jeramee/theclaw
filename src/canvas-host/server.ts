...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.theclawCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.theclawCanvasA2UIAction &&
        typeof window.theclawCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.theclawSendUserAction === "function";
  ...
  window.addEventListener("theclaw:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android TheClaw node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.theclawSendUserAction === "function"
        ? window.theclawSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.THECLAW_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.THECLAW_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>TheClaw Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
