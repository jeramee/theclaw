...
describe("canvas host state dir defaults", () => {
  ...
  it("uses THECLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("theclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("TheClaw Canvas");
      } finally {
        await handler.close();
      }
    });
