...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    const resolved = resolveCommandEnv({
      argv: ["node", "script.js"],
      baseEnv: {
        THECLAW_BASE_ENV: "base",
        THECLAW_TO_REMOVE: undefined,
      },
      env: {
        THECLAW_TEST_ENV: "ok",
      },
    ...
    expect(resolved.THECLAW_BASE_ENV).toBe("base");
    expect(resolved.THECLAW_TEST_ENV).toBe("ok");
    expect(resolved.THECLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.THECLAW_CLI).toBe(THECLAW_CLI_ENV_VALUE);
  });
