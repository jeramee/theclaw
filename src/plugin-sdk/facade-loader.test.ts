...
const originalBundledPluginsDir = process.env.THECLAW_BUNDLED_PLUGINS_DIR;
const originalDisableBundledPlugins = process.env.THECLAW_DISABLE_BUNDLED_PLUGINS;
const originalPluginStageDir = process.env.THECLAW_PLUGIN_STAGE_DIR;
const trustedBundledFixturesRoot = path.resolve("dist-runtime", "extensions");
const FACADE_LOADER_GLOBAL = "__theclawTestLoadBundledPluginPublicSurfaceModuleSync";
const STAGED_RUNTIME_DEP_NAME = "theclaw-facade-loader-runtime-dep";
...
function useBundledPluginDirOverrideForTest(dir: string): void {
  process.env.THECLAW_BUNDLED_PLUGINS_DIR = dir;
...
function createPackagedBundledPluginDirWithStagedRuntimeDep(params: {
  ...
  const env = {
    ...process.env,
    THECLAW_BUNDLED_PLUGINS_DIR: trustedBundledFixturesRoot,
    THECLAW_PLUGIN_STAGE_DIR: stageRoot,
  };
  trustedBundledFixtureDirs.push(pluginRoot);
  fs.mkdirSync(pluginRoot, { recursive: true });

  writeJsonFile(path.join(pluginRoot, "package.json"), {
    name: "@theclaw/plugin-demo",
    version: "0.0.0",
    type: "module",
    dependencies: {
...
afterEach(() => {
  ...
  if (originalBundledPluginsDir === undefined) {
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = originalBundledPluginsDir;
  }
  ...
  if (originalPluginStageDir === undefined) {
    ...
    process.env.THECLAW_PLUGIN_STAGE_DIR = originalPluginStageDir;
  }
});

describe("plugin-sdk facade loader", () => {
  it("honors trusted bundled plugin dir overrides", () => {
    const overrideA = createBundledPluginDir("theclaw-facade-loader-a-", "override-a");
    const overrideB = createBundledPluginDir("theclaw-facade-loader-b-", "override-b");
  ...
  it("falls back to package source surfaces when an override dir lacks a bundled plugin", () => {
    const overrideDir = createTrustedBundledFixtureRoot("theclaw-facade-loader-empty-");
  ...
  it("shares loaded facade ids with facade-runtime", () => {
    const dir = createBundledPluginDir("theclaw-facade-loader-ids-", "identity-check");
  ...
  it("uses the runtime-supported Jiti boundary for Windows dist facade loads", () => {
    const bundledPluginsDir = createTrustedBundledFixtureRoot(
      "theclaw-facade-loader-windows-dist-",
    );
  ...
  it("loads built bundled sync public surfaces through staged runtime deps", async () => {
    const fixture = createPackagedBundledPluginDirWithStagedRuntimeDep({
      marker: "staged",
      prefix: "theclaw-facade-loader-runtime-deps-",
    });
    useBundledPluginDirOverrideForTest(fixture.bundledPluginsDir);
    process.env.THECLAW_PLUGIN_STAGE_DIR = fixture.stageRoot;
  ...
  it("loads built bundled async public surfaces through staged runtime deps", async () => {
    const fixture = createPackagedBundledPluginDirWithStagedRuntimeDep({
      marker: "staged",
      prefix: "theclaw-facade-loader-built-async-",
    });
  ...
  it("breaks circular facade re-entry during module evaluation", () => {
    const dir = createCircularPluginDir("theclaw-facade-loader-circular-");
    useBundledPluginDirOverrideForTest(dir);
  ...
  it("clears the cache on load failure so retries re-execute", () => {
    const dir = createThrowingPluginDir("theclaw-facade-loader-throw-");
    useBundledPluginDirOverrideForTest(dir);
