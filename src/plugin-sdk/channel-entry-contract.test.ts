...
import type { TheClawPluginApi, PluginRegistrationMode } from "../plugins/types.js";
...
function createApi(registrationMode: PluginRegistrationMode): TheClawPluginApi {
  return {
    registrationMode,
    runtime: { registrationMode } as unknown as PluginRuntime,
    registerChannel: vi.fn(),
  } as unknown as TheClawPluginApi;
}
...
function createBundledChannelEntry(params: {
  importerPath: string;
  pluginId: string;
  registerCliMetadata?: (api: TheClawPluginApi) => void;
  registerFull?: (api: TheClawPluginApi) => void;
}) {
...
describe("defineBundledChannelEntry", () => {
  it("loads runtime sidecars during discovery registration", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-bundled-entry-runtime-"));
    ...
    const registerCliMetadata = vi.fn<(api: TheClawPluginApi) => void>();
    const registerFull = vi.fn<(api: TheClawPluginApi) => void>();
...
async function expectBuiltArtifactNodeRequireFastPath(
  ...
  vi.stubEnv("THECLAW_PLUGIN_LOAD_PROFILE", "1");
  ...
  try {
    ...
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-channel-entry-contract-"));
    tempDirs.push(tempRoot);
...
describe("loadBundledEntryExportSync", () => {
  ...
  it("normalizes Windows absolute sidecar paths before Jiti loads them", async () => {
    ...
    vi.doMock("../infra/boundary-file-read.js", () => ({
      openBoundaryFileSync: () => ({
        ok: true,
        path: "C:\\Users\\alice\\theclaw\\dist\\extensions\\feishu\\helper.ts",
    ...
    try {
      ...
      expect(
        channelEntryContract.loadBundledEntryExportSync<number>(
          "file:///C:/Users/alice/theclaw/dist/extensions/feishu/index.js",
          {
            specifier: "./helper.ts",
            exportName: "load",
          },
      ...
      expect(jitiLoad).toHaveBeenCalledWith(
        "file:///C:/Users/alice/theclaw/dist/extensions/feishu/helper.ts",
      );
    } finally {
      platformSpy.mockRestore();
  ...
  it("can disable source-tree fallback for dist bundled entry checks", () => {
    ...
    fs.writeFileSync(path.join(tempRoot, "package.json"), '{"name":"theclaw"}\n', "utf8");
    const pluginRoot = path.join(tempRoot, "dist", "extensions", "telegram");
    ...
    vi.stubEnv("THECLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK", "1");
