...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked THECLAW_HOME.
  delete process.env.THECLAW_HOME;
  process.env.THECLAW_STATE_DIR = path.join(base, ".theclaw");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "theclaw-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".theclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
