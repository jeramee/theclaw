...
function listAgents(cfg: TheClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: TheClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<TheClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: TheClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: TheClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: TheClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<TheClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  TheClawConfig,
  {
    bindingsRef: TheClawConfig["bindings"];
    agentsRef: TheClawConfig["agents"];
    sessionRef: TheClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: TheClawConfig): Map<string, ResolvedAgentRoute> {
