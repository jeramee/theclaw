...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("theclaw status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("theclaw security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set THECLAW_GATEWAY_TOKEN/THECLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("theclaw security audit --deep")}".`,
    });
  }
  return findings;
}
