...
function stubSlackPlugin(params: {
  resolveAccount: (cfg: TheClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: TheClawConfig, accountId: string | null | undefined) => unknown;
  isConfigured?: (account: unknown, cfg: TheClawConfig) => boolean;
}) {
...
function makeSlackHttpConfig(): TheClawConfig {
  return {
    channels: {
      slack: {
        enabled: true,
    ...
    const cases = [
      {
        ...
        plugin: (sourceConfig: TheClawConfig) =>
          stubSlackPlugin({
            inspectAccount: (cfg) => {
