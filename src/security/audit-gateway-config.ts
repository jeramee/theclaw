...
type CollectDangerousConfigFlags = (cfg: TheClawConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.THECLAW_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.THECLAW_GATEWAY_PASSWORD);
