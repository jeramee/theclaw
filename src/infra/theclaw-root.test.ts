...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__theclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "theclaw-root");
...
function setPackageRoot(root: string, name = "theclaw") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./theclaw-root.js").resolveTheClawPackageRootSync,
  asyncResolver: typeof import("./theclaw-root.js").resolveTheClawPackageRoot,
  opts: Parameters<typeof import("./theclaw-root.js").resolveTheClawPackageRootSync>[0],
...
vi.mock("./theclaw-root.fs.runtime.js", () => ({
  openClawRootFsSync: mockFsModule(),
  openClawRootFs: mockFsPromisesModule(),
}));

describe("resolveTheClawPackageRoot", () => {
  let resolveTheClawPackageRoot: typeof import("./theclaw-root.js").resolveTheClawPackageRoot;
  let resolveTheClawPackageRootSync: typeof import("./theclaw-root.js").resolveTheClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveTheClawPackageRoot, resolveTheClawPackageRootSync } =
      await import("./theclaw-root.js"));
  });
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "theclaw");
        const pkgRoot = path.join(project, "node_modules", "theclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "theclaw");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "theclaw.mjs")));
    ...
    {
      name: "falls through from a non-theclaw moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-theclaw");
    ...
    {
      name: "returns null for non-theclaw package roots",
      setup: () => {
        const pkgRoot = fx("not-theclaw");
        setPackageRoot(pkgRoot, "not-theclaw");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "theclaw.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "theclaw");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
    ...
    await expectResolvedPackageRoot(
      resolveTheClawPackageRootSync,
      resolveTheClawPackageRoot,
      opts,
      expected,
    );
  });
