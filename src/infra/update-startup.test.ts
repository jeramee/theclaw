...
vi.mock("./theclaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("./theclaw-root.js")>("./theclaw-root.js");
  return {
    ...actual,
...
describe("update-startup", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "theclaw-update-check-suite-" });
  let tempDir: string;
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv([
      "THECLAW_NO_AUTO_UPDATE",
      "THECLAW_STATE_DIR",
      "NODE_ENV",
      "VITEST",
    ]);
    process.env.THECLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveTheClawPackageRoot } = await import("./theclaw-root.js"));
    ...
    vi.mocked(resolveTheClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveTheClawPackageRoot).mockResolvedValue("/opt/theclaw");
  ...
  it("honors THECLAW_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.THECLAW_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    expect(runAutoUpdate).not.toHaveBeenCalled();
    expect(log.info).toHaveBeenCalledWith(
      "auto-update disabled by THECLAW_NO_AUTO_UPDATE",
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/theclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
