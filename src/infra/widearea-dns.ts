...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.THECLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*theclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "theclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "theclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "theclaw-gw");
  ...
  records.push(`_theclaw-gw._tcp IN PTR ${instanceLabel}._theclaw-gw._tcp`);
  records.push(`${instanceLabel}._theclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._theclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; theclaw-content-hash: ${contentHash}\n${contentBody}`;
}
