...
export function emitGatewayRestart(reasonOverride?: string): boolean {
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerTheClawRestart() uses schtasks to restart the gateway.
...
export function triggerTheClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.THECLAW_SYSTEMD_UNIT,
      process.env.THECLAW_PROFILE,
    );
  ...
  const label =
    process.env.THECLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.THECLAW_PROFILE);
