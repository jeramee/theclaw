...
vi.mock("node:child_process", async () => {
  const { mockNodeChildProcessSpawnSync } = await import("theclaw/plugin-sdk/test-node-mocks");
...
let triggerTheClawRestart: typeof import("./restart.js").triggerTheClawRestart;
...
beforeAll(async () => {
  ...
  ({ triggerTheClawRestart } = await import("./restart.js"));
});
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-theclaw/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "ctheclaw", `p${stalePidB}`, "ctheclaw-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
  ...
  it("uses explicit port override when provided", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePid}`, "ctheclaw"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerTheClawRestart", () => {
  it("continues when launchctl bootstrap reports the service is already loaded", () => {
    ...
    const result = triggerTheClawRestart();
    ...
    expect(result).toEqual({
      ...
      tried: [
        ...
        `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.theclaw.gateway.plist`,
        `launchctl kickstart gui/${uid}/ai.theclaw.gateway`,
      ],
    });
  });
});
