...
describe("package dist inventory", () => {
  it("tracks missing and stale dist files", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-" }, async (packageRoot) => {
  ...
  it("keeps npm-omitted dist artifacts out of the inventory", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-pack-" }, async (packageRoot) => {
      ...
      const omittedRuntimeDepsStamp = path.join(
        ...
        "extensions",
        "discord",
        ".theclaw-runtime-deps-stamp.json",
      );
      ...
      const omittedRuntimeDepsTempFile = path.join(
        ...
        "extensions",
        "discord",
        ".theclaw-runtime-deps-backup-node_modules-old",
        "left-pad",
        "index.js",
      );
      const omittedRuntimeDepsTempSymlink = path.join(
        ...
        "extensions",
        "amazon-bedrock",
        ".theclaw-runtime-deps-copy-KZmXaz",
        "node_modules",
        ".bin",
        "fxparser",
      );
  ...
  it("ignores runtime-created install staging dirs during installed dist verification", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-stage-" }, async (packageRoot) => {
  ...
  it("matches install-stage paths case-insensitively across path segments", () => {
    expect(
      isBundledRuntimeDepsInstallStagePath(
        "dist/extensions/brave/.theclaw-install-stage/node_modules/typebox/package.json",
      ),
    ).toBe(true);
    expect(
      isBundledRuntimeDepsInstallStagePath(
        "dist/Extensions/browser/.THECLAW-INSTALL-STAGE-AbC123/node_modules/playwright-core/package.json",
      ),
    ).toBe(true);
    expect(
      isBundledRuntimeDepsInstallStagePath(
    ...
    expect(
      isBundledRuntimeDepsInstallStagePath(
        "dist/extensions/browser/.theclaw-runtime-deps-copy-AbC123/package.json",
      ),
    ).toBe(false);
    expect(isBundledRuntimeDepsInstallStagePath("dist/extensions/.theclaw-install-stage")).toBe(
      false,
    );
  });

  it("rejects pre-populated install-stage debris at publish time", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-stage-publish-" }, async (packageRoot) => {
      ...
      await expect(collectBundledRuntimeDepsStagingDebrisPaths(packageRoot)).resolves.toEqual([
        "dist/extensions/browser/.theclaw-install-stage-AbC123",
        "dist/extensions/evil/.theclaw-install-stage",
      ]);
  ...
  it("rejects mixed-case install-stage debris on case-sensitive release builders", async () => {
    await withTempDir(
      { prefix: "theclaw-dist-inventory-stage-extensions-case-" },
      async (packageRoot) => {
    ...
    await withTempDir(
      { prefix: "theclaw-dist-inventory-stage-root-case-" },
      async (packageRoot) => {
        const mixedCaseStage = path.join(
          packageRoot,
          "Dist",
  ...
  it("treats a missing dist/extensions tree as no staging debris", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-no-extensions-" }, async (packageRoot) => {
  ...
  it("fails closed when the inventory is missing", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-missing-" }, async (packageRoot) => {
      await fs.mkdir(path.join(packageRoot, "dist"), { recursive: true });
      await expect(collectPackageDistInventoryErrors(packageRoot)).resolves.toEqual([
  ...
  it("rejects symlinked dist entries", async () => {
    await withTempDir({ prefix: "theclaw-dist-inventory-symlink-" }, async (packageRoot) => {
