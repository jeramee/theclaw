...
export const POSIX_THECLAW_TMP_DIR = "/tmp/theclaw";

type ResolvePreferredTheClawTmpDirOptions = {
...
export function resolvePreferredTheClawTmpDir(
  options: ResolvePreferredTheClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "theclaw" : `theclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[theclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback TheClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback TheClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_THECLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_THECLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_THECLAW_TMP_DIR)) {
      return POSIX_THECLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_THECLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_THECLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_THECLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_THECLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_THECLAW_TMP_DIR;
