...
const DEFAULT_VAPID_SUBJECT = "mailto:theclaw@localhost";
...
function resolveVapidSubjectFromEnv(): string {
  return process.env.THECLAW_VAPID_SUBJECT || DEFAULT_VAPID_SUBJECT;
}

export function resolveVapidPublicKeyFromEnv(): string | undefined {
  return process.env.THECLAW_VAPID_PUBLIC_KEY || undefined;
}

export function resolveVapidPrivateKeyFromEnv(): string | undefined {
  return process.env.THECLAW_VAPID_PRIVATE_KEY || undefined;
}
