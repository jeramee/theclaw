...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".theclaw");
...
function sharedMainOpsConfig(sharedStorePath: string): TheClawConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: TheClawConfig = OPS_WORK_CONFIG) {
...
describe("migrateOrphanedSessionKeys", () => {
  ...
  it("is idempotent — running twice produces same result", async () => {
    await withStateFixture(async ({ stateDir }) => {
      ...
      const env = { THECLAW_STATE_DIR: stateDir };
