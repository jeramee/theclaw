...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.THECLAW_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for TheClaw. You can keep running the web gateway without it: \`${formatCliCommand("theclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
