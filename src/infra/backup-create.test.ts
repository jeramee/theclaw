...
import { withTheClawTestState } from "../test-utils/theclaw-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "theclaw-backup-2026-01-01",
    archivePath: "/tmp/theclaw-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/theclaw-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.theclaw",
        "Skipped 1 path:",
        "- workspace: ~/Projects/theclaw (covered by ~/.theclaw)",
        "Created /tmp/theclaw-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.theclaw/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.theclaw/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.theclaw/config.json",
        "- credentials: ~/.theclaw/oauth",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/theclaw.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.theclaw\\");

    expect(filter(String.raw`C:\Users\me\.theclaw\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.theclaw\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withTheClawTestState(
      {
        layout: "state-only",
        prefix: "theclaw-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "theclaw.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(
          entries.some((entry) => entry.endsWith("/state/extensions/demo/theclaw.plugin.json")),
        ).toBe(true);
