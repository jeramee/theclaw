...
function candidateBinDirs(opts: EnsureTheClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // subprocesses keep using the same Node/Bun the current TheClaw process is on.
  try {
  ...
  // Bundled macOS app: `theclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "theclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.THECLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "theclaw"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `theclaw`
...
/**
 * Best-effort PATH bootstrap so skills that require the `theclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTheClawCliOnPath(opts: EnsureTheClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.THECLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.THECLAW_PATH_BOOTSTRAPPED = "1";
