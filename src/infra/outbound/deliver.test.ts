...
import { resolvePreferredTheClawTmpDir } from "../tmp-theclaw-dir.js";
...
const matrixChunkConfig: TheClawConfig = {
  channels: { matrix: { textChunkLimit: 4000 } } as TheClawConfig["channels"],
};

const expectedPreferredTmpRoot = resolvePreferredTheClawTmpDir();
...
async function runChunkedMatrixDelivery(params?: {
  ...
  const cfg: TheClawConfig = {
    channels: { matrix: { textChunkLimit: 2 } } as TheClawConfig["channels"],
  };
...
describe("deliverOutboundPayloads", () => {
  ...
  it("chunks direct adapter text and preserves delivery overrides across sends", async () => {
    ...
    const results = await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 2 } } } as TheClawConfig,
      channel: "matrix",
      to: "!room",
      accountId: "default",
  ...
  it("uses adapter-provided formatted senders and scoped media roots when available", async () => {
    ...
    const textResults = await deliverOutboundPayloads({
      cfg: { channels: { line: {} } } as TheClawConfig,
      channel: "line",
      to: "U123",
      accountId: "default",
    ...
    expect(
      sendFormattedMediaCall?.mediaLocalRoots?.some((root) =>
        root.endsWith(path.join(".theclaw", "workspace-work")),
      ),
    ).toBe(true);
    expect(sendMedia).not.toHaveBeenCalled();
  });

  it("includes TheClaw tmp root in plugin mediaLocalRoots", async () => {
    const sendMatrix = vi.fn().mockResolvedValue({ messageId: "m-media", roomId: "!room" });

    await deliverOutboundPayloads({
      cfg: { channels: { matrix: {} } } as TheClawConfig,
      channel: "matrix",
      to: "!room:example",
  ...
  it("lets explicit formatting options override configured chunking", async () => {
    ...
    await deliverOutboundPayloads({
      cfg: { channels: { matrix: { textChunkLimit: 4000 } } } as TheClawConfig,
      channel: "matrix",
      to: "!room",
      payloads: [{ text: "abcd" }],
  ...
  it("bails out without sending when a concurrent drain already claimed the queue entry", async () => {
    // Regression for theclaw/theclaw#70386: if a reconnect or startup drain
