...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: TheClawConfig | null = null;
  ...
  get(key: string, cfg: TheClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: TheClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: TheClawConfig): void {
