...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred TheClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideTheClaw = path.join(os.tmpdir(), "outside-theclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
