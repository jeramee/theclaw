...
describe("generic current-conversation bindings", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.THECLAW_STATE_DIR;
    testStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-current-bindings-"));
    process.env.THECLAW_STATE_DIR = testStateDir;
