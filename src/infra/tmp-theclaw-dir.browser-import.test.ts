...
describe("tmp-theclaw-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__theclawTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_THECLAW_TMP_DIR, resolvePreferredTheClawTmpDir } from "./src/infra/tmp-theclaw-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_THECLAW_TMP_DIR,
            resolverType: typeof resolvePreferredTheClawTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-theclaw-dir-browser-entry.ts",
      },
      write: false,
    });
