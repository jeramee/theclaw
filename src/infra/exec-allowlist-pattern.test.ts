...
describe("matchesExecAllowlistPattern", () => {
  ...
  it("expands home-prefix patterns", () => {
    const prevTheClawHome = process.env.THECLAW_HOME;
    const prevHome = process.env.HOME;
    process.env.THECLAW_HOME = "/srv/theclaw-home";
    process.env.HOME = "/home/other";
    const openClawHome = path.join(path.resolve("/srv/theclaw-home"), "bin", "tool");
    ...
    try {
      ...
      if (prevTheClawHome === undefined) {
        delete process.env.THECLAW_HOME;
      } else {
        process.env.THECLAW_HOME = prevTheClawHome;
      }
