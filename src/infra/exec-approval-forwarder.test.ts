...
function makeTargetsCfg(targets: Array<{ channel: string; to: string }>): TheClawConfig {
  return {
    approvals: {
      exec: {
        enabled: true,
...
function makeSessionCfg(options: { discordExecApprovalsEnabled?: boolean } = {}): TheClawConfig {
  return {
