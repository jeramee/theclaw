...
describe("unscopedPackageName", () => {
  it.each([
    { value: "@theclaw/matrix", expected: "matrix" },
...
describe("packageNameMatchesId", () => {
  it.each([
    { packageName: "@theclaw/matrix", id: "matrix", expected: true },
    { packageName: "@theclaw/matrix", id: "@theclaw/matrix", expected: true },
    { packageName: "@theclaw/matrix", id: "quietchat", expected: false },
    { packageName: " ", id: "matrix", expected: false },
    { packageName: "@theclaw/matrix", id: " ", expected: false },
...
describe("resolveSafeInstallDir", () => {
  it("resolves install dirs under the base directory", () => {
    expect(
      ...
      ok: true,
      path: path.join("/tmp/plugins", "@theclaw__matrix"),
    });
  });
...
describe("assertCanonicalPathWithinBase", () => {
  it("accepts in-base directories", async () => {
    await withTempDir({ prefix: "theclaw-install-safe-" }, async (baseDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "theclaw-install-safe-" }, async (baseDir) => {
        await withTempDir({ prefix: "theclaw-install-safe-outside-" }, async (outsideDir) => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir({ prefix: "theclaw-install-safe-" }, async (parentDir) => {
