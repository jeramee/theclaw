...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: TheClawConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: TheClawConfig) {
...
function resolveHeartbeatAgents(cfg: TheClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: TheClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: TheClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: TheClawConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: TheClawConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: TheClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
