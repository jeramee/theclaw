...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/theclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/theclaw-state", "devices"),
      pendingPath: path.join("/tmp/theclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/theclaw-state", "devices", "paired.json"),
    });
  });
