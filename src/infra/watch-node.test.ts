...
import { bundledPluginFile } from "theclaw/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "theclaw.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "theclaw-watch-node-" }, async (cwd) => {
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            PATH: "/usr/bin",
            THECLAW_WATCH_MODE: "1",
            THECLAW_WATCH_SESSION: "1700000000000-4242",
            THECLAW_NO_RESPAWN: "1",
            THECLAW_WATCH_COMMAND: "gateway --force",
          }),
        }),
      );
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    const runPromise = runWatch({
      ...
      createWatcher,
      env: {
        LAUNCH_JOB_LABEL: "ai.theclaw.gateway",
        PATH: "/usr/bin",
      },
      lockDisabled: true,
      process: fakeProcess,
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/theclaw/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/theclaw/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[theclaw] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[theclaw] Invalid package config: /tmp/theclaw/.pnpm/chokidar/package.json"],
        ["[theclaw] This usually means a file in node_modules is empty or truncated."],
        ["[theclaw] Recommended recovery:"],
        ["[theclaw]   rm -rf node_modules"],
        ["[theclaw]   pnpm store prune"],
        ["[theclaw]   pnpm install"],
        [""],
        ["[theclaw] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "theclaw-watch-node-lock-" }, async (cwd) => {
