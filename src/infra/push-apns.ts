...
const EXEC_APPROVAL_GENERIC_ALERT_BODY = "Open TheClaw to review this request.";
const EXEC_APPROVAL_NOTIFICATION_CATEGORY = "theclaw.exec-approval";
...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.THECLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.THECLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set THECLAW_APNS_TEAM_ID and THECLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.THECLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.THECLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.THECLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set THECLAW_APNS_PRIVATE_KEY_P8 or THECLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading THECLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
