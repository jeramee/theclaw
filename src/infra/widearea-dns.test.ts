...
const baseZoneOpts: WideAreaGatewayZoneOpts = {
  ...
  gatewayPort: 18789,
  displayName: "Mac Studio (TheClaw)",
...
describe("wide-area DNS discovery domain helpers", () => {
  it.each([
    { value: "theclaw.internal", expected: "theclaw.internal." },
    { value: "theclaw.internal.", expected: "theclaw.internal." },
    { value: "  theclaw.internal  ", expected: "theclaw.internal." },
    { value: "", expected: null },
    { value: "   ", expected: null },
  ...
  it.each([
    {
      ...
      params: {
        env: { THECLAW_WIDE_AREA_DOMAIN: "env.internal" } as NodeJS.ProcessEnv,
    ...
    },
    {
      ...
      params: {
        env: { THECLAW_WIDE_AREA_DOMAIN: "   " } as NodeJS.ProcessEnv,
        configDomain: " ",
      },
      expected: null,
    },
  ...
  it("builds the default zone path from the normalized domain", () => {
    expect(getWideAreaZonePath("theclaw.internal.")).toBe(
      path.join(utils.CONFIG_DIR, "dns", "theclaw.internal.db"),
    );
  });
});
...
describe("wide-area DNS-SD zone rendering", () => {
  it("renders a zone with gateway PTR/SRV/TXT records", () => {
    ...
    expectZoneRecords(txt, [
      `$ORIGIN theclaw.internal.`,
      `studio-london IN A 100.123.224.76`,
      `studio-london IN AAAA fd7a:115c:a1e0::8801:e04c`,
      `_theclaw-gw._tcp IN PTR studio-london._theclaw-gw._tcp`,
      `studio-london._theclaw-gw._tcp IN SRV 0 0 18789 studio-london`,
      `displayName=Mac Studio (TheClaw)`,
      `gatewayPort=18789`,
      `sshPort=22`,
      `cliPath=/opt/homebrew/bin/theclaw`,
  ...
  it.each([
    ...
    },
    {
      ...
      records: [
        `$ORIGIN theclaw.internal.`,
        `studio-london IN A 100.123.224.76`,
        `studio-london._theclaw-gw._tcp IN TXT`,
        `displayName=Mac Studio (TheClaw)`,
        `gatewayTls=1`,
...
describe("wide-area DNS zone writes", () => {
  ...
  it("skips rewriting unchanged content", async () => {
    ...
    expect(result).toEqual({
      zonePath: getWideAreaZonePath("theclaw.internal."),
      changed: false,
    });
  ...
  it("increments same-day serials when content changes", async () => {
    ...
    expect(writeSpy).toHaveBeenCalledWith(
      getWideAreaZonePath("theclaw.internal."),
