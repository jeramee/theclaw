  ...
  it("routes normal HTTP and WebSocket egress through an operator-managed proxy even when NO_PROXY includes loopback", async () => {
    ...
    const child = await runNodeModule(
      `
        ...
        try {
          const response = await undiciFetch(process.env.THECLAW_TEST_TARGET_URL, {
            signal: AbortSignal.timeout(5000),
          });
          const body = await response.text();
          const nodeHttp = await nodeHttpGet(process.env.THECLAW_TEST_NODE_HTTP_TARGET_URL);
          const explicitAgent = await nodeHttpGet(process.env.THECLAW_TEST_EXPLICIT_AGENT_TARGET_URL, {
            agent: new http.Agent(),
          });
          await expectFailure("node:https", () =>
            nodeHttpsProbe(process.env.THECLAW_TEST_NODE_HTTPS_TARGET_URL),
          );
          await websocketProbe(process.env.THECLAW_TEST_WS_TARGET_URL);
          await gatewayLoopbackBypassProbe(process.env.THECLAW_TEST_GATEWAY_BYPASS_WS_URL);
      ...
      {
        ...process.env,
        THECLAW_PROXY_URL: `http://127.0.0.1:${proxyPort}`,
        THECLAW_TEST_TARGET_URL: `http://127.0.0.1:${targetPort}/private-metadata`,
        THECLAW_TEST_NODE_HTTP_TARGET_URL: `http://127.0.0.1:${targetPort}/node-http-metadata`,
        THECLAW_TEST_EXPLICIT_AGENT_TARGET_URL: `http://127.0.0.1:${targetPort}/explicit-agent`,
        THECLAW_TEST_NODE_HTTPS_TARGET_URL: `https://127.0.0.1:${httpsLikeTargetPort}/https-connect-proof`,
        THECLAW_TEST_WS_TARGET_URL: `ws://127.0.0.1:${targetPort}/websocket-proxied`,
        THECLAW_TEST_GATEWAY_BYPASS_WS_URL: `ws://127.0.0.1:${targetPort}/gateway-bypass`,
