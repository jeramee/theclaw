/**
 * High-level lifecycle management for TheClaw's operator-managed network
 * proxy routing.
 *
 * TheClaw does not spawn or configure the filtering proxy. When enabled, it
...
const PROXY_ACTIVE_KEYS = ["THECLAW_PROXY_ACTIVE"] as const;
...
function captureProxyEnv(): ProxyEnvSnapshot {
  return {
    ...
    THECLAW_PROXY_ACTIVE: process.env["THECLAW_PROXY_ACTIVE"],
  };
}
...
function applyProxyEnv(proxyUrl: string): void {
  ...
  process.env["THECLAW_PROXY_ACTIVE"] = "1";
...
function resolveProxyUrl(config: ProxyConfig | undefined): string {
  const candidate = config?.proxyUrl?.trim() || process.env["THECLAW_PROXY_URL"]?.trim();
  if (!candidate) {
    throw new Error(
      "proxy: enabled but no HTTP proxy URL is configured; set proxy.proxyUrl " +
        "or THECLAW_PROXY_URL to an http:// forward proxy.",
    );
  }
