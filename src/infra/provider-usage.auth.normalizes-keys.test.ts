...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "theclaw-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".theclaw");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".theclaw", "agents", "main", "agent");
  }
  ...
  function buildSuiteEnv(
    ...
    const suiteEnv: NodeJS.ProcessEnv = {
      ...
      HOME: home,
      USERPROFILE: home,
      THECLAW_STATE_DIR: path.join(home, ".theclaw"),
      ...env,
    };
