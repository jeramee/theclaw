...
import { resolveTheClawPackageRoot } from "./theclaw-root.js";
...
function resolveAutoUpdatePolicy(cfg: TheClawConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: TheClawConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.THECLAW_NO_AUTO_UPDATE);
  ...
  const root = await resolveTheClawPackageRoot({
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("theclaw update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by THECLAW_NO_AUTO_UPDATE", {
