...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers THECLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for THECLAW_HOME",
      env: { THECLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when THECLAW_HOME is tilde-only and no fallback home exists",
      env: { THECLAW_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores THECLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          THECLAW_HOME: "/srv/theclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { THECLAW_HOME: "/srv/theclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/theclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/theclaw-home " },
      expected: "/srv/theclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/theclaw-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of THECLAW_HOME", () => {
    expect(
