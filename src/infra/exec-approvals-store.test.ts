...
const originalTheClawHome = process.env.THECLAW_HOME;
...
afterEach(() => {
  vi.restoreAllMocks();
  if (originalTheClawHome === undefined) {
    delete process.env.THECLAW_HOME;
  } else {
    process.env.THECLAW_HOME = originalTheClawHome;
  }
...
function createHomeDir(): string {
  ...
  tempDirs.push(dir);
  process.env.THECLAW_HOME = dir;
  return dir;
}

function approvalsFilePath(homeDir: string): string {
  return path.join(homeDir, ".theclaw", "exec-approvals.json");
}
...
describe("exec approvals store helpers", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    expect(path.normalize(resolveExecApprovalsPath())).toBe(
      path.normalize(path.join(dir, ".theclaw", "exec-approvals.json")),
    );
    expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
      path.normalize(path.join(dir, ".theclaw", "exec-approvals.sock")),
    );
  });
  ...
  it("accepts a symlinked THECLAW_HOME as the trusted approvals root", () => {
    ...
    process.env.THECLAW_HOME = linkedHome;

    saveExecApprovals({ version: 1, defaults: { security: "full" }, agents: {} });

    expect(
      fs.readFileSync(path.join(realHome, ".theclaw", "exec-approvals.json"), "utf8"),
  ...
  it("refuses to traverse symlinked approvals components below a symlinked home", () => {
    ...
    fs.symlinkSync(realHome, linkedHome, "dir");
    fs.symlinkSync(linkedStateTarget, path.join(realHome, ".theclaw"), "dir");
    process.env.THECLAW_HOME = linkedHome;
