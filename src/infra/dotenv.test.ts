...
const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "THECLAW_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "THECLAW_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "THECLAW_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "THECLAW_BUNDLED_SKILLS_DIR=./attacker-skills",
  "THECLAW_SKIP_BROWSER_CONTROL_SERVER=1",
] as const;
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-dotenv-test-"));
  const cwdDir = path.join(base, "cwd");
  const stateDir = path.join(base, "state");
  process.env.THECLAW_STATE_DIR = stateDir;
...
describe("loadDotEnv", () => {
  it("loads ~/.theclaw/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.theclaw/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        process.env.HOME = base;
        const defaultStateDir = path.join(base, ".theclaw");
        process.env.THECLAW_STATE_DIR = defaultStateDir;
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "theclaw", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "THECLAW_STATE_DIR=./evil-state",
            "THECLAW_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.THECLAW_STATE_DIR).toBe(stateDir);
        expect(process.env.THECLAW_CONFIG_PATH).toBeUndefined();
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "THECLAW_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "THECLAW_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "THECLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "THECLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "THECLAW_GATEWAY_TOKEN=attacker-token",
            "THECLAW_GATEWAY_PASSWORD=attacker-password",
            "THECLAW_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks THECLAW_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
        ...
        expect(process.env.THECLAW_STATE_DIR).toBeUndefined();
        expect(process.env.THECLAW_CONFIG_PATH).toBeUndefined();
      });
    });
  });

  it("blocks path-override vars (THECLAW_AGENT_DIR, THECLAW_BUNDLED_PLUGINS_DIR, PI_CODING_AGENT_DIR, THECLAW_OAUTH_DIR) from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "THECLAW_AGENT_DIR=./evil-agent",
            `THECLAW_BUNDLED_PLUGINS_DIR=${bundledPluginsDir}`,
            "PI_CODING_AGENT_DIR=./evil-coding",
            "THECLAW_OAUTH_DIR=./evil-oauth",
          ].join("\n"),
        );
        ...
        delete process.env.THECLAW_OAUTH_DIR;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.THECLAW_AGENT_DIR).toBeUndefined();
        expect(process.env.THECLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
        expect(process.env.THECLAW_OAUTH_DIR).toBeUndefined();
      });
    });
  });

  it("blocks THECLAW_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "THECLAW_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.THECLAW_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.THECLAW_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "THECLAW_PINNED_PYTHON=./attacker-python",
            "THECLAW_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.THECLAW_PINNED_PYTHON;
        delete process.env.THECLAW_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.THECLAW_PINNED_PYTHON).toBeUndefined();
        expect(process.env.THECLAW_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "THECLAW_PINNED_PYTHON=/trusted/python",
            "THECLAW_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.THECLAW_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.THECLAW_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "THECLAW_LIVE_GEMINI_KEY=sk-gemini-live",
            "THECLAW_LIVE_OPENAI_KEY=sk-openai-live",
            "THECLAW_GATEWAY_TOKEN=trusted-token",
            "THECLAW_GATEWAY_PASSWORD=trusted-password",
            "THECLAW_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.THECLAW_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.THECLAW_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.THECLAW_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.THECLAW_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.THECLAW_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.THECLAW_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.THECLAW_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "THECLAW_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  ...
  it("does not load gateway.env when THECLAW_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        process.env.HOME = base;
        process.env.THECLAW_STATE_DIR = customStateDir;
        await writeEnvFile(
          path.join(base, ".config", "theclaw", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.THECLAW_STATE_DIR).toBe(customStateDir);
  ...
  it("keeps the legacy state-dir fallback for CLI dotenv loading", async () => {
    await withIsolatedEnvAndCwd(async () => {
      const base = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-dotenv-legacy-"));
      const cwdDir = path.join(base, "cwd");
      const legacyStateDir = path.join(base, ".clawdbot");
      process.env.HOME = base;
      delete process.env.THECLAW_STATE_DIR;
      delete process.env.THECLAW_TEST_FAST;
      await fs.mkdir(cwdDir, { recursive: true });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "THECLAW_MDNS_HOSTNAME",
          "THECLAW_SESSION_CACHE_TTL_MS",
          "THECLAW_UPDATE_PACKAGE_SPEC",
          "THECLAW_GATEWAY_PORT",
          "THECLAW_GATEWAY_URL",
          "THECLAW_CLAWHUB_URL",
          "CLAWHUB_URL",
          "THECLAW_CLAWHUB_TOKEN",
          "CLAWHUB_TOKEN",
          "CLAWHUB_AUTH_TOKEN",
          "CLAWHUB_CONFIG_PATH",
          ...
          "THECLAW_SKIP_CHANNELS",
          "THECLAW_SKIP_PROVIDERS",
          "THECLAW_SKIP_CRON",
          "THECLAW_RAW_STREAM",
  ...
  it("still allows user-defined non-control vars through workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        ...
        expect(process.env.APP_GITHUB_REPO).toBe("theclaw/theclaw");
