...
let listedConfigs: Array<TheClawConfig | undefined> = [];
...
describe("video-generation runtime", () => {
  ...
  it("lists runtime video-generation providers through the provider registry", () => {
    ...
    expect(
      listRuntimeVideoGenerationProviders({ config: {} as TheClawConfig }, runtimeDeps),
    ).toEqual(registryProviders);
    expect(listedConfigs).toEqual([{} as TheClawConfig]);
  });
  ...
  it("builds a generic config hint without hardcoded provider ids", async () => {
    ...
    await expect(
      runGenerateVideo({ cfg: {} as TheClawConfig, prompt: "animate a cat" }),
    ).rejects.toThrow(
