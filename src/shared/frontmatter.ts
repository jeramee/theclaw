...
export function resolveTheClawManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type TheClawManifestRequires = {
  bins: string[];
  anyBins: string[];
  env: string[];
  config: string[];
...
export function resolveTheClawManifestRequires(
  metadataObj: Record<string, unknown>,
): TheClawManifestRequires | undefined {
  const requiresRaw =
...
export function resolveTheClawManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
export function resolveTheClawManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedTheClawManifestInstallBase = {
...
export function parseTheClawManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedTheClawManifestInstallBase | undefined {
  ...
  const spec: ParsedTheClawManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyTheClawManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedTheClawManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
