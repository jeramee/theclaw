...
async function writeWizardConfigFile(config: TheClawConfig): Promise<TheClawConfig> {
...
export async function runSetupWizard(
  ...
  await prompter.intro("TheClaw setup");
  await requireRiskAcknowledgement({ opts, prompter });

  const snapshot = await readSetupConfigFileSnapshot();
  let baseConfig: TheClawConfig = snapshot.valid
    ? snapshot.exists
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("theclaw doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("theclaw doctor")}`,
        `Inspect: ${formatCliCommand("theclaw plugins inspect --all")}`,
      ].join("\n"),
      "Plugin compatibility",
    );
  }

  const quickstartHint = `Configure details later via ${formatCliCommand("theclaw configure")}.`;
  ...
  let localGatewayToken = process.env.THECLAW_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.THECLAW_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: TheClawConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
