...
function hasMeaningfulConfig(config: TheClawConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.THECLAW_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh TheClaw setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: TheClawConfig) => Promise<TheClawConfig>;
}): Promise<void> {
  const [
  ...
  await params.prompter.outro("Migration complete. Run `theclaw doctor` next.");
}
