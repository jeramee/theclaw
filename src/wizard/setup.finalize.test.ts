...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: TheClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: TheClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: TheClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
function createEnabledFirecrawlSearchConfig(): TheClawConfig {
  return {
    tools: {
      web: {
        search: {
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe but omits auth from TUI hatch", async () => {
    ...
    process.env.THECLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
  ...
  it("uses the setup token for health checks to avoid local env token drift", async () => {
    vi.stubEnv("THECLAW_GATEWAY_TOKEN", "env-token");
    const prompter = createLaterPrompter();
  ...
  it("uses the resolved setup password for health checks", async () => {
    vi.stubEnv("THECLAW_GATEWAY_PASSWORD", "env-password");
