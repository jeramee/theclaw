    ...
    it.each([
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-catalog-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@theclaw/demo-channel",
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-channel-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const stateDir = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-catalog-state-"),
          );
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-catalog-rich-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              schemaVersion: 1,
              description:
                "Extension manifest. Declares plugin packages that TheClaw can discover during onboarding and install on demand via `theclaw plugins install`.",
              entries: [
                {
                  name: "@wecom/wecom-theclaw-plugin",
                  description:
                    "TheClaw WeCom (企业微信) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  theclaw: {
                    channel: {
          ...
          return {
            ...
            expected: {
              ...
              installSource: {
                defaultChoice: "npm",
                npm: {
                  spec: "@wecom/wecom-theclaw-plugin@1.2.3",
                  packageName: "@wecom/wecom-theclaw-plugin",
                  selector: "1.2.3",
      ...
      },
      {
        ...
        setup: () => {
          const dir = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-catalog-yuanbao-"),
          );
          const catalogPath = path.join(dir, "catalog.json");
          fs.writeFileSync(
            catalogPath,
            JSON.stringify({
              ...
              entries: [
                {
                  name: "theclaw-plugin-yuanbao",
                  description:
                    "TheClaw Yuanbao (元宝) channel plugin — community maintained, published on npm.",
                  source: "external",
                  kind: "channel",
                  theclaw: {
                    channel: {
                      id: "theclaw-plugin-yuanbao",
                      label: "Yuanbao",
                    ...
                    install: {
                      npmSpec: "theclaw-plugin-yuanbao@1.0.0",
                      defaultChoice: "npm",
                      minHostVersion: ">=2026.4.10",
          ...
          return {
            channelId: "theclaw-plugin-yuanbao",
            catalogPaths: [catalogPath],
            expected: {
              id: "theclaw-plugin-yuanbao",
              meta: {
    ...
    it.each([
      {
        ...
        setup: () => {
          const home = fs.mkdtempSync(
            path.join(resolvePreferredTheClawTmpDir(), "theclaw-catalog-home-"),
          );
          const catalogPath = path.join(home, "catalog.json");
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@theclaw/env-demo-channel",
          ...
          return {
            env: {
              ...process.env,
              THECLAW_PLUGIN_CATALOG_PATHS: "~/catalog.json",
              THECLAW_HOME: home,
              HOME: home,
            },
      ...
      },
      {
        ...
        setup: () => {
          ...
          writeCatalogFile(
            catalogPath,
            createCatalogEntry({
              packageName: "@theclaw/default-env-demo",
