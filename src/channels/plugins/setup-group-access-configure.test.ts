...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: TheClawConfig, policy: ChannelAccessPolicy) => TheClawConfig;
  resolveAllowlist: (params: { cfg: TheClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: TheClawConfig; resolved: TResolved }) => TheClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: TheClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: TheClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: TheClawConfig, policy: ChannelAccessPolicy): TheClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: TheClawConfig, policy: ChannelAccessPolicy): TheClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: TheClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: TheClawConfig; resolved: string[] }) => {
      calls.push("apply");
