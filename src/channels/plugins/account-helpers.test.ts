...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): TheClawConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as TheClawConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: TheClawConfig) => string[];
  input: TheClawConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: TheClawConfig, expected: string) {
