...
import type { TheClawPackageManifest } from "../../plugins/manifest.js";
...
const ENV_CATALOG_PATHS = ["THECLAW_PLUGIN_CATALOG_PATHS", "THECLAW_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = [
    resolveTheClawPackageRootSync({ cwd: process.cwd() }),
    resolveTheClawPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<TheClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
