...
vi.mock("../../plugins/jiti-loader-cache.js", async (importOriginal) => {
  ...
  function listCandidatePluginDirs(params: LoaderParams): string[] {
    ...
    const workspaceExtensionsDir = params.workspaceDir
      ? path.join(params.workspaceDir, ".theclaw", "extensions")
      : undefined;
  ...
  function loadTheClawPlugins(params: LoaderParams) {
    ...
    const channelSetups = listCandidatePluginDirs(params).flatMap((pluginDir) => {
      ...
      const theclaw = isRecord(packageJson) ? packageJson.theclaw : undefined;
      const setupEntry = isRecord(theclaw) ? theclaw.setupEntry : undefined;
...
describe("listReadOnlyChannelPluginsForConfig", () => {
  ...
  it("discovers trusted external channel plugins from the default agent workspace", () => {
    ...
    const pluginDir = path.join(workspaceDir, ".theclaw", "extensions", "external-chat-plugin");
    fs.mkdirSync(pluginDir, { recursive: true });
