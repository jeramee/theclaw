...
function getChannelConfigRecord(cfg: TheClawConfig, channelId: string): Record<string, unknown> {
...
function listManifestChannelAccountIds(cfg: TheClawConfig, channelId: string): string[] {
...
function rebindChannelPluginConfig(
  ...
  const rebind = (cfg: TheClawConfig) =>
...
export function resolveReadOnlyChannelPluginsForConfig(
  ...
  if (externalPluginIds.length > 0) {
    ...
    if (missingConfiguredChannelIds.length > 0 && options.includeSetupRuntimeFallback === true) {
      ...
      const registry = loadPluginLoaderModule().loadTheClawPlugins({
        config: cfg,
        activationSourceConfig: options.activationSourceConfig ?? cfg,
        env,
        workspaceDir,
