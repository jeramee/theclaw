...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      THECLAW_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { THECLAW_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { THECLAW_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-daemon-cli-"));
    ...
    process.env.THECLAW_STATE_DIR = tmpDir;
    process.env.THECLAW_CONFIG_PATH = path.join(tmpDir, "theclaw.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        THECLAW_PROFILE: "dev",
        THECLAW_STATE_DIR: "/tmp/theclaw-daemon-state",
        THECLAW_CONFIG_PATH: "/tmp/theclaw-daemon-state/theclaw.json",
        THECLAW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.theclaw.gateway.plist",
    });
