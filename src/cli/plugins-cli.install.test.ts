...
import { installedPluginRoot } from "theclaw/plugin-sdk/test-fixtures";
...
const CLI_STATE_ROOT = "/tmp/theclaw-state";
const ORIGINAL_THECLAW_STATE_DIR = process.env.THECLAW_STATE_DIR;
const PROFILE_STATE_ROOT = "/tmp/theclaw-ledger-profile";
...
function useProfileExtensionsDir(): string {
  process.env.THECLAW_STATE_DIR = PROFILE_STATE_ROOT;
  return path.join(PROFILE_STATE_ROOT, "extensions");
}

function createEnabledPluginConfig(pluginId: string): TheClawConfig {
  return {
    plugins: {
      entries: {
        [pluginId]: {
...
function createEmptyPluginConfig(): TheClawConfig {
  return {
    plugins: {
      entries: {},
    },
...
function createPathHookPackInstalledConfig(tmpRoot: string): TheClawConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function createNpmHookPackInstalledConfig(): TheClawConfig {
  return {
    hooks: {
      internal: {
        installs: {
...
function primeHookPackNpmFallback() {
  ...
  installPluginFromNpmSpec.mockResolvedValue({
    ok: false,
    error: "package.json missing theclaw.plugin.json",
  });
...
describe("plugins cli install", () => {
  ...
  afterEach(() => {
    if (ORIGINAL_THECLAW_STATE_DIR === undefined) {
      delete process.env.THECLAW_STATE_DIR;
    } else {
      process.env.THECLAW_STATE_DIR = ORIGINAL_THECLAW_STATE_DIR;
    }
  });
  ...
  it("fails closed for unrelated invalid config before installer side effects", async () => {
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Config invalid; run `theclaw doctor --fix` before installing plugins.",
    );
    expect(installPluginFromMarketplace).not.toHaveBeenCalled();
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(writeConfigFile).not.toHaveBeenCalled();
  ...
  it("does not persist incomplete config entries for config-gated bundled installs", async () => {
    ...
    const writtenConfig = writeConfigFile.mock.calls.at(-1)?.[0] as TheClawConfig;
  ...
  it("reports npm install failures without trying ClawHub when npm: prefix is used", async () => {
    ...
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing theclaw.hooks",
    });
  ...
  it("does not resolve npm: prefixed bundled plugin ids through bundled installs", async () => {
    loadConfig.mockReturnValue({ plugins: { load: { paths: [] } } } as TheClawConfig);
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
  ...
  it("passes dangerous force unsafe install to linked path probe installs", async () => {
    ...
    const enabledCfg = createEnabledPluginConfig("demo");
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-plugin-link-"));
  ...
  it("passes dangerous force unsafe install to linked hook-pack probe fallback", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-hook-link-"));
  ...
  it("does not fall back to hook pack for linked path when a no-flag security scan blocks", async () => {
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-link-plugin-"));
  ...
  it("passes dangerous force unsafe install to local hook-pack fallback installs", async () => {
    const tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-hook-install-"));
    primeHookPackPathFallback({
      tmpRoot,
  ...
  it("passes the active profile extensions dir to local path installs", async () => {
    const extensionsDir = useProfileExtensionsDir();
    const localPluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-local-plugin-"));
    const cfg = createEmptyPluginConfig();
    const enabledCfg = createEnabledPluginConfig("demo");
  ...
  it("suggests update or --force when npm plugin install target already exists", async () => {
    ...
    installPluginFromNpmSpec.mockResolvedValue({
      ok: false,
      error:
        "plugin already exists: /home/theclaw/.theclaw/extensions/lossless-claw (delete it first)",
    });
    installHooksFromNpmSpec.mockResolvedValue({
      ok: false,
      error: "package.json missing theclaw.hooks",
    ...
    expect(runtimeErrors.at(-1)).toContain(
      "Use `theclaw plugins update <id-or-npm-spec>` to upgrade the tracked plugin, or rerun install with `--force` to replace it.",
    );
    expect(runtimeErrors.at(-1)).not.toContain("Also not a valid hook pack");
  });
  ...
  it("still falls back to local hook pack when dangerous force unsafe install is set for non-security errors", async () => {
    const localHookDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-local-hook-pack-"));
    const cfg = {} as TheClawConfig;
    const installedCfg = {
      hooks: {
        internal: {
    ...
    installPluginFromPath.mockResolvedValue({
      ok: false,
      error: "package.json missing theclaw.plugin.json",
      code: "missing_theclaw_extensions",
    });
    installHooksFromPath.mockResolvedValue({
      ok: true,
      hookPackId: "demo-hooks",
  ...
  it("does not fall back to npm when ClawHub rejects a real package", async () => {
    installPluginFromClawHub.mockResolvedValue({
      ok: false,
      error: 'Use "theclaw skills install demo" instead.',
      code: "skill_package",
    });
    ...
    expect(installPluginFromNpmSpec).not.toHaveBeenCalled();
    expect(runtimeErrors.at(-1)).toContain('Use "theclaw skills install demo" instead.');
  });
