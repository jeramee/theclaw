...
const serveTheClawChannelMcp = mocks.serveTheClawChannelMcp;
...
vi.mock("../mcp/channel-server.js", () => ({
  serveTheClawChannelMcp: mocks.serveTheClawChannelMcp,
}));

const tempDirs: string[] = [];

async function createWorkspace(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-cli-mcp-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("mcp cli", () => {
  ...
  it("sets and shows a configured MCP server", async () => {
    await withTempHome("theclaw-cli-mcp-home-", async () => {
  ...
  it("starts the channel bridge with parsed serve options", async () => {
    await withTempHome("theclaw-cli-mcp-home-", async () => {
      ...
      expect(serveTheClawChannelMcp).toHaveBeenCalledWith({
