...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-bundled-skills-test-"));
    process.env.THECLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
