...
import type { TheClawConfig, ConfigFileSnapshot } from "../config/types.theclaw.js";
...
const loadInstalledPluginIndexInstallRecords = vi.fn(
  async (params: { config?: TheClawConfig } = {}) => params.config?.plugins?.installs ?? {},
);
...
vi.mock("../infra/theclaw-root.js", () => ({
  resolveTheClawPackageRoot: vi.fn(),
  resolveTheClawPackageRootSync: vi.fn(() => process.cwd()),
}));
...
vi.mock("../utils.js", async (importOriginal) => {
  ...
  return {
    ...
    resolveConfigDir: () => "/tmp/theclaw-config",
  };
});
...
const { resolveTheClawPackageRoot } = await import("../infra/theclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/theclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as TheClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/theclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveTheClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("theclaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("theclaw-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveTheClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("theclaw");
    ...
    serviceReadCommand.mockImplementation(async () =>
      (await serviceLoaded()) ? { programArguments: ["theclaw", "gateway", "run"] } : null,
    );
    ...
    prepareRestartScript.mockResolvedValue("/tmp/theclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "theclaw-gateway" }],
      hints: [],
    });
  ...
  it("bounds completion cache refresh during update follow-up", async () => {
    const root = createCaseDir("theclaw-completion-timeout");
    ...
    expect(spawnSync).toHaveBeenCalledWith(
      expect.any(String),
      [path.join(root, "theclaw.mjs"), "completion", "--write-state"],
      expect.objectContaining({
        env: expect.objectContaining({
          THECLAW_COMPLETION_SKIP_PLUGIN_COMMANDS: "1",
        }),
        timeout: 30_000,
      }),
    );
  ...
  it("logs friendly hint with manual refresh command when completion cache write times out", async () => {
    const root = createCaseDir("theclaw-completion-timeout-msg");
    pathExists.mockResolvedValue(true);
    ...
    expect(logs.some((line) => line.includes("theclaw completion --write-state"))).toBe(true);
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          ...
          THECLAW_UPDATE_POST_CORE: "1",
          THECLAW_UPDATE_POST_CORE_CHANNEL: "dev",
        }),
      }),
    );
  ...
  it("keeps downgrade post-update work in the current process", async () => {
    const downgradedRoot = createCaseDir("theclaw-downgraded-root");
  ...
  it("post-core resume mode skips the core update and only runs post-update tasks", async () => {
    await withEnvAsync(
      {
        THECLAW_UPDATE_POST_CORE: "1",
        THECLAW_UPDATE_POST_CORE_CHANNEL: "stable",
      },
      async () => {
  ...
  it("uses a fail-closed integrity policy for post-core plugin updates", async () => {
    ...
    await expect(
      onIntegrityDrift({
        pluginId: "demo",
        spec: "@theclaw/demo@1.0.0",
        resolvedSpec: "@theclaw/demo@1.0.0",
          ...
          outcomes: [
            {
              ...
              message:
                proceed === false
                  ? "Failed to update demo: aborted: npm package integrity drift detected for @theclaw/demo@1.0.0"
  ...
  it("preserves fresh-process plugin failure details in parent json output", async () => {
    ...
    spawn.mockImplementationOnce((_node, _argv, options) => {
      ...
      queueMicrotask(async () => {
        const resultPath = env?.THECLAW_UPDATE_POST_CORE_RESULT_PATH;
        if (resultPath) {
          await fs.writeFile(
            resultPath,
            JSON.stringify({
  ...
  it.each([
    {
      ...
      assert: () => {
        ...
        expect(logs.join("\n")).toContain("TheClaw update status");
      },
    },
    {
      name: "json output",
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "theclaw", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as TheClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "theclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
  ...
  it("warns but still runs package updates when disk space looks low", async () => {
    ...
    expectPackageInstallSpec("theclaw@latest");
    expect(defaultRuntime.exit).not.toHaveBeenCalledWith(1);
    expect(
      vi
  ...
  it("refuses package updates from inside the gateway service process", async () => {
    mockPackageInstallStatus(createCaseDir("theclaw-update"));

    await withEnvAsync(
      {
        THECLAW_SERVICE_MARKER: "theclaw",
        THECLAW_SERVICE_KIND: "gateway",
      },
      async () => {
        await updateCommand({ yes: true });
      },
  ...
  it("blocks package updates when the target requires a newer Node runtime", async () => {
    ...
    expect(errors.join("\n")).toContain("Node ");
    expect(errors.join("\n")).toContain(
      "Bare `npm i -g theclaw` can silently install an older compatible release.",
    );
  });
  ...
  it.each([
    ...
    },
    {
      ...
      expectedSpec: "github:theclaw/theclaw#main",
    },
    {
      name: "explicit git package spec",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("theclaw-update"));
        await updateCommand({ yes: true, tag: "github:theclaw/theclaw#main" });
      },
      expectedSpec: "github:theclaw/theclaw#main",
    },
    {
      name: "THECLAW_UPDATE_PACKAGE_SPEC override",
      run: async () => {
        mockPackageInstallStatus(createCaseDir("theclaw-update"));
        await withEnvAsync(
          { THECLAW_UPDATE_PACKAGE_SPEC: "http://10.211.55.2:8138/theclaw-next.tgz" },
          async () => {
            await updateCommand({ yes: true, tag: "latest" });
          },
        );
      },
      expectedSpec: "http://10.211.55.2:8138/theclaw-next.tgz",
    },
  ] as const)(
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "theclaw");
    mockPackageInstallStatus(tempDir);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("stops package post-update work when staged npm install verification fails", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-staged-fail-"));
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.20" }),
      "utf-8",
    );
    ...
    vi.mocked(runCommandWithTimeout).mockImplementation(async (argv) => {
      ...
      if (
        ...
        const stageRoot = path.join(stagePrefix, "lib", "node_modules", "theclaw");
        await fs.mkdir(path.join(stageRoot, "dist"), { recursive: true });
        await fs.writeFile(
          path.join(stageRoot, "package.json"),
          JSON.stringify({ name: "theclaw", version: "2026.4.25" }),
          "utf-8",
        );
  ...
  it("marks package post-update doctor as update-in-progress", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-package-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "theclaw");
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.21" }),
      "utf-8",
    );
  ...
  it("stops a running managed gateway before package replacement", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-stop-service-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "theclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    serviceReadCommand.mockResolvedValue({
      programArguments: ["theclaw", "gateway", "run"],
      environment: {
        THECLAW_SERVICE_MARKER: "theclaw",
        THECLAW_SERVICE_KIND: "gateway",
      },
  ...
  it("refreshes package installs even when the current version already matches the target", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-current-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "theclaw");
    const entryPath = path.join(pkgRoot, "dist", "index.js");
    mockPackageInstallStatus(pkgRoot);
    ...
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.23" }),
      "utf-8",
    );
    await fs.writeFile(entryPath, "export {};\n", "utf-8");
    for (const relativePath of TEST_BUNDLED_RUNTIME_SIDECAR_PATHS) {
  ...
  it("retries package updates without optional deps when npm global update fails", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-optional-"));
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "theclaw");
    mockPackageInstallStatus(pkgRoot);
    await fs.mkdir(pkgRoot, { recursive: true });
    await fs.writeFile(
      path.join(pkgRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("theclaw-update");
    const localAppData = createCaseDir("theclaw-localappdata");
  ...
  it("persists channel and runs post-update work after switching from package to git", async () => {
    const tempDir = createCaseDir("theclaw-update");
    const gitRoot = path.join(tempDir, "..", "theclaw");
    ...
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...
      resolved: { update: { channel: "stable" } } as TheClawConfig,
      sourceConfig: { update: { channel: "stable" } } as TheClawConfig,
      runtimeConfig: { update: { channel: "stable" } } as TheClawConfig,
      config: { update: { channel: "stable" } } as TheClawConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "git",
  ...
  it("explains why git updates cannot run with edited files", async () => {
    ...
    expect(logs.join("\n")).toContain(
      "Commit, stash, or discard the local changes, then rerun `theclaw update`.",
    );
  ...
  it("fails a JSON package update when fallback restart leaves the old gateway running", async () => {
    const updatedRoot = createCaseDir("theclaw-updated-root");
    ...
    },
    {
        ...
        await withEnvAsync(
          {
            ...
            THECLAW_CONFIG_PATH: "./config/theclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          cwd: root,
          env: expect.objectContaining({
            THECLAW_STATE_DIR: path.resolve("./state"),
            THECLAW_CONFIG_PATH: path.resolve("./config/theclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            THECLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ THECLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.THECLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("theclaw-update-wizard");
    await withEnvAsync({ THECLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/theclaw as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ THECLAW_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "theclaw"));
    });
