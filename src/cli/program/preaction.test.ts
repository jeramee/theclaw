...
import { repoInstallSpec } from "theclaw/plugin-sdk/test-fixtures";
...
beforeEach(() => {
  ...
  originalHideBanner = process.env.THECLAW_HIDE_BANNER;
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.THECLAW_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  function buildProgram() {
    const program = new Command().name("theclaw");
    program
      .command("agent")
  ...
  it("handles debug mode and plugin-required command preaction", async () => {
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "theclaw", "status", "--debug"],
    });
    ...
    expect(processTitleSetSpy).toHaveBeenCalledWith("theclaw-status");

    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "theclaw", "agents", "list"],
    });
  ...
  it("loads plugins for local agent runs", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "theclaw", "agent", "--local", "--message", "hi"],
    });
  ...
  it("keeps setup alias and channels add manifest-first", async () => {
    await runPreAction({
      parseArgv: ["onboard"],
      processArgv: ["node", "theclaw", "onboard"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "channels", "add"],
    });
  ...
  it("lets configure own config validation and plugin loading", async () => {
    await runPreAction({
      parseArgv: ["configure"],
      processArgv: ["node", "theclaw", "configure"],
    });
  ...
  it("only allows invalid config for explicit Matrix reinstall requests", async () => {
    await runPreAction({
      parseArgv: ["plugins", "install", "@theclaw/matrix"],
      processArgv: ["node", "theclaw", "plugins", "install", "@theclaw/matrix"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "plugins", "install", "alpha"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "plugins", "install", MATRIX_REPO_INSTALL_SPEC],
    });
    ...
    vi.clearAllMocks();
    await runPreAction({
      parseArgv: ["plugins", "install", "@theclaw/matrix", "--marketplace", "local/repo"],
      processArgv: [
        "node",
        "theclaw",
        "plugins",
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "theclaw", "--version"],
    });
    ...
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "theclaw", "status"],
    });
  ...
  it("applies --json stdout suppression only for explicit JSON output commands", async () => {
    await runPreAction({
      parseArgv: ["status"],
      processArgv: ["node", "theclaw", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "update", "status", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "config", "set", "gateway.auth.mode", "{bad", "--json"],
    });
  ...
  it("routes logs to stderr in --json mode so stdout stays clean", async () => {
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "theclaw", "agent", "--message", "hi", "--json"],
    });
    ...
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "config", "set", "gateway.auth.mode", "local", "--json"],
    });
  ...
  it("does not preload plugins for agents list JSON output", async () => {
    await runPreAction({
      parseArgv: ["agents", "list"],
      processArgv: ["node", "theclaw", "agents", "list", "--json"],
    });
  ...
  it("bypasses config guard for config validate", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "config", "validate"],
    });
  ...
  it("bypasses config guard for config validate when root option values are present", async () => {
    await runPreAction({
      parseArgv: ["config", "validate"],
      processArgv: ["node", "theclaw", "--profile", "work", "config", "validate"],
    });
  ...
  it("bypasses config guard for config schema", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "config", "schema"],
    });
  ...
  it("bypasses config guard for backup create", async () => {
    await runPreAction({
      ...
      processArgv: ["node", "theclaw", "backup", "create", "--json"],
    });
  ...
  it("routes logs to stderr during plugin loading in --json mode and restores after", async () => {
    ...
    await runPreAction({
      parseArgv: ["agent"],
      processArgv: ["node", "theclaw", "agent", "--local", "--message", "hi", "--json"],
    });
