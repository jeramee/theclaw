...
${formatHelpExamples([
  ['theclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['theclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'theclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'theclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['theclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'theclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.theclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
      const verboseLevel =
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.theclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['theclaw agents set-identity --agent main --name "TheClaw" --emoji "🦞"', "Set name + emoji."],
  ["theclaw agents set-identity --agent main --avatar avatars/theclaw.png", "Set avatar path."],
  [
    "theclaw agents set-identity --workspace ~/.theclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "theclaw agents set-identity --identity-file ~/.theclaw/workspace/IDENTITY.md --agent main",
