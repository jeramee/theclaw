...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.THECLAW_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.THECLAW_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.THECLAW_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.THECLAW_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.THECLAW_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.THECLAW_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.THECLAW_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "theclaw", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "theclaw"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "theclaw", "nodes", "list"], "theclaw");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "theclaw", "infer"], "theclaw");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "theclaw", "acp", "--help"], "theclaw");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "theclaw", "gateway", "--force"];
    process.argv = argv;
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "theclaw", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("theclaw");
  ...
  it.each([
    ...
    process.argv = ["node", "theclaw", ...args];
    const program = new Command().name("theclaw");
  ...
  it("keeps plugin CLI registrations available for the plugins command root", async () => {
    process.argv = ["node", "theclaw", "plugins"];
    const program = new Command().name("theclaw");
