...
export function registerMigrateCommand(program: Command) {
  const migrate = program
    ...
    .option("--no-backup", "Skip the pre-migration TheClaw backup")
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw migrate list", "Show available migration providers."],
          ["theclaw migrate hermes", "Preview Hermes migration, then prompt before applying."],
          ["theclaw migrate hermes --dry-run", "Preview Hermes migration only."],
          [
  ...
  addMigrationOptions(
    migrate
      ...
      .description("Preview a migration without changing TheClaw state"),
  ).action(async (provider, opts) => {
    await runCommandWithRuntime(defaultRuntime, async () => {
