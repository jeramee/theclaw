...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw status", "Show channel health + session summary."],
          ["theclaw status --all", "Full diagnosis (read-only)."],
          ["theclaw status --json", "Machine-readable output."],
          ["theclaw status --usage", "Show model provider usage/quota snapshots."],
          [
            "theclaw status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["theclaw status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.theclaw.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.theclaw.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
        await healthCommand(
  ...
  const sessionsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw sessions", "List all sessions."],
          ["theclaw sessions --agent work", "List sessions for one agent."],
          ["theclaw sessions --all-agents", "Aggregate sessions across agents."],
          ["theclaw sessions --active 120", "Only last 2 hours."],
          ["theclaw sessions --json", "Machine-readable output."],
          ["theclaw sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.theclaw.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "theclaw sessions cleanup --dry-run --fix-missing",
            "Also preview pruning entries with missing transcript files.",
          ],
          ["theclaw sessions cleanup --enforce", "Apply maintenance now."],
          ["theclaw sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["theclaw sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "theclaw sessions cleanup --enforce --store ./tmp/sessions.json",
