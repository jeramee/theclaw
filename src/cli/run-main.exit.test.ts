...
vi.mock("../infra/path-env.js", () => ({
  ensureTheClawCliOnPath: ensurePathMock,
}));
...
function makeProxyHandle() {
  return {
    ...
    envSnapshot: {
      ...
      THECLAW_PROXY_ACTIVE: undefined,
    },
    stop: vi.fn(async () => {}),
    kill: vi.fn(),
  };
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.THECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.THECLAW_HIDE_BANNER;
  });
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "theclaw", "status"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "theclaw", "gateway", "--force"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).toHaveBeenCalledWith("9.9.9-test", {
      argv: ["node", "theclaw", "gateway", "--force"],
    });
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.THECLAW_HIDE_BANNER = "1";

    await runCli(["node", "theclaw", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "theclaw", "gateway"]);
  });
  ...
  it("renders browser help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "theclaw", "browser", "--help"]);
  ...
  it("keeps root help on the precomputed path without proxy bootstrap", async () => {
    ...
    await runCli(["node", "theclaw", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "theclaw", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "theclaw", "gateway", "run"]],
    ["bare gateway runtime", ["node", "theclaw", "gateway"]],
    ["node runtime", ["node", "theclaw", "node", "run"]],
    ["local agent runtime", ["node", "theclaw", "agent", "--local"]],
    ["provider inference", ["node", "theclaw", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "theclaw", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "theclaw", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "theclaw", "skills", "search", "browser"]],
    ["update command", ["node", "theclaw", "update", "check"]],
    ["channel probe", ["node", "theclaw", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "theclaw", "channels", "capabilities"]],
    ["directory plugin command", ["node", "theclaw", "directory", "peers", "list"]],
    ["message plugin command", ["node", "theclaw", "message", "send", "--to", "demo"]],
    ["unknown plugin command", ["node", "theclaw", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "theclaw", "--help"]],
    ["root version", ["node", "theclaw", "--version"]],
    ["gateway help", ["node", "theclaw", "gateway", "--help"]],
    ["gateway run help", ["node", "theclaw", "gateway", "run", "--help"]],
    ["status", ["node", "theclaw", "status"]],
    ["health", ["node", "theclaw", "health"]],
    ["gateway status", ["node", "theclaw", "gateway", "status"]],
    ["gateway health", ["node", "theclaw", "gateway", "health"]],
    ["remote agent control-plane", ["node", "theclaw", "agent", "run"]],
    ["chat control-plane", ["node", "theclaw", "chat"]],
    ["terminal control-plane", ["node", "theclaw", "terminal"]],
    ["config", ["node", "theclaw", "config", "get", "proxy.enabled"]],
    ["completion", ["node", "theclaw", "completion", "zsh"]],
    ["debug proxy cli", ["node", "theclaw", "proxy", "start"]],
    ["agents list", ["node", "theclaw", "agents", "list"]],
    ["models list", ["node", "theclaw", "models", "list"]],
    ["models status without live probe", ["node", "theclaw", "models", "status"]],
    ["tasks list", ["node", "theclaw", "tasks", "list"]],
    ["migrate", ["node", "theclaw", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "theclaw", "plugins", "marketplace", "list"]);
  ...
  it("starts the managed proxy for unknown plugin commands by default", async () => {
    ...
    await runCli(["node", "theclaw", "googlemeet", "login"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "theclaw", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy after normal gateway runtime completion", async () => {
    ...
    await runCli(["node", "theclaw", "gateway", "run"]);
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "theclaw", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "theclaw", "onboard", "--modern", "--json"]);
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "theclaw", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    await expect(runCli(["node", "theclaw", "status"])).resolves.toBeUndefined();
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    await runCli(["node", "theclaw", "doctor", "--help"]);
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith(
        "[theclaw] Non-fatal uncaught exception (continuing):",
