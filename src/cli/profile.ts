...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.theclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.THECLAW_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.THECLAW_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.THECLAW_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.THECLAW_CONFIG_PATH)) {
    env.THECLAW_CONFIG_PATH = path.join(stateDir, "theclaw.json");
  }

  if (profile === "dev" && !env.THECLAW_GATEWAY_PORT?.trim()) {
    env.THECLAW_GATEWAY_PORT = "19001";
  }
}
