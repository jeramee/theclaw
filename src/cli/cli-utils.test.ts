...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "theclaw", "-V"] },
    { argv: ["node", "theclaw", "gateway"] },
    { argv: ["node", "theclaw", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "theclaw", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "theclaw", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "theclaw", "gateway", "status"] },
    { argv: ["node", "theclaw", "gateway", "call", "health"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "theclaw.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("theclaw.internal");
    } finally {
      log.mockRestore();
    }
  });
