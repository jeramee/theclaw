...
import { getRuntimeConfig, readConfigFileSnapshot, type TheClawConfig } from "../config/config.js";
...
function isConfiguredAuthPlugin(plugin: ChannelPlugin, cfg: TheClawConfig): boolean {
  const key = plugin.id;
...
function resolveConfiguredAuthChannelInput(cfg: TheClawConfig, mode: ChannelAuthMode): string {
