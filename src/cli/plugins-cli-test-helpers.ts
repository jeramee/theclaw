...
export const loadConfig: Mock<LoadConfigFn> = vi.fn<LoadConfigFn>(() => ({}) as TheClawConfig);
...
export const replaceConfigFile: AsyncUnknownMock = vi.fn(
  async (params: { nextConfig: TheClawConfig }) => await writeConfigFile(params.nextConfig),
) as AsyncUnknownMock;
export const resolveStateDir: Mock<() => string> = vi.fn(() => "/tmp/theclaw-state");
...
vi.mock("../config/config.js", () => ({
  ...
  writeConfigFile: ((config: TheClawConfig) =>
    invokeMock<
      [TheClawConfig],
...
vi.mock("../plugins/enable.js", () => ({
  enablePluginInConfig: ((cfg: TheClawConfig, pluginId: string) =>
    invokeMock<[TheClawConfig, string], unknown>(
...
export function resetPluginsCliTestState() {
  ...
  applyExclusiveSlotSelection.mockImplementation((({ config }: { config: TheClawConfig }) => ({
    config,
    warnings: [],
  })) as (...args: unknown[]) => unknown);
