    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "theclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["theclaw node status", "Check node host service status."],
          ["theclaw node install", "Install the node host service."],
          ["theclaw node start", "Start the installed node host service."],
          ["theclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.theclaw.ai/cli/node")}\n`,
    );
