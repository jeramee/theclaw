...
export function registerPluginsCli(program: Command) {
  const plugins = program
    .command("plugins")
    .description("Manage TheClaw plugins and extensions")
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.theclaw.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    ...
    .action(async (id: string | undefined, opts: PluginInspectOptions) => {
      ...
      lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "theclaw"}`);
  ...
  plugins
    ...
    .action(async (id: string) => {
      ...
      const cfg = (snapshot.sourceConfig ?? snapshot.config) as TheClawConfig;
      const enableResult = enablePluginInConfig(cfg, id);
      let next: TheClawConfig = enableResult.config;
  ...
  plugins
    ...
    .action(async (id: string, opts: PluginUninstallOptions) => {
      ...
      const sourceConfig = (snapshot.sourceConfig ?? snapshot.config) as TheClawConfig;
      const installRecords = await tracePluginLifecyclePhaseAsync(
        "install records load",
        () => loadInstalledPluginIndexInstallRecords(),
        { command: "uninstall" },
  ...
  plugins
    ...
    .action(async (opts: PluginRegistryOptions) => {
      ...
      if (inspection.refreshReasons.length > 0) {
        ...
        lines.push(
          `${theme.muted("Repair:")} ${theme.command("theclaw plugins registry --refresh")}`,
        );
      }
      defaultRuntime.log(lines.join("\n"));
    });
  ...
  plugins
    ...
    .action(async () => {
      ...
      const docs = formatDocsLink("/plugin", "docs.theclaw.ai/plugin");
      lines.push("");
