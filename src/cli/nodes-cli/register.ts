    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw nodes status", "List known nodes with live status."],
          ["theclaw nodes pairing pending", "Show pending node pairing requests."],
          ["theclaw nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'theclaw nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["theclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.theclaw.ai/cli/nodes")}\n`,
    );
