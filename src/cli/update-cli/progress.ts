...
const STEP_LABELS: Record<string, string> = {
  ...
  "theclaw doctor entry": "Checking doctor entrypoint",
  "theclaw doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g theclaw@latest");
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g theclaw@latest --omit=optional");
  }
