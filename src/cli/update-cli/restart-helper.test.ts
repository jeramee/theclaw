...
vi.mock("node:child_process", async () => {
  const { mockNodeBuiltinModule } = await import("theclaw/plugin-sdk/test-node-mocks");
...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-TheClawScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-TheClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "theclaw restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-TheClawListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-TheClawSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "default",
      });
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("systemctl --user restart 'theclaw-gateway.service'");
      // Script should self-cleanup
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("uses THECLAW_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "default",
        THECLAW_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.theclaw.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.theclaw.gateway'");
      expect(content).toContain("launchctl bootstrap 'gui/501'");
      expect(content).toContain('rm -f "$0"');
      await cleanupScript(scriptPath);
    });

    it("captures macOS launchctl stderr to ~/.theclaw/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.theclaw/logs/gateway-restart.log' 2>&1");
    ...
    it("uses THECLAW_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/theclaw-state/logs' 2>/dev/null && : >>'/tmp/theclaw-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/theclaw-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
    ...
    it("returns the final macOS launchctl kickstart failure after logging cleanup", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      const tmpDir = await makeTempDir("theclaw-restart-helper-");
...
`,
      ...
      expect(log).toContain("theclaw restart attempt source=update target=ai.theclaw.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.theclaw.gateway");
      expect(log).toContain("theclaw restart failed source=update status=42");
      expect(log).not.toContain("theclaw restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        THECLAW_LAUNCHD_LABEL: "ai.theclaw.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        THECLAW_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.theclaw.$(echo injected)");
      expect(log).not.toContain("target=ai.theclaw.injected");
    });

    it("uses THECLAW_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "default",
        THECLAW_LAUNCHD_LABEL: "com.custom.theclaw",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.theclaw'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'TheClaw Gateway'");
      expect(content).toContain("function Invoke-TheClawSchtasksWithTimeout");
      expect(content).toContain("function Get-TheClawScheduledTaskState");
      expect(content).toContain("function Invoke-TheClawStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("theclaw restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".theclaw\\gateway.cmd"',
      );
      expect(content).toContain("theclaw restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses THECLAW_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "default",
        THECLAW_WINDOWS_TASK_NAME: "TheClaw Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'TheClaw Gateway (custom)'");
      expect(content).toContain("Get-TheClawScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-TheClawSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-TheClawStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "production",
      });
      expect(content).toContain("theclaw-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        THECLAW_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.theclaw.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'TheClaw Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        THECLAW_LAUNCHD_LABEL: "ai.theclaw.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.theclaw.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
