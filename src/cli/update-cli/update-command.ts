...
const POST_CORE_UPDATE_ENV = "THECLAW_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "THECLAW_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "THECLAW_UPDATE_POST_CORE_RESULT_PATH";
...
function isRunningInsideGatewayService(
  ...
  if (env.THECLAW_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.THECLAW_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `Node ${process.versions.node ?? "unknown"} is too old for theclaw@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    "Upgrade Node to 22.14+ or Node 24, then rerun `theclaw update`.",
    "Bare `npm i -g theclaw` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g theclaw@latest`.",
  ].join("\n");
}
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("theclaw completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (expectedGatewayVersion: string | undefined) => {
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("theclaw gateway status --deep"), CLI_NAME)}\` for details.`,
    ];
    if (params.opts.json) {
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("theclaw gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (isPackageManagerUpdateMode(params.result.mode)) {
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("theclaw doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("theclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("theclaw gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir =
    params.opts.json === true
      ? await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-update-post-core-"))
      : null;
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  const devTargetRef =
    channel === "dev" ? process.env.THECLAW_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (updateInstallKind === "package" && isRunningInsideGatewayService()) {
    defaultRuntime.error(
      [
        ...
        "That path replaces the active TheClaw dist tree while the live gateway may still lazy-load old chunks.",
        `Run \`${replaceCliName(formatCliCommand("theclaw update"), CLI_NAME)}\` from a shell outside the gateway service, or stop the gateway service first and then update.`,
      ].join("\n"),
    );
    defaultRuntime.exit(1);
    return;
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating TheClaw..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `theclaw update`."),
      );
    }
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this TheClaw install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("theclaw doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("theclaw gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g theclaw@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g theclaw@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
