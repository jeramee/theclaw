...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "theclaw config set secrets.providers.vault --provider-source file --provider-path /etc/theclaw/secrets.json --provider-mode json",
);
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("theclaw doctor")}\` ${message}`;
}
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("theclaw plugins install <spec>")}`,
    `  ${formatCliCommand("theclaw plugins update <plugin-id>")}`,
    `  ${formatCliCommand("theclaw plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
export async function runConfigSet(opts: {
  ...
  try {
    ...
    const nextConfig = next as TheClawConfig;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "theclaw.json";
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.theclaw.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing theclaw.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for theclaw.json")
    .action(async () => {
