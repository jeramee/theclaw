...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update TheClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["theclaw update", "Update a source checkout (git)"],
        ["theclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["theclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["theclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["theclaw update --tag main", "One-off package install from GitHub main"],
        ["theclaw update --dry-run", "Preview actions without changing anything"],
        ["theclaw update --no-restart", "Update without restarting the service"],
        ["theclaw update --json", "Output result as JSON"],
        ["theclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["theclaw update wizard", "Interactive update wizard"],
        ["theclaw --update", "Shorthand for theclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run theclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.theclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw update status", "Show channel + version status."],
          ["theclaw update status --json", "JSON output."],
          ["theclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.theclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
