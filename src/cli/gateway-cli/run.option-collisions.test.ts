...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/theclaw-stability.json",
  path: "/tmp/theclaw-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/theclaw-control-ui" as string | null,
}));

const { runtimeErrors, defaultRuntime, resetRuntimeCapture } = createCliRuntimeCapture();

vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/theclaw-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/theclaw-test-missing-config.json",
  resolveStateDir: () => "/tmp",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/theclaw-control-ui";
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    const previousMarker = process.env.THECLAW_SERVICE_MARKER;
    process.env.THECLAW_SERVICE_MARKER = "gateway";
    try {
      ...
      if (previousMarker === undefined) {
        delete process.env.THECLAW_SERVICE_MARKER;
      } else {
        process.env.THECLAW_SERVICE_MARKER = previousMarker;
      }
    }
  ...
  it.each([
    ...
    delete process.env.THECLAW_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.THECLAW_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode even if loadConfig still says local", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `theclaw onboard --mode local` or `theclaw setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("restores last-known-good config before startup when the effective config is invalid", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/theclaw-test-missing-config.json",
      config: {},
      parsed: null,
    ...
    expect(gatewayLogMessages).toContain(
      "gateway: restored invalid effective config from last-known-good backup: /tmp/theclaw-test-missing-config.json",
    );
    expect(startGatewayServer).toHaveBeenCalledWith(
      19170,
      expect.objectContaining({
  ...
  it("allows password mode preflight when password is configured via SecretRef", async () => {
    configState.cfg = {
      gateway: {
        auth: {
          mode: "password",
          password: { source: "env", provider: "default", id: "THECLAW_GATEWAY_PASSWORD" },
        },
      },
      secrets: {
        defaults: {
