...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("theclaw-daemon-install-int-");
    configPath = path.join(tempHome, "theclaw.json");
    process.env.HOME = tempHome;
    process.env.THECLAW_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer TheClaw", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.THECLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
