...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      expect.stringContaining(formatCliCommand("theclaw gateway restart")),
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/theclaw-cli/theclaw.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/theclaw-daemon/theclaw.json",
            exists: true,
            valid: true,
