...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    THECLAW_STATE_DIR: "/tmp/theclaw-daemon",
    THECLAW_CONFIG_PATH: "/tmp/theclaw-daemon/theclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.THECLAW_STATE_DIR ?? "/tmp/theclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.THECLAW_CONFIG_PATH ?? `${stateDir}/theclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/theclaw-daemon/");
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.THECLAW_STATE_DIR = "/tmp/theclaw-cli";
    process.env.THECLAW_CONFIG_PATH = "/tmp/theclaw-cli/theclaw.json";
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(callGatewayStatusProbe).toHaveBeenCalledWith(
      expect.objectContaining({
        requireRpc: true,
        configPath: "/tmp/theclaw-daemon/theclaw.json",
      }),
    );
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/theclaw-cli/theclaw.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.THECLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.THECLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-status-config-"));
    const configPath = path.join(tmp, "theclaw.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    process.env.THECLAW_STATE_DIR = tmp;
    process.env.THECLAW_CONFIG_PATH = configPath;
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        THECLAW_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.THECLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "theclaw-gateway" }],
        hints: [],
      },
      healthy: false,
