...
const createInstallPlanFixture = vi.hoisted(() => {
  return async (params?: { wrapperPath?: string; env?: Record<string, string | undefined> }) => {
    ...
    if (params?.wrapperPath || params?.env?.THECLAW_WRAPPER) {
      environment.THECLAW_WRAPPER = params.wrapperPath ?? params.env?.THECLAW_WRAPPER;
    }
    return {
...
vi.mock("../../config/io.js", () => ({
  ...
  readConfigFileSnapshotForWrite: vi.fn(async () => ({
    ...
    writeOptions: { expectedConfigPath: "/tmp/theclaw.json" },
  })),
}));
...
vi.mock("../../daemon/program-args.js", () => ({
  ...
  resolveTheClawWrapperPath: async (value: string | undefined) => value?.trim() || undefined,
}));
...
function mockResolvedGatewayTokenSecretRef() {
  resolveSecretInputRefMock.mockReturnValue({
    ref: { source: "env", provider: "default", id: "THECLAW_GATEWAY_TOKEN" },
  });
  resolveSecretRefValuesMock.mockResolvedValue(
    new Map([["env:default:THECLAW_GATEWAY_TOKEN", "resolved-from-secretref"]]),
  );
}
...
describe("runDaemonInstall", () => {
  ...
  it("does not treat env-template gateway.auth.token as plaintext during install", async () => {
    loadConfigMock.mockReturnValue({
      gateway: { auth: { mode: "token", token: "${THECLAW_GATEWAY_TOKEN}" } },
    });
    mockResolvedGatewayTokenSecretRef();
  ...
  it("reinstalls when the loaded service still embeds THECLAW_GATEWAY_TOKEN", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["theclaw", "gateway", "run"],
      environment: {
        THECLAW_GATEWAY_TOKEN: "stale-service-token",
      },
    } as never);
    ...
    expect(actionState.warnings).toContain(
      "Gateway service THECLAW_GATEWAY_TOKEN differs from the current install plan; refreshing the install.",
    );
  });
  ...
  it("returns already-installed when the embedded gateway token matches the install plan", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["theclaw", "gateway", "run"],
      environment: {
        THECLAW_GATEWAY_TOKEN: "durable-token",
      },
    } as never);
  ...
  it("preserves wrapper env from an installed but unloaded service during forced reinstall", async () => {
    service.isLoaded.mockResolvedValue(false);
    service.readCommand.mockResolvedValue({
      programArguments: ["/usr/local/bin/theclaw-doppler", "gateway", "run"],
      environment: {
    ...
    expect(buildGatewayInstallPlanMock).toHaveBeenCalledWith(
      expect.objectContaining({
        wrapperPath: "/usr/local/bin/theclaw-doppler",
  ...
  it("reinstalls when wrapper command matches but wrapper env is missing", async () => {
    ...
    await runDaemonInstall({
      json: true,
      wrapper: "/usr/local/bin/theclaw-doppler",
    });

    expect(installDaemonServiceAndEmitMock).toHaveBeenCalledTimes(1);
    expect(actionState.warnings).toContain(
      "Gateway service THECLAW_WRAPPER differs from the current wrapper install plan; refreshing the install.",
    );
  });
  ...
  it("does not reinstall when THECLAW_GATEWAY_TOKEN comes from an env file", async () => {
    service.isLoaded.mockResolvedValue(true);
    service.readCommand.mockResolvedValue({
      programArguments: ["theclaw", "gateway", "run"],
      environment: {
  ...
  it("does not reuse stale service control env during forced reinstall", async () => {
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["theclaw", "gateway", "run"],
      environment: {
        THECLAW_STATE_DIR: "/tmp/theclaw-doctor-manual",
        THECLAW_CONFIG_PATH: "/tmp/theclaw-doctor-manual/theclaw.json",
        THECLAW_GATEWAY_TOKEN: "stale-service-token",
    ...
    try {
      ...
      expect(env.THECLAW_STATE_DIR).toBeUndefined();
      expect(env.THECLAW_CONFIG_PATH).toBeUndefined();
