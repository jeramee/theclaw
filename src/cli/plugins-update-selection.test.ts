      ...
      resolvePluginUpdateSelection({
        installs: {
          "voice-call": createNpmInstall({
            ...
            resolvedName: "@theclaw/voice-call",
          }),
        },
        rawId: "@theclaw/voice-call@beta",
      }),
    ).toEqual({
      pluginIds: ["voice-call"],
      specOverrides: {
        "voice-call": "@theclaw/voice-call@beta",
      },
    });
  });
  ...
  it("maps an explicit npm version update to the tracked plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "theclaw-codex-app-server@0.2.0-beta.4",
      }),
    ).toEqual({
      pluginIds: ["theclaw-codex-app-server"],
      specOverrides: {
  ...
  it("keeps recorded npm tags when update is invoked by plugin id", () => {
    expect(
      resolvePluginUpdateSelection({
        ...
        rawId: "theclaw-codex-app-server",
      }),
    ).toEqual({
      pluginIds: ["theclaw-codex-app-server"],
    });
