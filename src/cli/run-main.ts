...
function createGatewayCliMainStartupTrace(argv: string[]) {
  const enabled =
    isTruthyEnvValue(process.env.THECLAW_GATEWAY_STARTUP_TRACE) &&
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.THECLAW_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.THECLAW_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.THECLAW_CONTAINER) ?? null;
  ...
  try {
    ...
    if (shouldRunBareRootCrestodian) {
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `theclaw crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        console.error("[theclaw] Uncaught exception:", formatUncaughtError(error));
