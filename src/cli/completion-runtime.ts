...
export const COMPLETION_SKIP_PLUGIN_COMMANDS_ENV = "THECLAW_COMPLETION_SKIP_PLUGIN_COMMANDS";
...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# TheClaw Completion";
}
...
function updateCompletionProfile(
  ...
  const block = `# TheClaw Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(theclaw completion ...)` instead of cached file.
 */
...
export async function installCompletion(shell: string, yes: boolean, binName = "theclaw") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
