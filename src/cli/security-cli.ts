...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["theclaw security audit", "Run a local security audit."],
          ["theclaw security audit --deep", "Include best-effort live Gateway probe checks."],
          ["theclaw security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "theclaw security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          ["theclaw security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["theclaw security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.theclaw.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("TheClaw security audit"));
      lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
      lines.push(muted(`Run deeper: ${formatCliCommand("theclaw security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("theclaw security audit --fix")}`));
        if (!fixResult) {
