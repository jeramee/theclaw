...
describe("completion-cli write-state", () => {
  ...
  it("keeps completion cache generation alive when a subcli fails to register", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-completion-state-"));
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-completion-home-"));
    ...
    expect(await fs.readdir(cacheDir)).toEqual(
      expect.arrayContaining(["theclaw.bash", "theclaw.fish", "theclaw.ps1", "theclaw.zsh"]),
    );
