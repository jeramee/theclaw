...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.THECLAW_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.THECLAW_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: TheClawConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-theclaw-message-channel")) ?? undefined,
    accountId: normalizeOptionalString(getHeader(req, "x-theclaw-account-id")),
