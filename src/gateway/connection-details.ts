...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: TheClawConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : normalizeOptionalString(process.env.THECLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.THECLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set THECLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: theclaw doctor --fix",
