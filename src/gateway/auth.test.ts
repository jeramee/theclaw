...
describe("gateway auth", () => {
  ...
  it("resolves token/password from THECLAW gateway env vars", () => {
    expect(
      resolveGatewayAuth({
        authConfig: {},
        env: {
          THECLAW_GATEWAY_TOKEN: "env-token",
          THECLAW_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides THECLAW_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        THECLAW_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
