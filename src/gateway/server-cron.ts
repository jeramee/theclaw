...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.THECLAW_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: TheClawConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: TheClawConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: TheClawConfig, requestedAgentId: string) => {
