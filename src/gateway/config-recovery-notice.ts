...
export function formatConfigRecoveryNotice(params: {
  ...
  const configName = path.basename(params.configPath) || "theclaw.json";
  return [
    `Config recovery warning: TheClaw restored ${configName} from the last-known-good backup during ${params.phase} (${params.reason}).`,
