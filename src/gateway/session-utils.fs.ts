...
export function attachTheClawTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record.__theclaw && typeof record.__theclaw === "object" && !Array.isArray(record.__theclaw)
      ? (record.__theclaw as Record<string, unknown>)
      : {};
  return {
    ...record,
    __theclaw: {
  ...
  if (branchEntries) {
    ...
    for (const entry of branchEntries) {
      if (entry.type === "message" && entry.message) {
        messageSeq += 1;
        messages.push(
          attachTheClawTranscriptMeta(entry.message, {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (parsed?.message) {
        messageSeq += 1;
        messages.push(
          attachTheClawTranscriptMeta(parsed.message, {
      ...
      const model =
        typeof message.model === "string"
          ...
          : typeof parsed.model === "string"
            ? parsed.model.trim()
            : undefined;
      const isDeliveryMirror = modelProvider === "theclaw" && model === "delivery-mirror";
