...
import type { ConfigFileSnapshot, ModelDefinitionConfig, TheClawConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: TheClawConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: TheClawConfig) => config),
}));
...
vi.mock("../config/validation.js", () => ({
  validateConfigObjectWithPlugins: vi.fn((config: TheClawConfig) => ({
    ok: true,
    config,
    warnings: [],
  })),
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: TheClawConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/theclaw-startup-recovery.json";
const validConfig = {
  gateway: {
    mode: "local",
  },
...
function buildSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: params.valid,
    config: params.config ?? ({} as TheClawConfig),
...
describe("gateway startup config recovery", () => {
  ...
  it("keeps startup validation loud when last-known-good recovery is unavailable", async () => {
    ...
    await expect(
      ...
      `Invalid config at ${configPath}.\ngateway.mode: Expected 'local' or 'remote'\nRun "theclaw doctor --fix" to repair, then retry.`,
    );
  ...
  it("continues startup in degraded mode for plugin-local startup invalidity", async () => {
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires TheClaw >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
      legacyIssues: [],
    });
    ...
    expect(configIo.recoverConfigFromJsonRootSuffix).not.toHaveBeenCalled();
    expect(log.warn).toHaveBeenCalledWith(
      `gateway: skipped plugin config validation issue at plugins.entries.feishu: plugin feishu: plugin requires TheClaw >=2026.4.23, but this host is 2026.4.22; skipping load. Run "theclaw doctor --fix" to quarantine the plugin config.`,
    );
    expect(recoveryNotice.enqueueConfigRecoveryNotice).not.toHaveBeenCalled();
  });
  ...
  it("skips providers with stale model api enum values during startup", async () => {
    ...
    expect(configMutate.replaceConfigFile).not.toHaveBeenCalled();
    expect(log.warn).toHaveBeenCalledWith(
      'gateway: skipped model provider openrouter; configured provider api is invalid. Run "theclaw doctor --fix" to repair the config.',
    );
  });
