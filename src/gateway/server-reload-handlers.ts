...
type GatewayReloadHandlerParams = {
  ...
  logReload: GatewayReloadLog;
  createHealthMonitor: (config: TheClawConfig) => ChannelHealthMonitor | null;
};
...
> & {
  ...
  resolveSharedGatewaySessionGenerationForConfig: (config: TheClawConfig) => string | undefined;
...
export function createGatewayReloadHandlers(params: GatewayReloadHandlerParams) {
  ...
  const applyHotReload = async (plan: GatewayReloadPlan, nextConfig: TheClawConfig) => {
    ...
    if (plan.restartGmailWatcher) {
      ...
      await startGmailWatcherWithLogs({
        ...
        log: params.logHooks,
        onSkipped: () =>
          params.logHooks.info("skipping gmail watcher restart (THECLAW_SKIP_GMAIL_WATCHER=1)"),
      });
    }
    ...
    if (plan.restartChannels.size > 0) {
      if (
        ...
        params.logChannels.info(
          "skipping channel reload (THECLAW_SKIP_CHANNELS=1 or THECLAW_SKIP_PROVIDERS=1)",
        );
      } else {
  ...
  const requestGatewayRestart = (plan: GatewayReloadPlan, nextConfig: TheClawConfig): boolean => {
    setGatewaySigusr1RestartPolicy({ allowExternal: isRestartEnabled(nextConfig) });
