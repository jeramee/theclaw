...
function loadGatewayConfig(): TheClawConfig {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: TheClawConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallContext(opts: CallGatewayBaseOptions): ResolvedGatewayCallContext {
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.THECLAW_GATEWAY_URL);
  ...
  const config = opts.config ?? (canSkipConfigLoad ? ({} as TheClawConfig) : loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `theclaw doctor` for diagnostics.";
  }
  return message;
}
