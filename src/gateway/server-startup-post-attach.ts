...
import type { loadTheClawPlugins } from "../plugins/loader.js";
...
function shouldStartGatewayMemoryBackend(cfg: TheClawConfig): boolean {
  return cfg.memory?.backend === "qmd";
}

function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadTheClawPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { ensureTheClawModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveTheClawAgentDir();
  try {
    await ensureTheClawModelsJson(params.cfg, agentDir, {
...
export async function startGatewaySidecars(params: {
  cfg: TheClawConfig;
  pluginRegistry: ReturnType<typeof loadTheClawPlugins>;
  ...
  await measureStartup(params.startupTrace, "sidecars.channels", async () => {
    if (!skipChannels) {
      ...
      params.logChannels.info(
        "skipping channel start (THECLAW_SKIP_CHANNELS=1 or THECLAW_SKIP_PROVIDERS=1)",
      );
    }
  });
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: TheClawConfig;
    pluginRegistry: ReturnType<typeof loadTheClawPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
