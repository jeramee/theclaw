...
const hoisted = vi.hoisted(() => {
  ...
  const resolveTheClawAgentDir = vi.fn(() => "/tmp/theclaw-state/agents/default/agent");
  ...
  const ensureTheClawModelsJson = vi.fn(async () => undefined);
  return {
    startPluginServices,
    ...
    normalizeProviderId,
    resolveTheClawAgentDir,
    isCliProvider,
    resolveConfiguredModelRef,
    resolveEmbeddedAgentRuntime,
    ensureTheClawModelsJson,
  };
});
...
vi.mock("../config/paths.js", async () => {
  ...
  return {
    ...actual,
    STATE_DIR: "/tmp/theclaw-state",
    resolveConfigPath: vi.fn(() => "/tmp/theclaw-state/theclaw.json"),
    resolveGatewayPort: vi.fn(() => 18789),
    resolveStateDir: vi.fn(() => "/tmp/theclaw-state"),
  };
});
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveTheClawAgentDir: hoisted.resolveTheClawAgentDir,
}));
...
vi.mock("../agents/models-config.js", () => ({
  ensureTheClawModelsJson: hoisted.ensureTheClawModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    vi.stubEnv("THECLAW_SKIP_CHANNELS", "0");
    vi.stubEnv("THECLAW_SKIP_PROVIDERS", "0");
    ...
    hoisted.resolveTheClawAgentDir.mockClear();
    ...
    hoisted.ensureTheClawModelsJson.mockReset();
    hoisted.ensureTheClawModelsJson.mockResolvedValue(undefined);
  });
  ...
  it("starts channels without waiting for primary model prewarm completion", async () => {
    await withEnvAsync(
      { THECLAW_SKIP_CHANNELS: undefined, THECLAW_SKIP_PROVIDERS: undefined },
      async () => {
        ...
        const sidecarsPromise = startGatewaySidecars({
          ...
          defaultWorkspaceDir: "/tmp/theclaw-workspace",
          deps: {} as never,
          startChannels,
