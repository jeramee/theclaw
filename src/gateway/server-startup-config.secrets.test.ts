...
function gatewayTokenConfig(config: TheClawConfig): TheClawConfig {
  return {
    ...config,
    gateway: {
      ...config.gateway,
...
function asConfig(value: unknown): TheClawConfig {
  return value as TheClawConfig;
}

function buildSnapshot(config: TheClawConfig): ConfigFileSnapshot {
  const raw = `${JSON.stringify(config, null, 2)}\n`;
  return buildTestConfigSnapshot({
    path: "/tmp/theclaw-startup-secrets-test.json",
    exists: true,
    raw,
    parsed: config,
    valid: true,
...
function preparedSnapshot(config: TheClawConfig): PreparedSecretsRuntimeSnapshot {
  return {
    sourceConfig: config,
    config,
    authStores: [],
...
describe("gateway startup config secret preflight", () => {
  const previousSkipChannels = process.env.THECLAW_SKIP_CHANNELS;
  const previousSkipProviders = process.env.THECLAW_SKIP_PROVIDERS;

  afterEach(() => {
    if (previousSkipChannels === undefined) {
      delete process.env.THECLAW_SKIP_CHANNELS;
    } else {
      process.env.THECLAW_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.THECLAW_SKIP_PROVIDERS;
    } else {
      process.env.THECLAW_SKIP_PROVIDERS = previousSkipProviders;
    }
  });
