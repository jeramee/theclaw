...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.THECLAW_LIVE_CODEX_BIND);
...
async function writePluginBindingApproval(params: {
  ...
  const theclawDir = path.join(params.homeDir, ".theclaw");
  await fs.mkdir(theclawDir, { recursive: true });
  await fs.writeFile(
    path.join(theclawDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.THECLAW_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-live-codex-bind-"));
      const tempHome = path.join(tempRoot, "home");
      const stateDir = path.join(tempRoot, "state");
      const workspace = path.join(tempRoot, "workspace");
      const configPath = path.join(tempRoot, "theclaw.json");
      ...
      const bindModel =
        process.env.THECLAW_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        restoreEnvVar("THECLAW_CONFIG_PATH", previous.configPath);
        restoreEnvVar("THECLAW_GATEWAY_TOKEN", previous.gatewayToken);
        restoreEnvVar("HOME", previous.home);
        restoreEnvVar("THECLAW_SKIP_CANVAS_HOST", previous.skipCanvas);
        restoreEnvVar("THECLAW_SKIP_CHANNELS", previous.skipChannels);
        restoreEnvVar("THECLAW_SKIP_CRON", previous.skipCron);
        restoreEnvVar("THECLAW_SKIP_GMAIL_WATCHER", previous.skipGmail);
        restoreEnvVar("THECLAW_STATE_DIR", previous.stateDir);
      }
    },
