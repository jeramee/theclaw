...
let lastCreateTheClawToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/theclaw-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateTheClawToolsContext?.agentTo,
          agentThreadId: lastCreateTheClawToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createTheClawTools: (ctx: Record<string, unknown>) => {
      lastCreateTheClawToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateTheClawToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-theclaw-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-theclaw-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateTheClawToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateTheClawToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateTheClawToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateTheClawToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("threads senderIsOwner into tool creation before owner-only filtering", async () => {
    ...
    expect(lastCreateTheClawToolsContext?.senderIsOwner).toBe(false);
    ...
    expect(lastCreateTheClawToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-theclaw-message-to": "channel:24514",
        "x-theclaw-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
