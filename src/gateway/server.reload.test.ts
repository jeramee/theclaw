...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    prevSkipChannels = process.env.THECLAW_SKIP_CHANNELS;
    prevSkipGmail = process.env.THECLAW_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.THECLAW_SKIP_PROVIDERS;
    prevOpenAiApiKey = process.env.OPENAI_API_KEY;
    process.env.THECLAW_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.THECLAW_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.THECLAW_SKIP_GMAIL_WATCHER;
    } else {
      process.env.THECLAW_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.THECLAW_SKIP_PROVIDERS;
    } else {
      process.env.THECLAW_SKIP_PROVIDERS = prevSkipProviders;
    }
  ...
  async function writeConfigFile(config: unknown) {
    ...
    if (!configPath) {
      throw new Error("THECLAW_CONFIG_PATH is not set");
    }
  ...
  async function withNonMinimalGatewayServer(
    ...
    return await withEnvAsync({ THECLAW_TEST_MINIMAL_GATEWAY: undefined }, async () =>
  ...
  it("makes newly available catalog models visible in-process after hot reload", async () => {
    ...
    try {
      await withGatewayServer(async () => {
        ...
        const baseConfig: TheClawConfig = {
          agents: {
            defaults: {
              model: {
...
`,
    ...
    const previousGatewayTokenEnv = process.env.THECLAW_GATEWAY_TOKEN;
    ...
    try {
      ...
      if (previousGatewayTokenEnv === undefined) {
        ...
        process.env.THECLAW_GATEWAY_TOKEN = previousGatewayTokenEnv;
      }
