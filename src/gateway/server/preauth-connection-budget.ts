...
const UNKNOWN_CLIENT_IP_BUDGET_KEY = "__theclaw_unknown_client_ip__";

export function getMaxPreauthConnectionsPerIpFromEnv(env: NodeJS.ProcessEnv = process.env): number {
  const configured =
    env.THECLAW_MAX_PREAUTH_CONNECTIONS_PER_IP ||
    (env.VITEST && env.THECLAW_TEST_MAX_PREAUTH_CONNECTIONS_PER_IP);
  if (!configured) {
