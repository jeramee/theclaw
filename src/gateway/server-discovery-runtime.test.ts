...
const mocks = vi.hoisted(() => ({
  ...
  resolveWideAreaDiscoveryDomain: vi.fn(() => "theclaw.internal."),
  writeWideAreaGatewayZone: vi.fn(async () => ({
    changed: true,
    zonePath: "/tmp/theclaw.internal.db",
  })),
  formatBonjourInstanceName: vi.fn((name: string) => `${name} (TheClaw)`),
  resolveBonjourCliPath: vi.fn(() => "/usr/local/bin/theclaw"),
...
describe("startGatewayDiscovery", () => {
  ...
  it("starts registered local discovery services with gateway advertisement context", async () => {
    ...
    process.env.THECLAW_SSH_PORT = "2222";
  ...
  it("skips local discovery services for truthy THECLAW_DISABLE_BONJOUR values", async () => {
    process.env.NODE_ENV = "development";
    delete process.env.VITEST;
    process.env.THECLAW_DISABLE_BONJOUR = "yes";
  ...
  it("keeps wide-area DNS-SD publishing active when local discovery is off", async () => {
    ...
    const result = await startGatewayDiscovery({
      ...
      wideAreaDiscoveryDomain: "theclaw.internal.",
      tailscaleMode: "serve",
      mdnsMode: "off",
      gatewayDiscoveryServices: [service],
      logDiscovery: logs,
