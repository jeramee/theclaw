...
const { createTheClawTools } = await import("../agents/theclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createTheClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createTheClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["THECLAW_GATEWAY_PORT", "THECLAW_GATEWAY_TOKEN"]);
  ...
  process.env.THECLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.THECLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("THECLAW_CONFIG_PATH missing in gateway test environment");
      }
