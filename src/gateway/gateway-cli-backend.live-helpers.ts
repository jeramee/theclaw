...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("THECLAW_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.THECLAW_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.THECLAW_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "THECLAW_LIVE_CLI_BACKEND_ARGS",
      process.env.THECLAW_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `THECLAW_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "THECLAW_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.THECLAW_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.THECLAW_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.THECLAW_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.THECLAW_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.THECLAW_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.THECLAW_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.THECLAW_SKIP_PROVIDERS = "1";
  process.env.THECLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.THECLAW_SKIP_CRON = "1";
  process.env.THECLAW_SKIP_CANVAS_HOST = "1";
  process.env.THECLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.THECLAW_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("THECLAW_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("THECLAW_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("THECLAW_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("THECLAW_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("THECLAW_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("THECLAW_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("THECLAW_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("THECLAW_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("THECLAW_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("THECLAW_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("THECLAW_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
