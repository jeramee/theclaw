...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listTheClawPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    listTheClawPluginManifestMetadata: (
      params?: Parameters<typeof actual.listTheClawPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listTheClawPluginManifestMetadata(params);
      return actual.listTheClawPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listTheClawPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as TheClawConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listTheClawPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/theclaw.plugin.json",
    ...overrides,
  };
}
