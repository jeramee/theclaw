...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("theclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with THECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with THECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.THECLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://theclaw-gateway.ai:18789",
      onConnectError,
    });
