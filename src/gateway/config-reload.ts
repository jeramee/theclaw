...
function resolvePluginLocalInvalidReloadSnapshot(params: {
  ...
  for (const issue of params.snapshot.issues) {
    params.log.warn(
      `config reload skipped plugin config validation issue at ${issue.path}: ${issue.message}. Run "theclaw doctor --fix" to quarantine the plugin config.`,
    );
  }
  return {
    ...params.snapshot,
...
export function resolveGatewayReloadSettings(cfg: TheClawConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: TheClawConfig;
  initialCompareConfig?: TheClawConfig;
  initialInternalWriteHash?: string | null;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: TheClawConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: TheClawConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: TheClawConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (
    ...
    nextCompareConfig: TheClawConfig,
    afterWrite?: ConfigWriteNotification["afterWrite"],
  ) => {
