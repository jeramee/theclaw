...
const ensureTheClawModelsJsonMock = vi.fn<
  (
    config: unknown,
    agentDir: unknown,
    options?: unknown,
...
vi.mock("../agents/agent-paths.js", () => ({
  resolveTheClawAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureTheClawModelsJson: (config: unknown, agentDir: unknown, options?: unknown) =>
    ensureTheClawModelsJsonMock(config, agentDir, options),
}));
...
describe("gateway startup primary model warmup", () => {
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureTheClawModelsJsonMock).toHaveBeenCalledWith(
      cfg,
      "/tmp/agent",
  ...
  it("warns when scoped models.json preparation fails", async () => {
    ensureTheClawModelsJsonMock.mockRejectedValueOnce(new Error("models write failed"));
    const warn = vi.fn();
