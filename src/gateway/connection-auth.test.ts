...
function cfg(input: Partial<TheClawConfig>): TheClawConfig {
  return input as TheClawConfig;
}
...
const DEFAULT_ENV = {
  THECLAW_GATEWAY_TOKEN: "env-token",
  THECLAW_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when THECLAW env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when THECLAW env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when THECLAW env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
