...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.THECLAW_CONFIG_PATH) {
    configPaths.add(process.env.THECLAW_CONFIG_PATH);
  }
  if (process.env.THECLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.THECLAW_STATE_DIR, "theclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.THECLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "theclaw-test-no-bundled-extensions")
    : "theclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  const stateDir = process.env.THECLAW_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".theclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".theclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".theclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>theclaw-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__theclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__theclawTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["THECLAW_GATEWAY_TOKEN"]);
  const prev = process.env.THECLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.THECLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.THECLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
