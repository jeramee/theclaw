...
export function warnLegacyTheClawEnvVars(env: NodeJS.ProcessEnv = process.env): void {
  ...
  process.emitWarning(
    [
      `Legacy ${detectedPrefixes} environment variables were detected (${legacyVarCount} total), but TheClaw only reads THECLAW_* names now.`,
      "Rename them by replacing the legacy prefix with THECLAW_; the old names are ignored.",
    ].join("\n"),
    { code: "THECLAW_LEGACY_ENV_VARS", type: "DeprecationWarning" },
  );
  warned = true;
}

export function resetLegacyTheClawEnvWarningForTest(): void {
  warned = false;
}
