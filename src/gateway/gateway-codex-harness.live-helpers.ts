export const EXPECTED_CODEX_MODELS_COMMAND_TEXT = [
  ...
  "Current TheClaw session status reports the active model as:",
] as const;
...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsTheClawStatus =
    normalized.includes("theclaw is running on") ||
    normalized.includes("theclaw status:") ||
  ...
  return (
    isCurrentSessionStatus || (mentionsTheClawStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live theclaw config shows") ||
