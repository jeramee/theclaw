...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "theclaw-test-empty-bundled-plugins");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".theclaw", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["THECLAW_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);
  ...
  const workspaceDir = path.join(tempHome, "theclaw");
  await fs.mkdir(workspaceDir, { recursive: true });
  process.env.THECLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const configDir = path.join(tempHome, ".theclaw");
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "theclaw-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-wizard-home-"));
      process.env.HOME = tempHome;
      process.env.THECLAW_BUNDLED_PLUGINS_DIR = await createEmptyBundledPluginsDir(tempHome);
      process.env.THECLAW_DISABLE_BUNDLED_PLUGINS = "1";
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-minimal-gateway-home-"));
      const configPath = path.join(tempHome, ".theclaw", "theclaw.json");
      const bundledPluginsDir = path.join(tempHome, "theclaw-test-no-bundled-extensions");
      process.env.HOME = tempHome;
      process.env.THECLAW_STATE_DIR = path.join(tempHome, ".theclaw");
      process.env.THECLAW_CONFIG_PATH = configPath;
      process.env.THECLAW_SKIP_CHANNELS = "1";
      ...
      process.env.THECLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
      process.env.THECLAW_SKIP_PROVIDERS = "1";
      process.env.THECLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
      process.env.THECLAW_TEST_MINIMAL_GATEWAY = "1";
