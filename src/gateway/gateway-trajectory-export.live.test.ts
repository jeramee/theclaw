...
const CODEX_HARNESS_LIVE = process.env.THECLAW_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.THECLAW_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["THECLAW_TRAJECTORY", "THECLAW_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-theclaw-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.THECLAW_TRAJECTORY = "1";
      process.env.THECLAW_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".theclaw", "trajectory-exports", "bundle");
