...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([THECLAW_MODEL_ID, THECLAW_DEFAULT_MODEL_ID]);
  ids.add(`theclaw/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`theclaw/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== THECLAW_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
