...
import type { TheClawConfig, TtsConfig, TtsProviderConfigMap } from "../../config/types.js";
import {
...
function buildTalkTtsConfig(
  ...
  | { cfg: TheClawConfig; provider: string; providerConfig: TalkProviderConfig }
...
function getVoiceCallRealtimeConfig(config: TheClawConfig): {
  provider?: string;
...
function buildTalkRealtimeConfig(config: TheClawConfig, requestedProvider?: string) {
...
function buildRealtimeInstructions(): string {
  return `You are TheClaw's realtime voice interface. Keep spoken replies concise. If the user asks for code, repository state, tools, files, current TheClaw context, or deeper reasoning, call ${REALTIME_VOICE_AGENT_CONSULT_TOOL_NAME} and then summarize the result naturally.`;
}
