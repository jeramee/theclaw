...
vi.mock("../../infra/theclaw-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/theclaw-root.js")>(
    "../../infra/theclaw-root.js",
  );
  return {
    ...actual,
    resolveTheClawPackageRoot: async () => "/tmp/theclaw",
  };
});
...
describe("update.run restart scheduling", () => {
  ...
  it("blocks unmanaged global installs before package mutation when restart is unavailable", async () => {
    ...
    resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
      kind: "global",
      mode: "npm",
      root: "/tmp/theclaw-global",
      packageRoot: "/tmp/theclaw-global",
    });
