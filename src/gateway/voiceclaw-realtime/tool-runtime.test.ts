...
const previousToolTimeoutMs = process.env.THECLAW_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS;
const previousMaxConcurrentTools = process.env.THECLAW_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS;

afterEach(() => {
  restoreEnv("THECLAW_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS", previousToolTimeoutMs);
  restoreEnv("THECLAW_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS", previousMaxConcurrentTools);
});
...
describe("VoiceClawRealtimeToolRuntime", () => {
  ...
  it("does not turn non-cooperative cancellations into timeout injections", async () => {
    process.env.THECLAW_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS = "10";
    const runtime = new VoiceClawRealtimeToolRuntime([
  ...
  it("frees the concurrency slot after a non-cooperative tool times out", async () => {
    process.env.THECLAW_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS = "10";
    process.env.THECLAW_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS = "1";
    const runtime = new VoiceClawRealtimeToolRuntime([
      makeTool("stuck", async () => await new Promise<never>(() => {})),
    ...
    expect(callbacks.progress.map((entry) => entry.summary)).toContain(
      "stuck failed: TheClaw tool timed out after 10ms",
    );
