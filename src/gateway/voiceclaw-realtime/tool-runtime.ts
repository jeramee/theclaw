...
import { createTheClawCodingTools } from "../../agents/pi-tools.js";
...
type ToolRuntimeDeps = {
  createTools?: typeof createTheClawCodingTools;
};
...
export class VoiceClawRealtimeToolRuntime {
    ...
    if (this.inFlight.size >= this.maxConcurrentTools) {
      callbacks.sendToolResult(
        ...
        JSON.stringify({
          status: "busy",
          tool: event.name,
          error: "Too many TheClaw tools are already running.",
        }),
      );
      return true;
    }
  ...
  abortTool(callId: string): void {
    ...
    inFlight.controller.abort(new Error("TheClaw tool cancelled"));
  }
    ...
    try {
      ...
      const message =
        inFlight.abortReason === "timeout"
          ? `TheClaw tool timed out after ${this.timeoutMs}ms`
  ...
  private async executeToolWithTimeout(params: {
    ...
    const timeout = new Promise<never>((_, reject) => {
      inFlight.timeout = setTimeout(() => {
        if (inFlight.abortReason === "cancelled") {
          reject(new Error("TheClaw tool cancelled"));
          return;
        }
        inFlight.abortReason = "timeout";
        inFlight.controller.abort(new Error(`TheClaw tool timed out after ${this.timeoutMs}ms`));
        reject(new Error(`TheClaw tool timed out after ${this.timeoutMs}ms`));
      }, this.timeoutMs);
    });
...
export function createVoiceClawRealtimeToolRuntime(
  ...
  const createTools = options.deps?.createTools ?? createTheClawCodingTools;
...
function resolveToolTimeoutMs(): number {
  const value = Number.parseInt(process.env.THECLAW_VOICECLAW_REALTIME_TOOL_TIMEOUT_MS ?? "", 10);
  return Number.isFinite(value) && value > 0 ? value : DEFAULT_TOOL_TIMEOUT_MS;
}

function resolveMaxConcurrentTools(): number {
  const value = Number.parseInt(
    process.env.THECLAW_VOICECLAW_REALTIME_MAX_CONCURRENT_TOOLS ?? "",
    10,
  );
