...
export class VoiceClawRealtimeSession {
  ...
  private readonly gatewayConfig: TheClawConfig;
  ...
  private async startSession(config: VoiceClawSessionConfigEvent): Promise<void> {
    ...
    if (!authResult.ok) {
      this.send({ type: "error", message: "TheClaw gateway authentication failed", code: 401 });
    ...
    if (config.brainAgent !== "none" && this.auth.mode === "none" && !localDirect) {
      this.send({
        type: "error",
        message: "TheClaw real-time brain requires gateway auth for non-local connections",
        code: 403,
      });
    ...
    if (config.brainAgent !== "none" && !senderIsOwner) {
      this.send({
        type: "error",
        message: "TheClaw real-time brain requires owner-equivalent gateway auth",
        code: 403,
      });
