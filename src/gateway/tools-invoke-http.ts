...
function resolveMemoryToolDisableReasons(cfg: TheClawConfig): string[] {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-theclaw-message-channel") ?? "",
  );
  const accountId = normalizeOptionalString(getHeader(req, "x-theclaw-account-id"));
  const agentTo = normalizeOptionalString(getHeader(req, "x-theclaw-message-to"));
  const agentThreadId = normalizeOptionalString(getHeader(req, "x-theclaw-thread-id"));
