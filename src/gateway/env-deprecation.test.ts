...
import {
  resetLegacyTheClawEnvWarningForTest,
  warnLegacyTheClawEnvVars,
} from "./env-deprecation.js";

describe("warnLegacyTheClawEnvVars", () => {
  ...
  it("warns with counts and prefixes instead of secret-shaped env names", () => {
    ...
    expect(message).toContain("replacing the legacy prefix with THECLAW_");
    ...
    expect(options).toEqual({
      code: "THECLAW_LEGACY_ENV_VARS",
      type: "DeprecationWarning",
    });
  });

  it("does not warn for current THECLAW names", () => {
    warnLegacyTheClawEnvVars({ THECLAW_GATEWAY_TOKEN: "token" });

    expect(emitWarning).not.toHaveBeenCalled();
  });

  it("warns only once after a successful emit", () => {
    warnLegacyTheClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
    warnLegacyTheClawEnvVars({ MOLTBOT_GATEWAY_TOKEN: "old-token" });
  ...
  it("retries if emitWarning throws before the warning is emitted", () => {
    ...
    expect(() => warnLegacyTheClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" })).toThrow(
      "warning sink failed",
    );
    warnLegacyTheClawEnvVars({ CLAWDBOT_GATEWAY_TOKEN: "old-token" });
