...
function resetGatewayCallMocks() {
  ...
  const loadConfigForTests = getRuntimeConfig as unknown as () => TheClawConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.THECLAW_ALLOW_INSECURE_PRIVATE_WS;
  ...
  it("uses THECLAW_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.THECLAW_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.THECLAW_GATEWAY_TOKEN = "env-token";
  ...
  it("yields one event-loop turn before starting CLI pairing requests", async () => {
    ...
    __testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => TheClawConfig,
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env THECLAW_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.THECLAW_GATEWAY_URL;
    try {
      process.env.THECLAW_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env THECLAW_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.THECLAW_GATEWAY_URL;
      } else {
        process.env.THECLAW_GATEWAY_URL = prevUrl;
      }
    }
  });

  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-gateway-call-"));
    process.env.THECLAW_STATE_DIR = tempStateDir;
    process.env.THECLAW_CONFIG_PATH = path.join(tempStateDir, "missing-config.json");
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      __testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("theclaw doctor --fix");
  });

  it("allows ws:// private remote URLs only when THECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.THECLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
  ...
  it("allows ws:// hostname remote URLs when THECLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.THECLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://theclaw-gateway.ai:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://theclaw-gateway.ai:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.THECLAW_GATEWAY_TOKEN = "env-token";
    process.env.THECLAW_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.THECLAW_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  const explicitAuthCases = [
    {
      ...
      envKey: "THECLAW_GATEWAY_PASSWORD",
      envValue: "from-env",
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.THECLAW_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.THECLAW_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
