...
import type { ConfigFileSnapshot, TheClawConfig } from "../config/types.theclaw.js";
...
function resolveGatewayStartupConfigWithoutInvalidModelProviders(params: {
  ...
  for (const providerId of providerIds) {
    params.log.warn(
      `gateway: skipped model provider ${providerId}; configured provider api is invalid. Run "theclaw doctor --fix" to repair the config.`,
    );
  }
  return {
    ...params.snapshot,
...
function resolveGatewayStartupConfigWithoutInvalidPluginEntries(params: {
  ...
  for (const issue of params.snapshot.issues) {
    params.log.warn(
      `gateway: skipped plugin config validation issue at ${issue.path}: ${issue.message}. Run "theclaw doctor --fix" to quarantine the plugin config.`,
    );
  }
  return {
    ...params.snapshot,
...
export function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("theclaw doctor --fix")}" to repair, then retry.`
    : "";
...
function hasActiveGatewayAuthSecretRef(config: TheClawConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: TheClawConfig): TheClawConfig {
  const skipChannels =
    isTruthyEnvValue(process.env.THECLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.THECLAW_SKIP_PROVIDERS);
...
function assertRuntimeGatewayAuthNotKnownWeak(config: TheClawConfig): void {
