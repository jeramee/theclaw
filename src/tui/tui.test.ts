...
describe("resolveInitialTuiAgentId", () => {
  const cfg: TheClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/theclaw" },
        { id: "ops", workspace: "/tmp/theclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/theclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run theclaw devices list");
    expect(state.pairingHint).toContain("theclaw devices list");
  });
...
describe("resolveLocalAuthCliInvocation", () => {
  ...
  it("uses the packaged wrapper when dist is available", () => {
    expect(
      ...
      command: "/usr/bin/node",
      args: ["/repo/theclaw.mjs", "models", "auth", "login"],
    });
  });
});
