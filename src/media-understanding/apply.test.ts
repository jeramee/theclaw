...
const TEMP_MEDIA_PREFIX = "theclaw-media-";
...
function createGroqAudioConfig(): TheClawConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): TheClawConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): TheClawConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: TheClawConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredTheClawTmpDir();
