...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[theclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[theclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`theclaw proxy coverage\` for details.`,
  );
}
