...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: TheClawConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: TheClawConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildResolvedProviderMaps = (cfg?: TheClawConfig) =>
    buildCapabilityProviderMaps(resolver.listProviders(cfg));

  const listProviders = (cfg?: TheClawConfig): SpeechProviderPlugin[] => [
