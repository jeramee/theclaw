...
beforeAll(() => {
  fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-tts-status-"));
});
...
async function withStatusTempHome(run: (home: string) => Promise<void>): Promise<void> {
  ...
  process.env.THECLAW_STATE_DIR = path.join(home, ".theclaw");
  try {
    await run(home);
  } finally {
    restoreEnv("HOME", previousHome);
    restoreEnv("USERPROFILE", previousUserProfile);
    restoreEnv("THECLAW_HOME", previousTheClawHome);
    restoreEnv("THECLAW_STATE_DIR", previousStateDir);
  }
}
...
describe("resolveStatusTtsSnapshot", () => {
  it("uses prefs overrides without loading speech providers", async () => {
    await withStatusTempHome(async (home) => {
      const prefsPath = path.join(home, ".theclaw", "settings", "tts.json");
  ...
  it("derives the default prefs path from THECLAW_CONFIG_PATH when set", async () => {
    await withStatusTempHome(async (home) => {
      const stateDir = path.join(home, ".theclaw-dev");
      ...
      vi.stubEnv("THECLAW_CONFIG_PATH", path.join(stateDir, "theclaw.json"));
      try {
        expect(
          resolveStatusTtsSnapshot({
            cfg: {
