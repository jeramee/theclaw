...
describe("path-resolve helpers (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-path-direct-"));
  ...
  it("resolveAuthStorePath falls back to resolveTheClawAgentDir when agentDir is omitted", () => {
    // Omitting agentDir exercises the `agentDir ?? resolveTheClawAgentDir()`
    // nullish branch. With THECLAW_STATE_DIR set to our tempdir, the
  ...
  it("resolveAuthStorePathForDisplay preserves a tilde-rooted path unchanged", () => {
    ...
    const tildeAgentDir = "~fake-theclaw-no-expand";
...
describe("ensureAuthStoreFile (direct-import coverage attribution)", () => {
  ...
  beforeEach(async () => {
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-path-ensure-"));
    process.env.THECLAW_STATE_DIR = stateDir;
  });
