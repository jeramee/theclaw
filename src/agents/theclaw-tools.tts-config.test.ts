...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    textToSpeech: vi.fn(async () => ({
      success: true,
      audioPath: "/tmp/theclaw/tts-config-test.opus",
      provider: "microsoft",
...
vi.mock("./theclaw-plugin-tools.js", () => ({
  resolveTheClawPluginToolsForOptions: () => [],
}));

vi.mock("./theclaw-tools.nodes-workspace-guard.js", () => ({
...
describe("createTheClawTools TTS config wiring", () => {
  beforeEach(() => {
  ...
  it("passes the resolved shared config into the tts tool", async () => {
    ...
    const { __testing, createTheClawTools } = await import("./theclaw-tools.js");
...
describe("createTheClawTools cron context wiring", () => {
  beforeEach(() => {
    mocks.createCronToolOptions.mockClear();
  });

  it("passes preserved channel delivery context into the cron tool", async () => {
    const { createTheClawTools } = await import("./theclaw-tools.js");
