...
describe("runCliAgent bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-cli-bundle-mcp-"));
      ...
      const pluginRoot = path.join(tempHome, ".theclaw", "extensions", "bundle-probe");
  ...
  it(
    ...
    async () => {
      ...
      const { resetGlobalHookRunner } = await import("../plugins/hook-runner-global.js");
      const envSnapshot = captureEnv(["HOME"]);
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-cli-live-cleanup-"));
      process.env.HOME = tempHome;
      resetGlobalHookRunner();
