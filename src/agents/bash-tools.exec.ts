...
function stripTheClawPackageRunner(argv: string[]): string[] {
  const commandName = normalizeCommandBaseName(argv[0]);
  if (commandName === "theclaw") {
    return argv;
  }
  if (
    (commandName === "pnpm" || commandName === "npm" || commandName === "yarn") &&
    normalizeCommandBaseName(argv[1]) === "theclaw"
  ) {
    return argv.slice(1);
  }
  if (
    ...
    normalizeCommandBaseName(argv[2]) === "theclaw"
  ) {
    return argv.slice(2);
  }
  if (commandName === "npx" || commandName === "bunx") {
    ...
    if (normalizeCommandBaseName(argv[idx]) === "theclaw") {
      return argv.slice(idx);
    }
  }
  return argv;
...
function parseTheClawChannelsLoginShellCommand(raw: string): boolean {
  const argv = splitShellArgs(raw);
  if (!argv) {
    return false;
  }
  const theclawArgv = stripTheClawPackageRunner(argv);
  return (
    normalizeCommandBaseName(theclawArgv[0]) === "theclaw" &&
    (theclawArgv[1] === "channels" || theclawArgv[1] === "channel") &&
    theclawArgv[2] === "login"
  );
}
...
function rejectUnsafeControlShellCommand(command: string): void {
  ...
  for (const candidate of candidates) {
    ...
    if (parseTheClawChannelsLoginShellCommand(candidate)) {
      throw new Error(
        [
          "exec cannot run interactive TheClaw channel login commands.",
          "Run `theclaw channels login` in a terminal on the gateway host, or use the channel-specific login agent tool when available (for WhatsApp: `whatsapp_login`).",
        ].join(" "),
      );
    }
  }
