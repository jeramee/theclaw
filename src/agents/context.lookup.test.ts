...
const contextTestState = vi.hoisted(() => {
  const state = {
    ...
    ensureTheClawModelsJson: vi.fn(async () => {}),
...
vi.mock("./models-config.runtime.js", () => ({
  ensureTheClawModelsJson: contextTestState.ensureTheClawModelsJson,
}));

vi.mock("./agent-paths.js", () => ({
  resolveTheClawAgentDir: () => "/tmp/theclaw-agent",
}));
...
function mockContextDeps(params: {
  ...
  contextTestState.ensureTheClawModelsJson.mockClear();
  return { ensureTheClawModelsJson: contextTestState.ensureTheClawModelsJson };
}
...
describe("lookupContextTokens", () => {
  ...
  it("only warms eagerly for real theclaw startup commands that need model metadata", async () => {
    const { shouldEagerWarmContextWindowCache } = await importContextModule();

    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "chat"])).toBe(true);
    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "chat", "--help"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "theclaw", "matrix", "encryption", "help"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "help", "matrix"])).toBe(false);
    expect(
      shouldEagerWarmContextWindowCache(["node", "theclaw", "browser", "status", "--help"]),
    ).toBe(false);
    expect(
    ...
    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "logs", "--limit", "5"])).toBe(
      false,
    );
    expect(
      shouldEagerWarmContextWindowCache(["node", "theclaw", "memory", "search", "--json"]),
    ).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "status", "--json"])).toBe(false);
    expect(shouldEagerWarmContextWindowCache(["node", "theclaw", "sessions", "--json"])).toBe(
      false,
    );
    expect(
