 ...
 *
 * @see https://github.com/theclaw/theclaw/issues/47711
 */
...
export async function recoverOrphanedSubagentSessions(params: {
  ...
  const configChangePattern = /theclaw\.json|theclaw gateway restart|config\.patch/i;
      ...
      try {
        ...
        const resumeResult = await resumeOrphanedSession({
          ...
          configChangeHint: configChangeDetected
            ? "\n\n[config changes from your previous run were already applied — do not re-modify theclaw.json or restart the gateway]"
            : undefined,
          originalRunId: runId,
