...
const parseTheClawChannelsLoginShellCommand = __testing.parseTheClawChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("theclaw-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive TheClaw channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseTheClawChannelsLoginShellCommand("theclaw channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseTheClawChannelsLoginShellCommand(
        "pnpm exec theclaw channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseTheClawChannelsLoginShellCommand("theclaw channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-theclaw-channel-login", {
        command: "theclaw channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive TheClaw channel login commands/);
    await expect(
      tool.execute("call-wrapped-theclaw-channel-login", {
        command: "sudo -u theclaw bash -lc 'theclaw channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive TheClaw channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("theclaw-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("theclaw-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("theclaw-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("theclaw-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__theclaw_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
