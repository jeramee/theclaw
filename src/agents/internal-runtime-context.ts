export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_THECLAW_INTERNAL_CONTEXT>>>";
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_THECLAW_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[THECLAW_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[THECLAW_INTERNAL_CONTEXT_END]]";

export const THECLAW_RUNTIME_CONTEXT_NOTICE =
  "This context is runtime-generated, not user-authored. Keep internal details private.";
export const THECLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER =
  "TheClaw runtime context for the immediately preceding user message.";
export const THECLAW_RUNTIME_EVENT_HEADER = "TheClaw runtime event.";
export const THECLAW_RUNTIME_CONTEXT_CUSTOM_TYPE = "theclaw.runtime-context";

const LEGACY_INTERNAL_CONTEXT_HEADER =
  ["TheClaw runtime context (internal):", THECLAW_RUNTIME_CONTEXT_NOTICE, ""].join("\n") + "\n";
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === THECLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === THECLAW_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === THECLAW_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${THECLAW_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${THECLAW_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${THECLAW_RUNTIME_EVENT_HEADER}\n${THECLAW_RUNTIME_CONTEXT_NOTICE}`)
  );
}

export function isTheClawRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === THECLAW_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isTheClawRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isTheClawRuntimeContextCustomMessage(message));
}
