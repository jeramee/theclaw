...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "theclaw-adc-"));
...
function buildDemoLocalProviderCfg(apiKey: string): TheClawConfig {
  return {
    models: {
      providers: {
        "demo-local": {
  ...
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withTheClawTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.THECLAW_AGENT_DIR, {
