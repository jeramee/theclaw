...
describe("ensureAuthProfileStore", () => {
  ...
  function restoreAgentDirEnv(params: {
    ...
    restoreEnvValue("THECLAW_AGENT_DIR", params.previousAgentDir);
  ...
  it("migrates legacy auth.json and deletes it (PR #368)", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-profiles-"));
    try {
  ...
  it("merges main auth profiles into agent store and keeps agent overrides", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-merge-"));
    ...
    try {
      ...
      process.env.THECLAW_AGENT_DIR = mainDir;
  ...
  it("uses the main agent's newer OAuth profile when an agent still has a stale default profile", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-drift-"));
    const previousAgentDir = process.env.THECLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("keeps a newer agent replacement credential while repairing stale default references", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-drift-newer-agent-"));
    const previousAgentDir = process.env.THECLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("preserves a valid main default OAuth profile while replacing a stale agent override", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-drift-base-default-"));
    const previousAgentDir = process.env.THECLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it("keeps a stale default OAuth profile when the main profile belongs to a different identity", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-drift-mismatch-"));
    const previousAgentDir = process.env.THECLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const mainDir = path.join(root, "main-agent");
  ...
  it.each([
    ...
    ({ name, profile, expected }) => {
      withTempAgentDir("theclaw-auth-alias-", (agentDir) => {
        const storeData = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it("normalizes mode/apiKey aliases while migrating legacy auth.json", () => {
    withTempAgentDir("theclaw-auth-legacy-alias-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not load legacy flat auth-profiles.json entries at runtime", () => {
    const agentDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-auth-flat-profiles-"));
    try {
  ...
  it("merges legacy oauth.json into auth-profiles.json", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-oauth-migrate-"));
    ...
    try {
      ...
      process.env.THECLAW_STATE_DIR = root;
      process.env.THECLAW_AGENT_DIR = agentDir;
      ...
      clearRuntimeAuthProfileStoreSnapshots();
      restoreEnvValue("THECLAW_STATE_DIR", previousStateDir);
      restoreAgentDirEnv({ previousAgentDir, previousPiAgentDir });
      fs.rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("exposes provider-managed runtime auth without persisting copied tokens", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-external-auth-"));
    const previousAgentDir = process.env.THECLAW_AGENT_DIR;
    const previousPiAgentDir = process.env.PI_CODING_AGENT_DIR;
    try {
      const agentDir = path.join(root, "agent");
  ...
  it("does not write inherited auth stores during secrets runtime reads", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-secrets-runtime-"));
    const previousStateDir = process.env.THECLAW_STATE_DIR;
    try {
  ...
  it("logs one warning with aggregated reasons for rejected auth-profiles entries", () => {
    ...
    try {
      withTempAgentDir("theclaw-auth-invalid-", (agentDir) => {
        const invalidStore = {
          version: AUTH_STORE_VERSION,
          profiles: {
  ...
  it.each([
    ...
    },
    {
      ...
      prefix: "theclaw-nonstr-key-malformed-ref-",
      profileId: "openai:default",
      profile: {
        type: "api_key",
        provider: "openai",
    ...
    },
    {
      ...
      prefix: "theclaw-nonstr-token-ref-",
    ...
    },
    {
      ...
      prefix: "theclaw-nonstr-token-num-",
      profileId: "anthropic:default",
      profile: {
        type: "token",
        provider: "anthropic",
