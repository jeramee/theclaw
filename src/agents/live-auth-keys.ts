...
const GOOGLE_LIVE_SINGLE_KEY = "THECLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "THECLAW_LIVE_ANTHROPIC_KEY",
    listVar: "THECLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "THECLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `THECLAW_LIVE_${base}_KEY`;
