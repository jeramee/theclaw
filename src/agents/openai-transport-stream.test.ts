...
describe("openai transport stream", () => {
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("theclaw-openai-responses-transport");
    expect(prepared).toMatchObject({
      api: "theclaw-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("theclaw-anthropic-messages-transport");
    expect(prepared).toMatchObject({
      api: "theclaw-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "theclaw-google-generative-ai-transport",
    );
  });
    ...
    void streamFn!(
      {
        id: "anthropic/claude-sonnet-4",
        name: "Claude Sonnet 4",
        api: "theclaw-openai-completions-transport",
        provider: "openrouter",
        baseUrl: "https://proxy.example.com/v1",
        reasoning: true,
        input: ["text"],
        ...
        contextWindow: 200000,
        maxTokens: 8192,
      } as Model<"theclaw-openai-completions-transport">,
      {
        systemPrompt: "system",
        messages: [],
        tools: [],
  ...
  it("uses top-level instructions for Codex responses without dropping parity fields", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        theclaw_session_id: "session-123",
        theclaw_turn_id: "turn-123",
      },
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        theclaw_session_id: "session-123",
        theclaw_turn_id: "turn-123",
        theclaw_turn_attempt: "1",
        theclaw_transport: "stream",
      },
