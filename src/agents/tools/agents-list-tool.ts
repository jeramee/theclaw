...
function resolveAgentRuntimeMetadata(
  ...
  const envRuntime = normalizeRuntimeValue(process.env.THECLAW_AGENT_RUNTIME);
  if (envRuntime) {
    return {
      id: envRuntime,
      source: "env",
...
export function createAgentsListTool(opts?: {
  ...
  return {
    ...
    name: "agents_list",
    description:
      'List TheClaw agent ids you can target with `sessions_spawn` when `runtime="subagent"` (based on subagent allowlists).',
