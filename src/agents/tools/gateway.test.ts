...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    theclaw: process.env.THECLAW_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.theclaw === undefined) {
      delete process.env.THECLAW_GATEWAY_TOKEN;
    } else {
      process.env.THECLAW_GATEWAY_TOKEN = envSnapshot.theclaw;
    }
  });
  ...
  it("uses THECLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.THECLAW_GATEWAY_TOKEN = "local-env-token";
    configState.value = {
      gateway: {
