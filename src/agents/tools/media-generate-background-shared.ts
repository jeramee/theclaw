...
function buildMediaGenerationReplyInstruction(params: {
  ...
  if (params.status === "ok") {
    return [
      ...
      `If you cannot use the message tool, reply in your normal assistant voice and include the exact MEDIA: lines from the result so TheClaw attaches the finished ${params.completionLabel}.`,
...
function isAsyncMediaDirectSendEnabled(config: TheClawConfig | undefined): boolean {
