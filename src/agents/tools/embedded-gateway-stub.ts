...
interface EmbeddedGatewayRuntime {
  resolveSessionAgentId: (opts: { sessionKey: string; config: TheClawConfig }) => string;
  ...
  resolveEffectiveChatHistoryMaxChars: (cfg: TheClawConfig) => number;
  ...
  loadCombinedSessionStoreForGateway: (cfg: TheClawConfig) => {
    storePath: string;
    store: unknown;
  };
