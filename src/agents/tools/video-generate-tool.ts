...
const VideoGenerateToolSchema = Type.Object({
  ...
  durationSeconds: Type.Optional(
    Type.Number({
      description:
        "Optional target duration in seconds. TheClaw may round this to the nearest provider-supported duration.",
      minimum: 1,
    }),
  ),
  audio: Type.Optional(
...
export function createVideoGenerateTool(options?: {
  ...
  const cfg: TheClawConfig = options?.config ?? getRuntimeConfig();
  ...
  return {
    ...
    description:
      "Generate videos using configured providers. Generated videos are saved under TheClaw-managed media storage and delivered automatically as attachments. Duration requests may be rounded to the nearest provider-supported value.",
