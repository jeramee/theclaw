...
import { isTheClawOwnerOnlyCoreToolName } from "./owner-only-tools.js";
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `theclaw security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as TheClawConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as TheClawConfig);
...
export function createGatewayTool(opts?: {
  ...
  return {
    label: "Gateway",
    name: "gateway",
    ownerOnly: isTheClawOwnerOnlyCoreToolName("gateway"),
    description:
