...
function createDefaultSpawnConfig(): TheClawConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function replaceSpawnConfig(next: TheClawConfig): void {
...
async function createCrossAgentWorkspaceFixture(options?: {
  ...
  const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-acp-spawn-"));
...
describe("spawnAcpDirect", () => {
  ...
  it("rejects TheClaw config agent ids when runtime=acp targets a native agent", async () => {
    replaceSpawnConfig({
    ...
    expect(result).toHaveProperty("error", expect.stringContaining("TheClaw config agent"));
    expect(hoisted.initializeSessionMock).not.toHaveBeenCalled();
  ...
  it("maps TheClaw ACP runtime agent aliases to their configured harness id", async () => {
    replaceSpawnConfig({
      ...createDefaultSpawnConfig(),
      agents: {
        list: [
