...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseTheClawManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyTheClawManifestInstallCommonFields<SkillInstallSpec>(
    {
...
export function resolveTheClawMetadata(
  frontmatter: ParsedSkillFrontmatter,
): TheClawSkillMetadata | undefined {
  const metadataObj = resolveTheClawManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveTheClawManifestRequires(metadataObj);
  const install = resolveTheClawManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveTheClawManifestOs(metadataObj);
  return {
