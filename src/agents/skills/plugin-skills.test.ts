    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "theclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "theclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "theclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("theclaw-");
  const acpxRoot = await tempDirs.make("theclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("theclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("theclaw-");
  const pluginRoot = await tempDirs.make("theclaw-plugin-");
  const outsideDir = await tempDirs.make("theclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("theclaw-");
    const pluginRoot = await tempDirs.make("theclaw-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("theclaw-");
    const pluginRoot = await tempDirs.make("theclaw-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
