...
describe("provider attribution", () => {
  it("resolves the canonical TheClaw product and runtime version", () => {
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in TheClaw runtime wrapper.",
      product: "TheClaw",
      version: "2026.3.22",
      headers: {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { THECLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "theclaw",
        version: "2026.3.22",
        "User-Agent": "theclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { THECLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "theclaw",
      version: "2026.3.22",
      "User-Agent": "theclaw/2026.3.22",
    });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { THECLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ THECLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
