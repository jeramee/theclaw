...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses THECLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      THECLAW_HOME: "/srv/theclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/theclaw-home"), ".theclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".theclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("theclaw-workspace-");
  ...
  it("migrates legacy onboardingCompletedAt markers to setupCompletedAt", async () => {
    const tempDir = await makeTempWorkspace("theclaw-workspace-");
    await fs.mkdir(path.join(tempDir, ".theclaw"), { recursive: true });
    await fs.writeFile(
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-workspace-hardlink-"));
    try {
