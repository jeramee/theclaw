...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "THECLAW_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { THECLAW_LIVE_MODEL_IMAGE_PROBE: "false" },
        "THECLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { THECLAW_LIVE_MODEL_IMAGE_PROBE: "1" },
        "THECLAW_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
