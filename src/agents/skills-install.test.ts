...
import { resolveTheClawMetadata, resolveSkillInvocationPolicy } from "./skills/frontmatter.js";
...
metadata: {"theclaw":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveTheClawMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("theclaw-skills-install-");
...
async function withWorkspaceCase(
  ...
  const envSnapshot = captureEnv(["THECLAW_STATE_DIR"]);
  try {
...
describe("installSkill code safety scanning", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("THECLAW_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an TheClaw-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["THECLAW_STATE_DIR", "THECLAW_CONFIG_PATH"]);
    try {
      process.env.THECLAW_STATE_DIR = "/tmp/untrusted-state";
      process.env.THECLAW_CONFIG_PATH = "/tmp/untrusted-config/theclaw.json";
