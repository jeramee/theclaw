...
function makeFallbacksOnlyCfg(): TheClawConfig {
  return {
    agents: {
      defaults: {
        model: {
...
function makeProviderFallbackCfg(provider: string): TheClawConfig {
  return makeCfg({
    agents: {
      defaults: {
        model: {
...
async function makeAuthTempDir(): Promise<string> {
  authTempRoot ||= path.join("/tmp", "theclaw-auth-suite-mock");
...
// https://github.com/theclaw/theclaw/issues/23440
...
describe("runWithModelFallback", () => {
  ...
  it("sanitizes model identifiers in model_not_found warnings", async () => {
    const warnLogs = createWarnLogCapture("theclaw-model-fallback-test");
    try {
      const cfg = makeCfg();
      const run = vi
        .fn()
