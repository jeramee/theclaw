...
const originalRuntime = process.env.THECLAW_AGENT_RUNTIME;
const originalHarnessFallback = process.env.THECLAW_AGENT_HARNESS_FALLBACK;

afterEach(() => {
  clearAgentHarnesses();
  piRunAttempt.mockClear();
  if (originalRuntime == null) {
    delete process.env.THECLAW_AGENT_RUNTIME;
  } else {
    process.env.THECLAW_AGENT_RUNTIME = originalRuntime;
  }
  if (originalHarnessFallback == null) {
    delete process.env.THECLAW_AGENT_HARNESS_FALLBACK;
  } else {
    process.env.THECLAW_AGENT_HARNESS_FALLBACK = originalHarnessFallback;
  }
});

function createAttemptParams(config?: TheClawConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
describe("runAgentHarnessAttemptWithFallback", () => {
  ...
  it("falls back to the PI harness for a forced plugin harness only when explicitly configured", async () => {
    process.env.THECLAW_AGENT_RUNTIME = "codex";
    process.env.THECLAW_AGENT_HARNESS_FALLBACK = "pi";
...
describe("selectAgentHarness", () => {
  ...
  it("keeps an existing session pinned to its plugin harness even when env now forces PI", () => {
    process.env.THECLAW_AGENT_RUNTIME = "pi";
    registerFailingCodexHarness();
