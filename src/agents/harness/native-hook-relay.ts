        ...
        hookSpecificOutput: {
          ...
          decision:
            decision === "allow"
              ...
              : {
                  behavior: "deny",
                  message: message?.trim() || "Denied by TheClaw",
                },
        },
      })}\n`,
      stderr: "",
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveTheClawCliExecutable();
  const argv =
    executable === "theclaw"
      ? ["theclaw"]
...
async function runNativeHookRelayPreToolUse(params: {
  ...
  // TheClaw plugin returns adjusted params here, we intentionally ignore them.
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `theclaw-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveTheClawCliExecutable(): string {
  ...
  throw new Error("Cannot resolve TheClaw CLI executable path for native hook relay");
}
