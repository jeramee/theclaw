...
function resolveAttachmentLimits(config: TheClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".theclaw", "attachments");
  const relDir = path.posix.join(".theclaw", "attachments", attachmentId);
