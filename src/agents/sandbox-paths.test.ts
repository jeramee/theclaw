...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-managed-media-"));
...
async function withOutsideHardlinkInTheClawTmp<T>(
  params: {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside theclaw tmp root",
      media: path.join(os.tmpdir(), "outside-theclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked TheClaw tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked TheClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInTheClawTmp(
      {
        openClawTmpDir,
  ...
  it("rejects symlinked TheClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInTheClawTmp(
