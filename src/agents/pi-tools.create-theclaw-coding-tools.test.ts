...
import "./test-helpers/fast-theclaw-tools.js";
import { createTheClawCodingTools } from "./pi-tools.js";
...
function createToolsForStoredSession(storeTemplate: string, sessionKey: string) {
  return createTheClawCodingTools({
    sessionKey,
    config: {
      session: {
        store: storeTemplate,
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createTheClawCodingTools>) {
...
describe("createTheClawCodingTools", () => {
  const testConfig: TheClawConfig = {};

  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createTheClawCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createTheClawCodingTools({ config: testConfig, senderIsOwner: true });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createTheClawCodingTools({
      config: testConfig,
    ...
    const codexTools = createTheClawCodingTools({
      config: testConfig,
    ...
    const disabledConfig: TheClawConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createTheClawCodingTools({
    ...
    const anthropicTools = createTheClawCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: TheClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createTheClawCodingTools({
    ...
    const denied = createTheClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createTheClawCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("provides top-level object schemas for all tools", () => {
    const tools = createTheClawCodingTools({ config: testConfig });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createTheClawCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("filters session tools for sub-agent sessions by default", () => {
    const tools = createTheClawCodingTools({
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createTheClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createTheClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createTheClawCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createTheClawCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(codingTools.some((tool) => tool.name === "message")).toBe(false);

    const cronTools = createTheClawCodingTools({
      config: { tools: { profile: "coding" } },
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createTheClawCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createTheClawCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createTheClawCodingTools({
      modelProvider: "xai",
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createTheClawCodingTools();
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-read-"));
    try {
      ...
      const contents = "Hello from theclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "theclaw-sandbox");
    const sandbox = createPiToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "theclaw-workspace"),
    ...
    const tools = createTheClawCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-canonical-"));
    try {
      const tools = createTheClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-legacy-alias-"));
    try {
      const tools = createTheClawCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-structured-write-"));
    try {
      const tools = createTheClawCodingTools({ workspaceDir: tmpDir });
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-structured-edit-"));
    try {
