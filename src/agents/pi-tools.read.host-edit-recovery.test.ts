...
describe("edit tool recovery hardening", () => {
  ...
  it("adds current file contents to exact-match mismatch errors", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-edit-recovery-"));
  ...
  it("recovers tilde paths against the OS home even when THECLAW_HOME differs", async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-edit-recovery-"));
    const osHome = path.join(tmpDir, "home");
    const theclawHome = path.join(tmpDir, "theclaw-home");
    await fs.mkdir(osHome, { recursive: true });
    await fs.mkdir(theclawHome, { recursive: true });
    ...
    process.env.THECLAW_HOME = theclawHome;
    ...
    try {
      ...
      await expect(fs.access(path.join(theclawHome, "demo.txt"))).rejects.toBeDefined();
    } finally {
