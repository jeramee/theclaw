...
function buildCoreToolGroupMap() {
  ...
  const theclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInTheClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:theclaw": theclawTools,
