...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("theclaw-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("theclaw-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("theclaw-tilde-test-mkdir-");
  ...
  it("ignores THECLAW_HOME for write operations", async () => {
    const theclawHome = await createTempDir("theclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("theclaw-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");
    vi.stubEnv("THECLAW_HOME", theclawHome);

    createHostWorkspaceWriteTool(theclawHome, { workspaceOnly: false });
    ...
    await expect(fs.access(path.join(theclawHome, path.basename(testFile)))).rejects.toBeDefined();
  });

  it("ignores THECLAW_HOME for mkdir operations", async () => {
    const theclawHome = await createTempDir("theclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("theclaw-tilde-test-mkdir-");
    ...
    expect((await fs.stat(newDir)).isDirectory()).toBe(true);
    await expect(fs.access(path.join(theclawHome, path.basename(newDir)))).rejects.toBeDefined();
  });

  it("ignores THECLAW_HOME for readFile operations", async () => {
    const theclawHome = await createTempDir("theclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("theclaw-tilde-test-edit-");
    ...
    createHostWorkspaceEditTool(theclawHome, { workspaceOnly: false });
    const content = await mocks.editOps!.readFile(toTildePath(testFile));
  ...
  it("ignores THECLAW_HOME for access operations", async () => {
    const theclawHome = await createTempDir("theclaw-home-override-", os.tmpdir());
    const dir = await createTempDir("theclaw-tilde-test-edit-");
