...
describe("resolveAgentConfig", () => {
  ...
  it("should return undefined when agent id does not exist", () => {
    const cfg: TheClawConfig = {
      agents: {
        list: [{ id: "main", workspace: "~/theclaw" }],
      },
    };
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: TheClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: TheClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: TheClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: TheClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: TheClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgDisable: TheClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaultsWithoutAgentModel: TheClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: TheClawConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: TheClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: TheClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/theclaw");
  });

  it("uses THECLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "theclaw-home");
    vi.stubEnv("THECLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as TheClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".theclaw", "workspace"));
  });

  it("uses THECLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "theclaw-home");
    vi.stubEnv("THECLAW_HOME", home);
    // Clear state dir so it falls back to THECLAW_HOME
    vi.stubEnv("THECLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as TheClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".theclaw", "agents", "main", "agent"));
  });
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/theclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/theclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-agent-scope-"));
