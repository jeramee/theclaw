...
// `provider:"theclaw"` assistant entries written by the channel-delivery
...
const TRANSCRIPT_ONLY_THECLAW_MODELS = new Set<string>(["delivery-mirror", "gateway-injected"]);
...
function isTranscriptOnlyTheclawAssistant(message: AgentMessage): boolean {
  ...
  return (
    provider === "theclaw" &&
    typeof model === "string" &&
    TRANSCRIPT_ONLY_THECLAW_MODELS.has(model)
  );
}
...
export function normalizeAssistantReplayContent(messages: AgentMessage[]): AgentMessage[] {
  ...
  for (const message of messages) {
    ...
    if (isTranscriptOnlyTheclawAssistant(message)) {
