...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("theclaw-bundled-");
  const workspaceDir = await createTempDir("theclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: TheClawConfig) {
