...
describe("normalizeMessagesForLlmBoundary", () => {
  ...
  it("keeps runtime-context transcript entries out of the LLM boundary", () => {
    const input = [
      ...
      {
        role: "custom",
        customType: "theclaw.runtime-context",
    ...
    expect(output).not.toEqual(
      expect.arrayContaining([expect.objectContaining({ customType: "theclaw.runtime-context" })]),
    );
    ...
    expect(seenContext.messages).toEqual([
      ...
      },
      {
        ...
        content: [
          {
            type: "text",
            text: "[theclaw] missing tool result in session history; inserted synthetic error result for transcript repair.",
          },
        ],
        isError: true,
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    ...
    const legacy = buildAfterTurnRuntimeContext({
      attempt: {
        ...
        agentAccountId: "acct-1",
        authProfileId: "openai:p1",
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as TheClawConfig,
        skillsSnapshot: undefined,
        senderIsOwner: true,
        provider: "openai-codex",
        modelId: "gpt-5.4",
