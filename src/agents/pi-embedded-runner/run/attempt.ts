...
import { resolveTheClawReferencePaths } from "../../docs-path.js";
...
import { createTheClawCodingTools, resolveToolLoopDetectionConfig } from "../../pi-tools.js";
import {
...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    const agentDir = params.agentDir ?? resolveTheClawAgentDir();
    ...
    const toolsRaw =
      params.disableTools || isRawModelRun
        ? []
        : (() => {
            const allTools = createTheClawCodingTools({
              agentId: sessionAgentId,
    ...
    const openClawReferences = await resolveTheClawReferencePaths({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop TheClaw
      ...
      // exact TheClaw-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal TheClaw agent/tool prompt when `session.prompt()` starts.
      ...
      try {
        ...
        // double-compaction. See: https://github.com/theclaw/theclaw/issues/9282
        ...
        if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
          try {
            sessionManager.appendCustomEntry("theclaw:prompt-error", {
              timestamp: Date.now(),
              runId: params.runId,
              sessionId: params.sessionId,
              provider: params.provider,
      ...
      // See: https://github.com/theclaw/theclaw/issues/8643
