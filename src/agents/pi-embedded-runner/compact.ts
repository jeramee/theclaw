...
export async function compactEmbeddedPiSessionDirect(
  ...
  await ensureTheClawModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createTheClawCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createTheClawCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact TheClaw-managed registrations so custom tools survive startup.
