...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__theclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__theclaw__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__theclaw__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; TheClaw handles all routing internally.",
...
function buildDocsSection(params: {
  ...
  const lines = [
    "## Documentation",
    docsPath ? `TheClaw docs: ${docsPath}` : "TheClaw docs: https://docs.theclaw.ai",
    "Mirror: https://docs.theclaw.ai",
    sourcePath ? `Local source: ${sourcePath}` : undefined,
    "Source: https://github.com/theclaw/theclaw",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.ai",
    docsPath
      ? "For TheClaw behavior, commands, config, or architecture: consult local docs first."
      : "For TheClaw behavior, commands, config, or architecture: consult the docs mirror first.",
    ...
    sourcePath
      ? "If docs are incomplete or stale, inspect the local TheClaw source code before answering."
      : "If docs are incomplete or stale, review the TheClaw source on GitHub before answering.",
    "When diagnosing issues, run `theclaw status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running TheClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List TheClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List TheClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside TheClaw.";
  }

  const lines = [
    "You are a personal assistant running inside TheClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control TheClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    ...safetySection,
    "## TheClaw CLI Quick Reference",
    "TheClaw is controlled via subcommands. Do not invent commands.",
    ...
    "Do not chain `theclaw gateway stop` and `theclaw gateway start` as a restart substitute.",
    "If unsure, ask the user to run `theclaw help` (or `theclaw gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## TheClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, TheClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by TheClaw and included below in Project Context.",
    "",
