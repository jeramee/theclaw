...
function isLikelyTheClawCliProcess(argv: string[] = process.argv): boolean {
  const entryBasename = normalizeLowercaseStringOrEmpty(path.basename(argv[1] ?? ""));
  return (
    entryBasename === "theclaw" ||
    entryBasename === "theclaw.mjs" ||
...
export function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real TheClaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureTheClawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyTheClawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): TheClawConfig | undefined {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  CONTEXT_WINDOW_RUNTIME_STATE.loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureTheClawModelsJson(cfg);
    } catch {
