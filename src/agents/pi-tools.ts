...
import { createTheClawTools } from "./theclaw-tools.js";
...
function resolveExecConfig(params: { cfg?: TheClawConfig; agentId?: string }) {
...
export function createTheClawCodingTools(options?: {
  agentId?: string;
  ...
  const base = (createCodingTools(workspaceRoot) as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === "read") {
      ...
      const wrapped = createTheClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
