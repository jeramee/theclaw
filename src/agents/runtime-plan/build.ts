...
function asTheClawConfig(value: unknown): TheClawConfig | undefined {
  return value !== null && typeof value === "object" && !Array.isArray(value)
    ? (value as TheClawConfig)
    : undefined;
}
...
export function buildAgentRuntimeDeliveryPlan(
  ...
  const config = asTheClawConfig(params.config);
  return {
    ...
    prompt: {
      ...
      resolveSystemPromptContribution(context) {
        return resolveProviderSystemPromptContribution({
          ...
          context: {
            ...context,
            config: asTheClawConfig(context.config),
          },
        });
      },
    },
