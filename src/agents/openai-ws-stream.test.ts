...
describe("createOpenAIWebSocketStreamFn", () => {
  ...
  it("keeps native turn metadata stable across websocket retries and increments attempt", async () => {
    ...
    expect(firstPayload.metadata?.theclaw_session_id).toBe("sess-turn-metadata-retry");
    expect(firstPayload.metadata?.theclaw_transport).toBe("websocket");
    expect(firstPayload.metadata?.theclaw_turn_id).toBeTruthy();
    expect(secondPayload.metadata?.theclaw_turn_id).toBe(firstPayload.metadata?.theclaw_turn_id);
    expect(firstPayload.metadata?.theclaw_turn_attempt).toBe("1");
    expect(secondPayload.metadata?.theclaw_turn_attempt).toBe("2");
  });
  ...
  it("does not attach native OpenAI session headers or metadata for custom responses endpoints", async () => {
    ...
    expect(payload.metadata?.theclaw_session_id).toBeUndefined();
    expect(payload.metadata?.theclaw_transport).toBeUndefined();
  ...
  it("pools cleanly released sessions behind the explicit pool flag", async () => {
    ...
    try {
      releaseWsSession("registry-test", {
        allowPool: true,
        env: {
          THECLAW_OPENAI_WS_POOL: "1",
          THECLAW_OPENAI_WS_SESSION_POOL_IDLE_MS: "1000",
