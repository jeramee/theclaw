...
async function writeExecApprovalsConfig(config: Record<string, unknown>) {
  const approvalsPath = path.join(process.env.HOME ?? "", ".theclaw", "exec-approvals.json");
...
describe("exec approvals", () => {
  ...
  beforeAll(async () => {
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-exec-approvals-"));
  });

  beforeEach(async () => {
    previousHome = process.env.HOME;
    previousUserProfile = process.env.USERPROFILE;
    previousBundledPluginsDir = process.env.THECLAW_BUNDLED_PLUGINS_DIR;
    previousDisableBundledPlugins = process.env.THECLAW_DISABLE_BUNDLED_PLUGINS;
  ...
  afterEach(() => {
    ...
    if (previousDisableBundledPlugins === undefined) {
      ...
      process.env.THECLAW_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
    }
  });
  ...
  it("skips approval when node allowlist is satisfied", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-test-bin-"));
  ...
  it("runs a skill wrapper chain without prompting when the wrapper is allowlisted", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-skill-wrapper-"));
    try {
      const skillDir = path.join(tempDir, ".theclaw", "skills", "gog");
