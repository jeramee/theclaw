...
function resolveAnthropicOpusThinking(cfg: TheClawConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: TheClawConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function createProviderWithModelsConfig(provider: string, models: Array<Record<string, unknown>>) {
  ...
  } as Partial<TheClawConfig>;
}

function resolveConfiguredRefForTest(cfg: Partial<TheClawConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as TheClawConfig,
...
describe("model-selection", () => {
  ...
  describe("isCliProvider", () => {
    ...
    it("returns true for setup-registered cli backends", () => {
      expect(isCliProvider("claude-cli", {} as TheClawConfig)).toBe(true);
    });

    it("returns false for provider ids", () => {
      expect(isCliProvider("example-cli", {} as TheClawConfig)).toBe(false);
    });
  });
  ...
  describe("buildModelAliasIndex", () => {
    it("should build alias index from config", () => {
      const cfg: Partial<TheClawConfig> = {
        agents: {
          defaults: {
            models: {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("sanitizes control characters in providerless-model warnings", async () => {
      const warnLogs = createWarnLogCapture("theclaw-model-selection-test");
      try {
        const cfg: Partial<TheClawConfig> = {
          agents: {
            defaults: {
