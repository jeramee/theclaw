...
let configOverride: TheClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: TheClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/theclaw/theclaw/issues/31298
    previousFastTestEnv = process.env.THECLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.THECLAW_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    resetAnnounceQueuesForTests();
    // THECLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("TheClaw runtime context (internal):");
