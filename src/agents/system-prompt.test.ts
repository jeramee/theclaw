...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes a CLI quick reference section", () => {
    ...
    expect(prompt).toContain("## TheClaw CLI Quick Reference");
    ...
    expect(prompt).toContain("theclaw gateway restart");
    expect(prompt).toContain("Do not chain `theclaw gateway stop`");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    const prompt = buildAgentSystemPrompt({
      ...
      runtimeInfo: {
        channel: "webchat",
        canvasRootDir: "/Users/example/.theclaw-dev/canvas",
      },
    });
    ...
    expect(prompt).toContain(
      '[embed url="/__theclaw__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
      "Never use local filesystem paths or `file://...` URLs in `[embed ...]`.",
    );
    expect(prompt).toContain(
      "The active hosted embed root for this session is: `/Users/example/.theclaw-dev/canvas`.",
    );
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- agents_list: List TheClaw agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("TheClaw docs: /tmp/theclaw/docs");
    expect(prompt).toContain(
  ...
  it("includes docs guidance when docsPath is provided", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/theclaw",
      docsPath: "/tmp/theclaw/docs",
      sourcePath: "/tmp/theclaw",
    });

    expect(prompt).toContain("## Documentation");
    expect(prompt).toContain("TheClaw docs: /tmp/theclaw/docs");
    expect(prompt).toContain("Local source: /tmp/theclaw");
    expect(prompt).toContain(
      "For TheClaw behavior, commands, config, or architecture: consult local docs first.",
    );
    expect(prompt).toContain(
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("TheClaw docs: https://docs.theclaw.ai");
    expect(prompt).toContain("Source: https://github.com/theclaw/theclaw");
    expect(prompt).toContain(
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## TheClaw Self-Update");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/theclaw. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`theclaw ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for TheClaw subagents");
