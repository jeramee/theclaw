...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: TheClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<TheClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<TheClawConfig["agents"]>["list"]>[number]["tools"];
  }): TheClawConfig {
    return {
      tools: params.tools,
      agents: {
