...
function createClaudeCliOverrideConfig(config: CliBackendConfig): TheClawConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from TheClaw's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__theclaw__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__theclaw__*");
  ...
  it("keeps Claude permission mode unset when TheClaw exec policy is not YOLO", () => {
    const resolved = resolveCliBackendConfig("claude-cli", {
