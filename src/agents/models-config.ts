...
import { planTheClawModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: TheClawConfig): {
  config: TheClawConfig;
  sourceConfigForSecrets: TheClawConfig;
} {
...
export async function ensureTheClawModelsJson(
  config?: TheClawConfig,
  ...
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveTheClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planTheClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
