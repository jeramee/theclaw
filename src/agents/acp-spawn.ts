...
function resolveHeartbeatConfigForAgent(params: {
  cfg: TheClawConfig;
  agentId: string;
}): NonNullable<NonNullable<TheClawConfig["agents"]>["defaults"]>["heartbeat"] {
  ...
  if (requested) {
    ...
    if (configuredAgent && !isExplicitlyAllowedAcpAgent(params.cfg, requested)) {
      return {
        ok: false,
        error:
          `agentId "${requested}" is an TheClaw config agent, not an ACP harness. ` +
          'Use runtime="subagent" or omit runtime for TheClaw config agents. ' +
...
function isExplicitlyAllowedAcpAgent(cfg: TheClawConfig, agentId: string): boolean {
