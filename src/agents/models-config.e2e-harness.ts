...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousTheClawAgentDir: string | undefined;
  ...
  beforeEach(() => {
    ...
    previousTheClawAgentDir = process.env.THECLAW_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousTheClawAgentDir === undefined) {
      delete process.env.THECLAW_AGENT_DIR;
    } else {
      process.env.THECLAW_AGENT_DIR = previousTheClawAgentDir;
    }
...
const COPILOT_DISCOVERY_ENV_VARS = [
  ...
  "THECLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS",
];
...
export async function withUnsetCopilotTokenEnv<T>(fn: () => Promise<T>): Promise<T> {
  return withTempEnv(COPILOT_DISCOVERY_ENV_VARS, async () => {
    ...
    process.env.THECLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = "github-copilot";
    return fn();
  });
}
...
export function snapshotImplicitProviderEnv(env?: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  snapshot.THECLAW_BUNDLED_PLUGINS_DIR ??=
...
export async function resolveImplicitProvidersForTest(
  ...
  if (inferredPluginIds.length > 0) {
    env.THECLAW_TEST_ONLY_PROVIDER_PLUGIN_IDS = inferredPluginIds.join(",");
  }
...
export const CUSTOM_PROXY_MODELS_CONFIG: TheClawConfig = {
  models: {
    providers: {
      "custom-proxy": {
