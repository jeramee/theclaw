...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    envSnapshot = captureEnv(["THECLAW_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("theclaw-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("theclaw-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("theclaw-cli-bundle-mcp-bundled-");
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = emptyBundledDir;
...
export function createEnabledBundleProbeConfig(): TheClawConfig {
  return {
    plugins: {
      entries: {
