...
function resolveCliImagePath(image: ImageContent): string {
  ...
  const digest = crypto
    ...
    .update(image.data)
    .digest("hex");
  return path.join(resolvePreferredTheClawTmpDir(), "theclaw-cli-images", `${digest}${ext}`);
}
...
function resolveCliImageRoot(params: { backend: CliBackendConfig; workspaceDir: string }): string {
  if (params.backend.imagePathScope === "workspace") {
    return path.join(params.workspaceDir, ".theclaw-cli-images");
  }
  return path.join(resolvePreferredTheClawTmpDir(), "theclaw-cli-images");
}
...
export async function writeCliSystemPromptFile(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-cli-system-prompt-"),
  );
