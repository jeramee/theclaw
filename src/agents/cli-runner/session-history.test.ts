...
describe("loadCliSessionHistoryMessages", () => {
  ...
  it("reads the canonical session transcript instead of an arbitrary external path", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-cli-state-"));
    const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-cli-outside-"));
  ...
  it("honors custom session store roots when resolving hook history transcripts", () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-cli-state-"));
    const customStoreDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-cli-store-"));
    vi.stubEnv("THECLAW_STATE_DIR", stateDir);
...
describe("buildCliSessionHistoryPrompt", () => {
  it("renders TheClaw transcript history around the next user message", () => {
  ...
  it("caps rendered reseed history before adding the next user message", () => {
    ...
    expect(prompt).toContain("[TheClaw reseed history truncated]");
