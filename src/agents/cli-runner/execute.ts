...
const CLI_BACKEND_PRESERVE_ENV = "THECLAW_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.THECLAW_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.THECLAW_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.THECLAW_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.THECLAW_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.THECLAW_MCP_MESSAGE_CHANNEL || "<empty>"}`,
    `senderIsOwner=${childEnv.THECLAW_MCP_SENDER_IS_OWNER || "<empty>"}`,
  ].join(" ");
}
    ...
    return await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (
            env.THECLAW_MCP_TOKEN ||
            env.THECLAW_MCP_SESSION_KEY ||
            env.THECLAW_MCP_SENDER_IS_OWNER
          ) {
