...
const prepareDeps = {
  ...
  createMcpLoopbackServerConfig,
  resolveTheClawReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveTheClawReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveTheClawReferencePaths(params),
};
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ...
          THECLAW_MCP_AGENT_ID: sessionAgentId ?? "",
          THECLAW_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          THECLAW_MCP_SESSION_KEY: params.sessionKey ?? "",
          THECLAW_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
        }
      : undefined,
    warn: (message) => cliBackendLog.warn(message),
  });
  ...
  const loadTheClawHistoryMessages = () => {
  ...
  const openClawReferences = await prepareDeps.resolveTheClawReferencePaths({
    workspaceDir,
  ...
  try {
    const hookResult = await resolvePromptBuildHookResult({
      ...
      messages: loadTheClawHistoryMessages(),
      hookCtx: {
        runId: params.runId,
        agentId: sessionAgentId,
        sessionKey: params.sessionKey,
