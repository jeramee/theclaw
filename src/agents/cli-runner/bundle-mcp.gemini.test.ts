...
describe("prepareCliBundleMcpConfig gemini", () => {
  it("writes Gemini system settings for bundle MCP servers", async () => {
    const prepared = await prepareCliBundleMcpConfig({
      ...
      workspaceDir: "/tmp/theclaw-bundle-mcp-gemini",
    ...
    expect(prepared.env?.THECLAW_MCP_TOKEN).toBe("loopback-token-123");
    ...
    expect(raw.mcp?.allowed).toEqual(["theclaw"]);
    expect(raw.mcpServers?.theclaw?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(raw.mcpServers?.theclaw?.headers?.Authorization).toBe("Bearer loopback-token-123");
