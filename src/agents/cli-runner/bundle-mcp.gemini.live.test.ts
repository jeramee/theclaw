...
const LIVE = isLiveTestEnabled(["THECLAW_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "theclaw-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("theclaw_live_probe", "TheClaw Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an TheClaw-configured streamable-http server", async () => {
    const geminiCommand = process.env.THECLAW_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("theclawLiveProbe");
