...
const THECLAW_CLAUDE_PLUGIN_NAME = "theclaw-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredTheClawTmpDir(), "theclaw-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, THECLAW_CLAUDE_PLUGIN_NAME);
  ...
  const manifest = {
    name: THECLAW_CLAUDE_PLUGIN_NAME,
    version: "0.0.0",
    description: "Session-scoped TheClaw skills selected for this agent run.",
    skills: "./skills",
  };
  await fs.writeFile(
