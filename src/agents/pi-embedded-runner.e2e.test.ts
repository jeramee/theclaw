...
const ensureTheClawModelsJsonMock = vi.fn(async () => ({ wrote: false }));
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureTheClawModelsJson: (...args: Parameters<typeof ensureTheClawModelsJsonMock>) =>
        ensureTheClawModelsJsonMock(...args),
    };
  });
};
...
beforeAll(async () => {
  ...
  e2eWorkspace = await createEmbeddedPiRunnerTestWorkspace("theclaw-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureTheClawModelsJsonMock.mockReset();
  ensureTheClawModelsJsonMock.mockResolvedValue({ wrote: false });
