import { listTheClawPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
import {
...
const THECLAW_ATTRIBUTION_PRODUCT = "TheClaw";
const THECLAW_ATTRIBUTION_ORIGINATOR = "theclaw";
...
function formatTheClawUserAgent(version: string): string {
  return `${THECLAW_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listTheClawPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: THECLAW_ATTRIBUTION_PRODUCT,
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: THECLAW_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatTheClawUserAgent(identity.version),
    },
  };
}
