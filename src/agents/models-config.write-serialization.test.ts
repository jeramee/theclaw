...
import { resolveTheClawAgentDir } from "./agent-paths.js";
import {
...
const planTheClawModelsJsonMock = vi.fn();
...
beforeAll(async () => {
  vi.doMock("./models-config.plan.js", () => ({
    planTheClawModelsJson: (...args: unknown[]) => planTheClawModelsJsonMock(...args),
  }));
  ({ ensureTheClawModelsJson } = await import("./models-config.js"));
...
describe("models-config write serialization", () => {
  it("does not reuse default workspace plugin metadata for explicit agent dirs without workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureTheClawModelsJson({}, agentDir);

      expect(planTheClawModelsJsonMock).toHaveBeenCalledWith(
  ...
  it("reuses current plugin metadata for explicit agent dirs with matching workspace", async () => {
    await withModelsTempHome(async (home) => {
      ...
      await ensureTheClawModelsJson({}, agentDir, { workspaceDir });
  ...
  it("does not reuse scoped startup discovery cache for a different provider scope", async () => {
    await withModelsTempHome(async (home) => {
      planTheClawModelsJsonMock.mockImplementation(async () => ({ action: "skip" }));
      ...
      expect(planTheClawModelsJsonMock).toHaveBeenCalledTimes(2);
      expect(planTheClawModelsJsonMock).toHaveBeenLastCalledWith(
        expect.objectContaining({
          providerDiscoveryProviderIds: ["anthropic"],
          providerDiscoveryTimeoutMs: 5000,
        }),
  ...
  it("keeps the ready cache warm after models.json is written", async () => {
    await withModelsTempHome(async () => {
      await ensureTheClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);
      await ensureTheClawModelsJson(CUSTOM_PROXY_MODELS_CONFIG);

      expect(planTheClawModelsJsonMock).toHaveBeenCalledTimes(1);
    });
  });
  ...
  it("invalidates the ready cache when models.json changes externally", async () => {
    await withModelsTempHome(async () => {
      ...
      const modelPath = path.join(resolveTheClawAgentDir(), "models.json");
  ...
  it("keeps distinct config fingerprints cached without evicting each other", async () => {
    await withModelsTempHome(async () => {
      planTheClawModelsJsonMock.mockImplementation(async () => ({ action: "noop" }));
      ...
      await ensureTheClawModelsJson(first);
      await ensureTheClawModelsJson(second);
      await ensureTheClawModelsJson(first);
  ...
  it("serializes concurrent models.json writes to avoid overlap", async () => {
    await withModelsTempHome(async () => {
      ...
      try {
        await Promise.all([ensureTheClawModelsJson(first), ensureTheClawModelsJson(second)]);
      } finally {
