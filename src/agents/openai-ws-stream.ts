...
function resolveWsSessionPoolConfig(env: NodeJS.ProcessEnv = process.env): {
  ...
  const enabled =
    env.THECLAW_OPENAI_WS_POOL === "1" || env.THECLAW_OPENAI_WS_SESSION_POOL === "1";
  const rawIdleMs = Number(env.THECLAW_OPENAI_WS_SESSION_POOL_IDLE_MS);
...
function resolveNativeOpenAITransportTurnState(params: {
  ...
  return {
    ...
    metadata: {
      ...
      theclaw_transport: params.transport,
    },
  };
}
