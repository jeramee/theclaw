...
let ensureTheClawModelsJsonMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ensureTheClawModelsJsonMock = vi.fn().mockResolvedValue({ agentDir: "/tmp", wrote: false });
    vi.doMock("./models-config.js", () => ({
      ensureTheClawModelsJson: ensureTheClawModelsJsonMock,
    }));
    vi.doMock("./agent-paths.js", () => ({
      resolveTheClawAgentDir: () => "/tmp/theclaw",
    }));
  ...
  beforeEach(() => {
    resetModelCatalogCacheForTest();
    ensureTheClawModelsJsonMock.mockClear();
  });
  ...
  it("returns partial results on discovery errors", async () => {
    ...
    try {
      ...
      const result = await loadModelCatalog({ config: {} as TheClawConfig });
  ...
  it("does not prepare models.json when loading catalog in read-only mode", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as TheClawConfig, readOnly: true });

    expect(result).toEqual([{ id: "gpt-4.1", name: "GPT-4.1", provider: "openai" }]);
    expect(ensureTheClawModelsJsonMock).not.toHaveBeenCalled();
    expect(discoverAuthStorage).toHaveBeenCalledWith("/tmp/theclaw", { readOnly: true });
  });
