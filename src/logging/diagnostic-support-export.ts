...
const SUPPORT_EXPORT_PREFIX = "theclaw-diagnostics-";
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$THECLAW_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/theclaw-sanitized.jsonl`: sanitized log summaries and metadata",
