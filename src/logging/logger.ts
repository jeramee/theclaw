...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredTheClawTmpDir() : POSIX_THECLAW_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "theclaw.log")
    : `${POSIX_THECLAW_TMP_DIR}/theclaw.log`;
}
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: TheClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.THECLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[theclaw] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
