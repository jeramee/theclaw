...
describe("diagnostic support export", () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-support-export-"));
  ...
  it("writes a shareable zip without raw chats, webhook bodies, or secrets", async () => {
    ...
    const logTail: LogTailPayload = {
      file: path.join(tempDir, "logs", "theclaw.log"),
      cursor: 200,
      size: 200,
      truncated: false,
      reset: false,
    ...
    const result = await writeDiagnosticSupportExport({
      ...
      readStatusSnapshot: async () => ({
        service: {
          loaded: true,
          command: {
            programArguments: ["theclaw", "gateway", "run", "--token", fakeToken],
            environment: {
              HOME: tempDir,
              THECLAW_GATEWAY_TOKEN: fakeToken,
            },
          },
        },
        gateway: {
    ...
    expect(combined).toContain("$THECLAW_STATE_DIR");
    ...
    const sanitizedLogs = entries["logs/theclaw-sanitized.jsonl"];
    ...
    expect(status.data?.service?.command?.environment?.THECLAW_GATEWAY_TOKEN).toBe("<redacted>");
  ...
  it("redacts Windows USERPROFILE paths when HOME is unset", () => {
    ...
    const stateDir = `${userProfile}\\AppData\\Roaming\\theclaw`;
    const redaction = {
      env: {
    ...
    expect(redactSupportString(`${stateDir}\\logs\\gateway.log`, redaction)).toBe(
      "$THECLAW_STATE_DIR\\logs\\gateway.log",
    );
    expect(
    ...
    const status = sanitizeSupportSnapshotValue(
      {
        service: {
          command: {
            programArguments: [
              "node",
              `${userProfile}\\theclaw\\dist\\index.js`,
              "--config",
              `${stateDir}\\theclaw.json`,
            ],
            sourcePath: "c:\\users\\support-user\\AppData\\Local\\theclaw\\gateway-service.json",
          },
        },
      },
      redaction,
    ...
    expect(serialized).toContain("~\\\\theclaw\\\\dist\\\\index.js");
    expect(serialized).toContain("$THECLAW_STATE_DIR\\\\theclaw.json");
    expect(serialized).toContain("~\\\\AppData\\\\Local\\\\theclaw\\\\gateway-service.json");
  });
  ...
  it("keeps writing when log tail collection fails", async () => {
    ...
    await writeDiagnosticSupportExport({
      ...
      readLogTail: async () => {
        throw new Error(`log tail failed at ${tempDir}/theclaw.log with token ${fakeToken}`);
      },
    });

    const entries = await readZipTextEntries(outputPath);
    expect(Object.keys(entries).toSorted()).toContain("logs/theclaw-sanitized.jsonl");
