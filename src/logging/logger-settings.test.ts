...
let originalTheClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.THECLAW_TEST_FILE_LOG;
  originalTheClawLogLevel = process.env.THECLAW_LOG_LEVEL;
  delete process.env.THECLAW_TEST_FILE_LOG;
  delete process.env.THECLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.THECLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalTheClawLogLevel === undefined) {
    delete process.env.THECLAW_LOG_LEVEL;
  } else {
    process.env.THECLAW_LOG_LEVEL = originalTheClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.THECLAW_TEST_FILE_LOG = "1";
    readLoggingConfigMock.mockReturnValue({
      level: "debug",
      file: "/tmp/theclaw-configured.log",
      maxFileBytes: 2048,
    });
