import type { TheClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "theclaw-bundled": {
    ...
    canOverride: ["theclaw-bundled"],
    canBeOverriddenBy: ["theclaw-managed", "theclaw-plugin"],
  },
  "theclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["theclaw-bundled", "theclaw-plugin"],
    canBeOverriddenBy: ["theclaw-managed"],
  },
  "theclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["theclaw-bundled", "theclaw-managed", "theclaw-plugin"],
    canBeOverriddenBy: ["theclaw-managed"],
  },
  "theclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["theclaw-workspace"],
    canBeOverriddenBy: ["theclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "theclaw-plugin") {
    return { enabled: true };
  }
