...
`;
    ...
    expect(parsed.theclaw.emoji).toBe("💾");
    expect(parsed.theclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.theclaw.emoji).toBe("📝");
    expect(parsed.theclaw.events).toEqual(["command"]);
    expect(parsed.theclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.theclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"theclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"theclaw": {"events": ["test"]}}');
  });
...
describe("resolveTheClawMetadata", () => {
  it("extracts theclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveTheClawMetadata(frontmatter);
  ...
  it("returns undefined when theclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        theclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with TheClaw" },
            { id: "npm", kind: "npm", package: "@theclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@theclaw/hook");
  });
    ...
    "theclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with TheClaw" }],
      },
  }
---
...
`;
    ...
    const theclaw = resolveTheClawMetadata(frontmatter);
    expect(theclaw).toBeDefined();
    expect(theclaw?.emoji).toBe("💾");
    expect(theclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(theclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(theclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const theclaw = resolveTheClawMetadata(frontmatter);
    expect(theclaw?.emoji).toBe("disk");
    expect(theclaw?.events).toEqual(["command:new"]);
  });
});
