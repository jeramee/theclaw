...
This directory contains hooks that ship with TheClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.theclaw/workspace`)
...
homepage: https://docs.theclaw.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.theclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.theclaw/theclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'theclaw.*gateway' && pnpm theclaw gateway`
3. Enable the hook: `theclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.theclaw.ai/automation/hooks
