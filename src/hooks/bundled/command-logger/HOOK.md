...
homepage: https://docs.theclaw.ai/automation/hooks#command-logger
metadata:
  {
    "theclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.theclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.theclaw/logs/commands.log ~/.theclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/theclaw`:
   ```
   /home/username/.theclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
