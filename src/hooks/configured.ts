...
export function hasConfiguredInternalHooks(config: TheClawConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: TheClawConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
