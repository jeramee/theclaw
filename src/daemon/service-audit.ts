...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds THECLAW_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `theclaw gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service THECLAW_GATEWAY_TOKEN does not match gateway.auth.token in theclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.THECLAW_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.THECLAW_GATEWAY_TOKEN);
}
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `theclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
