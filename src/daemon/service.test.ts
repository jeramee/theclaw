...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer TheClaw", async () => {
    const tempHome = await makeTempWorkspace("theclaw-service-future-config-");
    const stateDir = path.join(tempHome, ".theclaw");
    const configPath = path.join(stateDir, "theclaw.json");
    const envSnapshot = captureEnv(["HOME", "THECLAW_STATE_DIR", "THECLAW_CONFIG_PATH"]);
    try {
  ...
  it("tracks installed, loaded, and running separately", async () => {
    const service = createService({
      ...
      readCommand: vi.fn(async () => ({
        ...
        environment: { THECLAW_GATEWAY_PORT: "18789" },
      })),
      readRuntime: vi.fn(async () => ({ status: "running" })),
    });

    const state = await readGatewayServiceState(service, {
      env: { THECLAW_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.THECLAW_GATEWAY_PORT).toBe("18789");
  });
});
