...
function readServiceProxyEnvironment(
  ...
  const proxyUrl = normalizeOptionalString(env.THECLAW_PROXY_URL);
  return proxyUrl ? { THECLAW_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.THECLAW_PROFILE;
  const wrapperPath = normalizeOptionalString(env.THECLAW_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = `${resolveGatewaySystemdServiceName(profile)}.service`;
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    THECLAW_PROFILE: profile,
    THECLAW_WRAPPER: wrapperPath,
    THECLAW_GATEWAY_PORT: String(port),
    THECLAW_LAUNCHD_LABEL: resolvedLaunchdLabel,
    THECLAW_SYSTEMD_UNIT: systemdUnit,
    THECLAW_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    THECLAW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    THECLAW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    THECLAW_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.THECLAW_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.THECLAW_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    THECLAW_GATEWAY_TOKEN: gatewayToken,
    THECLAW_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    THECLAW_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    THECLAW_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    THECLAW_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    THECLAW_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    THECLAW_LOG_PREFIX: "node",
    THECLAW_SERVICE_MARKER: NODE_SERVICE_MARKER,
    THECLAW_SERVICE_KIND: NODE_SERVICE_KIND,
    THECLAW_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    THECLAW_STATE_DIR: sharedEnv.stateDir,
    THECLAW_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.THECLAW_STATE_DIR;
  const configPath = env.THECLAW_CONFIG_PATH;
