...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.THECLAW_GATEWAY_PORT).toBe("18789");
    expect(env.THECLAW_GATEWAY_TOKEN).toBeUndefined();
    expect(env.THECLAW_SERVICE_MARKER).toBe("theclaw");
    expect(env.THECLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.THECLAW_SERVICE_VERSION).toBe("string");
    expect(env.THECLAW_SYSTEMD_UNIT).toBe("theclaw-gateway.service");
    expect(env.THECLAW_WINDOWS_TASK_NAME).toBe("TheClaw Gateway");
    if (process.platform === "darwin") {
      expect(env.THECLAW_LAUNCHD_LABEL).toBe("ai.theclaw.gateway");
    }
  });

  it("passes through THECLAW_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        THECLAW_WRAPPER: " /usr/local/bin/theclaw-doppler ",
      },
      port: 18789,
    });

    expect(env.THECLAW_WRAPPER).toBe("/usr/local/bin/theclaw-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".theclaw", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", THECLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.THECLAW_SYSTEMD_UNIT).toBe("theclaw-gateway-work.service");
    expect(env.THECLAW_WINDOWS_TASK_NAME).toBe("TheClaw Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.THECLAW_LAUNCHD_LABEL).toBe("ai.theclaw.work");
    }
  });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    ...
    expect(env.THECLAW_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("passes through THECLAW_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", THECLAW_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.THECLAW_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through THECLAW_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", THECLAW_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.THECLAW_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits THECLAW_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", THECLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", THECLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".theclaw"));
  });

  it("uses THECLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", THECLAW_STATE_DIR: "/var/lib/theclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/theclaw"));
  });

  it("expands ~ in THECLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", THECLAW_STATE_DIR: "~/theclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/theclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { THECLAW_STATE_DIR: "C:\\State\\theclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\theclaw");
  });
});
