...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when THECLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".theclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when THECLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", THECLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".theclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers THECLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        THECLAW_PROFILE: "rescue",
        THECLAW_STATE_DIR: "C:\\State\\theclaw",
      },
      expected: path.join("C:\\State\\theclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", THECLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".theclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem TheClaw Gateway",
          "cd /d C:\\Projects\\theclaw",
          "set NODE_ENV=production",
          "set THECLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\theclaw",
          environment: {
            NODE_ENV: "production",
            THECLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\theclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\TheClaw Share\\node.exe" "\\\\fileserver\\TheClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\TheClaw Share\\node.exe",
            "\\\\fileserver\\TheClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from THECLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ THECLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
