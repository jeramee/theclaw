...
export const THECLAW_WRAPPER_ENV_KEY = "THECLAW_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/theclaw -> .pnpm/theclaw@X.Y.Z/...
...
export async function resolveTheClawWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${THECLAW_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveTheClawWrapperPath(params.wrapperPath);
  if (wrapperPath) {
