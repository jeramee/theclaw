...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `theclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      THECLAW_LAUNCHD_LABEL: `ai.theclaw.launchd-int-${testId}`,
      THECLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
