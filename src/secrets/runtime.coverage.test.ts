...
type SecretRegistryEntry = {
  id: string;
  configFile: "theclaw.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.THECLAW_DEBUG_RUNTIME_COVERAGE === "1";
const COVERAGE_LOADABLE_PLUGIN_ORIGINS =
  buildCoverageLoadablePluginOrigins(COVERAGE_REGISTRY_ENTRIES);
const PLUGIN_OWNED_THECLAW_COVERAGE_EXCLUSIONS = new Set([
...
function applyConfigForTheClawTarget(
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every theclaw.json registry target when configured as active", async () => {
    const entries = COVERAGE_REGISTRY_ENTRIES.filter(
      (entry) =>
        entry.configFile === "theclaw.json" &&
        !PLUGIN_OWNED_THECLAW_COVERAGE_EXCLUSIONS.has(entry.id),
    );
    for (const batch of buildCoverageBatches(entries)) {
      logCoverageBatch("theclaw.json", batch);
      const config = {} as TheClawConfig;
      const env: Record<string, string> = {};
      for (const [index, entry] of batch.entries()) {
        const envId = `THECLAW_SECRET_TARGET_${entry.id}`;
        ...
        applyConfigForTheClawTarget(config, entry, envId, wildcardToken);
      }
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const batch of buildCoverageBatches(entries)) {
      ...
      for (const [index, entry] of batch.entries()) {
        const envId = `THECLAW_AUTH_SECRET_TARGET_${entry.id}`;
      ...
      const snapshot = await prepareAuthCoverageSnapshot({
        ...
        env,
        agentDirs: ["/tmp/theclaw-agent-main"],
