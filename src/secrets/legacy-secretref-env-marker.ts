...
function toCandidate(
  ...
  defaults: NonNullable<TheClawConfig["secrets"]>["defaults"] | undefined,
...
export function migrateLegacySecretRefEnvMarkers(config: TheClawConfig): {
  config: TheClawConfig;
  changes: string[];
} {
  ...
  const next = structuredClone(config) as TheClawConfig & Record<string, unknown>;
