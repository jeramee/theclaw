...
let compiledCoreTheClawTargetState: {
  knownTargetIds: Set<string>;
  openClawCompiledSecretTargets: CompiledTargetRegistryEntry[];
  openClawTargetsById: Map<string, CompiledTargetRegistryEntry[]>;
  targetsByType: Map<string, CompiledTargetRegistryEntry[]>;
...
const compiledBundledChannelTheClawTargets = new Map<
  string,
...
function getCompiledSecretTargetRegistryState() {
  ...
  const openClawCompiledSecretTargets = compiledSecretTargetRegistry.filter(
    (entry) => entry.configFile === "theclaw.json",
  );
...
function getCompiledCoreTheClawTargetState() {
  if (compiledCoreTheClawTargetState) {
    return compiledCoreTheClawTargetState;
  }
  const openClawCompiledSecretTargets = getCoreSecretTargetRegistry()
    .filter((entry) => entry.configFile === "theclaw.json")
    .map(compileTargetRegistryEntry);
  compiledCoreTheClawTargetState = {
...
function getCompiledBundledChannelTheClawTargets(
  channelId: string,
): CompiledTargetRegistryEntry[] | null {
  ...
  if (compiledBundledChannelTheClawTargets.has(normalizedChannelId)) {
    return compiledBundledChannelTheClawTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadBundledChannelSecretContractApi(normalizedChannelId)
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "theclaw.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledBundledChannelTheClawTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreTheClawTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  for (const entry of getCompiledCoreTheClawTargetState().openClawCompiledSecretTargets) {
    if (!entry.includeInPlan) {
      continue;
    }
  ...
  const explicitBundledChannelEntries = explicitBundledChannelId
    ? getCompiledBundledChannelTheClawTargets(explicitBundledChannelId)
    : null;
...
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreTheClawTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreTheClawTargetState()
