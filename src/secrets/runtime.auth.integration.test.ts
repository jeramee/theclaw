...
describe("secrets runtime snapshot auth integration", () => {
  ...
  it("recomputes config-derived agent dirs when refreshing active secrets runtime snapshots", async () => {
    await withTempHome("theclaw-secrets-runtime-agent-dirs-", async (home) => {
      const mainAgentDir = path.join(home, ".theclaw", "agents", "main", "agent");
      const opsAgentDir = path.join(home, ".theclaw", "agents", "ops", "agent");
