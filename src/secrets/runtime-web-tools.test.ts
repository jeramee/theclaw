...
function setConfiguredFetchProviderKey(configTarget: TheClawConfig, value: unknown): void {
  const plugins = ensureRecord(configTarget as Record<string, unknown>, "plugins");
  const entries = ensureRecord(plugins, "entries");
...
async function runRuntimeWebTools(params: { config: TheClawConfig; env?: NodeJS.ProcessEnv }) {
...
function readProviderKey(config: TheClawConfig, provider: ProviderUnderTest): unknown {
