...
describe("buildPairingReply", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["THECLAW_CONTAINER_HINT", "THECLAW_PROFILE"]);
    delete process.env.THECLAW_CONTAINER_HINT;
  ...
  function expectPairingApproveCommand(text: string, testCase: (typeof pairingReplyCases)[number]) {
    const commandRe = new RegExp(
      `(?:theclaw|theclaw) --profile isolated pairing approve ${testCase.channel} ${testCase.code}`,
    );
