...
export type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<TheClawConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<TheClawConfig["tools"]>["web"]>["search"]>;
...
function rawKeyValue(config: TheClawConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: TheClawConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: TheClawConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: TheClawConfig, result: TheClawConfig): TheClawConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: TheClawConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as TheClawConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  const choice = await prompter.select({
    ...
    options: [
      ...options,
      {
        ...
        label: "Skip for now",
        hint: "Configure later with theclaw configure --section web",
      },
    ],
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "TheClaw will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.theclaw.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — TheClaw will store a reference instead of the API key.",
