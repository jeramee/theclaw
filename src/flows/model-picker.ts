...
export type PromptDefaultModelResult = { model?: string; config?: TheClawConfig };
...
function resolveConfiguredModelRaw(cfg: TheClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: TheClawConfig): string[] {
...
function loadPickerModelCatalog(cfg: TheClawConfig): ReturnType<typeof loadModelCatalog> {
