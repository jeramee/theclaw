...
type AgentDefaults = NonNullable<TheClawConfig["agents"]>["defaults"];
...
type RunPreparedReplyParams = {
  ...
  sessionCfg: TheClawConfig["session"];
...
export async function runPreparedReply(
  ...
  const useFastReplyRuntime = shouldUseReplyFastTestRuntime({
    cfg,
    isFastTestEnv: process.env.THECLAW_TEST_FAST === "1",
  });
  ...
  const transcriptBodyBase = isHeartbeat
    ...
    : isBareSessionReset
      ? softResetTail || `[TheClaw session ${startupAction}]`
      : hasUserBody
        ? baseBodyFinal
