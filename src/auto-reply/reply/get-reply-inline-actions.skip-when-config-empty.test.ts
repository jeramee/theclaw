...
const { buildStatusReplyMock, createTheClawToolsMock, getChannelPluginMock, handleCommandsMock } =
  vi.hoisted(() => ({
    buildStatusReplyMock: vi.fn(),
    createTheClawToolsMock: vi.fn(),
...
vi.mock("../../agents/theclaw-tools.runtime.js", () => ({
  createTheClawTools: (...args: unknown[]) => createTheClawToolsMock(...args),
}));
...
describe("handleInlineActions", () => {
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(createTheClawToolsMock).toHaveBeenCalledWith(
      expect.objectContaining({
