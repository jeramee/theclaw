...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop theclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@theclaw_bot", { botUsername: "theclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@theclaw_bot", { botUsername: "theclaw_bot" })).toBe(true);
