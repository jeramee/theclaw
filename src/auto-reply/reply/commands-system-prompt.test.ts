...
const { createTheClawCodingToolsMock } = vi.hoisted(() => ({
  createTheClawCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/pi-tools.js", () => ({
  createTheClawCodingTools: createTheClawCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createTheClawCodingToolsMock.mockClear();
    createTheClawCodingToolsMock.mockReturnValue([]);
  });

  it("opts command tool builds into gateway subagent binding", async () => {
    await resolveCommandsSystemPromptBundle(makeParams());

    expect(vi.mocked(createTheClawCodingTools)).toHaveBeenCalledWith(
