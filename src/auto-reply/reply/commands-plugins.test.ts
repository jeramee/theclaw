...
vi.mock("../../plugins/toggle-config.js", () => ({
  setPluginEnabledInConfig: vi.fn((config: TheClawConfig, id: string, enabled: boolean) => ({
    ...config,
    plugins: {
      ...config.plugins,
      entries: {
...
function buildCfg(): TheClawConfig {
  return {
...
function buildPluginsParams(commandBodyNormalized: string, cfg: TheClawConfig) {
