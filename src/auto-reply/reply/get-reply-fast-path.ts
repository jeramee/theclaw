...
const COMPLETE_REPLY_CONFIG_SYMBOL = Symbol.for("theclaw.reply.complete-config");
const FULL_REPLY_RUNTIME_SYMBOL = Symbol.for("theclaw.reply.full-runtime");

type ReplyConfigWithMarker = TheClawConfig & {
...
function isSlowReplyTestAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  return (
    env.THECLAW_ALLOW_SLOW_REPLY_TESTS === "1" || env.THECLAW_STRICT_FAST_REPLY_CONFIG === "0"
  );
}
...
export function markCompleteReplyConfig<T extends TheClawConfig>(
  config: T,
...
export function withFastReplyConfig<T extends TheClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "fast" });
}

export function withFullRuntimeReplyConfig<T extends TheClawConfig>(config: T): T {
  return markCompleteReplyConfig(config, { runtimeMode: "full" });
}

export function isCompleteReplyConfig(config: unknown): config is TheClawConfig {
  return Boolean(
    config &&
...
export function resolveGetReplyConfig(params: {
  getRuntimeConfig: () => TheClawConfig;
  ...
  if (params.isFastTestEnv && !isCompleteReplyConfig(configOverride) && !isSlowReplyTestAllowed()) {
    throw new Error(
      "Fast reply tests must pass with withFastReplyConfig()/markCompleteReplyConfig(); set THECLAW_ALLOW_SLOW_REPLY_TESTS=1 to opt out.",
    );
  }
  ...
  return applyMergePatch(params.getRuntimeConfig(), configOverride) as TheClawConfig;
}
