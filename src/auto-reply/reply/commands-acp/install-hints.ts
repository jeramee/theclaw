...
export function resolveAcpInstallCommandHint(cfg: TheClawConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `theclaw plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^theclaw plugins install /u, "");
    ...
    return "theclaw plugins install acpx";
  }
