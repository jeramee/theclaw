...
type HomeEnvSnapshot = {
  ...
  THECLAW_STATE_DIR: string | undefined;
  THECLAW_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    THECLAW_AGENT_DIR: process.env.THECLAW_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".theclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.THECLAW_STATE_DIR = path.join(home, ".theclaw");
    process.env.THECLAW_AGENT_DIR = path.join(home, ".theclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".theclaw", "agent");
