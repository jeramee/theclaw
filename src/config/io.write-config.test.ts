...
import type { ConfigFileSnapshot } from "./types.theclaw.js";
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "theclaw-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { THECLAW_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("migrates shipped plugin install config records into the plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".theclaw", "theclaw.json");
      const pluginDir = path.join(home, ".theclaw", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "theclaw.plugin.json");
      ...
      try {
        ...
        await expect(
          readPersistedInstalledPluginIndex({
            stateDir: path.join(home, ".theclaw"),
          }),
  ...
  it("migrates shipped plugin install config records even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".theclaw", "theclaw.json");
      const pluginDir = path.join(home, ".theclaw", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".theclaw", "theclaw.json");
      const unwritableStatePath = path.join(home, ".theclaw");
  ...
  it("keeps writes inside an THECLAW_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".theclaw", "theclaw.json");
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "theclaw.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "theclaw.json"), "utf-8"),
      ) as {
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          THECLAW_TEST_CONFIG_OVERWRITE_LOG: "1",
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            $schema: "https://theclaw.ai/config.json",
            gateway: { mode: "local" },
          },
          null,
          2,
      ...
      expect(persisted.$schema).toBe("https://theclaw.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/theclaw-main" }] },
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".theclaw", "theclaw.json");
      const includePath = path.join(home, ".theclaw", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://theclaw.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/theclaw-test-required-plugin",
          source: "/tmp/theclaw-test-required-plugin/index.ts",
          manifestPath: "/tmp/theclaw-test-required-plugin/theclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        if (previousConfigPath === undefined) {
          ...
          process.env.THECLAW_CONFIG_PATH = previousConfigPath;
        }
      }
    });
  });
  ...
  it("notifies in-process reloaders with resolved source config when persisted env refs are restored", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".theclaw", "theclaw.json");
      const previousConfigPath = process.env.THECLAW_CONFIG_PATH;
      const previousGatewayToken = process.env.THECLAW_GATEWAY_TOKEN;
      process.env.THECLAW_CONFIG_PATH = configPath;
      process.env.THECLAW_GATEWAY_TOKEN = "gateway-token-runtime";
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
          {
            gateway: {
              mode: "local",
              auth: { mode: "token", token: "${THECLAW_GATEWAY_TOKEN}" },
            },
      ...
      try {
        ...
        if (previousGatewayToken === undefined) {
          ...
          process.env.THECLAW_GATEWAY_TOKEN = previousGatewayToken;
        }
      }
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/theclaw-test-demo",
          source: "/tmp/theclaw-test-demo/index.ts",
          manifestPath: "/tmp/theclaw-test-demo/theclaw.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      ...
      try {
        ...
        const postWriteSnapshot = await createConfigIO({
          env: { THECLAW_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
          homedir: () => home,
          logger: silentLogger,
