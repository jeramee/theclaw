...
describe("runtime snapshot state", () => {
  ...
  it("tracks snapshot metadata and cache keys across runtime refreshes", () => {
    const firstConfig: TheClawConfig = { gateway: { port: 18789 } };
    const secondConfig: TheClawConfig = { gateway: { port: 19001 } };
  ...
  it("selects runtime config only when input still matches the runtime source", () => {
    ...
    const scopedResolvedConfig: TheClawConfig = {
      ...runtimeConfig,
      tools: {
        experimental: {
          planTool: true,
  ...
  it("refreshes both snapshots from disk after a write when source + runtime snapshots exist", async () => {
    ...
    const loadFreshConfig = vi.fn<() => TheClawConfig>(() => ({
      gateway: { auth: { mode: "token" } },
    }));
    const nextSourceConfig: TheClawConfig = {
      gateway: { auth: { mode: "token" } },
      models: {
        providers: {
          openai: {
  ...
  it("notifies registered write listeners with committed runtime snapshots", () => {
    const seen: Array<{ configPath: string; runtimeConfig: TheClawConfig }> = [];
