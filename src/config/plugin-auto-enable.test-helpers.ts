...
export function makeTempDir(): string {
  return makeTrackedTempDir("theclaw-plugin-auto-enable", tempDirs);
}
...
export function makeIsolatedEnv(overrides: NodeJS.ProcessEnv = {}): NodeJS.ProcessEnv {
  ...
  return {
    ...
    THECLAW_BUNDLED_PLUGINS_DIR: path.join(process.cwd(), "extensions"),
    VITEST: "true",
    ...overrides,
  };
}
...
export function writePluginManifestFixture(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "theclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function makeRegistry(
  ...
  return {
    plugins: plugins.map((plugin) => ({
      ...
      manifestPath: `/fake/${plugin.id}/theclaw.plugin.json`,
    })),
    diagnostics: [],
  };
}
