...
import type { ConfigFileSnapshot, TheClawConfig } from "./types.js";
import { shouldWarnOnTouchedVersion } from "./version.js";

export const ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS_ENV =
  "THECLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS";
...
type FutureConfigGuardParams = {
  ...
  config?: Pick<TheClawConfig, "meta"> | null;
  currentVersion?: string;
...
export function resolveFutureConfigActionBlock(
  ...
  return {
    ...
    currentVersion,
    touchedVersion,
    message: `Refusing to ${params.action} because this TheClaw binary (${currentVersion}) is older than the config last written by TheClaw ${touchedVersion}.`,
    hints: [
      "Run the newer theclaw binary on PATH, or reinstall the intended gateway service from the newer install.",
