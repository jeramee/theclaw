...
import type { TheClawConfig, ConfigValidationIssue } from "./types.js";
...
const SECRETREF_POLICY_DOC_URL = "https://docs.theclaw.ai/reference/secretref-credential-surface";
...
function formatLegacySecretRefEnvMarkerMessage(candidate: {
  ...
  return [
    `${JSON.stringify(candidate.value)} is a legacy SecretRef marker and is not valid theclaw.json config.`,
    `Use a structured SecretRef object instead, for example ${replacement}.`,
    'Run "theclaw doctor --fix" to migrate valid secretref-env:<ENV_VAR> markers.',
    `See ${SECRETREF_POLICY_DOC_URL}.`,
  ].join(" ");
}
...
function collectLegacySecretRefEnvMarkerIssues(raw: unknown): ConfigValidationIssue[] {
  ...
  return collectLegacySecretRefEnvMarkerCandidates(raw as TheClawConfig).map((candidate) => ({
    path: candidate.path,
...
function validateIdentityAvatar(config: TheClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: TheClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
...
): { ok: true; config: TheClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TheClawSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = validated.data as TheClawConfig;
...
function validateConfigObjectWithPluginsBase(
  ...
  let registryInfo: RegistryInfo | null = opts.pluginMetadataSnapshot
    ...
    : null;
  let compatConfig: TheClawConfig | null | undefined;
  ...
  const ensureCompatConfig = (): TheClawConfig => {
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run theclaw doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native TheClaw config schema.
