...
import type { TheClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function coerceConfig(value: unknown): TheClawConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as TheClawConfig;
}
...
function stampConfigVersion(cfg: TheClawConfig): TheClawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: TheClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer TheClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as TheClawConfig, env);
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: TheClawConfig): TheClawConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run theclaw plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function loadConfig(): TheClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(effectiveConfigRaw as TheClawConfig, {
        env: deps.env,
        homedir: deps.homedir,
      });
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: TheClawConfig = {};
    ...
    try {
      ...
      const loadValidationPluginMetadataSnapshot = (config: TheClawConfig) => {
  ...
  async function readBestEffortConfig(): Promise<TheClawConfig> {
    const result = await readConfigFileSnapshotInternal();
  ...
  async function readSourceConfigBestEffort(): Promise<TheClawConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFile(
  ...
  ): Promise<{ persistedHash: string; persistedConfig: TheClawConfig }> {
    clearConfigCache();
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as TheClawConfig, unsetPaths);
    ...
    let cfgToWrite = persistCandidate as TheClawConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as TheClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.THECLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.THECLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
...
// module scope. `THECLAW_CONFIG_PATH` (and friends) are expected to work even
...
export function projectConfigOntoRuntimeSourceSnapshot(config: TheClawConfig): TheClawConfig {
...
export function loadConfig(): TheClawConfig {
  ...
  // reparsing theclaw.json on hot code paths.
  return loadPinnedRuntimeConfig(() => createConfigIO().loadConfig());
}

export function getRuntimeConfig(): TheClawConfig {
  return loadConfig();
}

export async function readBestEffortConfig(): Promise<TheClawConfig> {
  return await createConfigIO().readBestEffortConfig();
}

export async function readSourceConfigBestEffort(): Promise<TheClawConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: TheClawConfig = nextCfg;
  try {
