...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("theclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "theclaw-sandbox-browser-global" },
      agentBrowser: { network: "theclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("theclaw-sandbox-browser-agent");
  });
