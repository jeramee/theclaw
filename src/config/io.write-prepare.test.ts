...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "theclaw-web-search": {
                source: "npm",
                spec: "@ollama/theclaw-web-search",
                installPath: "/tmp/theclaw-web-search",
                resolvedName: "@ollama/theclaw-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "theclaw-web-search": {
                source: "npm",
                spec: "@ollama/theclaw-web-search@0.2.2",
                installPath: "/tmp/theclaw-web-search",
                resolvedName: "@ollama/theclaw-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("theclaw config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('theclaw config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/theclaw.json",
        expectedConfigPath: "/tmp/theclaw.json",
  ...
  it("keeps plugin AJV defaults out of the persisted candidate", () => {
    ...
    const nextConfig: TheClawConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("preserves normalized nested channel enabled keys during unrelated writes", () => {
    ...
    const nextConfig: TheClawConfig = {
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://theclaw.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
