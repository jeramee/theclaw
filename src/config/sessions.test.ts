...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ THECLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-theclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-theclaw#general");
  });
  ...
  it("derives session transcripts dir from THECLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { THECLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when THECLAW_STATE_DIR differs from stored paths", () => {
