...
function logConfigDocBaselineDebug(message: string): void {
  if (process.env.THECLAW_CONFIG_DOC_BASELINE_DEBUG === "1") {
    console.error(`[config-doc-baseline] ${message}`);
  }
}

function resolveRepoRoot(): string {
  const fromPackage = resolveTheClawPackageRootSync({
...
async function loadBundledConfigSchemaResponse(): Promise<ConfigSchemaResponse> {
  ...
  const env = {
    ...process.env,
    HOME: os.tmpdir(),
    THECLAW_STATE_DIR: path.join(os.tmpdir(), "theclaw-config-doc-baseline-state"),
    THECLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
  };
