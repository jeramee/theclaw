...
function collectModelRefs(cfg: TheClawConfig): string[] {
  const refs: string[] = [];
...
function hasConfiguredEmbeddedHarnessRuntime(cfg: TheClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isProviderConfigured(cfg: TheClawConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: TheClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: TheClawConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: TheClawConfig, plugin: PluginManifestRecord): boolean {
...
function collectCandidateChannelIds(cfg: TheClawConfig, env: NodeJS.ProcessEnv): string[] {
...
function hasConfiguredWebSearchPluginEntry(cfg: TheClawConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: TheClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasConfiguredPluginConfigEntry(cfg: TheClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    !!entries &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: TheClawConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: TheClawConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: TheClawConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: TheClawConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: TheClawConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: TheClawConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: TheClawConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: TheClawConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: TheClawConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: TheClawConfig): boolean {
  return (
...
function hasPluginEntries(cfg: TheClawConfig): boolean {
  const entries = cfg.plugins?.entries;
  return !!entries && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: TheClawConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: TheClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: TheClawConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: TheClawConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: TheClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: TheClawConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: TheClawConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: TheClawConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
function registerPluginEntry(cfg: TheClawConfig, pluginId: string): TheClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
