...
export function parseTheClawVersion(raw: string | null | undefined): TheClawVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeTheClawVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseTheClawVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameTheClawStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseTheClawVersion(a);
  const parsedB = parseTheClawVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareTheClawVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseTheClawVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseTheClawVersion(current);
  const parsedTouched = parseTheClawVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameTheClawStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareTheClawVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: TheClawVersion): number {
