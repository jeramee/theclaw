...
function collectConfigEnvVarsByTarget(cfg?: TheClawConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: TheClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: TheClawConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: TheClawConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.THECLAW_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
