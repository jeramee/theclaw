...
function createSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/theclaw.json",
    exists: true,
    raw: "{}",
...
describe("config mutate helpers", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "theclaw-config-mutate-" });
  ...
  it("writes through a single-file top-level plugins include", async () => {
    ...
    const pluginsPath = path.join(home, ".theclaw", "config", "plugins.json5");
