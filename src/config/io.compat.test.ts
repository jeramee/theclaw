...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-config-"));
  try {
    await run(home);
  } finally {
...
describe("config io paths", () => {
  it("uses ~/.theclaw/theclaw.json when config exists", async () => {
    await withTempHome(async (home) => {
      const configPath = await writeConfig(home, ".theclaw", 19001);
      const io = createIoForHome(home);
      expect(io.configPath).toBe(configPath);
    });
  });

  it("defaults to ~/.theclaw/theclaw.json when config is missing", async () => {
    await withTempHome(async (home) => {
      const io = createIoForHome(home);
      expect(io.configPath).toBe(path.join(home, ".theclaw", "theclaw.json"));
    });
  });

  it("uses THECLAW_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { THECLAW_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".theclaw", "theclaw.json"));
    });
  });

  it("honors explicit THECLAW_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".theclaw", 20002, "custom.json");
      const io = createIoForHome(home, { THECLAW_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("moves WhatsApp shared access defaults into accounts.default during runtime compat", () => {
    ...
    const next = migrated.next as TheClawConfig | null;
