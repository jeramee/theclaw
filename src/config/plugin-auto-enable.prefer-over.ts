...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.theclaw) || !isRecord(entry.theclaw.channel)) {
      continue;
    }
    const channel = entry.theclaw.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: TheClawConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: TheClawConfig, pluginId: string) => boolean;
