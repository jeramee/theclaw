...
import { resetConfigRuntimeState, type TheClawConfig } from "./config.js";
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  try {
    return await withTempHomeBase(fn, {
      prefix: "theclaw-config-",
      env: {
        THECLAW_CONFIG_PATH: undefined,
        THECLAW_BUNDLED_PLUGINS_DIR: undefined,
        THECLAW_DISABLE_BUNDLED_PLUGINS: undefined,
        THECLAW_DISABLE_PLUGIN_DISCOVERY_CACHE: undefined,
        THECLAW_DISABLE_PLUGIN_MANIFEST_CACHE: undefined,
        THECLAW_PLUGIN_CATALOG_PATHS: undefined,
        THECLAW_MPM_CATALOG_PATHS: undefined,
        THECLAW_PLUGIN_DISCOVERY_CACHE_MS: undefined,
        THECLAW_PLUGIN_MANIFEST_CACHE_MS: undefined,
        THECLAW_LOAD_SHELL_ENV: undefined,
        THECLAW_DEFER_SHELL_ENV_FALLBACK: undefined,
        THECLAW_SHELL_ENV_TIMEOUT_MS: undefined,
...
export async function writeTheClawConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.THECLAW_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected THECLAW_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeTheClawConfig(home, config);
