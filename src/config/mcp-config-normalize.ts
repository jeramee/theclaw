...
export type TheClawMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_THECLAW_TRANSPORT: Record<string, TheClawMcpHttpTransport | "stdio"> = {
...
export function resolveTheClawMcpTransportAlias(
  value: unknown,
): TheClawMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_THECLAW_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_THECLAW_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveTheClawMcpTransportAlias(next.type);
