...
import { normalizeTheClawVersionBase } from "../config/version.js";
...
function resolveReportedPluginVersion(
  ...
  return (
    normalizeTheClawVersionBase(resolveCompatibilityHostVersion(env)) ??
    normalizeTheClawVersionBase(plugin.version) ??
    plugin.version
  );
}
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "theclaw";
