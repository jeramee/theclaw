...
const RETAINED_RUNTIME_DEPS_MANIFEST = ".theclaw-runtime-deps.json";
...
const PLUGIN_ROOT_INSTALL_STAGE_DIR = ".theclaw-install-stage";
const BUNDLED_RUNTIME_DEPS_LOCK_DIR = ".theclaw-runtime-deps.lock";
...
const MIRRORED_PACKAGE_RUNTIME_DEP_PLUGIN_ID = "theclaw-core";
...
function formatRuntimeDepsLockTimeoutMessage(params: {
  ...
  return (
    ...
    `ownerFilePath=${params.owner.ownerFilePath}). If no TheClaw/npm install is running, remove the lock directory and retry.`
  );
}
...
export function isWritableDirectory(dir: string): boolean {
  ...
  try {
    probeDir = fs.mkdtempSync(path.join(dir, ".theclaw-write-probe-"));
...
function resolveBundledRuntimeDepsExternalBaseDirs(env: NodeJS.ProcessEnv): string[] {
  const explicit = env.THECLAW_PLUGIN_STAGE_DIR?.trim();
  if (explicit) {
...
function resolveExternalBundledRuntimeDepsInstallRoots(params: {
  ...
  const packageKey = `theclaw-${version}-${createPathHash(packageRoot)}`;
...
function resolveExistingExternalBundledRuntimeDepsRoots(params: {
  ...
  for (const externalBaseDir of externalBaseDirs) {
    ...
    if (!packageKey || !packageKey.startsWith("theclaw-")) {
      continue;
    }
...
function replaceNodeModulesDir(targetDir: string, sourceDir: string): void {
  ...
  const tempDir = fs.mkdtempSync(path.join(parentDir, ".theclaw-runtime-deps-copy-"));
...
function linkNodeModulesDir(targetDir: string, sourceDir: string): boolean {
  const parentDir = path.dirname(targetDir);
  const tempLink = path.join(parentDir, `.theclaw-runtime-deps-link-${process.pid}-${Date.now()}`);
  try {
...
function readBundledPluginRuntimeDepsManifest(
  ...
  const manifest = readJsonObject(path.join(pluginDir, "theclaw.plugin.json"));
...
export function resolveBundledRuntimeDependencyPackageInstallRootPlan(
  ...
  if (
    ...
    env.THECLAW_PLUGIN_STAGE_DIR?.trim() ||
...
export function installBundledRuntimeDeps(params: {
  ...
  try {
    ...
    // Always make npm see an TheClaw-owned package root. The package-level
    ...
    const installEnv = createBundledRuntimeDepsInstallEnv(params.env, {
      cacheDir: path.join(installExecutionRoot, ".theclaw-npm-cache"),
    });
