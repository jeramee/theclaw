...
import { listTheClawPluginManifestMetadata } from "./manifest-metadata-scan.js";

const tempRoots: string[] = [];

function createTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-manifest-metadata-"));
  tempRoots.push(root);
  return root;
}
...
describe("listTheClawPluginManifestMetadata", () => {
  afterEach(() => {
  ...
  it("prefers the active bundled manifest over stale persisted bundled installs", () => {
    ...
    writeJson(path.join(bundledRoot, "openai", "theclaw.plugin.json"), {
      id: "openai",
      providerEndpoints: [{ endpointClass: "openai-public", hosts: ["api.openai.com"] }],
    });
    writeJson(path.join(staleBundledRoot, "openai", "theclaw.plugin.json"), {
      id: "openai",
      providers: ["openai"],
    });
    writeJson(path.join(home, ".theclaw", "plugins", "installs.json"), {
      plugins: [{ rootDir: path.join(staleBundledRoot, "openai"), origin: "bundled" }],
    });

    const records = listTheClawPluginManifestMetadata({
      THECLAW_HOME: home,
