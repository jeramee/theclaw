...
function shouldUseInstalledManifestRegistryCache(params: {
  ...
  if (params.env.THECLAW_DISABLE_INSTALLED_PLUGIN_MANIFEST_REGISTRY_CACHE?.trim()) {
    return false;
  }
  return !params.env.THECLAW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim();
}
...
function buildInstalledManifestRegistryCacheKey(params: {
  ...
  return hashJson({
    ...
    request: {
      ...
      env: {
        THECLAW_VERSION: params.env.THECLAW_VERSION,
