...
function buildScanFailureBlockReason(params: { error: string; targetLabel: string }) {
  return `${params.targetLabel} blocked: code safety scan failed (${params.error}). Run "theclaw security audit --deep" for details.`;
}
...
function isTrustedTheClawPeerSymlink(relativePath: string): boolean {
  const segments = relativePath.split(/[\\/]+/);
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "theclaw") ||
    (segments.length === 3 &&
...
async function resolveTrustedHostTheClawRootRealPath(): Promise<string | null> {
...
function isTrustedHostTheClawPath(params: {
  resolvedTargetPath: string;
  trustedHostTheClawRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostTheClawRootRealPath !== null &&
    isPathInside(params.trustedHostTheClawRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    // Workspace package managers can leave peer links back to the TheClaw host
    // package. Trust only the exact peer-link shapes and only when the resolved
    // target stays inside the host package root.
    if (
      isTrustedTheClawPeerSymlink(params.symlinkRelativePath) &&
      isTrustedHostTheClawPath({
        resolvedTargetPath,
        trustedHostTheClawRootRealPath: params.trustedHostTheClawRootRealPath,
      })
    ) {
      return {};
    }
...
async function collectPackageManifestPaths(
  ...
  const trustedHostTheClawRootRealPath = await resolveTrustedHostTheClawRootRealPath();
      ...
      if (entry.isSymbolicLink()) {
        ...
        if (pathContainsNodeModulesSegment(relativeNextPath)) {
          const symlinkTargetInspection = await inspectNodeModulesSymlinkTarget({
            ...
            trustedHostTheClawRootRealPath,
          });
...
export async function scanBundleInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Bundle "{target}" has {count} suspicious code pattern(s). Run "theclaw security audit --deep" for details.`,
...
export async function scanPackageInstallSourceRuntime(
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    suspiciousMessage: `Plugin "{target}" has {count} suspicious code pattern(s). Run "theclaw security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanFileInstallSourceRuntime(
  ...
  const builtinScan = await scanFileTarget({
    ...
    suspiciousMessage: `Plugin file "{target}" has {count} suspicious code pattern(s). Run "theclaw security audit --deep" for details.`,
    targetName: params.pluginId,
...
export async function scanSkillInstallSourceRuntime(params: {
  ...
  const builtinScan = await scanDirectoryTarget({
    ...
    path: params.sourceDir,
    suspiciousMessage:
      'Skill "{target}" has {count} suspicious code pattern(s). Run "theclaw security audit --deep" for details.',
