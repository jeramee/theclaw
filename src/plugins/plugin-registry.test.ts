...
function makeTempDir() {
  return makeTrackedTempDir("theclaw-plugin-registry", tempDirs);
}
...
function createCandidate(rootDir: string): PluginCandidate {
  ...
  fs.writeFileSync(
    path.join(rootDir, "theclaw.plugin.json"),
    JSON.stringify({
      id: "demo",
      name: "Demo",
...
function createIndex(
  ...
  return {
    ...
    plugins: [
      {
        pluginId,
        manifestPath: path.join(pluginRoot, "theclaw.plugin.json"),
...
describe("plugin registry facade", () => {
  ...
  it("resolves contribution owners from a plugin lookup table without rereading manifests", () => {
    ...
    fs.unlinkSync(path.join(rootDir, "theclaw.plugin.json"));
      ...
      plugins: [
        {
          ...
          manifestPath: path.join(rootDir, "theclaw.plugin.json"),
          source: path.join(rootDir, "index.ts"),
          rootDir,
        },
      ],
  ...
  it("reads the persisted registry before deriving from discovered candidates", async () => {
    ...
    fs.writeFileSync(
      path.join(persistedRootDir, "theclaw.plugin.json"),
        ...
        plugins: [
          {
            ...
            manifestPath: path.join(persistedRootDir, "theclaw.plugin.json"),
  ...
  it("falls back to the derived registry when persisted bundled roots point at another checkout", async () => {
    ...
    await writePersistedInstalledPluginIndex(
      createIndex("persisted", {
        plugins: [
          {
            ...createIndex("persisted").plugins[0],
            manifestPath: path.join(staleBundledRootDir, "theclaw.plugin.json"),
    ...
    const result = loadPluginRegistrySnapshotWithMetadata({
      stateDir,
      candidates: [candidate],
      env: hermeticEnv({ THECLAW_BUNDLED_PLUGINS_DIR: rootDir }),
    });
  ...
  it("caches config-scoped derived registries when the persisted registry is missing", () => {
    ...
    const env = hermeticEnv({ THECLAW_BUNDLED_PLUGINS_DIR: bundledRoot });
    ...
    const manifestReadsAfterFirst = readFileSyncSpy.mock.calls.filter((call) =>
      String(call[0]).endsWith("theclaw.plugin.json"),
    ).length;
