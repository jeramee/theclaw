...
export const PLUGIN_MANIFEST_FILENAME = "theclaw.plugin.json";
...
export type PluginManifestActivation = {
   ...
   * TheClaw moves away from implicit startup sidecar loading. Set true when
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `theclaw qa`, for example `matrix`. */
  commandName: string;
...
// package.json "theclaw" metadata (used for setup/catalog)
...
export type TheClawPackageStartup = {
  /**
...
export type TheClawPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type TheClawPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: TheClawPackageSetupFeatures;
  channel?: PluginPackageChannel;
  install?: PluginPackageInstall;
  startup?: TheClawPackageStartup;
};
...
export type PackageManifest = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, TheClawPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): TheClawPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
