...
import type {
  ...
  TheClawPluginApi,
  TheClawPluginChannelRegistration,
  TheClawPluginCliCommandDescriptor,
  TheClawPluginCliRegistrar,
  TheClawPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  TheClawPluginGatewayRuntimeScopeSurface,
  TheClawGatewayDiscoveryService,
  TheClawPluginHttpRouteParams,
  TheClawPluginHookOptions,
  TheClawPluginNodeHostCommand,
  TheClawPluginReloadRegistration,
  TheClawPluginSecurityAuditCollector,
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("theclaw.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<TheClawPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<TheClawPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<TheClawPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
    if (record.origin !== "bundled") {
      pushDiagnostic({
        level: "error",
  ...
  const registerTool = (
    ...
    const factory: TheClawPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: TheClawPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: TheClawPluginHookOptions | undefined,
    config: TheClawPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: TheClawPluginHttpRouteParams) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: TheClawPluginChannelRegistration | ChannelPlugin,
    mode: PluginRegistrationMode = "full",
  ) => {
    const registrationCapabilities = resolvePluginRegistrationCapabilities(mode);
    const normalized =
      typeof (registration as TheClawPluginChannelRegistration).plugin === "object"
        ? (registration as TheClawPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    opts?: { commands?: string[]; descriptors?: TheClawPluginCliCommandDescriptor[] },
  ) => {
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is TheClawPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: TheClawPluginReloadRegistration) => {
    const normalize = (values?: string[]) =>
      (values ?? []).map((value) => value.trim()).filter(Boolean);
    const normalized: TheClawPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: TheClawPluginNodeHostCommand,
  ) => {
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: TheClawPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: TheClawPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: TheClawGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: TheClawPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<TheClawPluginApi["registerCompactionProvider"]>[0],
              ) => {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as TheClawConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
