...
export const DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV = "THECLAW_DISABLE_PERSISTED_PLUGIN_REGISTRY";

function formatDeprecatedPersistedRegistryDisableWarning(): string {
  return `${DISABLE_PERSISTED_PLUGIN_REGISTRY_ENV} is a deprecated break-glass compatibility switch; use \`theclaw plugins registry --refresh\` or \`theclaw doctor --fix\` to repair registry state.`;
}
...
function resolveDerivedSnapshotCacheKey(
  ...
  return JSON.stringify({
    ...
    disableBundled: env.THECLAW_DISABLE_BUNDLED_PLUGINS ?? "",
    vitest: env.VITEST ?? "",
  });
}
    ...
    if (persistedReadsEnabled && persistedIndex) {
      if (
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry policy does not match current config; using derived plugin index. Run `theclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          message:
            "Persisted plugin registry points at missing plugin files; using derived plugin index. Run `theclaw plugins registry --refresh` to update the persisted registry.",
        });
        ...
        diagnostics.push({
          ...
          code: "persisted-registry-stale-source",
          message:
            "Persisted plugin registry points at a different bundled plugin tree; using derived plugin index. Run `theclaw plugins registry --refresh` to update the persisted registry.",
        });
      } else {
        return {
