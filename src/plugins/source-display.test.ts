...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "theclaw", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".theclaw", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".theclaw", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "theclaw-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".theclaw", "extensions"),
      },
    });
  });
});
