...
import type { TheClawPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("theclaw-manifest-registry", tempDirs);
}

function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "theclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  format?: "theclaw" | "bundle";
  bundleFormat?: "codex" | "claude" | "cursor";
  packageManifest?: TheClawPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "theclaw.plugin.json");
  const linkedManifest = path.join(rootDir, "theclaw.plugin.json");
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expect(registry.diagnostics).toContainEqual(
      expect.objectContaining({
        ...
        source: path.join(dir, "theclaw.plugin.json"),
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "theclaw.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { THECLAW_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires TheClaw >=2026.3.22, but this host is 2026.3.21",
      expectWarn: false,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | theclaw.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { THECLAW_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("does not reuse cached load-path manifests across env home changes", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        THECLAW_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      cache: true,
      ...
      env: hermeticEnv({
        HOME: homeB,
        THECLAW_HOME: undefined,
        THECLAW_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
