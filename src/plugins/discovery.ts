...
function resolveDiscoveryCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.THECLAW_PLUGIN_DISCOVERY_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseDiscoveryCache(env: NodeJS.ProcessEnv): boolean {
  const disabled = env.THECLAW_DISABLE_PLUGIN_DISCOVERY_CACHE?.trim();
  if (disabled) {
    return false;
  }
...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @theclaw/voice-call -> voice-call).
...
export function discoverTheClawPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ownershipUid?: number | null;
  cache?: boolean;
      ...
      tracePluginLifecyclePhase(
        ...
        () => {
          ...
          for (const extraPath of extra) {
            ...
            if (bundledAlias) {
              result.diagnostics.push({
                level: "warn",
                source: trimmed,
                message: `ignored plugins.load.paths entry that points at TheClaw's ${bundledAlias.kind} bundled plugin directory; remove this redundant path or run theclaw doctor --fix`,
              });
              continue;
            }
            discoverFromPath({
          ...
          if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
            // Keep workspace auto-discovery constrained to the TheClaw extensions root.
