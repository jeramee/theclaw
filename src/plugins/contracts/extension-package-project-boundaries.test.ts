...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/theclaw-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/theclaw-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-config.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-io.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-network.ts",
  "packages/memory-host-sdk/src/host/theclaw-runtime-session.ts",
] as const;
...
function collectTheClawRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./theclaw-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@theclaw/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@theclaw/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@theclaw/memory-host-sdk");
    ...
    expect(collectTheClawRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
