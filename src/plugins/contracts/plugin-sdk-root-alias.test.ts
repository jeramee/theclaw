...
describe("plugin-sdk root alias", () => {
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.alias).toMatchObject({
      "theclaw/plugin-sdk": rootAliasPath,
      "@theclaw/plugin-sdk": rootAliasPath,
      "theclaw/plugin-sdk/group-access": expect.stringContaining(
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "theclaw/plugin-sdk/alpha",
      "@theclaw/plugin-sdk/alpha",
      "theclaw/plugin-sdk/group-access",
      "@theclaw/plugin-sdk/group-access",
      "theclaw/plugin-sdk/zeta",
      "@theclaw/plugin-sdk/zeta",
      "theclaw/plugin-sdk",
      "@theclaw/plugin-sdk",
    ]);
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { THECLAW_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect(aliasMap["theclaw/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap["@theclaw/plugin-sdk/qa-lab"]).toBe(
      path.join(packageRoot, "src", "plugin-sdk", "qa-lab.ts"),
    );
    expect(aliasMap).not.toHaveProperty("theclaw/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("theclaw/plugin-sdk/nested/path");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    expect(lazyModule.createJitiOptions.at(-1)?.alias).toMatchObject({
      "theclaw/plugin-sdk/channel-runtime": path.join(
        packageRoot,
        "src",
        "plugin-sdk",
        "channel-runtime.mts",
  ...
  it("loads legacy root exports through the merged root wrapper", { timeout: 240_000 }, () => {
    ...
    expect(typeof rootSdk.resolvePreferredTheClawTmpDir).toBe("function");
