...
} from "theclaw/plugin-sdk/plugin-test-runtime";
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "theclaw/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "theclaw/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { resolveWorkspacePackagePublicModuleUrl } from "../../plugin-sdk/test-helpers/public-surface-loader.js";

type TtsRuntimeModule = typeof import("theclaw/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("theclaw/plugin-sdk/speech-core");
...
const speechCoreRuntimeApiModuleId = resolveWorkspacePackagePublicModuleUrl({
  packageName: "@theclaw/speech-core",
...
function asLegacyTtsConfig(value: unknown): TheClawConfig {
  return value as TheClawConfig;
}

function asLegacyTheClawConfig(value: Record<string, unknown>): TheClawConfig {
  return value as unknown as TheClawConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): TheClawConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("theclaw/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: TheClawConfig): ResolvedTtsConfig {
  const rawConfig =
    ...
    describe("getTtsProvider", () => {
      ...
      it("passes cfg into auto-selection so model-provider Google keys can configure TTS", () => {
        withIsolatedSpeechProviderEnv(
          ...
          () => {
            const cfg = asLegacyTheClawConfig({
              agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
              models: {
                providers: {
                  google: {
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: TheClawConfig = asLegacyTheClawConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.THECLAW_TTS_PREFS;
      process.env.THECLAW_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.THECLAW_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: TheClawConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
