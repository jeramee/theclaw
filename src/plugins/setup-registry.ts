...
function collectConfiguredPluginEntryIds(config: TheClawConfig): string[] {
...
function resolveRegister(mod: TheClawPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: TheClawPluginModule;
  try {
    mod = getJiti(setupSource)(setupSource) as TheClawPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: TheClawPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so TheClaw ignored the plugin setup runtime entry. Remove setup-api/theclaw.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
