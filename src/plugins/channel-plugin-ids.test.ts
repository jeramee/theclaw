...
function withManifestLoadPaths<T extends { id: string }>(plugin: T): T {
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/theclaw.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: TheClawConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: TheClawConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: TheClawConfig) => {
      if (Object.prototype.hasOwnProperty.call(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: {
        THECLAW_STATE_DIR: "/tmp/theclaw-with-persisted-demo-channel",
      } as NodeJS.ProcessEnv,
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("includes required agent harness owner plugins when env forces the runtime", () => {
    expectStartupPluginIdsCase({
      ...
      env: { THECLAW_AGENT_RUNTIME: "codex" },
      expected: ["demo-channel", "browser", "codex", "memory-core"],
    });
  });
