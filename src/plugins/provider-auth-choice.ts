...
function resolveConfiguredDefaultModelPrimary(cfg: TheClawConfig): string | undefined {
...
async function applyDefaultModelFromAuthChoice(params: {
  ...
  preserveExistingDefaultModel: boolean | undefined;
  prompter: WizardPrompter;
  runSelectedModelHook: (config: TheClawConfig) => Promise<void>;
...
export async function runProviderPluginAuthMethod(params: {
  ...
  opts?: Partial<ProviderAuthOptionBag>;
}): Promise<{ config: TheClawConfig; defaultModel?: string }> {
...
export async function applyAuthChoiceLoadedPluginProvider(
  ...
  const resolveScopedRuntimeProviders = (config: TheClawConfig): ProviderPlugin[] =>
    resolvePluginProviders({
      config,
      workspaceDir,
      env: params.env,
...
export async function applyAuthChoicePluginProvider(
  ...
  const agentDir =
    params.agentDir ??
    (agentId === defaultAgentId ? resolveTheClawAgentDir() : resolveAgentDir(nextConfig, agentId));
  const workspaceDir =
