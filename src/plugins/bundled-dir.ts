...
const DISABLED_BUNDLED_PLUGINS_DIR = path.join(os.tmpdir(), "theclaw-empty-bundled-plugins");
const TEST_TRUST_BUNDLED_PLUGINS_DIR_ENV = "THECLAW_TEST_TRUST_BUNDLED_PLUGINS_DIR";
...
export function areBundledPluginsDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  const raw = normalizeOptionalLowercaseString(env.THECLAW_DISABLE_BUNDLED_PLUGINS);
  ...
  try {
    return fs.readdirSync(pluginsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fs.existsSync(path.join(pluginDir, "theclaw.plugin.json"))
      );
    });
  } catch {
    return false;
...
function resolveTrustedExistingOverride(resolvedOverride: string): string | null {
  ...
  const modulePackageRoot = resolveTheClawPackageRootSync({ moduleUrl: import.meta.url });
...
export function resolveBundledPluginsDir(env: NodeJS.ProcessEnv = process.env): string | undefined {
  ...
  const override = env.THECLAW_BUNDLED_PLUGINS_DIR?.trim();
  ...
  try {
    const argvRoot = resolveTheClawPackageRootSync({ argv1: process.argv[1] });
    ...
    const moduleRoot = resolveTheClawPackageRootSync({ moduleUrl: import.meta.url });
