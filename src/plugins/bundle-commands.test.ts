...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".theclaw", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "THECLAW_HOME", "THECLAW_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("theclaw-bundle-commands-home-");
      const workspaceDir = await createTempDir("theclaw-bundle-commands-workspace-");
