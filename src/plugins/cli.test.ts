...
const mocks = vi.hoisted(() => ({
  ...
  loadTheClawPluginCliRegistry: vi.fn(),
  loadTheClawPlugins: vi.fn(),
...
vi.mock("./loader.js", () => ({
  loadTheClawPluginCliRegistry: (...args: unknown[]) =>
    mocks.loadTheClawPluginCliRegistry(...args),
  loadTheClawPlugins: (...args: unknown[]) => mocks.loadTheClawPlugins(...args),
}));
...
function expectAutoEnabledCliLoad(params: {
  rawConfig: TheClawConfig;
  autoEnabledConfig: TheClawConfig;
  ...
  expect(mocks.loadTheClawPlugins).toHaveBeenCalledWith(
...
describe("registerPluginCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadTheClawPluginCliRegistry.mockReset();
    mocks.loadTheClawPluginCliRegistry.mockResolvedValue(createCliRegistry());
    mocks.loadTheClawPlugins.mockReset();
    mocks.loadTheClawPlugins.mockReturnValue({
      ...createCliRegistry(),
      diagnostics: [],
    });
    ...
    mocks.loadConfig.mockReturnValue({} as TheClawConfig);
  ...
  it("skips plugin CLI registrars when commands already exist", async () => {
    ...
    await registerPluginCliCommands(program, {} as TheClawConfig);
  ...
  it("forwards an explicit env to plugin loading", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as TheClawConfig, env);
  ...
  it("injects gateway-backed node runtime into plugin CLI commands", async () => {
    await registerPluginCliCommands(createProgram(), {} as TheClawConfig);
  ...
  it("loads root-help descriptors through the dedicated non-activating CLI collector", async () => {
    ...
    mocks.loadTheClawPluginCliRegistry.mockResolvedValue({
      cliRegistrars: [
        {
          pluginId: "matrix",
          register: vi.fn(),
    ...
    expect(mocks.loadTheClawPluginCliRegistry).toHaveBeenCalledWith(
      expect.objectContaining({
        config: autoEnabledConfig,
  ...
  it("keeps runtime CLI command registration on the full plugin loader for legacy channel plugins", async () => {
    ...
    expect(mocks.loadTheClawPluginCliRegistry).not.toHaveBeenCalled();
  });
  ...
  it("lazy-registers descriptor-backed plugin commands on first invocation", async () => {
    ...
    await registerPluginCliCommands(program, {} as TheClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("falls back to eager registration when descriptors do not cover every command root", async () => {
    ...
    await registerPluginCliCommands(createProgram(), {} as TheClawConfig, undefined, undefined, {
      mode: "lazy",
    });
  ...
  it("loads validated plugin CLI config when the snapshot is valid", async () => {
    const loadedConfig = { plugins: { enabled: true } } as TheClawConfig;
    mocks.readConfigFileSnapshot.mockResolvedValueOnce({
      valid: true,
      config: loadedConfig,
    });
  ...
  it("skips plugin CLI registration from validated config when the snapshot is invalid", async () => {
    ...
    expect(mocks.loadTheClawPlugins).not.toHaveBeenCalled();
  });
});
