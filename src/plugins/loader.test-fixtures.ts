...
export type PluginLoadConfig = NonNullable<Parameters<typeof loadTheClawPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadTheClawPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "theclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.THECLAW_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.THECLAW_DISABLE_BUNDLED_PLUGINS;
const prevPluginStageDir = process.env.THECLAW_PLUGIN_STAGE_DIR;
...
export function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "theclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".theclaw", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ THECLAW_STATE_DIR: stateDir, ...params.env }, () =>
    loadTheClawPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.THECLAW_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.THECLAW_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
  if (prevPluginStageDir === undefined) {
    delete process.env.THECLAW_PLUGIN_STAGE_DIR;
  } else {
    process.env.THECLAW_PLUGIN_STAGE_DIR = prevPluginStageDir;
  }
}
