...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before TheClaw consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether TheClaw should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume TheClaw's bundle
   ...
   *
   * Use this when the backend should consume a canonical TheClaw auth profile
   ...
   * `profile-only` treats the selected TheClaw auth profile as the sole auth
