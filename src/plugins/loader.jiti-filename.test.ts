...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginJitiLoader", () => {
  it("uses the bundled plugin module path as the jiti filename", async () => {
    ...
    const { loadTheClawPlugins } = await importFreshModule<typeof import("./loader.js")>(
      import.meta.url,
      "./loader.js?scope=jiti-filename",
    );

    const pluginRoot = writeBundledPluginFixture("demo");
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = pluginRoot;
