...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing theclaw.extensions; update the plugin package to include theclaw.extensions (for example ["./dist/index.js"]). See https://docs.theclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-theclaw-extensions';
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_THECLAW_EXTENSIONS: "missing_theclaw_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_THECLAW_EXTENSIONS: "empty_theclaw_extensions",
  INVALID_THECLAW_EXTENSIONS: "invalid_theclaw_extensions",
...
function ensureTheClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_THECLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json theclaw.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_THECLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "theclaw security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureTheClawExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
/**
 * After the staged plugin tree has been scanned, symlink the host theclaw
 * package for plugins that declare it as a peer dependency.
 */
async function linkTheClawPeerDependencies(params: {
  installedDir: string;
  peerDependencies: Record<string, string>;
  logger: PluginInstallLogger;
}): Promise<void> {
  const peers = Object.keys(params.peerDependencies).filter((name) => name === "theclaw");
  if (peers.length === 0) {
    return;
  }

  const hostRoot = resolveTheClawPackageRootSync({
  ...
  if (!hostRoot) {
    params.logger.warn?.(
      "Could not locate theclaw package root to symlink peerDependencies; plugin may fail to resolve theclaw at runtime.",
    );
    return;
  }
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureTheClawExtensions({
    manifest,
  });
  ...
  // Prefer the canonical `id` from theclaw.plugin.json over the npm package name.
  // This avoids a latent key-mismatch bug: if the manifest id (e.g. "memory-cognee")
  // differs from the npm package name (e.g. "cognee-theclaw"), the plugin registry
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid theclaw.plugin.json: ${ocManifestResult.error}`,
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json theclaw.install.minHostVersion: ${minHostVersionCheck.error}`,
    ...
    if (minHostVersionCheck.kind === "unknown_host_version") {
      return {
        ok: false,
        error: `plugin "${pluginId}" requires TheClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host version could not be determined. Re-run from a released build or set THECLAW_VERSION and retry.`,
        code: PLUGIN_INSTALL_ERROR_CODE.UNKNOWN_HOST_VERSION,
      };
    }
    return {
      ok: false,
      error: `plugin "${pluginId}" requires TheClaw >=${minHostVersionCheck.requirement.minimumLabel}, but this host is ${minHostVersionCheck.currentVersion}. Upgrade TheClaw and retry.`,
  ...
  if (!extensionValidation.ok) {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_THECLAW_EXTENSIONS,
    };
  }
  ...
  return await installPluginDirectoryIntoExtensions({
    ...
    afterInstall: async (installedDir) => {
      ...
      // outside the install root — a rule our trusted host-theclaw link
      ...
      await linkTheClawPeerDependencies({
        installedDir,
...
export async function installPluginFromNpmSpec(
  ...
  const flowResult = await runtime.installFromNpmSpecArchiveWithInstaller({
    tempDirPrefix: "theclaw-npm-pack-",
    spec,
    timeoutMs,
