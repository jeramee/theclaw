...
type LoadTheClawPlugins = typeof import("./loader.js").loadTheClawPlugins;
...
const loadTheClawPluginsMock = vi.fn<LoadTheClawPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/theclaw.plugin.json`,
  };
}
...
function getLastSetupLoadedPluginConfig() {
  const call = loadTheClawPluginsMock.mock.calls.at(-1)?.[0];
  expect(call).toBeDefined();
...
function createAutoEnabledProviderConfig() {
  const rawConfig: TheClawConfig = {
    plugins: {},
  };
  const autoEnabledConfig: TheClawConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadTheClawPlugins: (...args: Parameters<LoadTheClawPlugins>) =>
        loadTheClawPluginsMock(...args),
  ...
  beforeEach(() => {
    ...
    loadTheClawPluginsMock.mockReturnValue(registry);
