...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@theclaw/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@theclaw/plugin-twitch",
        },
      },
      {
        stateDir,
