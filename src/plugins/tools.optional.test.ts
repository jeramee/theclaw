...
function expectLoaderCall(overrides: Record<string, unknown>) {
  expect(loadTheClawPluginsMock).toHaveBeenCalledWith(expect.objectContaining(overrides));
}
  ...
  it.each([
    {
      ...
      expectedLoaderCall: {
        env: { THECLAW_HOME: "/srv/theclaw-home" },
      },
    },
    {
