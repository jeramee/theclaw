...
function createTempDir(): string {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-public-surface-loader-"));
...
function createPackagedPublicArtifactWithStagedRuntimeDep(): {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ name: "theclaw", version: "0.0.0", type: "module" }, null, 2),
    "utf8",
  );
  fs.writeFileSync(
...
describe("bundled plugin public surface loader", () => {
  ...
  it("loads built public artifacts through staged runtime deps", async () => {
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = fixture.bundledPluginsDir;
