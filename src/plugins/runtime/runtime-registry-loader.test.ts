...
const mocks = vi.hoisted(() => ({
  loadTheClawPlugins: vi.fn<typeof import("../loader.js").loadTheClawPlugins>(),
...
vi.mock("../loader.js", () => ({
  loadTheClawPlugins: (...args: Parameters<typeof mocks.loadTheClawPlugins>) =>
    mocks.loadTheClawPlugins(...args),
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadTheClawPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadTheClawPlugins>) => mocks.loadTheClawPlugins(...args),
    );
  ...
  it("does not cache scoped loads by explicit plugin ids", () => {
    ...
    expect(mocks.loadTheClawPlugins).toHaveBeenCalledTimes(2);
    expect(mocks.loadTheClawPlugins).toHaveBeenNthCalledWith(
      1,
  ...
  it("does not forward empty channel scopes for broad channel loads", () => {
    ...
    expect(
      (mocks.loadTheClawPlugins.mock.calls[0]?.[0] as { onlyPluginIds?: string[] }).onlyPluginIds,
    ).toBeUndefined();
  });
