...
const suiteTempRootTracker = createSuiteTempRootTracker("theclaw-plugin-install-npm-spec");
...
describe("installPluginFromNpmSpec", () => {
  ...
  it("handles prerelease npm specs correctly", async () => {
    const prereleaseMetadata = {
      id: "@theclaw/voice-call@0.0.2-beta.1",
      name: "@theclaw/voice-call",
...

    {
      ...
      if (!result.ok) {
        ...
        expect(result.error).toContain('"@theclaw/voice-call@beta"');
      }
    }
...

    {
      ...
      expect(result.npmResolution?.resolvedSpec).toBe("@theclaw/voice-call@0.0.2-beta.1");
      expectSingleNpmPackIgnoreScriptsCall({
        calls: run.mock.calls as Array<[unknown, unknown]>,
        expectedSpec: "@theclaw/voice-call@beta",
      });
      expect(packTmpDir).not.toBe("");
    }
  });
