...
import type {
  TheClawPluginHookOptions,
  TheClawPluginToolContext,
  TheClawPluginToolFactory,
  TheClawPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type TheClawPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<TheClawConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). TheClaw uses this to expose
 ...
 *
 * Runs after TheClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common TheClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and TheClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before TheClaw builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. TheClaw no
...
/**
 * Provider-owned stream wrapper hook after TheClaw applies its generic
 ...
 *
 * Return `true` or `false` to override TheClaw's built-in provider cache TTL
 ...
 *
 * Runs only after TheClaw exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that TheClaw should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after TheClaw loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, TheClaw falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * TheClaw calls this only from async model resolution paths. After it
   ...
   *
   * Runs after TheClaw performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before TheClaw registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic TheClaw wrappers.
   *
   ...
   *
   * Called after TheClaw resolves the raw configured credential but before the
   ...
   * hint than TheClaw's generic auth-store guidance.
   */
   ...
   * TheClaw's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when TheClaw should surface a provider-specific hint for
   ...
   * merging. TheClaw deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. TheClaw stores the
   ...
   * without replacing the full TheClaw-owned system prompt.
   */
   ...
   *
   * Runs after TheClaw's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep TheClaw's default fallback policy.
   */
   ...
   * rewrites after TheClaw has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before TheClaw handles
   ...
   *
   * TheClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `theclaw doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `theclaw doctor` should offer.
   *
   ...
   *
   * TheClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep TheClaw's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * TheClaw falls back to that stored profile.
   */
...
export type TheClawPluginGatewayMethod = {
  method: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active TheClaw session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current TheClaw configuration */
  config: TheClawConfig;
 ...
 */
export type TheClawPluginCommandDefinition = {
...
export type TheClawPluginHttpRouteAuth = "gateway" | "plugin";
export type TheClawPluginHttpRouteMatch = "exact" | "prefix";
export type TheClawPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type TheClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type TheClawPluginHttpRouteParams = {
  path: string;
  handler: TheClawPluginHttpRouteHandler;
  auth: TheClawPluginHttpRouteAuth;
  match?: TheClawPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: TheClawPluginGatewayRuntimeScopeSurface;
  replaceExisting?: boolean;
};

export type TheClawPluginCliContext = {
  program: Command;
  config: TheClawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type TheClawPluginCliRegistrar = (ctx: TheClawPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want TheClaw to keep a plugin command lazy-loaded while still
 ...
 */
export type TheClawPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type TheClawPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type TheClawPluginNodeHostCommand = {
  command: string;
  cap?: string;
  handle: (paramsJSON?: string | null) => Promise<string>;
};

export type TheClawPluginSecurityAuditContext = {
  config: TheClawConfig;
  sourceConfig: TheClawConfig;
  env: NodeJS.ProcessEnv;
  stateDir: string;
  configPath: string;
};

export type TheClawPluginSecurityAuditCollector = (
  ctx: TheClawPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type TheClawGatewayDiscoveryAdvertiseContext = {
...
export type TheClawGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: TheClawGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type TheClawPluginServiceContext = {
  config: TheClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type TheClawPluginService = {
  id: string;
  start: (ctx: TheClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: TheClawPluginServiceContext) => void | Promise<void>;
};

export type TheClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type TheClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `theclaw.plugin.json` via
  ...
  configSchema?: TheClawPluginConfigSchema;
  reload?: TheClawPluginReloadRegistration;
  nodeHostCommands?: TheClawPluginNodeHostCommand[];
  securityAuditCollectors?: TheClawPluginSecurityAuditCollector[];
  register?: (api: TheClawPluginApi) => void;
  activate?: (api: TheClawPluginApi) => void;
};

export type TheClawPluginModule = TheClawPluginDefinition | ((api: TheClawPluginApi) => void);
...
export type PluginConfigMigration = (config: TheClawConfig) =>
  | {
      config: TheClawConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `theclaw migrate`. */
...
export type TheClawPluginApi = {
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | TheClawPluginToolFactory,
    opts?: TheClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: TheClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: TheClawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: TheClawPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: TheClawPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every top-level command root, TheClaw can keep
       * the plugin registrar lazy in the normal root CLI path. Command-only
       * registrations stay on the eager compatibility path.
       */
      descriptors?: TheClawPluginCliCommandDescriptor[];
    },
  ) => void;
  registerReload: (registration: TheClawPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: TheClawPluginNodeHostCommand) => void;
  registerSecurityAuditCollector: (collector: TheClawPluginSecurityAuditCollector) => void;
  registerService: (service: TheClawPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: TheClawGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `theclaw migrate` (migration capability). */
   ...
   */
  registerCommand: (command: TheClawPluginCommandDefinition) => void;
