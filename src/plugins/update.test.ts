...
import { bundledPluginRootAt } from "theclaw/plugin-sdk/test-fixtures";
...
function createSuccessfulNpmUpdateResult(params?: {
  ...
  return {
    ok: true,
    pluginId: params?.pluginId ?? "opik-theclaw",
    targetDir: params?.targetDir ?? "/tmp/opik-theclaw",
...
function createCodexAppServerInstallConfig(params: {
  ...
  return {
    plugins: {
      installs: {
        "theclaw-codex-app-server": {
          source: "npm" as const,
          spec: params.spec,
          installPath: "/tmp/theclaw-codex-app-server",
          ...(params.resolvedName ? { resolvedName: params.resolvedName } : {}),
          ...(params.resolvedSpec ? { resolvedSpec: params.resolvedSpec } : {}),
        },
      },
...
function createInstalledPackageDir(params: { name?: string; version: string }): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-plugin-update-test-"));
  tempDirs.push(dir);
  fs.writeFileSync(
...
function createBundledSource(params?: { pluginId?: string; localPath?: string; npmSpec?: string }) {
  ...
  return {
    ...
    npmSpec: params?.npmSpec ?? `@theclaw/${pluginId}`,
  };
}
...
function expectCodexAppServerInstallState(params: {
  ...
  expect(params.result.config.plugins?.installs?.["theclaw-codex-app-server"]).toMatchObject({
    source: "npm",
    spec: params.spec,
    installPath: "/tmp/theclaw-codex-app-server",
    version: params.version,
...
describe("updateNpmInstalledPlugins", () => {
  ...
  it.each([
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "opik-theclaw",
        spec: "@opik/opik-theclaw",
        integrity: "sha512-old",
        installPath: "/tmp/opik-theclaw",
      }),
      pluginIds: ["opik-theclaw"],
      dryRun: true,
      expectedCall: {
        spec: "@opik/opik-theclaw",
    ...
    },
    {
      ...
      config: createNpmInstallConfig({
        pluginId: "theclaw-codex-app-server",
        spec: "theclaw-codex-app-server@0.2.0-beta.3",
        integrity: "sha512-old",
        installPath: "/tmp/theclaw-codex-app-server",
      }),
      pluginIds: ["theclaw-codex-app-server"],
      specOverrides: {
        "theclaw-codex-app-server": "theclaw-codex-app-server@0.2.0-beta.4",
      },
      installerResult: createSuccessfulNpmUpdateResult({
        pluginId: "theclaw-codex-app-server",
        targetDir: "/tmp/theclaw-codex-app-server",
        version: "0.2.0-beta.4",
      }),
      expectedCall: {
        spec: "theclaw-codex-app-server@0.2.0-beta.4",
        expectedIntegrity: undefined,
      },
    },
  ] as const)(
  ...
  it("expands home-relative install paths before checking installed npm versions", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-plugin-update-home-"));
    tempDirs.push(home);
    const installPath = path.join(home, ".theclaw", "extensions", "lossless-claw");
    ...
    const result = await updateNpmInstalledPlugins({
      config: createNpmInstallConfig({
        pluginId: "lossless-claw",
        spec: "@martian-engineering/lossless-claw",
        installPath: "~/.theclaw/extensions/lossless-claw",
        resolvedName: "@martian-engineering/lossless-claw",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@martian-engineering/lossless-claw@0.9.0",
        integrity: "sha512-same",
  ...
  it("aborts exact pinned npm plugin updates on integrity drift by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockImplementation(
      async (params: {
        ...
        const proceed = await params.onIntegrityDrift?.({
          ...
          resolution: {
            integrity: "sha512-new",
            resolvedSpec: "@opik/opik-theclaw@0.2.5",
            version: "0.2.5",
          },
        });
        if (proceed === false) {
          return {
            ok: false,
            error: "aborted: npm package integrity drift detected for @opik/opik-theclaw@0.2.5",
          };
        }
    ...
    expect(warn).toHaveBeenCalledWith(
      'Integrity drift for "opik-theclaw" (@opik/opik-theclaw@0.2.5): expected sha512-old, got sha512-new',
    );
    expect(result.changed).toBe(false);
    expect(result.config).toBe(config);
    expect(result.outcomes).toEqual([
      {
        ...
        status: "error",
        message:
          "Failed to update opik-theclaw: aborted: npm package integrity drift detected for @opik/opik-theclaw@0.2.5",
      },
    ]);
  });
  ...
  it.each([
    {
      ...
      installerResult: {
        ...
        error: "Package not found on npm: @theclaw/missing.",
      },
      config: createNpmInstallConfig({
        pluginId: "missing",
        spec: "@theclaw/missing",
        installPath: "/tmp/missing",
      }),
      pluginId: "missing",
      expectedMessage: "Failed to check missing: npm package not found for @theclaw/missing.",
    },
    {
  ...
  it.each([
    {
      ...
      config: createCodexAppServerInstallConfig({
        spec: "theclaw-codex-app-server@beta",
        resolvedName: "theclaw-codex-app-server",
        resolvedSpec: "theclaw-codex-app-server@0.2.0-beta.3",
      }),
      expectedSpec: "theclaw-codex-app-server@beta",
    ...
    },
    {
      ...
      installerResult: {
        ...
        extensions: ["index.ts"],
        npmResolution: {
          name: "theclaw-codex-app-server",
          version: "0.2.0-beta.4",
          resolvedSpec: "theclaw-codex-app-server@0.2.0-beta.4",
        },
      },
      config: createCodexAppServerInstallConfig({
        spec: "theclaw-codex-app-server",
      }),
      specOverrides: {
        "theclaw-codex-app-server": "theclaw-codex-app-server@beta",
      },
      expectedSpec: "theclaw-codex-app-server@beta",
      expectedVersion: "0.2.0-beta.4",
      expectedResolvedSpec: "theclaw-codex-app-server@0.2.0-beta.4",
    },
  ] as const)(
    "$name",
    async ({
      ...
      expectNpmUpdateCall({
        spec: expectedSpec,
        expectedPluginId: "theclaw-codex-app-server",
      });
  ...
  it("migrates legacy unscoped install keys when a scoped npm package updates", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@theclaw/voice-call",
      targetDir: "/tmp/theclaw-voice-call",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.allow).toEqual(["@theclaw/voice-call"]);
    expect(result.config.plugins?.deny).toEqual(["@theclaw/voice-call"]);
    expect(result.config.plugins?.slots?.memory).toBe("@theclaw/voice-call");
    expect(result.config.plugins?.entries?.["@theclaw/voice-call"]).toEqual({
      enabled: false,
      hooks: { allowPromptInjection: false },
    });
    expect(result.config.plugins?.entries?.["voice-call"]).toBeUndefined();
    expect(result.config.plugins?.installs?.["@theclaw/voice-call"]).toMatchObject({
      source: "npm",
      spec: "@theclaw/voice-call",
      installPath: "/tmp/theclaw-voice-call",
      version: "0.0.2",
    });
  ...
  it("migrates context engine slot when a plugin id changes during update", async () => {
    installPluginFromNpmSpecMock.mockResolvedValue({
      ok: true,
      pluginId: "@theclaw/context-engine",
      targetDir: "/tmp/theclaw-context-engine",
      version: "0.0.2",
      extensions: ["index.ts"],
    });
    ...
    expect(result.config.plugins?.slots?.contextEngine).toBe("@theclaw/context-engine");
    expect(result.config.plugins?.installs?.["@theclaw/context-engine"]).toMatchObject({
      source: "npm",
      spec: "@theclaw/context-engine",
      installPath: "/tmp/theclaw-context-engine",
      version: "0.0.2",
    });
  ...
  it("reuses the recorded managed extensions root when updating external plugins", async () => {
    const installPath = "/var/theclaw/extensions/demo";
    const extensionsDir = "/var/theclaw/extensions";
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        pluginId: "demo",
        targetDir: installPath,
...
describe("syncPluginsForUpdateChannel", () => {
  ...
  it("forwards an explicit env to bundled plugin source resolution", async () => {
    ...
    const env = { THECLAW_HOME: "/srv/theclaw-home" } as NodeJS.ProcessEnv;
  ...
  it("uses the provided env when matching bundled load and install paths", async () => {
    const bundledHome = "/tmp/theclaw-home";
    mockBundledSources(
      createBundledSource({
  ...
  it("installs an externalized bundled plugin and rewrites its old bundled path plugin index", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/theclaw-plugins/legacy-chat",
        version: "2.0.0",
        npmResolution: {
          name: "@theclaw/legacy-chat",
          version: "2.0.0",
          resolvedSpec: "@theclaw/legacy-chat@2.0.0",
        },
      }),
    );
    ...
    expect(result.config.plugins?.installs?.["legacy-chat"]).toMatchObject({
      source: "npm",
      spec: "@theclaw/legacy-chat",
      installPath: "/tmp/theclaw-plugins/legacy-chat",
      version: "2.0.0",
      resolvedName: "@theclaw/legacy-chat",
  ...
  it("externalizes bundled plugins that were enabled by default", async () => {
    ...
    installPluginFromNpmSpecMock.mockResolvedValue(
      createSuccessfulNpmUpdateResult({
        ...
        targetDir: "/tmp/theclaw-plugins/default-chat",
        version: "2.0.0",
      }),
    );
    ...
    expect(result.config.plugins?.installs?.["default-chat"]).toMatchObject({
      source: "npm",
      spec: "@theclaw/default-chat",
      installPath: "/tmp/theclaw-plugins/default-chat",
      version: "2.0.0",
    });
  });
