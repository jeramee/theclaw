...
function makeTempRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-bundled-runtime-root-"));
  tempRoots.push(root);
  return root;
}
...
describe("prepareBundledPluginRuntimeRoot", () => {
  it("materializes root JavaScript chunks in external mirrors", () => {
    ...
    const env = { ...process.env, THECLAW_PLUGIN_STAGE_DIR: stageDir };
  ...
  it("does not copy staged runtime mirror dist files onto themselves", () => {
    const stageDir = makeTempRoot();
    const installRoot = path.join(stageDir, "theclaw-2026.4.26-alpha");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "theclaw", version: "2026.4.26", type: "module" }),
      "utf8",
    );
  ...
  it("mirrors canonical dist chunks when loading from dist-runtime", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.27", type: "module" }),
      "utf8",
    );
    fs.writeFileSync(
  ...
  it("fingerprints runtime mirror source roots before taking the mirror lock", () => {
    ...
    fs.writeFileSync(
      path.join(canonicalPluginRoot, "package.json"),
      JSON.stringify({ name: "@theclaw/qqbot", version: "1.0.0", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(
      path.join(runtimePluginRoot, "index.js"),
    ...
    const lockPath = path.join(installRoot, ".theclaw-runtime-mirror.lock");
