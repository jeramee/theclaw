...
const APPROVALS_PATH = "~/.theclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "theclaw-app-server:thread:",
  "theclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("theclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("theclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to TheClaw instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
