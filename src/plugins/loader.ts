...
import { discoverTheClawPlugins, type PluginCandidate } from "./discovery.js";
...
const BUNDLED_RUNTIME_MIRROR_LOCK_DIR = ".theclaw-runtime-mirror.lock";
...
function createGuardedPluginRegistrationApi(api: TheClawPluginApi): {
  api: TheClawPluginApi;
  close: () => void;
} {
  let closed = false;
  return {
...
function runPluginRegisterSync(
  register: NonNullable<TheClawPluginDefinition["register"]>,
  api: Parameters<NonNullable<TheClawPluginDefinition["register"]>>[0],
): void {
...
function prepareBundledPluginRuntimeDistMirror(params: {
  ...
  ensureTheClawPluginSdkAlias(mirrorDistRoot);
...
function mirrorCanonicalBundledRuntimeDistRoot(params: {
  ...
  ensureTheClawPluginSdkAlias(targetCanonicalDistRoot);
...
function ensureTheClawPluginSdkAlias(distRoot: string): void {
  const pluginSdkDir = path.join(distRoot, "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(distRoot, "extensions", "node_modules", "theclaw");
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadTheClawPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadTheClawPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: TheClawPluginDefinition;
  register?: TheClawPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as TheClawPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as TheClawPluginDefinition;
...
function isPluginLoadDebugEnabled(env: NodeJS.ProcessEnv): boolean {
  const normalized = normalizeLowercaseStringOrEmpty(env.THECLAW_PLUGIN_LOAD_DEBUG);
...
function recordPluginError(params: {
  ...
  const errorText =
    process.env.THECLAW_PLUGIN_LOADER_DEBUG_STACKS === "1" &&
...
function resolveCandidateDuplicateRank(params: {
  candidate: ReturnType<typeof discoverTheClawPlugins>["candidates"][number];
...
function compareDuplicateCandidateOrder(params: {
  left: ReturnType<typeof discoverTheClawPlugins>["candidates"][number];
  right: ReturnType<typeof discoverTheClawPlugins>["candidates"][number];
  manifestByRoot: Map<string, ReturnType<typeof loadPluginManifestRegistry>["plugins"][number]>;
  provenance: PluginProvenanceIndex;
  env: NodeJS.ProcessEnv;
}): number {
...
export function loadTheClawPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  const {
    env,
    cfg,
    normalized,
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (
        ...
        try {
          ...
          if (path.resolve(installRoot) !== path.resolve(pluginRoot)) {
            ...
            ensureTheClawPluginSdkAlias(path.dirname(path.dirname(pluginRoot)));
          }
        } catch (error) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into TheClaw yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: TheClawPluginModule | null = null;
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => getJiti(safeSource)(safeImportSource) as TheClawPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: TheClawPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => getJiti(safeRuntimeSource)(safeRuntimeImportSource) as TheClawPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'theclaw plugins list' for details.`,
        );
      }
    }
...
export async function loadTheClawPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  const discovery = discoverTheClawPlugins({
    workspaceDir: options.workspaceDir,
    extraPaths: normalized.loadPaths,
    cache: false,
    env,
