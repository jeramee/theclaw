...
function writeBundledPlugin(params: {
  ...
  delete process.env.THECLAW_DISABLE_BUNDLED_PLUGINS;
  process.env.THECLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".theclaw", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ THECLAW_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
      ...
      JSON.stringify(
        {
          ...
          theclaw: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.THECLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadTheClawPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadTheClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadTheClawPlugins>[0], "cache" | "config">,
) {
  return loadTheClawPlugins({
    cache: false,
    ...options,
...
function createEscapingEntryFixture(params: { id: string; sourceBody: string }) {
  ...
  fs.writeFileSync(
    path.join(pluginDir, "theclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadTheClawPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadTheClawPlugins>;
  loadSecond: () => ReturnType<typeof loadTheClawPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadTheClawPlugins>;
  loadVariant: () => ReturnType<typeof loadTheClawPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const theclawHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(theclawHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
  ...
  const env = {
    ...process.env,
    THECLAW_HOME: theclawHome,
    HOME: ignoredHome,
    THECLAW_STATE_DIR: stateDir,
    THECLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
  };
  return { plugin, env };
}
...
describe("loadTheClawPlugins", () => {
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "theclaw.plugin.json"));
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "theclaw", "plugin-sdk");
    ...
    __testing.ensureTheClawPluginSdkAlias(distRoot);
...
module.exports = {
  ...
  register() {
    ...
    fs.writeFileSync(
      path.join(plugin.dir, "theclaw.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
          channels: ["discord"],
...
`,
  ...
  it("loads bundled runtime deps from an external stage dir", () => {
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = bundledDir;
    process.env.THECLAW_PLUGIN_STAGE_DIR = stageDir;
    fs.writeFileSync(
      path.join(plugin.dir, "package.json"),
      JSON.stringify(
        {
  ...
  it("loads bundled plugins from symlinked package roots with an external stage dir", () => {
    ...
    const stageDir = makeTempDir();
    const aliasRoot = path.join(makeTempDir(), "theclaw-alias");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({ name: "theclaw", version: "2026.4.25", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
      path.join(plugin.dir, "package.json"),
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = path.join(aliasRoot, "dist", "extensions");
    process.env.THECLAW_PLUGIN_STAGE_DIR = stageDir;
  ...
  it("loads copied external runtime mirrors with package-root runtime deps", () => {
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "theclaw.plugin.json"),
      JSON.stringify(
        {
          id: "alpha",
          enabledByDefault: true,
  ...
  it("materializes root JavaScript chunks in external runtime mirrors", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.24", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = path.join(actualInstallRoot, "dist", "extensions");
    const reloadedRegistry = loadTheClawPlugins({
      cache: false,
      config: {
        plugins: {
          enabled: true,
  ...
  it("loads bundled plugins with plugin-sdk imports from an external stage dir", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "theclaw", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      ...
      [
        ...
        `import { normalizeLowercaseStringOrEmpty } from "theclaw/plugin-sdk/text-runtime";`,
        `export default {`,
        `  id: "telegram",`,
        `  register(api) {`,
        `    api.registerCommand({`,
    ...
    expect(fs.existsSync(path.join(bundledDir, "node_modules", "theclaw"))).toBe(false);
  });
  ...
  it("loads dist-runtime wrappers from an external stage dir without rewriting mirrors on reload", async () => {
    ...
    const lockPath = path.join(installRootPlan.installRoot, ".theclaw-runtime-mirror.lock");
  ...
  it("loads native ESM deps from a layered baseline stage dir", () => {
    ...
    const env = {
      THECLAW_PLUGIN_STAGE_DIR: [baselineStageDir, writableStageDir].join(path.delimiter),
    };
    ...
    process.env.THECLAW_BUNDLED_PLUGINS_DIR = bundledDir;
    process.env.THECLAW_PLUGIN_STAGE_DIR = env.THECLAW_PLUGIN_STAGE_DIR;
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadTheClawPlugins>) => {
...
module.exports = { id: "manifest-only-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "theclaw.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__theclaw_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__theclaw_loader_reentry_error";
        const reenterFnMarker = "__theclaw_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadTheClawPlugins>[0]) => loadTheClawPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadTheClawPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadTheClawPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__theclaw_runtime_registry_reentry_marker";
        const resolverMarker = "__theclaw_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadTheClawPlugins(options);
        const scoped = loadTheClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadTheClawPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("only publishes plugin commands to the global registry during activating loads", async () => {
    ...
    const active = loadTheClawPlugins({
      cache: false,
      workspaceDir: plugin.dir,
      config: {
        plugins: {
  ...
  it("replaces prior plugin hook registrations on activating reloads", async () => {
    ...
    loadTheClawPlugins(loadOptions);
    loadTheClawPlugins(loadOptions);
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadTheClawPlugins(options);
    ...
    const second = loadTheClawPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__theclawDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadTheClawPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__theclawSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__theclawGatewayBootRegisterCount";
    const plugin = writePlugin({
  ...
  it("preserves the gateway-bindable hook runner across later default-mode activating loads", () => {
    ...
    const gatewayRegistry = loadTheClawPlugins({
    ...
    const defaultRegistry = loadTheClawPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadTheClawPlugins({
              ...
              env: {
                ...process.env,
                THECLAW_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadTheClawPlugins({
              ...
              env: {
                ...process.env,
                THECLAW_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadTheClawPlugins({
              ...
              env: {
                ...process.env,
                HOME: homeA,
                THECLAW_HOME: undefined,
                THECLAW_STATE_DIR: stateDir,
                THECLAW_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadTheClawPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(theclawHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadTheClawPlugins(options),
          loadVariant: () =>
            loadTheClawPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadTheClawPlugins({
      env: {
        ...
        HOME: homeDir,
        THECLAW_HOME: undefined,
        THECLAW_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers THECLAW_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const theclawHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "theclaw-home-demo",
      dir: path.join(theclawHome, "plugins", "theclaw-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "theclaw-home-demo", register() {} };`,
    });
    ...
    const registry = loadTheClawPlugins({
      ...
      config: {
        plugins: {
          allow: ["theclaw-home-demo"],
          entries: {
            "theclaw-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/theclaw-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "theclaw-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ THECLAW_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadTheClawPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadTheClawPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@theclaw/nested-default-channel",
          theclaw: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "theclaw.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadTheClawPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
    });
    ...
    const scopedSetupRegistry = loadTheClawPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@theclaw/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@theclaw/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@theclaw/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@theclaw/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@theclaw/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@theclaw/setup-entry-throws-test",
          theclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@theclaw/setup-entry-throws-sibling-test",
          theclaw: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "theclaw.plugin.json"),
      JSON.stringify(
        {
    ...
    const scenarios = [
          ...
          return withEnv(
            {
              ...
              THECLAW_BUNDLED_PLUGINS_DIR: undefined,
            },
            () =>
              loadTheClawPlugins({
                cache: false,
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "theclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "theclaw.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryCoreDir, "theclaw.plugin.json"),
            JSON.stringify(
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "theclaw.plugin.json"),
            JSON.stringify(
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ THECLAW_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ THECLAW_STATE_DIR: stateDir }, () => {
  ...
  if (!Object.getOwnPropertyDescriptor(runtime, "channel")) {
    ...
    const registry = withEnv({ THECLAW_STATE_DIR: stateDir }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", async () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("theclaw/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ THECLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadTheClawPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__theclawLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("theclaw/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("theclaw/plugin-sdk");
