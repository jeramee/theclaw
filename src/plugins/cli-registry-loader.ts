...
export type PluginCliCommandGroupEntry = {
  pluginId: string;
  placeholders: readonly TheClawPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: TheClawPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadTheClawPluginCliRegistry(
...
): Promise<TheClawPluginCliCommandDescriptor[]> {
  try {
