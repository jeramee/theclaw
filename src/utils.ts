...
export function resolveConfigDir(
  ...
  const configPath = env.THECLAW_CONFIG_PATH?.trim();
  if (configPath) {
    return path.dirname(resolveUserPath(configPath, env, homedir));
  }
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".theclaw");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.THECLAW_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$THECLAW_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via THECLAW_STATE_DIR.
