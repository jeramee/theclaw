---
summary: "How to route TheClaw runtime HTTP and WebSocket traffic through an operator-managed filtering proxy"
title: "Network proxy"
read_when:
  - You want defense-in-depth against SSRF and DNS rebinding attacks
  - Configuring an external forward proxy for TheClaw runtime traffic
---

# Network Proxy

TheClaw can route runtime HTTP and WebSocket traffic through an operator-managed forward proxy. This is optional defense in depth for deployments that want central egress control, stronger SSRF protection, and better network auditability.

TheClaw does not ship, download, start, configure, or certify a proxy. You run the proxy technology that fits your environment, and TheClaw routes normal process-local HTTP and WebSocket clients through it.
...
- Operational control: enforce destination rules, network segmentation, rate limits, or outbound allowlists without rebuilding TheClaw.

TheClaw still keeps application-level SSRF guards such as `fetchWithSsrFGuard`. Proxy routing is an additional process-level guardrail for normal HTTP and WebSocket egress, not a replacement for guarded fetches or an OS-level network sandbox.

## How TheClaw Routes Traffic

When `proxy.enabled=true` and a proxy URL is configured, protected runtime processes such as `theclaw gateway run`, `theclaw node run`, and `theclaw agent --local` route normal HTTP and WebSocket egress through the configured proxy:
...
The public contract is the routing behavior, not the internal Node hooks used to implement it. TheClaw Gateway control-plane WebSocket clients use a narrow direct path for local loopback Gateway RPC traffic when the Gateway URL uses a literal loopback IP such as `127.0.0.1` or `[::1]`. That control-plane path must be able to reach loopback Gateways even when the operator proxy blocks loopback destinations. Normal runtime HTTP and WebSocket requests still use the configured proxy.

The proxy URL itself must use `http://`. HTTPS destinations are still supported through the proxy with HTTP `CONNECT`; this only means TheClaw expects a plain HTTP forward-proxy listener such as `http://127.0.0.1:3128`.

While the proxy is active, TheClaw clears `no_proxy`, `NO_PROXY`, and `GLOBAL_AGENT_NO_PROXY`. Those bypass lists are destination-based, so leaving `localhost` or `127.0.0.1` there would let high-risk SSRF targets skip the filtering proxy.

On shutdown, TheClaw restores the previous proxy environment and resets cached process routing state.
...
```bash
THECLAW_PROXY_URL=http://127.0.0.1:3128 theclaw gateway run
```

`proxy.proxyUrl` takes precedence over `THECLAW_PROXY_URL`.
...
For managed gateway services started with `theclaw gateway start`, prefer storing the URL in config:

```bash
theclaw config set proxy.enabled true
theclaw config set proxy.proxyUrl http://127.0.0.1:3128
theclaw gateway install --force
theclaw gateway start
```

The environment fallback is best for foreground runs. If you use it with an installed service, put `THECLAW_PROXY_URL` in the service durable environment, such as `$THECLAW_STATE_DIR/.env` or `~/.theclaw/.env`, then reinstall the service so launchd, systemd, or Scheduled Tasks starts the gateway with that value.

For `theclaw --container ...` commands, TheClaw forwards `THECLAW_PROXY_URL` into the container-targeted child CLI when it is set. The URL must be reachable from inside the container; `127.0.0.1` refers to the container itself, not the host. TheClaw rejects loopback proxy URLs for container-targeted commands unless you explicitly override that safety check.

## Proxy Requirements

The proxy policy is the security boundary. TheClaw cannot verify that the proxy blocks the right targets.

Configure the proxy to:

- Bind only to loopback or a private trusted interface.
- Restrict access so only the TheClaw process, host, container, or service account can use it.
...
TheClaw application-level classifier logic lives in `src/infra/net/ssrf.ts` and `src/shared/net/ip.ts`. The relevant parity hooks are `BLOCKED_HOSTNAMES`, `BLOCKED_IPV4_SPECIAL_USE_RANGES`, `BLOCKED_IPV6_SPECIAL_USE_RANGES`, `RFC2544_BENCHMARK_PREFIX`, and the embedded IPv4 sentinel handling for NAT64, 6to4, Teredo, ISATAP, and IPv4-mapped forms. Those files are useful references when maintaining an external proxy policy, but TheClaw does not automatically export or enforce those rules in your proxy.
...
Validate the proxy from the same host, container, or service account that runs TheClaw:
...
Then enable TheClaw proxy routing:
...
- User local WebUIs and local model servers should be allowlisted in the operator proxy policy when needed; TheClaw does not expose a general local-network bypass for them.
...
- TheClaw does not inspect, test, or certify your proxy policy.
