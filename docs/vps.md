---
summary: "Run TheClaw on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for TheClaw
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the TheClaw Gateway on any Linux server or cloud VPS. This page helps you
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/theclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- `THECLAW_NO_RESPAWN=1` avoids extra startup overhead from a self-respawn path.
...
- Add service env for a stable startup path:
  ...
  - `NODE_COMPILE_CACHE=/var/tmp/theclaw-compile-cache`
...
For the standard `theclaw onboard --install-daemon` path, edit the user unit:
...
`theclaw-gateway.service` via `sudo systemctl edit theclaw-gateway.service`.
