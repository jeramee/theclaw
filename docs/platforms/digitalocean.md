---
summary: "TheClaw on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up TheClaw on DigitalOcean
  - Looking for cheap VPS hosting for TheClaw
title: "DigitalOcean (platform)"
---

# TheClaw on DigitalOcean

## Goal

Run a persistent TheClaw Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
```bash
theclaw config set gateway.bind tailnet
theclaw gateway restart
```
...
```bash
theclaw channels login whatsapp
# Scan QR code
```
...
- `~/.theclaw/workspace/` — workspace (SOUL.md, memory, etc.)
...
journalctl --user -u theclaw-gateway.service --no-pager -n 50
```
