---
summary: "TheClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up TheClaw on a Raspberry Pi
  - Running TheClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (platform)"
---

# TheClaw on Raspberry Pi

## Goal

Run a persistent, always-on TheClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl --user -u theclaw-gateway.service -f
```

## 9) Access the TheClaw Dashboard
...
```bash
ssh user@gateway-host 'theclaw dashboard --no-open'
```
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/theclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/theclaw-compile-cache
mkdir -p /var/tmp/theclaw-compile-cache
export THECLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `THECLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running TheClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
systemctl --user edit theclaw-gateway.service
```

```ini
[Service]
Environment=THECLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/theclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep TheClaw state/cache on SSD-backed storage to avoid SD-card
...
Most TheClaw features work on ARM64, but some external binaries may need ARM builds:
...
systemctl --user is-enabled theclaw-gateway.service

# Enable if not
systemctl --user enable theclaw-gateway.service
...
```bash
# Check logs
journalctl --user -u theclaw-gateway.service --no-pager -n 100

# Common fix: rebuild
cd ~/theclaw  # if using hackable install
npm run build
systemctl --user restart theclaw-gateway.service
```
