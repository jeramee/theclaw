...
service via the external `theclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.theclaw.gateway`
  (or `ai.theclaw.<profile>` when using `--profile`/`THECLAW_PROFILE`; legacy `com.theclaw.*` is supported).
