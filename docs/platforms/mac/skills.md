...
The macOS app surfaces TheClaw skills via the gateway; it does not parse skills locally.

## Data source

- `skills.status` (gateway) returns all skills plus eligibility and missing requirements
  (including allowlist blocks for bundled skills).
- Requirements are derived from `metadata.theclaw.requires` in each `SKILL.md`.

## Install actions

- `metadata.theclaw.install` defines install options (brew/node/go/uv).
...
- The app stores keys in `~/.theclaw/theclaw.json` under `skills.entries.<skillKey>`.
