...
- Same path: run the app from a fixed location (for TheClaw, `dist/TheClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.theclaw.mac
sudo tccutil reset ScreenCapture ai.theclaw.mac
...
Workaround: move files into the TheClaw workspace (`~/.theclaw/workspace`) if you want to avoid per-folder grants.
