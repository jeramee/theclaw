...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `theclaw.internal.`), **or**
...
Run `theclaw devices list` again before approval.
...
theclaw gateway call node.list --params "{}"
```
...
- `THECLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine TheClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight TheClaw builds.
...
```bash
export THECLAW_APNS_TEAM_ID="TEAMID"
export THECLAW_APNS_KEY_ID="KEYID"
export THECLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.theclaw/credentials/apns
chmod 700 ~/.theclaw/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.theclaw/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.theclaw/credentials/apns/AuthKey_KEYID.p8
export THECLAW_APNS_PRIVATE_KEY_PATH="$HOME/.theclaw/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_theclaw-gw._tcp` on `local.` and, when configured, the same
...
`theclaw.internal.`) and Tailscale split DNS.
...
```bash
theclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__theclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__theclaw__/canvas/` and `/__theclaw__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through TheClaw node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through TheClaw by invoking node
...
```bash
theclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__theclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
theclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `theclaw devices list` and approve manually.
