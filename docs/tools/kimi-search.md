...
TheClaw supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), TheClaw reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.theclaw/.env`.

If you omit `baseUrl`, TheClaw defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, TheClaw defaults to `kimi-k2.6`.
