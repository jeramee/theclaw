...
`theclaw agent` runs a single agent turn from the command line without needing
...
<Steps>
  <Step title="Run a simple agent turn">
    ```bash
    theclaw agent --message "What is the weather today?"
    ```
  ...
  <Step title="Target a specific agent or session">
    ...
    theclaw agent --to +15555550123 --message "Status update"

    # Reuse an existing session
    theclaw agent --session-id abc123 --message "Continue the task"
    ```
  ...
  <Step title="Deliver the reply to a channel">
    ...
    theclaw agent --to +15555550123 --message "Report ready" --deliver

    # Deliver to Slack
    theclaw agent --agent ops --message "Generate report" \
...
theclaw agent --agent ops --message "Alert" --deliver --reply-channel telegram --reply-to "@admin"
```
