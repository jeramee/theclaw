---
summary: "ClawHub: public registry for TheClaw skills and plugins, native install flows, and the clawhub CLI"
read_when:
...
ClawHub is the public registry for **TheClaw skills and plugins**.

- Use native `theclaw` commands to search, install, and update skills, and to install plugins from ClawHub.
...
<Steps>
  ...
  <Step title="Use">
    Start a new TheClaw session — it picks up the new skill.
  </Step>
...
<Tabs>
  <Tab title="Skills">
    ...
    Native `theclaw` commands install into your active workspace and
    persist source metadata so later `update` calls can stay on ClawHub.

  </Tab>
  <Tab title="Plugins">
    ```bash
    theclaw plugins install clawhub:<package>
...
<Note>
`theclaw plugins install clawhub:...` only accepts installable plugin
families. If a ClawHub package is actually a skill, TheClaw stops and
points you at `theclaw skills install <slug>` instead.

Anonymous ClawHub plugin installs also fail closed for private packages.
Community or other non-official channels can still install, but TheClaw
...
- A public registry for TheClaw skills and plugins.
...
your current working directory. If an TheClaw workspace is configured,
`clawhub` falls back to that workspace unless you override `--workdir`
(or `CLAWHUB_WORKDIR`). TheClaw loads workspace skills from
`<workspace>/skills` and picks them up in the **next** session.

If you already use `~/.theclaw/skills` or bundled skills, workspace
...
<AccordionGroup>
  ...
  <Accordion title="Moderation">
    ...
    - Interested in becoming a moderator? Ask in the TheClaw Discord and contact a moderator or maintainer.
...
<ParamField path="--workdir <dir>" type="string">
  Working directory. Default: current dir; falls back to TheClaw workspace.
</ParamField>
...
<AccordionGroup>
  ...
  <Accordion title="Browse / inspect plugins">
    ...
    `package explore` and `package inspect` are the ClawHub CLI surfaces for plugin/package discovery and metadata inspection. Native TheClaw installs still use `theclaw plugins install clawhub:<package>`.
...
Code plugins must include the required TheClaw metadata in
`package.json`:
...
<AccordionGroup>
  ...
  <Accordion title="Sync scanning and fallback roots">
    ...
    `~/theclaw/skills` and `~/.theclaw/skills`). This is designed to
