...
For session-backed agent runs, TheClaw starts music generation as a
...
<Note>
Not all providers support all parameters. TheClaw still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, TheClaw
...
- **Status lookup:** `theclaw tasks list` or `theclaw tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** TheClaw injects an internal completion event back
...
```bash
THECLAW_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
