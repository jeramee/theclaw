...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in TheClaw
...
Today this applies to PI embedded runs and TheClaw dynamic tools in the Codex
app-server harness. Tokenjuice hooks TheClaw's tool-result middleware and
...
```bash
theclaw config set plugins.entries.tokenjuice.enabled true
```

Equivalent:

```bash
theclaw plugins enable tokenjuice
```

TheClaw already ships the plugin. There is no separate `plugins install`
or `tokenjuice install theclaw` step.
...
```bash
theclaw config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
theclaw plugins disable tokenjuice
```
