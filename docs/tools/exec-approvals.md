...
host-local `ask: "always"` in `~/.theclaw/exec-approvals.json` keeps
...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `theclaw approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `theclaw exec-policy show`                                      | Local-machine merged view.                                                             |
| `theclaw exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `theclaw` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, TheClaw also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.theclaw/exec-approvals.sock",
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, TheClaw treats inline code-eval forms as approval-only
...
**both** policy layers — requested exec policy in TheClaw config
...
- In YOLO mode, TheClaw does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when TheClaw's requested exec
...
<Steps>
  ...
  <Step title="Match the host approvals file">
    ```bash
    theclaw approvals set --stdin <<'EOF'
    {
      version: 1,
      defaults: {
        security: "full",
...
```bash
theclaw exec-policy preset yolo
```

That local shortcut updates both:

- Local `tools.exec.host/security/ask`.
- Local `~/.theclaw/exec-approvals.json` defaults.

It is intentionally local-only. To change gateway-host or node-host
approvals remotely, use `theclaw approvals set --gateway` or
`theclaw approvals set --node <id|name|ip>`.
...
```bash
theclaw approvals set --node <id|name|ip> --stdin <<'EOF'
{
  version: 1,
  defaults: {
    security: "full",
...
- `theclaw exec-policy` does not synchronize node approvals.
- `theclaw exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `theclaw approvals --node ...`.
...
edit its local `~/.theclaw/exec-approvals.json` directly.

CLI: `theclaw approvals` supports gateway or node editing — see
...
When an async exec approval is denied, TheClaw prevents the agent from
