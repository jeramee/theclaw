...
TheClaw uses **[AgentSkills](https://agentskills.io)-compatible** skill
folders to teach the agent how to use tools. Each skill is a directory
containing a `SKILL.md` with YAML frontmatter and instructions. TheClaw
...
TheClaw loads skills from these sources, **highest precedence first**:
...
| 4   | Managed/local skills  | `~/.theclaw/skills`             |
...
| Shared managed/local | `~/.theclaw/skills`                        | All agents on that machine  |
...
`theclaw.plugin.json` (paths relative to the plugin root). Plugin skills
...
`metadata.theclaw.requires.config` on the plugin's config entry.
...
[ClawHub](https://clawhub.ai) is the public skills registry for TheClaw.
Use native `theclaw skills` commands for discover/install/update, or the
separate `clawhub` CLI for publish/sync workflows. Full guide:
[ClawHub](/tools/clawhub).

| Action                             | Command                                |
| ---------------------------------- | -------------------------------------- |
| Install a skill into the workspace | `theclaw skills install <skill-slug>` |
| Update all installed skills        | `theclaw skills update --all`         |
| Sync (scan + publish updates)      | `clawhub sync --all`                   |

Native `theclaw skills install` installs into the active workspace
...
configured TheClaw workspace). TheClaw picks that up as
...
- `theclaw skills install <slug>` is different — it downloads a ClawHub skill folder into the workspace and does not use the installer-metadata path above.
...
TheClaw follows the AgentSkills spec for layout/intent. The parser used
...
<ParamField path="homepage" type="string">
  URL surfaced as "Website" in the macOS Skills UI. Also supported via `metadata.theclaw.homepage`.
</ParamField>
...
TheClaw filters skills at load time using `metadata` (single-line JSON):
...
<ParamField path="requires.config" type="string[]">
  List of `theclaw.json` paths that must be truthy.
</ParamField>
...
If no `metadata.theclaw` is present, the skill is always eligible (unless
disabled in config or blocked by `skills.allowBundled` for bundled skills).

<Note>
Legacy `metadata.clawdbot` blocks are still accepted when
`metadata.theclaw` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, TheClaw lists each entry so you can see the available artifacts.
    - Installer specs can include `os: ["darwin"|"linux"|"win32"]` to filter options by platform.
    - Node installs honor `skills.install.nodeManager` in `theclaw.json` (default: npm; options: npm/pnpm/yarn/bun). This only affects skill installs; the Gateway runtime should still be Node — Bun is not recommended for WhatsApp/Telegram.
    - Gateway-backed installer selection is preference-driven: when install specs mix kinds, TheClaw prefers Homebrew when `skills.install.preferBrew` is enabled and `brew` exists, then `uv`, then the configured node manager, then other fallbacks like `go` or `download`.
    - If every install spec is `download`, TheClaw surfaces all download options instead of collapsing to one preferred installer.

  </Accordion>
  <Accordion title="Per-installer details">
    - **Go installs:** if `go` is missing and `brew` is available, the gateway installs Go via Homebrew first and sets `GOBIN` to Homebrew's `bin` when possible.
    - **Download installs:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`), `extract` (default: auto when archive detected), `stripComponents`, `targetDir` (default: `~/.theclaw/tools/<skillKey>`).
...
under `skills.entries` in `~/.theclaw/theclaw.json`:
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience for skills that declare `metadata.theclaw.primaryEnv`. Supports plaintext or SecretRef.
</ParamField>
...
defines `metadata.theclaw.skillKey`, use that key under `skills.entries`.

<Note>
For stock image generation/editing inside TheClaw, use the core
...
When an agent run starts, TheClaw:
...
For the bundled `claude-cli` backend, TheClaw also materializes the same
...
TheClaw still owns precedence, per-agent allowlists, gating, and
`skills.entries.*` env/API key injection. Other CLI backends use the
prompt catalog only.

## Snapshots and refresh

TheClaw snapshots the eligible skills **when a session starts** and
...
session, TheClaw refreshes the snapshot so visible skills stay aligned
with the current agent.

### Skills watcher

By default, TheClaw watches skill folders and bumps the skills snapshot
...
TheClaw can treat macOS-only skills as eligible when the required
...
probes, TheClaw clears its cached bin matches so agents no longer see
skills that cannot currently run there.

## Token impact

When skills are eligible, TheClaw injects a compact XML list of available
...
TheClaw ships a baseline set of skills as **bundled skills** with the
install (npm package or TheClaw.app). `~/.theclaw/skills` exists for
