...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.theclaw/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so TheClaw picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    theclaw agent --message "give me a greeting"
    ```
...
| `metadata.theclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.theclaw.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.theclaw.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `theclaw agent --message "..."` to test before sharing
...
| `~/.theclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with TheClaw) | Low        | Global                |
