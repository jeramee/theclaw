---
summary: "TheClaw tools and plugins overview: what the agent can do and how to extend it"
read_when:
  - You want to understand what tools TheClaw provides
...
<Steps>
  ...
  <Step title="Plugins package everything together">
    ...
    TheClaw), others are **external** (published on npm by the community).
...
These tools ship with TheClaw and are available without installing any plugins:
...
TheClaw fails closed when an explicit allowlist resolves to no callable tools.
...
| `group:theclaw`   | All built-in TheClaw tools (excludes plugin tools)                                                       |
