...
<Warning>
Auth is per-agent: each agent reads from its own `agentDir` auth store at `~/.theclaw/agents/<agentId>/agent/auth-profiles.json`. Credentials are **not** shared between agents. Never reuse `agentDir` across agents. If you want to share creds, copy `auth-profiles.json` into the other agent's `agentDir`.
</Warning>
...
<Note>
Legacy `agent.*` configs are migrated by `theclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
</Note>
...
<Steps>
  ...
  <Step title="Verify sandbox containers">
    ```bash
    docker ps --filter "name=theclaw-sbx-"
    ```
  </Step>
  ...
  <Step title="Monitor logs">
    ```bash
    tail -f "${THECLAW_STATE_DIR:-$HOME/.theclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
    ```
  </Step>
</Steps>
