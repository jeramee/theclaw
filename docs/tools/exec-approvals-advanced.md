...
- allowlist entries live in host-local `~/.theclaw/exec-approvals.json` under `agents.<id>.allowlist` (or via Control UI / `theclaw approvals allowlist ...`).
- `theclaw security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `theclaw doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, TheClaw still rejects the `env` builtin in safe-bin
...
After an approved async exec finishes, TheClaw sends a followup `agent` turn to the same session.
...
- Matrix native prompts include `com.theclaw.approval` custom event content on the first prompt
  event so TheClaw-aware Matrix clients can read structured approval state while stock clients
...
topics, TheClaw preserves the topic for the approval prompt and the post-approval follow-up.
