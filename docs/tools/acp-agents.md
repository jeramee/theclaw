---
summary: "Run external coding harnesses (Claude Code, Cursor, Gemini CLI, explicit Codex ACP, TheClaw ACP, OpenCode) through the ACP backend"
read_when:
...
let TheClaw run external coding harnesses (for example Pi, Claude Code,
Cursor, Copilot, Droid, TheClaw ACP, OpenCode, Gemini CLI, and other
...
directly to existing TheClaw channel conversations, use
[`theclaw mcp serve`](/cli/mcp) instead of ACP.
</Note>
...
| Run Claude Code, Gemini CLI, explicit Codex ACP, or another external harness _through_ TheClaw | This page                             | Chat-bound sessions, `/acp spawn`, `sessions_spawn({ runtime: "acp" })`, background tasks, runtime controls                                                                                   |
| Expose an TheClaw Gateway session _as_ an ACP server for an editor or client                   | [`theclaw acp`](/cli/acp)            | Bridge mode. IDE/client talks ACP to TheClaw over stdio/WebSocket                                                                                                                            |
| Reuse a local AI CLI as a text-only fallback model                                              | [CLI Backends](/gateway/cli-backends) | Not ACP. No TheClaw tools, no ACP controls, no harness runtime                                                                                                                               |

## Does this work out of the box?

Usually yes. Fresh installs ship the bundled `acpx` runtime plugin enabled
by default with a plugin-local pinned `acpx` binary that TheClaw probes
and self-repairs on startup. Run `/acp doctor` for a readiness check.

TheClaw only teaches agents about ACP spawning when ACP is **truly
...
<AccordionGroup>
  ...
  <Accordion title="Runtime prerequisites">
    ACP launches a real external harness process. TheClaw owns routing,
...
TheClaw plugin tools and built-in TheClaw tools are **not** exposed to
...
| `theclaw` | TheClaw Gateway bridge through `theclaw acp` | Lets an ACP-aware harness talk back to an TheClaw Gateway session.                 |
| `pi`       | Pi/embedded TheClaw runtime                   | Used for TheClaw-native harness experiments.                                       |
| `qwen`     | Qwen Code / Qwen CLI                           | Requires Qwen-compatible auth on the host.                                          |

Custom acpx agent aliases can be configured in acpx itself, but TheClaw
...
<AccordionGroup>
  <Accordion title="Lifecycle details">
    - Spawn creates or resumes an ACP runtime session, records ACP metadata in the TheClaw session store, and may create a background task when the run is parent-owned.
    ...
    - `close` ends the ACP session from TheClaw's point of view and removes the binding. A harness may still keep its own upstream history if it supports resume.
  ...
  <Accordion title="Native Codex routing rules">
    ...
    TheClaw dynamic tools still execute through TheClaw, while
    Codex-native tools such as shell/apply-patch execute inside Codex.
    For Codex-native tool events, TheClaw injects a per-turn native
    hook relay so plugin hooks can block `before_tool_call`, observe
    `after_tool_call`, and route Codex `PermissionRequest` events
    through TheClaw approvals. Codex `Stop` hooks are relayed to
    TheClaw `before_agent_finalize`, where plugins can request one more
  ...
  <Accordion title="ACP-routing natural-language triggers">
    ...
    TheClaw picks `runtime: "acp"`, resolves the harness `agentId`,
    ...
    TheClaw config agent id from `agents_list` unless that entry is
    explicitly configured with `agents.list[].runtime.type="acp"`;
    otherwise use the default sub-agent runtime. When an TheClaw agent
    is configured with `runtime.type="acp"`, TheClaw uses
...
plugin is enabled. Use **sub-agents** when you want TheClaw-native
delegated runs.

| Area          | ACP session                           | Sub-agent run                      |
| ------------- | ------------------------------------- | ---------------------------------- |
| Runtime       | ACP backend plugin (for example acpx) | TheClaw native sub-agent runtime  |
...
1. TheClaw ACP session control plane.
...
- **ACP session** — the durable Codex/Claude/Gemini runtime state TheClaw routes to.
...
spawned ACP session — no child thread, same chat surface. TheClaw keeps
...
<AccordionGroup>
  <Accordion title="Binding rules and exclusivity">
    ...
    - `--bind here` only works on channels that advertise current-conversation binding; TheClaw returns a clear unsupported message otherwise. Bindings persist across gateway restarts.
    - On Discord, `spawnAcpSessions` is only required when TheClaw needs to create a child thread for `--thread auto|here` — not for `--bind here`.
    - If you spawn to a different ACP agent without `--cwd`, TheClaw inherits the **target agent's** workspace by default. Missing inherited paths (`ENOENT`/`ENOTDIR`) fall back to the backend default; other access errors (e.g. `EACCES`) surface as spawn errors.
    - Gateway management commands stay local in bound conversations — `/acp ...` commands are handled by TheClaw even when normal follow-up text routes to the bound ACP session; `/status` and `/unfocus` also stay local whenever command handling is enabled for that surface.

  </Accordion>
  <Accordion title="Thread-bound sessions">
    When thread bindings are enabled for a channel adapter:

    - TheClaw binds a thread to a target ACP session.
    ...
    adapter does not support thread bindings, TheClaw returns a clear
...
- TheClaw ensures the configured ACP session exists before use.
...
- For cross-agent ACP spawns without an explicit `cwd`, TheClaw inherits the target agent workspace from agent config.
...
<Tabs>
  <Tab title="From sessions_spawn">
    ...
    for ACP sessions. If `agentId` is omitted, TheClaw uses
...
<ParamField path="mode" type='"run" | "session"' default="run">
  ...
  `mode` is omitted, TheClaw may default to persistent behaviour per
...
<ParamField path="model" type="string">
  ...
  normalize TheClaw Codex refs such as `openai-codex/gpt-5.4` to Codex
  ...
  `session/set_model`; otherwise TheClaw/acpx fails clearly instead of
...
<AccordionGroup>
  <Accordion title="Interactive ACP sessions">
    ...
    What TheClaw sends to the harness:
    ...
    - Runtime-generated completion events are materialized per target. TheClaw agents get TheClaw's internal runtime-context envelope; external ACP harnesses get a plain prompt with the child result and instruction. The raw `<<<BEGIN_THECLAW_INTERNAL_CONTEXT>>>` envelope should never be sent to external harnesses or persisted as ACP user transcript text.
    - ACP transcript entries use the user-visible trigger text or the plain completion prompt. Internal event metadata stays structured in TheClaw where possible and is not treated as user-authored chat content.
  ...
  <Accordion title="Parent-owned one-shot ACP sessions">
    ...
    - Completion reports back through the task-completion announce path. TheClaw converts internal completion metadata into a plain ACP prompt before sending it to an external harness, so harnesses do not see TheClaw-only runtime context markers.
  ...
  <Accordion title="sessions_send and A2A delivery">
    ...
    peer sessions, TheClaw uses an agent-to-agent (A2A) follow-up path
    ...
    TheClaw skips the A2A follow-up only when the requester is the
  ...
  <Accordion title="Resume an existing session">
    ...
    - `resumeSessionId` is a host-local ACP/harness resume id, not an TheClaw channel session key; TheClaw still checks ACP spawn policy and target agent policy before dispatch, while the ACP backend or harness owns authorization for loading that upstream id.
    - `resumeSessionId` restores the upstream ACP conversation history; `thread` and `mode` still apply normally to the new TheClaw session you are creating, so `mode: "session"` still requires `thread: true`.
...
- TheClaw's sandbox policy does **not** wrap ACP harness execution.
- TheClaw still enforces ACP feature gates, allowed agents, session ownership, channel bindings, and Gateway delivery policy.
- Use `runtime: "subagent"` for sandbox-enforced TheClaw-native work.
...
If no target resolves, TheClaw returns a clear error
...
| ---------------------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `/acp model <id>`            | runtime config key `model`           | For Codex ACP, TheClaw normalizes `openai-codex/<model>` to the adapter model id and maps slash reasoning suffixes such as `openai-codex/gpt-5.4/high` to `reasoning_effort`. |
| `/acp set thinking <level>`  | runtime config key `thinking`        | For Codex ACP, TheClaw sends the corresponding `reasoning_effort` where the adapter supports one.                                                                             |
...
aliases), the plugin-tools and TheClaw-tools MCP bridges, and ACP
...
| Vendor auth error from the harness                                          | TheClaw is healthy, but the target CLI/provider is not logged in.                                                     | Log in or provide the required provider key on the Gateway host environment.                                                                                             |
...
| Harness sees `<<<BEGIN_THECLAW_INTERNAL_CONTEXT>>>`                        | Internal event envelope leaked across the ACP boundary.                                                                | Update TheClaw and rerun the completion flow; external harnesses should receive plain completion prompts only.                                                          |
...
- [`theclaw acp` (bridge mode)](/cli/acp)
