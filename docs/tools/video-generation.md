...
TheClaw agents can generate videos from text prompts, reference images, or
...
TheClaw treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    theclaw config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
...
1. TheClaw submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, TheClaw wakes the same session with an internal completion event.
...
generation. Use `theclaw tasks list` or `theclaw tasks show <taskId>` to
...
Generated video files are saved under TheClaw-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, TheClaw
...
```bash
theclaw tasks list
theclaw tasks show <taskId>
theclaw tasks cancel <taskId>
```
...
<Note>
Not all providers support all parameters. TheClaw normalizes duration to
...
TheClaw resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@theclaw/byteplus-modelark`](https://www.npmjs.com/package/@theclaw/byteplus-modelark)
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. TheClaw submits the
...
```bash
THECLAW_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- Per-provider operation cap from
  `THECLAW_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `THECLAW_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your TheClaw config:
...
```bash
theclaw config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
