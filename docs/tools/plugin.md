---
summary: "Install, configure, and manage TheClaw plugins"
read_when:
...
Plugins extend TheClaw with new capabilities: channels, model providers,
...
search, and more. Some plugins are **core** (shipped with TheClaw), others
...
<Steps>
  ...
  <Step title="Install a plugin">
    ```bash
    # From npm
    theclaw plugins install @theclaw/voice-call

    # From a local directory or archive
    theclaw plugins install ./my-plugin
    theclaw plugins install ./my-plugin.tgz
    ```
...
```text
/plugin install clawhub:@theclaw/voice-call
...
`theclaw doctor --fix`. The only recovery exception is a narrow bundled-plugin
reinstall path for plugins that opt into
`theclaw.install.allowInvalidConfigRecovery`.
...
load, and keeps other plugins and channels online. Run `theclaw doctor --fix`
...
Run `theclaw doctor --fix` to remove the stale channel/plugin entries; unknown
...
Gateway startup skips plugin discovery/load work and `theclaw doctor` preserves
...
Packaged TheClaw installs do not eagerly install every bundled plugin's
runtime dependency tree. When a bundled TheClaw-owned plugin is active from
...
`theclaw plugins install`.

## Plugin types

TheClaw recognizes two plugin formats:

| Format     | How it works                                                       | Examples                                               |
| ---------- | ------------------------------------------------------------------ | ------------------------------------------------------ |
| **Native** | `theclaw.plugin.json` + runtime module; executes in-process       | Official plugins, community npm packages               |
| **Bundle** | Codex/Claude/Cursor-compatible layout; mapped to TheClaw features | `.codex-plugin/`, `.claude-plugin/`, `.cursor-plugin/` |

Both show up under `theclaw plugins list`. See [Plugin Bundles](/plugins/bundles) for bundle details.
...
Native plugin npm packages must declare `theclaw.extensions` in `package.json`.
...
Use `theclaw.runtimeExtensions` when published runtime files do not live at the
...
| Plugin          | Package                | Docs                                 |
| --------------- | ---------------------- | ------------------------------------ |
| Matrix          | `@theclaw/matrix`     | [Matrix](/channels/matrix)           |
| Microsoft Teams | `@theclaw/msteams`    | [Microsoft Teams](/channels/msteams) |
| Nostr           | `@theclaw/nostr`      | [Nostr](/channels/nostr)             |
| Voice Call      | `@theclaw/voice-call` | [Voice Call](/plugins/voice-call)    |
| Zalo            | `@theclaw/zalo`       | [Zalo](/channels/zalo)               |
| Zalo Personal   | `@theclaw/zalouser`   | [Zalo Personal](/plugins/zalouser)   |

### Core (shipped with TheClaw)
...
<AccordionGroup>
  ...
  <Accordion title="Other">
    - `browser` — bundled browser plugin for the browser tool, `theclaw browser` CLI, `browser.request` gateway method, browser runtime, and default browser control service (enabled by default; disable before replacing it)
...
watch + in-process restart enabled (the default `theclaw gateway` path), that
...
`theclaw plugins list` is a local plugin registry/config snapshot. An
...
wrapper processes, send restarts to the actual `theclaw gateway run` process,
or use `theclaw gateway restart` against the running Gateway.

<Accordion title="Plugin states: disabled vs missing vs invalid">
  - **Disabled**: plugin exists but enablement rules turned it off. Config is preserved.
  - **Missing**: config references a plugin id that discovery did not find.
  - **Invalid**: plugin exists but its config does not match the declared schema. Gateway startup skips only that plugin; `theclaw doctor --fix` can quarantine the invalid entry by disabling it and removing its config payload.

</Accordion>

## Discovery and precedence

TheClaw scans for plugins in this order (first match wins):

<Steps>
  <Step title="Config paths">
    `plugins.load.paths` — explicit file or directory paths. Paths that point
    back at TheClaw's own packaged bundled plugin directories are ignored;
    run `theclaw doctor --fix` to remove those stale aliases.
  </Step>

  <Step title="Workspace plugins">
    `\<workspace\>/.theclaw/<plugin-root>/*.ts` and `\<workspace\>/.theclaw/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Global plugins">
    `~/.theclaw/<plugin-root>/*.ts` and `~/.theclaw/<plugin-root>/*/index.ts`.
  </Step>

  <Step title="Bundled plugins">
    Shipped with TheClaw. Many are enabled by default (model providers, speech).
...
`/app/extensions/synology-chat`, TheClaw treats that mounted source directory
...
Set `THECLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS=1` to force packaged dist bundles
...
- Run `theclaw gateway status --deep --require-rpc` and confirm the active
...
- Restart the live Gateway after plugin install/config/code changes. In wrapper
  ...
  `theclaw gateway run` process.
- Use `theclaw plugins inspect <id> --json` to confirm hook registrations and
...
- For proof of the effective session model, use `theclaw sessions` or the
...
- Run `theclaw plugins list --enabled --verbose` to see every enabled plugin
  and origin.
- Run `theclaw plugins inspect <id> --json` for each suspected plugin and
  compare `channels`, `channelConfigs`, `tools`, and diagnostics.
- Run `theclaw plugins registry --refresh` after installing or removing
...
- If the duplicate is accidental, disable one side with
  ...
  install.
- If you explicitly enabled both plugins, TheClaw keeps that request and
...
```bash
theclaw plugins list                       # compact inventory
theclaw plugins list --enabled            # only enabled plugins
theclaw plugins list --verbose            # per-plugin detail lines
theclaw plugins list --json               # machine-readable inventory
theclaw plugins inspect <id>              # deep detail
theclaw plugins inspect <id> --json       # machine-readable
theclaw plugins inspect --all             # fleet-wide table
theclaw plugins info <id>                 # inspect alias
theclaw plugins doctor                    # diagnostics
theclaw plugins registry                  # inspect persisted registry state
theclaw plugins registry --refresh        # rebuild persisted registry
theclaw doctor --fix                      # repair plugin registry state

theclaw plugins install <package>         # install (ClawHub first, then npm)
theclaw plugins install clawhub:<pkg>     # install from ClawHub only
theclaw plugins install npm:<pkg>         # install from npm only
theclaw plugins install <spec> --force    # overwrite existing install
theclaw plugins install <path>            # install from local path
theclaw plugins install -l <path>         # link (no copy) for dev
theclaw plugins install <plugin> --marketplace <source>
theclaw plugins install <plugin> --marketplace https://github.com/<owner>/<repo>
theclaw plugins install <spec> --pin      # record exact resolved npm spec
theclaw plugins install <spec> --dangerously-force-unsafe-install
theclaw plugins update <id-or-npm-spec> # update one plugin
theclaw plugins update <id-or-npm-spec> --dangerously-force-unsafe-install
theclaw plugins update --all            # update all
theclaw plugins uninstall <id>          # remove config and plugin index records
...
Bundled plugins ship with TheClaw. Many are enabled by default (for example
bundled model providers, bundled speech providers, and the bundled browser
plugin). Other bundled plugins still need `theclaw plugins enable <id>`.

`--force` overwrites an existing installed plugin or hook pack in place. Use
`theclaw plugins update <id-or-npm-spec>` for routine upgrades of tracked npm
plugins. It is not supported with `--link`, which reuses the source path instead
of copying over a managed install target.

When `plugins.allow` is already set, `theclaw plugins install` adds the
...
TheClaw keeps a persisted local plugin registry as the cold read model for
...
the registry is missing, stale, or invalid, `theclaw plugins registry
...
`theclaw plugins update <id-or-npm-spec>` applies to tracked installs. Passing
...
the resolved version and recorded artifact identity, TheClaw skips the update
...
override instead, while `theclaw skills install` remains the separate ClawHub
...
`theclaw plugins inspect <id>` also reports detected bundle capabilities plus
...
See [`theclaw plugins` CLI reference](/cli/plugins) for full details.
...
TheClaw loads the entry object and calls `register(api)` during plugin
...
TheClaw may evaluate the trusted plugin entry or channel plugin module to build
