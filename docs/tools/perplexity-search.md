...
TheClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, TheClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `theclaw configure --section web`. It stores the key in
`~/.theclaw/theclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.theclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
