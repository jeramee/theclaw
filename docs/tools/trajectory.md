---
summary: "Export redacted trajectory bundles for debugging an TheClaw agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an TheClaw session
...
Trajectory capture is TheClaw's per-session flight recorder. It records a
...
TheClaw writes the bundle under the workspace:

```text
.theclaw/trajectory-exports/theclaw-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.theclaw/trajectory-exports/`. Absolute
...
Trajectory capture is on by default for TheClaw agent runs.
...
```json
{
  "traceSchema": "theclaw-trajectory",
  "schemaVersion": 1
}
```
...
| `metadata.json`       | TheClaw version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
TheClaw also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `THECLAW_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export THECLAW_TRAJECTORY_DIR=/var/lib/theclaw/trajectories
```

When this variable is set, TheClaw writes one JSONL file per session id in that
directory.
...
Set `THECLAW_TRAJECTORY=0` before starting TheClaw:

```bash
export THECLAW_TRAJECTORY=0
```
...
TheClaw redacts sensitive values before writing export files:
...
- confirm TheClaw was started without `THECLAW_TRAJECTORY=0`
- check whether `THECLAW_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.theclaw/trajectory-exports/`
