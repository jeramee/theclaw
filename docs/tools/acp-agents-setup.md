...
read_when:
  ...
  - Enabling the plugin-tools or TheClaw-tools MCP bridge
...
When TheClaw uses the acpx backend, prefer these values for `agentId` unless your acpx config defines custom agent aliases.
...
Direct acpx CLI usage can also target arbitrary adapters via `--agent <command>`, but that raw escape hatch is an acpx CLI feature (not the normal TheClaw `agentId` path).

Model control is adapter-capability dependent. Codex ACP model refs are
normalized by TheClaw before startup. Other harnesses need ACP `models` plus
`session/set_model` support; if a harness exposes neither that ACP capability
nor its own startup model flag, TheClaw/acpx cannot force a model selection.
...
```bash
theclaw plugins install acpx
theclaw config set plugins.entries.acpx.enabled true
```

Local workspace install during development:

```bash
theclaw plugins install ./path/to/local/acpx-plugin
```
...
live probe. Set `THECLAW_ACPX_RUNTIME_STARTUP_PROBE=1` only when you need the
...
- `command` accepts an absolute path, relative path (resolved from the TheClaw workspace), or command name.
...
When you install TheClaw globally with `npm install -g theclaw`, the acpx
...
normally and reports the missing dependency through `theclaw acp doctor`.

### Plugin tools MCP bridge

By default, ACPX sessions do **not** expose TheClaw plugin-registered tools to
the ACP harness.

If you want ACP agents such as Codex or Claude Code to call installed
TheClaw plugin tools such as memory recall/store, enable the dedicated bridge:

```bash
theclaw config set plugins.entries.acpx.config.pluginToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `theclaw-plugin-tools` into ACPX session
  bootstrap.
- Exposes plugin tools already registered by installed and enabled TheClaw
  plugins.
...
### TheClaw tools MCP bridge

By default, ACPX sessions also do **not** expose built-in TheClaw tools through
MCP. Enable the separate core-tools bridge when an ACP agent needs selected
built-in tools such as `cron`:

```bash
theclaw config set plugins.entries.acpx.config.openClawToolsMcpBridge true
```

What this does:

- Injects a built-in MCP server named `theclaw-tools` into ACPX session
  bootstrap.
- Exposes selected built-in TheClaw tools. The initial server exposes `cron`.
...
```bash
theclaw config set plugins.entries.acpx.config.timeoutSeconds 180
```
...
```bash
theclaw config set plugins.entries.acpx.config.probeAgent claude
```
...
These ACPX harness permissions are separate from TheClaw exec approvals and separate from CLI-backend vendor bypass flags such as Claude CLI `--permission-mode bypassPermissions`. ACPX `approve-all` is the harness-level break-glass switch for ACP sessions.
...
```bash
theclaw config set plugins.entries.acpx.config.permissionMode approve-all
theclaw config set plugins.entries.acpx.config.nonInteractivePermissions fail
```

Restart the gateway after changing these values.

<Warning>
TheClaw defaults to `permissionMode=approve-reads` and `nonInteractivePermissions=fail`. In non-interactive ACP sessions, any write or exec that triggers a permission prompt can fail with `AcpRuntimeError: Permission prompt unavailable in non-interactive mode`.
