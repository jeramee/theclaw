...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ...
    `openai-codex` OAuth profile is configured, TheClaw routes image
...
nearby geometry option instead of the exact requested one, TheClaw remaps to
...
TheClaw tries providers in this order:
...
<AccordionGroup>
  ...
  <Accordion title="Auto-detection is auth-aware">
    A provider default only enters the candidate list when TheClaw can
...
<AccordionGroup>
  <Accordion title="OpenAI gpt-image-2 (and gpt-image-1.5)">
    ...
    `openai-codex` OAuth profile is configured, TheClaw reuses the same
    ...
    `https://chatgpt.com/backend-api/codex` for image requests. TheClaw
    ...
    TheClaw forwards `prompt`, `count`, `size`, `quality`, `outputFormat`,
    and reference images to OpenAI. OpenAI does **not** receive
    `aspectRatio` or `resolution` directly; when possible TheClaw maps
    ...
    transparency-capable OpenAI image model. TheClaw routes default
  ...
  <Accordion title="OpenRouter image models">
    ...
    TheClaw forwards `prompt`, `count`, reference images, and
  ...
  <Accordion title="xAI grok-imagine-image">
    ...
    - Resolutions: `1K`, `2K`
    - Outputs: returned as TheClaw-managed image attachments

    TheClaw intentionally does not expose xAI-native `quality`, `mask`,
...
```text
/tool image_generate action=generate model=openai/gpt-image-2 prompt="A clean editorial poster for TheClaw image generation" size=3840x2160 count=1
```
  </Tab>
...
`theclaw infer image edit`; `--openai-background` remains as an
