...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/theclaw`)
...
If the page fails to load, start the Gateway first: `theclaw gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `theclaw devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. TheClaw keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `theclaw devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.theclaw/theclaw.json` (`config.get`, `config.set`).
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop theclaw`, `please stop`) to abort out-of-band.
...
| `push/vapid-keys.json` (under the TheClaw state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `THECLAW_VAPID_PUBLIC_KEY`
- `THECLAW_VAPID_PRIVATE_KEY`
- `THECLAW_VAPID_SUBJECT` (defaults to `mailto:theclaw@localhost`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    theclaw gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, TheClaw **blocks** Control UI connections without device identity.
...
```bash
THECLAW_CONTROL_UI_BASE_PATH=/theclaw/ pnpm ui:build
```
