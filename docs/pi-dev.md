...
A sane workflow for working on the Pi integration in TheClaw.
...
```bash
THECLAW_LIVE_TEST=1 pnpm test src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm theclaw agent --message "Hello" --thinking low`
...
State lives under the TheClaw state directory. Default is `~/.theclaw`. If `THECLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `theclaw.json` for config
