...
manual `CI` workflow with that target, and dispatches `TheClaw Release Checks`
...
For recovery, `Full Release Validation` and `TheClaw Release Checks` both
...
`TheClaw Release Checks` uses the trusted workflow ref to resolve the selected
...
Use `Package Acceptance` when the question is "does this installable TheClaw
...
1. `resolve_package` checks out `workflow_ref`, resolves one package candidate,
   writes `.artifacts/docker-e2e-package/theclaw-current.tgz`, writes
...
2. `docker_acceptance` calls
   `theclaw-live-and-e2e-checks-reusable.yml` with `ref=workflow_ref` and
...
- `source=npm`: accepts only `theclaw@beta`, `theclaw@latest`, or an exact
  TheClaw release version such as `theclaw@2026.4.27-beta.2`. Use this for
  published beta/stable acceptance.
- `source=ref`: packs a trusted `package_ref` branch, tag, or full commit SHA.
  The resolver fetches TheClaw branches/tags, verifies the selected commit is
  reachable from repository branch history or a release tag, installs deps in a
  detached worktree, and packs it with `scripts/package-theclaw-for-docker.mjs`.
...
gh workflow run package-acceptance.yml \
  ...
  -f workflow_ref=main \
  -f source=url \
  -f package_url=https://example.com/theclaw-current.tgz \
...
`e2ee-smoke`, `e2ee-deep`, and `e2ee-cli` jobs. `TheClaw Release Checks` also
...
push. Use `THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the change
...
`THECLAW_TESTBOX_ALLOW_MASS_DELETIONS=1` for that sanity run.
...
| `blacksmith-6vcpu-macos-latest`  | `macos-node` on `theclaw/theclaw`; forks fall back to `macos-latest`                                                                                                                                                                                                                                                                                                                                                                                                  |
| `blacksmith-12vcpu-macos-latest` | `macos-swift` on `theclaw/theclaw`; forks fall back to `macos-latest`                                                                                                                                                                                                                                                                                                                                                                                                 |
