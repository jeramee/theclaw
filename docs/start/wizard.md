...
CLI onboarding is the **recommended** way to set up TheClaw on macOS,
...
`theclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`theclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.theclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `theclaw doctor` first.
</Note>
...
- CLI command reference: [`theclaw onboard`](/cli/onboard)
