...
On the first agent run, TheClaw bootstraps the workspace (default
`~/.theclaw/workspace`):
...
For embedded/local model runs, TheClaw keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `theclaw onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.theclaw/workspace`).
</Note>
