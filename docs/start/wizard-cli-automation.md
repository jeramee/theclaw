---
summary: "Scripted onboarding and agent setup for the TheClaw CLI"
read_when:
...
Use `--non-interactive` to automate `theclaw onboard`.
...
<AccordionGroup>
  ...
  <Accordion title="Custom provider example">
    ...
    TheClaw marks common vision model IDs as image-capable automatically. Add `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
Anthropic setup-token remains available as a supported onboarding token path, but TheClaw now prefers Claude CLI reuse when available.
For production, prefer an Anthropic API key.

## Add another agent

Use `theclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.theclaw/workspace-<agentId>`.
