---
summary: "Get TheClaw installed and run your first chat in minutes."
read_when:
...
Install TheClaw, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.theclaw/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.theclaw/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run TheClaw as a service account or want custom paths:

- `THECLAW_HOME` — home directory for internal path resolution
- `THECLAW_STATE_DIR` — override the state directory
- `THECLAW_CONFIG_PATH` — override the config file path
