...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in TheClaw
...
Running TheClaw via Node with `tsx` fails at startup with:

```
[theclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `theclaw status` and `gateway:watch` worked.
