---
summary: "Deploy TheClaw on Render with Infrastructure-as-Code"
read_when:
  - Deploying TheClaw to Render
...
Deploy TheClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/theclaw/theclaw)
...
repository configures everything needed to run TheClaw:
...
services:
  - type: web
    ...
    envVars:
      - key: THECLAW_GATEWAY_PORT
        value: "8080"
      - key: THECLAW_STATE_DIR
        value: /data/.theclaw
      - key: THECLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: THECLAW_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means TheClaw state
...
`THECLAW_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original TheClaw repository, Render will not auto-deploy your TheClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For TheClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
- Missing `THECLAW_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `THECLAW_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `theclaw backup create` in the Render shell.
