---
summary: "Install TheClaw — installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs TheClaw, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://theclaw.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want TheClaw and Node kept under a local prefix such as
`~/.theclaw`, without depending on a system-wide Node install:

```bash
curl -fsSL https://theclaw.ai/install-cli.sh | bash
```
...
`theclaw update --channel dev` and `theclaw update --channel stable`. See
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=1 npm install -g theclaw@latest
```
...
Or skip the link and use `pnpm theclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.

### Install from GitHub main

```bash
npm install -g github:theclaw/theclaw#main
```
...
```bash
theclaw --version      # confirm the CLI is available
theclaw doctor         # check for config issues
theclaw gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `theclaw onboard --install-daemon` or `theclaw gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy TheClaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove TheClaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `theclaw` not found

If the install succeeded but `theclaw` is not found in your terminal:
