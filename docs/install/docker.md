---
summary: "Optional Docker-based setup and onboarding for TheClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run TheClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export THECLAW_IMAGE="ghcr.io/theclaw/theclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/theclaw/theclaw/pkgs/container/theclaw).
  ...
  <Step title="Complete onboarding">
    ...
    `theclaw-gateway` directly. `theclaw-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm theclaw-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm theclaw-cli channels login

    # Telegram
    docker compose run --rm theclaw-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm theclaw-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t theclaw:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node theclaw-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node theclaw-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d theclaw-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `THECLAW_EXTRA_MOUNTS`
or `THECLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
include it with `-f docker-compose.yml -f docker-compose.extra.yml`.
</Note>

<Note>
Because `theclaw-cli` shares `theclaw-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d theclaw-gateway`, run onboarding
and setup-time config writes through `theclaw-gateway` with
...
| Variable                                   | Purpose                                                         |
| ------------------------------------------ | --------------------------------------------------------------- |
| `THECLAW_IMAGE`                           | Use a remote image instead of building locally                  |
| `THECLAW_DOCKER_APT_PACKAGES`             | Install extra apt packages during build (space-separated)       |
| `THECLAW_EXTENSIONS`                      | Pre-install plugin deps at build time (space-separated names)   |
| `THECLAW_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`) |
| `THECLAW_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                   |
| `THECLAW_PLUGIN_STAGE_DIR`                | Container path for generated bundled plugin deps and mirrors    |
| `THECLAW_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)          |
| `THECLAW_DOCKER_SOCKET`                   | Override Docker socket path                                     |
| `THECLAW_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)   |
| `THECLAW_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays               |
...
| `THECLAW_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded  |
...
`THECLAW_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export THECLAW_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="theclaw-gateway"
./scripts/docker/setup.sh
```

The official TheClaw Docker release image includes the bundled
...
```bash
docker compose exec theclaw-gateway node dist/index.js health --token "$THECLAW_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `THECLAW_GATEWAY_BIND=lan` so host access to
...
When TheClaw runs in Docker, `127.0.0.1` inside the container is the container
...
`THECLAW_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `THECLAW_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `THECLAW_CONFIG_DIR` to `/home/node/.theclaw` and
`THECLAW_WORKSPACE_DIR` to `/home/node/.theclaw/workspace`, so those paths
survive container replacement.

That mounted config directory is where TheClaw keeps:

- `theclaw.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `THECLAW_GATEWAY_TOKEN`
...
`theclaw-plugin-runtime-deps` mounted at
`/var/lib/theclaw/plugin-runtime-deps`. Keeping that high-churn tree out of the
...
The default Compose file sets `THECLAW_PLUGIN_STAGE_DIR` to that path for both
`theclaw-gateway` and `theclaw-cli`, so `theclaw doctor --fix`, channel
...
the `theclaw-plugin-runtime-deps` Docker volume, and rolling file logs under
`/tmp/theclaw/`.
...
<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ...
    ```bash
    export THECLAW_SANDBOX=1
    export THECLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm theclaw-cli gateway probe
    docker compose run -T --rm theclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `theclaw-cli` uses `network_mode: "service:theclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `theclaw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.theclaw`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/theclaw-config /path/to/theclaw-workspace
    ```
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export THECLAW_HOME_VOLUME="theclaw_home"`
    2. **Bake system deps**: `export THECLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `THECLAW_HOME_VOLUME` or `THECLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/theclaw/theclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    TheClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm theclaw-cli dashboard --no-open
    docker compose run --rm theclaw-cli devices list
    docker compose run --rm theclaw-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm theclaw-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm theclaw-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [Updating](/install/updating) — keeping TheClaw up to date
