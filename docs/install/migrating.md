...
read_when:
  - You are moving TheClaw to a new laptop or server
  - You are coming from another agent system and want to keep state
  - You are upgrading an in-place plugin
title: "Migration guide"
---

TheClaw supports three migration paths: importing from another agent system, moving an existing install to a new machine, and upgrading a plugin in place.

## Import from another agent system

Use the bundled migration providers to bring instructions, MCP servers, skills, model config, and (opt-in) API keys into TheClaw. Plans are previewed before any change, secrets are redacted in reports, and apply is backed by a verified backup.
...
The CLI entry point is [`theclaw migrate`](/cli/migrate). Onboarding can also offer migration when it detects a known source (`theclaw onboard --flow import`).

## Move TheClaw to a new machine

Copy the **state directory** (`~/.theclaw/` by default) and your **workspace** to preserve:

- **Config** — `theclaw.json` and all gateway settings.
...
<Tip>
Run `theclaw status` on the old machine to confirm your state directory path. Custom profiles use `~/.theclaw-<profile>/` or a path set via `THECLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    cd ~
    tar -czf theclaw-state.tgz .theclaw
    ```

    If you use multiple profiles (for example `~/.theclaw-work`), archive each separately.

  </Step>

  <Step title="Install TheClaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine. It is fine if onboarding creates a fresh `~/.theclaw/`. You will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `THECLAW_STATE_DIR` and the new one does not, channels will appear logged out and sessions will be empty. Launch the gateway with the **same** profile or state-dir you migrated, then rerun `theclaw doctor`.
  </Accordion>

  <Accordion title="Copying only theclaw.json">
...
- [ ] `theclaw status` shows the gateway running.
...
- [`theclaw migrate`](/cli/migrate): CLI reference for cross-system imports.
- [Install overview](/install): all installation methods.
- [Doctor](/gateway/doctor): post-migration health check.
- [Uninstall](/install/uninstall): removing TheClaw cleanly.
