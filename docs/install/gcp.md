---
summary: "Run TheClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want TheClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# TheClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent TheClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "TheClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the TheClaw Gateway in Docker
- Persist `~/.theclaw` + `~/.theclaw/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.theclaw` state includes `theclaw.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-theclaw-project --name="TheClaw Gateway"
    gcloud config set project my-theclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create theclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh theclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the TheClaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    THECLAW_IMAGE=theclaw:latest
    THECLAW_GATEWAY_TOKEN=
    THECLAW_GATEWAY_BIND=lan
    THECLAW_GATEWAY_PORT=18789

    THECLAW_CONFIG_DIR=/home/$USER/.theclaw
    THECLAW_WORKSPACE_DIR=/home/$USER/.theclaw/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.theclaw
    ```

    Leave `THECLAW_GATEWAY_TOKEN` blank unless you explicitly want to
    manage it through `.env`; TheClaw writes a random gateway token to
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `THECLAW_GATEWAY_TOKEN`.
    ...
    services:
      theclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - THECLAW_GATEWAY_BIND=${THECLAW_GATEWAY_BIND}
          - THECLAW_GATEWAY_PORT=${THECLAW_GATEWAY_PORT}
          - THECLAW_GATEWAY_TOKEN=${THECLAW_GATEWAY_TOKEN}
        ...
        ports:
          ...
          - "127.0.0.1:${THECLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`THECLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm theclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh theclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop theclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type theclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start theclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create theclaw-deploy \
     --display-name="TheClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-theclaw-project \
     --member="serviceAccount:theclaw-deploy@my-theclaw-project.iam.gserviceaccount.com" \
