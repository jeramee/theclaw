---
summary: "Host TheClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up TheClaw on Oracle Cloud
  - Looking for free VPS hosting for TheClaw
  - Want 24/7 TheClaw on a small server
title: "Oracle Cloud"
---

Run a persistent TheClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@theclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=theclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `theclaw doctor --non-interactive` and check logs with `journalctl --user -u theclaw-gateway.service -n 50`.
...
- [Updating](/install/updating) -- keep TheClaw up to date
