---
summary: "Deploy TheClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run TheClaw on a Kubernetes cluster
  - You want to test TheClaw in a Kubernetes environment
title: "Kubernetes"
---

# TheClaw on Kubernetes

A minimal starting point for running TheClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

TheClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/theclaw 18789:18789 -n theclaw
...
```bash
kubectl get secret theclaw-secrets -n theclaw -o jsonpath='{.data.THECLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: theclaw (configurable via THECLAW_NAMESPACE)
├── Deployment/theclaw        # Single pod, init container + gateway
├── Service/theclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/theclaw-config  # theclaw.json + AGENTS.md
└── Secret/theclaw-secrets    # Gateway token + API keys
```
...
Edit `theclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret theclaw-secrets -n theclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/theclaw -n theclaw
```

### Custom namespace

```bash
THECLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/theclaw/theclaw:latest # or pin to a specific version from https://github.com/theclaw/theclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # theclaw.json + AGENTS.md
