...
TheClaw ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/theclaw`, override with
  `THECLAW_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
theclaw update --tag 2026.4.1-beta.1
...
theclaw update --tag theclaw@2026.4.1-beta.1
```

Notes:

- `--tag` applies to **package (npm) installs only**. Git installs ignore it.
- The tag is not persisted. Your next `theclaw update` uses your configured
  channel as usual.
- Downgrade protection: if the target version is older than your current version,
  TheClaw prompts for confirmation (skip with `--yes`).
...
Preview what `theclaw update` would do without making changes:

```bash
theclaw update --dry-run
theclaw update --channel beta --dry-run
theclaw update --tag 2026.4.1-beta.1 --dry-run
theclaw update --dry-run --json
```
...
When you switch channels with `theclaw update`, TheClaw also syncs plugin
sources:
