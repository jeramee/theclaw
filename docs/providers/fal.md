---
summary: "fal image and video generation setup in TheClaw"
title: "Fal"
read_when:
  - You want to use fal image generation in TheClaw
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate or video_generate
---

TheClaw ships a bundled `fal` provider for hosted image and video generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    theclaw onboard --auth-choice fal-api-key
    ```
  </Step>
...
explicit transparent-background control in TheClaw, so `background:
...
<Tip>
Use `theclaw models list --provider fal` to see the full list of available fal
