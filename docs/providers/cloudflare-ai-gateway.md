...
read_when:
  - You want to use Cloudflare AI Gateway with TheClaw
  - You need the account ID, gateway ID, or API key env var
---

Cloudflare AI Gateway sits in front of provider APIs and lets you add analytics, caching, and controls. For Anthropic, TheClaw uses the Anthropic Messages API through your Gateway endpoint.
...
When thinking is enabled for Anthropic Messages models, TheClaw strips trailing
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    theclaw onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your TheClaw config:
  ...
  <Step title="Verify the model is available">
    ```bash
    theclaw models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
