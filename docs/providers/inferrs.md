---
summary: "Run TheClaw through inferrs (OpenAI-compatible local server)"
read_when:
  - You want to run TheClaw against a local inferrs server
  - You are serving Gemma or another model through inferrs
  - You need the exact TheClaw compat flags for inferrs
title: "Inferrs"
---

[inferrs](https://github.com/ericcurtin/inferrs) can serve local models behind an
OpenAI-compatible `/v1` API. TheClaw works with `inferrs` through the generic
`openai-completions` path.

`inferrs` is currently best treated as a custom self-hosted OpenAI-compatible
backend, not a dedicated TheClaw provider plugin.
...
<Steps>
  ...
  <Step title="Add an TheClaw provider entry">
...
<AccordionGroup>
  <Accordion title="Why requiresStringContent matters">
    ...
    <Warning>
    If TheClaw runs fail with an error like:
    ...
    TheClaw will flatten pure text content parts into plain strings before sending
    the request.
  ...
  <Accordion title="Gemma and tool-schema caveat">
    ...
    `/v1/chat/completions` requests but still fail on full TheClaw agent-runtime
    turns.
    ...
    That disables TheClaw's tool schema surface for the model and can reduce prompt
    pressure on stricter local backends.

    If tiny direct requests still work but normal TheClaw agent turns continue to
    crash inside `inferrs`, the remaining issue is usually upstream model/server
    behavior rather than TheClaw's transport layer.
...
<AccordionGroup>
  ...
  <Accordion title="Direct /v1/chat/completions calls pass but theclaw infer model run fails">
  ...
  <Accordion title="inferrs still crashes on larger agent turns">
    If TheClaw no longer gets schema errors but `inferrs` still crashes on larger
...
<CardGroup cols={2}>
  <Card title="Local models" href="/gateway/local-models" icon="server">
    Running TheClaw against local model servers.
  </Card>
