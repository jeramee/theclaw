---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in TheClaw"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in TheClaw
---
...
```bash
theclaw onboard --deepinfra-api-key <key>
```
...
## Supported TheClaw surfaces

The bundled plugin registers all DeepInfra surfaces that match current
TheClaw provider contracts:

| Surface                  | Default model                      | TheClaw config/tool                                     |
| ------------------------ | ---------------------------------- | -------------------------------------------------------- |
...
native model types. TheClaw does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

TheClaw dynamically discovers available DeepInfra models at startup. Use
