---
summary: "Run TheClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run TheClaw against a local SGLang server
  - You want OpenAI-compatible /v1 endpoints with your own models
title: "SGLang"
---

SGLang can serve open-source models via an **OpenAI-compatible** HTTP API.
TheClaw can connect to SGLang using the `openai-completions` API.

TheClaw can also **auto-discover** available models from SGLang when you opt
...
TheClaw treats `sglang` as a local OpenAI-compatible provider that supports
...
define `models.providers.sglang`, TheClaw will query:
