...
read_when:
  - You want to use Volcano Engine or Doubao models with TheClaw
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    theclaw onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    theclaw models list --provider volcengine
    theclaw models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
Then enable it in `theclaw.json`:
...
For voice-note targets, TheClaw asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `theclaw onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, TheClaw falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.theclaw/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running TheClaw as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about TheClaw setup.
  </Card>
</CardGroup>
