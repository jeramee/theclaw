---
summary: "Use OpenRouter's unified API to access many models in TheClaw"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in TheClaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice openrouter-api-key
    ```
  </Step>
  <Step title="(Optional) Switch to a specific model">
    ...
    ```bash
    theclaw models set openrouter/<provider>/<model>
    ```
...
TheClaw submits text-to-video and image-to-video jobs to OpenRouter, polls
...
On real OpenRouter requests (`https://openrouter.ai/api/v1`), TheClaw also adds
OpenRouter's documented app-attribution headers:

| Header                    | Value                 |
| ------------------------- | --------------------- |
| `HTTP-Referer`            | `https://theclaw.ai` |
| `X-OpenRouter-Title`      | `TheClaw`            |
| `X-OpenRouter-Categories` | `cli-agent`           |

<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, TheClaw
...
<AccordionGroup>
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that TheClaw uses for
    better prompt-cache reuse on system/developer prompt blocks.
  </Accordion>

  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, TheClaw maps the selected thinking level to
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: TheClaw keeps
    Gemini thought-signature sanitation there, but does not enable native Gemini
    replay validation or bootstrap rewrites.
  </Accordion>

  <Accordion title="Provider routing metadata">
    If you pass OpenRouter provider routing under model params, TheClaw forwards
