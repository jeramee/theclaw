---
summary: "Use Z.AI (GLM models) with TheClaw"
read_when:
  - You want Z.AI / GLM models in TheClaw
  - You need a simple ZAI_API_KEY setup
title: "Z.AI"
---

Z.AI is the API platform for **GLM** models. It provides REST APIs for GLM and uses API keys
for authentication. Create your API key in the Z.AI console. TheClaw uses the `zai` provider
with a Z.AI API key.
...
<Tabs>
  <Tab title="Auto-detect endpoint">
    **Best for:** most users. TheClaw detects the matching Z.AI endpoint from the key and applies the correct base URL automatically.

    <Steps>
      <Step title="Run onboarding">
        ```bash
        theclaw onboard --auth-choice zai-api-key
        ```
      </Step>
      ...
      <Step title="Verify the model is available">
        ```bash
        theclaw models list --provider zai
        ```
      </Step>
    </Steps>
    ...
    **Best for:** users who want to force a specific Coding Plan or general API surface.
    ...
    <Steps>
      <Step title="Pick the right onboarding choice">
        ...
        theclaw onboard --auth-choice zai-coding-global

        # Coding Plan CN (China region)
        theclaw onboard --auth-choice zai-coding-cn

        # General API
        theclaw onboard --auth-choice zai-global

        # General API CN (China region)
        theclaw onboard --auth-choice zai-cn
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
TheClaw currently seeds the bundled `zai` provider with:
...
<AccordionGroup>
  ...
  <Accordion title="Thinking and preserved thinking">
    Z.AI thinking follows TheClaw's `/think` controls. With thinking off,
    TheClaw sends `thinking: { type: "disabled" }` to avoid responses that
    ...
    When enabled and thinking is on, TheClaw sends
