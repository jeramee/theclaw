...
title: "Cerebras"
read_when:
  - You want to use Cerebras with TheClaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice cerebras-api-key
    ```
  </Step>
  <Step title="Verify models are available">
    ```bash
    theclaw models list --provider cerebras
    ```
  </Step>
</Steps>
...
TheClaw ships a static Cerebras catalog for the public OpenAI-compatible endpoint:
...
is available to that process, for example in `~/.theclaw/.env` or through
`env.shellEnv`.
</Note>
