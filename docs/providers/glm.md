---
summary: "GLM model family overview + how to use it in TheClaw"
read_when:
  - You want GLM models in TheClaw
...
GLM is a **model family** (not a company) available through the Z.AI platform. In TheClaw, GLM
...
<Steps>
  ...
  <Step title="Set GLM as the default model">
    ```bash
    theclaw config set agents.defaults.model.primary "zai/glm-5.1"
    ```
  </Step>
...
<Tip>
`zai-api-key` lets TheClaw detect the matching Z.AI endpoint from the key and
...
TheClaw currently seeds the bundled `zai` provider with these GLM refs:
...
<AccordionGroup>
  <Accordion title="Endpoint auto-detection">
    When you use the `zai-api-key` auth choice, TheClaw inspects the key format
