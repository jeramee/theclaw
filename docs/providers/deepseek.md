...
title: "DeepSeek"
read_when:
  - You want to use DeepSeek with TheClaw
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice deepseek-api-key
    ```
  ...
  <Step title="Verify models are available">
    ```bash
    theclaw models list --provider deepseek
    ```
    ...
    ```bash
    theclaw models list --all --provider deepseek
    ```
...
<Tip>
V4 models support DeepSeek's `thinking` control. TheClaw also replays
...
`reasoning_content` on follow-up requests. TheClaw handles this inside the
...
DeepSeek `reasoning_content`. TheClaw fills that missing field on replayed
assistant messages for DeepSeek V4 thinking requests so the provider can accept
the history without requiring `/new`.

When thinking is disabled in TheClaw (including the UI **None** selection),
TheClaw sends DeepSeek `thinking: { type: "disabled" }` and strips replayed
...
```bash
THECLAW_LIVE_PROVIDERS=deepseek \
THECLAW_LIVE_MODELS="deepseek/deepseek-v4-flash,deepseek/deepseek-v4-pro" \
