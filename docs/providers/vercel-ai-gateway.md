...
read_when:
  - You want to use Vercel AI Gateway with TheClaw
...
<Tip>
TheClaw auto-discovers the Gateway `/v1/models` catalog, so
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    theclaw onboard --auth-choice ai-gateway-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    theclaw models list --provider vercel-ai-gateway
    ```
  </Step>
</Steps>
...
TheClaw accepts Vercel Claude shorthand model refs and normalizes them at
runtime:
...
configuration. TheClaw resolves the canonical form automatically.
</Tip>
...
<AccordionGroup>
  <Accordion title="Environment variable for daemon processes">
    If the TheClaw Gateway runs as a daemon (launchd/systemd), make sure
    ...
    `~/.theclaw/.env` or via `env.shellEnv` to ensure the gateway process can
    read it.
    </Warning>
