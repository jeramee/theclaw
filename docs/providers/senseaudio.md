...
TheClaw's shared `tools.media.audio` pipeline. TheClaw posts multipart audio
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. TheClaw uploads the
...
<Note>
SenseAudio is batch STT only in TheClaw. Voice Call realtime transcription
