---
summary: "Use Synthetic's Anthropic-compatible API in TheClaw"
read_when:
...
TheClaw registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
TheClaw's Anthropic client appends `/v1` to the base URL automatically, so use
...
`theclaw models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that TheClaw appends `/v1` automatically.
