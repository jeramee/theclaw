---
summary: "Alibaba Model Studio Wan video generation in TheClaw"
title: "Alibaba Model Studio"
read_when:
  - You want to use Alibaba Wan video generation in TheClaw
  - You need Model Studio or DashScope API key setup for video generation
---

TheClaw ships a bundled `alibaba` video-generation provider for Wan models on
...
<Steps>
  ...
  <Step title="Verify the provider is available">
    ```bash
    theclaw models list --provider alibaba
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Auth key priority">
    TheClaw checks for auth keys in this order:
