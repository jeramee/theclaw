---
summary: "Use Xiaomi MiMo models with TheClaw"
read_when:
  - You want Xiaomi MiMo models in TheClaw
  - You need XIAOMI_API_KEY setup
title: "Xiaomi MiMo"
---

Xiaomi MiMo is the API platform for **MiMo** models. TheClaw uses the Xiaomi
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice xiaomi-api-key
    ```

    Or pass the key directly:

    ```bash
    theclaw onboard --auth-choice xiaomi-api-key --xiaomi-api-key "$XIAOMI_API_KEY"
    ```

  </Step>
  <Step title="Verify the model is available">
    ```bash
    theclaw models list --provider xiaomi
    ```
  </Step>
</Steps>
...
targets such as Feishu and Telegram, TheClaw transcodes Xiaomi output to 48kHz
...
<AccordionGroup>
  ...
  <Accordion title="Troubleshooting">
    ...
    - When the Gateway runs as a daemon, ensure the key is available to that process (for example in `~/.theclaw/.env` or via `env.shellEnv`).

    <Warning>
    Keys set only in your interactive shell are not visible to daemon-managed gateway processes. Use `~/.theclaw/.env` or `env.shellEnv` config for persistent availability.
    </Warning>
