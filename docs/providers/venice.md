---
summary: "Use Venice AI privacy-focused models in TheClaw"
read_when:
  - You want privacy-focused inference in TheClaw
...
<Steps>
  ...
  <Step title="Configure TheClaw">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        theclaw onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    theclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, TheClaw shows all available Venice models. Pick based on your needs:
...
```bash
theclaw models set venice/kimi-k2-5
theclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
theclaw models list | grep venice
```

You can also run `theclaw configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, TheClaw fills the required DeepSeek V4
...
TheClaw keeps that provider-specific replay fix separate from the native
...
TheClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
theclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
theclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
theclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
theclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
theclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `theclaw models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
