---
summary: "Run TheClaw with LM Studio"
read_when:
  - You want to run TheClaw with open source models via LM Studio
...
If LM Studio authentication is disabled, you can leave the API key blank during interactive TheClaw setup.
...
```bash
theclaw models set lmstudio/qwen/qwen3.5-9b
```

LM Studio model keys follow a `author/model-name` format (e.g. `qwen/qwen3.5-9b`). TheClaw
...
For unauthenticated LM Studio servers, omit the key; TheClaw stores a local non-secret marker.
...
`usage` object, TheClaw recovers token counts from llama.cpp-style
...
options, TheClaw preserves those native values in model compat metadata. For
binary thinking models that advertise `allowed_options: ["off", "on"]`,
TheClaw maps disabled thinking to `off` and enabled `/think` levels to `on`
