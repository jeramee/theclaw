---
summary: "Use OpenAI via API keys or Codex subscription in TheClaw"
read_when:
  - You want to use OpenAI models in TheClaw
...
ChatGPT-plan coding agent through OpenAI's Codex clients. TheClaw keeps those
surfaces separate so config stays predictable.

TheClaw supports three OpenAI-family routes. The model prefix selects the
...
OpenAI explicitly supports subscription OAuth usage in external tools and workflows like TheClaw.
...
| `openai-codex`                     | Provider prefix   | OpenAI Codex OAuth/subscription route through the normal TheClaw PI runner.                      |
| `codex` plugin                     | Plugin            | Bundled TheClaw plugin that provides native Codex app-server runtime and `/codex` chat controls. |
...
native `/codex` chat controls available. `theclaw doctor` warns about that
...
enable the bundled Codex app-server plugin. TheClaw enables that plugin only
...
through PI, `theclaw doctor` warns and leaves the route unchanged.
</Note>

## TheClaw feature coverage

| OpenAI capability         | TheClaw surface                                           | Status                                                 |
| ------------------------- | ---------------------------------------------------------- | ------------------------------------------------------ |
...
TheClaw can use OpenAI, or an OpenAI-compatible embedding endpoint, for
...
`queryInputType` and `documentInputType` under `memorySearch`. TheClaw forwards
    ...
    **Best for:** direct API access and usage-based billing.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        theclaw onboard --auth-choice openai-api-key
        ```

        Or pass the key directly:

        ```bash
        theclaw onboard --openai-api-key "$OPENAI_API_KEY"
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        theclaw models list --provider openai
        ```
      </Step>
    </Steps>
    ...
    <Warning>
    TheClaw does **not** expose `openai/gpt-5.3-codex-spark`. Live OpenAI API requests reject that model, and the current Codex catalog does not expose it either.
    </Warning>
    ...
    **Best for:** using your ChatGPT/Codex subscription instead of a separate API key. Codex cloud requires ChatGPT sign-in.
    ...
    <Steps>
      <Step title="Run Codex OAuth">
        ```bash
        theclaw onboard --auth-choice openai-codex
        ```

        Or run OAuth directly:

        ```bash
        theclaw models auth login --provider openai-codex
        ```
        ...
        ```bash
        theclaw models auth login --provider openai-codex --device-code
        ```
      </Step>
      <Step title="Set the default model">
        ```bash
        theclaw config set agents.defaults.model.primary openai-codex/gpt-5.5
        ```
      </Step>
      <Step title="Verify the model is available">
        ```bash
        theclaw models list --provider openai-codex
        ```
      </Step>
    </Steps>
    ...
    <Note>
    Onboarding no longer imports OAuth material from `~/.codex`. Sign in with browser OAuth (default) or the device-code flow above — TheClaw manages the resulting credentials in its own agent auth store.
    </Note>

    ### Status indicator

    Chat `/status` shows which model runtime is active for the current session.
    The default PI harness appears as `Runtime: TheClaw Pi Default`. When the
    ...
    `openai-codex/*` route is selected, `theclaw doctor` warns that the model
    ...
    TheClaw treats model metadata and the runtime context cap as separate values.
    ...
    TheClaw uses upstream Codex catalog metadata for `gpt-5.5` when it is
    present. If live Codex discovery omits the `openai-codex/gpt-5.5` row while
    the account is authenticated, TheClaw synthesizes that OAuth model row so
...
`agentRuntime.id: "codex"`, but its auth is still account-based. TheClaw
selects auth in this order:

1. An explicit TheClaw `openai-codex` auth profile bound to the agent.
...
profile is selected, TheClaw also keeps `CODEX_API_KEY` and `OPENAI_API_KEY`
...
still accepted. TheClaw also protects the public OpenAI and
...
`theclaw infer image edit` when starting from an input file.
...
`openai-codex` OAuth profile is configured, TheClaw resolves that stored OAuth
...
`browser.ssrfPolicy.dangerouslyAllowPrivateNetwork: true`; TheClaw keeps
...
```
/tool image_generate model=openai/gpt-image-2 prompt="A polished launch poster for TheClaw on macOS" size=3840x2160 count=1
```
...
TheClaw adds a shared GPT-5 prompt contribution for GPT-5-family runs across providers. It applies by model id, so `openai-codex/gpt-5.5`, `openai/gpt-5.5`, `openrouter/openai/gpt-5.5`, `opencode/gpt-5.5`, and other compatible GPT-5 refs receive the same overlay. Older GPT-4.x models do not.
...
The GPT-5 contribution adds a tagged behavior contract for persona persistence, execution safety, tool discipline, output shape, completion checks, and verification. Channel-specific reply and silent-message behavior stays in the shared TheClaw system prompt and outbound delivery policy. The GPT-5 guidance is always enabled for matching models. The friendly interaction-style layer is separate and configurable.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash
    theclaw config set agents.defaults.promptOverlays.gpt5.personality off
    ```
  </Tab>
</Tabs>
...
<AccordionGroup>
  ...
  <Accordion title="Speech-to-text">
    ...
    TheClaw's media-understanding transcription surface.

    - Default model: `gpt-4o-transcribe`
    - Endpoint: OpenAI REST `/v1/audio/transcriptions`
    - Input path: multipart audio file upload
    - Supported by TheClaw wherever inbound audio transcription uses
...
generation by overriding the base URL. On the image-generation path, TheClaw
...
TheClaw recognizes these Azure host suffixes for the Azure image-generation
route:
...
For image-generation requests on a recognized Azure host, TheClaw:
...
TheClaw 2026.4.22 or later. Earlier versions treat any custom
...
routed through the bundled `openai` provider, the `model` field in TheClaw
...
TheClaw. If an Azure request fails with a validation error, check the
...
TheClaw's hidden attribution headers — see the **Native vs OpenAI-compatible
...
<AccordionGroup>
  <Accordion title="Transport (WebSocket vs SSE)">
    TheClaw uses WebSocket-first with SSE fallback (`"auto"`) for both `openai/*` and `openai-codex/*`.

    In `"auto"` mode, TheClaw:
  ...
  <Accordion title="WebSocket warm-up">
    TheClaw enables WebSocket warm-up by default for `openai/*` and `openai-codex/*` to reduce first-turn latency.
  ...
  <Accordion title="Fast mode">
    TheClaw exposes a shared fast-mode toggle for `openai/*` and `openai-codex/*`:

    - **Chat/UI:** `/fast status|on|off`
    - **Config:** `agents.defaults.models["<provider>/<model>"].params.fastMode`

    When enabled, TheClaw maps fast mode to OpenAI priority processing (`service_tier = "priority"`). Existing `service_tier` values are preserved, and fast mode does not rewrite `reasoning` or `text.verbosity`.
  ...
  <Accordion title="Priority processing (service_tier)">
    OpenAI's API exposes priority processing via `service_tier`. Set it per model in TheClaw:
    ...
    <Warning>
    `serviceTier` is only forwarded to native OpenAI endpoints (`api.openai.com`) and native Codex endpoints (`chatgpt.com/backend-api`). If you route either provider through a proxy, TheClaw leaves `service_tier` untouched.
    </Warning>
  ...
  <Accordion title="Strict-agentic GPT mode">
    For GPT-5-family runs on `openai/*`, TheClaw can use a stricter embedded execution contract:
  ...
  <Accordion title="Native vs OpenAI-compatible routes">
    TheClaw treats direct OpenAI, Codex, and Azure OpenAI endpoints differently from generic OpenAI-compatible `/v1` proxies:
