...
Deepgram is a speech-to-text API. In TheClaw it is used for inbound
...
For batch transcription, TheClaw uploads the complete audio file to Deepgram
and injects the transcript into the reply pipeline (`{{Transcript}}` +
`[Audio]` block). For Voice Call streaming, TheClaw forwards live G.711
...
<Steps>
  ...
  <Step title="Send a voice note">
    Send an audio message through any connected channel. TheClaw transcribes it
