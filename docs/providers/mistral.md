---
summary: "Use Mistral models and Voxtral transcription with TheClaw"
read_when:
  - You want to use Mistral models in TheClaw
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

TheClaw supports Mistral for both text/image model routing (`mistral/...`) and
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    theclaw onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    theclaw onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    theclaw models list --provider mistral
    ```
  </Step>
</Steps>
...
TheClaw currently ships this bundled Mistral catalog:
...
<Note>
TheClaw defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning (mistral-small-latest)">
    ...
    TheClaw maps the session **thinking** level to Mistral's API:

    | TheClaw thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
