---
summary: "Symptom first troubleshooting hub for TheClaw"
read_when:
  - TheClaw is not working and you need the fastest path to a fix
...
- `theclaw status` → shows configured channels and no obvious auth errors.
- `theclaw status --all` → full report is present and shareable.
- `theclaw gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `theclaw gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `theclaw doctor` → no blocking config/service errors.
- `theclaw channels status --probe` → reachable gateway returns live per-account
...
- `theclaw logs --follow` → steady activity, no repeating fatal errors.
...
## Local OpenAI-compatible backend works directly but fails in TheClaw

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `theclaw infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on TheClaw agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger TheClaw prompts crash the
...
## Plugin install fails with missing theclaw extensions

If install fails with `package.json missing theclaw.extensions`, the plugin package
is using an old shape that TheClaw no longer accepts.

Fix in the plugin package:

1. Add `theclaw.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `theclaw plugins install <package>` again.

Example:

```json
{
  "name": "@theclaw/my-plugin",
  "version": "1.2.3",
  "theclaw": {
...
```mermaid
flowchart TD
  A[TheClaw is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `theclaw gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    theclaw config get tools.exec.host
    theclaw config get tools.exec.security
    theclaw config get tools.exec.ask
    ...
    ```bash
    theclaw config set tools.exec.host gateway
    theclaw config set tools.exec.security full
    theclaw config set tools.exec.ask off
    theclaw gateway restart
    ```
  ...
  <Accordion title="Browser tool fails">
    ...
    - `theclaw` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `theclaw browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
