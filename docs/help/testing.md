...
TheClaw has three Vitest suites (unit/integration, e2e, live) and a small set
...
- [Matrix QA](/concepts/qa-matrix) — reference for `pnpm theclaw qa matrix`.
...
- Linux VM-backed QA lane: `pnpm theclaw qa suite --runner multipass --scenario channel-chat-baseline`
...
- Docker live model sweep: `pnpm test:docker:live-models`
  - Each selected model now runs a text turn plus a small file-read-style probe.
    ...
    Disable the extra probes with `THECLAW_LIVE_MODEL_FILE_PROBE=0` or
    `THECLAW_LIVE_MODEL_IMAGE_PROBE=0` when isolating provider failures.
  - CI coverage: daily `TheClaw Scheduled Live And E2E Checks` and manual
    `TheClaw Release Checks` both call the reusable live/E2E workflow with
    `include_live_suites: true`, which includes separate Docker live model
    matrix jobs sharded by provider.
  - For focused CI reruns, dispatch `TheClaw Live And E2E Checks (Reusable)`
  ...
  - Add new high-signal provider secrets to `scripts/ci-hydrate-live-auth.sh`
    plus `.github/workflows/theclaw-live-and-e2e-checks-reusable.yml` and its
...
- Codex app-server harness smoke: `pnpm test:docker:live-codex-harness`
  - Runs gateway agent turns through the plugin-owned Codex app-server harness,
    ...
    `THECLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=0` when isolating other Codex
    app-server failures. For a focused sub-agent check, disable the other probes:
    `THECLAW_LIVE_CODEX_HARNESS_IMAGE_PROBE=0 THECLAW_LIVE_CODEX_HARNESS_MCP_PROBE=0 THECLAW_LIVE_CODEX_HARNESS_GUARDIAN_PROBE=0 THECLAW_LIVE_CODEX_HARNESS_SUBAGENT_PROBE=1 pnpm test:docker:live-codex-harness`.
    This exits after the sub-agent probe unless
    `THECLAW_LIVE_CODEX_HARNESS_SUBAGENT_ONLY=0` is set.
...
- Crestodian first-run Docker smoke: `pnpm test:docker:crestodian-first-run`
  - Starts from an empty TheClaw state dir, routes bare `theclaw` to
    ...
    `pnpm theclaw qa suite --scenario crestodian-ring-zero-setup`.
- Moonshot/Kimi cost smoke: with `MOONSHOT_API_KEY` set, run
  `theclaw models list --provider moonshot --json`, then run an isolated
  `theclaw agent --local --session-id live-kimi-cost --message 'Reply exactly: KIMI_LIVE_OK' --thinking off --json`
...
`e2ee-deep`, and `e2ee-cli` jobs. `TheClaw Release Checks` runs parity plus
the fast Matrix and Telegram lanes before release approval.

- `pnpm theclaw qa suite`
...
- `pnpm theclaw qa suite --runner multipass`
...
- `pnpm test:docker:npm-onboard-channel-agent`
  - Builds an npm tarball from the current checkout, installs it globally in
    ...
    endpoint.
  - Use `THECLAW_NPM_ONBOARD_CHANNEL=discord` to run the same packaged-install
    lane with Discord.
- `pnpm test:docker:session-runtime-context`
  - Runs a deterministic built-app Docker smoke for embedded runtime context
    transcripts. It verifies hidden TheClaw runtime context is persisted as a
    ...
    `theclaw doctor --fix` rewrites it to the active branch with a backup.
- `pnpm test:docker:npm-telegram-live`
  - Installs an TheClaw package candidate in Docker, runs installed-package
  ...
  - Defaults to `THECLAW_NPM_TELEGRAM_PACKAGE_SPEC=theclaw@beta`; set
    `THECLAW_NPM_TELEGRAM_PACKAGE_TGZ=/path/to/theclaw-current.tgz` or
    `THECLAW_CURRENT_PACKAGE_TGZ` to test a resolved local tarball instead of
    installing from the registry.
  - Uses the same Telegram env credentials or Convex credential source as
    `pnpm theclaw qa telegram`. For CI/release automation, set
    `THECLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex` plus
    `THECLAW_QA_CONVEX_SITE_URL` and the role secret. If
    `THECLAW_QA_CONVEX_SITE_URL` and a Convex role secret are present in CI,
    the Docker wrapper selects Convex automatically.
  - `THECLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci|maintainer` overrides the shared
    `THECLAW_QA_CREDENTIAL_ROLE` for this lane only.
...
- GitHub Actions also exposes `Package Acceptance` for side-run product proof
  ...
  the normalized `theclaw-current.tgz` as `package-under-test`, then runs the
...
gh workflow run package-acceptance.yml --ref main \
  -f source=url \
  -f package_url=https://registry.npmjs.org/theclaw/-/theclaw-VERSION.tgz \
...
- `pnpm test:docker:bundled-channel-deps`
  - Packs and installs the current TheClaw build in Docker, starts the Gateway
  ...
  - Also installs a known older npm baseline, enables Telegram before running
    `theclaw update --tag <candidate>`, and verifies the candidate's
...
- `pnpm test:parallels:npm-update`
  - Runs the native packaged-install update smoke across Parallels guests. Each
    ...
    the installed `theclaw update` command in the same guest and verifies the
  ...
  - The OpenAI lane uses `openai/gpt-5.5` for the live agent-turn proof by
    ...
    `THECLAW_PARALLELS_OPENAI_MODEL` when deliberately validating another
    OpenAI model.
  ...
  - The script writes nested lane logs under `/tmp/theclaw-parallels-npm-update.*`.
...
- `pnpm theclaw qa telegram`
  - Runs the Telegram live QA lane against a real private group using the driver and SUT bot tokens from env.
  - Requires `THECLAW_QA_TELEGRAM_GROUP_ID`, `THECLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`, and `THECLAW_QA_TELEGRAM_SUT_BOT_TOKEN`. The group id must be the numeric Telegram chat id.
  - Supports `--credential-source convex` for shared pooled credentials. Use env mode by default, or set `THECLAW_QA_CREDENTIAL_SOURCE=convex` to opt into pooled leases.
  - Exits non-zero when any scenario fails. Use `--allow-failures` when you
    want artifacts without a failing exit code.
...
When `--credential-source convex` (or `THECLAW_QA_CREDENTIAL_SOURCE=convex`) is enabled for
`theclaw qa telegram`, QA lab acquires an exclusive lease from a Convex-backed pool, heartbeats
...
- `THECLAW_QA_CONVEX_SITE_URL` (for example `https://your-deployment.convex.site`)
- One secret for the selected role:
  - `THECLAW_QA_CONVEX_SECRET_MAINTAINER` for `maintainer`
  - `THECLAW_QA_CONVEX_SECRET_CI` for `ci`
- Credential role selection:
  - CLI: `--credential-role maintainer|ci`
  - Env default: `THECLAW_QA_CREDENTIAL_ROLE` (defaults to `ci` in CI, `maintainer` otherwise)

Optional env vars:

- `THECLAW_QA_CREDENTIAL_LEASE_TTL_MS` (default `1200000`)
- `THECLAW_QA_CREDENTIAL_HEARTBEAT_INTERVAL_MS` (default `30000`)
- `THECLAW_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS` (default `90000`)
- `THECLAW_QA_CREDENTIAL_HTTP_TIMEOUT_MS` (default `15000`)
- `THECLAW_QA_CONVEX_ENDPOINT_PREFIX` (default `/qa-credentials/v1`)
- `THECLAW_QA_CREDENTIAL_OWNER_ID` (optional trace id)
- `THECLAW_QA_ALLOW_INSECURE_HTTP=1` allows loopback `http://` Convex URLs for local-only development.

`THECLAW_QA_CONVEX_SITE_URL` should use `https://` in normal operation.

Maintainer admin commands (pool add/remove/list) require
`THECLAW_QA_CONVEX_SECRET_MAINTAINER` specifically.

CLI helpers for maintainers:

```bash
pnpm theclaw qa credentials doctor
pnpm theclaw qa credentials add --kind telegram --payload-file qa/telegram-credential.json
pnpm theclaw qa credentials list --kind telegram
pnpm theclaw qa credentials remove --credential-id <credential-id>
```
...
Default endpoint contract (`THECLAW_QA_CONVEX_SITE_URL` + `/qa-credentials/v1`):
...
<AccordionGroup>
  <Accordion title="Projects, shards, and scoped lanes">
    ...
    - `pnpm test:changed` expands changed git paths into cheap scoped lanes by default: direct test edits, sibling `*.test.ts` files, explicit source mappings, and local import-graph dependents. Config/setup/package edits do not broad-run tests unless you explicitly use `THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed`.
    ...
    - Import-light unit tests from agents, commands, plugins, auto-reply helpers, `plugin-sdk`, and similar pure utility areas route through the `unit-fast` lane, which skips `test/setup-theclaw-runtime.ts`; stateful/runtime-heavy files stay on the existing lanes.
  ...
  <Accordion title="Vitest pool and isolation defaults">
    ...
    - `scripts/run-vitest.mjs` adds `--no-maglev` for Vitest child Node
      ...
      Set `THECLAW_VITEST_ENABLE_MAGLEV=1` to compare against stock V8
      behavior.
  ...
  <Accordion title="Fast local iteration">
    ...
    - `pnpm test:changed` routes through cheap scoped lanes by default. Use
      `THECLAW_TEST_CHANGED_BROAD=1 pnpm test:changed` only when the agent
    ...
    - The base Vitest config marks the projects/config files as
      ...
      wiring changes.
    - The config keeps `THECLAW_VITEST_FS_MODULE_CACHE` enabled on supported
      hosts; set `THECLAW_VITEST_FS_MODULE_CACHE_PATH=/abs/path` if you want
...
- Useful overrides:
  - `THECLAW_E2E_WORKERS=<n>` to force worker count (capped at 16).
  - `THECLAW_E2E_VERBOSE=1` to re-enable verbose console output.
- Scope:
...
- Scope:
  ...
  - Exercises TheClaw's OpenShell backend over real `sandbox ssh-config` + SSH exec
...
- Useful overrides:
  - `THECLAW_E2E_OPENSHELL=1` to enable the test when running the broader e2e suite manually
  - `THECLAW_E2E_OPENSHELL_COMMAND=/path/to/openshell` to point at a non-default CLI binary or wrapper script
...
- Default: **enabled** by `pnpm test:live` (sets `THECLAW_LIVE_TEST=1`)
- Scope:
...
- By default, live runs still isolate `HOME` and copy config/auth material into a temp test home so unit fixtures cannot mutate your real `~/.theclaw`.
- Set `THECLAW_LIVE_USE_REAL_HOME=1` only when you intentionally need live tests to use your real home directory.
- `pnpm test:live` now defaults to a quieter mode: it keeps `[live] ...` progress output, but suppresses the extra `~/.profile` notice and mutes gateway bootstrap logs/Bonjour chatter. Set `THECLAW_LIVE_TEST_QUIET=0` if you want the full startup logs back.
- API key rotation (provider-specific): set `*_API_KEYS` with comma/semicolon format or `*_API_KEY_1`, `*_API_KEY_2` (for example `OPENAI_API_KEYS`, `ANTHROPIC_API_KEYS`, `GEMINI_API_KEYS`) or per-live override via `THECLAW_LIVE_*_KEY`; tests retry on rate limit responses.
- Progress/heartbeat output:
  ...
  - Tune direct-model heartbeats with `THECLAW_LIVE_HEARTBEAT_MS`.
  - Tune gateway/probe heartbeats with `THECLAW_LIVE_GATEWAY_HEARTBEAT_MS`.
...
- Docker live runners default to a smaller smoke cap so a full Docker sweep stays practical:
  `test:docker:live-models` defaults to `THECLAW_LIVE_MAX_MODELS=12`, and
  `test:docker:live-gateway` defaults to `THECLAW_LIVE_GATEWAY_SMOKE=1`,
  `THECLAW_LIVE_GATEWAY_MAX_MODELS=8`,
  `THECLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS=45000`, and
  `THECLAW_LIVE_GATEWAY_MODEL_TIMEOUT_MS=90000`. Override those env vars when you
  explicitly want the larger exhaustive scan.
- `test:docker:all` builds the live Docker image once via `test:docker:live-build`, packs TheClaw once as an npm tarball through `scripts/package-theclaw-for-docker.mjs`, then builds/reuses two `scripts/e2e/Dockerfile` images. The bare image is only the Node/Git runner for install/update/plugin-dependency lanes; those lanes mount the prebuilt tarball. The functional image installs the same tarball into `/app` for built-app functionality lanes. Docker lane definitions live in `scripts/lib/docker-e2e-scenarios.mjs`; planner logic lives in `scripts/lib/docker-e2e-plan.mjs`; `scripts/test-docker-all.mjs` executes the selected plan. The aggregate uses a weighted local scheduler: `THECLAW_DOCKER_ALL_PARALLELISM` controls process slots, while resource caps keep heavy live, npm-install, and multi-service lanes from all starting at once. If a single lane is heavier than the active caps, the scheduler can still start it when the pool is empty and then keeps it running alone until capacity is available again. Defaults are 10 slots, `THECLAW_DOCKER_ALL_LIVE_LIMIT=9`, `THECLAW_DOCKER_ALL_NPM_LIMIT=10`, and `THECLAW_DOCKER_ALL_SERVICE_LIMIT=7`; tune `THECLAW_DOCKER_ALL_WEIGHT_LIMIT` or `THECLAW_DOCKER_ALL_DOCKER_LIMIT` only when the Docker host has more headroom. The runner performs a Docker preflight by default, removes stale TheClaw E2E containers, prints status every 30 seconds, stores successful lane timings in `.artifacts/docker-tests/lane-timings.json`, and uses those timings to start longer lanes first on later runs. Use `THECLAW_DOCKER_ALL_DRY_RUN=1` to print the weighted lane manifest without building or running Docker, or `node scripts/test-docker-all.mjs --plan-json` to print the CI plan for selected lanes, package/image needs, and credentials.
...
- Npm tarball onboarding/channel/agent smoke: `pnpm test:docker:npm-onboard-channel-agent` installs the packed TheClaw tarball globally in Docker, configures OpenAI via env-ref onboarding plus Telegram by default, verifies doctor repairs activated plugin runtime deps, and runs one mocked OpenAI agent turn. Reuse a prebuilt tarball with `THECLAW_CURRENT_PACKAGE_TGZ=/path/to/theclaw-*.tgz`, skip the host rebuild with `THECLAW_NPM_ONBOARD_HOST_BUILD=0`, or switch channel with `THECLAW_NPM_ONBOARD_CHANNEL=discord`.
- Update channel switch smoke: `pnpm test:docker:update-channel-switch` installs the packed TheClaw tarball globally in Docker, switches from package `stable` to git `dev`, verifies the persisted channel and plugin post-update work, then switches back to package `stable` and checks update status.
...
- Bun global install smoke: `bash scripts/e2e/bun-global-install-smoke.sh` packs the current tree, installs it with `bun install -g` in an isolated home, and verifies `theclaw infer image providers --json` returns bundled image providers instead of hanging. Reuse a prebuilt tarball with `THECLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ=/path/to/theclaw-*.tgz`, skip the host build with `THECLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0`, or copy `dist/` from a built Docker image with `THECLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE=theclaw-dockerfile-smoke:local`.
- Installer Docker smoke: `bash scripts/test-install-sh-docker.sh` shares one npm cache across its root, update, and direct-npm containers. Update smoke defaults to npm `latest` as the stable baseline before upgrading to the candidate tarball. Override with `THECLAW_INSTALL_SMOKE_UPDATE_BASELINE=2026.4.22` locally, or with the Install Smoke workflow's `update_baseline_version` input on GitHub. Non-root installer checks keep an isolated npm cache so root-owned cache entries do not mask user-local install behavior. Set `THECLAW_INSTALL_SMOKE_NPM_CACHE_DIR=/path/to/cache` to reuse the root/update/direct-npm cache across local reruns.
- Install Smoke CI skips the duplicate direct-npm global update with `THECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1`; run the script locally without that env when direct `npm install -g` coverage is needed.
- Agents delete shared workspace CLI smoke: `pnpm test:docker:agents-delete-shared-workspace` (script: `scripts/e2e/agents-delete-shared-workspace-docker.sh`) builds the root Dockerfile image by default, seeds two agents with one workspace in an isolated container home, runs `agents delete --json`, and verifies valid JSON plus retained workspace behavior. Reuse the install-smoke image with `THECLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_IMAGE=theclaw-dockerfile-smoke:local THECLAW_AGENTS_DELETE_SHARED_WORKSPACE_E2E_SKIP_BUILD=1`.
...
- Plugins (install smoke, ClawHub install/uninstall, marketplace updates, and Claude-bundle enable/inspect): `pnpm test:docker:plugins` (script: `scripts/e2e/plugins-docker.sh`)
  Set `THECLAW_PLUGINS_E2E_CLAWHUB=0` to skip the live ClawHub block, or override the default package with `THECLAW_PLUGINS_E2E_CLAWHUB_SPEC` and `THECLAW_PLUGINS_E2E_CLAWHUB_ID`.
...
- Bundled plugin runtime deps: `pnpm test:docker:bundled-channel-deps` builds a small Docker runner image by default, builds and packs TheClaw once on the host, then mounts that tarball into each Linux install scenario. Reuse the image with `THECLAW_SKIP_DOCKER_BUILD=1`, skip the host rebuild after a fresh local build with `THECLAW_BUNDLED_CHANNEL_HOST_BUILD=0`, or point at an existing tarball with `THECLAW_CURRENT_PACKAGE_TGZ=/path/to/theclaw-*.tgz`. The full Docker aggregate and release-path bundled-channel chunks pre-pack this tarball once, then shard bundled channel checks into independent lanes, including separate update lanes for Telegram, Discord, Slack, Feishu, memory-lancedb, and ACPX. Release chunks split channel smokes, update targets, and setup/runtime contracts into `bundled-channels-core`, `bundled-channels-update-a`, `bundled-channels-update-b`, and `bundled-channels-contracts`; the aggregate `bundled-channels` chunk remains available for manual reruns. The release workflow also splits provider installer chunks and bundled plugin install/uninstall chunks; legacy `package-update`, `plugins-runtime`, and `plugins-integrations` chunks remain aggregate aliases for manual reruns. Use `THECLAW_BUNDLED_CHANNELS=telegram,slack` to narrow the channel matrix when running the bundled lane directly, or `THECLAW_BUNDLED_CHANNEL_UPDATE_TARGETS=telegram,acpx` to narrow the update scenario. The lane also verifies that `channels.<id>.enabled=false` and `plugins.entries.<id>.enabled=false` suppress doctor/runtime-dependency repair.
- Narrow bundled plugin runtime deps while iterating by disabling unrelated scenarios, for example:
  `THECLAW_BUNDLED_CHANNEL_SCENARIOS=0 THECLAW_BUNDLED_CHANNEL_UPDATE_SCENARIO=0 THECLAW_BUNDLED_CHANNEL_ROOT_OWNED_SCENARIO=0 THECLAW_BUNDLED_CHANNEL_SETUP_ENTRY_SCENARIO=0 pnpm test:docker:bundled-channel-deps`.

To prebuild and reuse the shared functional image manually:

```bash
THECLAW_DOCKER_E2E_IMAGE=theclaw-docker-e2e-functional:local pnpm test:docker:e2e-build
THECLAW_DOCKER_E2E_IMAGE=theclaw-docker-e2e-functional:local THECLAW_SKIP_DOCKER_BUILD=1 pnpm test:docker:mcp-channels
```

Suite-specific image overrides such as `THECLAW_GATEWAY_NETWORK_E2E_IMAGE` still win when set. When `THECLAW_SKIP_DOCKER_BUILD=1` points at a remote shared image, the scripts pull it if it is not already local. The QR and installer Docker tests keep their own Dockerfiles because they validate package/install behavior rather than the shared built-app runtime.
...
`.pnpm-store`, `.worktrees`, `__theclaw_vitest__`, and app-local `.build` or
Gradle output directories so Docker live runs do not spend minutes copying
machine-specific artifacts.
They also set `THECLAW_SKIP_CHANNELS=1` so gateway live probes do not start
...
`THECLAW_LIVE_GATEWAY_*` as well when you need to narrow or exclude gateway
live coverage from that Docker lane.
`test:docker:openwebui` is a higher-level compatibility smoke: it starts an
TheClaw gateway container with the OpenAI-compatible HTTP endpoints enabled,
starts a pinned Open WebUI container against that gateway, signs in through
Open WebUI, verifies `/api/models` exposes `theclaw/default`, then sends a
...
This lane expects a usable live model key, and `THECLAW_PROFILE_FILE`
...
container, starts a second container that spawns `theclaw mcp serve`, then
...
- `THECLAW_CONFIG_DIR=...` (default: `~/.theclaw`) mounted to `/home/node/.theclaw`
- `THECLAW_WORKSPACE_DIR=...` (default: `~/.theclaw/workspace`) mounted to `/home/node/.theclaw/workspace`
- `THECLAW_PROFILE_FILE=...` (default: `~/.profile`) mounted to `/home/node/.profile` and sourced before running tests
- `THECLAW_DOCKER_PROFILE_ENV_ONLY=1` to verify only env vars sourced from `THECLAW_PROFILE_FILE`, using temporary config/workspace dirs and no external CLI auth mounts
- `THECLAW_DOCKER_CLI_TOOLS_DIR=...` (default: `~/.cache/theclaw/docker-cli-tools`) mounted to `/home/node/.npm-global` for cached CLI installs inside Docker
- External CLI auth dirs/files under `$HOME` are mounted read-only under `/host-auth...`, then copied into `/home/node/...` before tests start
  ...
  - Narrowed provider runs mount only the needed dirs/files inferred from `THECLAW_LIVE_PROVIDERS` / `THECLAW_LIVE_GATEWAY_PROVIDERS`
  - Override manually with `THECLAW_DOCKER_AUTH_DIRS=all`, `THECLAW_DOCKER_AUTH_DIRS=none`, or a comma list like `THECLAW_DOCKER_AUTH_DIRS=.claude,.codex`
- `THECLAW_LIVE_GATEWAY_MODELS=...` / `THECLAW_LIVE_MODELS=...` to narrow the run
- `THECLAW_LIVE_GATEWAY_PROVIDERS=...` / `THECLAW_LIVE_PROVIDERS=...` to filter providers in-container
- `THECLAW_SKIP_DOCKER_BUILD=1` to reuse an existing `theclaw:local-live` image for reruns that do not need a rebuild
- `THECLAW_LIVE_REQUIRE_PROFILE_KEYS=1` to ensure creds come from the profile store (not env)
- `THECLAW_OPENWEBUI_MODEL=...` to choose the model exposed by the gateway for the Open WebUI smoke
- `THECLAW_OPENWEBUI_PROMPT=...` to override the nonce-check prompt used by the Open WebUI smoke
