...
This is handy when you need to toggle obscure settings without editing `theclaw.json`.
...
Use `THECLAW_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
THECLAW_PLUGIN_LIFECYCLE_TRACE=1 theclaw plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm theclaw ...`
also measures source-runner overhead.

## Temporary CLI debug timing

TheClaw keeps `src/cli/debug-timing.ts` as a small helper for local
...
`theclaw models list`, a temporary patch in
...
```bash
THECLAW_DEBUG_TIMING=1 pnpm theclaw models list --all --provider moonshot
```

Example output from a temporary `models list` investigation:

```text
TheClaw CLI debug timing: models list
...
```bash
THECLAW_DEBUG_TIMING=json pnpm theclaw models list --all --provider moonshot \
...
extension `package.json` and `theclaw.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.theclaw-dev` and
...
```bash
pnpm gateway:dev
THECLAW_PROFILE=dev theclaw tui
```

If you don’t have a global install yet, run the CLI via `pnpm theclaw ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `THECLAW_PROFILE=dev`
   - `THECLAW_STATE_DIR=~/.theclaw-dev`
   - `THECLAW_CONFIG_PATH=~/.theclaw-dev/theclaw.json`
   - `THECLAW_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`THECLAW_SKIP_CHANNELS=1`).
...
```bash
THECLAW_PROFILE=dev theclaw gateway --dev --reset
```
...
## Raw stream logging (TheClaw)

TheClaw can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.theclaw/logs/raw-stream.jsonl
```

Equivalent env vars:

```bash
THECLAW_RAW_STREAM=1
THECLAW_RAW_STREAM_PATH=~/.theclaw/logs/raw-stream.jsonl
```

Default file:

`~/.theclaw/logs/raw-stream.jsonl`
