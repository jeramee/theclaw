...
summary: "Specification for making the bundled Codex app-server harness honor TheClaw context-engine plugins"
read_when:
...
Make the bundled Codex app-server harness honor the same TheClaw context-engine
...
TheClaw-level compaction policy as far as the Codex app-server boundary allows.
...
That means the required hook point is in TheClaw-controlled code. The external
boundary is the Codex app-server protocol itself: TheClaw can control what it
...
For Codex harness turns, TheClaw should preserve this lifecycle:

1. Read the mirrored TheClaw session transcript.
...
8. Mirror the Codex result back into the TheClaw transcript.
...
11. Preserve Codex native compaction signals and TheClaw compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. TheClaw should
not try to mutate the app-server's internal history except through supported
protocol calls.

TheClaw's transcript mirror remains the source for TheClaw features:
...
The context-engine interface returns TheClaw `AgentMessage[]`, not a Codex
...
- Label it clearly as TheClaw assembled context.
...
```text
TheClaw assembled context for this turn:
...
inside TheClaw and preserves context-engine semantics.
...
1. TheClaw context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit TheClaw compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
TheClaw compaction should prefer the context engine's `compact()` result for
the TheClaw transcript mirror and plugin state.
...
the TheClaw session file. Preserve that behavior.

Also ensure context-engine state cleanup continues to happen through existing
TheClaw session lifecycle paths. Do not add Codex-specific cleanup unless the
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole TheClaw compaction when the context engine is primary.
...
Avoid requiring lossless-claw in TheClaw core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for TheClaw/plugin state.
