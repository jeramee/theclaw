---
summary: "Move one TheClaw session's reply route between linked chat channels"
...
Channel docking is call forwarding for one TheClaw session.
...
Alice can message TheClaw on Telegram and Discord:
...
TheClaw keeps the current session context and changes the reply route:
...
4. Keep the same TheClaw session, but receive future replies in Discord.
