...
`node-llama-cpp` runtime package next to TheClaw, then point `local.modelPath`
at a GGUF file:
...
TheClaw indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
80-token overlap) and stores them in a per-agent SQLite database.

- **Index location:** `~/.theclaw/memory/<agentId>.sqlite`
...
- **Reindex on demand:** `theclaw memory index --force`
...
**Memory search disabled?** Check `theclaw memory status`. If no provider is
...
**Stale results?** Run `theclaw memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** TheClaw falls back to in-process cosine similarity
