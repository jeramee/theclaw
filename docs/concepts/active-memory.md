...
Paste this into `theclaw.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, TheClaw can show:
...
case-insensitive. If `allowedChatIds` is non-empty and TheClaw cannot resolve a
...
`config.promptOverride` replaces the default Active Memory prompt. TheClaw
...
5. Verify memory search itself works with `theclaw memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, TheClaw auto-detects the first
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `theclaw memory status --deep` to inspect the memory-search backend
      and index health.
