---
summary: "What the TheClaw system prompt contains and how it is assembled"
read_when:
...
TheClaw builds a custom system prompt for every agent run. The prompt is **TheClaw-owned** and does not use the pi-coding-agent default prompt.

The prompt is assembled by TheClaw and injected into each agent run.

Provider plugins can contribute cache-aware prompt guidance without replacing
the full TheClaw-owned prompt. The provider runtime can:
...
- **TheClaw Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to TheClaw docs (repo or npm package) and when to read them.
...
TheClaw can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Skills**, **Memory Recall**, **TheClaw
...
For channel auto-reply runs, TheClaw can omit the generic **Silent Replies**
...
occurs, TheClaw can inject a warning block in Project Context; control this with
...
When eligible skills exist, TheClaw injects a compact **available skills list**
...
points to the local TheClaw docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.theclaw.ai](https://docs.theclaw.ai).

The same section also includes the TheClaw source location. Git checkouts expose the local
...
consult docs first for TheClaw behavior, commands, configuration, or architecture, and to
run `theclaw status` itself when possible (asking the user only when it lacks access).
