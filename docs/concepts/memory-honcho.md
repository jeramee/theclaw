...
[Honcho](https://honcho.dev) adds AI-native memory to TheClaw. It persists
...
```bash
theclaw plugins install @honcho-ai/theclaw-honcho
theclaw honcho setup
...
Settings live under `plugins.entries["theclaw-honcho"].config`:
    ...
    entries: {
      "theclaw-honcho": {
        config: {
          ...
          workspaceId: "theclaw", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `theclaw honcho setup` detects and
...
```bash
theclaw honcho setup                        # Configure API key and migrate files
theclaw honcho status                       # Check connection status
theclaw honcho ask <question>               # Query Honcho about the user
theclaw honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/theclaw-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho TheClaw integration guide](https://docs.honcho.dev/v3/guides/integrations/theclaw)
- [Memory](/concepts/memory) -- TheClaw memory overview
