...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.theclaw/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.theclaw/agents/<agentId>/sessions`.
...
Skills are loaded from each agent workspace plus shared roots such as `~/.theclaw/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-skill-allowlists).
...
- State dir: `~/.theclaw` (or `THECLAW_STATE_DIR`)
- Workspace: `~/.theclaw/workspace` (or `~/.theclaw/workspace-<agentId>`)
- Agent dir: `~/.theclaw/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.theclaw/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, TheClaw runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.theclaw/workspace` (or `~/.theclaw/workspace-<profile>` when `THECLAW_PROFILE` is set).
- State defaults to `~/.theclaw/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    theclaw agents add coding
    theclaw agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.theclaw/agents/<agentId>`.
  ...
  <Step title="Create channel accounts">
    ...
    ```bash
    theclaw channels login --channel whatsapp --account work
    ```
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.theclaw/workspace-alex" },
      { id: "mia", workspace: "~/.theclaw/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, TheClaw upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, TheClaw falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.theclaw/workspace-main" },
          { id: "coding", workspace: "~/.theclaw/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.theclaw/workspace-main" },
          { id: "alerts", workspace: "~/.theclaw/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    theclaw channels login --channel whatsapp --account personal
    theclaw channels login --channel whatsapp --account biz
    ```

    `~/.theclaw/theclaw.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.theclaw/workspace-home",
            agentDir: "~/.theclaw/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.theclaw/workspace-work",
            agentDir: "~/.theclaw/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.theclaw/credentials/whatsapp/personal
              // authDir: "~/.theclaw/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.theclaw/credentials/whatsapp/biz
              // authDir: "~/.theclaw/credentials/whatsapp/biz",
            },
          },
        },
      },
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.theclaw/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.theclaw/workspace-opus",
