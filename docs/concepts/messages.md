...
TheClaw handles inbound messages through a pipeline of session resolution, queueing, streaming, tool execution, and reasoning visibility. This page maps the path from inbound message to reply.
...
Channels can redeliver the same message after reconnects. TheClaw keeps a
...
TheClaw keeps that boundary explicit:
...
TheClaw separates the **prompt body** from the **command body**:
...
TheClaw can expose or hide model reasoning:
...
When a turn also has pending tool media, such as generated TTS audio, TheClaw
strips the silent text but still delivers the media attachment.
TheClaw resolves that behavior by conversation type:
...
TheClaw also uses silent replies for internal runner failures that happen
