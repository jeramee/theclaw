...
TheClaw gives agents tools to work across sessions, inspect status, and
...
After the target responds, TheClaw can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `theclaw-tui` are
not session keys.
...
`subagents` is the control-plane helper for already spawned TheClaw
...
the completion origin only identifies a channel TheClaw can still reuse the
