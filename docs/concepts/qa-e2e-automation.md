...
The private QA stack is meant to exercise TheClaw in a more realistic,
...
Every QA flow runs under `pnpm theclaw qa <subcommand>`. Many have `pnpm qa:*`
script aliases; both forms are supported.

| Command                                             | Purpose                                                                                                                                                                |
| --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `qa run`                                            | Bundled QA self-check; writes a Markdown report.                                                                                                                       |
| `qa suite`                                          | Run repo-backed scenarios against the QA gateway lane. Aliases: `pnpm theclaw qa suite --runner multipass` for a disposable Linux VM.                                 |
...
```bash
pnpm theclaw qa docker-build-image
pnpm qa:lab:build
pnpm qa:lab:up:fast
pnpm qa:lab:watch
```
...
`theclaw.run`, `theclaw.harness.run`, `theclaw.model.call`,
`theclaw.context.assembled`, and `theclaw.message.delivery` must be present;
model calls must not export `StreamAbandoned` on successful turns; raw diagnostic IDs and
`theclaw.content.*` attributes must stay out of the trace. It writes
...
```bash
pnpm theclaw qa suite --runner multipass --scenario channel-chat-baseline
```

This boots a fresh Multipass guest, installs dependencies, builds TheClaw
...
- `THECLAW_QA_TELEGRAM_GROUP_ID` — numeric chat id (string).
- `THECLAW_QA_TELEGRAM_DRIVER_BOT_TOKEN`
- `THECLAW_QA_TELEGRAM_SUT_BOT_TOKEN`

Optional:

- `THECLAW_QA_TELEGRAM_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts (default redacts).
...
- `telegram-qa-observed-messages.json` — bodies redacted unless `THECLAW_QA_TELEGRAM_CAPTURE_CONTENT=1`.
...
Targets one real private Discord guild channel with two bots: a driver bot controlled by the harness and a SUT bot started by the child TheClaw gateway through the bundled Discord plugin. Verifies channel mention handling and that the SUT bot has registered the native `/help` command with Discord.

Required env when `--credential-source env`:

- `THECLAW_QA_DISCORD_GUILD_ID`
- `THECLAW_QA_DISCORD_CHANNEL_ID`
- `THECLAW_QA_DISCORD_DRIVER_BOT_TOKEN`
- `THECLAW_QA_DISCORD_SUT_BOT_TOKEN`
- `THECLAW_QA_DISCORD_SUT_APPLICATION_ID` — must match the SUT bot user id returned by Discord (the lane fails fast otherwise).

Optional:

- `THECLAW_QA_DISCORD_CAPTURE_CONTENT=1` keeps message bodies in observed-message artifacts.
...
- `discord-qa-observed-messages.json` — bodies redacted unless `THECLAW_QA_DISCORD_CAPTURE_CONTENT=1`.

### Convex credential pool

Both Telegram and Discord lanes can lease credentials from a shared Convex pool instead of reading the env vars above. Pass `--credential-source convex` (or set `THECLAW_QA_CREDENTIAL_SOURCE=convex`); QA Lab acquires an exclusive lease, heartbeats it for the duration of the run, and releases it on shutdown. Pool kinds are `"telegram"` and `"discord"`.
...
- `mock-openai` is the scenario-aware TheClaw mock. It remains the default
...
- how `theclaw qa <runner>` is mounted beneath the shared `qa` root
...
4. Mount the runner as `theclaw qa <runner>` instead of registering a competing root command. Runner plugins should declare `qaRunners` in `theclaw.plugin.json` and export a matching `qaRunnerCliRegistrations` array from `runtime-api.ts`. Keep `runtime-api.ts` light; lazy CLI and runner execution should stay behind separate entrypoints.
...
For the inventory of available scenarios — useful when sizing follow-up work or wiring a new transport — run `pnpm theclaw qa coverage` (add `--json` for machine-readable output).

For character and style checks, run the same scenario across multiple live model
refs and write a judged Markdown report:

```bash
pnpm theclaw qa character-eval \
