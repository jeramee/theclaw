...
TheClaw selects models in this order:
...
<AccordionGroup>
  <Accordion title="Related model surfaces">
    - `agents.defaults.models` is the allowlist/catalog of models TheClaw can use (plus aliases).
...
- User session selections are exact. `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` store `modelOverrideSource: "user"`; if that selected provider/model is unreachable, TheClaw fails visibly instead of falling through to another configured model.
...
<AccordionGroup>
  <Accordion title="Clobber protection rules">
    `theclaw config set` protects model/provider maps from accidental clobbers. A plain object assignment to `agents.defaults.models`, `models.providers`, or `models.providers.<id>.models` is rejected when it would remove existing entries. Use `--merge` for additive changes; use `--replace` only when the provided value should become the complete target value.

    Interactive provider setup and `theclaw configure --section model` also merge provider-scoped selections into the existing allowlist, so adding Codex, Ollama, or another provider does not drop unrelated model entries. Configure preserves an existing `agents.defaults.model.primary` when provider auth is re-applied. Explicit default-setting commands such as `theclaw models auth login --provider <id> --set-default` and `theclaw models set <model>` still replace `agents.defaults.model.primary`.
...
If `agents.defaults.models` is set, it becomes the **allowlist** for `/model` and for session overrides. When a user selects a model that isn't in that allowlist, TheClaw returns:
...
<AccordionGroup>
  ...
  <Accordion title="Persistence and live switching">
    ...
    - If a run is already active, TheClaw marks a live switch as pending and only restarts into the new model at a clean retry point.
  ...
  <Accordion title="Ref parsing">
    ...
    - If you omit the provider, TheClaw resolves the input in this order:
      1. alias match
      2. unique configured-provider match for that exact unprefixed model id
      3. deprecated fallback to the configured default provider — if that provider no longer exposes the configured default model, TheClaw instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
  </Accordion>
</AccordionGroup>
...
theclaw models set <provider/model>
theclaw models set-image <provider/model>

theclaw models aliases list
theclaw models aliases add <alias> <provider/model>
theclaw models aliases remove <alias>

theclaw models fallbacks list
theclaw models fallbacks add <provider/model>
theclaw models fallbacks remove <provider/model>
theclaw models fallbacks clear

theclaw models image-fallbacks list
theclaw models image-fallbacks add <provider/model>
theclaw models image-fallbacks remove <provider/model>
theclaw models image-fallbacks clear
```

`theclaw models` (no subcommand) is a shortcut for `models status`.
...
`theclaw models scan` inspects OpenRouter's **free model catalog** and can optionally probe models for tool and image support.
...
When live probes run in a TTY, you can select fallbacks interactively. In non-interactive mode, pass `--yes` to accept defaults. Metadata-only results are informational; `--set-default` and `--set-image` require live probes so TheClaw does not configure an unusable keyless OpenRouter model.

## Models registry (`models.json`)

Custom providers in `models.providers` are written into `models.json` under the agent directory (default `~/.theclaw/agents/<agentId>/agent/models.json`). This file is merged by default unless `models.mode` is set to `replace`.
...
<Note>
Marker persistence is source-authoritative: TheClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values. This applies whenever TheClaw regenerates `models.json`, including command-driven paths like `theclaw agent`.
</Note>
