---
summary: "How TheClaw remembers things across sessions"
...
TheClaw remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.theclaw/workspace`).
...
<Info>
TheClaw auto-detects your embedding provider from available API keys. If you
...
Before [compaction](/concepts/compaction) summarizes your conversation, TheClaw
...
```bash
theclaw memory rem-backfill --rollback
theclaw memory rem-backfill --rollback-short-term
```

## CLI

```bash
theclaw memory status          # Check index status and provider
theclaw memory search "query"  # Search from the command line
theclaw memory index --force   # Rebuild the index
```
