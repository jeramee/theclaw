...
TheClaw has two separate streaming layers:
...
media block early, TheClaw remembers that delivery for the turn. If the final
...
distinct text around media that was already streamed, TheClaw still sends the
...
When block streaming is enabled, TheClaw can **merge consecutive block chunks**
