...
alongside TheClaw. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, TheClaw falls back to the
...
TheClaw creates a self-contained QMD home under
`~/.theclaw/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- TheClaw creates collections from your workspace memory files and any
...
- Searches use the configured `searchMode` (default: `search`; also supports
  `vsearch` and `query`). `search` is BM25-only, so TheClaw skips semantic
  vector readiness probes and embedding maintenance in that mode. If a mode
  fails, TheClaw retries with `qmd query`.
- With QMD releases that advertise multi-collection filters, TheClaw groups
  same-source collections into one QMD search invocation. Older QMD releases
  keep the compatible per-collection fallback.
- If QMD fails entirely, TheClaw falls back to the builtin SQLite engine.
  ...
  `theclaw memory status` and one-shot CLI probes still recheck QMD directly.
...
TheClaw keeps the QMD search path compatible with both current and older QMD
installs.

On startup, TheClaw checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, TheClaw searches all
...
Older QMD builds only accept one collection filter. When TheClaw detects one
...
process, so you can tune QMD globally without adding new TheClaw config:
...
collection under `~/.theclaw/agents/<id>/qmd/sessions/`.
...
When scope denies a search, TheClaw logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If TheClaw
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but TheClaw still reports
...
with `theclaw memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs TheClaw uses.

**Many QMD subprocesses during search?** Update QMD if possible. TheClaw uses
...
`memory.qmd.searchMode = "search"`. TheClaw treats that mode as lexical-only,
...
the next startup reconciliation. TheClaw recreates stale managed collections
...
TheClaw's builtin symlink rules. Keep temporary monorepo checkouts under
