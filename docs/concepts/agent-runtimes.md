---
summary: "How TheClaw separates model providers, models, channels, and agent runtimes"
...
| ------------- | ------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `openai-codex` | How TheClaw authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`          | The model selected for the agent turn.                              |
| Agent runtime | `pi`, `codex`, `claude-cli`           | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp    | Where messages enter and leave TheClaw.                            |
...
implements the `codex` runtime. Public config uses `agentRuntime.id`; `theclaw
doctor --fix` rewrites older runtime-policy keys to that shape.

There are two runtime families:

- **Embedded harnesses** run inside TheClaw's prepared agent loop. Today this
...
| Surface                                              | TheClaw name/config                 | What it does                                                                                        |
| ---------------------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------- |
| Codex OAuth provider route                           | `openai-codex/*` model refs          | Uses ChatGPT/Codex subscription OAuth through the normal TheClaw PI runner.                        |
...
That means TheClaw selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean the channel, model
provider catalog, or TheClaw session store becomes Codex.
...
3. If the user asks for **Codex OAuth/subscription auth on the normal TheClaw
...
| Surface                     | TheClaw PI embedded                    | Codex app-server                                                            |
| --------------------------- | --------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | TheClaw through the PI embedded runner | Codex app-server                                                            |
| Canonical thread state      | TheClaw transcript                     | Codex thread, plus TheClaw transcript mirror                               |
| TheClaw dynamic tools      | Native TheClaw tool loop               | Bridged through the Codex adapter                                           |
| Native shell and file tools | PI/TheClaw path                        | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native TheClaw context assembly        | TheClaw projects assembled context into the Codex turn                     |
| Compaction                  | TheClaw or selected context engine     | Codex-native compaction, with TheClaw notifications and mirror maintenance |
| Channel delivery            | TheClaw                                | TheClaw                                                                    |

This ownership split is the main design rule:

- If TheClaw owns the surface, TheClaw can provide normal plugin hook behavior.
- If the native runtime owns the surface, TheClaw needs runtime events or native hooks.
- If the native runtime owns canonical thread state, TheClaw should mirror and project context, not rewrite unsupported internals.

## Runtime selection

TheClaw chooses an embedded runtime after provider and model resolution:
...
2. `THECLAW_AGENT_RUNTIME=<id>` forces that runtime for new or reset sessions.
...
5. If no runtime claims a turn in `auto` mode and `fallback: "pi"` is set
   (the default), TheClaw uses PI as the compatibility fallback. Set
...
If `theclaw doctor` warns that the `codex` plugin is enabled while
...
When a runtime is not PI, it should document what TheClaw surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether TheClaw can edit history or only mirror it.                                   |
| Do TheClaw dynamic tools work?        | Messaging, sessions, cron, and TheClaw-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around TheClaw-owned tools. |
