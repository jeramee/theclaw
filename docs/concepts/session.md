---
summary: "How TheClaw manages conversation sessions"
read_when:
...
TheClaw organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `theclaw security audit`.
...
TheClaw automatically bounds session storage over time. By default, it runs
...
For production-sized `maxEntries` limits, Gateway runtime writes use a small high-water buffer and clean back down to the configured cap in batches. This avoids running full store cleanup on every isolated cron session. `theclaw sessions cleanup --enforce` applies the cap immediately.

Preview with `theclaw sessions cleanup --dry-run`.

## Inspecting sessions

- `theclaw status` -- session store path and recent activity.
- `theclaw sessions --json` -- all sessions (filter with `--active <minutes>`).
