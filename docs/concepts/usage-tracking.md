...
- `/usage cost` in chats: local cost summary aggregated from TheClaw session logs.
- CLI: `theclaw status --usage` prints a full per-provider breakdown.
- CLI: `theclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
- **MiniMax**: API key or MiniMax OAuth auth profile. TheClaw treats
  ...
  quota, so TheClaw inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, TheClaw prefers the
...
can supply plugin-specific usage auth logic; otherwise TheClaw falls back to
