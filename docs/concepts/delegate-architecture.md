---
summary: "Delegate architecture: running TheClaw as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run TheClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an TheClaw agent that:
...
TheClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of TheClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.theclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.theclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.theclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through TheClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
theclaw agents add delegate
```

This creates:

- Workspace: `~/.theclaw/workspace-delegate`
- State: `~/.theclaw/agents/delegate/agent`
- Sessions: `~/.theclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.theclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.theclaw/workspace-delegate",
        tools: {
...
~/.theclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.theclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.theclaw/workspace-org",
        agentDir: "~/.theclaw/agents/org-assistant/agent",
...
recall view. TheClaw redacts credential/token-like text, truncates long
