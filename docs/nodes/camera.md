...
TheClaw supports **camera capture** for agent workflows:
...
```bash
theclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
theclaw nodes camera snap --node <id> --facing front
theclaw nodes camera clip --node <id> --duration 3000
theclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`theclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `theclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
theclaw nodes camera list --node <id>            # list camera ids
theclaw nodes camera snap --node <id>            # prints MEDIA:<path>
theclaw nodes camera snap --node <id> --max-width 1280
theclaw nodes camera snap --node <id> --delay-ms 2000
theclaw nodes camera snap --node <id> --device-id <id>
theclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
theclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
theclaw nodes camera clip --node <id> --device-id <id>
theclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `theclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
theclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
