...
- macOS local MLX playback uses the bundled `theclaw-mlx-tts` helper when present, or an executable on `PATH`. Set `THECLAW_MLX_TTS_BIN` to point at a custom helper binary during development.
