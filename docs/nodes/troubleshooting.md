...
```bash
theclaw nodes describe --node <idOrNameOrIp>
theclaw nodes canvas snapshot --node <idOrNameOrIp>
theclaw logs --follow
```
...
theclaw nodes status
theclaw approvals get --node <idOrNameOrIp>
theclaw approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`theclaw approvals get --node ...`), not in the gateway pairing record.
