---
summary: "Gateway config reference for core TheClaw keys, defaults, and links to dedicated subsystem references"
...
Core config reference for `~/.theclaw/theclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Covers the main TheClaw config surfaces and links out when a subsystem has its own deeper reference. Channel- and plugin-owned command catalogs and deep memory/QMD knobs live on their own pages rather than on this one.

Code truth:

- `theclaw config schema` prints the live JSON Schema used for validation and Control UI, with bundled/plugin/channel metadata merged in when available
...
Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — TheClaw uses safe defaults when omitted.
...
TheClaw-managed MCP server definitions live under `mcp.servers` and are
consumed by embedded Pi and other runtime adapters. The `theclaw mcp list`,
...
- `mcp.servers`: named stdio or remote MCP server definitions for runtimes that
  ...
  `type: "http"` is a CLI-native alias that `theclaw mcp set` and
  `theclaw doctor --fix` normalize into the canonical `transport` field.
...
See [MCP](/cli/mcp#theclaw-as-an-mcp-client-registry) and
...
- `install.nodeManager`: node installer preference for `metadata.theclaw.install`
...
- Loaded from `~/.theclaw/extensions`, `<workspace>/.theclaw/extensions`, plus `plugins.load.paths`.
- Discovery accepts native TheClaw plugins plus compatible Codex bundles and Claude bundles, including manifestless Claude default-layout bundles.
...
- `plugins.entries.<id>.config`: plugin-defined config object (validated by native TheClaw plugin schema when available).
- Channel plugin account/runtime settings live under `channels.<id>` and should be described by the owning plugin's manifest `channelConfigs` metadata, not by a central TheClaw option registry.
...
- Enabled Claude bundle plugins can also contribute embedded Pi defaults from `settings.json`; TheClaw applies those as sanitized agent settings, not as raw TheClaw config patches.
...
- `profiles.*.cdpUrl` accepts `http://`, `https://`, `ws://`, and `wss://`.
  Use HTTP(S) when you want TheClaw to discover `/json/version`; use WS(S)
...
- If an externally managed CDP service is reachable through loopback, set that
  profile's `attachOnly: true`; otherwise TheClaw treats the loopback port as a
...
- Local managed `theclaw` profiles auto-assign `cdpPort` and `cdpUrl`; only
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or THECLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `THECLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
- `THECLAW_ALLOW_INSECURE_PRIVATE_WS=1`: client-side process-environment
  break-glass override that allows plaintext `ws://` to trusted private-network
  IPs; default remains loopback-only for plaintext. There is no `theclaw.json`
...
- `THECLAW_APNS_RELAY_BASE_URL` / `THECLAW_APNS_RELAY_TIMEOUT_MS`: temporary env overrides for the relay config above.
- `THECLAW_APNS_RELAY_ALLOW_HTTP=true`: development-only escape hatch for loopback HTTP relay URLs. Production relay URLs should stay on HTTPS.
...
```bash
THECLAW_CONFIG_PATH=~/.theclaw/a.json \
THECLAW_STATE_DIR=~/.theclaw-a \
theclaw gateway --port 19001
```

Convenience flags: `--dev` (uses `~/.theclaw-dev` + port `19001`), `--profile <name>` (uses `~/.theclaw-<name>`).
...
```json5
{
  gateway: {
    tls: {
      enabled: false,
      autoGenerate: false,
      certPath: "/etc/theclaw/tls/server.crt",
      keyPath: "/etc/theclaw/tls/server.key",
      caPath: "/etc/theclaw/tls/ca-bundle.crt",
    },
  },
}
```
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.theclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-theclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `THECLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
{
  canvasHost: {
    root: "~/.theclaw/workspace/canvas",
    liveReload: true,
    // enabled: false, // or THECLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__theclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__theclaw__/a2ui/`
...
- Also serves A2UI at `/__theclaw__/a2ui/`.
...
- Hostname defaults to the system hostname when it is a valid DNS label, falling back to `theclaw`. Override with `THECLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.theclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `theclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.theclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${THECLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- `secrets apply` targets supported `theclaw.json` credential paths.
...
```json5
{
  secrets: {
    providers: {
      ...
      filemain: {
        source: "file",
        path: "~/.theclaw/secrets.json",
        mode: "json",
        timeoutMs: 5000,
      },
      vault: {
        source: "exec",
        command: "/usr/local/bin/theclaw-vault-resolver",
...
- Legacy flat `auth-profiles.json` maps such as `{ "provider": { "apiKey": "..." } }` are not a runtime format; `theclaw doctor --fix` rewrites them to canonical `provider:default` API-key profiles with a `.legacy-flat.*.bak` backup.
...
- Legacy OAuth imports from `~/.theclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/theclaw/theclaw-YYYY-MM-DD.log`.
- Set `logging.file` for a stable path.
- `consoleLevel` bumps to `debug` when `--verbose`.
- `maxFileBytes`: maximum active log file size in bytes before rotation (positive integer; default: `104857600` = 100 MB). TheClaw keeps up to five numbered archives beside the active file.
...
```json5
{
  diagnostics: {
    ...
    cacheTrace: {
      enabled: false,
      filePath: "~/.theclaw/logs/cache-trace.jsonl",
...
- `THECLAW_OTEL_PRELOADED=1`: environment toggle for hosts that already registered a global OpenTelemetry SDK. TheClaw then skips plugin-owned SDK startup/shutdown while keeping diagnostic listeners active.
...
- `cacheTrace.filePath`: output path for cache trace JSONL (default: `$THECLAW_STATE_DIR/logs/cache-trace.jsonl`).
...
- `cli.banner.taglineMode` controls banner tagline style:
  ...
  - `"default"`: fixed neutral tagline (`All your chats, one TheClaw.`).
  - `"off"`: no tagline text (banner title/version still shown).
- To hide the entire banner (not just taglines), set env `THECLAW_HIDE_BANNER=1`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `theclaw doctor --fix` can strip unknown keys).
...
- Paths: resolved relative to the including file, but must stay inside the top-level config directory (`dirname` of `theclaw.json`). Absolute/`../` forms are allowed only when they still resolve inside that boundary.
- TheClaw-owned writes that change only one top-level section backed by a single-file include write through to that included file. For example, `plugins install` updates `plugins: { $include: "./plugins.json5" }` in `plugins.json5` and leaves `theclaw.json` intact.
- Root includes, include arrays, and includes with sibling overrides are read-only for TheClaw-owned writes; those writes fail closed instead of flattening the config.
