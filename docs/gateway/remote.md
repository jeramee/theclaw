...
- Use the macOS app's **Remote over SSH** mode (Settings → General → TheClaw runs).
...
- `theclaw health` and `theclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `theclaw gateway status`, `theclaw gateway health`, `theclaw gateway probe`, and `theclaw gateway call` can also target the forwarded URL via `--url` when needed.

<Note>
Replace `18789` with your configured `gateway.port` (or `--port` or `THECLAW_GATEWAY_PORT`).
</Note>
...
- URL override safety:
  ...
  - Env URL overrides (`THECLAW_GATEWAY_URL`) may use env credentials only (`THECLAW_GATEWAY_TOKEN` / `THECLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `THECLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `THECLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `THECLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `THECLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `THECLAW_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `THECLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as
  break-glass. There is no `theclaw.json` equivalent; this must be process
...
```bash
theclaw config set gateway.remote.token "<your-token>"
```

#### Step 4: create the LaunchAgent

Save this as `~/Library/LaunchAgents/ai.theclaw.ssh-tunnel.plist`:
...
<dict>
    <key>Label</key>
    <string>ai.theclaw.ssh-tunnel</string>
...
```bash
launchctl bootstrap gui/$UID ~/Library/LaunchAgents/ai.theclaw.ssh-tunnel.plist
```

The tunnel will start automatically at login, restart on crash, and keep the forwarded port live.

<Note>
If you have a leftover `com.theclaw.ssh-tunnel` LaunchAgent from an older setup, unload and delete it.
</Note>
...
```bash
launchctl kickstart -k gui/$UID/ai.theclaw.ssh-tunnel
```

Stop the tunnel:

```bash
launchctl bootout gui/$UID/ai.theclaw.ssh-tunnel
```
