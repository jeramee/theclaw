...
TheClaw uses Bonjour (mDNS / DNS‑SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS‑SD records for `_theclaw-gw._tcp` under a dedicated zone
   (example: `theclaw.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

TheClaw supports any discovery domain; `theclaw.internal.` is just an example.
...
```bash
theclaw dns setup --apply
```

This installs CoreDNS and configures it to:

- listen on port 53 only on the gateway’s Tailscale interfaces
- serve your chosen domain (example: `theclaw.internal.`) from `~/.theclaw/dns/<domain>.db`

Validate from a tailnet‑connected machine:

```bash
dns-sd -B _theclaw-gw._tcp theclaw.internal.
dig @<TAILNET_IPV4> -p 53 _theclaw-gw._tcp.theclaw.internal PTR +short
```
...
`_theclaw-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.theclaw/theclaw.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_theclaw-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin; wide-area DNS-SD publishing remains
Gateway-owned.

## Service types

- `_theclaw-gw._tcp` — gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _theclaw-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _theclaw-gw._tcp local.
  ```
...
invalid DNS-label character, TheClaw falls back to `theclaw.local`. Set
`THECLAW_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_theclaw-gw._tcp`.
...
bundled LAN discovery plugin for that TheClaw config:

```bash
theclaw plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `THECLAW_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `THECLAW_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `THECLAW_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `THECLAW_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep THECLAW_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled Bonjour in Docker with
   `THECLAW_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `THECLAW_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. TheClaw retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `THECLAW_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `theclaw plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `theclaw plugins enable bonjour` restores the default LAN discovery plugin.
- `THECLAW_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `THECLAW_DISABLE_BONJOUR`).
- `THECLAW_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When `THECLAW_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.theclaw/theclaw.json` controls the Gateway bind mode.
- `THECLAW_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `THECLAW_SSH_PORT`).
- `THECLAW_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `THECLAW_TAILNET_DNS`).
- `THECLAW_CLI_PATH` overrides the advertised CLI path (legacy: `THECLAW_CLI_PATH`).
