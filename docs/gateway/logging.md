...
- Default rolling log file is under `/tmp/theclaw/` (one file per day): `theclaw-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.theclaw/theclaw.json`:
  - `logging.file`
  - `logging.level`
...
TheClaw can mask sensitive tokens before log or transcript output leaves the
...
`theclaw gateway` supports a per-gateway style switch:
