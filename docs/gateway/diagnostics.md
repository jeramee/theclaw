...
TheClaw can create a local diagnostics zip that is safe to attach to bug
...
Persisted bundles live under `~/.theclaw/logs/stability/` when events exist.

## Useful options

```bash
theclaw gateway diagnostics export \
  --output theclaw-diagnostics.zip \
  --log-lines 5000 \
  --log-bytes 1000000
```
