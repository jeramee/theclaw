---
summary: "Use OpenShell as a managed sandbox backend for TheClaw agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for TheClaw. Instead of running Docker
containers locally, TheClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- TheClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, TheClaw creates an OpenShell
...
- You edit files locally outside TheClaw and want those changes visible in the
...
- When the sandbox is first created, TheClaw seeds the remote workspace from
...
- TheClaw does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside TheClaw after the initial seed, the remote sandbox does **not** see those changes. Use `theclaw sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"theclaw"`  | Sandbox source for first-time create                  |
...
1. TheClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. TheClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `theclaw sandbox` commands
