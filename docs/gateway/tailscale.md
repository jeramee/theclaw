...
TheClaw can auto-configure Tailscale **Serve** (tailnet) or **Funnel** (public) for the
...
- `funnel`: Public HTTPS via `tailscale funnel`. TheClaw requires a shared password.
- `off`: Default (no Tailscale automation).

Status and audit output use **Tailscale exposure** for this TheClaw Serve/Funnel
mode. `off` means TheClaw is not managing Serve or Funnel; it does not mean the
...
- `token` (default when `THECLAW_GATEWAY_TOKEN` is set)
- `password` (shared secret via `THECLAW_GATEWAY_PASSWORD` or config)
...
(`tailscale-user-login`) without supplying a token/password. TheClaw verifies
the identity by resolving the `x-forwarded-for` address via the local Tailscale
daemon (`tailscale whois`) and matching it to the header before accepting it.
TheClaw only treats a request as Serve when it arrives from loopback with
...
Prefer `THECLAW_GATEWAY_PASSWORD` over committing a password to disk.

## CLI examples

```bash
theclaw gateway --tailscale serve
theclaw gateway --tailscale funnel --auth password
```
...
- Set `gateway.tailscale.resetOnExit` if you want TheClaw to undo `tailscale serve`
