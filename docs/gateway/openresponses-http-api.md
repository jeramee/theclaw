...
TheClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`theclaw agent`), so routing/permissions/config match your Gateway.
...
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-theclaw-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-theclaw-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "theclaw"`, `model: "theclaw/default"`, `model: "theclaw/<agentId>"`, or `x-theclaw-agent-id`
- use `x-theclaw-model` when you want to override the selected agent's backend model
- use `x-theclaw-session-key` for explicit session routing
- use `x-theclaw-message-channel` when you want a non-default synthetic ingress channel context
...
- `previous_response_id`: TheClaw reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
TheClaw normalizes common OpenAI-style aliases before those counters reach
