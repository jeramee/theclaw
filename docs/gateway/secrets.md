...
TheClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `THECLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, TheClaw runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). TheClaw validates the resolved target path.
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/TheClaw QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/theclaw"],
...
- TheClaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__THECLAW_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `theclaw.json` refs)
...
When reload-time activation fails after a healthy state, TheClaw enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `theclaw memory` remote-memory paths and `theclaw qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `theclaw status`, `theclaw status --all`, `theclaw channels status`, `theclaw channels resolve`, `theclaw security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `theclaw secrets reload`.
...
<Steps>
  <Step title="Audit current state">
    ```bash
    theclaw secrets audit --check
    ```
  </Step>
  <Step title="Configure SecretRefs">
    ```bash
    theclaw secrets configure
    ```
  </Step>
  <Step title="Re-audit">
    ```bash
...
<AccordionGroup>
  <Accordion title="secrets audit">
    ...
    - plaintext values at rest (`theclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `theclaw.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `theclaw secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `theclaw.json` plus `auth-profiles.json` for one agent scope
    ...
    - `theclaw secrets configure --providers-only`
    - `theclaw secrets configure --skip-provider-setup`
    - `theclaw secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    theclaw secrets apply --from /tmp/theclaw-secrets-plan.json
    theclaw secrets apply --from /tmp/theclaw-secrets-plan.json --allow-exec
    theclaw secrets apply --from /tmp/theclaw-secrets-plan.json --dry-run
    theclaw secrets apply --from /tmp/theclaw-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
TheClaw intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
