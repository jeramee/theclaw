...
read_when:
  - Setting up TheClaw for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

TheClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.theclaw/theclaw.json`.
The active config path must be a regular file. Symlinked `theclaw.json`
layouts are unsupported for TheClaw-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `THECLAW_CONFIG_PATH` directly at the real file.

If the file is missing, TheClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `theclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    theclaw onboard       # full onboarding flow
    theclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.theclaw/theclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
TheClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`theclaw config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`theclaw doctor`, `theclaw logs`, `theclaw health`, `theclaw status`)
- Run `theclaw doctor` to see exact issues
- Run `theclaw doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup.
If `theclaw.json` later fails validation (or drops `gateway.mode`, shrinks
sharply, or has a stray log line prepended), TheClaw preserves the broken file
...
When every validation issue is scoped to `plugins.entries.<id>...`, TheClaw
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `theclaw config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
        ...
        list: [
          {
            id: "main",
            groupChat: {
              mentionPatterns: ["@theclaw", "theclaw"],
            },
          },
        ],
      },
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `theclaw.json`.
    ...
    ```bash
    theclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `THECLAW_APNS_RELAY_BASE_URL` and `THECLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `THECLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-theclaw-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **TheClaw-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      TheClaw updates that included file and leaves `theclaw.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for TheClaw-owned writes instead of
...
The Gateway watches `~/.theclaw/theclaw.json` and applies changes automatically — no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits by restoring the last-known-good config. TheClaw-owned
...
`.clobbered.*` file next to `theclaw.json`, fix the rejected payload, then run
`theclaw config validate`. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-restored-last-known-good-config)
...
When you edit a source file that is referenced through `$include`, TheClaw plans
...
```bash
theclaw gateway call config.get --params '{}'  # capture payload.hash
theclaw gateway call config.patch --params '{
...
TheClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.theclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, TheClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `THECLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${THECLAW_GATEWAY_TOKEN}" } },
