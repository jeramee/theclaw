...
- `theclaw gateway status` shows `Runtime: running`, `Connectivity probe: ok`, and a `Capability: ...` line.
- `theclaw doctor` reports no blocking config/service issues.
- `theclaw channels status --probe` shows live per-account transport status and, where supported, probe/audit results such as `works` or `audit ok`.

## Split brain installs and newer config guard

Use this when a gateway service unexpectedly stops after an update, or logs show that one `theclaw` binary is older than the version that last wrote `theclaw.json`.

TheClaw stamps config writes with `meta.lastTouchedVersion`. Read-only commands can still inspect a config written by a newer TheClaw, but process and service mutations refuse to continue from an older binary. Blocked actions include gateway service start, stop, restart, uninstall, forced service reinstall, service-mode gateway startup, and `gateway --force` port cleanup.

```bash
which theclaw
theclaw --version
theclaw gateway status --deep
theclaw config get meta.lastTouchedVersion
```

<Steps>
  <Step title="Fix PATH">
    Fix `PATH` so `theclaw` resolves to the newer install, then rerun the action.
  </Step>
  ...
  <Step title="Remove stale wrappers">
    Remove stale system package or old wrapper entries that still point at an old `theclaw` binary.
  </Step>
</Steps>

<Warning>
For intentional downgrade or emergency recovery only, set `THECLAW_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1` for the single command. Leave it unset for normal operation.
</Warning>
...
theclaw config get agents.defaults.models
```
...
- TheClaw model runs fail only on normal agent turns
...
theclaw infer model run --model <provider/model> --prompt "hi" --json
theclaw logs --follow
```

Look for:

- direct tiny calls succeed, but TheClaw runs fail only on larger prompts
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `incomplete turn detected ... stopReason=stop payloads=0` → the backend completed the Chat Completions request but returned no user-visible assistant text for that turn. TheClaw retries replay-safe empty OpenAI-compatible turns once; persistent failures usually mean the backend is emitting empty/non-text content or suppressing final-answer text.
    - direct tiny requests succeed, but TheClaw agent runs fail with backend/model crashes (for example Gemma on some `inferrs` builds) → TheClaw transport is likely already correct; the backend is failing on the larger agent-runtime prompt shape.
  ...
  <Accordion title="Fix options">
    ...
    2. Set `compat.supportsTools: false` for models/backends that cannot handle TheClaw's tool schema surface reliably.
    ...
    4. If tiny direct requests keep passing while TheClaw agent turns still crash inside the backend, treat it as an upstream server/model limitation and file a repro there with the accepted payload shape.
  </Accordion>
</AccordionGroup>
...
theclaw status
theclaw channels status --probe
theclaw pairing list --channel <channel> [--account <id>]
theclaw config get channels
theclaw logs --follow
```
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.                                                                                                                                                 | Paste/set token in the client and retry. For dashboard paths: `theclaw config get gateway.auth.token` then paste into Control UI settings.                                                                                                                                              |
...
| `PAIRING_REQUIRED`           | Device identity needs approval. Check `error.details.reason` for `not-paired`, `scope-upgrade`, `role-upgrade`, or `metadata-upgrade`, and use `requestId` / `remediationHint` when present. | Approve pending request: `theclaw devices list` then `theclaw devices approve <requestId>`. Scope/role upgrades use the same flow after you review the requested access.                                                                                                               |
...
If `theclaw devices rotate` / `revoke` / `remove` is denied unexpectedly:

- paired-device token sessions can manage only **their own** device unless the caller also has `operator.admin`
- `theclaw devices rotate --scope ...` can only request operator scopes that the caller session already holds
...
theclaw logs --follow
theclaw doctor
theclaw gateway status --deep   # also scan system-level services
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    - `Gateway start blocked: set gateway.mode=local` or `existing config is missing gateway.mode` → local gateway mode is not enabled, or the config file was clobbered and lost `gateway.mode`. Fix: set `gateway.mode="local"` in your config, or re-run `theclaw onboard --mode local` / `theclaw setup` to restamp the expected local-mode config. If you are running TheClaw via Podman, the default config path is `~/.theclaw/theclaw.json`.
    ...
    - `System-level TheClaw gateway service detected` from doctor → a systemd system unit exists while the user-level service is missing. Remove or disable the duplicate before allowing doctor to install a user service, or set `THECLAW_SERVICE_REPAIR_POLICY=external` if the system unit is the intended supervisor.
    - `Gateway service port does not match current gateway config` → the installed supervisor still pins the old `--port`. Run `theclaw doctor --fix` or `theclaw gateway install --force`, then restart the gateway service.
...
Use this when the Gateway starts, but logs say it restored `theclaw.json`.
...
- A timestamped `theclaw.json.clobbered.*` file beside the active config
...
<AccordionGroup>
  <Accordion title="What happened">
    ...
    - TheClaw preserved the rejected payload as `.clobbered.*`.
    ...
    - If all validation issues were under `plugins.entries.<id>...`, TheClaw would not restore the whole file. Plugin-local failures stay loud while unrelated user settings remain in the active config.

  </Accordion>
  <Accordion title="Inspect and repair">
    ```bash
    CONFIG="$(theclaw config file)"
  ...
  <Accordion title="Common signatures">
    ...
    - `.rejected.*` exists → an TheClaw-owned config write failed schema or clobber checks before commit.
  ...
  <Accordion title="Fix options">
    ...
    2. Copy only the intended keys from `.clobbered.*` or `.rejected.*`, then apply them with `theclaw config set` or `config.patch`.
    3. Run `theclaw config validate` before restarting.
...
Use this when `theclaw gateway probe` reaches something, but still prints a warning block.

```bash
theclaw gateway probe
theclaw gateway probe --json
theclaw gateway probe --ssh user@gateway-host
```
...
theclaw cron status
theclaw cron list
theclaw cron runs --id <jobId> --limit 20
theclaw system heartbeat last
theclaw logs --follow
```
...
<AccordionGroup>
  <Accordion title="Common signatures">
    ...
    - `heartbeat skipped` with `reason=empty-heartbeat-file` → `HEARTBEAT.md` exists but only contains blank lines / markdown headers, so TheClaw skips the model call.
...
theclaw nodes describe --node <idOrNameOrIp>
theclaw approvals get --node <idOrNameOrIp>
theclaw logs --follow
theclaw status
```
...
theclaw browser start --browser-profile theclaw
...
<AccordionGroup>
  ...
  <Accordion title="Element / screenshot / upload signatures">
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `theclaw browser stop --browser-profile <name>` to close the active control session and release Playwright/CDP emulation state without restarting the whole gateway.
...
- [Browser (TheClaw-managed)](/tools/browser)
...
<AccordionGroup>
  <Accordion title="1. Auth and URL override behavior changed">
    ```bash
    theclaw gateway status
    theclaw config get gateway.mode
    theclaw config get gateway.remote.url
    theclaw config get gateway.auth.mode
    ```
  ...
  <Accordion title="2. Bind and auth guardrails are stricter">
    ```bash
    theclaw config get gateway.bind
    theclaw config get gateway.auth.mode
    theclaw config get gateway.auth.token
    theclaw gateway status
    theclaw logs --follow
    ```
