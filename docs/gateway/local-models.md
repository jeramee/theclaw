---
summary: "Run TheClaw on local LLMs (LM Studio, vLLM, LiteLLM, custom OpenAI endpoints)"
read_when:
...
Local is doable, but TheClaw expects large context + strong defenses against prompt injection. Small cards truncate context and leak safety. Aim high: **≥2 maxed-out Mac Studios or equivalent GPU rig (~$30k+)**. A single **24 GB** GPU works only for lighter prompts with higher latency. Use the **largest / full-size model variant you can run**; aggressively quantized or “small” checkpoints raise prompt-injection risk (see [Security](/gateway/security)).

If you want the lowest-friction local setup, start with [LM Studio](/providers/lmstudio) or [Ollama](/providers/ollama) and `theclaw onboard`. This page is the opinionated guide for higher-end local stacks and custom OpenAI-compatible local servers.
...
If `api` is omitted on a custom provider with a `baseUrl`, TheClaw defaults to
...
- hidden TheClaw attribution headers (`originator`, `version`, `User-Agent`)
...
- Some local models emit standalone bracketed tool requests as text, such as
  `[tool_name]` followed by JSON and `[END_TOOL_REQUEST]`. TheClaw promotes
...
- If a model emits JSON, XML, or ReAct-style text that looks like a tool call
  but the provider did not emit a structured invocation, TheClaw leaves it as
...
- If tools appear as assistant text instead of running, for example raw JSON,
  ...
  It overrides TheClaw's default proxy value of `tool_choice: "auto"`.
  Replace `local/my-local-model` with the exact provider/model ref shown by
  `theclaw models list`.

  ```bash
  theclaw config set agents.defaults.models '{"local/my-local-model":{"params":{"extra_body":{"tool_choice":"required"}}}}' --strict-json --merge
  ```

- Some smaller or stricter local backends are unstable with TheClaw's full
  ...
  ```bash
  theclaw infer model run --local --model <provider/model> --prompt "Reply with exactly: pong" --json
  ```
  ...
  ```bash
  theclaw infer model run --gateway --model <provider/model> --prompt "Reply with exactly: pong" --json
  ```
  ...
  If that succeeds but normal TheClaw agent turns fail, first try
...
- If the backend still fails only on larger TheClaw runs, the remaining issue
  is usually upstream model/server capacity or a backend bug, not TheClaw's
  transport layer.
...
- Local server says `terminated`, `ECONNRESET`, or closes the stream mid-turn?
  TheClaw records a low-cardinality `model.call.error.failureKind` plus the
  TheClaw process RSS/heap snapshot in diagnostics. For LM Studio/Ollama
...
- TheClaw warns when the detected context window is below **32k** and blocks below **16k**. If you hit that preflight, raise the server/model context limit or choose a larger model.
- Context errors? Lower `contextWindow` or raise your server limit.
- OpenAI-compatible server returns `messages[].content ... expected a string`?
  Add `compat.requiresStringContent: true` on that model entry.
- Direct tiny `/v1/chat/completions` calls work, but `theclaw infer model run --local`
