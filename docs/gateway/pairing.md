...
theclaw nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), TheClaw treats
...
Pairing state is stored under the Gateway state directory (default `~/.theclaw`):

- `~/.theclaw/nodes/paired.json`
- `~/.theclaw/nodes/pending.json`

If you override `THECLAW_STATE_DIR`, the `nodes/` folder moves with it.
