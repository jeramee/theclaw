---
summary: "Run multiple TheClaw Gateways on one host (isolation, ports, and profiles)"
read_when:
...
theclaw --profile rescue onboard
theclaw --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `theclaw --profile rescue onboard`:
...
`theclaw --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.theclaw/workspace-rescue`)
...
# extra gateway
theclaw --profile ops setup
theclaw --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
theclaw --profile main setup
theclaw --profile main gateway --port 18789
...
theclaw --profile ops gateway install --port 19789
```
...
- `THECLAW_CONFIG_PATH` — per-instance config file
- `THECLAW_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `THECLAW_GATEWAY_PORT` / `--port`).
...
```bash
THECLAW_CONFIG_PATH=~/.theclaw/main.json \
THECLAW_STATE_DIR=~/.theclaw \
theclaw gateway --port 18789

THECLAW_CONFIG_PATH=~/.theclaw/rescue.json \
THECLAW_STATE_DIR=~/.theclaw-rescue \
...
theclaw --profile rescue gateway status --deep
theclaw --profile rescue gateway probe
theclaw status
theclaw --profile rescue status
theclaw --profile rescue browser status
```
