---
summary: "CLI reference for `theclaw cron` (schedule and run background jobs)"
read_when:
...
<Tip>
Run `theclaw cron --help` for the full command surface. See [Cron jobs](/automation/cron-jobs) for the conceptual guide.
</Tip>
...
`theclaw cron list` and `theclaw cron show <job-id>` preview the resolved delivery route. For `channel: "last"`, the preview shows whether the route resolved from the main or current session, or will fail closed.
...
Skipped runs are tracked separately from execution errors. They do not affect retry backoff, but `theclaw cron edit <job-id> --failure-alert-include-skipped` can opt failure alerts into repeated skipped-run notifications.
...
`theclaw cron run` returns as soon as the manual run is queued. Successful responses include `{ ok: true, enqueued: true, runId }`. Use `theclaw cron runs --id <job-id>` to follow the eventual outcome.

<Note>
`theclaw cron run <job-id>` force-runs by default. Use `--due` to keep the older "only run if due" behavior.
</Note>
...
- When a job has `--model` but no fallback list is configured, TheClaw passes an explicit empty fallback override so the agent primary is not appended as a hidden retry target.
...
```bash
theclaw cron edit <job-id> --announce --channel telegram --to "123456789"
```

Disable delivery for an isolated job:

```bash
theclaw cron edit <job-id> --no-deliver
```

Enable lightweight bootstrap context for an isolated job:

```bash
theclaw cron edit <job-id> --light-context
```

Announce to a specific channel:

```bash
theclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
```

Announce to a Telegram forum topic:

```bash
theclaw cron edit <job-id> --announce --channel telegram --to "-1001234567890" --thread-id 42
```
...
theclaw cron run <job-id>
theclaw cron run <job-id> --due
theclaw cron runs --id <job-id> --limit 50
```
...
```bash
theclaw cron edit <job-id> --agent ops
theclaw cron edit <job-id> --clear-agent
theclaw cron edit <job-id> --session current
theclaw cron edit <job-id> --session "session:daily-brief"
```
...
```bash
theclaw cron edit <job-id> --announce --channel slack --to "channel:C1234567890"
theclaw cron edit <job-id> --best-effort-deliver
theclaw cron edit <job-id> --no-best-effort-deliver
theclaw cron edit <job-id> --no-deliver
```
