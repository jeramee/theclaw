---
summary: "CLI reference for `theclaw logs` (tail gateway logs via RPC)"
read_when:
...
`theclaw logs` also accepts the standard Gateway client flags:
...
theclaw logs
theclaw logs --follow
theclaw logs --follow --interval 2000
theclaw logs --limit 500 --max-bytes 500000
theclaw logs --json
theclaw logs --plain
theclaw logs --no-color
theclaw logs --limit 500
theclaw logs --local-time
theclaw logs --follow --local-time
theclaw logs --url ws://127.0.0.1:18789 --token "$THECLAW_GATEWAY_TOKEN"
```

## Notes

- Use `--local-time` to render timestamps in your local timezone.
- If the local loopback Gateway asks for pairing, `theclaw logs` falls back to the configured local log file automatically. Explicit `--url` targets do not use this fallback.
