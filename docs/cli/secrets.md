---
summary: "CLI reference for `theclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `theclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
theclaw secrets reload
theclaw secrets reload --json
theclaw secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan TheClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `theclaw.json` refs)
...
```bash
theclaw secrets audit
theclaw secrets audit --check
theclaw secrets audit --json
theclaw secrets audit --allow-exec
```
...
```bash
theclaw secrets configure
theclaw secrets configure --plan-out /tmp/theclaw-secrets-plan.json
theclaw secrets configure --apply --yes
theclaw secrets configure --providers-only
theclaw secrets configure --skip-provider-setup
theclaw secrets configure --agent ops
theclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `theclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, TheClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
theclaw secrets apply --from /tmp/theclaw-secrets-plan.json --json
```
...
- `theclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.theclaw/.env` known secret keys whose values were migrated
