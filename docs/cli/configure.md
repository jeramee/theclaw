---
summary: "CLI reference for `theclaw configure` (interactive configuration prompts)"
read_when:
...
<Note>
The **Model** section includes a multi-select for the `agents.defaults.models` allowlist (what shows up in `/model` and the model picker). Provider-scoped setup choices merge their selected models into the existing allowlist instead of replacing unrelated providers already in the config. Re-running provider auth from configure preserves an existing `agents.defaults.model.primary`. Use `theclaw models auth login --provider <id> --set-default` or `theclaw models set <model>` when you intentionally want to change the default model.
</Note>
...
<Tip>
`theclaw config` without a subcommand opens the same wizard. Use `theclaw config get|set|unset` for non-interactive edits.
</Tip>

For web search, `theclaw configure --section web` lets you choose a provider
...
theclaw configure --section model --section channels
theclaw configure --section gateway --section daemon
```
