...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an TheClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`theclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to TheClaw channel
conversations instead of hosting an ACP harness session, use
[`theclaw mcp serve`](/cli/mcp) instead.
...
- TheClaw acts as an ACP server
- an IDE or ACP client connects to TheClaw
- TheClaw forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where TheClaw runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to TheClaw: use `theclaw acp`
- TheClaw should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the TheClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
theclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
theclaw acp --url wss://gateway-host:18789 --token-file ~/.theclaw/gateway.token

# Attach to an existing session key
theclaw acp --session agent:main:main

# Attach by label (must already exist)
theclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
theclaw acp --session agent:main:main --reset-session
```
...
theclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.theclaw/gateway.token

# Override the server command (default: theclaw)
theclaw acp client --server "node" --server-args theclaw.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run TheClaw through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass “yolo” switch for that harness session.

## How to use this

Use ACP when an IDE (or other client) speaks Agent Client Protocol and you want
it to drive an TheClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `theclaw acp` over stdio.

Example config (persisted):

```bash
theclaw config set gateway.remote.url wss://gateway-host:18789
theclaw config set gateway.remote.token <token>
```
...
```bash
theclaw acp --session agent:main:main
theclaw acp --session agent:design:main
theclaw acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see TheClaw plugin tools or selected
...
[TheClaw tools MCP bridge](/tools/acp-agents-setup#theclaw-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
TheClaw bot over ACP, use `acpx` with its built-in `theclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx theclaw` at `theclaw acp`.
3. Target the TheClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default TheClaw ACP session
acpx theclaw exec "Summarize the active TheClaw session state."

# Persistent named session for follow-up turns
acpx theclaw sessions ensure --name codex-bridge
acpx theclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my TheClaw work agent for recent context relevant to this repo."
```

If you want `acpx theclaw` to target a specific Gateway and session key every
time, override the `theclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "theclaw": {
      "command": "env THECLAW_HIDE_BANNER=1 THECLAW_SUPPRESS_NOTES=1 theclaw acp --url ws://127.0.0.1:18789 --token-file ~/.theclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local TheClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env THECLAW_HIDE_BANNER=1 THECLAW_SUPPRESS_NOTES=1 node theclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an TheClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “TheClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`THECLAW_GATEWAY_TOKEN`, `THECLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`THECLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `THECLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `theclaw acp client` sets `THECLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `theclaw`).
