---
summary: "TheClaw CLI index: command list, global flags, and links to per-command pages"
read_when:
  - Finding the right `theclaw` subcommand
  - Looking up global flags or output styling rules
title: "CLI reference"
---

`theclaw` is the main CLI entry point. Each core command has either a
...
| Flag                    | Purpose                                                               |
| ----------------------- | --------------------------------------------------------------------- |
| `--dev`                 | Isolate state under `~/.theclaw-dev` and shift default ports         |
| `--profile <name>`      | Isolate state under `~/.theclaw-<name>`                              |
| `--container <name>`    | Target a named container for execution                                |
| `--no-color`            | Disable ANSI colors (`NO_COLOR=1` is also respected)                  |
| `--update`              | Shorthand for [`theclaw update`](/cli/update) (source installs only) |
...
```
theclaw [--dev] [--profile <name>] <command>
  crestodian
  setup
  onboard
  configure
...
Plugins can add additional top-level commands (for example `theclaw voicecall`).
...
`theclaw status --usage` and the Control UI surface provider usage/quota when
