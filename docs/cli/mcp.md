---
summary: "Expose TheClaw channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to TheClaw-backed channels
  - Running `theclaw mcp serve`
  - Managing TheClaw-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`theclaw mcp` has two jobs:

- run TheClaw as an MCP server with `theclaw mcp serve`
- manage TheClaw-owned outbound MCP server definitions with `list`, `show`, `set`, and `unset`

In other words:

- `serve` is TheClaw acting as an MCP server
- `list` / `show` / `set` / `unset` is TheClaw acting as an MCP client-side registry for other MCP servers its runtimes may consume later

Use [`theclaw acp`](/cli/acp) when TheClaw should host a coding harness session itself and route that runtime through ACP.

## TheClaw as an MCP server

This is the `theclaw mcp serve` path.

### When to use `serve`

Use `theclaw mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to TheClaw-backed channel conversations
- you already have a local or remote TheClaw Gateway with routed sessions
- you want one MCP server that works across TheClaw's channel backends instead of running separate per-channel bridges

Use [`theclaw acp`](/cli/acp) instead when TheClaw should host the coding runtime itself and keep the agent session inside TheClaw.

### How it works

`theclaw mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote TheClaw Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `theclaw mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the TheClaw Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `theclaw agent` and `theclaw infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by TheClaw (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when TheClaw already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    theclaw mcp serve --url wss://gateway-host:18789 --token-file ~/.theclaw/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    theclaw mcp serve --url wss://gateway-host:18789 --password-file ~/.theclaw/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    theclaw mcp serve --verbose
    theclaw mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the TheClaw equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying TheClaw channel configuration
...
TheClaw ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `theclaw mcp serve`
...
## TheClaw as an MCP client registry

This is the `theclaw mcp list`, `show`, `set`, and `unset` path.

These commands do not expose TheClaw over MCP. They manage TheClaw-owned MCP server definitions under `mcp.servers` in TheClaw config.

Those saved definitions are for runtimes that TheClaw launches or configures later, such as embedded Pi and other runtime adapters. TheClaw stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write TheClaw config
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Pi consumes TheClaw `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

### Saved MCP server definitions

TheClaw also stores a lightweight MCP server registry in config for surfaces that want TheClaw-managed MCP definitions.

Commands:

- `theclaw mcp list`
- `theclaw mcp show [name]`
- `theclaw mcp set <name> <json>`
- `theclaw mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `theclaw mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
theclaw mcp list
theclaw mcp show context7 --json
theclaw mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
theclaw mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
theclaw mcp unset context7
```
...
| --------------------- | -------------------------------------------------------------------------------------- |
| `url`                 | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`           | Set to `"streamable-http"` to select this transport; when omitted, TheClaw uses `sse` |
| `headers`             | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs` | Per-server connection timeout in ms (optional)                                         |

TheClaw config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `theclaw mcp set` and repaired by `theclaw doctor --fix` in existing config, but `transport` is what embedded Pi consumes directly.
