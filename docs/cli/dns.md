---
summary: "CLI reference for `theclaw dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You’re setting up split DNS for a custom discovery domain (example: theclaw.internal)
title: "DNS"
---
...
```bash
theclaw dns setup
theclaw dns setup --domain theclaw.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `theclaw.internal`)
...
- zone file path
- current tailnet IPs
- recommended `theclaw.json` discovery config
...
- If `--domain` is omitted, TheClaw uses `discovery.wideArea.domain` from config.
