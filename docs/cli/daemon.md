---
summary: "CLI reference for `theclaw daemon` (legacy alias for gateway service management)"
read_when:
  - You still use `theclaw daemon ...` in scripts
...
`theclaw daemon ...` maps to the same service control surface as `theclaw gateway ...` service commands.

## Usage

```bash
theclaw daemon status
theclaw daemon install
theclaw daemon start
theclaw daemon stop
theclaw daemon restart
theclaw daemon uninstall
```
...
Use [`theclaw gateway`](/cli/gateway) for current docs and examples.
