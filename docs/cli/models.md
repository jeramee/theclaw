---
summary: "CLI reference for `theclaw models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
theclaw models list
theclaw models set <model-or-alias>
theclaw models scan
```

`theclaw models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise TheClaw falls back to matching OAuth/API-key
...
the command uses `THECLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR` if set, otherwise the
...
- If you omit the provider, TheClaw resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, TheClaw
...
By default TheClaw tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `THECLAW_AGENT_DIR`/`PI_CODING_AGENT_DIR`)
...
- `no_model`: provider auth exists, but TheClaw could not resolve a probeable
...
```bash
theclaw models auth add
theclaw models auth login --provider <id>
theclaw models auth setup-token --provider <id>
theclaw models auth paste-token
```
...
`theclaw plugins list` to see which providers are installed.
Use `theclaw models auth --agent <id> <subcommand>` to write auth results to a
...
```bash
theclaw models auth login --provider openai-codex --set-default
```
...
- Anthropic note: Anthropic staff told us TheClaw-style Claude CLI usage is allowed again, so TheClaw treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported TheClaw token path, but TheClaw now prefers Claude CLI reuse and `claude -p` when available.
