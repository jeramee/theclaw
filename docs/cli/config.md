---
summary: "CLI reference for `theclaw config` (get/set/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `theclaw.json`: get/set/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `theclaw configure`).

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `theclaw config` without a subcommand.
</ParamField>
...
```bash
theclaw config file
theclaw config --section model
theclaw config --section gateway --section daemon
theclaw config schema
theclaw config get browser.executablePath
...
theclaw config set agents.defaults.heartbeat.every "2h"
theclaw config set agents.list[0].tools.exec.node "node-id-or-name"
theclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
theclaw config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
theclaw config set secrets.providers.vaultfile --provider-source file --provider-path /etc/theclaw/secrets.json --provider-mode json
theclaw config unset plugins.entries.brave.config.webSearch.apiKey
...
theclaw config validate --json
```

### `config schema`

Print the generated JSON schema for `theclaw.json` to stdout as JSON.
...
```bash
theclaw config schema > theclaw.schema.json
```
...
```bash
theclaw config get agents.defaults.workspace
theclaw config get agents.list[0].id
```
...
theclaw config set agents.list[1].tools.exec.node "node-id-or-name"
```
...
```bash
theclaw config set agents.defaults.heartbeat.every "0m"
theclaw config set gateway.port 19001 --strict-json
theclaw config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
theclaw config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
theclaw config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`theclaw config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    theclaw config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    theclaw config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    theclaw config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/theclaw-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    theclaw config set --batch-file ./config-set.batch.json --dry-run
    ```
...
theclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/theclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `theclaw.json`.
...
<Tabs>
  <Tab title="Success example">
    ```json
    {
      "ok": true,
      "operations": 1,
      "configPath": "~/.theclaw/theclaw.json",
      "inputModes": ["builder"],
      "checks": {
        "schema": false,
        "resolvability": true,
...
`theclaw config set` and other TheClaw-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `theclaw.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `theclaw.json` layouts are unsupported for writes; use `THECLAW_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
theclaw config set gateway.reload.mode hybrid --dry-run
theclaw config set gateway.reload.mode hybrid
theclaw config validate
```
...
Whole-file recovery is reserved for globally broken config, such as parse errors, root-level schema failures, legacy migration failures, or mixed plugin and root failures. If validation fails only under `plugins.entries.<id>...`, TheClaw keeps the active `theclaw.json` in place and reports the plugin-local issue instead of restoring `.last-good`. This prevents plugin schema changes or `minHostVersion` skew from rolling back unrelated user settings such as models, providers, auth profiles, channels, gateway exposure, tools, memory, browser, or cron config.

## Subcommands

- `config file`: Print the active config file path (resolved from `THECLAW_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `theclaw config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `theclaw configure` or `theclaw doctor --fix`. `theclaw chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `theclaw config set` or `theclaw configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `theclaw config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `theclaw doctor` or `theclaw doctor --fix` for migration and repair help.
  </Step>
</Steps>
