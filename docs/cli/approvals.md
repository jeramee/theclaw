---
summary: "CLI reference for `theclaw approvals` and `theclaw exec-policy`"
read_when:
...
`theclaw exec-policy` is the local convenience command for keeping the requested
...
- synchronize local `tools.exec.*` and local `~/.theclaw/exec-approvals.json`

Examples:

```bash
theclaw exec-policy show
theclaw exec-policy show --json

theclaw exec-policy preset yolo
theclaw exec-policy preset cautious --json

theclaw exec-policy set --host gateway --security full --ask off --ask-fallback full
```
...
- `theclaw exec-policy show` marks `host=node` scopes as node-managed at runtime instead of deriving an effective policy from the local approvals file

If you need to edit remote host approvals directly, keep using `theclaw approvals set --gateway`
or `theclaw approvals set --node <id|name|ip>`.

## Common commands

```bash
theclaw approvals get
theclaw approvals get --node <id|name|ip>
theclaw approvals get --gateway
```

`theclaw approvals get` now shows the effective exec policy for local, gateway, and node targets:
...
```bash
theclaw approvals set --file ./exec-approvals.json
theclaw approvals set --stdin <<'EOF'
{ version: 1, defaults: { security: "full", ask: "off" } }
EOF
theclaw approvals set --node <id|name|ip> --file ./exec-approvals.json
theclaw approvals set --gateway --file ./exec-approvals.json
```
...
This changes the **host approvals file** only. To keep the requested TheClaw policy aligned, also set:
...
```bash
theclaw approvals allowlist add "~/Projects/**/bin/rg"
theclaw approvals allowlist add --agent main --node <id|name|ip> "/usr/bin/uptime"
theclaw approvals allowlist add --agent "*" "/usr/bin/uname"

theclaw approvals allowlist remove "~/Projects/**/bin/rg"
```
...
- `--node` uses the same resolver as `theclaw nodes` (id, name, ip, or id prefix).
- `--agent` defaults to `"*"`, which applies to all agents.
- The node host must advertise `system.execApprovals.get/set` (macOS app or headless node host).
- Approvals files are stored per host at `~/.theclaw/exec-approvals.json`.
