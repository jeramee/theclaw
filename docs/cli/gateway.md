---
summary: "TheClaw Gateway CLI (`theclaw gateway`) — run, query, and discover gateways"
read_when:
...
The Gateway is TheClaw's WebSocket server (channels, nodes, sessions, hooks). Subcommands in this page live under `theclaw gateway …`.
...
<CardGroup cols={3}>
  ...
  <Card title="Discovery overview" href="/gateway/discovery">
    How TheClaw advertises and finds gateways.
  </Card>
...
<AccordionGroup>
  <Accordion title="Startup behavior">
    - By default, the Gateway refuses to start unless `gateway.mode=local` is set in `~/.theclaw/theclaw.json`. Use `--allow-unconfigured` for ad-hoc/dev runs.
    - `theclaw onboard --mode local` and `theclaw setup` are expected to write `gateway.mode=local`. If the file exists but `gateway.mode` is missing, treat that as a broken or clobbered config and repair it instead of assuming local mode implicitly.
...
<ParamField path="--token <token>" type="string">
  Token override (also sets `THECLAW_GATEWAY_TOKEN` for the process).
</ParamField>
...
- Set `THECLAW_GATEWAY_STARTUP_TRACE=1` to log phase timings during Gateway startup, including per-phase `eventLoopMax` delay and plugin lookup-table timings for installed-index, manifest registry, startup planning, and owner-map work.
...
```bash
theclaw gateway health --url ws://127.0.0.1:18789
```
...
```bash
theclaw gateway usage-cost
theclaw gateway usage-cost --days 7
theclaw gateway usage-cost --json
```
...
```bash
theclaw gateway stability
theclaw gateway stability --type payload.large
theclaw gateway stability --bundle latest
theclaw gateway stability --bundle latest --export
theclaw gateway stability --json
```
...
<AccordionGroup>
  <Accordion title="Privacy and bundle behavior">
    ...
    - On fatal Gateway exits, shutdown timeouts, and restart startup failures, TheClaw writes the same diagnostic snapshot to `~/.theclaw/logs/stability/theclaw-stability-*.json` when the recorder has events. Inspect the newest bundle with `theclaw gateway stability --bundle latest`; `--limit`, `--type`, and `--since-seq` also apply to bundle output.
...
```bash
theclaw gateway diagnostics export
theclaw gateway diagnostics export --output theclaw-diagnostics.zip
theclaw gateway diagnostics export --json
```
...
theclaw gateway status --require-rpc
```
...
```bash
theclaw gateway call status
theclaw gateway call logs.tail --params '{"sinceMs": 60000}'
```
...
responsible for eventually exec'ing `theclaw` or Node with those args.

```bash
cat > ~/.local/bin/theclaw-doppler <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
exec doppler run --project my-project --config production -- theclaw "$@"
EOF
chmod +x ~/.local/bin/theclaw-doppler

theclaw gateway install --wrapper ~/.local/bin/theclaw-doppler --force
theclaw gateway restart
```
...
`THECLAW_WRAPPER` in the service environment for later forced reinstalls, updates, and doctor
repairs.

```bash
THECLAW_WRAPPER="$HOME/.local/bin/theclaw-doppler" theclaw gateway install --force
theclaw doctor
```

To remove a persisted wrapper, clear `THECLAW_WRAPPER` while reinstalling:

```bash
THECLAW_WRAPPER= theclaw gateway install --force
theclaw gateway restart
```
...
<AccordionGroup>
  ...
  <Accordion title="Auth and SecretRefs at install time">
    ...
    - For password auth on `gateway run`, prefer `THECLAW_GATEWAY_PASSWORD`, `--password-file`, or a SecretRef-backed `gateway.auth.password` over inline `--password`.
    - In inferred auth mode, shell-only `THECLAW_GATEWAY_PASSWORD` does not relax install token requirements; use durable config (`gateway.auth.password` or config `env`) when installing a managed service.
...
`gateway discover` scans for Gateway beacons (`_theclaw-gw._tcp`).

- Multicast DNS-SD: `local.`
- Unicast DNS-SD (Wide-Area Bonjour): choose a domain (example: `theclaw.internal.`) and set up split DNS + a DNS server; see [Bonjour](/gateway/bonjour).
...
```bash
theclaw gateway discover --timeout 4000
theclaw gateway discover --json | jq '.beacons[].wsUrl'
```
