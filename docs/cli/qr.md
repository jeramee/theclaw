---
summary: "CLI reference for `theclaw qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
theclaw qr
theclaw qr --setup-code-only
theclaw qr --json
theclaw qr --remote
theclaw qr --url wss://gateway.example/ws
```
...
- With `--remote`, TheClaw requires either `gateway.remote.url` or
