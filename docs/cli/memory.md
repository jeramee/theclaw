---
summary: "CLI reference for `theclaw memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
theclaw memory status
theclaw memory status --deep
theclaw memory status --fix
theclaw memory index --force
theclaw memory search "meeting notes"
theclaw memory search --query "deployment" --max-results 20
theclaw memory promote --limit 10 --min-score 0.75
theclaw memory promote --apply
theclaw memory promote --json --min-recall-count 0 --min-unique-queries 0
theclaw memory promote-explain "router vlan"
theclaw memory promote-explain "router vlan" --json
theclaw memory rem-harness
theclaw memory rem-harness --json
theclaw memory status --json
theclaw memory status --deep --index
theclaw memory status --deep --index --verbose
theclaw memory status --agent main
theclaw memory index --agent main --verbose
```
...
```bash
theclaw memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `theclaw cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
theclaw memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
theclaw memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
