---
summary: "CLI reference for `theclaw node` (headless node host)"
read_when:
...
`theclaw node run` and `theclaw node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `THECLAW_GATEWAY_TOKEN` / `THECLAW_GATEWAY_PASSWORD` are checked first.
...
network, set `THECLAW_ALLOW_INSECURE_PRIVATE_WS=1`. Without it, node startup
fails closed and asks you to use `wss://`, an SSH tunnel, or Tailscale.
This is a process-environment opt-in, not an `theclaw.json` config key.
`theclaw node install` persists it into the supervised node service when it is
...
```bash
theclaw node install --host <gateway-host> --port 18789
```
...
theclaw node uninstall
```

Use `theclaw node run` for a foreground node host (no service).
...
- `theclaw approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, TheClaw prepares a canonical `systemRunPlan`
