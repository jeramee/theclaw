---
summary: "CLI reference for `theclaw agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
theclaw agent --to +15555550123 --message "status update" --deliver
theclaw agent --agent ops --message "Summarize logs"
theclaw agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
...
theclaw agent --agent ops --message "Run locally" --local
```
...
- Marker writes are source-authoritative: TheClaw persists markers from the active source config snapshot, not from resolved runtime secret values.
