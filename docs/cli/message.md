---
summary: "CLI reference for `theclaw message` (send + channel actions)"
read_when:
...
```
theclaw message <subcommand> [flags]
```
...
- `theclaw message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, TheClaw will attempt a live directory lookup when the provider supports it.
...
- `theclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, TheClaw does not force a `default` account SecretRef scope
...
```
theclaw message send --channel discord \
...
```bash
theclaw message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
theclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
theclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
theclaw message poll --channel msteams \
...
```
theclaw message react --channel slack \
...
```
theclaw message react --channel signal \
...
```
theclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
theclaw message send --channel telegram --target @mychat \
