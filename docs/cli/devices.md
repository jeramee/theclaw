---
summary: "CLI reference for `theclaw devices` (device pairing + token rotation/revocation)"
read_when:
...
### `theclaw devices remove <deviceId>`
...
```
theclaw devices remove <deviceId>
theclaw devices remove <deviceId> --json
```

### `theclaw devices clear --yes [--pending]`

Clear paired devices in bulk.

```
theclaw devices clear --yes
theclaw devices clear --yes --pending
theclaw devices clear --yes --pending --json
```

### `theclaw devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, TheClaw only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), TheClaw supersedes the previous pending entry and issues a new `requestId`. Run `theclaw devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
TheClaw keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `theclaw devices list`
or use `theclaw devices approve --latest` to preview the exact upgrade before
approving it.
...
```
theclaw devices approve
theclaw devices approve <requestId>
theclaw devices approve --latest
```
...
### `theclaw devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
theclaw devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `theclaw devices revoke --device <id> --role <role>`
...
```
theclaw devices revoke --device <deviceId> --role node
```
...
```bash
theclaw devices rotate --device <deviceId> --role operator
```
