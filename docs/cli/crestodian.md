...
read_when:
  - You run theclaw with no command and want to understand Crestodian
  - You need a configless-safe way to inspect or repair TheClaw
...
Crestodian is TheClaw's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `theclaw` with no command starts Crestodian in an interactive terminal.
Running `theclaw crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`theclaw tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same TheClaw reference discovery as regular agents. In a Git checkout,
...
[https://github.com/theclaw/theclaw](https://github.com/theclaw/theclaw), with explicit
...
theclaw
theclaw crestodian
theclaw crestodian --json
theclaw crestodian --message "models"
theclaw crestodian --message "validate config"
theclaw crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
theclaw crestodian --message "set default model openai/gpt-5.5" --yes
theclaw onboard --modern
```
...
config set-ref gateway.auth.token env THECLAW_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
- `theclaw.json` is missing
- `theclaw.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`theclaw --help` and `theclaw --version` still use the normal fast paths.
Noninteractive `theclaw` exits with a short message instead of printing root
...
```text
~/.theclaw/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`theclaw onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `theclaw onboard` still runs classic onboarding.
...
planner turn through TheClaw's normal runtime paths. It first uses the
configured TheClaw model. If no configured model is usable yet, it can fall
...
`theclaw tui`, `theclaw chat`, and `theclaw terminal` still open the normal
...
TheClaw: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
TheClaw: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
TheClaw: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `theclaw` for agent handoff.
...
That lane starts with an empty state dir, routes bare `theclaw` to Crestodian,
