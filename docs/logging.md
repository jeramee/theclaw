...
read_when:
  - You need a beginner-friendly overview of TheClaw logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

TheClaw has two main log surfaces:
...
TheClaw keeps up to five numbered archives beside the active file, such as
`theclaw-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.theclaw/theclaw.json`:

```json
{
  "logging": {
    "file": "/path/to/theclaw.log"
  }
}
```
...
If the local loopback Gateway asks for pairing, `theclaw logs` falls back to
...
```bash
theclaw channels logs --channel whatsapp
```
...
TheClaw preserves the original structured log arguments alongside these fields
...
`theclaw gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
theclaw gateway
theclaw gateway --verbose --ws-log compact
theclaw gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.theclaw/theclaw.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/theclaw/theclaw-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`THECLAW_LOG_LEVEL`** environment variable (e.g. `THECLAW_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `theclaw.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `theclaw --log-level debug gateway run`), which overrides the environment variable for that command.
...
TheClaw writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
TheClaw can redact sensitive tokens before they hit console output, file logs,
...
policy. TheClaw still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `THECLAW_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `theclaw doctor` first.
