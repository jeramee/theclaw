---
summary: "Synology Chat webhook setup and TheClaw config"
read_when:
  - Setting up Synology Chat with TheClaw
...
Synology Chat ships as a bundled plugin in current TheClaw releases, so normal
...
```bash
theclaw plugins install ./path/to/local/synology-chat-plugin
```
...
1. Ensure the Synology Chat plugin is available.
   - Current packaged TheClaw releases already bundle it.
   - Older/custom installs can add it manually from a source checkout with the command above.
   - `theclaw onboard` now shows Synology Chat in the same channel setup list as `theclaw channels add`.
   - Non-interactive setup: `theclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your TheClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in TheClaw.
   - Guided: `theclaw onboard`
   - Direct: `theclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
5. Restart gateway and send a DM to the Synology Chat bot.

Webhook auth details:

- TheClaw accepts the outgoing webhook token from `body.token`, then
  `?token=...`, then headers.
...
- Pairing approvals work with:
  - `theclaw pairing list synology-chat`
  - `theclaw pairing approve synology-chat <CODE>`
...
```bash
theclaw message send --channel synology-chat --target 123456 --text "Hello from TheClaw"
theclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```

Media sends are supported by URL-based file delivery.
Outbound file URLs must use `http` or `https`, and private or otherwise blocked network targets are rejected before TheClaw forwards the URL to the NAS webhook.
...
Give each enabled account a distinct `webhookPath`. TheClaw now rejects duplicate exact paths
...
- `Invalid token`:
  ...
  - a reverse proxy stripped the token header before the request reached TheClaw
- `Rate limit exceeded`:
