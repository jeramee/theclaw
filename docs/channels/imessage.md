...
```bash
theclaw pairing list imessage
theclaw pairing approve imessage <CODE>
```
  ...
  <Tab title="Remote Mac over SSH">
    TheClaw only requires a stdio-compatible `cliPath`, so you can point `cliPath` at a wrapper script that SSHes to a remote Mac and runs `imsg`.
...
```json5
{
  channels: {
    imessage: {
      enabled: true,
      cliPath: "~/.theclaw/scripts/imsg-ssh",
...
```
    ...
    If `remoteHost` is not set, TheClaw attempts to auto-detect it by parsing the SSH wrapper script.
    `remoteHost` must be `host` or `user@host` (no spaces or SSH options).
    TheClaw uses strict host-key checking for SCP, so the relay host key must already exist in `~/.ssh/known_hosts`.
...
- Full Disk Access is required for the process context running TheClaw/`imsg` (Messages DB access).
...
<Tabs>
  ...
  <Tab title="Sessions and deterministic replies">
    ...
    If that `chat_id` is explicitly configured under `channels.imessage.groups`, TheClaw treats it as group traffic (group gating + group session isolation).
...
<AccordionGroup>
  <Accordion title="Dedicated bot macOS user (separate iMessage identity)">
    ...
    4. Create SSH wrapper so TheClaw can run `imsg` in that user context.
...
```
  ...
  <Accordion title="DMs are ignored">
    ...
    - pairing approvals (`theclaw pairing list imessage`)
...
```
    ...
    Confirm Full Disk Access + Automation are granted for the process context that runs TheClaw/`imsg`.
