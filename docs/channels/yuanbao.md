...
Tencent Yuanbao is Tencent's AI assistant platform. The TheClaw channel plugin
connects Yuanbao bots to TheClaw over WebSocket so they can interact with users
...
> **Requires TheClaw 2026.4.10 or above.** Run `theclaw --version` to check. Upgrade with `theclaw update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  theclaw channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
theclaw channels login --channel yuanbao
```
...
```bash
theclaw pairing list yuanbao
theclaw pairing approve yuanbao <CODE>
```
...
3. Check logs: `theclaw logs --follow`

### Bot does not receive messages

1. Ensure the bot is created and approved in the Yuanbao app
2. Ensure `appKey` and `appSecret` are correctly configured
3. Ensure the gateway is running: `theclaw gateway status`
4. Check logs: `theclaw logs --follow`
...
3. Restart the gateway: `theclaw gateway restart`
