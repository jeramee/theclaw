...
Nextcloud Talk ships as a bundled plugin in current TheClaw releases, so
...
```bash
theclaw plugins install @theclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
theclaw plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "TheClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
4. Configure TheClaw:
   ...
   ```bash
   theclaw channels add --channel nextcloud-talk \
...
- Approve via:
  - `theclaw pairing list nextcloud-talk`
  - `theclaw pairing approve nextcloud-talk <CODE>`
