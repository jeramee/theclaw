...
read_when:
  - You want TheClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables TheClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current TheClaw releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`theclaw onboard`) and `theclaw channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install it manually.

```bash
theclaw plugins install @theclaw/nostr
```

Use a local checkout (dev workflows):

```bash
theclaw plugins install --link <path-to-local-nostr-plugin>
```
...
theclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "theclaw@example.com",
        lud16: "theclaw@example.com",
      },
    },
  },
}
