...
read_when:
  ...
  - Reviewing TheClaw security posture
title: "Pairing"
---

“Pairing” is TheClaw’s explicit **owner approval** step.
...
Supported channels: `bluebubbles`, `discord`, `feishu`, `googlechat`, `imessage`, `irc`, `line`, `matrix`, `mattermost`, `msteams`, `nextcloud-talk`, `nostr`, `theclaw-weixin`, `signal`, `slack`, `synology-chat`, `telegram`, `twitch`, `whatsapp`, `zalo`, `zalouser`.

### Where the state lives

Stored under `~/.theclaw/credentials/`:
...
3. On your phone, open the TheClaw iOS app → Settings → Gateway.
...
<Note>
An already paired device does not get broader access silently. If it reconnects asking for more scopes or a broader role, TheClaw keeps the existing approval as-is and creates a fresh pending upgrade request. Use `theclaw devices list` to compare the currently approved access with the newly requested access before you approve.
</Note>
...
- The legacy `node.pair.*` API (CLI: `theclaw nodes pending|approve|reject|remove|rename`) is a
