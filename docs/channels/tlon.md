...
Tlon is a decentralized messenger built on Urbit. TheClaw connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current TheClaw releases, so normal packaged
...
```bash
theclaw plugins install @theclaw/tlon
```

Local checkout (when running from a git repo):

```bash
theclaw plugins install ./path/to/local/tlon-plugin
```
...
By default, TheClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `theclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, TheClaw replies in-thread.
