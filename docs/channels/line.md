...
read_when:
  - You want to connect TheClaw to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to TheClaw via the LINE Messaging API. The plugin runs as a webhook
...
LINE ships as a bundled plugin in current TheClaw releases, so normal
...
```bash
theclaw plugins install @theclaw/line
```

Local checkout (when running from a git repo):

```bash
theclaw plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so TheClaw applies strict pre-auth body limits and timeout before verification.
- TheClaw processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
theclaw pairing list line
theclaw pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.theclaw/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. TheClaw validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
