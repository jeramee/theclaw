...
Zalo ships as a bundled plugin in current TheClaw releases, so normal packaged
builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo, install it
manually:

- Install via CLI: `theclaw plugins install @theclaw/zalo`
- Or from a source checkout: `theclaw plugins install ./path/to/local/zalo-plugin`
...
This page reflects current TheClaw behavior for **Zalo Bot Creator / Marketplace bots**.
...
- Approve via:
  - `theclaw pairing list zalo`
  - `theclaw pairing approve zalo <CODE>`
...
This table summarizes current **Zalo Bot Creator / Marketplace bot** behavior in TheClaw.
...
- Example: `theclaw message send --channel zalo --target 123456789 --message "hi"`.

## Troubleshooting

**Bot doesn't respond:**

- Check that the token is valid: `theclaw channels status --probe`
- Verify the sender is approved (pairing or allowFrom)
- Check gateway logs: `theclaw logs --follow`
