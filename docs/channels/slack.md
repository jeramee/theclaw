...
TheClaw sets the Slack SDK client pong timeout to 15 seconds by default for Socket Mode. Override the transport settings only when you need workspace- or host-specific tuning:
...
```json
{
  "display_information": {
    "name": "TheClaw",
    "description": "Slack connector for TheClaw"
  },
  "features": {
    "bot_user": { "display_name": "TheClaw", "always_online": true },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/theclaw",
        "description": "Send a message to TheClaw",
        "should_escape": false
      }
    ]
  },
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `theclaw pairing approve slack <code>`.
...
- If streaming fails mid-reply, TheClaw falls back to normal delivery for remaining payloads.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while TheClaw is processing a reply, then removes it when the run finishes. This is most useful outside of thread replies, which use a default "is typing..." status indicator.
...
<AccordionGroup>
  <Accordion title="Inbound attachments">
    ...
    Downloads use bounded idle and total timeouts. If Slack file retrieval stalls or fails, TheClaw keeps processing the message and falls back to the file placeholder.
...
- The interactive callback values are TheClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, TheClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
```
  ...
  <Accordion title="Socket mode not connecting">
    ...
    If `theclaw channels status --probe --json` shows `botTokenStatus` or
