...
<Note>
Requires TheClaw 2026.4.25 or above. Run `theclaw --version` to check. Upgrade with `theclaw update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  theclaw channels login --channel feishu
  ```
...
```bash
theclaw pairing list feishu
theclaw pairing approve feishu <CODE>
```
...
5. Ensure the gateway is running: `theclaw gateway status`
6. Check logs: `theclaw logs --follow`
...
of raw `file_key` JSON. When `tools.media.audio` is configured, TheClaw
...
conversion fails, TheClaw falls back to a file attachment and logs the reason.
...
topic session key. Normal group replies that TheClaw turns into threads keep
