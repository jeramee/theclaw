...
TheClaw routes replies **back to the channel where a message came from**. The
...
TheClaw infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, TheClaw still records inbound session metadata, but it
...
when a guarded path must not create a new TheClaw session. In that mode,
TheClaw may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when TheClaw would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.theclaw/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.theclaw`):

- `~/.theclaw/agents/<agentId>/sessions/sessions.json`
