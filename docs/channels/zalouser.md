...
read_when:
  - Setting up Zalo Personal for TheClaw
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside TheClaw.
...
Zalo Personal ships as a bundled plugin in current TheClaw releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install it manually:

- Install via CLI: `theclaw plugins install @theclaw/zalouser`
- Or from a source checkout: `theclaw plugins install ./path/to/local/zalouser-plugin`
...
- `theclaw pairing list zalouser`
- `theclaw pairing approve zalouser <code>`
...
- On startup, TheClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, TheClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in TheClaw state. Example:
...
- TheClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, TheClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `theclaw channels logout --channel zalouser && theclaw channels login --channel zalouser`
...
- The channel now runs fully in TheClaw without external CLI binaries.
