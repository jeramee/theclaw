...
TheClaw has three public release lanes:
...
- Every stable TheClaw release ships the npm package and macOS app together;
...
6. Run `TheClaw NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For beta, tag `vYYYY.M.D-beta.N`, publish with npm dist-tag `beta`, then run
   post-publish package acceptance against the published `theclaw@YYYY.M.D-beta.N`
   or `theclaw@beta` package. If a pushed or published beta needs a fix, cut
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `TheClaw Release Checks` for install smoke, package acceptance, Docker
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `theclaw@beta`, `theclaw@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=theclaw@beta -f suite_profile=product -f telegram_mode=mock-openai`
  Common profiles:
...
- Release checks now run in a separate manual workflow:
  `TheClaw Release Checks`
- `TheClaw Release Checks` also runs the QA Lab mock parity gate plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `TheClaw Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/theclaw-cross-os-release-checks-reusable.yml` directly
...
- `TheClaw Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an TheClaw branch or release tag
- `TheClaw NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `THECLAW_LIVE_TEST=1 THECLAW_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Run `RELEASE_TAG=vYYYY.M.D node --import tsx scripts/theclaw-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/theclaw-npm-postpublish-verify.ts YYYY.M.D`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `THECLAW_NPM_TELEGRAM_PACKAGE_SPEC=theclaw@YYYY.M.D-beta.N THECLAW_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex THECLAW_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `THECLAW_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `theclaw/releases-private/.github/workflows/theclaw-npm-dist-tags.yml`
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=theclaw@YYYY.M.D-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `TheClaw Release Checks`, and
optionally dispatches standalone post-publish Telegram E2E when
`npm_telegram_package_spec` is set. `TheClaw Release Checks` then fans out
...
`TheClaw Release Checks` uses the trusted workflow ref to resolve the target
...
gh workflow run full-release-validation.yml \
  ...
  -f mode=both \
  -f evidence_package_spec=theclaw@YYYY.M.D-beta.N \
  -f npm_telegram_package_spec=theclaw@YYYY.M.D-beta.N \
...
The Docker box lives in `TheClaw Release Checks` through
`theclaw-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `TheClaw Release Checks`. It is the agentic
...
`scripts/resolve-theclaw-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `theclaw@beta`, `theclaw@latest`, or an exact TheClaw release
  version
...
`TheClaw Release Checks` runs Package Acceptance with `source=ref`,
...
`TheClaw NPM Release` accepts these operator-controlled inputs:
...
`TheClaw Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an TheClaw branch or
  release tag.
...
- For `TheClaw NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `TheClaw Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `TheClaw NPM Release` with `preflight_only=true`
...
6. Run `TheClaw NPM Release` again with `preflight_only=false`, the same
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/theclaw/theclaw/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/theclaw/theclaw/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/theclaw-npm-release.yml`](https://github.com/theclaw/theclaw/blob/main/.github/workflows/theclaw-npm-release.yml)
- [`.github/workflows/theclaw-release-checks.yml`](https://github.com/theclaw/theclaw/blob/main/.github/workflows/theclaw-release-checks.yml)
- [`.github/workflows/theclaw-cross-os-release-checks-reusable.yml`](https://github.com/theclaw/theclaw/blob/main/.github/workflows/theclaw-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-theclaw-package-candidate.mjs`](https://github.com/theclaw/theclaw/blob/main/scripts/resolve-theclaw-package-candidate.mjs)
- [`scripts/theclaw-npm-release-check.ts`](https://github.com/theclaw/theclaw/blob/main/scripts/theclaw-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/theclaw/theclaw/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/theclaw/theclaw/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`theclaw/maintainers/release/README.md`](https://github.com/theclaw/maintainers/blob/main/release/README.md)
