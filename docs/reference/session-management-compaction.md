...
TheClaw manages sessions end-to-end across these areas:
...
TheClaw is designed around a single **Gateway process** that owns session state.
...
TheClaw persists sessions in two layers:
...
- Transcripts: `~/.theclaw/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

TheClaw resolves these via `src/config/sessions.ts`.
...
Normal Gateway writes batch `maxEntries` cleanup for production-sized caps, so a store may briefly exceed the configured cap before the next high-water cleanup rewrites it back down. `theclaw sessions cleanup --enforce` still applies the configured cap immediately.

TheClaw no longer creates automatic `sessions.json.bak.*` rotation backups during Gateway writes. The legacy `session.maintenance.rotateBytes` key is ignored and `theclaw doctor --fix` removes it from older configs.
...
In `mode: "warn"`, TheClaw reports potential evictions but does not mutate the store/files.

Run maintenance on demand:

```bash
theclaw sessions cleanup --dry-run
theclaw sessions cleanup --enforce
```
...
- `cron.runLog.maxBytes` + `cron.runLog.keepLines` prune `~/.theclaw/cron/runs/<jobId>.jsonl` files (defaults: `2_000_000` bytes and `2000` lines).
...
TheClaw intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
When TheClaw splits a long transcript into compaction chunks, it keeps
assistant tool calls paired with their matching `toolResult` entries.

- If the token-share split lands between a tool call and its result, TheClaw
...
- If a trailing tool-result block would otherwise push the chunk over target,
  TheClaw preserves that pending tool block and keeps the unsummarized tail
  intact.
...
These are Pi runtime semantics (TheClaw consumes the events, but Pi decides when to compact).

TheClaw can also trigger a preflight local compaction before opening the next
...
reopen cost, not raw archival: TheClaw still runs normal semantic compaction,
...
TheClaw also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, TheClaw bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, TheClaw leaves it alone.
...
- When `agents.defaults.compaction.truncateAfterCompaction` is enabled,
  TheClaw rotates the active transcript to a compacted successor JSONL after
...
- If the provider fails or returns an empty result, TheClaw falls back to built-in LLM summarization automatically.
...
- `theclaw status` (CLI)
- `theclaw sessions` / `sessions --json`
...
TheClaw supports “silent” turns for background tasks where the user should not see intermediate output.
...
- TheClaw strips/suppresses this in the delivery layer.
...
As of `2026.1.10`, TheClaw also suppresses **draft/typing streaming** when a
...
TheClaw uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but TheClaw’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `theclaw status`.
