---
summary: "How TheClaw builds prompt context and reports token usage + costs"
read_when:
...
TheClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

TheClaw assembles its own system prompt on every run. It includes:
...
For images, TheClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from TheClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `theclaw status --usage` and `theclaw channels list` show
...
available; otherwise TheClaw falls back to matching OAuth/API-key credentials
...
TheClaw keeps provider usage accounting separate from the current context
...
`cacheWrite`. If pricing is missing, TheClaw shows tokens only. OAuth tokens
...
Provider prompt caching only applies within the cache TTL window. TheClaw can
...
Anthropic's 1M context window is currently beta-gated. TheClaw can inject the
...
TheClaw skips the `context-1m-*` beta header because Anthropic currently
