...
This is the full reference for `theclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.theclaw/theclaw.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `theclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - **Anthropic setup-token**: still available in onboarding/configure, though TheClaw now prefers Claude CLI reuse when available.
    ...
    - Auth profiles live in `~/.theclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth). `~/.theclaw/credentials/oauth.json` is legacy import-only.
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    that agent's `auth-profiles.json` (for example
    `~/.theclaw/agents/<agentId>/agent/auth-profiles.json`, or the matching
    `$THECLAW_STATE_DIR/...` path) to the gateway host. `credentials/oauth.json`
    is only a legacy import source.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.theclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `theclaw pairing approve <channel> <code>` or use allowlists.
  ...
  <Step title="Web search">
    ...
    - Configure later: `theclaw configure --section web`.
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `theclaw health`.
    - Tip: `theclaw status --deep` adds the live gateway health probe to status output, including channel probes when supported (requires a reachable gateway).
...
```bash
export THECLAW_GATEWAY_TOKEN="your-token"
theclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env THECLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.theclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.theclaw/theclaw.json`:
...
`theclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.theclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.theclaw/agents/<agentId>/sessions/`.
