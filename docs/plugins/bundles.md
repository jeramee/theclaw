---
summary: "Install and use Codex, Claude, and Cursor bundles as TheClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how TheClaw maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

TheClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
TheClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native TheClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native TheClaw plugins, TheClaw
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    theclaw plugins install ./my-bundle

    # Archive
    theclaw plugins install ./my-bundle.tgz
...
## What TheClaw maps from bundles

Not every bundle feature runs in TheClaw today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal TheClaw skills                                           | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                  | Claude, Cursor |
| Hook packs    | TheClaw-style `HOOK.md` + `handler.ts` layouts                                             | Codex          |
...
- bundle skill roots load as normal TheClaw skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal TheClaw skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal TheClaw hook-pack
...
- TheClaw merges bundle MCP config into the effective embedded Pi settings as
  `mcpServers`
- TheClaw exposes supported bundle MCP tools during embedded Pi agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, TheClaw uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in TheClaw config. `theclaw mcp set` and `theclaw doctor --fix` normalize the common alias.
...
TheClaw registers bundle MCP tools with provider-safe names in the form
...
- Claude `settings.json` is imported as default embedded Pi settings when the
  bundle is enabled
- TheClaw sanitizes shell override keys before applying them
...
- TheClaw loads `.lsp.json` plus any manifest-declared `lspServers` paths
...
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `theclaw plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but TheClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit TheClaw best when they use skill roots and TheClaw-style
...
TheClaw checks for native plugin format first:

1. `theclaw.plugin.json` or valid `package.json` with `theclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, TheClaw uses the native path. This prevents
dual-format packages from being partially installed as bundles.

## Runtime dependencies and cleanup

- Bundled plugin runtime dependencies ship inside the TheClaw package under
  `dist/*`. TheClaw does **not** run `npm install` at startup for bundled
...
- TheClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `theclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. TheClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    TheClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
