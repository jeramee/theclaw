...
The plugin SDK is exposed as a set of narrow subpaths under `theclaw/plugin-sdk/`.
...
| `plugin-sdk/config-schema`                | `TheClawSchema`                                                                                                                                                             |
...
| `plugin-sdk/plugin-test-api`              | Minimal `TheClawPluginApi` mock builder for direct plugin registration unit tests                                                                                           |
...
<AccordionGroup>
  <Accordion title="Channel subpaths">
    ...
    | `plugin-sdk/config-schema` | Root `theclaw.json` Zod schema export (`TheClawSchema`) |
    ...
    | `plugin-sdk/bundled-channel-config-schema` | Bundled TheClaw channel config schemas for maintained bundled plugins only |
    ...
    | `plugin-sdk/discord` | Deprecated Discord compatibility facade for published `@theclaw/discord@2026.3.13`; new plugins should use generic channel SDK subpaths |
  ...
  <Accordion title="Runtime and storage subpaths">
    ...
    | `plugin-sdk/config-types` | Type-only config surface for plugin config shapes such as `TheClawConfig` and channel/provider config types |
    ...
    | `plugin-sdk/agent-harness` | Experimental trusted-plugin surface for low-level agent harnesses: harness types, active-run steer/abort helpers, TheClaw tool bridge helpers, runtime-plan tool policy helpers, terminal outcome classification, tool progress formatting/detail helpers, and attempt result utilities |
