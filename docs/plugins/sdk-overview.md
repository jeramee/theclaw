...
read_when:
  ...
  - You want a reference for all registration methods on TheClawPluginApi
...
prefer `theclaw/plugin-sdk/channel-core`; keep `theclaw/plugin-sdk/core` for
...
`theclaw.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. TheClaw's
...
<Warning>
  ...
  `theclaw/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`theclaw/plugin-sdk/discord` is also kept as a deprecated compatibility facade
for the published `@theclaw/discord@2026.3.13` package. Do not copy that import
...
The `register(api)` callback receives an `TheClawPluginApi` object with these
methods:
...
cannot register this middleware; keep normal TheClaw plugin hooks for work
...
Gateway on a local discovery transport such as mDNS/Bonjour. TheClaw calls the
...
- User config still wins. TheClaw merges `agents.defaults.cliBackends.<id>` over the
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `theclaw/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep TheClaw as the source of truth for due checks and execution.
...
| `api.config`             | `TheClawConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `theclaw/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when TheClaw is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through the TheClaw SDK
...
- **`@theclaw/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@theclaw/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `theclaw/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `theclaw/plugin-sdk/speech`, `.../provider-model-shared`, or another
