---
summary: "Set up Codex Computer Use for Codex-mode TheClaw agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode TheClaw agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. TheClaw
...
Use this page when TheClaw is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## TheClaw.app and Peekaboo

TheClaw.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want TheClaw.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode TheClaw agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an TheClaw node and exposes mobile
...
TheClaw-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through TheClaw's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the TheClaw command:

```bash
cua-driver mcp-config --client theclaw
```

or register the stdio server yourself:

```bash
theclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
```
...
available as a normal TheClaw MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. TheClaw
...
With this config, TheClaw checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, TheClaw asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, TheClaw also tries to
...
plugin command surface is available. These are TheClaw chat/runtime commands,
not `theclaw codex ...` CLI subcommands:
...
TheClaw uses the same app-server API that Codex itself exposes. The
...
During install, TheClaw polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, TheClaw prefers
...
`computer-use` is registered, TheClaw tries to add the standard bundled
...
TheClaw reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If TheClaw says Computer Use
...
TheClaw intentionally fails closed when `computerUse.enabled` is true. A
...
fresh TheClaw session.

**A Computer Use tool says `Native hook relay unavailable`.** The Codex-native
tool hook reached TheClaw with a stale or missing relay registration. Start a
fresh TheClaw session with `/new` or `/reset`. If it keeps happening, restart
