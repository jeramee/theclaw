...
read_when:
  - You see the THECLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the THECLAW_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before TheClaw 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external TheClaw plugin
---

TheClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`theclaw/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`theclaw/plugin-sdk/infra-runtime`** — a broad runtime helper barrel that
...
- **`theclaw/plugin-sdk/config-runtime`** — a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`theclaw/extension-api`** — a bridge that gave plugins direct access to
...
TheClaw does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`theclaw/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `theclaw/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `TheClawConfig` | `theclaw/plugin-sdk/config-types` |
    | Already-loaded config assertions and plugin-entry config lookup | `theclaw/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `theclaw/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `theclaw/plugin-sdk/config-mutation` |
    | Session store helpers | `theclaw/plugin-sdk/session-store-runtime` |
    | Markdown table config | `theclaw/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `theclaw/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `theclaw/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `theclaw/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `theclaw/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `theclaw/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "theclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "theclaw/plugin-sdk/compat";
    ...
    import { resolveControlCommandGate } from "theclaw/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedPiAgent } from "theclaw/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `theclaw/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `theclaw/plugin-sdk/system-event-runtime` |
    | Heartbeat event and visibility helpers | `theclaw/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `theclaw/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `theclaw/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `theclaw/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `theclaw/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `theclaw/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `theclaw/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `theclaw/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `theclaw/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `theclaw/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `theclaw/plugin-sdk/error-runtime` |
    | Transport readiness waits | `theclaw/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `theclaw/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `theclaw/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `theclaw/plugin-sdk/number-runtime` |
    | Process-local async lock | `theclaw/plugin-sdk/async-lock-runtime` |
    | File locks | `theclaw/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `theclaw/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `TheClawSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | TheClaw-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@theclaw/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`theclaw/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`theclaw/plugin-sdk/command-status`)**: same signatures, same
    exports — just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "theclaw/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "theclaw/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `theclaw/plugin-sdk/channel-inbound` or
    `theclaw/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `theclaw/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `theclaw/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `theclaw/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `theclaw/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    TheClaw no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. TheClaw downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="TheClawSchemaType alias → TheClawConfig">
    `TheClawSchemaType` re-exported from `theclaw/plugin-sdk` is now a
    one-line alias for `TheClawConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { TheClawSchemaType } from "theclaw/plugin-sdk";
    // After
    import type { TheClawConfig } from "theclaw/plugin-sdk/config-schema";
    ```
...
```bash
THECLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 theclaw gateway run
THECLAW_SUPPRESS_EXTENSION_API_WARNING=1 theclaw gateway run
```
