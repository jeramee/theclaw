...
read_when:
  - You maintain an TheClaw plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

TheClaw keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core TheClaw repo as a separate
...
```sh
theclaw-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. TheClaw
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `theclaw` package.

### Maintainer acceptance lane

Use Blacksmith Testbox for the installable-package acceptance lane when validating
the external inspector against TheClaw plugin packages. Run it from a clean
TheClaw checkout after the package is built:

```sh
blacksmith testbox warmup ci-check-testbox.yml --ref main --idle-timeout 90
blacksmith testbox run --id <tbx_id> "pnpm install && pnpm build && npm exec --yes @theclaw/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
blacksmith testbox run --id <tbx_id> "npm exec --yes @theclaw/plugin-inspector@0.1.0 -- ./extensions/discord --json"
blacksmith testbox run --id <tbx_id> "npm exec --yes @theclaw/plugin-inspector@0.1.0 -- <clawhub-plugin-dir> --json"
...
TheClaw should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `theclaw/plugin-sdk/compat`
- legacy hook-only plugin shapes and `before_agent_start`
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `theclaw/extension-api`,
  `theclaw/plugin-sdk/channel-runtime`, `theclaw/plugin-sdk/command-auth`
  status builders, `theclaw/plugin-sdk/test-utils` (replaced by focused
  `theclaw/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `TheClawSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `theclaw/plugin-sdk/channel-route`
...
- deprecated implicit startup sidecar loading for plugins that have not declared
  ...
  `THECLAW_DISABLE_LEGACY_IMPLICIT_STARTUP_SIDECARS=1`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `theclaw plugins registry --refresh` and
  `theclaw doctor --fix`
