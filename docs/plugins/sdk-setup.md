...
read_when:
  ...
  - You are defining plugin config schemas or package.json theclaw metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`theclaw.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `theclaw` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/theclaw-my-channel",
      "version": "1.0.0",
      "type": "module",
      "theclaw": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / ClawHub baseline">
    ```json theclaw-clawhub-package.json
    {
      "name": "@myorg/theclaw-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "theclaw": {
        "extensions": ["./index.ts"],
...
`theclaw.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`theclaw.install` is package metadata, not manifest metadata.
...
| `minHostVersion`             | `string`             | Minimum supported TheClaw version in the form `>=x.y.z`.                        |
...
<AccordionGroup>
  ...
  <Accordion title="Pinned npm installs">
    ...
    ```json
    {
      "theclaw": {
        "install": {
          "npmSpec": "@wecom/wecom-theclaw-plugin@1.2.3",
...
When enabled, TheClaw loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `theclaw.plugin.json` in the package root. TheClaw uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to TheClaw",
  "configSchema": {
    "type": "object",
...
The `setup-entry.ts` file is a lightweight alternative to `index.ts` that TheClaw loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `theclaw/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When TheClaw uses setupEntry instead of the full entry">
...
```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "theclaw/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `theclaw.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `theclaw onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "theclaw/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `theclaw/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `theclaw/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `theclaw/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "theclaw/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/theclaw-my-channel",
      docsPath: "/channels/my-channel",
    });
...
<Tabs>
  <Tab title="Auto (ClawHub then npm)">
    ```bash
    theclaw plugins install @myorg/theclaw-my-plugin
    ```

    TheClaw tries ClawHub first and falls back to npm automatically.

  </Tab>
  <Tab title="ClawHub only">
    ```bash
    theclaw plugins install clawhub:@myorg/theclaw-my-plugin
    ```
  </Tab>
...
```bash
theclaw plugins install <package-name>
```

<Info>
For npm-sourced installs, `theclaw plugins install` runs project-local `npm install --ignore-scripts` (no lifecycle scripts), ignoring inherited global npm install settings. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>

<Note>
Bundled TheClaw-owned plugins are the only startup repair exception: when a packaged install sees one enabled by plugin config, legacy channel config, or its bundled default-enabled manifest, startup installs that plugin's missing runtime dependencies before import. Third-party plugins should not rely on startup installs; keep using the explicit plugin installer.
</Note>
