...
Plugin hooks are in-process extension points for TheClaw plugins. Use them
...
current plugin options; TheClaw injects it per handler without mutating the
...
- TheClaw strips `toolResult.details` before provider replay and compaction
...
`before_agent_start` and `agent_end` include `event.runId` when TheClaw can
...
TheClaw continues; it does not cancel plugin-owned network work unless the
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when TheClaw can derive a
...
Codex native `Stop` hooks are relayed into this hook as TheClaw
...
reach the next model turn exactly once. TheClaw drains queued injections before
...
context when syncing external wake schedulers, and keep TheClaw as the
