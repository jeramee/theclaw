...
Message presentation is TheClaw's shared contract for rich outbound chat UI.
...
```bash
theclaw message send --channel slack \
  --target channel:C123 \
...
```bash
theclaw message send --channel telegram \
...
Use helpers from `theclaw/plugin-sdk/interactive-runtime` when bridging older
code:
