...
read_when:
  - You are building an TheClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native TheClaw plugin manifest** only.
...
TheClaw auto-detects those bundle layouts too, but they are not validated
against the `theclaw.plugin.json` schema described here.

For compatible bundles, TheClaw currently reads bundle metadata plus declared
...
TheClaw runtime expectations.

Every native TheClaw plugin **must** ship a `theclaw.plugin.json` file in the
**plugin root**. TheClaw uses this manifest to validate configuration
...
`theclaw.plugin.json` is the metadata TheClaw reads **before it loads your
...
- QA runner metadata the shared `theclaw qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; TheClaw still reads this during the deprecation window.                                               |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                        |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that TheClaw can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                          |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `theclaw qa` host before plugin runtime loads.                                                                                                                                    |
...
TheClaw reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                        | User-facing label. If omitted, TheClaw falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. TheClaw
...
Every plugin should set `activation.onStartup` intentionally as TheClaw moves
...
`THECLAW_DISABLE_LEGACY_IMPLICIT_STARTUP_SIDECARS=1` to disable only that
...
the shared `theclaw qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `theclaw qa`, for example `matrix`.    |
...
TheClaw also includes `setup.providers[].envVars` in generic provider auth and
...
TheClaw can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. TheClaw treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `theclaw.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
TheClaw reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that TheClaw can
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `theclaw migrate`.          |
...
TheClaw reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, TheClaw can still load the plugin, but
...
the same defaults through `package.json#theclaw.channel.commands` alongside
...
When `channels.chat` is configured, TheClaw considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, TheClaw disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, TheClaw
...
Use `modelSupport` when TheClaw should infer your provider plugin from
...
TheClaw applies this precedence:
...
Use `modelCatalog` when TheClaw should know provider model metadata before
...
provider-filtered list uses an alias, TheClaw can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching TheClaw model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when TheClaw
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the TheClaw provider id, for example `z-ai` for a `zai` provider. |
...
### TheClaw Provider Index

The TheClaw Provider Index is TheClaw-owned preview metadata for providers
...
4. TheClaw Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `theclaw doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `theclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `theclaw` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if TheClaw must know it before loading plugin code, put it in `theclaw.plugin.json`
...
`theclaw` block instead of `theclaw.plugin.json`.

Important examples:

| Field                                                             | What it means                                                                                                                                                                        |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `theclaw.extensions`                                             | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `theclaw.runtimeExtensions`                                      | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `theclaw.setupEntry`                                             | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `theclaw.runtimeSetupEntry`                                      | Declares the built JavaScript setup entrypoint for installed packages. Must stay inside the plugin package directory.                                                                |
| `theclaw.channel`                                                | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `theclaw.channel.commands`                                       | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `theclaw.channel.configuredState`                                | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `theclaw.channel.persistedAuthState`                             | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `theclaw.install.npmSpec` / `theclaw.install.localPath`         | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `theclaw.install.defaultChoice`                                  | Preferred install path when multiple install sources are available.                                                                                                                  |
| `theclaw.install.minHostVersion`                                 | Minimum supported TheClaw host version, using a semver floor like `>=2026.3.22`.                                                                                                    |
| `theclaw.install.expectedIntegrity`                              | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `theclaw.install.allowInvalidConfigRecovery`                     | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `theclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen` | Lets setup-only channel surfaces load before the full channel plugin during startup.                                                                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#theclaw.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `theclaw.plugin.json`.

`theclaw.install.minHostVersion` is enforced during install and manifest
...
`"npmSpec": "@wecom/wecom-theclaw-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `theclaw.setupEntry` when status, channel list,
...
entrypoint fields. For example, `theclaw.runtimeExtensions` cannot make an
escaping `theclaw.extensions` path loadable.

`theclaw.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `theclaw doctor --fix`.

`theclaw.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`theclaw.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
TheClaw discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.

Precedence, highest to lowest:

1. **Config-selected** — a path explicitly pinned in `plugins.entries.<id>`
2. **Bundled** — plugins shipped with TheClaw
3. **Global install** — plugins installed into the global TheClaw plugin root
...
- The manifest is **required for native TheClaw plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `TheClawPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
