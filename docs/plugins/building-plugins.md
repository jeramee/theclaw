---
summary: "Create your first TheClaw plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
read_when:
  - You want to create a new TheClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to TheClaw
---
...
You do not need to add your plugin to the TheClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`theclaw plugins install <package-name>`. TheClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect TheClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
`theclaw/plugin-sdk/channel-setup`. It produces a setup adapter + wizard pair
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json theclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to TheClaw",
      "activation": {
        "onStartup": true
      },
      "configSchema": {
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** validate and publish with ClawHub, then install:
    ...
    TheClaw also checks ClawHub before npm for bare package specs like
    `@myorg/theclaw-my-plugin`.
...
plugins should prefer regular TheClaw plugin hooks unless TheClaw grows an
...
The `/approve` command handles both exec and plugin approvals with bounded fallback: when an exec approval id is not found, TheClaw retries the same id through plugin approvals. Plugin approval forwarding can be configured independently via `approvals.plugin` in config.

If custom approval plumbing needs to detect that same bounded fallback case,
prefer `isApprovalNotFoundError` from `theclaw/plugin-sdk/error-runtime`
...
Always import from focused `theclaw/plugin-sdk/<subpath>` paths:
...
import { ... } from "theclaw/plugin-sdk";
```
...
package-root seam instead of promoting it into `theclaw/plugin-sdk/*`.

Some generated `theclaw/plugin-sdk/<bundled-id>` helper seams still exist for
...
<Check>**package.json** has correct `theclaw` metadata</Check>
<Check>**theclaw.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [theclaw/theclaw](https://github.com/theclaw/theclaw/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official TheClaw X account [@theclaw](https://x.com/theclaw) for release announcements.
