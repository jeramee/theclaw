...
An **agent harness** is the low level executor for one prepared TheClaw agent
...
runtime and the normal TheClaw provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the TheClaw
  session transcript
...
Before a harness is selected, TheClaw has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the TheClaw transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an TheClaw-owned
...
**Import:** `theclaw/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "theclaw/plugin-sdk/agent-harness";
...
TheClaw chooses a harness after provider/model resolution:
...
2. `THECLAW_AGENT_RUNTIME=<id>` forces a registered harness with that id for
   sessions that are not already pinned.
3. `THECLAW_AGENT_RUNTIME=pi` forces the built-in PI harness.
4. `THECLAW_AGENT_RUNTIME=auto` asks registered harnesses if they support the
   resolved provider/model.
5. If no registered harness matches, TheClaw uses PI unless PI fallback is
   disabled.
...
provider/model. Once a plugin harness has claimed a run, TheClaw does not
...
TheClaw. The harness then claims that provider in `supports(...)`.
...
- app-server request: TheClaw sends the bare model id to Codex and lets the
  harness talk to the native app-server protocol

The Codex plugin is additive. Plain `openai/gpt-*` refs continue to use the
normal TheClaw provider path unless you force the Codex harness with
...
TheClaw requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
TheClaw only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning TheClaw to the newer tested stable line.
...
`theclaw/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so TheClaw's fallback policy can decide whether to retry on a
...
The bundled `codex` harness is the native Codex mode for embedded TheClaw
...
compaction, and app-server execution. TheClaw still owns the chat channel,
...
By default, TheClaw runs embedded agents with `agents.defaults.agentRuntime`
set to `{ id: "auto", fallback: "pi" }`. In `auto` mode, registered plugin
harnesses can claim a provider/model pair. If none match, TheClaw falls back
to PI.
...
want TheClaw to silently fall back to PI, keep `runtime: "auto"` and disable
the fallback:
...
`THECLAW_AGENT_RUNTIME` still overrides the configured runtime. Use
`THECLAW_AGENT_HARNESS_FALLBACK=none` to disable PI fallback from the
environment.

```bash
THECLAW_AGENT_RUNTIME=codex \
THECLAW_AGENT_HARNESS_FALLBACK=none \
theclaw gateway run
```
...
Keep that binding explicitly associated with the TheClaw session, and keep
mirroring user-visible assistant/tool output into the TheClaw transcript.

The TheClaw transcript remains the compatibility layer for:
...
If your harness stores a sidecar binding, implement `reset(...)` so TheClaw can
clear it when the owning TheClaw session is reset.

## Tool and media results

Core constructs the TheClaw tool list and passes it into the prepared attempt.
