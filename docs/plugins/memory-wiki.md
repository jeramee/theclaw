...
If the active memory plugin exposes shared recall artifacts, TheClaw can search
...
currently exposing public bridge inputs yet. Run `theclaw wiki doctor` first,
...
`theclaw wiki status`, `theclaw wiki doctor`, and `theclaw wiki bridge
...
theclaw wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
