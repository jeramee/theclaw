---
summary: "Step-by-step guide to building a messaging channel plugin for TheClaw"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect TheClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects TheClaw to a
...
<Info>
  If you have not built any TheClaw plugin before, read
...
Channel plugins do not need their own send/edit/react tools. TheClaw keeps one
...
Use `theclaw/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `theclaw/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If a channel needs native approval delivery, keep channel code focused on target normalization plus transport/presentation facts. Use `createChannelExecApprovalProfile`, `createChannelNativeOriginTargetResolver`, `createChannelApproverDmTargetResolver`, and `createApproverRestrictedNativeApprovalCapability` from `theclaw/plugin-sdk/approval-runtime`. Put the channel-specific facts behind `approvalCapability.nativeRuntime`, ideally via `createChannelApprovalNativeRuntimeAdapter(...)` or `createLazyChannelApprovalNativeRuntimeAdapter(...)`, so core can assemble the handler and own request filtering, routing, dedupe, expiry, gateway subscription, and routed-elsewhere notices. `nativeRuntime` is split into a few smaller seams:
...
- `theclaw/plugin-sdk/approval-auth-runtime`
- `theclaw/plugin-sdk/approval-client-runtime`
- `theclaw/plugin-sdk/approval-delivery-runtime`
- `theclaw/plugin-sdk/approval-gateway-runtime`
- `theclaw/plugin-sdk/approval-handler-adapter-runtime`
- `theclaw/plugin-sdk/approval-handler-runtime`
- `theclaw/plugin-sdk/approval-native-runtime`
- `theclaw/plugin-sdk/approval-reply-runtime`
- `theclaw/plugin-sdk/channel-runtime-context`

Likewise, prefer `theclaw/plugin-sdk/setup-runtime`,
`theclaw/plugin-sdk/setup-adapter-runtime`,
`theclaw/plugin-sdk/reply-runtime`,
`theclaw/plugin-sdk/reply-dispatch-runtime`,
`theclaw/plugin-sdk/reply-reference`, and
`theclaw/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `theclaw/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `theclaw/plugin-sdk/setup-adapter-runtime` is the narrow env-aware adapter
  seam for `createEnvPatchedAccountSetupAdapter`
- `theclaw/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `theclaw.setupEntry` in
...
- use the broader `theclaw/plugin-sdk/setup` seam only when you also need the
...
- `theclaw/plugin-sdk/account-core`,
  `theclaw/plugin-sdk/account-id`,
  `theclaw/plugin-sdk/account-resolution`, and
  `theclaw/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `theclaw/plugin-sdk/inbound-envelope` and
  `theclaw/plugin-sdk/inbound-reply-dispatch` for inbound route/envelope and
  record-and-dispatch wiring
- `theclaw/plugin-sdk/messaging-targets` for target parsing/matching
- `theclaw/plugin-sdk/outbound-media` and
  `theclaw/plugin-sdk/outbound-runtime` for media loading plus outbound
  identity/send delegates and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `theclaw/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `theclaw/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `theclaw/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `theclaw/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `theclaw/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `theclaw/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
} from "theclaw/plugin-sdk/channel-inbound";
...
`theclaw/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.

The older `resolveMentionGating*` helpers remain on
`theclaw/plugin-sdk/channel-inbound` as compatibility exports only. New code
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#theclaw-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/theclaw-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "theclaw": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect TheClaw to Acme Chat."
        }
      }
    }
    ```

    ```json theclaw.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { TheClawConfig } from "theclaw/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "theclaw/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so TheClaw
  ...
  <Step title="Add a setup entry">
    ...
    import { defineSetupPluginEntry } from "theclaw/plugin-sdk/channel-core";
    import { acmeChatPlugin } from "./src/channel.js";

    export default defineSetupPluginEntry(acmeChatPlugin);
    ```

    TheClaw loads this instead of the full entry when the channel is disabled
    ...
    `theclaw/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    TheClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to TheClaw.
...
├── package.json              # theclaw.channel metadata
├── theclaw.plugin.json      # Manifest with config schema
