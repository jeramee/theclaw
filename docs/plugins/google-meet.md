...
read_when:
  - You want an TheClaw agent to join a Google Meet call
  - You want an TheClaw agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for TheClaw — the plugin is explicit by design:
...
- Realtime voice can call back into the full TheClaw agent when deeper
...
```bash
theclaw googlemeet setup --transport chrome-node --mode transcribe
```
...
join. Use `theclaw googlemeet setup --json` for scripts or machine-readable
...
```bash
theclaw googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
theclaw googlemeet create --transport chrome-node --mode realtime
```

Create only the URL without joining:

```bash
theclaw googlemeet create --no-join
```
...
- Browser fallback: used when OAuth credentials are absent. TheClaw uses the
  ...
  the TheClaw Chrome profile on the node to already be signed in to Google.
...
TheClaw's microphone/camera permission grant and avoid the Meet **Use
...
Local Chrome joins through the signed-in TheClaw browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by TheClaw. For
...
You do **not** need a full TheClaw Gateway or model API key inside a macOS VM
...
- Gateway host: TheClaw Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: TheClaw CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update TheClaw in the VM, then enable the bundled plugin there:

```bash
theclaw plugins enable google-meet
```

Start the node host in the VM:

```bash
theclaw node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
THECLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  theclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
THECLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  theclaw node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
theclaw node restart
```

`THECLAW_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`theclaw.json` setting. `theclaw node install` stores it in the LaunchAgent
...
```bash
theclaw googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, TheClaw browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, TheClaw auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `theclaw node run` in the VM,
  approve pairing, and make sure `theclaw plugins enable google-meet` and
  `theclaw plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses TheClaw
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. TheClaw
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by TheClaw; use separate virtual devices or Loopback-style routing
...
TheClaw does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with TheClaw, review BlackHole's
...
Chrome transport opens the Meet URL through TheClaw browser control and joins
as the signed-in TheClaw browser profile. On macOS, the plugin checks for
...
```bash
theclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
theclaw googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local TheClaw audio
...
meeting; TheClaw does not discover those from the Meet page.
...
theclaw plugins list | grep -E 'google-meet|voice-call'
theclaw googlemeet setup
```
...
`THECLAW_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes TheClaw requests:
...
`meetings.space.readonly` lets TheClaw resolve Meet URLs/codes to spaces.
...
```bash
theclaw googlemeet auth login --json
```
...
```bash
THECLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
THECLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
theclaw googlemeet auth login --json
```
...
THECLAW_GOOGLE_MEET_CLIENT_ID="your-client-id" \
THECLAW_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
theclaw googlemeet auth login --json --manual
```
...
support existed, rerun `theclaw googlemeet auth login --json` so the refresh
...
```bash
theclaw googlemeet doctor --oauth --json
```
...
```bash
theclaw googlemeet doctor --oauth --create-space --json
theclaw googlemeet create --no-join --json
```
...
```bash
theclaw googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
theclaw googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `theclaw googlemeet auth login
...
- `THECLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `THECLAW_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `THECLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `THECLAW_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `THECLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `THECLAW_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `THECLAW_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
theclaw googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
theclaw googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
theclaw googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
theclaw googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
theclaw googlemeet latest --today
theclaw googlemeet calendar-events --today --json
theclaw googlemeet artifacts --event "Weekly sync"
theclaw googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
theclaw googlemeet latest --meeting https://meet.google.com/abc-defg-hij
theclaw googlemeet artifacts --conference-record conferenceRecords/abc123 --json
theclaw googlemeet attendance --conference-record conferenceRecords/abc123 --json
```

Write a readable report:

```bash
theclaw googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
theclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
theclaw googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
theclaw googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
THECLAW_LIVE_TEST=1 \
THECLAW_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
pnpm test:live -- extensions/google-meet/google-meet.live.test.ts
```

Live smoke environment:

- `THECLAW_LIVE_TEST=1` enables guarded live tests.
- `THECLAW_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `THECLAW_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `THECLAW_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `THECLAW_GOOGLE_MEET_CLIENT_SECRET`,
  `THECLAW_GOOGLE_MEET_ACCESS_TOKEN`, and
  `THECLAW_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `THECLAW_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the TheClaw browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the TheClaw browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, TheClaw reports `manualActionRequired: true` or a
...
- `chrome.guestName: "TheClaw Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through TheClaw browser automation on `chrome-node`
...
- `realtime.instructions`: brief spoken replies, with
  `theclaw_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional TheClaw agent id for
  `theclaw_agent_consult`; defaults to `main`
...
VM. In both cases the realtime model and `theclaw_agent_consult` run on the
...
TheClaw tools, it can call `theclaw_agent_consult`.

The consult tool runs the regular TheClaw agent behind the scenes with recent
...
Meet lane should consult a dedicated TheClaw agent workspace, model defaults,
...
```bash
theclaw googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
```bash
theclaw googlemeet setup
theclaw nodes status --connected
theclaw nodes invoke \
  --node parallels-macos \
...
```bash
theclaw plugins list | grep google-meet
theclaw googlemeet setup
```
...
```bash
theclaw plugins enable google-meet
theclaw plugins enable browser
THECLAW_ALLOW_INSECURE_PRIVATE_WS=1 \
  theclaw node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; TheClaw
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, TheClaw
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `THECLAW_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `theclaw googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the TheClaw Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. TheClaw should click **Use microphone** or, for
...
utterance but TheClaw did not see new output bytes reach the Chrome audio
bridge.
...
```bash
theclaw googlemeet recover-tab
theclaw googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
theclaw voicecall smoke --to "+15555550123"
```
...
```bash
theclaw voicecall smoke --to "+15555550123" --yes
```
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: TheClaw owns the
