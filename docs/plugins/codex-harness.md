---
summary: "Run TheClaw embedded agent turns through the bundled Codex app-server harness"
title: "Codex harness"
read_when:
...
The bundled `codex` plugin lets TheClaw run embedded agent turns through the
...
TheClaw still owns chat channels, session files, model selection, tools,
...
| --------------------------------- | --------------------------------------------------- | ----------------------------------------------------------------------------- |
| Native embedded runtime           | `agentRuntime.id: "codex"`                          | Runs TheClaw embedded agent turns through Codex app-server.                  |
...
| Native hook relay                 | Plugin hooks around Codex-native events             | Lets TheClaw observe/block supported Codex-native tool/finalization events.  |
...
- replace TheClaw channel delivery, session files, auth-profile storage, or
  message routing
...
Native Codex turns keep TheClaw plugin hooks as the public compatibility layer.
These are in-process TheClaw hooks, not Codex `hooks.json` command hooks:
...
TheClaw dynamic tool results after TheClaw executes the tool and before the
result is returned to Codex. This is separate from the public
`tool_result_persist` plugin hook, which transforms TheClaw-owned transcript
tool-result writes.
...
`agentRuntime.id: "codex"` or `THECLAW_AGENT_RUNTIME=codex` when they
...
`openai-codex/*`, `theclaw doctor` warns instead of changing the route. That is
...
| ------------------------------------------- | -------------------------- | -------------------------------------- | --------------------------- | ------------------------------ |
| OpenAI API through normal TheClaw runner   | `openai/gpt-*`             | omitted or `runtime: "pi"`             | OpenAI provider             | `Runtime: TheClaw Pi Default` |
| Codex OAuth/subscription through PI         | `openai-codex/gpt-*`       | omitted or `runtime: "pi"`             | OpenAI Codex OAuth provider | `Runtime: TheClaw Pi Default` |
...
| --------------------------------------------- | -------------------------------------------- | ------------------------------------------------------------------------- |
| `openai/gpt-5.4`                              | OpenAI provider through TheClaw/PI plumbing | You want current direct OpenAI Platform API access with `OPENAI_API_KEY`. |
| `openai-codex/gpt-5.5`                        | OpenAI Codex OAuth through TheClaw/PI       | You want ChatGPT/Codex subscription auth with the default PI runner.      |
...
GPT-5.5 is currently subscription/OAuth-only in TheClaw. Use
...
`theclaw doctor` warns when all of these are true:
...
"native Codex app-server runtime." TheClaw does not make that leap. The warning
means:
...
TheClaw records the selected harness id on that session and keeps using it for
later turns in the same session id. Change `agentRuntime` config or
`THECLAW_AGENT_RUNTIME` when you want future sessions to use another harness;
...
`Runtime: TheClaw Pi Default`, and the Codex app-server harness appears as
`Runtime: OpenAI Codex`.

## Requirements

- TheClaw with the bundled `codex` plugin available.
...
- Codex auth available to the app-server process or to TheClaw's Codex auth
  bridge.

The plugin blocks older or unversioned app-server handshakes. That keeps
TheClaw on the protocol surface it has been tested against.

For live and Docker smoke tests, auth usually comes from the Codex CLI account
or an TheClaw `openai-codex` auth profile. Local stdio app-server launches can
...
that runtime is forced, TheClaw still tries the Codex harness and fails closed
...
| "Use my ChatGPT/Codex subscription with normal TheClaw" | `openai-codex/*` model refs                      |
...
TheClaw only advertises ACP spawn guidance to agents when ACP is enabled,
...
```bash
THECLAW_AGENT_RUNTIME=codex theclaw gateway run
```

With Codex forced, TheClaw fails early if the Codex plugin is disabled, the
app-server is too old, or the app-server cannot start. Set
`THECLAW_AGENT_HARNESS_FALLBACK=pi` only if you intentionally want PI to handle
...
TheClaw session and the Codex harness creates or resumes its sidecar app-server
thread as needed. `/reset` clears the TheClaw session binding for that thread
...
By default, the plugin starts TheClaw's managed Codex binary locally with:
...
By default, TheClaw starts local Codex harness sessions in YOLO mode:
...
Stdio app-server launches inherit TheClaw's process environment by default,
but TheClaw owns the Codex app-server account bridge. Auth is selected in this
order:

1. An explicit TheClaw Codex auth profile for the agent.
...
When TheClaw sees a ChatGPT subscription-style Codex auth profile, it removes
...
| `clearEnv`          | `[]`                                     | Extra environment variable names removed from the spawned stdio app-server process after TheClaw builds its inherited environment. |
...
- `THECLAW_CODEX_APP_SERVER_BIN`
- `THECLAW_CODEX_APP_SERVER_ARGS`
- `THECLAW_CODEX_APP_SERVER_MODE=yolo|guardian`
- `THECLAW_CODEX_APP_SERVER_APPROVAL_POLICY`
- `THECLAW_CODEX_APP_SERVER_SANDBOX`

`THECLAW_CODEX_APP_SERVER_BIN` bypasses the managed binary when
`appServer.command` is unset.

`THECLAW_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
`plugins.entries.codex.config.appServer.mode: "guardian"` instead, or
`THECLAW_CODEX_APP_SERVER_MODE=guardian` for one-off local testing. Config is
...
The short version: TheClaw does not vendor the desktop-control app or execute
...
`cua-driver mcp` with `theclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'`.
...
When `computerUse.autoInstall` is true, TheClaw can register the standard
...
Model switching stays TheClaw-controlled. When an TheClaw session is attached
...
generic and works on any channel that supports TheClaw text commands.
...
- `/codex resume <thread-id>` attaches the current TheClaw session to an existing Codex thread.
...
normal turns. On the next message, TheClaw resumes that Codex thread, passes the
currently selected TheClaw model into app-server, and keeps extended history
enabled.
...
| Layer                                 | Owner                    | Purpose                                                             |
| ------------------------------------- | ------------------------ | ------------------------------------------------------------------- |
| TheClaw plugin hooks                 | TheClaw                 | Product/plugin compatibility across PI and Codex harnesses.         |
| Codex app-server extension middleware | TheClaw bundled plugins | Per-turn adapter behavior around TheClaw dynamic tools.            |
| Codex native hooks                    | Codex                    | Low-level Codex lifecycle and native tool policy from Codex config. |

TheClaw does not use project or global Codex `hooks.json` files to route
TheClaw plugin behavior. For the supported native tool and permission bridge,
TheClaw injects per-thread Codex config for `PreToolUse`, `PostToolUse`,
...
TheClaw plugin hooks in the v1 contract.

For TheClaw dynamic tools, TheClaw executes the tool after Codex asks for the
call, so TheClaw fires the plugin and middleware behavior it owns in the
harness adapter. For Codex-native tools, Codex owns the canonical tool record.
TheClaw can mirror selected events, but it cannot rewrite the native Codex
...
notifications and TheClaw adapter state, not native Codex hook commands.
TheClaw's `before_compaction`, `after_compaction`, `llm_input`, and
...
They do not invoke TheClaw plugin hooks.

## V1 support contract

Codex mode is not PI with a different model call underneath. Codex owns more of
the native model loop, and TheClaw adapts its plugin and session surfaces
...
| TheClaw channel routing and delivery         | Supported                               | Telegram, Discord, Slack, WhatsApp, iMessage, and other channels stay outside the model runtime.                                                                                                      |
| TheClaw dynamic tools                        | Supported                               | Codex asks TheClaw to execute these tools, so TheClaw stays in the execution path.                                                                                                                  |
| Prompt and context plugins                    | Supported                               | TheClaw builds prompt overlays and projects context into the Codex turn before starting or resuming the thread.                                                                                      |
...
| Dynamic tool hooks                            | Supported                               | `before_tool_call`, `after_tool_call`, and tool-result middleware run around TheClaw-owned dynamic tools.                                                                                            |
...
| Native permission policy                      | Supported through the native hook relay | Codex `PermissionRequest` can be routed through TheClaw policy where the runtime exposes it. If TheClaw returns no decision, Codex continues through its normal guardian or user approval path.     |
| App-server trajectory capture                 | Supported                               | TheClaw records the request it sent to app-server and the app-server notifications it receives.                                                                                                      |

Not supported in Codex runtime v1:

| Surface                                             | V1 boundary                                                                                                                                     | Future path                                                                               |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| Native tool argument mutation                       | Codex native pre-tool hooks can block, but TheClaw does not rewrite Codex-native tool arguments.                                               | Requires Codex hook/schema support for replacement tool input.                            |
| Editable Codex-native transcript history            | Codex owns canonical native thread history. TheClaw owns a mirror and can project future context, but should not mutate unsupported internals. | Add explicit Codex app-server APIs if native thread surgery is needed.                    |
| `tool_result_persist` for Codex-native tool records | That hook transforms TheClaw-owned transcript writes, not Codex-native tool records.                                                           | Could mirror transformed records, but canonical rewrite needs Codex support.              |
| Rich native compaction metadata                     | TheClaw observes compaction start and completion, but does not receive a stable kept/dropped list, token delta, or summary payload.            | Needs richer Codex compaction events.                                                     |
| Compaction intervention                             | Current TheClaw compaction hooks are notification-level in Codex mode.                                                                         | Add Codex pre/post compaction hooks if plugins need to veto or rewrite native compaction. |
| Byte-for-byte model API request capture             | TheClaw can capture app-server requests and notifications, but Codex core builds the final OpenAI API request internally.                      | Needs a Codex model-request tracing event or debug API.                                   |

## Tools, media, and compaction

The Codex harness changes the low-level embedded agent executor only.

TheClaw still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal TheClaw delivery path.

The native hook relay is intentionally generic, but the v1 support contract is
limited to the Codex-native tool and permission paths that TheClaw tests. In
...
Codex hook event is an TheClaw plugin surface until the runtime contract names
it.

For `PermissionRequest`, TheClaw only returns explicit allow or deny decisions
...
Codex MCP tool approval elicitations are routed through TheClaw's plugin
...
delegated to Codex app-server. TheClaw keeps a transcript mirror for channel
...
reasoning or plan records when the app-server emits them. Today, TheClaw only
...
TheClaw is writing an TheClaw-owned session transcript tool result.
...
**TheClaw uses PI instead of Codex:** `agentRuntime.id: "auto"` can still use PI as the
...
stable `0.125.0` protocol floor is what TheClaw tests.
