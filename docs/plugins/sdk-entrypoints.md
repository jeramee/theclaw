...
when TheClaw loads an installed package and let npm packages avoid runtime
TypeScript compilation. If an installed package only declares a TypeScript
source entry, TheClaw will use a matching built `dist/*.js` peer when one
...
**Import:** `theclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `TheClawPluginConfigSchema \| () => TheClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: TheClawPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `theclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- TheClaw resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `theclaw/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: TheClawPluginApi) => void`                               | No       | —                   |
| `registerFull`        | `(api: TheClawPluginApi) => void`                               | No       | —                   |
...
- Discovery registration is non-activating, not import-free. TheClaw may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and TheClaw
...
TheClaw loads this instead of the full entry when a channel is disabled,
...
- `theclaw/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
  import-safe setup patch adapters, lookup-note output,
  `promptResolvedAllowFrom`, `splitSetupEntries`, and delegated setup proxies
- `theclaw/plugin-sdk/channel-setup` for optional-install setup surfaces
- `theclaw/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`theclaw/plugin-sdk/channel-entry-contract` instead. That contract lets the
...
the plugin entry and the channel plugin object so TheClaw can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want TheClaw to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; TheClaw rejects descriptor names outside
...
TheClaw classifies loaded plugins by their registration behavior:
...
Use `theclaw plugins inspect <id>` to see a plugin's shape.
