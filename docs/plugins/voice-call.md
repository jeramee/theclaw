...
read_when:
  - You want to place an outbound voice call from TheClaw
...
Voice calls for TheClaw via a plugin. Supports outbound notifications,
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `theclaw doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `theclaw doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `theclaw_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal TheClaw tools.
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call theclaw_agent_consult before using deeper tools.",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `theclaw doctor --fix`; committed config should use `tts.providers.<provider>`.
...
```bash
theclaw voicecall call --to "+15555550123" --message "Hello from TheClaw"
theclaw voicecall start --to "+15555550123"   # alias for call
theclaw voicecall continue --call-id <id> --message "Any questions?"
theclaw voicecall speak --call-id <id> --message "One moment"
theclaw voicecall dtmf --call-id <id> --digits "ww123456#"
theclaw voicecall end --call-id <id>
theclaw voicecall status --call-id <id>
theclaw voicecall tail
theclaw voicecall latency                      # summarize turn latency from logs
