...
At startup, TheClaw does roughly this:
...
The manifest is the control-plane source of truth. TheClaw uses it to:
...
- Gateway startup planning uses `activation.onStartup` for explicit startup
  imports and startup opt-outs; every plugin should declare it as TheClaw
...
TheClaw keeps short in-process caches for:
...
metadata from the persisted installed plugin index, TheClaw also keeps a small
...
- Set `THECLAW_DISABLE_PLUGIN_DISCOVERY_CACHE=1` or
  `THECLAW_DISABLE_PLUGIN_MANIFEST_CACHE=1` to disable these caches.
- Set `THECLAW_DISABLE_INSTALLED_PLUGIN_MANIFEST_REGISTRY_CACHE=1` to disable
  only the installed-index manifest-registry fallback cache.
- Tune cache windows with `THECLAW_PLUGIN_DISCOVERY_CACHE_MS` and
  `THECLAW_PLUGIN_MANIFEST_CACHE_MS`.
...
TheClaw still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, TheClaw calls hooks in this rough order.
...
| --  | _(built-in model lookup)_         | TheClaw tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
that still runs on TheClaw's normal inference loop.
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of TheClaw's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as TheClaw adds those
...
- TheClaw only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "TheClaw plugin runtime helpers",
    count: 5,
  },
});
```
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-theclaw-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-theclaw-scopes` only when the header is explicitly present
  - if `x-theclaw-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-theclaw-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `theclaw/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `theclaw/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `theclaw/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `theclaw/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `theclaw/plugin-sdk/config-schema` | Root `theclaw.json` Zod schema (`TheClawSchema`) |
...
<Info>
`theclaw/plugin-sdk/channel-runtime`, `theclaw/plugin-sdk/config-runtime`,
and `theclaw/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `theclaw/plugin-sdk/*` subpaths. Never
...
`theclaw/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape TheClaw writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to TheClaw's
...
- if both `catalog` and `discovery` are registered, TheClaw uses `catalog`
...
- Read-only command paths such as `theclaw status`, `theclaw status --all`,
  `theclaw channels status`, `theclaw channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `theclaw.extensions`:
...
Security guardrail: every `theclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `theclaw plugins install` installs plugin dependencies with a
...
Optional: `theclaw.setupEntry` can point at a lightweight setup-only module.
When TheClaw needs setup surfaces for a disabled channel plugin, or
...
Optional: `theclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let TheClaw load the
...
Channel plugins can advertise setup/discovery metadata via `theclaw.channel` and
install hints via `theclaw.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@theclaw/nextcloud-talk",
  "theclaw": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
    ...
    "install": {
      "npmSpec": "@theclaw/nextcloud-talk",
...
Useful `theclaw.channel` fields beyond the minimal example:
...
TheClaw can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.theclaw/mpm/plugins.json`
- `~/.theclaw/mpm/catalog.json`
- `~/.theclaw/plugins/catalog.json`

Or point `THECLAW_PLUGIN_CATALOG_PATHS` (or `THECLAW_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "theclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `theclaw.install` block. The
...
```ts
import { buildMemorySystemPromptAddition } from "theclaw/plugin-sdk/core";
...
2. add typed plugin registration/runtime surfaces
   Extend `TheClawPluginApi` and/or `api.runtime` with the smallest useful
...
This is how TheClaw stays opinionated without becoming hardcoded to one
