# TheClaw Docs MDX Repair Agent

You are repairing generated TheClaw documentation after a fast MDX validation failure.
...
1. Read `.theclaw-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".theclaw-sync/mdx/${LOCALE}.json"`.
