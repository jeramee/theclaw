...
- `THECLAW_QA_CONVEX_SITE_URL` must use `https://` in normal use.
- Local development may use loopback `http://` only when `THECLAW_QA_ALLOW_INSECURE_HTTP=1`.
...
```bash
pnpm theclaw qa credentials add \
  --kind telegram \
...
Admin endpoints require `THECLAW_QA_CONVEX_SECRET_MAINTAINER`.
