...
import { bundledPluginRoot } from "theclaw/plugin-sdk/test-fixtures";
...
describe("buildOfficialChannelCatalog", () => {
  it("includes publishable official channel plugins and skips non-publishable entries", () => {
    const repoRoot = makeRepoRoot("theclaw-official-channel-catalog-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@theclaw/whatsapp",
      version: "2026.3.23",
      description: "TheClaw WhatsApp channel plugin",
      theclaw: {
        channel: {
          id: "whatsapp",
          label: "WhatsApp",
    ...
    expect(buildOfficialChannelCatalog({ repoRoot }).entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          theclaw: expect.objectContaining({
            channel: expect.objectContaining({
              id: "wecom",
              label: "WeCom",
            }),
            install: {
              npmSpec: "@wecom/wecom-theclaw-plugin@2026.4.23",
              defaultChoice: "npm",
              expectedIntegrity:
        ...
        expect.objectContaining({
          ...
          theclaw: expect.objectContaining({
            ...
            install: {
              npmSpec: "theclaw-plugin-yuanbao@2.11.0",
              defaultChoice: "npm",
              expectedIntegrity:
  ...
  it("keeps official external catalog npm sources exactly pinned", () => {
    const repoRoot = makeRepoRoot("theclaw-official-channel-catalog-policy-");
    ...
    for (const entry of entries) {
      const installSource = describePluginInstallSource(entry.theclaw?.install ?? {});
  ...
  it("writes the official catalog under dist", () => {
    const repoRoot = makeRepoRoot("theclaw-official-channel-catalog-write-");
    writeJson(path.join(repoRoot, "extensions", "whatsapp", "package.json"), {
      name: "@theclaw/whatsapp",
      theclaw: {
        channel: {
