...
describe("install.sh", () => {
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "theclaw-install-nvm-"));
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "theclaw-install-sh-gum-"));
    try {
...
describe("install.sh duplicate TheClaw install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/theclaw" "$root/fnm/theclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/theclaw/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/theclaw/package.json"
      collect_theclaw_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      THECLAW_BIN="$root/fnm/.bin/theclaw"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_theclaw_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple TheClaw global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/theclaw");
    expect(result.stdout).toContain("/fnm/theclaw");
    expect(result.stdout).toContain("Active theclaw:");
    expect(result.stdout).toContain("npm uninstall -g theclaw");
  });

  it("stays quiet when only one TheClaw npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/theclaw"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/theclaw/package.json"
      collect_theclaw_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_theclaw_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple TheClaw global installs detected");
  });
});
