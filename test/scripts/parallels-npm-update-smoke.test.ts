...
describe("parallels npm update smoke", () => {
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("$env:THECLAW_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(script).toContain(
      "THECLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/theclaw update",
    );
    expect(script).toContain("THECLAW_DISABLE_BUNDLED_PLUGINS=1 theclaw update");
    expect(script).toContain(
      "THECLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/theclaw gateway stop",
    );
    expect(script).toContain("THECLAW_DISABLE_BUNDLED_PLUGINS=1 theclaw gateway stop");
  });
});
