...
describe("plugin update unchanged Docker E2E", () => {
  it("seeds current plugin install ledger state before checking config stability", () => {
    ...
    const configSeedStart = script.indexOf('cat > \\"\\$THECLAW_CONFIG_PATH\\"');
    const configSeedEnd = script.indexOf('cat > \\"\\$HOME/.theclaw/plugins/installs.json\\"');
  ...
  it("bounds the update command and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("THECLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS");
