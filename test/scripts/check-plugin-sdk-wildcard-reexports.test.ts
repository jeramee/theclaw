  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "theclaw/plugin-sdk/foo";',
          'export type * from "theclaw/plugin-sdk/bar";',
          'export { named } from "theclaw/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "theclaw/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "theclaw/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "theclaw/plugin-sdk/foo";',
          'export type { Named } from "theclaw/plugin-sdk/foo";',
