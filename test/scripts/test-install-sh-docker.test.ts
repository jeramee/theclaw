...
const RELEASE_CHECKS_WORKFLOW_PATH = ".github/workflows/theclaw-release-checks.yml";
const LIVE_E2E_WORKFLOW_PATH = ".github/workflows/theclaw-live-and-e2e-checks-reusable.yml";
...
describe("test-install-sh-docker", () => {
  ...
  it("uses npm latest as the update baseline and resolves it to the concrete packed version", () => {
    ...
    expect(workflow).toContain(
      "THECLAW_INSTALL_SMOKE_UPDATE_BASELINE: ${{ inputs.update_baseline_version || 'latest' }}",
    );
  });
  ...
  it("can reuse dist from the already-built root Docker smoke image", () => {
    ...
    expect(script).toContain('UPDATE_DIST_IMAGE="${THECLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"');
  ...
  it("allows repository branch history and release tags for secret-backed Docker release checks", () => {
    ...
    expect(workflow).toContain("reachable from an TheClaw branch or release tag");
  });
...
describe("install-sh smoke runner", () => {
  it("wraps long npm/update operations with heartbeat and install-size audits", () => {
    ...
    expect(script).toContain("theclaw update --tag");
  ...
  it("covers plain npm global installs and npm-driven updates", () => {
    ...
    expect(script).toContain('SKIP_NPM_GLOBAL="${THECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"');
    expect(script).toContain('NPM_CACHE_DIR="${THECLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"');
    ...
    expect(script).toContain("THECLAW_INSTALL_SMOKE_MODE=npm-global");
...
describe("bun global install smoke", () => {
  it("packs the current tree and verifies image-provider discovery through Bun", () => {
    ...
    expect(script).toContain("THECLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE");
  });
  ...
  it("gates workflow Bun install smoke to scheduled and release-check runs", () => {
    ...
    expect(workflow).toContain(
      "THECLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE: theclaw-dockerfile-smoke:local",
    );
    ...
    expect(workflow).toContain("THECLAW_CI_WORKFLOW_BUN_GLOBAL_INSTALL_SMOKE");
    ...
    expect(workflow).toContain('THECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL: "1"');
