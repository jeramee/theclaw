...
const LIVE_E2E_WORKFLOW = ".github/workflows/theclaw-live-and-e2e-checks-reusable.yml";
const NPM_TELEGRAM_WORKFLOW = ".github/workflows/npm-telegram-beta-e2e.yml";
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/theclaw-release-checks.yml";
...
describe("package acceptance workflow", () => {
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-theclaw-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/theclaw-live-and-e2e-checks-reusable.yml",
    );
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: theclaw@${{ needs.resolve_package.outputs.package_version }}",
    );
    expect(workflow).toContain(
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("THECLAW_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain("Download current-run TheClaw Docker E2E package");
    expect(workflow).toContain("Download previous-run TheClaw Docker E2E package");
    ...
    expect(workflow).toContain("THECLAW_DOCKER_E2E_REPO_ROOT:");
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("THECLAW_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("THECLAW_DOCKER_PULL_TIMEOUT_SECONDS");
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).not.toContain(
      "THECLAW_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: THECLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      'command: THECLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" bash .release-harness/scripts/test-live-gateway-models-docker.sh',
    );
    expect(workflow).toContain(
      'command: THECLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: THECLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: THECLAW_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    ...
    expect(build).toContain('ROOT_DIR="${THECLAW_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    expect(build).toContain('source "$SCRIPT_ROOT_DIR/scripts/lib/docker-build.sh"');
    expect(stage).toContain(
      'local scripts_dir="${THECLAW_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("THECLAW_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "THECLAW_QA_CONVEX_SITE_URL: ${{ secrets.THECLAW_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "THECLAW_QA_CONVEX_SECRET_CI: ${{ secrets.THECLAW_QA_CONVEX_SECRET_CI }}",
    );
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm theclaw qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain('pnpm theclaw qa matrix "${matrix_args[@]}"');
