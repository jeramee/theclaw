...
describe("docker build helper", () => {
  it("forces BuildKit for centralized Docker builds", () => {
    ...
    expect(helper).toContain("THECLAW_DOCKER_BUILD_RETRIES");
  ...
  it("lets Testbox fall back to building when a reused Docker image is missing", () => {
    ...
    expect(helper).toContain("THECLAW_DOCKER_BUILD_ON_MISSING");
    expect(helper).toContain("THECLAW_TESTBOX");
    ...
    expect(liveCliBackend).not.toContain(
      'echo "==> Reuse live-test image: $LIVE_IMAGE_NAME (THECLAW_SKIP_DOCKER_BUILD=1)"',
    );
  });
  ...
  it("preserves pnpm lookup paths for scheduled Docker child lanes", () => {
    ...
    expect(scheduler).toContain("THECLAW_DOCKER_ALL_PNPM_COMMAND");
  });
  ...
  it("keeps package acceptance plugin coverage offline-capable", () => {
    ...
    expect(scenarios).toContain("THECLAW_PLUGINS_E2E_CLAWHUB=0");
  ...
  it("caps package acceptance legacy compatibility at 2026.4.25", () => {
    ...
    expect(scripts.join("\n")).toContain("THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT");
    expect(scripts.join("\n")).toContain(
  ...
  it("keeps bundled plugin install/uninstall sweep chunkable", () => {
    ...
    expect(runner).toContain("THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL");
    expect(runner).toContain("THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX");
    expect(runner).toContain('"theclaw.plugin.json"');
    expect(runner).toContain("read -r plugin_id plugin_dir requires_config");
    expect(runner).toContain('node "$THECLAW_ENTRY" plugins install "$plugin_id"');
    expect(runner).toContain('node "$THECLAW_ENTRY" plugins uninstall "$plugin_id" --force');
  ...
  it("passes installer tag env to bash, not curl", () => {
    ...
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | THECLAW_BETA=1 bash');
    expect(runner).toContain('curl -fsSL "$INSTALL_URL" | THECLAW_VERSION="$INSTALL_TAG" bash');
    expect(runner).not.toContain('THECLAW_BETA=1 curl -fsSL "$INSTALL_URL" | bash');
    expect(runner).not.toContain(
      'THECLAW_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash',
    );
  });
  ...
  it("keeps ClawHub plugin Docker smoke hermetic by default", () => {
    ...
    expect(runner).toContain('THECLAW_CLAWHUB_URL="http://127.0.0.1:');
    expect(runner).toContain("live ClawHub can rate-limit CI");
    expect(runner).toContain('[[ -z "${THECLAW_CLAWHUB_URL:-}" && -z "${CLAWHUB_URL:-}" ]]');
  });
});
