...
const CHECK_SCRIPT = "scripts/check-theclaw-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "theclaw-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "theclaw", version }));
    writeFileSync(
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><theclaw-app></theclaw-app>",
    ...
    const tarball = join(root, "theclaw.tgz");
...
describe("check-theclaw-package-tarball", () => {
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("TheClaw package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
