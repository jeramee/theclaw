...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.THECLAW_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "THECLAW_TEST_FAST", value: process.env.THECLAW_TEST_FAST },
    {
      key: "THECLAW_STRICT_FAST_REPLY_CONFIG",
      value: process.env.THECLAW_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "THECLAW_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.THECLAW_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "THECLAW_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.THECLAW_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "THECLAW_STATE_DIR", value: process.env.THECLAW_STATE_DIR },
    { key: "THECLAW_CONFIG_PATH", value: process.env.THECLAW_CONFIG_PATH },
    { key: "THECLAW_GATEWAY_PORT", value: process.env.THECLAW_GATEWAY_PORT },
    { key: "THECLAW_BRIDGE_ENABLED", value: process.env.THECLAW_BRIDGE_ENABLED },
    { key: "THECLAW_BRIDGE_HOST", value: process.env.THECLAW_BRIDGE_HOST },
    { key: "THECLAW_BRIDGE_PORT", value: process.env.THECLAW_BRIDGE_PORT },
    { key: "THECLAW_CANVAS_HOST_PORT", value: process.env.THECLAW_CANVAS_HOST_PORT },
    { key: "THECLAW_TEST_HOME", value: process.env.THECLAW_TEST_HOME },
    { key: "THECLAW_AGENT_DIR", value: process.env.THECLAW_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.THECLAW_TEST_HOME = tempHome;
  process.env.THECLAW_TEST_FAST = "1";
  process.env.THECLAW_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.THECLAW_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.THECLAW_BRIDGE_ENABLED;
  delete process.env.THECLAW_BRIDGE_HOST;
  delete process.env.THECLAW_BRIDGE_PORT;
  delete process.env.THECLAW_CANVAS_HOST_PORT;
  ...
  if (process.platform === "win32") {
    process.env.THECLAW_STATE_DIR = path.join(tempHome, ".theclaw");
  }
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.THECLAW_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.THECLAW_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".theclaw");
  const priorIsolatedHome = params.env.THECLAW_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".theclaw")
  ) {
    realStateDir = path.join(params.realHome, ".theclaw");
  }
  const tempStateDir = path.join(params.tempHome, ".theclaw");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.THECLAW_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.THECLAW_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "theclaw.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "theclaw.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    ...
    process.env.THECLAW_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.THECLAW_LIVE_USE_REAL_HOME);
