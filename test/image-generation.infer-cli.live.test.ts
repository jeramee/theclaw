...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.THECLAW_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through theclaw infer", () => {
    const outputBase = path.join(os.tmpdir(), `theclaw-infer-image-${process.pid}.png`);
