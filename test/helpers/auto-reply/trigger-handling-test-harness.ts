...
import type { TheClawConfig } from "../../../src/config/types.theclaw.js";
...
const piEmbeddedMocks = getSharedMocks("theclaw.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("theclaw.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("theclaw.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("theclaw.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    theclawHome: process.env.THECLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("THECLAW_HOME", snapshot.theclawHome);
  restoreKey("THECLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.THECLAW_STATE_DIR = join(home, ".theclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "theclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".theclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): TheClawConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-6" },
        workspace: join(home, "theclaw"),
