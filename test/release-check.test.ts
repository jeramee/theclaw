...
import { bundledDistPluginFile, bundledPluginFile } from "theclaw/plugin-sdk/test-fixtures";
import { describe, expect, it } from "vitest";
import { listBundledPluginPackArtifacts } from "../scripts/lib/bundled-plugin-build-entries.mjs";
import { listPluginSdkDistArtifacts } from "../scripts/lib/plugin-sdk-entries.mjs";
import { WORKSPACE_TEMPLATE_PACK_PATHS } from "../scripts/lib/workspace-bootstrap-smoke.mjs";
import { collectInstalledRootDependencyManifestErrors } from "../scripts/theclaw-npm-postpublish-verify.ts";
import {
...
describe("packed CLI smoke", () => {
  ...
  it("builds a packed CLI smoke env with packaged-install guardrails", () => {
    expect(
      createPackedCliSmokeEnv(
        {
          ...
          THECLAW_CONFIG_PATH: "/tmp/leaky-config.json",
        },
        { HOME: "/tmp/smoke-home", THECLAW_STATE_DIR: "/tmp/smoke-state" },
      ),
    ).toEqual({
      PATH:
      ...
      TMPDIR: "/tmp/original-tmp",
      SystemRoot: "C:\\Windows",
      THECLAW_DISABLE_BUNDLED_ENTRY_SOURCE_FALLBACK: "1",
      THECLAW_NO_ONBOARD: "1",
      THECLAW_SUPPRESS_NOTES: "1",
      THECLAW_STATE_DIR: "/tmp/smoke-state",
    });
  });
});
...
describe("collectBundledExtensionManifestErrors", () => {
  it("flags invalid bundled extension install metadata", () => {
    expect(
      ...
      "bundled extension 'broken' manifest invalid | theclaw.install.npmSpec must be a non-empty string",
    ]);
  });
      ...
      collectBundledExtensionManifestErrors([
        {
          ...
          packageJson: {
            theclaw: {
              install: { npmSpec: "@theclaw/broken", minHostVersion: "2026.3.14" },
            },
          },
        },
      ]),
    ).toEqual([
      "bundled extension 'broken' manifest invalid | theclaw.install.minHostVersion must use a semver floor in the form \">=x.y.z\"",
    ]);
  });
  ...
  it("flags non-object install metadata instead of throwing", () => {
    ...
    ).toEqual(["bundled extension 'broken' manifest invalid | theclaw.install must be an object"]);
  });
});
...
describe("bundled plugin root runtime mirrors", () => {
  ...
  it("derives required root mirrors from built root dist imports", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-root-mirror-"));
  ...
  it("does not derive root mirrors for root chunks sourced from the owning plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-root-mirror-owned-"));
  ...
  it("does not require root deps for root chunks sourced from the owning installed plugin", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-root-owned-installed-"));

    try {
      mkdirSync(join(tempRoot, "dist", "extensions", "memory-lancedb"), { recursive: true });
      writeFileSync(
        join(tempRoot, "package.json"),
        `{"name":"theclaw","dependencies":{}}\n`,
        "utf8",
      );
      writeFileSync(
        join(tempRoot, "dist", "extensions", "memory-lancedb", "package.json"),
        `{"name":"@theclaw/memory-lancedb","dependencies":{"@lancedb/lancedb":"^0.27.2"}}\n`,
        "utf8",
      );
      writeFileSync(
  ...
  it("still requires root deps for root-owned installed chunks", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-root-owned-installed-missing-"));
...
describe("collectForbiddenPackPaths", () => {
  ...
  it("blocks legacy runtime dependency stamps from npm pack output", () => {
    expect(
      collectForbiddenPackPaths([
        "dist/index.js",
        "dist/extensions/browser/.TheClaw-Install-Stage/package.json",
        "dist/extensions/codex/.theclaw-runtime-deps-backup-node_modules-old/zod/index.js",
        "dist/extensions/discord/.theclaw-runtime-deps-stamp.json",
      ]),
    ).toEqual([
      "dist/extensions/browser/.TheClaw-Install-Stage/package.json",
      "dist/extensions/codex/.theclaw-runtime-deps-backup-node_modules-old/zod/index.js",
  ...
  it("blocks root dist chunks that still reference private qa lab sources", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-release-private-qa-"));
  ...
  it("blocks private QA paths in the generated dist inventory", () => {
    const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-release-inventory-"));
  ...
  it("requires the shipped channel catalog, control ui, and optional bundled metadata", () => {
    ...
    expect(missing).toEqual(
      expect.arrayContaining([
        ...
        bundledDistPluginFile("matrix", "theclaw.plugin.json"),
        ...
        bundledDistPluginFile("whatsapp", "theclaw.plugin.json"),
...
describe("collectPackUnpackedSizeErrors", () => {
  it("accepts pack results within the unpacked size budget", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("theclaw-2026.3.14.tgz", 120_354_302)]),
    ).toEqual([]);
  });

  it("flags oversized pack results that risk low-memory startup failures", () => {
    expect(
      collectPackUnpackedSizeErrors([makePackResult("theclaw-2026.3.12.tgz", 224_002_564)]),
    ).toEqual([
      "theclaw-2026.3.12.tgz unpackedSize 224002564 bytes (213.6 MiB) exceeds budget 211812352 bytes (202.0 MiB). Investigate duplicate channel shims, copied extension trees, or other accidental pack bloat before release.",
    ]);
  });
  ...
  it("fails closed when npm pack output omits unpackedSize for every result", () => {
    expect(
      collectPackUnpackedSizeErrors([
        { filename: "theclaw-2026.3.14.tgz" },
        { filename: "theclaw-extra.tgz", unpackedSize: Number.NaN },
      ]),
    ).toEqual([
...
describe("collectInstalledBundledPluginRuntimeDepErrors", () => {
  ...
  it("returns no errors when declared deps are installed at the theclaw package root", () => {
    const packageRoot = createPackageRoot();
    try {
      writeBundledPluginPackageJson(packageRoot, "whatsapp", {
        name: "@theclaw/whatsapp",
        dependencies: { "@whiskeysockets/baileys": "7.0.0-rc.9" },
        theclaw: { bundle: { stageRuntimeDependencies: true } },
      });
  ...
  it("checks canonical external runtime-deps roots for packed installs", () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        JSON.stringify({ name: "theclaw", version: "2026.4.25-beta.1" }, null, 2),
      );
      ...
      const externalCandidates = candidates.filter(
        (candidate) =>
          candidate.startsWith(join(homeRoot, ".theclaw", "plugin-runtime-deps")) &&
