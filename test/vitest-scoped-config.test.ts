...
import { BUNDLED_PLUGIN_TEST_GLOB, bundledPluginFile } from "theclaw/plugin-sdk/test-fixtures";
...
describe("resolveVitestIsolation", () => {
      ...
      expect.arrayContaining(
        PRIVATE_PLUGIN_SDK_SUBPATHS.map((subpath) =>
          expect.objectContaining({
            find: `theclaw/plugin-sdk/${subpath}`,
  ...
  it("ignores the legacy isolation escape hatches", () => {
    expect(resolveVitestIsolation({ THECLAW_TEST_ISOLATE: "1" })).toBe(false);
    expect(resolveVitestIsolation({ THECLAW_TEST_NO_ISOLATE: "0" })).toBe(false);
    expect(resolveVitestIsolation({ THECLAW_TEST_NO_ISOLATE: "false" })).toBe(false);
  });
...
describe("createScopedVitestConfig", () => {
  ...
  it("loads scoped include overrides from THECLAW_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-vitest-scoped-"));
    try {
...
describe("scoped vitest configs", () => {
  ...
  it("keeps the process lane off the theclaw runtime setup", () => {
  ...
  it("keeps selected plugin-sdk and commands light lanes off the theclaw runtime setup", () => {
  ...
  it("keeps the ui lane off both the theclaw runtime setup and unit-fast excludes", () => {
  ...
  it("loads channel include overrides from THECLAW_VITEST_INCLUDE_FILE", () => {
    const tempDirs: string[] = [];
    const tempDir = makeTempDir(tempDirs, "theclaw-vitest-channels-");
    try {
      const includeFile = path.join(tempDir, "include.json");
      fs.writeFileSync(
        includeFile,
