...
import { bundledPluginFile, bundledPluginRoot } from "theclaw/plugin-sdk/test-fixtures";
...
describe("parsePluginReleaseSelection", () => {
  ...
  it("dedupes and sorts comma or whitespace separated package names", () => {
    expect(
      parsePluginReleaseSelection(" @theclaw/zalo, @theclaw/feishu  @theclaw/zalo "),
    ).toEqual(["@theclaw/feishu", "@theclaw/zalo"]);
  });
});
...
describe("collectPublishablePluginPackages", () => {
  it("collects publishable npm plugins from extension package manifests", () => {
    const repoDir = makeTempRepoRoot(tempDirs, "theclaw-plugin-npm-release-");
    ...
    writeJsonFile(join(repoDir, "extensions", "demo-plugin", "package.json"), {
      name: "@theclaw/demo-plugin",
      version: "2026.4.10",
      theclaw: {
        extensions: ["./index.ts"],
        install: {
          npmSpec: "@theclaw/demo-plugin",
        },
        release: {
          publishToNpm: true,
        },
    ...
    expect(collectPublishablePluginPackages(repoDir)).toEqual([
      {
        ...
        packageName: "@theclaw/demo-plugin",
        version: "2026.4.10",
        channel: "stable",
        publishTag: "latest",
        installNpmSpec: "@theclaw/demo-plugin",
      },
    ]);
  });
});
...
describe("resolveSelectedPublishablePluginPackages", () => {
  ...
  it("throws when the selection contains an unknown package name", () => {
    expect(() =>
      resolveSelectedPublishablePluginPackages({
        plugins: publishablePlugins,
        selection: ["@theclaw/missing"],
      }),
    ).toThrowError("Unknown or non-publishable plugin package selection: @theclaw/missing.");
  });
});
