...
export type TheClawVitestPool = "forks" | "threads";
...
export function resolveLocalVitestMaxWorkers(
  ...
  pool: TheClawVitestPool = resolveDefaultVitestPool(env),
): number {
...
function hasWorkerOverride(env: Record<string, string | undefined>): boolean {
  return Boolean((env.THECLAW_VITEST_MAX_WORKERS ?? env.THECLAW_TEST_WORKERS)?.trim());
}
...
export const sharedVitestConfig = {
  ...
  resolve: {
    alias: [
      {
        find: "theclaw/extension-api",
        replacement: path.join(repoRoot, "src", "extensionAPI.ts"),
      },
      {
        find: "@theclaw/qa-channel/api.js",
