...
This document describes how the TheClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`theclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
TheClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing TheClaw Gateway
...
- ACP client spawns `theclaw acp` and speaks ACP messages over stdio.
...
`theclaw acp` resolves the Gateway URL and auth from CLI flags or config:
