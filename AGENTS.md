...
- Repo: `https://github.com/theclaw/theclaw`
...
- Extensions cross into core only via `theclaw/plugin-sdk/*`, manifest metadata, injected runtime helpers, documented barrels (`api.ts`, `runtime-api.ts`).
...
- CLI: `pnpm theclaw ...` or `pnpm dev`; build: `pnpm build`.
...
- Vitest flags only; no Jest flags like `--runInBand`. For serial runs use `pnpm test:serial` or `THECLAW_VITEST_MAX_WORKERS=1 pnpm test ...`.
...
- Testbox full-suite profile: `blacksmith testbox run --id <ID> "env NODE_OPTIONS=--max-old-space-size=4096 THECLAW_TEST_PROJECTS_PARALLEL=6 THECLAW_VITEST_MAX_WORKERS=1 pnpm test"`. For installable package proof, prefer the GitHub `Package Acceptance` workflow over ad hoc Testbox commands.
...
- Search/dedupe: prefer `gh search issues 'repo:theclaw/theclaw is:open <terms>' --json number,title,state,updatedAt --limit 20`.
...
- Wait matrix:
  ...
  - release/manual only: `Docker Release`, `TheClaw NPM Release`, `macOS Release`, `TheClaw Release Checks`, `Cross-OS Release Checks`, `NPM Telegram Beta E2E`.
...
- Naming: **TheClaw** product/docs; `theclaw` CLI/package/path/config.
...
- Changelog placement: active version `### Changes`/`### Fixes`; every added entry must include at least one `Thanks @author` attribution, using credited GitHub username(s). Never add `Thanks @codex`, `Thanks @theclaw`, or `Thanks @steipete`.
...
- PR/issue workflows: `$theclaw-pr-maintainer`. `/landpr`: `~/.codex/prompts/landpr.md`.

## Security / Release

- Never commit real phone numbers, videos, credentials, live config.
- Secrets: channel/provider creds in `~/.theclaw/credentials/`; model auth profiles in `~/.theclaw/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Release docs: `docs/reference/RELEASING.md`; use `$theclaw-release-maintainer`.
- GHSA/advisories: `$theclaw-ghsa-maintainer`.
...
- Mac gateway: use app or `theclaw gateway restart/status --deep`; no ad-hoc tmux gateway. Logs: `./scripts/clawlog.sh`.
...
- Mobile LAN pairing: plaintext `ws://` loopback-only. Private-network `ws://` needs `THECLAW_ALLOW_INSECURE_PRIVATE_WS=1`; Tailscale/public use `wss://` or tunnel.
- A2UI hash `src/canvas-host/a2ui/.bundle.hash`: generated; ignore unless running `pnpm canvas:a2ui:bundle`; commit separately.

## Ops / Footguns

- Remote install docs: `docs/install/{exe-dev,fly,hetzner}.md`. Parallels smoke: `$theclaw-parallels-smoke`; Discord roundtrip: `parallels-discord-roundtrip`.
- Rebrand/migration/config warnings: run `theclaw doctor`.
