...
**TheClaw** is a _personal AI assistant_ you run on your own devices.
...
```bash
npm install -g theclaw@latest
# or: pnpm add -g theclaw@latest

theclaw onboard --install-daemon
```

TheClaw Onboard installs the Gateway daemon (launchd/systemd user service) so it stays running.

## Quick start (TL;DR)

Runtime: **Node 24 (recommended) or Node 22.14+**.

Full beginner guide (auth, pairing, channels): [Getting started](https://docs.theclaw.ai/start/getting-started)
...
# Send a message
theclaw message send --target +1234567890 --message "Hello from TheClaw"
...
Note: signed builds required for macOS permissions to stick across rebuilds (see [macOS Permissions](https://docs.theclaw.ai/platforms/mac/permissions)).
...
# First run only (or after resetting local TheClaw config/workspace)
pnpm theclaw setup
...
`pnpm theclaw setup` writes the local config/workspace needed for `pnpm gateway:watch`. It is safe to re-run, but you normally only need it on first setup or after resetting local state. `pnpm gateway:watch` does not rebuild `dist/control-ui`, so rerun `pnpm ui:build` after `ui/` changes or use `pnpm ui:dev` when iterating on the Control UI. If you want this checkout to run onboarding directly, use `pnpm theclaw onboard --install-daemon`.

Note: `pnpm theclaw ...` runs TypeScript directly (via `tsx`). `pnpm build` produces `dist/` for running via Node / the packaged `theclaw` binary, while `pnpm gateway:watch` rebuilds the runtime on demand during the dev loop.
...
- Workspace root: `~/.theclaw/workspace` (configurable via `agents.defaults.workspace`).
- Injected prompt files: `AGENTS.md`, `SOUL.md`, `TOOLS.md`.
- Skills: `~/.theclaw/workspace/skills/<skill>/SKILL.md`.

## Configuration

Minimal `~/.theclaw/theclaw.json` (model + defaults):
