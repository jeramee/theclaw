...
VERSION = "0.5.0-streaming-tracked-content-and-path-rename"
DEFAULT_ROOT = "/home/jeramee/.openclaw/workspace/control-repos/tier2/theclaw"

# One contract used for BOTH file contents and tracked paths/filenames.
...
def apply_content_replacements(root: Path, files: list[Path], report_path: Path, *, dry_run: bool) -> dict:
    summary = {
        "tracked_files_checked_for_content": 0,
        "content_binary_or_skipped_files": 0,
        "content_files_changed": 0,
        "content_replacement_total": 0,
    }
    ...
    for rel_path in files:
        ...
        if not looks_rewriteable_content(abs_path):
            summary["content_binary_or_skipped_files"] += 1
            continue

        summary["tracked_files_checked_for_content"] += 1
        ...
        summary["content_files_changed"] += 1
        summary["content_replacement_total"] += replacement_total
...
def scan_leftover_tracked_content(root: Path, files: list[Path], report_path: Path, *, limit: int) -> dict:
    ...
    for rel_path in files:
        ...
        renamed_rel = Path(rename_path_text(rel_path.as_posix()))
        abs_path = root / renamed_rel
        ...
        for needle in LEFTOVER_NEEDLES:
            ...
            files_with_hits.add(renamed_rel.as_posix())
            write_jsonl(report_path, {
                "phase": "leftover",
                "path": renamed_rel.as_posix(),
...
def main() -> int:
    ...
    content_summary = apply_content_replacements(root, files, report_path, dry_run=dry_run)
    path_changes = collect_path_renames(root, files)
    ...
    leftover_summary = scan_leftover_tracked_content(root, files, report_path, limit=args.leftover_limit)
    ...
    print(f"tracked_files_seen={summary['tracked_files_seen']}")
    print(f"content_files_changed={summary['content_files_changed']}")
    print(f"content_replacement_total={summary['content_replacement_total']}")
