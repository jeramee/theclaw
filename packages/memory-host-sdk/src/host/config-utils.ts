...
function resolveRequiredHomeDir(
  ...
  const explicitHome = normalizeHomeValue(env.THECLAW_HOME);
...
export function resolveStateDir(
  ...
  if (env.THECLAW_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
...
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const profile = env.THECLAW_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".theclaw", `workspace-${profile}`);
  }
  return path.join(home, ".theclaw", "workspace");
}

function listAgentEntries(cfg: TheClawConfig): AgentConfig[] {
  return Array.isArray(cfg.agents?.list)
    ? cfg.agents.list.filter((entry): entry is AgentConfig => Boolean(entry))
    : [];
}

function resolveDefaultAgentId(cfg: TheClawConfig): string {
...
function resolveAgentConfig(cfg: TheClawConfig, agentId: string): AgentConfig | undefined {
