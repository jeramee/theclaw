...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `theclaw: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const respawnWithoutCompileCacheIfNeeded = () => {
  ...
  if (process.env.THECLAW_SOURCE_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["theclaw: missing dist/entry.(m)js (build output)."];
  ...
  lines.push(
    "For pinned GitHub installs, use `npm install -g github:theclaw/theclaw#<ref>` instead of a raw `/archive/<ref>.tar.gz` URL.",
  );
  lines.push("For releases, use `npm install -g theclaw@latest`.");
  return lines.join("\n");
};
...
const isHelpFastPathDisabled = () =>
  process.env.THECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
