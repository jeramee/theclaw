...
const extensionRestartMetadataFiles = new Set(["theclaw.plugin.json", "package.json"]);
...
export const resolveBuildRequirement = (deps) => {
  if (deps.env.THECLAW_FORCE_BUILD === "1") {
    return { shouldBuild: true, reason: "force_build" };
  }
  if (
    deps.env.THECLAW_BUILD_PRIVATE_QA === "1" &&
...
export const resolveRuntimePostBuildRequirement = (deps) => {
  if (deps.env.THECLAW_FORCE_RUNTIME_POSTBUILD === "1") {
...
const BUILD_REASON_LABELS = {
  force_build: "forced by THECLAW_FORCE_BUILD",
...
const RUNTIME_POSTBUILD_REASON_LABELS = {
  force_runtime_postbuild: "forced by THECLAW_FORCE_RUNTIME_POSTBUILD",
...
const RUN_NODE_OUTPUT_LOG_ENV = "THECLAW_RUN_NODE_OUTPUT_LOG";
const RUN_NODE_BUILD_LOCK_TIMEOUT_ENV = "THECLAW_RUN_NODE_BUILD_LOCK_TIMEOUT_MS";
const RUN_NODE_BUILD_LOCK_POLL_ENV = "THECLAW_RUN_NODE_BUILD_LOCK_POLL_MS";
const RUN_NODE_BUILD_LOCK_STALE_ENV = "THECLAW_RUN_NODE_BUILD_LOCK_STALE_MS";
...
const logRunner = (message, deps) => {
  if (deps.env.THECLAW_RUNNER_LOG === "0") {
    return;
  }
  const line = `[theclaw] ${message}\n`;
...
const runTheClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["theclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
...
const closeRunNodeOutputTee = async (deps, exitCode) => {
  ...
  try {
    ...
    deps.stderr.write(
      `[theclaw] Failed to write output log: ${error?.message ?? "unknown error"}\n`,
    );
...
const shouldSkipCleanWatchRuntimeSync = (deps) => deps.env.THECLAW_WATCH_MODE === "1";
...
const shouldUseExistingDistForGatewayClient = (deps, buildRequirement) =>
  ...
  deps.env.THECLAW_FORCE_BUILD !== "1" &&
...
export async function runNodeMain(params = {}) {
  ...
  if (deps.args[0] === "qa") {
    deps.env.THECLAW_BUILD_PRIVATE_QA = "1";
    deps.env.THECLAW_ENABLE_PRIVATE_QA_CLI = "1";
  }
  ...
  try {
    ...
    if (!buildRequirement.shouldBuild) {
      ...
      exitCode = await runTheClaw(deps);
