...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${THECLAW_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${THECLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${THECLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
COMMAND_TIMEOUT_MS="${THECLAW_BUN_GLOBAL_SMOKE_TIMEOUT_MS:-180000}"
SMOKE_DIR=""
PACK_DIR=""
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "THECLAW_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (THECLAW_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set THECLAW_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
  ...
  PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/theclaw-bun-pack.XXXXXX")"
  pack_json_file="$PACK_DIR/pack.json"

  echo "==> Pack TheClaw tarball"
...
' "$pack_json_file" "$PACK_DIR"
  ...
  if [ -z "$PACKAGE_TGZ" ] || [ ! -f "$PACKAGE_TGZ" ]; then
    echo "missing packed TheClaw tarball" >&2
    exit 1
  fi
}
...
main() {
  ...
  echo "==> Bun global install packed TheClaw"
  "$bun_path" install -g "$PACKAGE_TGZ" --no-progress

  theclaw_bin="$BUN_INSTALL/bin/theclaw"
  if [ ! -x "$theclaw_bin" ]; then
    theclaw_bin="$(command -v theclaw || true)"
  fi
  if [ -z "$theclaw_bin" ] || [ ! -x "$theclaw_bin" ]; then
    echo "Bun global install did not create an executable theclaw binary" >&2
    exit 1
  fi

  echo "==> TheClaw version through Bun global install"
  run_with_timeout "$COMMAND_TIMEOUT_MS" "$theclaw_bin" --version

  echo "==> TheClaw image providers through Bun global install"
  local providers_json
  providers_json="$(run_with_timeout "$COMMAND_TIMEOUT_MS" "$theclaw_bin" infer image providers --json)"
  THECLAW_IMAGE_PROVIDERS_JSON="$providers_json" node - <<'NODE'
const raw = process.env.THECLAW_IMAGE_PROVIDERS_JSON ?? "";
let parsed;
try {
