...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-plugins-e2e" THECLAW_PLUGINS_E2E_IMAGE)"

docker_e2e_build_or_reuse "$IMAGE_NAME" plugins

THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugins empty)"
DOCKER_ENV_ARGS=(
  -e COREPACK_ENABLE_DOWNLOAD_PROMPT=0
  -e "THECLAW_TEST_STATE_SCRIPT_B64=$THECLAW_TEST_STATE_SCRIPT_B64"
)
for env_name in \
  THECLAW_PLUGINS_E2E_CLAWHUB \
  THECLAW_PLUGINS_E2E_CLAWHUB_SPEC \
  THECLAW_PLUGINS_E2E_CLAWHUB_ID \
  THECLAW_CLAWHUB_URL \
  CLAWHUB_URL \
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/theclaw-plugins-run.XXXXXX")"
...
THECLAW_PLUGIN_HOME="$HOME/.theclaw/$BUNDLED_PLUGIN_ROOT_DIR"
...
const ledgerPath = path.join(process.env.HOME, ".theclaw", "plugins", "installs.json");
const ledger = fs.existsSync(ledgerPath)
  ? JSON.parse(fs.readFileSync(ledgerPath, "utf8"))
  : {
      version: 1,
      warning:
        "DO NOT EDIT. This file is generated by TheClaw plugin install/update/uninstall commands. Use `theclaw plugins install/update/uninstall` instead.",
      records: {},
    };
...
run_logged() {
  local label="$1"
  shift
  local log_file="/tmp/theclaw-plugins-e2e-${label}.log"
...
JS
  cat > "$dir/theclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/theclaw.plugin.json" "$id"
const fs = require("node:fs");
...
demo_plugin_root="$THECLAW_PLUGIN_HOME/$demo_plugin_id"
...
JS
cat > "$demo_plugin_root/theclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins.json
node "$THECLAW_ENTRY" plugins inspect demo-plugin --json > /tmp/plugins-inspect.json
...
pack_dir="$(mktemp -d "/tmp/theclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@theclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "theclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/theclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
run_logged install-tgz node "$THECLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins2.json
node "$THECLAW_ENTRY" plugins inspect demo-plugin-tgz --json > /tmp/plugins2-inspect.json
...
dir_plugin="$(mktemp -d "/tmp/theclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@theclaw/demo-plugin-dir",
  "version": "0.0.1",
  "theclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/theclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
run_logged install-dir node "$THECLAW_ENTRY" plugins install "$dir_plugin"
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins3.json
node "$THECLAW_ENTRY" plugins inspect demo-plugin-dir --json > /tmp/plugins3-inspect.json
...
file_pack_dir="$(mktemp -d "/tmp/theclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@theclaw/demo-plugin-file",
  "version": "0.0.1",
  "theclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/theclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
run_logged install-file node "$THECLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins4.json
node "$THECLAW_ENTRY" plugins inspect demo-plugin-file --json > /tmp/plugins4-inspect.json
...
bundle_root="$THECLAW_PLUGIN_HOME/$bundle_plugin_id"
...
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins-bundle-disabled.json
node - <<'NODE'
const fs = require("node:fs");
...
run_logged enable-claude-bundle node "$THECLAW_ENTRY" plugins enable claude-bundle-e2e
node "$THECLAW_ENTRY" plugins inspect claude-bundle-e2e --json > /tmp/plugins-bundle-inspect.json
node - <<'NODE'
const fs = require("node:fs");
...
slash_install_dir="$(mktemp -d "/tmp/theclaw-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@theclaw/slash-install-plugin",
  "version": "0.0.1",
  "theclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/theclaw.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
run_logged install-slash-plugin node "$THECLAW_ENTRY" plugins install "$slash_install_dir"
node "$THECLAW_ENTRY" plugins inspect slash-install-plugin --json > /tmp/plugin-command-install-show.json
node - <<'NODE'
const fs = require("node:fs");
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "theclaw/fixture-marketplace"
    }
  }
}
JSON

node "$THECLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
run_logged install-marketplace-shortcut node "$THECLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
run_logged install-marketplace-direct node "$THECLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
node "$THECLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-shortcut-inspect.json
node "$THECLAW_ENTRY" plugins inspect marketplace-direct --json > /tmp/plugins-marketplace-direct-inspect.json
...
const allowLegacyCompat = process.env.THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1";
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
run_logged update-marketplace-shortcut-dry-run node "$THECLAW_ENTRY" plugins update marketplace-shortcut --dry-run
run_logged update-marketplace-shortcut node "$THECLAW_ENTRY" plugins update marketplace-shortcut
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
node "$THECLAW_ENTRY" plugins inspect marketplace-shortcut --json > /tmp/plugins-marketplace-updated-inspect.json
...
if [ "${THECLAW_PLUGINS_E2E_CLAWHUB:-1}" = "0" ]; then
  echo "Skipping ClawHub plugin install and uninstall (THECLAW_PLUGINS_E2E_CLAWHUB=0)."
else
echo "Testing ClawHub plugin install and uninstall..."
CLAWHUB_PLUGIN_SPEC="${THECLAW_PLUGINS_E2E_CLAWHUB_SPEC:-clawhub:theclaw-now4real}"
CLAWHUB_PLUGIN_ID="${THECLAW_PLUGINS_E2E_CLAWHUB_ID:-now4real}"
...
const packageName = "theclaw-now4real";
...
NODE
  ...
  for _ in $(seq 1 100); do
    if [[ -s "$server_port_file" ]]; then
      export THECLAW_CLAWHUB_URL="http://127.0.0.1:$(cat "$server_port_file")"
...
if [[ -z "${THECLAW_CLAWHUB_URL:-}" && -z "${CLAWHUB_URL:-}" ]]; then
  # Keep the release-path smoke hermetic; live ClawHub can rate-limit CI.
  clawhub_fixture_dir="$(mktemp -d "/tmp/theclaw-clawhub-fixture.XXXXXX")"
...
const baseUrl = (process.env.THECLAW_CLAWHUB_URL || process.env.CLAWHUB_URL || "https://clawhub.ai")
  .replace(/\/+$/, "");
const token =
  process.env.THECLAW_CLAWHUB_TOKEN ||
...
run_logged install-clawhub node "$THECLAW_ENTRY" plugins install "$CLAWHUB_PLUGIN_SPEC"
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins-clawhub-installed.json
node "$THECLAW_ENTRY" plugins inspect "$CLAWHUB_PLUGIN_ID" --json > /tmp/plugins-clawhub-inspect.json
...
const extensionsRoot = path.join(process.env.HOME, ".theclaw", "extensions");
...
run_logged uninstall-clawhub node "$THECLAW_ENTRY" plugins uninstall "$CLAWHUB_PLUGIN_SPEC" --force
node "$THECLAW_ENTRY" plugins list --json > /tmp/plugins-clawhub-uninstalled.json
...
const configAfterUninstallPath = path.join(process.env.HOME, ".theclaw", "theclaw.json");
