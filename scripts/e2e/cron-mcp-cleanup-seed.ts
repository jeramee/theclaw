...
import { applyDockerOpenAiProviderConfig, type TheClawConfig } from "./docker-openai-seed.ts";
...
process.title = "theclaw-cron-mcp-cleanup-probe";
...
async function main() {
  const stateDir = process.env.THECLAW_STATE_DIR?.trim() || path.join(os.homedir(), ".theclaw");
  const configPath =
    process.env.THECLAW_CONFIG_PATH?.trim() || path.join(stateDir, "theclaw.json");
