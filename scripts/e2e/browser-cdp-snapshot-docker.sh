...
BASE_IMAGE="$(docker_e2e_resolve_image "theclaw-browser-cdp-base-e2e" THECLAW_BROWSER_CDP_BASE_E2E_IMAGE)"
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-browser-cdp-snapshot-e2e" THECLAW_BROWSER_CDP_SNAPSHOT_E2E_IMAGE)"
SKIP_BUILD="${THECLAW_BROWSER_CDP_SNAPSHOT_E2E_SKIP_BUILD:-0}"
PORT="18789"
CDP_PORT="19222"
FIXTURE_PORT="18080"
TOKEN="browser-cdp-e2e-token"
CONTAINER_NAME="theclaw-browser-cdp-e2e-$$"
DOCKER_COMMAND_TIMEOUT="${THECLAW_BROWSER_CDP_SNAPSHOT_DOCKER_COMMAND_TIMEOUT:-900s}"
...
if [ "${THECLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$SKIP_BUILD" = "1" ]; then
  ...
  build_dir="$(mktemp -d "${TMPDIR:-/tmp}/theclaw-browser-cdp-build.XXXXXX")"
...
THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 browser-cdp-snapshot empty)"
...
eval \"\$(printf '%s' \"\${THECLAW_TEST_STATE_SCRIPT_B64:?missing THECLAW_TEST_STATE_SCRIPT_B64}\" | base64 -d)\"
{
  ...
  printf 'export THECLAW_HOME=%q\n' \"\$THECLAW_HOME\"
  printf 'export THECLAW_STATE_DIR=%q\n' \"\$THECLAW_STATE_DIR\"
  printf 'export THECLAW_CONFIG_PATH=%q\n' \"\$THECLAW_CONFIG_PATH\"
  printf 'export THECLAW_AGENT_DIR=%q\n' \"\${THECLAW_AGENT_DIR-}\"
  printf 'export PI_CODING_AGENT_DIR=%q\n' \"\${PI_CODING_AGENT_DIR-}\"
} >/tmp/theclaw-test-state-env
entry=dist/index.mjs
[ -f \"\$entry\" ] || entry=dist/index.js
mkdir -p /tmp/theclaw-browser-cdp/chrome
find dist -maxdepth 1 -type f -name 'pw-ai-*.js' ! -name 'pw-ai-state-*' -exec mv {} /tmp/theclaw-browser-cdp/ \;
...
chromium --headless=new --no-sandbox --disable-gpu --disable-dev-shm-usage \\
  ...
  --user-data-dir=/tmp/theclaw-browser-cdp/chrome \\
...
<html>
  <body>
    <main>
      <button>Save</button>
      <a href=\"https://docs.theclaw.ai/browser-cdp-live\">Docs</a>
...
set -euo pipefail
source /tmp/theclaw-test-state-env
entry=dist/index.mjs
[ -f \"\$entry\" ] || entry=dist/index.js
...
for (const needle of [
  'button \"Save\"',
  'link \"Docs\"',
  'https://docs.theclaw.ai/browser-cdp-live',
