...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".theclaw", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(customEntry.customType === "theclaw.runtime-context", "unexpected custom message type");
  ...
  assert(
    !userText.includes("THECLAW_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-session-runtime-context-"));
  process.env.HOME = root;
  process.env.THECLAW_STATE_DIR = path.join(root, ".theclaw");
  process.env.THECLAW_CONFIG_PATH = path.join(process.env.THECLAW_STATE_DIR, "theclaw.json");
  try {
    ...
    if (process.env.THECLAW_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
