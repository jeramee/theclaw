#!/usr/bin/env bash
# Verifies `theclaw plugins update` is a no-op for an already-current plugin.
...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-plugin-update-e2e" THECLAW_PLUGIN_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${THECLAW_PLUGIN_UPDATE_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz plugin-update "${THECLAW_CURRENT_PACKAGE_TGZ:-}")"
...
THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 plugin-update empty)"
...
npm install -g --prefix /tmp/npm-prefix \"\$package_tgz\" --no-fund --no-audit >/tmp/theclaw-install.log 2>&1
entry=\"/tmp/npm-prefix/lib/node_modules/theclaw/dist/index.mjs\"
[ -f \"\$entry\" ] || entry=/tmp/npm-prefix/lib/node_modules/theclaw/dist/index.js
package_version=\$(node -p \"require('/tmp/npm-prefix/lib/node_modules/theclaw/package.json').version\")
THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT=\$(PACKAGE_VERSION=\"\$package_version\" node -e 'const version = process.env.PACKAGE_VERSION || \"\"; const match = new RegExp(\"^(\\\\d{4})\\\\.(\\\\d{1,2})\\\\.(\\\\d{1,2})(?:[-+].*)?\").exec(version); if (!match) { console.log(\"0\"); process.exit(0); } const value = [Number(match[1]), Number(match[2]), Number(match[3])]; const max = [2026, 4, 25]; for (let i = 0; i < value.length; i += 1) { if (value[i] < max[i]) { console.log(\"1\"); process.exit(0); } if (value[i] > max[i]) { console.log(\"0\"); process.exit(0); } } console.log(\"1\");')
export THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT
export NPM_CONFIG_REGISTRY=http://127.0.0.1:4873
export PATH=\"/tmp/npm-prefix/bin:\$PATH\"

mkdir -p \"\$HOME/.theclaw/extensions/lossless-claw\"
cat > \"\$HOME/.theclaw/extensions/lossless-claw/package.json\" <<'JSON'
{
...
JSON
mkdir -p \"\$HOME/.theclaw/plugins\"
cat > \"\$HOME/.theclaw/plugins/installs.json\" <<'JSON'
{
  \"version\": 1,
  \"warning\": \"DO NOT EDIT. This file is generated by TheClaw plugin registry commands.\",
...
cat > /tmp/theclaw-e2e-registry.mjs <<'NODE'
...
NODE
node /tmp/theclaw-e2e-registry.mjs >/tmp/theclaw-e2e-registry.log 2>&1 &
registry_pid=\$!
...
if [ \"\$registry_ready\" -ne 1 ]; then
  ...
  cat /tmp/theclaw-e2e-registry.log || true
  exit 1
fi

before_config_hash=\"\"
if [ \"\$THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT\" != \"1\" ]; then
  before_config_hash=\$(sha256sum \"\$THECLAW_CONFIG_PATH\" | awk '{print \$1}')
fi
plugin_update_timeout_seconds=\"\${THECLAW_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}\"
...
node --input-type=module > /tmp/plugin-update-before.json <<'NODE'
  ...
  const config = readJson(path.join(home, \".theclaw\", \"theclaw.json\"));
  const index = readJson(path.join(home, \".theclaw\", \"plugins\", \"installs.json\"));
...
if [ -n \"\$before_config_hash\" ]; then
  after_config_hash=\$(sha256sum \"\$THECLAW_CONFIG_PATH\" | awk '{print \$1}')
