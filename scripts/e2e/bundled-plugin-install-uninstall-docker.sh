...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-bundled-plugin-install-uninstall-e2e" THECLAW_BUNDLED_PLUGIN_INSTALL_UNINSTALL_E2E_IMAGE)"

docker_e2e_build_or_reuse "$IMAGE_NAME" bundled-plugin-install-uninstall
THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 bundled-plugin-install-uninstall empty)"
...
for env_name in \
  THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL \
  THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX \
  THECLAW_BUNDLED_PLUGIN_SWEEP_IDS; do
  env_value="${!env_name:-}"
...
RUN_LOG="$(mktemp "${TMPDIR:-/tmp}/theclaw-bundled-plugin-install-uninstall.XXXXXX")"
...
const explicit = (process.env.THECLAW_BUNDLED_PLUGIN_SWEEP_IDS || "")
  .split(/[,\s]+/u)
...
const manifestEntries = fs
  ...
  .map((entry) => {
    const manifestPath = path.join(extensionRoot, entry.name, "theclaw.plugin.json");
...
const total = Number.parseInt(process.env.THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL || "1", 10);
const index = Number.parseInt(process.env.THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX || "0", 10);
if (!Number.isInteger(total) || total < 1) {
  throw new Error(`THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL must be >= 1, got ${process.env.THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL}`);
}
if (!Number.isInteger(index) || index < 0 || index >= total) {
  throw new Error(`THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX}`);
}
...
echo "Selected ${#plugin_entries[@]} bundled plugins for shard ${THECLAW_BUNDLED_PLUGIN_SWEEP_INDEX:-0}/${THECLAW_BUNDLED_PLUGIN_SWEEP_TOTAL:-1}: ${selected_labels[*]}"
...
const indexPath = path.join(process.env.HOME, ".theclaw", "plugins", "installs.json");
...
const managedPath = path.join(process.env.HOME, ".theclaw", "extensions", pluginId);
...
for plugin_entry in "${plugin_entries[@]}"; do
  ...
  install_log="/tmp/theclaw-install-${plugin_index}.log"
  uninstall_log="/tmp/theclaw-uninstall-${plugin_index}.log"
  echo "Installing bundled plugin: $plugin_id ($plugin_dir)"
  node "$THECLAW_ENTRY" plugins install "$plugin_id" >"$install_log" 2>&1 || {
    cat "$install_log"
    exit 1
  }
  ...
  node "$THECLAW_ENTRY" plugins uninstall "$plugin_id" --force >"$uninstall_log" 2>&1 || {
    cat "$uninstall_log"
    exit 1
  }
