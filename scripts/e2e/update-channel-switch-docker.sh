...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-update-channel-switch-e2e" THECLAW_UPDATE_CHANNEL_SWITCH_E2E_IMAGE)"
SKIP_BUILD="${THECLAW_UPDATE_CHANNEL_SWITCH_E2E_SKIP_BUILD:-0}"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz update-channel-switch "${THECLAW_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
THECLAW_TEST_STATE_SCRIPT_B64="$(
  node "$ROOT_DIR/scripts/lib/theclaw-test-state.mjs" shell \
...
const packageJsonPath = "/tmp/theclaw-git/package.json";
...
fs.mkdirSync("/tmp/theclaw-git/dist/control-ui", { recursive: true });
fs.writeFileSync("/tmp/theclaw-git/dist/control-ui/index.html", "<!doctype html><title>fixture</title>\n");
NODE
(
  cd "$git_root"
...
git config --global user.email "docker-e2e@theclaw.local"
git config --global user.name "TheClaw Docker E2E"
...
package_version="$(node -p "JSON.parse(require(\"node:fs\").readFileSync(\"/tmp/npm-prefix/lib/node_modules/theclaw/package.json\", \"utf8\")).version")"
THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT="$(
...
export THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT

eval "$(printf "%s" "${THECLAW_TEST_STATE_SCRIPT_B64:?missing THECLAW_TEST_STATE_SCRIPT_B64}" | base64 -d)"

export THECLAW_GIT_DIR="$git_root"
export THECLAW_UPDATE_DEV_TARGET_REF="$fixture_sha"

echo "==> package -> git dev channel"
set +e
dev_json="$(theclaw update --channel dev --yes --json --no-restart)"
dev_status=$?
set -e
printf "%s\n" "$dev_json"
...
if (config.update?.channel !== "dev") {
  if (process.env.THECLAW_PACKAGE_ACCEPTANCE_LEGACY_COMPAT === "1") {
...
status_json="$(theclaw update status --json)"
printf "%s\n" "$status_json"
...
set +e
stable_json="$(theclaw update --channel stable --tag "$pkg_tgz_path" --yes --json --no-restart)"
stable_status=$?
set -e
printf "%s\n" "$stable_json"
