...
# git installs. Both fixtures come from the same prepared TheClaw npm tarball.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "$ROOT_DIR/scripts/lib/docker-e2e-image.sh"
source "$ROOT_DIR/scripts/lib/docker-e2e-package.sh"
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-doctor-install-switch-e2e" THECLAW_DOCTOR_INSTALL_SWITCH_E2E_IMAGE)"
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz doctor-switch "${THECLAW_CURRENT_PACKAGE_TGZ:-}")"
# Bare lanes mount the package artifact instead of baking app sources into the image.
docker_e2e_package_mount_args "$PACKAGE_TGZ"
THECLAW_TEST_STATE_FUNCTION_B64="$(docker_e2e_test_state_function_b64)"
...
docker run --rm \
  ...
  -e "THECLAW_TEST_STATE_FUNCTION_B64=$THECLAW_TEST_STATE_FUNCTION_B64" \
  "${DOCKER_E2E_PACKAGE_ARGS[@]}" \
  "$IMAGE_NAME" \
  bash -lc '
  set -euo pipefail
  eval "$(printf "%s" "${THECLAW_TEST_STATE_FUNCTION_B64:?missing THECLAW_TEST_STATE_FUNCTION_B64}" | base64 -d)"

  # Keep logs focused; the npm global install step can emit noisy deprecation warnings.
  export npm_config_loglevel=error
  export npm_config_fund=false
  export npm_config_audit=false
  export THECLAW_DISABLE_BUNDLED_PLUGINS=1

  # Stub systemd/loginctl so doctor + daemon flows work in Docker.
  export PATH="/tmp/theclaw-bin:$PATH"
  mkdir -p /tmp/theclaw-bin

  cat > /tmp/theclaw-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/theclaw-bin/systemctl

  cat > /tmp/theclaw-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
LOGINCTL
  ...
  (
    cd "$git_root"
    npm install --omit=optional --no-fund --no-audit >/tmp/theclaw-git-install.log 2>&1
    git init -q
    git config user.email "docker-e2e@theclaw.local"
    git config user.name "TheClaw Docker E2E"
    git add -A
    git commit -qm "test fixture"
  )
  npm_log="/tmp/theclaw-doctor-switch-npm-install.log"
  if ! npm install -g --prefix /tmp/npm-prefix "$package_tgz" >"$npm_log" 2>&1; then
    cat "$npm_log"
    exit 1
  fi

	  npm_bin="/tmp/npm-prefix/bin/theclaw"
	  npm_root="/tmp/npm-prefix/lib/node_modules/theclaw"
...
NODE
  ...
  run_flow() {
    ...
    local install_log="/tmp/theclaw-doctor-switch-${name}-install.log"
    local doctor_log="/tmp/theclaw-doctor-switch-${name}-doctor.log"
    local command_timeout="${THECLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-300s}"

    echo "== Flow: $name =="
    theclaw_test_state_create "switch-${name}" empty
    ...
    unit_path="$HOME/.config/systemd/user/theclaw-gateway.service"
    if [ ! -f "$unit_path" ]; then
  ...
  run_wrapper_flow() {
    ...
    local install_log="/tmp/theclaw-doctor-switch-${name}-install.log"
    local reinstall_log="/tmp/theclaw-doctor-switch-${name}-reinstall.log"
    local env_repair_log="/tmp/theclaw-doctor-switch-${name}-env-repair.log"
    local doctor_log="/tmp/theclaw-doctor-switch-${name}-doctor.log"
    local clear_log="/tmp/theclaw-doctor-switch-${name}-clear.log"
    local command_timeout="${THECLAW_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-300s}"
    ...
    local wrapper="$HOME/.local/bin/theclaw-wrapper"
    cat > "$wrapper" <<WRAPPER
#!/usr/bin/env bash
set -euo pipefail
printf "%s\n" "\$@" >> "$HOME/theclaw-wrapper-argv.log"
exec "$npm_bin" "\$@"
WRAPPER
    chmod +x "$wrapper"

    local unit_path="$HOME/.config/systemd/user/theclaw-gateway.service"
    ...
    assert_env_value "$unit_path" "THECLAW_WRAPPER" "$wrapper"
    ...
    sed -i "/^Environment=THECLAW_WRAPPER=/d" "$unit_path"
    ...
    sed -i "s#^Environment=THECLAW_WRAPPER=.*#Environment=THECLAW_WRAPPER=/tmp/stale-theclaw-wrapper#" "$unit_path"
    if ! timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
      cat "$env_repair_log"
      exit 1
    fi
    ...
    if ! grep -Fq "Gateway service invokes THECLAW_WRAPPER:" "$doctor_log"; then
    ...
    if ! timeout "$command_timeout" env THECLAW_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
      cat "$clear_log"
      exit 1
    fi
    assert_no_env_key "$unit_path" "THECLAW_WRAPPER"
