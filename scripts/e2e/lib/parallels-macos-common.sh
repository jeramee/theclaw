...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/theclaw-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/theclaw-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${THECLAW_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${THECLAW_SECRET_ENV_NAME:-}" ] && [ -n "${THECLAW_SECRET_FILE:-}" ] && [ -f "$THECLAW_SECRET_FILE" ]; then
  secret_value="$(cat "$THECLAW_SECRET_FILE")"
  rm -f "$THECLAW_SECRET_FILE"
  export "${THECLAW_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "THECLAW_SECRET_ENV_NAME=$api_key_env" \
      "THECLAW_SECRET_FILE=$secret_path" \
      "THECLAW_WRAPPER_FILE=$wrapper_path" \
