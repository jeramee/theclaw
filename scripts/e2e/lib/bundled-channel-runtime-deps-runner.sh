...
run_bundled_channel_runtime_dep_scenarios() {
  if [ "$RUN_CHANNEL_SCENARIOS" != "0" ]; then
    IFS=',' read -r -a CHANNEL_SCENARIOS <<<"${THECLAW_BUNDLED_CHANNELS:-${CHANNEL_ONLY:-telegram,discord,slack,feishu,memory-lancedb}}"
    for channel_scenario in "${CHANNEL_SCENARIOS[@]}"; do
      ...
      case "$channel_scenario" in
        ...
        *)
          echo "Unsupported THECLAW_BUNDLED_CHANNELS entry: $channel_scenario" >&2
          exit 1
          ;;
      esac
    done
