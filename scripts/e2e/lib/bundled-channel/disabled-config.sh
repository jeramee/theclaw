...
assert_dep_absent_everywhere() {
  ...
  if ! node - <<'NODE' "$THECLAW_PLUGIN_STAGE_DIR" "$dep_path"
const fs = require("node:fs");
const path = require("node:path");

const stageDir = process.argv[2];
const depName = process.argv[3];
const manifestName = ".theclaw-runtime-deps.json";
const matches = [];
...
NODE
  then
    ...
    cat /tmp/theclaw-disabled-config-doctor.log >&2
    exit 1
  fi
}
...
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/theclaw-disabled-config-install.log 2>&1
...
if ! theclaw doctor --non-interactive >/tmp/theclaw-disabled-config-doctor.log 2>&1; then
...
if grep -Eq "(grammy|@slack/web-api|discord-api-types)" /tmp/theclaw-disabled-config-doctor.log; then
