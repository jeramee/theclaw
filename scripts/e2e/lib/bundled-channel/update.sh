...
run_update_scenario() {
  ...
  if ! timeout "$DOCKER_RUN_TIMEOUT" docker run --rm \
    ...
    -e THECLAW_BUNDLED_CHANNEL_UPDATE_BASELINE_VERSION="$UPDATE_BASELINE_VERSION" \
    -e "THECLAW_BUNDLED_CHANNEL_UPDATE_TARGETS=${THECLAW_BUNDLED_CHANNEL_UPDATE_TARGETS:-telegram,discord,slack,feishu,memory-lancedb,acpx}" \
    -e "THECLAW_TEST_STATE_SCRIPT_B64=$state_script_b64" \
...
export OPENAI_API_KEY="sk-theclaw-bundled-channel-update-e2e"
export THECLAW_NO_ONBOARD=1
export THECLAW_UPDATE_PACKAGE_SPEC=""

TOKEN="bundled-channel-update-token"
PORT="18790"
UPDATE_TARGETS="${THECLAW_BUNDLED_CHANNEL_UPDATE_TARGETS:-telegram,discord,slack,feishu,memory-lancedb,acpx}"

package_root() {
  printf "%s/theclaw" "$(npm root -g)"
}

stage_root() {
  printf "%s/.theclaw/plugin-runtime-deps" "$HOME"
}

poison_home_npm_project() {
  printf '{"name":"theclaw-home-prefix-poison","private":true}\n' >"$HOME/package.json"
...
assert_no_unknown_stage_roots() {
  if find "$(stage_root)" -maxdepth 1 -type d -name 'theclaw-unknown-*' -print -quit 2>/dev/null | grep -q .; then
    echo "runtime deps created second-generation unknown stage roots" >&2
    find "$(stage_root)" -maxdepth 1 -type d -name 'theclaw-*' -print | sort >&2 || true
    exit 1
  fi
}
    ...
    entries: {
      ...
      "memory-lancedb": {
        ...
        config: {
          ...
          dbPath: "~/.theclaw/memory/lancedb-update-e2e",
          autoCapture: false,
          autoRecall: false,
        },
      },
...
const doctor = steps.find((step) => step?.name === "theclaw doctor");
if (!doctor) {
  throw new Error("missing theclaw doctor step");
}
if (Number(doctor.exitCode ?? 1) !== 0) {
  throw new Error(`theclaw doctor step failed: ${JSON.stringify(doctor)}`);
}
NODE
}
...
run_update_and_capture() {
  ...
  local out_file="$2"
  set +e
  theclaw update --tag "$update_target" --yes --json >"$out_file" 2>"/tmp/theclaw-$label-update.stderr"
  local status=$?
  set -e
  if [ "$status" -ne 0 ]; then
    echo "theclaw update failed for $label with exit code $status" >&2
    cat "$out_file" >&2 || true
    cat "/tmp/theclaw-$label-update.stderr" >&2 || true
    exit "$status"
  fi
}
...
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/theclaw-update-baseline-install.log 2>&1
...
if should_run_update_target telegram; then
  ...
  set +e
  theclaw doctor --non-interactive >/tmp/theclaw-baseline-doctor.log 2>&1
  ...
  run_update_and_capture telegram /tmp/theclaw-update-telegram.json
  cat /tmp/theclaw-update-telegram.json
  assert_update_ok /tmp/theclaw-update-telegram.json "$candidate_version"
  ...
  remove_runtime_dep telegram grammy
  assert_no_dep_available telegram grammy
  if ! THECLAW_UPDATE_IN_PROGRESS=1 theclaw doctor --non-interactive >/tmp/theclaw-update-mode-doctor.log 2>&1; then
    echo "update-mode doctor failed while repairing Telegram deps" >&2
    cat /tmp/theclaw-update-mode-doctor.log >&2
    exit 1
  fi
  assert_dep_available telegram grammy
  assert_no_unknown_stage_roots
...
if should_run_update_target discord; then
  ...
  run_update_and_capture discord /tmp/theclaw-update-discord.json
  cat /tmp/theclaw-update-discord.json
  assert_update_ok /tmp/theclaw-update-discord.json "$candidate_version"
...
if should_run_update_target slack; then
  ...
  run_update_and_capture slack /tmp/theclaw-update-slack.json
  cat /tmp/theclaw-update-slack.json
  assert_update_ok /tmp/theclaw-update-slack.json "$candidate_version"
...
if should_run_update_target feishu; then
  ...
  run_update_and_capture feishu /tmp/theclaw-update-feishu.json
  cat /tmp/theclaw-update-feishu.json
  assert_update_ok /tmp/theclaw-update-feishu.json "$candidate_version"
...
if should_run_update_target memory-lancedb; then
  ...
  run_update_and_capture memory-lancedb /tmp/theclaw-update-memory-lancedb.json
  cat /tmp/theclaw-update-memory-lancedb.json
  assert_update_ok /tmp/theclaw-update-memory-lancedb.json "$candidate_version"
...
if should_run_update_target acpx; then
  ...
  run_update_and_capture acpx /tmp/theclaw-update-acpx.json
  cat /tmp/theclaw-update-acpx.json
  assert_update_ok /tmp/theclaw-update-acpx.json "$candidate_version"
