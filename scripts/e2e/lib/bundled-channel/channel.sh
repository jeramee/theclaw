...
run_channel_scenario() {
  ...
  if ! timeout "$DOCKER_RUN_TIMEOUT" docker run --rm \
    ...
    -e THECLAW_CHANNEL_UNDER_TEST="$channel" \
    -e THECLAW_DEP_SENTINEL="$dep_sentinel" \
...
export OPENAI_API_KEY="sk-theclaw-bundled-channel-deps-e2e"
export THECLAW_NO_ONBOARD=1

TOKEN="bundled-channel-deps-token"
PORT="18789"
CHANNEL="${THECLAW_CHANNEL_UNDER_TEST:?missing THECLAW_CHANNEL_UNDER_TEST}"
DEP_SENTINEL="${THECLAW_DEP_SENTINEL:?missing THECLAW_DEP_SENTINEL}"
gateway_pid=""
    ...
    entries: {
      ...
      "memory-lancedb": {
        ...
        config: {
          ...
          dbPath: "~/.theclaw/memory/lancedb-e2e",
          autoCapture: false,
          autoRecall: false,
        },
      },
...
start_gateway() {
  ...
  if [ "$skip_sidecars" = "1" ]; then
    THECLAW_SKIP_CHANNELS=1 THECLAW_SKIP_PROVIDERS=1 \
      theclaw gateway --port "$PORT" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  else
    theclaw gateway --port "$PORT" --bind loopback --allow-unconfigured >"$log_file" 2>&1 &
  fi
  gateway_pid="$!"
...
assert_channel_status() {
  ...
  local out="/tmp/theclaw-channel-status-$channel.json"
  local err="/tmp/theclaw-channel-status-$channel.err"
  local parse_err="/tmp/theclaw-channel-status-$channel.parse.err"
  local parse_out="/tmp/theclaw-channel-status-$channel.parse.out"
  for _ in $(seq 1 30); do
    if theclaw gateway call channels.status \
    ...
    if grep -Eq "\\[gateway\\] ready \\(.*\\b$channel\\b" /tmp/theclaw-"$channel"-*.log 2>/dev/null; then
  ...
  cat /tmp/theclaw-"$channel"-*.log >&2 2>/dev/null || true
  return 1
}
...
assert_no_install_stage() {
  local channel="$1"
  local stage="$package_root/dist/extensions/$channel/.theclaw-install-stage"
  if [ -e "$stage" ]; then
...
start_gateway "/tmp/theclaw-$CHANNEL-baseline.log" 1
wait_for_gateway_health "/tmp/theclaw-$CHANNEL-baseline.log"
stop_gateway
...
start_gateway "/tmp/theclaw-$CHANNEL-first.log"
wait_for_gateway_health "/tmp/theclaw-$CHANNEL-first.log"
assert_installed_once "/tmp/theclaw-$CHANNEL-first.log" "$CHANNEL" "$DEP_SENTINEL"
...
start_gateway "/tmp/theclaw-$CHANNEL-second.log"
wait_for_gateway_health "/tmp/theclaw-$CHANNEL-second.log"
assert_not_installed "/tmp/theclaw-$CHANNEL-second.log" "$CHANNEL"
assert_no_install_stage "$CHANNEL"
assert_channel_status "$CHANNEL"
stop_gateway
