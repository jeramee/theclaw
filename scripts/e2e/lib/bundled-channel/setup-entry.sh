...
export THECLAW_PLUGIN_STAGE_DIR="$HOME/.theclaw/plugin-runtime-deps"
mkdir -p "$THECLAW_PLUGIN_STAGE_DIR"
...
if ! npm install -g "$package_tgz" --no-fund --no-audit >/tmp/theclaw-setup-entry-install.log 2>&1; then
  cat /tmp/theclaw-setup-entry-install.log >&2 || true
  exit 1
fi
...
for channel in "${!SETUP_ENTRY_DEP_SENTINELS[@]}"; do
  ...
  if find "$THECLAW_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$dep_sentinel/package.json" -type f | grep -q .; then
    ...
    find "$THECLAW_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
    exit 1
  fi
done

echo "Running packaged guided WhatsApp setup; runtime deps should be staged before finalize..."
THECLAW_PACKAGE_ROOT="$root" node --input-type=module - <<'NODE'
import path from "node:path";
import { readdir } from "node:fs/promises";
import { pathToFileURL } from "node:url";

const root = process.env.THECLAW_PACKAGE_ROOT;
if (!root) {
  throw new Error("missing THECLAW_PACKAGE_ROOT");
}
const distDir = path.join(root, "dist");
...
if ! find "$THECLAW_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/@whiskeysockets/baileys/package.json" -type f | grep -q .; then
...
theclaw doctor --non-interactive >/tmp/theclaw-setup-entry-doctor.log 2>&1
...
for channel in "${!SETUP_ENTRY_DEP_SENTINELS[@]}"; do
  ...
  if ! find "$THECLAW_PLUGIN_STAGE_DIR" -maxdepth 12 -path "*/node_modules/$dep_sentinel/package.json" -type f | grep -q .; then
    ...
    cat /tmp/theclaw-setup-entry-doctor.log >&2
    find "$THECLAW_PLUGIN_STAGE_DIR" -maxdepth 12 -type f | sort | head -160 >&2 || true
    exit 1
  fi
done
