...
npm install -g "$package_tgz" --no-fund --no-audit >/tmp/theclaw-load-failure-install.log 2>&1
...
{
  "name": "@theclaw/load-failure-alpha",
  "version": "2026.4.21",
  "private": true,
  "type": "module",
  "theclaw": {
...
(
  cd "$root"
  THECLAW_BUNDLED_PLUGINS_DIR="$root/dist/extensions" node --input-type=module - <<'NODE'
