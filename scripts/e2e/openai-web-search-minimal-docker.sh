...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-openai-web-search-minimal-e2e" THECLAW_OPENAI_WEB_SEARCH_MINIMAL_E2E_IMAGE)"
SKIP_BUILD="${THECLAW_OPENAI_WEB_SEARCH_MINIMAL_E2E_SKIP_BUILD:-0}"
PORT="18789"
MOCK_PORT="80"
...
THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 openai-web-search-minimal empty)"
...
run_logged openai-web-search-minimal docker run --rm \
  ...
  -e "THECLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "OPENAI_API_KEY=sk-theclaw-web-search-minimal-e2e" \
...
export THECLAW_SKIP_BROWSER_CONTROL_SERVER=1
export THECLAW_SKIP_ACPX_RUNTIME=1
export THECLAW_SKIP_ACPX_RUNTIME_PROBE=1

PORT="${PORT:?missing PORT}"
MOCK_PORT="${MOCK_PORT:?missing MOCK_PORT}"
TOKEN="${THECLAW_GATEWAY_TOKEN:?missing THECLAW_GATEWAY_TOKEN}"
SUCCESS_MARKER="THECLAW_SCHEMA_E2E_OK"
RAW_SCHEMA_ERROR="400 The following tools cannot be used with reasoning.effort 'minimal': web_search."
MOCK_REQUEST_LOG="/tmp/theclaw-openai-web-search-minimal-requests.jsonl"
GATEWAY_LOG="/tmp/theclaw-openai-web-search-minimal-gateway.log"
mock_pid=""
gateway_pid=""
...
dump_debug_logs() {
  ...
  for file in \
    "$GATEWAY_LOG" \
    /tmp/theclaw-openai-web-search-minimal-mock.log \
    /tmp/theclaw-openai-web-search-minimal-client-success.log \
    /tmp/theclaw-openai-web-search-minimal-client-reject.log \
    "$MOCK_REQUEST_LOG" \
    "$THECLAW_STATE_DIR/theclaw.json"; do
    if [ -f "$file" ]; then
      echo "--- $file ---" >&2
...
cat >"$THECLAW_STATE_DIR/theclaw.json" <<JSON
{
  "agents": {
    "defaults": {
...
cat >/tmp/theclaw-openai-web-search-minimal-mock.mjs <<'NODE'
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/v1/models") {
    writeJson(res, 200, {
      object: "list",
      data: [{ id: "gpt-5", object: "model", owned_by: "theclaw-e2e" }],
    });
    return;
  }
...
node /tmp/theclaw-openai-web-search-minimal-mock.mjs >/tmp/theclaw-openai-web-search-minimal-mock.log 2>&1 &
mock_pid="$!"
...
cat >/tmp/theclaw-openai-web-search-minimal-client.mjs <<'NODE'
...
const token = process.env.THECLAW_GATEWAY_TOKEN;
const mode = process.argv[2];
const sessionKey = `agent:main:openai-web-search-minimal:${mode}`;
const message =
  mode === "reject"
    ? "FORCE_SCHEMA_REJECT"
    : "Return exactly THECLAW_SCHEMA_E2E_OK.";
const id = mode === "reject" ? "schema-reject" : "schema-success";

if (!port || !token) throw new Error("missing PORT/THECLAW_GATEWAY_TOKEN");
...
PORT="$PORT" THECLAW_GATEWAY_TOKEN="$TOKEN" node /tmp/theclaw-openai-web-search-minimal-client.mjs success >/tmp/theclaw-openai-web-search-minimal-client-success.log 2>&1
...
const success = responseEntries.find((entry) => JSON.stringify(entry.body).includes("THECLAW_SCHEMA_E2E_OK"));
if (!success) {
...
PORT="$PORT" THECLAW_GATEWAY_TOKEN="$TOKEN" node /tmp/theclaw-openai-web-search-minimal-client.mjs reject >/tmp/theclaw-openai-web-search-minimal-client-reject.log 2>&1
