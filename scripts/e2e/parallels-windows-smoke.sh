...
VM_NAME="Windows 11"
SNAPSHOT_HINT="pre-theclaw-native-e2e-2026-03-12"
MODE="both"
PROVIDER="openai"
API_KEY_ENV=""
AUTH_CHOICE=""
...
MODEL_ID=""
MODEL_ID_EXPLICIT=0
INSTALL_URL="https://theclaw.ai/install.ps1"
HOST_PORT="18426"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/theclaw-parallels-windows.XXXXXX)"
...
TIMEOUT_UPDATE_S="${THECLAW_PARALLELS_WINDOWS_UPDATE_TIMEOUT_S:-1200}"
...
TIMEOUT_AGENT_S="${THECLAW_PARALLELS_WINDOWS_AGENT_TIMEOUT_S:-900}"
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-theclaw-native-e2e-2026-03-12"
  ...
  --install-url <url>        Installer URL for latest release. Default: https://theclaw.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run theclaw update --channel dev.
  --target-package-spec <npm-spec>
                             ...
                             Example: theclaw@2026.3.13-beta.1
...
# theclaw.cmd preserves multi-word --message args reliably here; Start-Process
# against the shim can re-split argv and make Commander reject the turn.
\$output = & \$theclaw @args 2>&1
if (\$null -ne \$output) {
...
guest_run_agent_turn_process() {
  ...
  runner_basename="theclaw-parallels-agent-runner-$RANDOM-$RANDOM.ps1"
  ...
  runner_name="theclaw-parallels-agent-$RANDOM-$RANDOM.ps1"
  stdout_name="theclaw-parallels-agent-$RANDOM-$RANDOM.out.log"
  stderr_name="theclaw-parallels-agent-$RANDOM-$RANDOM.err.log"
  done_name="theclaw-parallels-agent-$RANDOM-$RANDOM.done"
...
EOF
  ...
  while true; do
    ...
    if [[ $done_rc -eq 0 && -n "$done_status" ]]; then
      ...
      guest_powershell_poll 20 "\$runner = Join-Path \$env:TEMP '$runner_name'; \$done = Join-Path \$env:TEMP '$done_name'; Remove-Item \$runner, \$done -Force -ErrorAction SilentlyContinue"
      warn "theclaw agent finished without OK response (exit $done_status)"
      return 1
    fi
    ...
    if (( SECONDS >= poll_deadline )); then
      ...
      guest_powershell_poll 20 "\$runner = Join-Path \$env:TEMP '$runner_name'; \$done = Join-Path \$env:TEMP '$done_name'; Remove-Item \$runner, \$done -Force -ErrorAction SilentlyContinue"
      warn "theclaw agent timed out after $TIMEOUT_AGENT_S seconds"
      return 1
    fi
    sleep 2
  done
...
$processes = Get-CimInstance Win32_Process | Where-Object {
  $_.CommandLine -match 'theclaw(.cmd|.mjs|\\dist\\index.js)?.*gateway'
}
foreach ($process in $processes) {
  Stop-Process -Id $process.ProcessId -Force -ErrorAction SilentlyContinue
}
if ($processes) {
  "Stopped TheClaw gateway processes for local agent turn"
}
EOF
)"
}
...
baseline_install_version() {
  ...
  npm view "theclaw@$INSTALL_VERSION" version --userconfig "$(mktemp)"
}
...
req = urllib.request.Request(
    ...
    headers={
        "User-Agent": "theclaw-parallels-smoke",
...
ensure_current_build() {
  ...
  lock_owned=0
  if [[ "${THECLAW_PARALLELS_BUILD_LOCK_HELD:-0}" != "1" ]]; then
    acquire_build_lock
    lock_owned=1
  fi
...
ensure_guest_git() {
  ...
  guest_powershell "$(cat <<EOF
\$depsRoot = Join-Path \$env:LOCALAPPDATA 'TheClaw\deps'
...
pack_main_tgz() {
  ...
  set +e
  {
    THECLAW_PARALLELS_BUILD_LOCK_HELD=1 ensure_current_build &&
      write_package_dist_inventory &&
  ...
  release_build_lock
  [[ $rc -eq 0 ]] || return "$rc"
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/theclaw-main-$short_head.tgz"
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/theclaw-parallels-windows-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
write_latest_install_runner_script() {
  ...
  WINDOWS_LATEST_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-install-latest.ps1"
...
try {
  ...
  & (Join-Path \$env:APPDATA 'npm\theclaw.cmd') --version *>&1 | Tee-Object -FilePath \$LogPath -Append | Out-Null
  if (\$LASTEXITCODE -ne 0) {
    throw "theclaw --version failed with exit code \$LASTEXITCODE"
  }
...
write_baseline_npm_install_runner_script() {
  WINDOWS_BASELINE_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-install-baseline-npm.ps1"
...
try {
  $portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'TheClaw\deps') 'portable-git') ''
  $env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
  $theclaw = Join-Path $env:APPDATA 'npm\theclaw.cmd'

  Write-ProgressLog 'install.start'
  Invoke-Logged 'npm install baseline release' {
    & npm.cmd install -g "theclaw@$Version" --no-fund --no-audit --loglevel=error
  }

  Write-ProgressLog 'install.version'
  Invoke-Logged 'theclaw --version' { & $theclaw --version }
...
install_baseline_npm_release() {
  ...
  runner_name="theclaw-install-baseline-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-install-baseline-$RANDOM-$RANDOM.log"
  done_name="theclaw-install-baseline-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-install-baseline-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-install-baseline-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
...
install_latest_release() {
  ...
  runner_name="theclaw-install-latest-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-install-latest-$RANDOM-$RANDOM.log"
  done_name="theclaw-install-latest-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-install-latest-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
  startup_checked=0
...
recover_successful_published_install() {
  ...
  [[ -f "$log_path" ]] || return 1
  grep -Fq "TheClaw installed successfully" "$log_path" || return 1
  ...
  set +e
  installed_version="$(guest_run_theclaw "" "" "--version")"
  rc=$?
  set -e
  installed_version="${installed_version//$'\r'/}"
  if [[ $rc -ne 0 || "$installed_version" != *"TheClaw $expected_version "* ]]; then
...
install_main_tgz() {
  ...
  runner_name="theclaw-install-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-install-$RANDOM-$RANDOM.log"
  done_name="theclaw-install-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-install-log-state.XXXXXX")"
  npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-install-npm-log-state.XXXXXX")"
  : >"$log_state_path"
  : >"$npm_log_state_path"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_INSTALL_S + 60))
...
write_dev_update_runner_script() {
  WINDOWS_DEV_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-update-dev.ps1"
...
try {
  ...
  $theclaw = Join-Path $env:APPDATA 'npm\theclaw.cmd'
  $gitRoot = Join-Path $env:USERPROFILE 'theclaw'
  $gitEntry = Join-Path $gitRoot 'theclaw.mjs'
...
"@ | Set-Content -Path $pnpmPsShim -Encoding UTF8
  ...
  Invoke-Logged 'theclaw update --channel dev --yes --json' {
    & $theclaw update --channel dev --yes --json
  }
  ...
  Invoke-Logged 'git theclaw --version' { & node.exe $gitEntry --version }
  Invoke-Logged 'git theclaw update status --json' { & node.exe $gitEntry update status --json }
...
run_dev_channel_update() {
  ...
  runner_name="theclaw-update-dev-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-update-dev-$RANDOM-$RANDOM.log"
  done_name="theclaw-update-dev-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-update-dev-log-state.XXXXXX")"
  : >"$log_state_path"
  start_seconds="$SECONDS"
...
if ($busy) {
  throw 'dev update still has active npm/pnpm/theclaw processes'
}
$gitEntry = Join-Path $env:USERPROFILE 'theclaw\theclaw.mjs'
if (-not (Test-Path $gitEntry)) {
...
write_install_runner_script() {
  WINDOWS_INSTALL_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-install-main.ps1"
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_run_theclaw "" "" "--version")"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
write_onboard_runner_script() {
  WINDOWS_ONBOARD_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-onboard-$PROVIDER.ps1"
...
try {
  ...
  \$cmdLine = ('"{0}" onboard --non-interactive --mode local --auth-choice ${AUTH_CHOICE} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json >> "{1}" 2>&1' -f \$theclaw, \$LogPath)
...
run_ref_onboard() {
  ...
  runner_name="theclaw-onboard-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-onboard-$RANDOM-$RANDOM.log"
  done_name="theclaw-onboard-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
  poll_deadline=$((SECONDS + TIMEOUT_ONBOARD_S + 60))
  startup_checked=0
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-onboard-log-state.XXXXXX")"
  : >"$log_state_path"
...
verify_gateway() {
  guest_run_theclaw "" "" gateway status --deep --require-rpc
}
...
verify_gateway_reachable() {
  ...
  for attempt in 1 2 3 4 5 6; do
    probe_json="$(
      guest_run_theclaw "" "" gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json
    )"
    printf '%s\n' "$probe_json"
...
$portableGit = Join-Path (Join-Path (Join-Path $env:LOCALAPPDATA 'TheClaw\deps') 'portable-git') ''
$env:PATH = "$portableGit\cmd;$portableGit\mingw64\bin;$portableGit\usr\bin;$env:PATH"
$gitEntry = Join-Path (Join-Path $env:USERPROFILE 'theclaw') 'theclaw.mjs'
if (-not (Test-Path $gitEntry)) {
...
run_gateway_daemon_action() {
  ...
  local runner_name log_name done_name done_status launcher_state
  local poll_rc state_rc log_rc start_seconds poll_deadline startup_checked
  runner_name="theclaw-gateway-$action-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-gateway-$action-$RANDOM-$RANDOM.log"
  done_name="theclaw-gateway-$action-$RANDOM-$RANDOM.done"
  start_seconds="$SECONDS"
...
try {
  \$theclaw = Join-Path \$env:APPDATA 'npm\theclaw.cmd'
  & \$theclaw gateway $action *>&1 | Tee-Object -FilePath \$log -Append | Out-Null
...
show_gateway_status_compat() {
  if guest_run_theclaw "" "" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_run_theclaw "" "" gateway status --deep --require-rpc
    return
  fi
  guest_run_theclaw "" "" gateway status --deep
}

verify_turn() {
  guest_run_theclaw "" "" models set "$MODEL_ID"
  guest_run_theclaw "" "" config set agents.defaults.skipBootstrap true --strict-json
  guest_powershell "$(cat <<'EOF'
...
run_fresh_main_lane() {
  ...
  if phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "theclaw-main-fresh.tgz"; then
    ...
    phase_run "fresh.install-main-retry" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "theclaw-main-fresh.tgz" || return $?
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-baseline-package" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "theclaw-main-upgrade.tgz" || return $?
