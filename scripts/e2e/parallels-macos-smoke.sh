...
MODEL_ID=""
MODEL_ID_EXPLICIT=0
INSTALL_URL="https://theclaw.ai/install.sh"
HOST_PORT="18425"
...
SNAPSHOT_STATE=""
SNAPSHOT_NAME=""
GUEST_THECLAW_BIN="/opt/homebrew/bin/theclaw"
GUEST_THECLAW_ENTRY="/opt/homebrew/lib/node_modules/theclaw/theclaw.mjs"
...
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/theclaw-parallels-smoke.XXXXXX)"
...
TIMEOUT_UPDATE_DEV_S="${THECLAW_PARALLELS_MACOS_UPDATE_DEV_TIMEOUT_S:-1200}"
TIMEOUT_VERIFY_S=60
TIMEOUT_ONBOARD_S=180
TIMEOUT_GATEWAY_S=180
TIMEOUT_AGENT_S="${THECLAW_PARALLELS_MACOS_AGENT_TIMEOUT_S:-240}"
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://theclaw.ai/install.sh
...
resolve_guest_git_theclaw_entry() {
  local guest_home
  guest_home="$(resolve_guest_current_user_home)"
  printf '%s/theclaw/theclaw.mjs\n' "$guest_home"
}
...
spawn {*}$cmd
send -- "printf '__THECLAW_READY__\\n'\r"
expect "__THECLAW_READY__"
log_user 0
send -- "export PS1='' PROMPT='' PROMPT2='' RPROMPT=''\r"
send -- "stty -echo\r"

send -- "cat >/tmp/theclaw-prl.sh <<'__THECLAW_SCRIPT__'\r"
send -- $script
if {![string match "*\n" $script]} {
  send -- "\r"
}
send -- "__THECLAW_SCRIPT__\r"
send -- "/bin/bash /tmp/theclaw-prl.sh; rc=\$?; rm -f /tmp/theclaw-prl.sh; printf '__THECLAW_RC__:%s\\n' \"\$rc\"; exit \"\$rc\"\r"
log_user 1

set rc 1
set saw_rc 0
expect {
  -re {__THECLAW_RC__:(-?[0-9]+)} {
...
guest_current_user_sh() {
  ...
  script+="$1"
  if headless_guest_fallback; then
    script_path="/tmp/theclaw-prl-${BASHPID:-$$}-$RANDOM.sh"
    local guest_home
    guest_home="$(parallels_macos_resolve_desktop_home "$VM_NAME" "$GUEST_CURRENT_USER")"
...
markers = [
    ...
    if line.startswith("__THECLAW_RC__:")
]
if not markers:
    raise SystemExit(1)
...
printf '__THECLAW_RC__:%s\n' "\$status"
printf '%s\n' "\$status" > "$done_path"
exit "\$status"
EOF
)"
  write_runner_cmd="/bin/rm -f $(shell_quote "$runner_path")"$'\n'
  write_runner_cmd+="cat > $(shell_quote "$runner_path") <<'__THECLAW_RUNNER__'"$'\n'
  write_runner_cmd+="$runner_body"$'\n'
  write_runner_cmd+="__THECLAW_RUNNER__"$'\n'
  ...
  guest_home="$(resolve_guest_current_user_home)"
  guest_log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-guest-log-state.XXXXXX")"
  latest_npm_log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-guest-npm-log-state.XXXXXX")"
  npm_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-guest-npm-log-path.XXXXXX")"
...
matches = re.findall(r"^__THECLAW_RC__:(-?\d+)$", text, flags=re.MULTILINE)
if not matches:
    raise SystemExit(1)
print(matches[-1])
PY
...
install_latest_release() {
  ...
  guest_current_user_sh "$(cat <<EOF
export THECLAW_NO_ONBOARD=1
curl -fsSL $install_url_q -o /tmp/theclaw-install.sh
bash /tmp/theclaw-install.sh${version_arg_q}
$GUEST_THECLAW_BIN --version
EOF
)"
}
...
ensure_guest_pnpm_for_dev_update() {
  ...
  bootstrap_root="/tmp/theclaw-smoke-pnpm-bootstrap"
...
repair_legacy_dev_source_checkout_if_needed() {
  ...
  bootstrap_bin="/tmp/theclaw-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/theclaw"
  update_entry="$update_root/theclaw.mjs"
  ...
  guest_current_user_exec /usr/bin/git clone --depth 1 --branch main \
    https://github.com/theclaw/theclaw.git "$update_root"
...
run_dev_channel_update() {
  ...
  bootstrap_bin="/tmp/theclaw-smoke-pnpm-bootstrap/node_modules/.bin"
  update_root="$(resolve_guest_current_user_home)/theclaw"
  update_log="/tmp/theclaw-smoke-update-dev.log"
  update_done="/tmp/theclaw-smoke-update-dev.done"
  update_runner="/tmp/theclaw-smoke-update-dev.sh"
  ensure_guest_pnpm_for_dev_update
...
/usr/bin/env NODE_OPTIONS=--max-old-space-size=4096 \
  THECLAW_DISABLE_BUNDLED_PLUGINS=1 \
  $GUEST_NODE_BIN $GUEST_THECLAW_ENTRY update --channel dev --yes --json
EOF
  ...
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_THECLAW_ENTRY" --version
  printf 'update-dev: git-status\n'
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_THECLAW_ENTRY" update status --json
}

verify_dev_channel_update() {
  local status_json
  status_json="$(guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_THECLAW_ENTRY" update status --json)"
  printf '%s\n' "$status_json"
...
verify_version_contains() {
  ...
  local version
  version="$(
    guest_current_user_exec "$GUEST_THECLAW_BIN" --version 2>&1
  )"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
...
start_server() {
  ...
  ) >/tmp/theclaw-parallels-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
check_path "\$root/theclaw"
check_path "\$root/theclaw/extensions"
if [ -d "\$root/theclaw/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "\$extension_dir"
  done < <(/usr/bin/find "\$root/theclaw/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi
EOF
)"
...
start_manual_gateway_if_needed() {
  ...
  guest_gateway_log="/tmp/theclaw-parallels-macos-gateway.log"
  runner_log="/tmp/theclaw-parallels-gateway-start.log"
  done_path="/tmp/theclaw-parallels-gateway-start.done"
  runner_path="/tmp/theclaw-parallels-gateway-start.sh"
  printf 'manual gateway launch transport=%s user=%s\n' "$GUEST_CURRENT_USER_TRANSPORT" "$GUEST_CURRENT_USER"
  launch_cmd="$(cat <<EOF
set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'theclaw.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'theclaw-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'theclaw.mjs gateway' >/dev/null 2>&1 || true
/usr/bin/env \\
  ...
  THECLAW_HOME=$(shell_quote "$guest_home") \\
  THECLAW_STATE_DIR=$(shell_quote "$guest_home/.theclaw") \\
  THECLAW_CONFIG_PATH=$(shell_quote "$guest_home/.theclaw/theclaw.json") \\
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_THECLAW_ENTRY") gateway run --bind loopback --port 18789 --force \\
...
verify_gateway() {
  ...
  for attempt in 1 2 3 4 5 6 7 8; do
    if guest_current_user_exec "$GUEST_THECLAW_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return 0
    fi
    if (( attempt < 8 )); then
...
show_gateway_status_compat() {
  if guest_current_user_exec "$GUEST_THECLAW_BIN" gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_current_user_exec "$GUEST_THECLAW_BIN" gateway status --deep --require-rpc
    return
  fi
  guest_current_user_exec "$GUEST_THECLAW_BIN" gateway status --deep
}

verify_turn() {
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_THECLAW_ENTRY" models set "$MODEL_ID"
  guest_current_user_exec "$GUEST_NODE_BIN" "$GUEST_THECLAW_ENTRY" config set agents.defaults.skipBootstrap true --strict-json
  guest_current_user_sh "$(cat <<EOF
...
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") \
  $(shell_quote "$GUEST_NODE_BIN") $(shell_quote "$GUEST_THECLAW_ENTRY") agent \
  --agent main \
...
resolve_dashboard_url() {
  local dashboard_url
  dashboard_url="$(
    guest_current_user_cli "$GUEST_THECLAW_BIN" dashboard --no-open \
      | awk '/^Dashboard URL: / { sub(/^Dashboard URL: /, ""); print; exit }'
  )"
  dashboard_url="${dashboard_url//$'\r'/}"
  dashboard_url="${dashboard_url//$'\n'/}"
  [[ -n "$dashboard_url" ]] || {
    echo "failed to resolve dashboard URL from theclaw dashboard --no-open" >&2
    return 1
  }
  printf '%s\n' "$dashboard_url"
}
...
verify_dashboard_load() {
  ...
  # `theclaw dashboard --no-open` can hang under the Tahoe Parallels transport
...
while [ \$SECONDS -lt \$deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 "\$dashboard_http_url" >/tmp/theclaw-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>TheClaw Control</title>' /tmp/theclaw-dashboard-smoke.html >/dev/null; then
      if grep -F '<theclaw-app></theclaw-app>' /tmp/theclaw-dashboard-smoke.html >/dev/null; then
        dashboard_ready=1
        break
      fi
    fi
...
grep -F '<title>TheClaw Control</title>' /tmp/theclaw-dashboard-smoke.html >/dev/null
grep -F '<theclaw-app></theclaw-app>' /tmp/theclaw-dashboard-smoke.html >/dev/null
...
PY
  ...
  script="$(cat <<EOF
cat >/tmp/theclaw-discord-token <<'__THECLAW_TOKEN__'
$DISCORD_TOKEN_VALUE
__THECLAW_TOKEN__
cat >/tmp/theclaw-discord-guilds.json <<'__THECLAW_GUILDS__'
$guilds_json
__THECLAW_GUILDS__
token="\$(tr -d '\n' </tmp/theclaw-discord-token)"
guilds_json="\$(cat /tmp/theclaw-discord-guilds.json)"
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY config set channels.discord.token "\$token"
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY config set channels.discord.enabled true
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY config set channels.discord.groupPolicy allowlist
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY config set channels.discord.guilds "\$guilds_json" --strict-json
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY gateway restart
for _ in 1 2 3 4 5 6 7 8; do
  if $GUEST_NODE_BIN $GUEST_THECLAW_ENTRY gateway status --deep --require-rpc >/dev/null 2>&1; then
    break
  fi
  sleep 2
done
$GUEST_NODE_BIN $GUEST_THECLAW_ENTRY channels status --probe --json
rm -f /tmp/theclaw-discord-token /tmp/theclaw-discord-guilds.json
EOF
)"
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "theclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  if upgrade_uses_host_tgz; then
    phase_run "upgrade.install-main" "$(install_main_timeout)" install_main_tgz "$host_ip" "theclaw-main-upgrade.tgz"
