...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-cron-mcp-cleanup-e2e" THECLAW_IMAGE)"
PORT="18789"
TOKEN="cron-mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="theclaw-cron-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t theclaw-cron-mcp-client-log.XXXXXX)"
...
THECLAW_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 cron-mcp-cleanup empty)"
...
docker run --rm \
  ...
  -e "GW_TOKEN=$TOKEN" \
  -e "THECLAW_ALLOW_INSECURE_PRIVATE_WS=1" \
  ...
  bash -lc "set -euo pipefail
    ...
    export SUCCESS_MARKER=THECLAW_CRON_MCP_CLEANUP_OK
    export MOCK_REQUEST_LOG=/tmp/theclaw-cron-mock-openai-requests.jsonl
    export THECLAW_DOCKER_OPENAI_BASE_URL=\"http://127.0.0.1:\$MOCK_PORT/v1\"
