...
RUN_PLATFORMS="all"
JSON_OUTPUT=0
RUN_DIR="$(mktemp -d /tmp/theclaw-parallels-npm-update.XXXXXX)"
MAIN_TGZ_DIR="$(mktemp -d)"
MAIN_TGZ_PATH=""
BUILD_LOCK_DIR="${TMPDIR:-/tmp}/theclaw-parallels-build.lock"
...
PROGRESS_STALE_S=60
TIMEOUT_UPDATE_S="${THECLAW_PARALLELS_NPM_UPDATE_TIMEOUT_S:-1200}"
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: theclaw@latest
  --update-target <target>    Target passed to guest 'theclaw update --tag'.
                             Default: host-served tgz packed from current checkout.
                             Examples: latest, beta, 2026.4.10, http://host/theclaw.tgz
...
case "$PROVIDER" in
  openai)
    ...
    [[ "$MODEL_ID_EXPLICIT" -eq 1 ]] || MODEL_ID="${THECLAW_PARALLELS_OPENAI_MODEL:-openai/gpt-5.5}"
    [[ -n "$API_KEY_ENV" ]] || API_KEY_ENV="OPENAI_API_KEY"
    ;;
  anthropic)
    AUTH_CHOICE="apiKey"
    AUTH_KEY_FLAG="anthropic-api-key"
    [[ "$MODEL_ID_EXPLICIT" -eq 1 ]] || MODEL_ID="${THECLAW_PARALLELS_ANTHROPIC_MODEL:-anthropic/claude-sonnet-4-6}"
    [[ -n "$API_KEY_ENV" ]] || API_KEY_ENV="ANTHROPIC_API_KEY"
    ;;
  minimax)
    AUTH_CHOICE="minimax-global-api"
    AUTH_KEY_FLAG="minimax-api-key"
    [[ "$MODEL_ID_EXPLICIT" -eq 1 ]] || MODEL_ID="${THECLAW_PARALLELS_MINIMAX_MODEL:-minimax/MiniMax-M2.7}"
...
resolve_latest_version() {
  npm view theclaw version --userconfig "$(mktemp)"
}
...
pack_main_tgz() {
  ...
  [[ $rc -eq 0 ]] || return "$rc"
  MAIN_TGZ_PATH="$MAIN_TGZ_DIR/theclaw-main-$CURRENT_HEAD_SHORT.tgz"
...
preflight_registry_update_target() {
  ...
  if [[ "$baseline_version" == "$target_version" ]]; then
    die "--update-target $UPDATE_TARGET resolves to theclaw@$target_version, same as baseline $PACKAGE_SPEC; publish or choose a newer --update-target before running VM update coverage"
  fi
}

write_windows_update_script() {
  WINDOWS_UPDATE_SCRIPT_PATH="$MAIN_TGZ_DIR/theclaw-main-update.ps1"
...
function Invoke-Logged {
  ...
  try {
    ...
    $PSNativeCommandUseErrorActionPreference = $false
    # Merge native stderr into stdout before logging so npm/theclaw warnings do not
...
function Test-GatewayLogReady {
  $logDir = Join-Path $env:LOCALAPPDATA 'Temp\theclaw'
  if (-not (Test-Path $logDir)) {
    return $false
  }
  $logFile = Get-ChildItem -Path $logDir -Filter 'theclaw-*.log' -File -ErrorAction SilentlyContinue |
...
function Wait-GatewayRpcReady {
  param(
    [Parameter(Mandatory = $true)][string]$TheClawPath,
    [int]$Attempts = 20,
    [int]$SleepSeconds = 3
  )
  ...
  for ($attempt = 1; $attempt -le $Attempts; $attempt++) {
    ...
    try {
      $probeOutput = Invoke-CaptureLogged 'theclaw gateway probe' { & $TheClawPath gateway probe --url ws://127.0.0.1:18789 --timeout 5000 --json }
      $probe = $probeOutput | ConvertFrom-Json
      if (-not $probe.ok) {
        throw 'gateway probe returned without RPC readiness'
      }
      Invoke-CaptureLogged 'theclaw gateway status' { & $TheClawPath gateway status --deep --require-rpc } | Out-Null
      return $true
    } catch {
...
function Stop-GatewayScheduledTaskIfPresent {
  ...
  try {
    $PSNativeCommandUseErrorActionPreference = $false
    schtasks /End /TN 'TheClaw Gateway' 2>$null | Out-Null
  } catch {
  } finally {
    $PSNativeCommandUseErrorActionPreference = $previousNativeErrorPreference
  }
...
function Stop-TheClawUpdateProcesses {
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.theclaw\theclaw.json'
...
function Invoke-TheClawUpdateWithTimeout {
  param(
    [Parameter(Mandatory = $true)][string]$TheClawPath,
    [Parameter(Mandatory = $true)][string]$UpdateTarget,
  ...
  $updateJob = Start-Job -ScriptBlock {
    ...
    $previousDisableBundledPlugins = $env:THECLAW_DISABLE_BUNDLED_PLUGINS
    $env:THECLAW_DISABLE_BUNDLED_PLUGINS = '1'
    try {
      $output = & $Path update --tag $Target --yes --json *>&1
    } finally {
      if ($null -eq $previousDisableBundledPlugins) {
        Remove-Item Env:THECLAW_DISABLE_BUNDLED_PLUGINS -ErrorAction SilentlyContinue
      } else {
        $env:THECLAW_DISABLE_BUNDLED_PLUGINS = $previousDisableBundledPlugins
      }
    }
    [pscustomobject]@{
      ExitCode = $LASTEXITCODE
  ...
  } -ArgumentList $TheClawPath, $UpdateTarget
  ...
  if ($null -ne $completed) {
    ...
    if ($result.ExitCode -ne 0) {
      throw "theclaw update failed with exit code $($result.ExitCode)"
    }
    return
  }
  ...
  Remove-Job $updateJob -Force -ErrorAction SilentlyContinue
  Write-ProgressLog 'update.theclaw-update.timeout'
  'theclaw update timed out after package install window; killing stale update/completion processes and verifying installed version' | Tee-Object -FilePath $LogPath -Append | Out-Null
  Stop-TheClawUpdateProcesses
}

function Invoke-TheClawAgentWithTimeout {
  param(
    [Parameter(Mandatory = $true)][string]$TheClawPath,
    [Parameter(Mandatory = $true)][string]$SessionId,
  ...
  $stdout = Join-Path $env:TEMP ("theclaw-parallels-agent-{0}.out.log" -f ([guid]::NewGuid().ToString('N')))
  $stderr = Join-Path $env:TEMP ("theclaw-parallels-agent-{0}.err.log" -f ([guid]::NewGuid().ToString('N')))
  $agentJob = Start-Job -ScriptBlock {
    ...
    exit $LASTEXITCODE
  } -ArgumentList $TheClawPath, $SessionId, $message, $stdout, $stderr
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    if ($agentJob.State -in @('Completed', 'Failed', 'Stopped')) {
      ...
      if ($jobState -ne 'Completed') {
        throw "theclaw agent failed with job state $jobState"
      }
      throw 'theclaw agent finished without OK response'
    }
  }
  ...
  throw "theclaw agent timed out after ${TimeoutSeconds}s"
}
...
function Start-GatewayRunFallback {
  ...
  Stop-TheClawGatewayProcesses
  $entry = Join-Path $env:APPDATA 'npm\node_modules\theclaw\dist\index.js'
  if (-not (Test-Path $entry)) {
    throw "theclaw dist entry missing: $entry"
  }
  $node = (Get-Command node.exe -ErrorAction Stop).Source
  $stdout = Join-Path $env:TEMP 'theclaw-parallels-npm-update-gateway.log'
  $stderr = Join-Path $env:TEMP 'theclaw-parallels-npm-update-gateway.err.log'
  ...
  if (-not (Wait-GatewayRpcReady -TheClawPath $TheClawPath -Attempts 20 -SleepSeconds 3)) {
    if (Test-Path $stdout) {
...
function Complete-WorkspaceSetup {
  $workspace = $env:THECLAW_WORKSPACE_DIR
  if (-not $workspace) {
    $workspace = Join-Path $env:USERPROFILE '.theclaw\workspace'
  }
  $stateDir = Join-Path $workspace '.theclaw'
...
function Restart-GatewayWithRecovery {
  ...
  if ($null -ne $restartCompleted) {
    ...
    if ($restartResult.ExitCode -ne 0) {
      ...
      "theclaw gateway restart failed with exit code $($restartResult.ExitCode)" | Tee-Object -FilePath $LogPath -Append | Out-Null
    }
  } else {
    $restartFailed = $true
    Stop-Job $restartJob -ErrorAction SilentlyContinue
    Write-ProgressLog 'update.restart-gateway.timeout'
    'theclaw gateway restart timed out after 20s; continuing to RPC readiness checks' | Tee-Object -FilePath $LogPath -Append | Out-Null
  }
  Remove-Job $restartJob -Force -ErrorAction SilentlyContinue

  Write-ProgressLog 'update.gateway-status'
  if (Wait-GatewayRpcReady -TheClawPath $TheClawPath) {
    return
  }
  Write-ProgressLog 'update.gateway-start-recover'
  Stop-TheClawGatewayProcesses
  Invoke-Logged 'theclaw gateway start' { & $TheClawPath gateway start }
  Write-ProgressLog 'update.gateway-status-recover'
  if (-not (Wait-GatewayRpcReady -TheClawPath $TheClawPath)) {
    Start-GatewayRunFallback -TheClawPath $TheClawPath
  }
}

try {
  $env:PATH = "$env:LOCALAPPDATA\TheClaw\deps\portable-git\cmd;$env:LOCALAPPDATA\TheClaw\deps\portable-git\mingw64\bin;$env:LOCALAPPDATA\TheClaw\deps\portable-git\usr\bin;$env:PATH"
  ...
  $theclaw = Join-Path $env:APPDATA 'npm\theclaw.cmd'
  Remove-FuturePluginEntries
  Stop-TheClawGatewayProcesses
  Write-ProgressLog 'update.theclaw-update'
  Invoke-TheClawUpdateWithTimeout -TheClawPath $theclaw -UpdateTarget $UpdateTarget
  Write-ProgressLog 'update.verify-version'
  $version = Invoke-CaptureLogged 'theclaw --version' { & $theclaw --version }
  ...
  Invoke-Logged 'theclaw update status' { & $theclaw update status --json }
  Write-ProgressLog 'update.set-model'
  Invoke-Logged 'theclaw models set' { & $theclaw models set $ModelId }
  ...
  Restart-GatewayWithRecovery -TheClawPath $theclaw
  Stop-TheClawGatewayProcesses
  Complete-WorkspaceSetup
  Write-ProgressLog 'update.agent-turn'
  $exitCode = Invoke-TheClawAgentWithTimeout -TheClawPath $theclaw -SessionId $SessionId
...
start_server() {
  ...
  ) >/tmp/theclaw-parallels-npm-update-http.log 2>&1 &
  SERVER_PID=$!
  sleep 1
...
verify_macos_update_after_transport_loss() {
  ...
  local script_path="/tmp/theclaw-npm-update-macos-recover.sh"
...
export THECLAW_PLUGIN_STAGE_DIR="\$HOME/.theclaw/plugin-runtime-deps-parallels"
busy="\$(/bin/ps -axo command | /usr/bin/egrep 'theclaw update|npm install|pnpm install|pnpm run build' | /usr/bin/egrep -v 'egrep|theclaw-npm-update-macos-recover' || true)"
gateway_listener_ready() {
  /usr/sbin/lsof -tiTCP:18789 -sTCP:LISTEN >/dev/null 2>&1
}
gateway_log_ready() {
  latest="\$(/bin/ls -t /tmp/theclaw/theclaw-*.log 2>/dev/null | /usr/bin/head -n 1 || true)"
  [ -n "\$latest" ] || return 1
...
if [ -n "\$busy" ]; then
  printf 'update still has active npm/pnpm/theclaw processes\n%s\n' "\$busy" >&2
  exit 1
fi
version="\$(/opt/homebrew/bin/theclaw --version)"
printf '%s\n' "\$version"
...
gateway_smoke_ready || /opt/homebrew/bin/theclaw gateway restart || true
gateway_ready=0
...
if [ "\$gateway_ready" != "1" ]; then
  /opt/homebrew/bin/theclaw gateway start || true
  for _ in 1 2 3 4 5 6; do
    if gateway_smoke_ready; then
      gateway_ready=1
      break
...
workspace="\${THECLAW_WORKSPACE_DIR:-\$HOME/.theclaw/workspace}"
mkdir -p "\$workspace/.theclaw"
...
- Purpose: Parallels npm update smoke test assistant.
IDENTITY_EOF
cat > "\$workspace/.theclaw/workspace-state.json" <<'STATE_EOF'
{
  "version": 1,
  "setupCompletedAt": "2026-01-01T00:00:00.000Z"
}
...
rm -f "\$workspace/BOOTSTRAP.md"
  /opt/homebrew/bin/theclaw models set "$MODEL_ID"
  /opt/homebrew/bin/theclaw config set agents.defaults.skipBootstrap true --strict-json
/opt/homebrew/bin/theclaw agent --agent main --session-id "parallels-npm-update-macos-transport-recovery-$expected_needle" --message "Reply with exact ASCII text OK only." --json
EOF
...
\$ErrorActionPreference = 'Stop'
\$theclaw = Join-Path \$env:APPDATA 'npm\\theclaw.cmd'
if (-not (Test-Path \$theclaw)) {
  throw "theclaw shim missing: \$theclaw"
}
\$busy = Get-CimInstance Win32_Process |
  Where-Object {
    \$_.CommandLine -and
    (\$_.CommandLine -match 'theclaw update|npm install|pnpm install|pnpm run build')
  }
if (\$busy) {
  throw 'update still has active npm/pnpm/theclaw processes'
}
\$version = & \$theclaw --version
...
function Stop-GatewayListeners {
  ...
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object {
      \$_.CommandLine -and (
        \$_.CommandLine -match 'theclaw.*gateway --port 18789' -or
        \$_.CommandLine -match 'theclaw.*gateway run' -or
...
\$gatewayReady = \$false
for (\$i = 0; \$i -lt 6; \$i++) {
  if (Test-GatewayWritable \$theclaw) {
    \$gatewayReady = \$true
    break
  }
  Start-Sleep -Seconds 2
...
if (-not \$gatewayReady) {
  ...
  \$entry = Join-Path \$env:APPDATA 'npm\\node_modules\\theclaw\\dist\\index.js'
  \$node = (Get-Command node.exe -ErrorAction Stop).Source
  \$stdout = Join-Path \$env:TEMP 'theclaw-parallels-npm-update-recover-gateway.log'
  \$stderr = Join-Path \$env:TEMP 'theclaw-parallels-npm-update-recover-gateway.err.log'
  Start-Process -FilePath \$node -ArgumentList @(\$entry, 'gateway', 'run', '--bind', 'loopback', '--port', '18789', '--force') -WindowStyle Hidden -RedirectStandardOutput \$stdout -RedirectStandardError \$stderr | Out-Null
  for (\$i = 0; \$i -lt 20; \$i++) {
    if (Test-GatewayWritable \$theclaw) {
      \$gatewayReady = \$true
...
Set-Item -Path ('Env:' + '$API_KEY_ENV') -Value \$providerValue
  & \$theclaw models set '$MODEL_ID'
  & \$theclaw config set agents.defaults.skipBootstrap true --strict-json
\$workspace = \$env:THECLAW_WORKSPACE_DIR
if (-not \$workspace) {
  \$workspace = Join-Path \$env:USERPROFILE '.theclaw\\workspace'
}
...
Remove-Item (Join-Path \$workspace 'BOOTSTRAP.md') -Force -ErrorAction SilentlyContinue
Stop-GatewayListeners
\$agentStdout = Join-Path \$env:TEMP ("theclaw-parallels-agent-{0}.out.log" -f ([guid]::NewGuid().ToString('N')))
\$agentStderr = Join-Path \$env:TEMP ("theclaw-parallels-agent-{0}.err.log" -f ([guid]::NewGuid().ToString('N')))
\$agentJob = Start-Job -ScriptBlock {
  ...
  exit \$LASTEXITCODE
} -ArgumentList \$theclaw, \$agentStdout, \$agentStderr
...
while ((Get-Date) -lt \$agentDeadline) {
  ...
  if (\$agentJob.State -in @('Completed', 'Failed', 'Stopped')) {
    ...
    if (\$agentJobState -ne 'Completed') {
      throw "theclaw agent failed with job state \$agentJobState"
    }
    throw 'theclaw agent finished without OK response'
    break
  }
...
if (\$null -ne \$agentJob) {
  ...
  throw 'theclaw agent timed out after 600s'
}
EOF
  )"
  local rc=$?
...
text = pathlib.Path(sys.argv[1]).read_text(encoding="utf-8", errors="replace")
matches = re.findall(r"TheClaw [^\r\n]+", text)
matches = [match for match in matches if re.search(r"TheClaw \d", match)]
...
run_windows_script_via_log() {
  ...
  runner_name="theclaw-update-$RANDOM-$RANDOM.ps1"
  log_name="theclaw-update-$RANDOM-$RANDOM.log"
  done_name="theclaw-update-$RANDOM-$RANDOM.done"
  log_state_path="$(mktemp "${TMPDIR:-/tmp}/theclaw-update-log-state.XXXXXX")"
  : >"$log_state_path"
  provider_key_b64="$(
...
run_macos_update() {
  ...
  cat <<EOF | prlctl exec "$MACOS_VM" /usr/bin/tee /tmp/theclaw-main-update.sh >/dev/null
set -euo pipefail
export PATH=/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin
...
const configPath = path.join(os.homedir(), ".theclaw", "theclaw.json");
...
stop_theclaw_gateway_processes() {
  THECLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/theclaw gateway stop >/dev/null 2>&1 || true
  /usr/bin/pkill -9 -f theclaw-gateway || true
  /usr/bin/pkill -9 -f 'theclaw gateway run' || true
  /usr/bin/pkill -9 -f 'theclaw.mjs gateway' || true
...
THECLAW_DISABLE_BUNDLED_PLUGINS=1 /opt/homebrew/bin/theclaw update --tag "$update_target" --yes --json
...
/opt/homebrew/bin/theclaw update status --json
  /opt/homebrew/bin/theclaw models set "$MODEL_ID"
  /opt/homebrew/bin/theclaw config set agents.defaults.skipBootstrap true --strict-json
...
/opt/homebrew/bin/theclaw gateway restart || true
gateway_ready=0
...
if [ "\$gateway_ready" != "1" ]; then
  stop_theclaw_gateway_processes
  /opt/homebrew/bin/theclaw gateway run --bind loopback --port 18789 --force >/tmp/theclaw-parallels-npm-update-macos-gateway.log 2>&1 </dev/null &
  for _ in 1 2 3 4 5 6 7 8; do
    if gateway_smoke_ready; then
      gateway_ready=1
      break
...
if [ "\$gateway_ready" != "1" ]; then
  tail -n 120 /tmp/theclaw-parallels-npm-update-macos-gateway.log 2>/dev/null || true
fi
if [ "\$gateway_ready" != "1" ]; then
  /opt/homebrew/bin/theclaw gateway status --deep --require-rpc
fi
workspace="\${THECLAW_WORKSPACE_DIR:-\$HOME/.theclaw/workspace}"
mkdir -p "\$workspace/.theclaw"
cat > "\$workspace/IDENTITY.md" <<'IDENTITY_EOF'
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
/opt/homebrew/bin/theclaw agent --agent main --session-id parallels-npm-update-macos-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  macos_desktop_user_exec /bin/bash /tmp/theclaw-main-update.sh
}
...
run_linux_update() {
  local update_target="$1"
  local expected_needle="$2"
  cat <<EOF | prlctl exec "$LINUX_VM" /usr/bin/tee /tmp/theclaw-main-update.sh >/dev/null
set -euo pipefail
export HOME=/root
cd "\$HOME"
scrub_future_plugin_entries() {
...
stop_theclaw_gateway_processes() {
  THECLAW_DISABLE_BUNDLED_PLUGINS=1 theclaw gateway stop >/dev/null 2>&1 || true
  pkill -9 -f theclaw-gateway || true
  pkill -9 -f 'theclaw gateway run' || true
  pkill -9 -f 'theclaw.mjs gateway' || true
...
scrub_future_plugin_entries
stop_theclaw_gateway_processes
THECLAW_DISABLE_BUNDLED_PLUGINS=1 theclaw update --tag "$update_target" --yes --json
...
STATE_EOF
rm -f "\$workspace/BOOTSTRAP.md"
theclaw agent --local --agent main --session-id parallels-npm-update-linux-$expected_needle --message "Reply with exact ASCII text OK only." --json
EOF
  prlctl exec "$LINUX_VM" /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" /bin/bash /tmp/theclaw-main-update.sh
}
...
if [[ -z "$PACKAGE_SPEC" ]]; then
  PACKAGE_SPEC="theclaw@$LATEST_VERSION"
fi
preflight_registry_update_target
resolve_current_head
...
if platform_enabled linux; then
  THECLAW_PARALLELS_LINUX_DISABLE_BONJOUR=1 bash "$ROOT_DIR/scripts/e2e/parallels-linux-smoke.sh" \
    --mode fresh \
    --provider "$PROVIDER" \
    --model "$MODEL_ID" \
    --api-key-env "$API_KEY_ENV" \
...
say "Run same-guest theclaw update to $UPDATE_TARGET_EFFECTIVE"
update_monitor_args=()
if platform_enabled macos; then
