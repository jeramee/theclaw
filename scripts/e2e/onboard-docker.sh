...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-onboard-e2e" THECLAW_ONBOARD_E2E_IMAGE)"
THECLAW_TEST_STATE_FUNCTION_B64="$(
  node "$ROOT_DIR/scripts/lib/theclaw-test-state.mjs" shell-function \
    | base64 \
    | tr -d '\n'
)"
...
docker run --rm -t \
  ...
  eval "$(printf "%s" "${THECLAW_TEST_STATE_FUNCTION_B64:?missing THECLAW_TEST_STATE_FUNCTION_B64}" | base64 -d)"
	  ...
	  if [ -f dist/index.mjs ]; then
	    THECLAW_ENTRY="dist/index.mjs"
	  elif [ -f dist/index.js ]; then
	    THECLAW_ENTRY="dist/index.js"
	  else
	    echo "Missing dist/index.(m)js (build output):"
	    ls -la dist || true
	    exit 1
	  fi
	  export THECLAW_ENTRY

  # Provide a minimal trash shim to avoid noisy "missing trash" logs in containers.
  export PATH="/tmp/theclaw-bin:$PATH"
  mkdir -p /tmp/theclaw-bin
  cat > /tmp/theclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$THECLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_theclaw_env "$state_ref"

    input_fifo="$(mktemp -u "/tmp/theclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/theclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$state_ref" "node \"$THECLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  set_isolated_theclaw_env() {
    local state_ref="$1"
    theclaw_test_state_create "$state_ref" empty
  }
  ...
  run_case_logged() {
    local label="$1"
    shift
    local log_path="/tmp/theclaw-onboard-${label}.log"
  ...
  run_case_local_basic() {
    set_isolated_theclaw_env local-basic
    run_case_logged local-basic node "$THECLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
    ...
    workspace_dir="$THECLAW_STATE_DIR/workspace"
    config_path="$THECLAW_CONFIG_PATH"
    sessions_dir="$THECLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    set_isolated_theclaw_env remote-non-interactive
	    # Smoke test non-interactive remote config write.
	    run_case_logged remote-non-interactive node "$THECLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    set_isolated_theclaw_env reset-config
    # Seed a remote config to exercise reset path.
	    cat > "$THECLAW_CONFIG_PATH" <<'"'"'JSON'"'"'
{
  "meta": {},
...
JSON
	    ...
	    run_case_logged reset-config node "$THECLAW_ENTRY" onboard \
	      --non-interactive \
	      --accept-risk \
      --flow quickstart \
      --mode local \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels channels "node \"$THECLAW_ENTRY\" configure --section channels" send_channels_flow
...
NODE
  ...
  run_case_skills() {
    local home_dir
    set_isolated_theclaw_env skills
    home_dir="$HOME"
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$THECLAW_ENTRY\" configure --section skills" send_skills_flow
