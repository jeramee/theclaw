...
// against the TheClaw npm tarball installed in the functional image.
...
export async function connectMcpClient(params: {
  ...
  const tokenDir = "/tmp/theclaw-mcp-client";
  ...
  const transport = new StdioClientTransport({
    ...
    env: {
      ...
      THECLAW_STATE_DIR: "/tmp/theclaw-mcp-client",
    },
    stderr: "pipe",
  });
  transport.stderr?.on("data", (chunk) => {
    process.stderr.write(`[theclaw mcp] ${String(chunk)}`);
  });
