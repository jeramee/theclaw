...
async function main() {
  ...
  const stateDir =
    ...
    (await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-crestodian-first-run-")));
  ...
  assert(
    shouldStartCrestodianForBareRoot(["node", "theclaw"]),
    "bare theclaw invocation did not route to Crestodian",
  );
  process.exitCode = undefined;
  await runCli(["node", "theclaw", "onboard", "--modern", "--non-interactive", "--json"]);
  assert(
  ...
  const config = JSON.parse(await fs.readFile(configPath, "utf8")) as TheClawConfig;
  assert(
