#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized TheClaw Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "theclaw-openwebui-e2e" THECLAW_OPENWEBUI_E2E_IMAGE)"
...
MODEL="${THECLAW_OPENWEBUI_MODEL:-openai/gpt-4.1-mini}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${THECLAW_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="${THECLAW_OPENWEBUI_GATEWAY_PORT:-18789}"
WEBUI_PORT="${THECLAW_OPENWEBUI_PORT:-8080}"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${THECLAW_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${THECLAW_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="theclaw-openwebui-e2e-$$"
GW_NAME="theclaw-openwebui-gateway-$$"
OW_NAME="theclaw-openwebui-$$"
DOCKER_COMMAND_TIMEOUT="${THECLAW_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-600s}"
DOCKER_PULL_TIMEOUT="${THECLAW_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
docker_cmd docker run -d \
  ...
  -e "THECLAW_OPENWEBUI_MODEL=$MODEL" \
  -e "THECLAW_SKIP_CHANNELS=1" \
  -e "THECLAW_SKIP_GMAIL_WATCHER=1" \
  -e "THECLAW_SKIP_CRON=1" \
  -e "THECLAW_SKIP_CANVAS_HOST=1" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/theclaw-openwebui-config.XXXXXX.json)"
    THECLAW_CONFIG_BATCH_PATH="$batch_file" node - <<'"'"'NODE'"'"' "$openai_api_key"
const fs = require("node:fs");

const openaiApiKey = process.argv[2];
const batchPath = process.env.THECLAW_CONFIG_BATCH_PATH;
const entries = [
  ...
  { path: "gateway.auth.token", value: process.env.THECLAW_GATEWAY_TOKEN },
  { path: "gateway.http.endpoints.chatCompletions.enabled", value: true },
  { path: "agents.defaults.model.primary", value: process.env.THECLAW_OPENWEBUI_MODEL },
];
...
EOF
    cat > "$workspace/.theclaw/workspace-state.json" <<'"'"'EOF'"'"'
{
  "version": 1,
...
docker_cmd docker run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="TheClaw E2E" \
  -e WEBUI_SECRET_KEY="theclaw-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="TheClaw E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="theclaw/default" \
...
echo "Running Open WebUI -> TheClaw smoke..."
