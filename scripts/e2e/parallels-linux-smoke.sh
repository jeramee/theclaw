...
MAIN_TGZ_PATH=""
SERVER_PID=""
RUN_DIR="$(mktemp -d /tmp/theclaw-parallels-linux.XXXXXX)"
...
TIMEOUT_AGENT_S="${THECLAW_PARALLELS_LINUX_AGENT_TIMEOUT_S:-300}"
...
case "${THECLAW_PARALLELS_LINUX_DISABLE_BONJOUR:-}" in
...
start_server() {
  ...
  while :; do
    ...
    ) >/tmp/theclaw-parallels-linux-http.log 2>&1 &
    SERVER_PID=$!
    sleep 1
...
install_latest_release() {
  guest_exec curl -fsSL "$INSTALL_URL" -o /tmp/theclaw-install.sh
  if [[ -n "$INSTALL_VERSION" ]]; then
    guest_exec /usr/bin/env THECLAW_NO_ONBOARD=1 bash /tmp/theclaw-install.sh --version "$INSTALL_VERSION" --no-onboard
  else
    guest_exec /usr/bin/env THECLAW_NO_ONBOARD=1 bash /tmp/theclaw-install.sh --no-onboard
  fi
  guest_exec theclaw --version
}
...
verify_version_contains() {
  local needle="$1"
  local version
  version="$(guest_exec theclaw --version)"
  printf '%s\n' "$version"
  case "$version" in
    *"$needle"*) ;;
    *)
...
run_ref_onboard() {
  guest_exec /usr/bin/env "$API_KEY_ENV=$API_KEY_VALUE" theclaw onboard \
    --non-interactive \
    --mode local \
...
set -euo pipefail
plugin_dir=/root/.theclaw/test-bad-plugin
mkdir -p "$plugin_dir"
cat >"$plugin_dir/package.json" <<'JSON'
{
  "name": "@theclaw/test-bad-plugin",
  "version": "1.0.0",
  "theclaw": {
...
JSON
cat >"$plugin_dir/theclaw.plugin.json" <<'JSON'
{
  "id": "test-bad-plugin",
  "configSchema": {
    "type": "object",
...
config_path = Path("/root/.theclaw/theclaw.json")
config = {}
...
verify_bad_plugin_diagnostic() {
  guest_bash_script <<'EOF'
grep -F "failed to load setup entry" /tmp/theclaw-parallels-linux-gateway.log
EOF
}
...
start_gateway_background() {
  ...
  if [[ "$DISABLE_BONJOUR_FOR_GATEWAY" -eq 1 ]]; then
    bonjour_env=" THECLAW_DISABLE_BONJOUR=1"
  fi
  cmd="$(cat <<EOF
pkill -f "theclaw gateway run" >/dev/null 2>&1 || true
rm -f /tmp/theclaw-parallels-linux-gateway.log
setsid sh -lc 'exec env THECLAW_HOME=/root THECLAW_STATE_DIR=/root/.theclaw THECLAW_CONFIG_PATH=/root/.theclaw/theclaw.json${bonjour_env} ${API_KEY_ENV}=${api_key_value_q} theclaw gateway run --bind loopback --port 18789 --force >/tmp/theclaw-parallels-linux-gateway.log 2>&1' >/dev/null 2>&1 < /dev/null &
EOF
)"
...
show_gateway_status_compat() {
  if guest_exec theclaw gateway status --help | grep -Fq -- "--require-rpc"; then
    guest_exec theclaw gateway status --deep --require-rpc
    return
  fi
  guest_exec theclaw gateway status --deep
}

verify_gateway_status() {
  local attempt
  for attempt in 1 2 3 4 5 6 7 8; do
    if guest_exec theclaw gateway status --deep --require-rpc --timeout 15000; then
      return 0
    fi
    if (( attempt < 8 )); then
...
verify_local_turn() {
  guest_exec theclaw models set "$MODEL_ID"
  guest_exec theclaw config set agents.defaults.skipBootstrap true --strict-json
  prepare_agent_workspace
  guest_exec /bin/sh -lc "$(cat <<EOF
exec /usr/bin/env $(shell_quote "$API_KEY_ENV=$API_KEY_VALUE") theclaw agent \
  --local \
  --agent main \
...
run_fresh_main_lane() {
  ...
  phase_run "fresh.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "theclaw-main-fresh.tgz"
...
run_upgrade_lane() {
  ...
  phase_run "upgrade.install-main" "$TIMEOUT_INSTALL_S" install_main_tgz "$host_ip" "theclaw-main-upgrade.tgz"
