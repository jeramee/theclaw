...
type DriverMode = "token" | "webhook" | "theclaw";
...
function resolveStateDir(): string {
  const override = process.env.THECLAW_STATE_DIR?.trim();
  if (override) {
    return override.startsWith("~")
      ? path.resolve(process.env.HOME || "", override.slice(1))
      : path.resolve(override);
  }
  const home = process.env.THECLAW_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".theclaw");
}
...
function usage(): string {
  return (
    ...
    "--channel <discord-channel-id> [--token <driver-token> | --driver webhook --bot-token <bot-token> | --driver theclaw] [options]\n\n" +
    "Manual live smoke only (not CI). Sends a plain-language instruction in Discord and verifies:\n" +
    "1) TheClaw spawned an ACP thread binding\n" +
    "2) agent replied in that bound thread with the expected ACK token\n\n" +
    "Options:\n" +
    "  --channel <id>               Parent Discord channel id (required)\n" +
    "  --driver <token|webhook|theclaw> Driver transport mode (default: token)\n" +
    ...
    "  --theclaw-bin <path>        TheClaw CLI binary for driver=theclaw (default: theclaw)\n" +
    "  --json                       Emit JSON output\n" +
    "\n" +
    "Environment fallbacks:\n" +
    "  THECLAW_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  THECLAW_DISCORD_SMOKE_DRIVER\n" +
    "  THECLAW_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  THECLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  THECLAW_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  THECLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  THECLAW_DISCORD_SMOKE_AGENT\n" +
    "  THECLAW_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  THECLAW_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  THECLAW_DISCORD_SMOKE_POLL_MS\n" +
    "  THECLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH\n" +
    "  THECLAW_DISCORD_SMOKE_THECLAW_BIN"
  );
}

function parseArgs(): Args {
  const channelId = resolveArg("--channel") || process.env.THECLAW_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver") || process.env.THECLAW_DISCORD_SMOKE_DRIVER || "token";
  const normalizedDriverMode = driverModeRaw.trim().toLowerCase();
  const driverMode: DriverMode =
    normalizedDriverMode === "webhook"
      ? "webhook"
      : normalizedDriverMode === "theclaw"
        ? "theclaw"
        : normalizedDriverMode === "token"
          ? "token"
          : "token";
  const driverToken =
    resolveArg("--token") || process.env.THECLAW_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix") || process.env.THECLAW_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX || "Bot";
  const botToken =
    resolveArg("--bot-token") ||
    process.env.THECLAW_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix") ||
    process.env.THECLAW_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent = resolveArg("--agent") || process.env.THECLAW_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention") || process.env.THECLAW_DISCORD_SMOKE_MENTION_USER_ID || undefined;
  const instruction =
    resolveArg("--instruction") || process.env.THECLAW_DISCORD_SMOKE_INSTRUCTION || undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms") || process.env.THECLAW_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms") || process.env.THECLAW_DISCORD_SMOKE_POLL_MS,
    1_500,
  );
  ...
  const threadBindingsPath =
    ...
    process.env.THECLAW_DISCORD_SMOKE_THREAD_BINDINGS_PATH ||
    defaultBindingsPath;
  const theclawBin =
    resolveArg("--theclaw-bin") || process.env.THECLAW_DISCORD_SMOKE_THECLAW_BIN || "theclaw";
...
async function theclawCliJson<T>(params: { theclawBin: string; args: string[] }): Promise<T> {
  const result = await execFileAsync(params.theclawBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`theclaw ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithTheclaw(params: {
  theclawBin: string;
  target: string;
  limit: number;
}): Promise<DiscordMessage[]> {
  const response = await theclawCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    theclawBin: params.theclawBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "theclaw") {
    return await readMessagesWithTheclaw({
      theclawBin: params.args.theclawBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        authHeader: botAuthHeader,
        body: {
          name: `theclaw-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      setupStage = "send-message";
      const sent = await theclawCliJson<{
        payload?: {
          result?: {
            messageId?: string;
          };
      ...
      if (!sentMessageId) {
        throw new Error("theclaw message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    return {
  ...
  try {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "theclaw"
            ? await readMessagesWithTheclaw({
                theclawBin: args.theclawBin,
                target: threadId,
                limit: 50,
              })
    ...
    if (!ackMessage) {
      ...
      return {
        ...
        stage: "wait-ack",
        smokeId,
        error: `Thread bound (${threadId}) but timed out waiting for ACK token "${ackToken}" from TheClaw.`,
        diagnostics: {
          bindingCandidates: [
            {
