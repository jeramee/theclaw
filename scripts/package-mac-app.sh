...
# Build and bundle TheClaw into a minimal .app we can open.
# Outputs to dist/TheClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/TheClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="TheClaw"
MLX_TTS_HELPER_PRODUCT="theclaw-mlx-tts"
MLX_TTS_HELPER_ROOT="$ROOT_DIR/apps/macos-mlx-tts"
MLX_TTS_HELPER_BUILD_ROOT="$MLX_TTS_HELPER_ROOT/.build"
BUNDLE_ID="${BUNDLE_ID:-ai.theclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/theclaw/theclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/TheClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :TheClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TheClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/TheClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/TheClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/TheClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/TheClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/TheClaw/Resources/TheClaw.icns" "$APP_ROOT/Contents/Resources/TheClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/TheClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying TheClawKit resources"
THECLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/TheClawKit_TheClawKit.bundle"
if [ -d "$THECLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/TheClawKit_TheClawKit.bundle"
  cp -R "$THECLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/TheClawKit_TheClawKit.bundle"
else
  echo "WARN: TheClawKit resource bundle not found at $THECLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running TheClaw"
killall -q TheClaw 2>/dev/null || true
