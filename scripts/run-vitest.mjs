...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.THECLAW_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestNoOutputTimeoutMs(env = process.env) {
  return parsePositiveInt(env.THECLAW_VITEST_NO_OUTPUT_TIMEOUT_MS);
}
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.THECLAW_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.THECLAW_VITEST_MAX_WORKERS ?? env.THECLAW_TEST_WORKERS);
}
