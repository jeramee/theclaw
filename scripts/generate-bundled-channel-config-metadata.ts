...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const theclawMeta =
    ...
    "theclaw" in source.packageJson
      ? (source.packageJson.theclaw as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    theclawMeta &&
    typeof theclawMeta.channel === "object" &&
    theclawMeta.channel &&
    !Array.isArray(theclawMeta.channel)
      ? (theclawMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
