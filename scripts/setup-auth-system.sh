#!/bin/bash
# Setup TheClaw Auth Management System
...
echo "=== TheClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'theclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  TheClaw message: Send warning via TheClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/theclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/theclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/theclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now theclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status theclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
