...
const PUBLISHED_INSTALLER_BASE_URL = "https://theclaw.ai";
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.THECLAW_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.THECLAW_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.THECLAW_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `theclaw@${previousVersion}` : "theclaw@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.THECLAW_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.THECLAW_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
...
async function runUpgradeLane(params) {
  ...
  try {
    ...
    const updateResult = await runTheClaw({
      lane,
      env: updateEnv,
      args: updateArgs,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        THECLAW_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `theclaw-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".theclaw");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    THECLAW_HOME: lane.homeDir,
    THECLAW_STATE_DIR: lane.stateDir,
    THECLAW_CONFIG_PATH: join(lane.stateDir, "theclaw.json"),
    THECLAW_DISABLE_BONJOUR: "1",
    THECLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.THECLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
function isSelfSwappedPackageProcessExit(stderr) {
  return (
    ...
    stderr.includes("[theclaw] Failed to start CLI:") &&
    stderr.includes("ERR_MODULE_NOT_FOUND") &&
    /[\\/]node_modules[\\/]theclaw[\\/]dist[\\/]/u.test(stderr)
  );
}

export function resolveExplicitBaselineVersion(baselineSpec) {
  const trimmed = baselineSpec.trim();
  if (!trimmed || trimmed === "theclaw@latest") {
    return "";
  }
  if (trimmed.startsWith("theclaw@")) {
    return trimmed.slice("theclaw@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "theclaw@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view theclaw@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'theclaw.cmd'),
      (Join-Path $npmPrefix 'theclaw.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command theclaw -ErrorAction Stop
...
Write-Output "__THECLAW_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__THECLAW_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed theclaw path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v theclaw >/dev/null 2>&1",
    'printf "__THECLAW_PATH__=%s\\n" "$(command -v theclaw)"',
    "theclaw --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__THECLAW_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed theclaw path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `theclaw update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.theclaw/logs/${params.lane.name}-discord-gateway.log`),
    });
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.THECLAW_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.THECLAW_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.THECLAW_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.THECLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "theclaw/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.THECLAW_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.THECLAW_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.THECLAW_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing THECLAW_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "THECLAW_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.THECLAW_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.THECLAW_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...params.env,
      THECLAW_BROWSER_CONTROL_MODULE: pathToFileURL(overridePath).href,
      THECLAW_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      THECLAW_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
  ...
  try {
    while (Date.now() < deadline) {
      ...
      try {
        ...
        if (
          response.ok &&
          html.includes("<title>TheClaw Control</title>") &&
          html.includes("<theclaw-app></theclaw-app>")
        ) {
          logStream.write(
        ...
        logStream.write(
          `${new Date().toISOString()} dashboard-not-ready status=${response.status} title=${html.includes("<title>TheClaw Control</title>")} app=${html.includes("<theclaw-app></theclaw-app>")}\n`,
        );
      } catch (error) {
        logStream.write(
...
async function runTheClaw(params) {
...
function installedPackageRoot(prefixDir, platform = process.platform) {
  return platform === "win32"
    ? join(prefixDir, "node_modules", "theclaw")
    : join(prefixDir, "lib", "node_modules", "theclaw");
}

function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "theclaw.mjs");
}
