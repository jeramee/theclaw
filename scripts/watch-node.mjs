...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[theclaw] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[theclaw] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[theclaw] This usually means a file in node_modules is empty or truncated.");
  console.error("[theclaw] Recommended recovery:");
  console.error("[theclaw]   rm -rf node_modules");
  console.error("[theclaw]   pnpm store prune");
  console.error("[theclaw]   pnpm install");
  console.error("");
  console.error("[theclaw] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.THECLAW_WATCH_MODE = "1";
  childEnv.THECLAW_WATCH_SESSION = watchSession;
  ...
  childEnv.THECLAW_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.THECLAW_WATCH_COMMAND = deps.args.join(" ");
  }
