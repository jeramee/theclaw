#!/usr/bin/env bash
# Rootless TheClaw in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-theclaw-podman.sh launch        # Start gateway
#   ./scripts/run-theclaw-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   theclaw --container theclaw dashboard --no-open
#   theclaw --container theclaw channels login
#
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      THECLAW_GATEWAY_TOKEN|THECLAW_PODMAN_CONTAINER|THECLAW_PODMAN_IMAGE|THECLAW_IMAGE|THECLAW_PODMAN_PULL|THECLAW_PODMAN_GATEWAY_HOST_PORT|THECLAW_GATEWAY_PORT|THECLAW_PODMAN_BRIDGE_HOST_PORT|THECLAW_BRIDGE_PORT|THECLAW_GATEWAY_BIND|THECLAW_PODMAN_USERNS|THECLAW_BIND_MOUNT_OPTIONS|THECLAW_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-theclaw-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${THECLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.theclaw}"
ENV_FILE="${THECLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${THECLAW_CONFIG_DIR:-$EFFECTIVE_HOME/.theclaw}"
ENV_FILE="${THECLAW_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${THECLAW_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${THECLAW_PODMAN_CONTAINER:-theclaw}"
THECLAW_IMAGE="${THECLAW_PODMAN_IMAGE:-${THECLAW_IMAGE:-theclaw:local}}"
PODMAN_PULL="${THECLAW_PODMAN_PULL:-never}"
HOST_GATEWAY_PORT="${THECLAW_PODMAN_GATEWAY_HOST_PORT:-${THECLAW_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${THECLAW_PODMAN_BRIDGE_HOST_PORT:-${THECLAW_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${THECLAW_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$THECLAW_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v theclaw >/dev/null 2>&1; then
    return 0
  fi
  THECLAW_CONTAINER="" THECLAW_CONFIG_DIR="$config_dir" \
    theclaw config get gateway.bind 2>/dev/null || true
}
...
# THECLAW_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${THECLAW_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
create_token_env_file() {
  ...
  chmod 600 "$tmp"
  printf 'THECLAW_GATEWAY_TOKEN=%s\n' "$token" >"$tmp"
  printf '%s' "$tmp"
}
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v theclaw >/dev/null 2>&1; then
    echo "Warning: theclaw not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    THECLAW_CONTAINER="" THECLAW_CONFIG_DIR="$config_dir" \
      theclaw config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    THECLAW_CONTAINER="" THECLAW_CONFIG_DIR="$config_dir" \
      theclaw config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  THECLAW_CONTAINER="" THECLAW_CONFIG_DIR="$config_dir" \
    theclaw config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${THECLAW_GATEWAY_TOKEN:-}" ]]; then
  export THECLAW_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "THECLAW_GATEWAY_TOKEN" "$THECLAW_GATEWAY_TOKEN"
  echo "Generated THECLAW_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/theclaw.json"
...
PODMAN_USERNS="${THECLAW_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported THECLAW_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (THECLAW_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${THECLAW_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${THECLAW_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$THECLAW_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.theclaw/.npm \
    -e THECLAW_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.theclaw:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.theclaw/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$THECLAW_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME theclaw dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME theclaw devices approve --latest  # if pairing required"
