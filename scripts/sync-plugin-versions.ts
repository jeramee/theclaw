...
const THECLAW_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncTheClawDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.theclaw;
  if (!current || current === "workspace:*" || !THECLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.theclaw?.compat;
  const current = compat?.pluginApi;
  if (!current || !THECLAW_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildTheClawVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.theclaw?.build;
  const current = build?.theclawVersion;
  if (!current) {
    return false;
  }
  ...
  build.theclawVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(rootDir = resolve(".")) {
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncTheClawDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncTheClawDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildTheClawVersionChanged = syncBuildTheClawVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildTheClawVersionChanged;
    if (!packageChanged) {
