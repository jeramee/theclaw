...
- regenerates apps/ios/TheClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${THECLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid THECLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid THECLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid THECLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid THECLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing THECLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(THECLAW_URL_SLASH)$(THECLAW_URL_SLASH)#g'
)"
...
THECLAW_CODE_SIGN_STYLE = Automatic
THECLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
THECLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
THECLAW_APP_BUNDLE_ID = ai.theclaw.client
THECLAW_SHARE_BUNDLE_ID = ai.theclaw.client.share
THECLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.theclaw.client.activitywidget
THECLAW_WATCH_APP_BUNDLE_ID = ai.theclaw.client.watchkitapp
THECLAW_WATCH_EXTENSION_BUNDLE_ID = ai.theclaw.client.watchkitapp.extension
THECLAW_APP_PROFILE =
THECLAW_SHARE_PROFILE =
THECLAW_PUSH_TRANSPORT = relay
THECLAW_PUSH_DISTRIBUTION = official
THECLAW_URL_SLASH = /
THECLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
THECLAW_PUSH_APNS_ENVIRONMENT = production
EOF
