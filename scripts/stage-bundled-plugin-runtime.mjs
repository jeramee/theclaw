...
function removeStaleTheClawSelfReference(sourcePluginNodeModulesDir, repoRoot) {
  if (!fs.existsSync(sourcePluginNodeModulesDir)) {
    return;
  }

  const selfReferencePath = path.join(sourcePluginNodeModulesDir, "theclaw");
  try {
...
function ensureTheClawExtensionAlias(params) {
  const pluginSdkDir = path.join(params.repoRoot, "dist", "plugin-sdk");
  if (!fs.existsSync(pluginSdkDir)) {
    return;
  }

  const aliasDir = path.join(params.distExtensionsRoot, "node_modules", "theclaw");
...
function shouldCopyRuntimeFile(relativePath) {
  return (
    ...
    isPathOrNestedPath(relativePath, "theclaw.plugin.json") ||
...
function linkPluginNodeModules(params) {
  ...
  removeStaleTheClawSelfReference(params.sourcePluginNodeModulesDir, params.repoRoot);
  ensureSymlink(
...
export function stageBundledPluginRuntime(params = {}) {
  ...
  ensureTheClawExtensionAlias({ repoRoot, distExtensionsRoot });
