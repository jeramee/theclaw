...
# build the runner/functional images with the prepared TheClaw package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${THECLAW_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$THECLAW_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${THECLAW_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because THECLAW_DOCKER_BUILD_ON_MISSING/THECLAW_TESTBOX allows fallback."
      else
        echo "Docker image not found: $image_name" >&2
        echo "Build it first or unset THECLAW_SKIP_DOCKER_BUILD." >&2
        return 1
      fi
    else
      return 0
  ...
  if [ "$target" = "functional" ]; then
    ...
    build_args+=(--build-context "theclaw_package=$package_context")
  fi
