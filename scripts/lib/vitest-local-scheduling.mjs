...
function isSystemThrottleDisabled(env) {
  const normalized = env.THECLAW_VITEST_DISABLE_SYSTEM_THROTTLE?.trim().toLowerCase();
...
export function resolveLocalVitestEnv(env = process.env) {
  const normalizedLocalCheck = env.THECLAW_LOCAL_CHECK?.trim().toLowerCase();
...
export function resolveLocalVitestScheduling(
  ...
  const override = parsePositiveInt(env.THECLAW_VITEST_MAX_WORKERS ?? env.THECLAW_TEST_WORKERS);
  if (override !== null) {
