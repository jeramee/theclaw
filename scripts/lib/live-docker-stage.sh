...
theclaw_live_stage_source_tree() {
  local dest_dir="${1:?destination directory required}"
  local stage_mode="${THECLAW_LIVE_DOCKER_SOURCE_STAGE_MODE:-copy}"

  if [ "$stage_mode" = "symlink" ]; then
    echo "THECLAW_LIVE_DOCKER_SOURCE_STAGE_MODE=symlink is disabled; using copy staging." >&2
  fi
...
theclaw_live_link_runtime_tree() {
  local dest_dir="${1:?destination directory required}"
  ...
  if [ -d /app/dist-runtime/extensions ]; then
    export THECLAW_BUNDLED_PLUGINS_DIR=/app/dist-runtime/extensions
  elif [ -d /app/dist/extensions ]; then
    export THECLAW_BUNDLED_PLUGINS_DIR=/app/dist/extensions
  fi
}

theclaw_live_stage_node_modules() {
  local dest_dir="${1:?destination directory required}"
...
theclaw_live_stage_state_dir() {
  local dest_dir="${1:?destination directory required}"
  local source_dir="${HOME}/.theclaw"
  ...
  export THECLAW_STATE_DIR="$dest_dir"
  export THECLAW_CONFIG_PATH="$dest_dir/theclaw.json"
}

theclaw_live_prepare_staged_config() {
  if [ ! -f "${THECLAW_CONFIG_PATH:-}" ]; then
    return 0
  fi
