...
export const DEFAULT_E2E_BARE_IMAGE = "theclaw-docker-e2e-bare:local";
export const DEFAULT_E2E_FUNCTIONAL_IMAGE = "theclaw-docker-e2e-functional:local";
...
export function parseLiveMode(raw) {
  ...
  throw new Error(
    `THECLAW_DOCKER_ALL_LIVE_MODE must be one of: all, skip, only. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function parseProfile(raw) {
  ...
  throw new Error(
    `THECLAW_DOCKER_ALL_PROFILE must be one of: ${DEFAULT_PROFILE}, ${RELEASE_PATH_PROFILE}. Got: ${JSON.stringify(raw)}`,
  );
}
...
export function lanesNeedTheClawPackage(poolLanes) {
...
export function buildPlanJson(params) {
  ...
  return {
    ...
    needs: {
      ...
      package: lanesNeedTheClawPackage(scheduledLanes),
    },
