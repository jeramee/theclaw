...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "theclaw-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "theclaw.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".theclaw", "workspace");
