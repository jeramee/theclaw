...
function usage() {
  return `Usage:
  node scripts/lib/theclaw-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/theclaw-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/theclaw-test-state.mjs shell-function
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "theclaw-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(scenario) {
  if (scenario === "external-service") {
    return {
      THECLAW_SERVICE_REPAIR_POLICY: "external",
    };
  }
  return {};
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'THECLAW_TEST_STATE_JSON'`,
    json,
    "THECLAW_TEST_STATE_JSON",
  ].join("\n");
}
...
export async function createState(options = {}) {
  const label = normalizeLabel(options.label);
  const root = await fs.mkdtemp(path.join(os.tmpdir(), `theclaw-${label}-`));
...
export function renderShellSnippet(options = {}) {
  ...
  const template = `/tmp/theclaw-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    `THECLAW_TEST_STATE_HOME="$(mktemp -d ${shellQuote(template)})"`,
    'export HOME="$THECLAW_TEST_STATE_HOME"',
    'export USERPROFILE="$THECLAW_TEST_STATE_HOME"',
    'export THECLAW_HOME="$THECLAW_TEST_STATE_HOME"',
    'export THECLAW_STATE_DIR="$THECLAW_TEST_STATE_HOME/.theclaw"',
    'export THECLAW_CONFIG_PATH="$THECLAW_STATE_DIR/theclaw.json"',
    'export THECLAW_TEST_WORKSPACE_DIR="$THECLAW_TEST_STATE_HOME/workspace"',
    'mkdir -p "$THECLAW_STATE_DIR" "$THECLAW_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$THECLAW_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  return `theclaw_test_state_create() {
  ...
  case "$scenario" in
    ...
    *)
      echo "unknown TheClaw test-state scenario: $scenario" >&2
      return 1
      ;;
  esac
  case "$raw_label" in
    /*)
      THECLAW_TEST_STATE_HOME="$raw_label"
      mkdir -p "$THECLAW_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      THECLAW_TEST_STATE_HOME="$(mktemp -d "/tmp/theclaw-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$THECLAW_TEST_STATE_HOME"
  export USERPROFILE="$THECLAW_TEST_STATE_HOME"
  ...
  export THECLAW_CONFIG_PATH="$THECLAW_STATE_DIR/theclaw.json"
  export THECLAW_TEST_WORKSPACE_DIR="$THECLAW_TEST_STATE_HOME/workspace"
  unset THECLAW_AGENT_DIR
  unset PI_CODING_AGENT_DIR
  unset THECLAW_SERVICE_REPAIR_POLICY
  mkdir -p "$THECLAW_STATE_DIR" "$THECLAW_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$THECLAW_CONFIG_PATH" <<'THECLAW_TEST_STATE_JSON'
{}
THECLAW_TEST_STATE_JSON
      ;;
    update-stable)
...
THECLAW_TEST_STATE_JSON
      ;;
    external-service)
      export THECLAW_SERVICE_REPAIR_POLICY="external"
      cat > "$THECLAW_CONFIG_PATH" <<'THECLAW_TEST_STATE_JSON'
{}
THECLAW_TEST_STATE_JSON
      ;;
