...
function applyDefaultParallelVitestWorkerBudget(specs, env) {
  if (env.THECLAW_VITEST_MAX_WORKERS || env.THECLAW_TEST_WORKERS || isCiLikeEnv(env)) {
    return specs;
  }
  ...
  return specs.map((spec) => ({
    ...
    env: {
      ...spec.env,
      THECLAW_VITEST_MAX_WORKERS: String(vitestMaxWorkers),
    },
  }));
}
...
async function main() {
  ...
  const isExplicitParallelMultiConfigRun =
    Boolean(baseEnv.THECLAW_TEST_PROJECTS_PARALLEL) &&
    runSpecs.length > 1 &&
    !runSpecs.some((spec) => spec.watchMode);
  ...
  if (isParallelShardRun) {
    ...
    if (concurrency > 1) {
      ...
      if (
        !isCiLikeEnv(baseEnv) &&
        !baseEnv.THECLAW_TEST_PROJECTS_PARALLEL &&
        !baseEnv.THECLAW_VITEST_MAX_WORKERS &&
        !baseEnv.THECLAW_TEST_WORKERS &&
