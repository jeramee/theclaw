...
tsx_version="${THECLAW_RELEASE_TSX_VERSION:-${TSX_VERSION:-4.21.0}}"
script_path="${THECLAW_RELEASE_CHECKS_SCRIPT:-workflow/scripts/theclaw-cross-os-release-checks.ts}"
...
temp_root="${THECLAW_RELEASE_TSX_TOOL_ROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}"
if command -v cygpath >/dev/null 2>&1; then
  temp_root="$(cygpath -u "${temp_root}")"
fi

tool_dir="${THECLAW_RELEASE_TSX_TOOL_DIR:-${temp_root}/theclaw-release-tsx-${tsx_version}}"
