...
REMOTE_URL="${THECLAW_REF_REMOTE:-https://github.com/theclaw/theclaw.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-theclaw-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves TheClaw branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve TheClaw ref: ${REF}" >&2
exit 1
