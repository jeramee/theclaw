...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "theclaw.json");
...
function sanitizedEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    LOGNAME: process.env.LOGNAME ?? "theclaw-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "theclaw-bench",
    npm_config_update_notifier: "false",
    THECLAW_CONFIG: configPath,
    THECLAW_CONFIG_PATH: configPath,
    THECLAW_GATEWAY_STARTUP_TRACE: "1",
    THECLAW_HOME: root,
    THECLAW_LOCAL_CHECK: "0",
    THECLAW_NO_RESPAWN: "1",
    THECLAW_STATE_DIR: path.join(root, "state"),
    THECLAW_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
  return env;
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "theclaw-gateway-bench-"));
