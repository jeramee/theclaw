...
import { discoverTheClawPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']theclaw\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']theclaw\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']theclaw\/plugin-sdk\/channel-runtime["']/,
    label: "theclaw/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']theclaw\/plugin-sdk\/config-runtime["']/,
    label: "theclaw/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']theclaw\/plugin-sdk\/infra-runtime["']/,
    label: "theclaw/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic theclaw/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy theclaw/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused theclaw/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
