...
#
# Suggested cron: */30 * * * * /home/admin/theclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send TheClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., theclaw-alerts)
...
STATE_FILE="$HOME/.theclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via TheClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use theclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via TheClaw to $NOTIFY_PHONE..."
            theclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! TheClaw is down. Run: ssh l36 '~/theclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
