...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/theclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "TheClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/theclaw/scripts/mobile-reauth.sh'" --id theclaw-auth
        ;;
    *)
