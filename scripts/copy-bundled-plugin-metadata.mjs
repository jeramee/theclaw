...
function shouldCopyBundledPluginMetadata(id, env) {
  ...
  return env.THECLAW_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "theclaw.plugin.json");
    ...
    if (packageJson.theclaw && "extensions" in packageJson.theclaw) {
      packageJson.theclaw = {
        ...packageJson.theclaw,
        extensions: rewritePackageExtensions(packageJson.theclaw.extensions),
        ...(typeof packageJson.theclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.theclaw.setupEntry) }
          : {}),
      };
    }
