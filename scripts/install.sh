...
# TheClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one TheClaw."
...
GUM_VERSION="${THECLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 TheClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://theclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$THECLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.theclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_theclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.theclaw-* "$npm_root"/theclaw 2>/dev/null || true
}

extract_theclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_theclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/theclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/theclaw"|"/usr/local/bin/theclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/theclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale theclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing theclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing TheClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_theclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "TheClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*theclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_theclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_theclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "TheClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an theclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${THECLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${THECLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((THECLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${THECLAW_NO_ONBOARD:-0}
NO_PROMPT=${THECLAW_NO_PROMPT:-0}
DRY_RUN=${THECLAW_DRY_RUN:-0}
INSTALL_METHOD=${THECLAW_INSTALL_METHOD:-}
THECLAW_VERSION=${THECLAW_VERSION:-latest}
USE_BETA=${THECLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/theclaw"
GIT_DIR=${THECLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${THECLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${THECLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${THECLAW_VERBOSE:-0}"
VERIFY_INSTALL="${THECLAW_VERIFY_INSTALL:-0}"
THECLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
TheClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/theclaw)
...
Environment variables:
  THECLAW_INSTALL_METHOD=git|npm
  THECLAW_VERSION=latest|next|main|<semver>|<spec>
  THECLAW_BETA=0|1
  THECLAW_GIT_DIR=...
  THECLAW_GIT_UPDATE=0|1
  THECLAW_NO_PROMPT=1
  THECLAW_VERIFY_INSTALL=1
  THECLAW_DRY_RUN=1
  THECLAW_NO_ONBOARD=1
  THECLAW_VERBOSE=1
  THECLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected TheClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a TheClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_theclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"theclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://theclaw.ai/install.sh | bash"
}
...
ensure_theclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/theclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/theclaw" ]]; then
        ln -sf "$npm_root/theclaw/dist/entry.js" "${npm_bin}/theclaw"
        ui_info "Created theclaw bin link at ${npm_bin}/theclaw"
    fi
    return 0
}

# Check for existing TheClaw installation
check_existing_theclaw() {
    if [[ -n "$(type -P theclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing TheClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
collect_theclaw_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${THECLAW_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_theclaw_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/theclaw"
        ...
        local version=""
        version="$(theclaw_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_theclaw_npm_root_candidates)
}

warn_duplicate_theclaw_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_theclaw_global_installs)

    if [[ "${#installs[@]}" -le 1 ]]; then
        return 0
    fi

    ui_warn "Multiple TheClaw global installs detected"
    echo "  Different Node/npm environments can run different TheClaw versions."

    local active_node active_npm active_theclaw
    active_node="$(command -v node 2>/dev/null || true)"
    active_npm="$(command -v npm 2>/dev/null || true)"
    active_theclaw="${THECLAW_BIN:-}"
    if [[ -z "$active_theclaw" ]]; then
        active_theclaw="$(type -P theclaw 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active theclaw: ${INFO}${active_theclaw:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make theclaw show as \"command not found\" in new terminals."
...
warn_theclaw_not_found() {
    ui_warn "Installed, but theclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t theclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named theclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_theclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P theclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/theclaw" ]]; then
        echo "${npm_bin}/theclaw"
        return 0
    fi
...
install_theclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/theclaw/theclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing TheClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing TheClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning TheClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building TheClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/theclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/theclaw"
    ui_success "TheClaw wrapper installed to \$HOME/.local/bin/theclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view theclaw dist-tags.beta 2>/dev/null || true)"
...
resolve_package_install_spec() {
    ...
    normalized_value="$(to_lowercase_ascii "$value")"
    if [[ "$normalized_value" == "main" ]]; then
        echo "github:theclaw/theclaw#main"
        return 0
    fi
    if is_explicit_package_install_spec "$value"; then
        echo "$value"
...
install_theclaw() {
    local package_name="theclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            THECLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="theclaw"
        else
            THECLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${THECLAW_VERSION}" ]]; then
        THECLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${THECLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${THECLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing TheClaw v${resolved_version}"
    else
        ui_info "Installing TheClaw (${THECLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${THECLAW_VERSION}")"

    if ! install_theclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_theclaw_paths
        install_theclaw_npm "${install_spec}"
    fi

    if [[ "${THECLAW_VERSION}" == "latest" && "${package_name}" == "theclaw" ]]; then
        if ! resolve_theclaw_bin &> /dev/null; then
            ui_warn "npm install theclaw@latest failed; retrying theclaw@next"
            cleanup_npm_theclaw_paths
            install_theclaw_npm "theclaw@next"
        fi
    fi

    ensure_theclaw_bin_link || true

    ui_success "TheClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${THECLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_theclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (theclaw not on PATH yet)"
        warn_theclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${THECLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.theclaw/workspace-${profile}"
    else
        echo "${HOME}/.theclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${THECLAW_CONFIG_PATH:-$HOME/.theclaw/theclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run theclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but theclaw not on PATH; skipping onboarding"
        warn_theclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run theclaw onboard to retry"
        return
    }
}
...
resolve_theclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${THECLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v theclaw &> /dev/null; then
        claw="$(command -v theclaw)"
    fi
    if [[ -n "$claw" ]]; then
        ...
        version="$(extract_theclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/theclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/theclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: theclaw not on PATH yet"
        warn_theclaw_not_found
        return 1
    fi

    run_quiet_step "Checking TheClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: theclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_theclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found TheClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set THECLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if check_existing_theclaw; then
        is_upgrade=true
    fi
    ...
    ui_stage "Installing TheClaw"

    local final_git_dir=""
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        # Clean up npm global install if switching to git
        if npm list -g theclaw &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g theclaw 2>/dev/null || true
        ...
        install_theclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/theclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/theclaw"
    ...
    THECLAW_BIN="$(resolve_theclaw_bin || true)"
    warn_duplicate_theclaw_global_installs || true
    ...
    installed_version=$(resolve_theclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 TheClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 TheClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/theclaw"
        ui_kv "Update command" "theclaw update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://theclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running theclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                THECLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                THECLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                THECLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run theclaw doctor and theclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run theclaw onboard later"
        else
            local config_path="${THECLAW_CONFIG_PATH:-$HOME/.theclaw/theclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (theclaw not on PATH yet)"
                    warn_theclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run theclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v theclaw &> /dev/null; then
        local claw="${THECLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_theclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (theclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if THECLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: theclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${THECLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
