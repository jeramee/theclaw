...
resolve_default_smoke_platform() {
  local host_os
  local host_arch
  if [[ -n "${THECLAW_INSTALL_SMOKE_PLATFORM:-}" ]]; then
    printf "%s" "$THECLAW_INSTALL_SMOKE_PLATFORM"
    return
  fi
...
const parsed = JSON.parse(raw);
const budgetOverride = process.env.THECLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES;
const budgetBytes = budgetOverride ? Number(budgetOverride) : undefined;
if (budgetOverride && !Number.isFinite(budgetBytes)) {
  throw new Error(
    `THECLAW_INSTALL_SMOKE_PACK_UNPACKED_BUDGET_BYTES must be numeric, got ${JSON.stringify(
      budgetOverride,
    )}`,
  );
}
...
SMOKE_IMAGE="${THECLAW_INSTALL_SMOKE_IMAGE:-theclaw-install-smoke:local}"
NONROOT_IMAGE="${THECLAW_INSTALL_NONROOT_IMAGE:-theclaw-install-nonroot:local}"
SMOKE_PLATFORM="$(resolve_default_smoke_platform)"
NONROOT_PLATFORM="${THECLAW_INSTALL_NONROOT_PLATFORM:-$SMOKE_PLATFORM}"
INSTALL_URL="${THECLAW_INSTALL_URL:-https://theclaw.bot/install.sh}"
CLI_INSTALL_URL="${THECLAW_INSTALL_CLI_URL:-https://theclaw.bot/install-cli.sh}"
PACKAGE_NAME="${THECLAW_INSTALL_PACKAGE:-theclaw}"
SKIP_NONROOT="${THECLAW_INSTALL_SMOKE_SKIP_NONROOT:-0}"
SKIP_SMOKE_IMAGE_BUILD="${THECLAW_INSTALL_SMOKE_SKIP_IMAGE_BUILD:-0}"
SKIP_NONROOT_IMAGE_BUILD="${THECLAW_INSTALL_NONROOT_SKIP_IMAGE_BUILD:-0}"
SKIP_UPDATE="${THECLAW_INSTALL_SMOKE_SKIP_UPDATE:-0}"
SKIP_NPM_GLOBAL="${THECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL:-0}"
UPDATE_BASELINE_VERSION="${THECLAW_INSTALL_SMOKE_UPDATE_BASELINE:-latest}"
UPDATE_PACKAGE_SPEC="${THECLAW_INSTALL_SMOKE_UPDATE_PACKAGE_SPEC:-}"
UPDATE_DIST_IMAGE="${THECLAW_INSTALL_SMOKE_UPDATE_DIST_IMAGE:-}"
UPDATE_SKIP_LOCAL_BUILD="${THECLAW_INSTALL_SMOKE_UPDATE_SKIP_LOCAL_BUILD:-0}"
UPDATE_HOST_ALIAS="${THECLAW_INSTALL_SMOKE_UPDATE_HOST:-host.docker.internal}"
UPDATE_PORT="${THECLAW_INSTALL_SMOKE_UPDATE_PORT:-}"
UPDATE_EXPECT_VERSION="${THECLAW_INSTALL_SMOKE_UPDATE_EXPECT_VERSION:-}"
LATEST_DIR="$(mktemp -d)"
...
NPM_CACHE_DIR="${THECLAW_INSTALL_SMOKE_NPM_CACHE_DIR:-}"
NPM_CACHE_OWNED=0
...
if [[ "$SKIP_UPDATE" == "1" ]]; then
  echo "==> Skip update smoke (THECLAW_INSTALL_SMOKE_SKIP_UPDATE=1)"
else
  prepare_update_tarball
  prepare_update_host_access
  prepare_npm_cache
  ...
  docker run --rm -t \
    ...
    -e THECLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e THECLAW_INSTALL_METHOD=npm \
    -e THECLAW_INSTALL_FRESH_VERSION="$UPDATE_EXPECT_VERSION" \
    -e THECLAW_INSTALL_FRESH_TAG_URL="$FRESH_TAG_URL" \
    -e THECLAW_INSTALL_LATEST_OUT="/out/latest" \
    -e THECLAW_NO_ONBOARD=1 \
    -e THECLAW_NO_PROMPT=1 \
  ...
  docker run --rm -t \
    ...
    "${NPM_CACHE_DOCKER_ARGS[@]}" \
    -e THECLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e THECLAW_INSTALL_SMOKE_MODE=update \
    -e THECLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
    -e THECLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
    -e THECLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
    -e THECLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
    -e THECLAW_NO_ONBOARD=1 \
    -e THECLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$SMOKE_IMAGE"
  ...
  if [[ "$SKIP_NPM_GLOBAL" == "1" ]]; then
    echo "==> Skip direct npm global smoke (THECLAW_INSTALL_SMOKE_SKIP_NPM_GLOBAL=1)"
  else
    ...
    docker run --rm -t \
      ...
      "${NPM_CACHE_DOCKER_ARGS[@]}" \
      -e THECLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
      -e THECLAW_INSTALL_SMOKE_MODE=npm-global \
      -e THECLAW_INSTALL_UPDATE_BASELINE="$UPDATE_BASELINE_VERSION" \
      -e THECLAW_INSTALL_UPDATE_BASELINE_TAG_URL="$BASELINE_TAG_URL" \
      -e THECLAW_INSTALL_UPDATE_EXPECT_VERSION="$UPDATE_EXPECT_VERSION" \
      -e THECLAW_INSTALL_UPDATE_TAG_URL="$UPDATE_TAG_URL" \
...
if [[ "$SKIP_NONROOT" == "1" ]]; then
  echo "==> Skip non-root installer smoke (THECLAW_INSTALL_SMOKE_SKIP_NONROOT=1)"
else
  ...
  docker run --rm -t \
    ...
    -e THECLAW_INSTALL_PACKAGE="$PACKAGE_NAME" \
    -e THECLAW_INSTALL_METHOD=npm \
    -e THECLAW_INSTALL_EXPECT_VERSION="$LATEST_VERSION" \
    -e THECLAW_NO_ONBOARD=1 \
    -e THECLAW_NO_PROMPT=1 \
    -e DEBIAN_FRONTEND=noninteractive \
    "$NONROOT_IMAGE"
...
if [[ "${THECLAW_INSTALL_SMOKE_SKIP_CLI:-0}" == "1" ]]; then
  echo "==> Skip CLI installer smoke (THECLAW_INSTALL_SMOKE_SKIP_CLI=1)"
  exit 0
fi
...
docker run --rm -t \
  ...
  --entrypoint /bin/bash \
  -e THECLAW_INSTALL_URL="$INSTALL_URL" \
  -e THECLAW_INSTALL_CLI_URL="$CLI_INSTALL_URL" \
  -e THECLAW_NO_ONBOARD=1 \
  -e THECLAW_NO_PROMPT=1 \
  -e DEBIAN_FRONTEND=noninteractive \
