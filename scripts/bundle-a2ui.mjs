...
const a2uiAppDir = path.join(rootDir, "apps", "shared", "TheClawKit", "Tools", "CanvasA2UI");
...
export function getBundleHashRepoInputPaths(repoRoot = rootDir) {
  return [
    ...
    path.join(repoRoot, "apps", "shared", "TheClawKit", "Tools", "CanvasA2UI"),
  ];
}
...
async function main() {
  ...
  if (!hasRendererDir || !hasAppDir) {
    ...
    if (process.env.THECLAW_SPARSE_PROFILE || process.env.THECLAW_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI sources missing; skipping bundle because THECLAW_A2UI_SKIP_MISSING=1 or THECLAW_SPARSE_PROFILE is set.",
      );
      return;
    }
