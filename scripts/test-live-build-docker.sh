...
DOCKER_BUILD_EXTENSIONS="${THECLAW_DOCKER_BUILD_EXTENSIONS:-${THECLAW_EXTENSIONS:-}}"
...
if [[ -n "${DOCKER_BUILD_EXTENSIONS}" ]]; then
  DOCKER_BUILD_ARGS+=(--build-arg "THECLAW_EXTENSIONS=${DOCKER_BUILD_EXTENSIONS}")
fi

if [[ "${THECLAW_SKIP_DOCKER_BUILD:-}" == "1" ]]; then
  ...
  echo "==> Live-test image not available; building because THECLAW_DOCKER_BUILD_ON_MISSING/THECLAW_TESTBOX allows fallback."
fi
