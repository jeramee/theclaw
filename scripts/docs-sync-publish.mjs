...
const SYNC_SUPPORT_FILES = [
  {
    ...
    target: path.join(".theclaw-sync", "check-docs-mdx.mjs"),
  },
  {
    source: path.join(ROOT, "scripts", "lib", "mintlify-accordion.mjs"),
    target: path.join(".theclaw-sync", "lib", "mintlify-accordion.mjs"),
  },
  {
    source: path.join(ROOT, ".github", "codex", "prompts", "docs-mdx-repair.md"),
    target: path.join(".theclaw-sync", "docs-mdx-repair.md"),
  },
];
...
function writeSyncMetadata(targetRoot, args) {
  ...
  writeJson(path.join(targetRoot, ".theclaw-sync", "source.json"), metadata);
}
