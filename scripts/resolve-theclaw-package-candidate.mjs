...
const DEFAULT_OUTPUT_NAME = "theclaw-current.tgz";
export const THECLAW_PACKAGE_SPEC_RE =
  /^theclaw@(beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-beta\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-theclaw-package-candidate.mjs --source <ref|npm|url|artifact> --output-dir <dir> [options]
...
export function validateTheClawPackageSpec(spec) {
  if (!THECLAW_PACKAGE_SPEC_RE.test(spec)) {
    throw new Error(
      `package_spec must be theclaw@beta, theclaw@latest, or an exact TheClaw release version; got: ${spec}`,
    );
  }
}
...
async function resolveTrustedRepoRef(ref) {
  ...
  throw new Error(
    `package_ref ${ref} resolved to ${selectedSha}, which is not reachable from an TheClaw branch or release tag`,
  );
}
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `theclaw-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => /^theclaw-.*\.tgz$/u.test(entry))
      .toSorted((a, b) => a.localeCompare(b))
      .at(-1);
  }
  if (!filename) {
    throw new Error(`npm pack produced no TheClaw tarball in ${outputDir}`);
  }
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await run("node", [
        "scripts/package-theclaw-for-docker.mjs",
        "--source-dir",
      ...
      validateTheClawPackageSpec(options.packageSpec);
  ...
  await run("node", ["scripts/check-theclaw-package-tarball.mjs", target]);
  ...
  if (pkg.name !== "theclaw") {
    throw new Error(`package candidate must be named "theclaw"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
