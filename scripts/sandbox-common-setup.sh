...
BASE_IMAGE="${BASE_IMAGE:-theclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-theclaw-sandbox-common:bookworm-slim}"
...
THECLAW_DOCKER_BUILD_USE_BUILDX="${THECLAW_DOCKER_BUILD_USE_BUILDX:-0}"
THECLAW_DOCKER_BUILD_CACHE_FROM="${THECLAW_DOCKER_BUILD_CACHE_FROM:-}"
THECLAW_DOCKER_BUILD_CACHE_TO="${THECLAW_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=theclaw.sandbox=1)
NOTE
