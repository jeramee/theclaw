...
const WATCH_GATEWAY_SKIP_ENV = {
  ...
  THECLAW_SKIP_ACPX_RUNTIME: "1",
  THECLAW_SKIP_ACPX_RUNTIME_PROBE: "1",
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const shellSource = [
    'echo "$$" > "$THECLAW_WATCH_PID_FILE"',
    'mkdir -p "$THECLAW_HOME/.theclaw"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$THECLAW_HOME/.theclaw/theclaw.json"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    THECLAW_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    THECLAW_HOME: isolatedHomeDir,
...
async function runTimedWatch(options, outputDir) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-gateway-watch-"));
