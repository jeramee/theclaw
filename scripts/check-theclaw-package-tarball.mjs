...
function usage() {
  return "Usage: node scripts/check-theclaw-package-tarball.mjs <theclaw.tgz>";
}
...
if (!fs.existsSync(tarball)) {
  fail(`TheClaw package tarball does not exist: ${tarball}`);
}
...
if (errors.length > 0) {
  fail(`TheClaw package tarball integrity failed:\n${errors.join("\n")}`);
}

for (const warning of warnings) {
  console.warn(`TheClaw package tarball integrity warning: ${warning}`);
}
console.log("TheClaw package tarball integrity passed.");
