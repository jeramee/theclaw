 ...
 * `theclaw update`. Use it to verify the completion installation flow
...
\`theclaw update\`, \`theclaw doctor\`, and \`theclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# TheClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`theclaw completion --install\` later to enable.`));
    return;
  }
