...
// legacy eager-install escape hatch is explicitly enabled; `theclaw doctor
...
const DISABLE_POSTINSTALL_ENV = "THECLAW_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "THECLAW_DISABLE_PLUGIN_REGISTRY_MIGRATION";
const EAGER_BUNDLED_PLUGIN_DEPS_ENV = "THECLAW_EAGER_BUNDLED_PLUGIN_DEPS";
...
function isStagedRuntimeDependencyPath(relativePath) {
  return /^dist\/extensions\/[^/]+\/(?:node_modules|\.theclaw-install-stage(?:-[^/]+)?)(?:\/|$)/u.test(
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.theclaw-hotfix-${randomUUID()}`,
      ));
  const writeFile =
