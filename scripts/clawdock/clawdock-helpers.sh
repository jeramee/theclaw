#!/usr/bin/env bash
# ClawDock - Docker helpers for TheClaw
# Inspired by Simon Willison's "Running TheClaw in Docker"
# https://til.simonwillison.net/llms/theclaw-docker
#
# Installation:
...
# Common paths to check for TheClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/theclaw"
  "${HOME}/workspace/theclaw"
  "${HOME}/projects/theclaw"
  "${HOME}/dev/theclaw"
  "${HOME}/code/theclaw"
  "${HOME}/src/theclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found TheClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/theclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ TheClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/theclaw/theclaw.git ~/theclaw"
    echo "  cd ~/theclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^THECLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d theclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart theclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f theclaw-gateway
}
...
clawdock-show-config() {
  ...
  local config_dir="${HOME}/.theclaw"
  echo -e "${_CLR_BOLD}Config directory:${_CLR_RESET} ${_CLR_CYAN}${config_dir}${_CLR_RESET}"
  echo ""

  # Show theclaw.json
  if [[ -f "${config_dir}/theclaw.json" ]]; then
    echo -e "${_CLR_BOLD}${config_dir}/theclaw.json${_CLR_RESET}"
    echo -e "${_CLR_DIM}$(cat "${config_dir}/theclaw.json")${_CLR_RESET}"
  else
    echo -e "${_CLR_YELLOW}No theclaw.json found${_CLR_RESET}"
  fi
  echo ""
...
clawdock-workspace() {
  cd ~/.theclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec theclaw-gateway \
    bash -c 'echo "alias theclaw=\"./theclaw.mjs\"" > /tmp/.bashrc_theclaw && bash --rcfile /tmp/.bashrc_theclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm theclaw-cli "$@"
}
...
clawdock-update() {
  ...
  _clawdock_compose build theclaw-gateway || { echo "❌ Build failed"; return 1; }

  echo ""
  echo "♻️  Recreating container with new image..."
  _clawdock_compose down 2>&1 | _clawdock_filter_warnings
  _clawdock_compose up -d theclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-rebuild() {
  _clawdock_compose build theclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "THECLAW_GATEWAY_TOKEN=$token" theclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" theclaw-gateway \
    bash -c './theclaw.mjs config set gateway.remote.token "$TOKEN" && ./theclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec theclaw-gateway \
    bash -c "./theclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart theclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm theclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec theclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'theclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for TheClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (theclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to theclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.theclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'theclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'theclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.theclaw.ai${_CLR_RESET}"
  echo ""
}
