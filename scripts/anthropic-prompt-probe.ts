...
// Usage:
// THECLAW_PROMPT_TRANSPORT=direct|gateway
// THECLAW_PROMPT_MODE=extra|override
// THECLAW_PROMPT_TEXT='...'
// THECLAW_PROMPT_CAPTURE=1
...
import { resolveTheClawAgentDir } from "../src/agents/agent-paths.js";
...
const TRANSPORT = process.env.THECLAW_PROMPT_TRANSPORT?.trim() === "direct" ? "direct" : "gateway";
const GATEWAY_PROMPT_MODE =
  process.env.THECLAW_PROMPT_MODE?.trim() === "override" ? "override" : "extra";
const PROMPT_TEXT = process.env.THECLAW_PROMPT_TEXT?.trim() ?? "";
const PROMPT_LIST_JSON = process.env.THECLAW_PROMPT_LIST_JSON?.trim() ?? "";
const USER_PROMPT = process.env.THECLAW_USER_PROMPT?.trim() || "is clawd here?";
const ENABLE_CAPTURE = process.env.THECLAW_PROMPT_CAPTURE === "1";
const INCLUDE_RAW = process.env.THECLAW_PROMPT_INCLUDE_RAW === "1";
const CLAUDE_BIN = process.env.CLAUDE_BIN?.trim() || "claude";
const NODE_BIN = process.env.THECLAW_NODE_BIN?.trim() || process.execPath;
const TIMEOUT_MS = Number(process.env.THECLAW_PROMPT_TIMEOUT_MS ?? "45000");
const GATEWAY_TIMEOUT_MS = Number(process.env.THECLAW_PROMPT_GATEWAY_TIMEOUT_MS ?? "120000");
const SETUP_TOKEN_RAW = process.env.THECLAW_LIVE_SETUP_TOKEN?.trim() ?? "";
const SETUP_TOKEN_VALUE = process.env.THECLAW_LIVE_SETUP_TOKEN_VALUE?.trim() ?? "";
const SETUP_TOKEN_PROFILE = process.env.THECLAW_LIVE_SETUP_TOKEN_PROFILE?.trim() ?? "";
const DIRECT_CLAUDE_ARGS = ["-p", "--append-system-prompt"];

if (!PROMPT_TEXT && !PROMPT_LIST_JSON) {
  throw new Error("missing THECLAW_PROMPT_TEXT or THECLAW_PROMPT_LIST_JSON");
}
...
function resolveSetupTokenSource(): TokenSource {
  ...
  if (!match) {
    throw new Error(
      "no Anthropics setup-token profile found; set THECLAW_LIVE_SETUP_TOKEN_VALUE or THECLAW_LIVE_SETUP_TOKEN_PROFILE",
    );
  }
  return { profileId: match.id, token: validateSetupToken(match.token) };
}
...
async function runDirectPrompt(prompt: string): Promise<PromptResult> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-direct-prompt-probe-"));
...
async function startGatewayProcess(params: {
  ...
  const child = spawn(
    NODE_BIN,
    ["theclaw.mjs", "gateway", "--port", String(params.port), "--bind", "loopback", "--force"],
    {
      cwd: process.cwd(),
      env: {
        ...process.env,
        THECLAW_CONFIG_PATH: params.configPath,
        THECLAW_STATE_DIR: params.stateDir,
        THECLAW_AGENT_DIR: params.agentDir,
        THECLAW_GATEWAY_TOKEN: params.gatewayToken,
        THECLAW_SKIP_CHANNELS: "1",
        THECLAW_SKIP_GMAIL_WATCHER: "1",
        THECLAW_SKIP_CANVAS_HOST: "1",
        THECLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
        THECLAW_DISABLE_BONJOUR: "1",
        THECLAW_SKIP_CRON: "1",
        THECLAW_TEST_MINIMAL_GATEWAY: "1",
        THECLAW_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir,
        ANTHROPIC_API_KEY: "",
        ANTHROPIC_API_KEY_OLD: "",
      },
      stdio: ["ignore", "pipe", "pipe"],
...
async function runGatewayPrompt(prompt: string): Promise<PromptResult> {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "theclaw-gateway-prompt-probe-"));
  const stateDir = path.join(tmpDir, "state");
  const agentDir = path.join(stateDir, "agents", "main", "agent");
  const bundledPluginsDir = path.join(tmpDir, "bundled-plugins-empty");
  const configPath = path.join(tmpDir, "theclaw.json");
