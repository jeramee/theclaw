...
import type { TheClawConfig } from "../src/config/config.js";
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.theclaw?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.theclaw?.channel?.order;
      const labelRaw = parsed.theclaw?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".theclaw-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "theclaw-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "theclaw-build",
    ...
    THECLAW_PLUGIN_MANIFEST_CACHE_MS: "0",
...
function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "theclaw", "browser", "--help"]);`,
  ...
  const result = spawnSync(
    ...
    ["--import", "tsx", "--input-type=module", "--eval", inlineModule],
    {
      ...
      env: {
        ...renderContext.env,
        THECLAW_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
      },
