#!/usr/bin/env node
// Builds the TheClaw package artifact used by Docker E2E.
...
async function newestTheClawTarball(outputDir, packOutput) {
  let fromOutput = "";
  ...
  const packed = entries
    ...
    .at(-1);
  if (!packed) {
    throw new Error(`missing packed TheClaw tarball in ${outputDir}`);
  }
...
async function main() {
  ...
  if (!options.skipBuild) {
    console.error("==> Building TheClaw package artifacts");
    await run("pnpm", ["build"], sourceDir);
    console.error("==> Building TheClaw Control UI artifacts");
    await run("pnpm", ["ui:build"], sourceDir);
  }

  console.error("==> Writing TheClaw package inventory");
  await run(
    "node",
    [
      "--import",
  ...
  console.error("==> Packing TheClaw package");
  const packOutput = await runCapture(
    "npm",
    ["pack", "--silent", "--ignore-scripts", "--pack-destination", outputDir],
    sourceDir,
  );
  let tarball = await newestTheClawTarball(outputDir, packOutput);
  ...
  console.error("==> Checking TheClaw package tarball");
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-theclaw-package-tarball.mjs"), tarball],
    sourceDir,
  );
