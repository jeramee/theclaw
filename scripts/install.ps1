# TheClaw Installer for Windows (PowerShell)
# Usage: iwr -useb https://theclaw.ai/install.ps1 | iex
# Or: & ([scriptblock]::Create((iwr -useb https://theclaw.ai/install.ps1))) -NoOnboard

param(
    [ValidateSet("npm", "git")]
    [string]$InstallMethod = "npm",
    [string]$Tag = "latest",
    [string]$GitDir = "$env:USERPROFILE\theclaw",
    [switch]$NoOnboard,
    [switch]$NoGitUpdate,
    [switch]$DryRun
)
...
function Write-Banner {
    Write-Host ""
    Write-Host "${ACCENT}  🦞 TheClaw Installer$NC" -Level info
    Write-Host "${MUTED}  All your chats, one TheClaw.$NC" -Level info
    Write-Host ""
}
...
function Install-TheClawNpm {
    param([string]$Target = "latest")

    $installSpec = Resolve-PackageInstallSpec -Target $Target
    
    Write-Host "Installing TheClaw ($installSpec)..." -Level info
    ...
    try {
        ...
        Write-Host "TheClaw installed" -Level success
        return $true
    } catch {
...
function Install-TheClawGit {
    param([string]$RepoDir, [switch]$Update)
    
    Write-Host "Installing TheClaw from git..." -Level info
    
    if (!(Test-Path $RepoDir)) {
        Write-Host "  Cloning repository..." -Level info
        git clone https://github.com/theclaw/theclaw.git $RepoDir 2>&1
    } elseif ($Update) {
...
@echo off
node "$entryPath" %*
"@ | Out-File -FilePath "$wrapperDir\theclaw.cmd" -Encoding ASCII -Force
...
function Complete-Install {
    ...
    throw "TheClaw installation failed with exit code $($script:InstallExitCode)."
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install TheClaw from git to $GitDir" -Level info
        } else {
            try {
                npm uninstall -g theclaw 2>$null | Out-Null
            } catch { }
            if (!(Install-TheClawGit -RepoDir $GitDir -Update:(-not $NoGitUpdate))) {
                return (Fail-Install)
            }
        }
    } else {
        ...
        if ($DryRun) {
            Write-Host "[DRY RUN] Would install TheClaw via npm ($((Resolve-PackageInstallSpec -Target $Tag)))" -Level info
        } else {
            $gitWrapper = "$env:USERPROFILE\.local\bin\theclaw.cmd"
            if (Test-Path $gitWrapper) {
                Remove-Item -Force $gitWrapper
                Write-Host "Removed git wrapper (switching to npm)" -Level info
            }
            if (!(Install-TheClawNpm -Target $Tag)) {
                return (Fail-Install)
            }
        }
    }
    ...
    if (!$NoOnboard -and !$DryRun) {
        Write-Host ""
        Write-Host "Run 'theclaw onboard' to complete setup" -Level info
    }
    
    Write-Host ""
    Write-Host "🦞 TheClaw installed successfully!" -Level success
    return $true
}
