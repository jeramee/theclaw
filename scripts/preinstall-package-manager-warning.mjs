...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[theclaw] warning: detected ${packageManager} for install lifecycle.`,
    "[theclaw] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[theclaw] prefer: corepack pnpm install",
  ].join("\n");
}
