...
const EXPECTED_REPOSITORY_URL = "https://github.com/theclaw/theclaw";
...
const skipPackValidationEnv = "THECLAW_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "theclaw") {
    errors.push(`package.json name must be "theclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.theclaw !== "theclaw.mjs") {
    errors.push(
      `package.json bin.theclaw must be "theclaw.mjs"; found "${pkg.bin?.theclaw ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`theclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `theclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
