...
function resolveInstalledBinaryPath(prefixDir: string): string {
  return process.platform === "win32"
    ? join(prefixDir, "theclaw.cmd")
    : join(prefixDir, "bin", "theclaw");
}
...
function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".theclaw", "theclaw.json");
  mkdirSync(join(homeDir, ".theclaw"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
        ...
        models: {
          providers: {
            openai: {
              apiKey: "sk-theclaw-release-check",
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-theclaw-release-check",
  });
  ...
  execFileSync(process.execPath, [join(packageRoot, "theclaw.mjs"), "plugins", "doctor"], {
    cwd: packageRoot,
    stdio: "inherit",
    env,
  });
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "theclaw-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "theclaw");
    ...
    execFileSync(
      process.execPath,
      [join(packageRoot, "theclaw.mjs"), "completion", "--write-state"],
      {
        cwd: packageRoot,
        stdio: "inherit",
        env: {
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.theclaw-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.theclaw-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from theclaw/plugin-sdk.
