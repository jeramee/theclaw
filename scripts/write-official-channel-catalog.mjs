...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.theclaw) ? packageJson.theclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.theclaw?.channel?.id) || left.name;
    const rightId = trimString(right.theclaw?.channel?.id) || right.name;
