...
function makePluginOwnedTempDir(pluginDir, label) {
  return makeOwnedTempDir(pluginDir, `.theclaw-runtime-deps-${label}-`);
}
...
function replaceDirAtomically(targetPath, sourcePath) {
  ...
  const backupPath = makeTempDir(
    targetParentDir,
    `.theclaw-runtime-deps-backup-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function writeJsonAtomically(targetPath, value) {
  ...
  const tempDir = makeOwnedTempDir(
    targetParentDir,
    `.theclaw-runtime-deps-stamp-${sanitizeTempPrefixSegment(path.basename(targetPath))}-`,
  );
...
function shouldStageRuntimeDeps(packageJson) {
  return packageJson.theclaw?.bundle?.stageRuntimeDependencies === true;
}
...
function createRuntimeInstallManifest(pluginId, pinnedGroups) {
  const manifest = {
    name: `theclaw-runtime-deps-${sanitizeTempPrefixSegment(pluginId)}`,
    private: true,
    version: "0.0.0",
  };
...
function resolveLegacyRuntimeDepsStampPath(pluginDir) {
  return path.join(pluginDir, ".theclaw-runtime-deps-stamp.json");
}
...
function removeStaleRuntimeDepsTempDirs(pluginDir) {
  ...
  for (const entry of fs.readdirSync(pluginDir, { withFileTypes: true })) {
    if (entry.name.startsWith(".theclaw-runtime-deps-")) {
