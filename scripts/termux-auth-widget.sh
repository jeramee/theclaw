...
# TheClaw Auth Widget for Termux
...
SERVER="${THECLAW_SERVER:-l36}"

# Check auth status
termux-toast "Checking TheClaw auth..."

STATUS=$(ssh "$SERVER" '$HOME/theclaw/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/theclaw/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|THECLAW_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/theclaw/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|THECLAW_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: TheClaw won't work until re-auth"
                ;;
        esac
        ;;
