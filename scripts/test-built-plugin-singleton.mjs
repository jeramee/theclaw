...
const { clearPluginCommands, getPluginCommandSpecs, loadTheClawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadTheClawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "theclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  JSON.stringify(
    {
      name: "@theclaw/build-smoke-plugin",
      type: "module",
      theclaw: {
        extensions: ["./index.js"],
      },
...
fs.writeFileSync(
  path.join(distPluginDir, "theclaw.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'theclaw/plugin-sdk';",
...
const registry = loadTheClawPlugins({
  ...
  env: {
    ...process.env,
    THECLAW_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
